...
_cb_post_render(Ecore_Evas *ee EFL_UNUSED)
{
   ...
   data = core_buffer_data_get(win.buffer);
   paint_pixels(data, 0, WIDTH, HEIGHT, core_loop_time_get() * 1000);
...
main(void)
{
   ...
   win.ee = core_evas_new(NULL, 0, 0, WIDTH, HEIGHT, NULL);
   win.e = core_evas_get(win.ee);
   ...
   win.buffer = core_buffer_new("shm", WIDTH, HEIGHT, 0, 0);
   data = core_buffer_data_get(win.buffer);
   paint_pixels(data, 0, WIDTH, HEIGHT, 0);
   evas_object_image_data_set(win.img, data);
   evas_object_image_data_update_add(win.img, 0, 0, WIDTH, HEIGHT);

   core_evas_show(win.ee);

   core_evas_callback_post_render_set(win.ee, _cb_post_render);

   core_main_loop_begin();

   core_buffer_free(win.buffer);
