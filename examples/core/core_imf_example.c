 ...
 *
 * @verbatim
 * gcc -o core_imf_example core_imf_example.c `pkg-config --cflags --libs ecore evas eina ecore-evas ecore-imf ecore-imf-evas`
 * @endverbatim
 */
...
_mouse_down_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info)
{
   ...
   if (en->imf_context)
     {
        Ecore_IMF_Event_Mouse_Down core_ev;
        core_imf_evas_event_mouse_down_wrap(ev, &core_ev);
        if (core_imf_context_filter_event(en->imf_context,
                                           CORE_IMF_EVENT_MOUSE_DOWN,
                                           (Ecore_IMF_Event *)&core_ev))
          return;

        // core_imf_context_reset should be called before calculating new cursor position
        core_imf_context_reset(en->imf_context);
     }
...
_mouse_up_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info)
{
   ...
   if (en->imf_context)
     {
        Ecore_IMF_Event_Mouse_Up core_ev;
        core_imf_evas_event_mouse_up_wrap(ev, &core_ev);
        if (core_imf_context_filter_event(en->imf_context,
                                           CORE_IMF_EVENT_MOUSE_UP,
                                           (Ecore_IMF_Event *)&core_ev))
          return;
     }
...
_entry_focus_in_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info EFL_UNUSED)
{
   ...
   if (en->imf_context)
     core_imf_context_focus_in(en->imf_context);
...
_entry_focus_out_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info EFL_UNUSED)
{
   ...
   if (en->imf_context)
     {
        // core_imf_context_reset should be called for flushing the preedit string in focus-out event handler
        core_imf_context_reset(en->imf_context);
        core_imf_context_focus_out(en->imf_context);
     }
}
   ...
   if (en->cursor && en->imf_context)
     {
        ...
        core_imf_context_cursor_position_set(en->imf_context, cursor_pos);
        core_imf_context_cursor_location_set(en->imf_context, x + cx, y + cy, cw, ch);
        core_imf_context_bidi_direction_set(en->imf_context, (Ecore_IMF_BiDi_Direction)dir);
     }
}
...
static Efl_Bool
_core_imf_retrieve_surrounding_cb(void *data, Ecore_IMF_Context *ctx EFL_UNUSED, char **text, int *cursor_pos)
{
...
static void
_core_imf_event_delete_surrounding_cb(void *data, Ecore_IMF_Context *ctx EFL_UNUSED, void *event_info)
{
...
static void
_core_imf_event_commit_cb(void *data, Ecore_IMF_Context *ctx EFL_UNUSED, void *event_info)
{
   Entry *en = data;
...
static void
_core_imf_event_preedit_changed_cb(void *data, Ecore_IMF_Context *ctx, void *event_info EFL_UNUSED)
{
   ...
   core_imf_context_preedit_string_with_attributes_get(imf_context, &preedit_string, &attrs, &cursor_pos);
        ...
        if (attrs)
          {
             EINA_LIST_FOREACH(attrs, l, attr)
               {
                  if (attr->preedit_type == CORE_IMF_PREEDIT_TYPE_SUB1) // style type
                    {
                       // apply appropriate style such as underline
                    }
                  else if (attr->preedit_type == CORE_IMF_PREEDIT_TYPE_SUB2 || attr->preedit_type == CORE_IMF_PREEDIT_TYPE_SUB3)
                    {
                       // apply appropriate style such as underline
                    }
               }
...
_key_down_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info)
{
   ...
   if (en->imf_context)
     {
        Ecore_IMF_Event_Key_Down core_ev;
        core_imf_evas_event_key_down_wrap(ev, &core_ev);
        if (core_imf_context_filter_event(en->imf_context,
                                           CORE_IMF_EVENT_KEY_DOWN,
                                           (Ecore_IMF_Event *)&core_ev))
          return;
     }
...
_key_up_cb(void *data, Evas *e EFL_UNUSED, Evas_Object *o EFL_UNUSED, void *event_info)
{
   ...
   if (en->imf_context)
     {
        Ecore_IMF_Event_Key_Up core_ev;

        core_imf_evas_event_key_up_wrap(ev, &core_ev);
        if (core_imf_context_filter_event(en->imf_context,
                                           CORE_IMF_EVENT_KEY_UP,
                                           (Ecore_IMF_Event *)&core_ev))
          return;
     }
}
...
create_input_field(Evas *evas, Entry *en, Evas_Coord x, Evas_Coord y, Evas_Coord w, Evas_Coord h)
{
   ...
   const char *default_id = core_imf_context_default_id_get();
   if (!default_id)
     {
        fprintf(stderr, "Can't create core_imf_context\n");
        return;
     }

   en->imf_context = core_imf_context_add(default_id);
   core_imf_context_client_canvas_set(en->imf_context, evas);
   ...
   core_imf_context_retrieve_surrounding_callback_set(en->imf_context, _core_imf_retrieve_surrounding_cb, en);

   // register commit event callback
   core_imf_context_event_callback_add(en->imf_context, CORE_IMF_CALLBACK_COMMIT, _core_imf_event_commit_cb, en);

   // register preedit changed event handler
   core_imf_context_event_callback_add(en->imf_context, CORE_IMF_CALLBACK_PREEDIT_CHANGED, _core_imf_event_preedit_changed_cb, en);

   // register surrounding delete event callback
   core_imf_context_event_callback_add(en->imf_context, CORE_IMF_CALLBACK_DELETE_SURROUNDING, _core_imf_event_delete_surrounding_cb, en);
}
...
delete_input_field(Entry *en)
{
   ...
   if (en->imf_context)
     {
        core_imf_context_del(en->imf_context);
        en->imf_context = NULL;
     }
}
...
main(void)
{
   ...
   if (!core_evas_init())
     {
        fprintf(stderr, "failed to call core_evas_init()\n");
        return EXIT_FAILURE;
     }
   ...
   if (!ee)
     {
        fprintf(stderr, "failed to call core_evas_new\n");
        return EXIT_FAILURE;
     }
   ...
   if (!evas)
     {
        fprintf(stderr, "failed to call core_evas_get\n");
        return EXIT_FAILURE;
     }
   ...
   core_main_loop_begin(); // begin mainloop
