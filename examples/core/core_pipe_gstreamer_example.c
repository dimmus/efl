//Compile with:
// gcc -o core_pipe_gstreamer_example core_pipe_gstreamer_example.c `pkg-config --libs --cflags ecore gstreamer-1.0`
// or
// gcc -o core_pipe_gstreamer_example core_pipe_gstreamer_example.c `pkg-config --libs --cflags ecore gstreamer-0.10`
...
static GstElement *_build_pipeline(gchar *filename, Core_Pipe *pipe);
        ...
        gpointer user_data)
{
   ...
   pipe = (Core_Pipe *)user_data;
   printf("handoff : %p\n", arg0);
   gst_buffer_ref(arg0);
   core_pipe_write(pipe, &arg0, sizeof(arg0));
}
...
static GstElement *
_build_pipeline(gchar *filename, Core_Pipe *pipe)
{
   GstElement *pipeline;
   GstElement *filesrc;
   GstElement *demuxer;
