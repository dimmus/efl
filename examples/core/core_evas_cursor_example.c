 ...
 *
 * @verbatim
 * gcc -o core_evas_cursor_example core_evas_cursor_example.c `pkg-config --libs --cflags evas ecore ecore-evas`
 * @endverbatim
 */
#ifdef HAVE_CONFIG_H
...
_resize_cb(Ecore_Evas *ee)
{
   ...
   evas_object_resize(core_evas_data_get(ee, "bg"), w, h);
}
...
_mouse_pos_print(void *data)
{
   ...
   devs = evas_device_list(core_evas_get(data), NULL);
   ...
   EINA_LIST_FOREACH(devs, l, pointer)
     {
        ...
        if (efl_input_device_type_get(pointer) != EFL_INPUT_DEVICE_TYPE_MOUSE)
          continue;
        core_evas_pointer_device_xy_get(data, pointer, &x, &y);
...
_cursor_set(Ecore_Evas *ee, Efl_Input_Device *pointer)
{
   ...
   obj = evas_object_rectangle_add(core_evas_get(ee));
   evas_object_color_set(obj, rand() % 256, rand() % 256, rand() % 256, 255);
   evas_object_resize(obj, 30, 30);
   evas_object_show(obj);
   core_evas_object_cursor_device_set(ee, pointer, obj, 0, 10, 10);
}
...
main(int argc EFL_UNUSED, char *argv[] EFL_UNUSED)
{
   ...
   driver = core_evas_engine_name_get(ee);
   printf("Using driver %s\n", driver);
   if ((!strcmp(driver, "fb")  || !strcmp(driver, "software_x11")) &&
       !core_evas_vnc_start(ee, "localhost", -1, NULL, NULL, NULL))
     {
   ...
   t = core_timer_add(TIMEOUT, _mouse_pos_print, ee);
   efl_event_callback_add(e, EFL_CANVAS_SCENE_EVENT_DEVICE_ADDED,
                          _device_added, ee);
   core_evas_callback_resize_set(ee, _resize_cb);
   core_evas_callback_delete_request_set(ee, _delete_request_cb);
