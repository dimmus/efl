// Compile with:
// gcc -o core_audio_custom core_audio_custom.c `pkg-config --libs --cflags ecore ecore-audio`
...
int read_cb(void *data EFL_UNUSED, Eo *eo_obj, void *buf, int len)
{
  ...
  volume = core_audio_obj_volume_get(eo_obj);
...
main(int argc, const char *argv[])
{
   ...
   core_audio_obj_in_samplerate_set(in, 44100);
   core_audio_obj_in_channels_set(in, 1);
   core_audio_obj_volume_set(in, 0.7);
   core_audio_obj_vio_set(in, &vio, NULL, NULL);
