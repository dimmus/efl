...
{
   // read input from parent thread/loop status changed - read what we can
   Eo *obj = ev->object;
   char buf[4096];
        ...
        if (!err)
          {
             ...
             Eina_Error err1 = efl_io_writer_write(obj, &slice, NULL);
             if (!err1)
               {
...
{
   // the "main function" of the thread that gets called with arguments
   // just like efl_main for the main loop
   Eo *obj = ev->object;
   Eina_Accessor *args_access = efl_core_command_line_command_access(obj);
   void *s = "", *ss = "";
