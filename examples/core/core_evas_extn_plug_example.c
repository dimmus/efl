 ...
 *
 * For checking with ecore evas extn socket, please run with core_evas_extn_socket_example.
 *
 * @verbatim
 * gcc -o core_evas_extn_plug_example core_evas_extn_plug_example.c `pkg-config --libs --cflags evas ecore ecore-evas`
 * @endverbatim
 */
...
_on_delete(Ecore_Evas *ee)
{
   ...
   color = core_evas_data_get(ee, "color");
   if (color) free(color);
   core_main_loop_quit();
}
...
_button_1_up(void *data, Evas *e EFL_UNUSED, Evas_Object *obj EFL_UNUSED, void *event_info EFL_UNUSED)
{
   ...
   if (!color)
     {
        ...
        b = 0;
        a = 255;
	    core_evas_data_set(ee, "color", color);
     }
   else
     {
        r = ((color->r) + 100) % 255;
   ...
   core_evas_msg_parent_send(ee, MSG_DOMAIN_CONTROL_OBJECT, MSG_ID_BG_COLOR, color, sizeof(Msg_Color));
}

static void
_core_evas_msg_handle(Ecore_Evas *ee, int msg_domain, int msg_id, void *data, int size)
{
   if (!data) return;
        ...
        if (msg_id == MSG_ID_TEXT)
          {
             ...
             text = core_evas_data_get(ee, "text");
...
main(void)
{
   ...
   core_evas_title_set(ee, "Ecore Evas Extn Plug Example");
   core_evas_show(ee);
   ...
   plug = core_evas_extn_plug_new(ee);
   core_evas_data_set(ee, "plug", plug);
   if (!plug)
     {
   ...
   ee_plug = core_evas_object_core_evas_get(plug);
   core_evas_data_set(ee_plug, "text", text);

   if(!core_evas_extn_plug_connect(plug, "socket_exam_service", 0, EFL_FALSE))
     {
   ...
   core_evas_callback_msg_handle_set(ee_plug, _core_evas_msg_handle);
