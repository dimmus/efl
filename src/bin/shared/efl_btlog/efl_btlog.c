...
typedef Efl_Bool (*Translate_Func)(const char        *prog,
                                   const char        *bin_dir,
                                   const char        *bin_name,
...
static Efl_Bool       color         = EFL_TRUE;
static Efl_Bool       show_comments = EFL_TRUE;
static Efl_Bool       show_compact  = EFL_FALSE;
           ...
           int               *file_line)
{
  ...
  snprintf(buf,
           sizeof(buf),
           "%s -f -e %s/%s -C -a 0x%llx",
           prog,
           bin_dir,
           bin_name,
           addr);
  p = popen(buf, "r");
  if (!p) return EFL_FALSE;
      ...
      int               *file_line)
{
  ...
  *file_line = -1;
  ...
  if ((*s == '-') ||
      (*s == '+')) /* objc methods... will contain an extra space */
    func_space_count = 2;
  else func_space_count = 1;

  do
  {
    if (*s == ' ') spaces++;

    if ((spaces == func_space_count) && (func_done == EFL_FALSE))
    {
      *s         = '\0';
...
{
  static char
    local[PATH_MAX + sizeof(" 0x1234567890123456789 0x1234567890123456789\n")];
...
bt_append(Eina_List *btl, const char *btline)
{
  ...
  if (translation) btline = translation;
...
_translation_function_detect(const Translation_Desc *desc)
{
  ...
  return (_translate == NULL) ? EFL_FALSE : EFL_TRUE;
}
  ...
  for (i = 1; i < argc; i++)
  {
    if (!strcmp(argv[i], "-h"))
    {
      printf("Usage: efl_btlog [-n]\n"
             ...
             "Example:\n\n"
             "\tcat log.txt | efl_btlog\n"
             "\n");
      eina_shutdown();
      return 0;
    }
    else if (!strcmp(argv[i], "-n")) color = EFL_FALSE;
    else if (!strcmp(argv[i], "-C")) show_comments = EFL_FALSE;
    else if (!strcmp(argv[i], "-c")) show_compact = EFL_TRUE;
  }
...
do_show:
  EINA_LIST_FOREACH(btl, l, bt)
  {
    if (bt->comment && show_comments) fputs(bt->comment, stdout);
    if (!bt->bin_dir) continue;
