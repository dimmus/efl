...
property_free(DBus_Property *property)
{
    property->iface->properties =
        eina_inlist_remove(property->iface->properties,
...
property_new(DBus_Interface *iface)
{
    ...
    iface->properties =
        eina_inlist_append(iface->properties, EINA_INLIST_GET(prop));
    prop->iface = iface;
    return prop;
}
...
method_new(DBus_Interface *iface)
{
    ...
    iface->methods =
        eina_inlist_append(iface->methods, EINA_INLIST_GET(method));
    method->iface = iface;
    return method;
}
...
method_free(DBus_Method *method)
{
    ...
    method->iface->methods =
        eina_inlist_remove(method->iface->methods, EINA_INLIST_GET(method));
    free(method->c_name);
    free(method->cb_name);
...
signal_free(DBus_Signal *sig)
{
    ...
    EINA_INLIST_FOREACH_SAFE(sig->args, inlist, arg)
        arg_free(arg);
    sig->iface->signals =
        eina_inlist_remove(sig->iface->signals, EINA_INLIST_GET(sig));
    free(sig->c_name);
...
interface_free(DBus_Interface *iface)
{
    ...
    {
        DBus_Signal *sig =
            EINA_INLIST_CONTAINER_GET(iface->signals, DBus_Signal);
        signal_free(sig);
    }
    while (iface->methods)
    {
        DBus_Method *method =
            EINA_INLIST_CONTAINER_GET(iface->methods, DBus_Method);
        method_free(method);
    }
    while (iface->properties)
    {
        DBus_Property *property =
            EINA_INLIST_CONTAINER_GET(iface->properties, DBus_Property);
        property_free(property);
    }
    iface->obj->ifaces =
        eina_inlist_remove(iface->obj->ifaces, EINA_INLIST_GET(iface));
    free(iface->c_name);
    free(iface->name);
    free(iface);
}
...
object_free(DBus_Object *obj)
{
    while (obj->ifaces)
    {
        DBus_Interface *iface =
            EINA_INLIST_CONTAINER_GET(obj->ifaces, DBus_Interface);
