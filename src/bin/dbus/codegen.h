...
typedef struct _DBus_Method
{
   ...
   unsigned int arg_without_name;
   DBus_Interface *iface;
   Efl_Bool in_complex:1;
   Efl_Bool out_complex:1;
   Efl_Bool no_reply:1;
} DBus_Method;
...
Efl_Bool parser(void *data, Eina_Simple_XML_Type type, const char *content, unsigned offset, unsigned length);
...
//utils
Efl_Bool file_read(const char *file_name, char **buffer);
Efl_Bool file_write(const char *file_name, const char *buffer);
Efl_Bool util_h_write(void);
