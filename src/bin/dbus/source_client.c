...
static const char *
dbus_type2c_type2(const char *dbus_type, Efl_Bool with_const)
{
    switch (dbus_type[0])
    {
        case 's'://string
        ...
        case 'g'://signature
            {
                if (with_const) return "const char *";
                else return "char *";
            }
...
{
    return dbus_type2c_type2(dbus_type, EFL_TRUE);
}

static void
source_client_complex_method_call_generate(const DBus_Method *method,
                                           Efl_Strbuf        *c_code,
                                           Efl_Strbuf        *h)
{
    DBus_Arg   *arg;
    Efl_Strbuf *full_signature = eina_strbuf_new();

    EINA_INLIST_FOREACH(method->args, arg)
    {
        if (arg->direction == 'o') continue;
    ...
    if (method->no_reply)
    {
        eina_strbuf_append_printf(
            h,
            "void %s_call(Eldbus_Proxy *proxy, Eina_Value *args);\n",
        ...
        eina_strbuf_append_printf(
            c_code,
            "   Eldbus_Message *msg = eldbus_proxy_method_call_new(proxy, "
            "\"%s\");\n",
            method->name);
        eina_strbuf_append_printf(
            c_code,
            "   if (!eldbus_message_from_eina_value(\"%s\", msg, args))\n",
            eina_strbuf_string_get(full_signature));
        eina_strbuf_append_printf(c_code, "     {\n");
        eina_strbuf_append_printf(
            c_code,
            "        ERR(\"Error: Filling message from eina value.\");\n");
        eina_strbuf_append_printf(c_code,
        ...
        eina_strbuf_append_printf(
            c_code,
            "   eldbus_proxy_send(proxy, msg, %s, NULL, -1);\n",
            method->cb_name);
        eina_strbuf_append_printf(c_code, "}\n");
        goto end;
    }
    ...
    eina_strbuf_append_printf(h,
                              "Eldbus_Pending *%s_call",
                              prefix_append(method->c_name));
    eina_strbuf_append_printf(
        h,
        "(Eldbus_Proxy *proxy, %s cb, const void *data, Eina_Value *args);\n",
    ...
    eina_strbuf_append_printf(
        c_code,
        "   msg = eldbus_proxy_method_call_new(proxy, \"%s\");\n",
        method->name);
    eina_strbuf_append_printf(
        c_code,
        "   if (!eldbus_message_from_eina_value(\"%s\", msg, args))\n",
    ...
    eina_strbuf_append_printf(
        c_code,
        "   p = eldbus_proxy_send(proxy, msg, %s, cb, -1);\n",
        method->cb_name);
...
static void
source_client_simple_method_call_no_reply_generate(const DBus_Method *method,
                                                   Efl_Strbuf        *c_code,
                                                   Efl_Strbuf        *h)
{
    DBus_Arg   *arg;
    Efl_Strbuf *full_signature = eina_strbuf_new();
    Efl_Strbuf *args_call      = eina_strbuf_new();

    eina_strbuf_append_printf(h,
                              "void %s_call(Eldbus_Proxy *proxy",
                              prefix_append(method->c_name));
    eina_strbuf_append_printf(c_code,
                              "\nvoid\n%s_call(Eldbus_Proxy *proxy",
                              prefix_append(method->c_name));
    ...
    eina_strbuf_append_printf(
        c_code,
        "   if (!eldbus_message_arguments_append(msg, \"%s\"%s))\n",
        eina_strbuf_string_get(full_signature),
        eina_strbuf_string_get(args_call));
    eina_strbuf_append_printf(c_code, "     {\n");
    eina_strbuf_append_printf(c_code,
...
static void
source_client_simple_method_call_generate(const DBus_Method *method,
                                          Efl_Strbuf        *c_code,
                                          Efl_Strbuf        *h)
{
    DBus_Arg   *arg;
    ...
    eina_strbuf_append_printf(h,
                              ...
                              prefix_append(method->c_name));
    eina_strbuf_append_printf(h,
                              "(Eldbus_Proxy *proxy, %s cb, const void *data",
                              prefix_append(method->function_cb));
    eina_strbuf_append_printf(c_code,
                              "\nEldbus_Pending *\n%s_call",
                              prefix_append(method->c_name));
...
static void
source_client_complex_method_callback_generate(const DBus_Method *method,
                                               Efl_Strbuf        *c_code,
                                               Efl_Strbuf        *h)
{
    eina_strbuf_append_printf(
        h,
        "typedef void (*%s)(Eldbus_Proxy *proxy, void *data, Eldbus_Pending "
    ...
    eina_strbuf_append_printf(
        c_code,
        "   void *user_data = eldbus_pending_data_del(pending, "
        "\"__user_data\");\n");
    eina_strbuf_append_printf(c_code,
                              "   %s cb = data;\n",
                              prefix_append(method->function_cb));
    ...
    eina_strbuf_append_printf(
        c_code,
        "   Eldbus_Proxy *proxy = eldbus_pending_data_del(pending, "
        "\"__user_proxy\");\n");
    eina_strbuf_append_printf(
        c_code,
...
static void
source_client_simple_method_callback_generate(const DBus_Method *method,
                                              Efl_Strbuf        *c_code,
                                              Efl_Strbuf        *h)
{
    Efl_Strbuf *full_signature = eina_strbuf_new();
    DBus_Arg   *arg;
    Efl_Strbuf *end_cb        = eina_strbuf_new();
    Efl_Strbuf *arguments_get = eina_strbuf_new();

    eina_strbuf_append_printf(
        h,
        "typedef void (*%s)(Eldbus_Proxy *proxy, void *data, Eldbus_Pending "
        "*pending, Eldbus_Error_Info *error",
        prefix_append(method->function_cb));
    ...
    eina_strbuf_append_printf(
        c_code,
        "        cb(proxy, user_data, pending, &error_info%s);\n",
    ...
    eina_strbuf_append_printf(
        ...
        eina_strbuf_string_get(full_signature),
        eina_strbuf_string_get(arguments_get));
    eina_strbuf_append_printf(c_code, "     {\n");
    eina_strbuf_append_printf(
        c_code,
    ...
    eina_strbuf_append_printf(c_code, "     }\n");
    eina_strbuf_append_printf(c_code,
                              "   cb(proxy, user_data, pending, NULL%s);\n",
                              eina_strbuf_string_get(end_cb));
    eina_strbuf_append_printf(c_code, "   return;\n");
    eina_strbuf_append_printf(c_code, "}\n");
...
static void
source_client_method_generate(const DBus_Method *method,
                              Efl_Strbuf        *c_code,
                              Efl_Strbuf        *h)
{
    if (!method->no_reply)
    {
        ...
        else source_client_simple_method_callback_generate(method, c_code, h);
    }
    ...
    {
        if (method->no_reply)
            source_client_simple_method_call_no_reply_generate(method,
                                                               c_code,
                                                               h);
        else source_client_simple_method_call_generate(method, c_code, h);
    }
}

static void
source_client_signal_generate(const DBus_Signal *sig,
                              Efl_Strbuf        *c_code,
                              Efl_Strbuf        *h,
                              Efl_Strbuf        *c_init_function,
                              Efl_Strbuf        *c_header)
{
    DBus_Arg   *arg;
    Efl_Strbuf *full_signature = eina_strbuf_new();
    Efl_Strbuf *parameters     = eina_strbuf_new();
    Efl_Strbuf *string_copy    = eina_strbuf_new();
    Efl_Strbuf *string_free    = eina_strbuf_new();

    eina_strbuf_append_printf(
        c_init_function,
        "   eldbus_proxy_signal_handler_add(proxy, \"%s\", %s, proxy);\n",
        sig->name,
        sig->cb_name);
    ...
    EINA_INLIST_FOREACH(sig->args, arg)
    {
        ...
        eina_strbuf_append_printf(h,
                                  "   %s%s;\n",
                                  dbus_type2c_type2(arg->type, EFL_FALSE),
                                  arg->c_name);

        if (!strcmp(arg->type, "s") || !strcmp(arg->type, "o"))
        {
            eina_strbuf_append_printf(string_copy,
                                      "   s_data->%s = strdup(s_data->%s);\n",
                                      arg->c_name,
                                      arg->c_name);
            eina_strbuf_append_printf(string_free,
                                      "   free(s_data->%s);\n",
                                      arg->c_name);
        }
    }
...
jump_simple_stuff:
   ...
   //free function
    eina_strbuf_append_printf(
        c_code,
        "\nstatic void\n%s(void *user_data EFL_UNUSED, void *func_data)\n{\n",
        sig->free_function);
    eina_strbuf_append_printf(c_code,
                              "   %s *s_data = func_data;\n",
                              sig->struct_name);
    ...
    else eina_strbuf_append(c_code, eina_strbuf_string_get(string_free));
   ...
   //cb function
    ...
    if (sig->complex)
    {
        eina_strbuf_append_printf(
            c_code,
            "   s_data->value = eldbus_message_to_eina_value(msg);\n");
        goto end_signal;
    }
    eina_strbuf_append_printf(
        c_code,
        "   if (!eldbus_message_arguments_get(msg, \"%s\"%s))\n",
        eina_strbuf_string_get(full_signature),
        eina_strbuf_string_get(parameters));
    eina_strbuf_append_printf(c_code, "     {\n");
    eina_strbuf_append_printf(
        c_code,
        "        ERR(\"Error: Getting arguments from message.\");\n");
...
end_signal:
    eina_strbuf_append_printf(c_code,
                              "   ecore_event_add(%s, s_data, %s, NULL);\n",
                              sig->signal_event,
                              sig->free_function);
    eina_strbuf_append_printf(c_code, "}\n");
...
{
    if (prop->complex) return "Eldbus_Codegen_Property_Complex_Get_Cb";
    switch (prop->type[0])
    {
        case 's':
        case 'o':
...
static void
source_client_property_generate_get(const DBus_Property *prop,
                                    Efl_Strbuf          *c_code,
                                    Efl_Strbuf          *h)
{
   //callback
    ...
    eina_strbuf_append_printf(
        c_code,
        "        cb(user_data, pending, \"%s\", proxy, &error_info, %s);\n",
        prop->name,
        null_or_zero(prop->type));
    eina_strbuf_append_printf(c_code, "        return;\n");
    eina_strbuf_append_printf(c_code, "     }\n");
    ...
    if (prop->complex)
    {
        ...
        eina_strbuf_append_printf(
            c_code,
            "   cb(user_data, pending, \"%s\", proxy, NULL, &stack_value);\n",
            prop->name);
        eina_strbuf_append_printf(c_code,
    ...
    else
    {
        ...
        eina_strbuf_append_printf(
            c_code,
            "   cb(user_data, pending, \"%s\", proxy, NULL, v);\n",
            prop->name);
    }
    eina_strbuf_append_printf(c_code, "}\n");
   ...
   //call
    eina_strbuf_append_printf(h,
                              "Eldbus_Pending *%s_propget(Eldbus_Proxy *proxy, "
                              "%s cb, const void *data);\n",
                              prefix_append(prop->c_name),
                              prop_cb_get(prop));

    eina_strbuf_append_printf(c_code,
                              "\nEldbus_Pending *\n%s_propget(Eldbus_Proxy "
                              "*proxy, %s cb, const void *data)\n{\n",
                              prefix_append(prop->c_name),
                              prop_cb_get(prop));
    eina_strbuf_append_printf(c_code, "   Eldbus_Pending *p;\n");
    eina_strbuf_append_printf(
    ...
    eina_strbuf_append_printf(
        c_code,
        "   p = eldbus_proxy_property_get(proxy, \"%s\", %s, cb);\n",
        prop->name,
        prop->cb_name);
    eina_strbuf_append_printf(c_code, "   if (data)\n");
    eina_strbuf_append_printf(
...
static void
source_client_property_generate_set(const DBus_Property *prop,
                                    Efl_Strbuf          *c_code,
                                    Efl_Strbuf          *h)
{
   //callback
    eina_strbuf_append_printf(
        c_code,
        "\nstatic void\n%s_set(void *data, const Eldbus_Message *msg, "
        "Eldbus_Pending *pending)\n{\n",
        prop->cb_name);
    eina_strbuf_append_printf(c_code, "   const char *error, *error_msg;\n");
    eina_strbuf_append_printf(
    ...
    eina_strbuf_append_printf(
        c_code,
        "        cb(user_data, \"%s\", proxy, pending, &error_info);\n",
        prop->name);
    eina_strbuf_append_printf(c_code, "        return;\n");
    eina_strbuf_append_printf(c_code, "     }\n");
    eina_strbuf_append_printf(
        c_code,
        "   cb(user_data, \"%s\", proxy, pending, NULL);\n",
        prop->name);
    eina_strbuf_append_printf(c_code, "}\n");

    //call
    eina_strbuf_append_printf(h,
                              "Eldbus_Pending *%s_propset(Eldbus_Proxy *proxy, "
                              "Eldbus_Codegen_Property_Set_Cb cb, const void "
                              "*data, const void *value);\n",
                              prefix_append(prop->c_name));

    eina_strbuf_append_printf(c_code,
                              "\nEldbus_Pending *\n%s_propset(Eldbus_Proxy "
                              "*proxy, Eldbus_Codegen_Property_Set_Cb cb, "
    ...
    eina_strbuf_append_printf(c_code,
                              "   p = eldbus_proxy_property_set(proxy, \"%s\", "
                              "\"%s\", value, %s_set, cb);\n",
                              prop->name,
                              prop->type,
                              prop->cb_name);
...
static void
source_client_property_generate(const DBus_Property *prop,
                                Efl_Strbuf          *c_code,
                                Efl_Strbuf          *h)
{
...
void
source_client_generate(DBus_Object *path,
                       const char  *prefix,
                       ...
                       const char  *output_name)
{
    ...
    {
        Efl_Strbuf    *h, *c_init_function, *c_header, *c_code;
        DBus_Method   *method;
        DBus_Signal   *sig;
        DBus_Property *prop;
        char          *file_name, *aux;
        ...
        if (interface_name && strcmp(interface_name, iface->name)) continue;
        found           = EFL_TRUE;
        ...
        eina_strbuf_append_printf(h, "#include <Efl_Shared.h>\n");
        eina_strbuf_append_printf(h, "#include <Efl_Core.h>\n");
        eina_strbuf_append_printf(h, "#include <Efl_Dbus.h>\n");
        eina_strbuf_append_printf(h, "#include \"eldbus_utils.h\"\n\n");
        eina_strbuf_append_printf(
            h,
            "Eldbus_Proxy *%s_proxy_get(Eldbus_Connection *conn, const char "
            "*bus, const char *path);\n",
            prefix_append(iface->c_name));
        eina_strbuf_append_printf(h,
                                  "void %s_proxy_unref(Eldbus_Proxy *proxy);\n",
                                  prefix_append(iface->c_name));
        eina_strbuf_append_printf(h,
                                  "void %s_log_domain_set(int id);\n",
                                  prefix_append(iface->c_name));

        if (interface_name && output_name)
            eina_strbuf_append_printf(c_header,
                                      "#include \"%s.h\"\n\n",
                                      output_name);
        else
            eina_strbuf_append_printf(c_header,
        ...
        eina_strbuf_append_printf(
            c_init_function,
            "\nEldbus_Proxy *\n%s_proxy_get(Eldbus_Connection *conn, const "
            "char *bus, const char *path)\n{\n",
            prefix_append(iface->c_name));
        ...
        EINA_INLIST_FOREACH(iface->signals, sig)
            source_client_signal_generate(sig,
                                          c_code,
                                          h,
                                          c_init_function,
                                          c_header);
        ...
        else file_name = string_build("eldbus_%s.h", iface->c_name);
        ...
        else file_name = string_build("eldbus_%s.c", iface->c_name);
