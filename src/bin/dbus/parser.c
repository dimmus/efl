...
static Efl_Bool attributes_parse(const char                  *content,
                                 unsigned                     length,
...
obj_attributes_parser(void *data, const char *key, const char *value)
{
  ...
  if (!strcmp(key, NAME_ATTR)) obj->name = strdup(value);

  return EFL_TRUE;
}

static Efl_Bool
iface_attributes_parser(void *data  EFL_UNUSED,
                        const char *key,
                        const char *value)
{
  if (!strcmp(key, NAME_ATTR)) iface->name = strdup(value);

  return EFL_TRUE;
}

static Efl_Bool
signal_attributes_parser(void *data  EFL_UNUSED,
                         const char *key,
                         const char *value)
{
  if (!strcmp(key, NAME_ATTR)) d_signal->name = strdup(value);

  return EFL_TRUE;
}

static Efl_Bool
arg_attributes_parser(void *data EFL_UNUSED, const char *key, const char *value)
{
  DBus_Arg *arg = data;
  if (!strcmp(key, NAME_ATTR)) arg->name = strdup(value);
  else if (!strcmp(key, TYPE_ATTR)) arg->type = strdup(value);
  else if (!strcmp(key, DIRECTION_ATTR)) arg->direction = value[0];

  return EFL_TRUE;
}

static Efl_Bool
method_attributes_parser(void *data  EFL_UNUSED,
                         const char *key,
                         const char *value)
{
  if (!strcmp(key, NAME_ATTR)) method->name = strdup(value);

  return EFL_TRUE;
}

static Efl_Bool
property_attributes_parser(void *data  EFL_UNUSED,
                           const char *key,
                           const char *value)
{
  if (!strcmp(key, NAME_ATTR)) property->name = strdup(value);
  else if (!strcmp(key, TYPE_ATTR))
  {
    property->type = strdup(value);
    if (value[1] || value[0] == 'v') property->complex = EFL_TRUE;
  }
  else if (!strcmp(key, ACCESS_ATTR))
  {
    if (!strcmp(value, ACCESS_ATTR_VALUE_READ)) property->access = ACCESS_READ;
    else if (!strcmp(value, ACCESS_ATTR_VALUE_WRITE))
      property->access = ACCESS_WRITE;
    else property->access = (ACCESS_WRITE | ACCESS_READ);
  }
...
static Efl_Bool
open_object(const char   *content,
            unsigned      length,
            Efl_Bool      is_open_empty,
            DBus_Object **ptr_obj)
{
  Efl_Bool     r;
  DBus_Object *obj = *ptr_obj;

  if (is_open_empty) return EFL_TRUE;
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(obj, EFL_FALSE);
...
static Efl_Bool
open_interface(const char  *content,
               unsigned     length,
               Efl_Bool     is_open_empty,
               DBus_Object *obj)
{
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(iface, EFL_FALSE);
  ...
  if (is_open_empty) interface_close();
...
static Efl_Bool
open_signal(const char *content, unsigned length, Efl_Bool is_open_empty)
{
  Efl_Bool    r;
  char       *tmp;
  int         i;
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(d_signal, EFL_FALSE);
  ...
  d_signal->struct_name =
    string_build("%s_%s_Data", iface->c_name, d_signal->name);
...
annotation_attributes_parser(void *data, const char *key, const char *value)
{
  ...
  {
    if (!strcmp(value, ANNOTATION_NO_REPLY)) annotation->type = NO_REPLY;
  }
...
open_annotation(const char *content, unsigned length)
{
  ...
  r               = attributes_parse(content,
                       length,
                       annotation_attributes_parser,
                       &annotation);

  if (annotation.type == NO_REPLY)
  {
    Efl_Bool value = EFL_FALSE;
    if (!strcmp(annotation.value, "true")) value = EFL_TRUE;
    free(annotation.value);

    if (method) method->no_reply = value;
  }
...
open_arg(const char *content, unsigned length)
{
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(arg, EFL_FALSE);
  ...
  else arg->c_name = dbus_name_to_c(arg->name);
...
method_close(void)
{
  ...
  EINA_INLIST_FOREACH(method->args, arg)
  {
    if ((arg->type[1]) || (arg->type[0] == 'v'))
    {
      if (arg->direction == 'o') method->out_complex = EFL_TRUE;
      else method->in_complex = EFL_TRUE;
    }
  }
  if (method->no_reply)
  {
...
static Efl_Bool
open_method(const char *content, unsigned lenght, Efl_Bool is_open_empty)
{
  Efl_Bool r;
  char    *tmp;
  int      i;
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(method, EFL_FALSE);
...
open_property(const char *content, unsigned length)
{
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(property, EFL_FALSE);
...
{
  if (!strncmp(content, INTERFACE_TAG, INTERFACE_TAG_LENGTH)) interface_close();
  if (!strncmp(content, SIGNAL_TAG, SIGNAL_TAG_LENGTH)) signal_close();
  else if (!strncmp(content, METHOD_TAG, METHOD_TAG_LENGTH)) method_close();
       ...
       unsigned             length)
{
  ...
  switch (type)
  {
    ...
    case EINA_SIMPLE_XML_OPEN_EMPTY:
      {
        r = open_tag(content, length, type == EINA_SIMPLE_XML_OPEN_EMPTY, obj);
        break;
      }
...
static Efl_Bool
attributes_parse(const char                  *content,
                 unsigned                     length,
                 Eina_Simple_XML_Attribute_Cb func,
                 const void                  *data)
{
