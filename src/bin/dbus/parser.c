...
static Efl_Bool attributes_parse(const char *content, unsigned length, Eina_Simple_XML_Attribute_Cb func, const void *data);
...
static Efl_Bool
iface_attributes_parser(void *data EFL_UNUSED, const char *key, const char *value)
{
   if (!strcmp(key, NAME_ATTR))
...
static Efl_Bool
signal_attributes_parser(void *data EFL_UNUSED, const char *key, const char *value)
{
   if (!strcmp(key, NAME_ATTR))
...
static Efl_Bool
method_attributes_parser(void *data EFL_UNUSED, const char *key, const char *value)
{
   if (!strcmp(key, NAME_ATTR))
...
static Efl_Bool
property_attributes_parser(void *data EFL_UNUSED, const char *key, const char *value)
{
   if (!strcmp(key, NAME_ATTR))
...
static Efl_Bool
open_object(const char *content, unsigned length, Efl_Bool is_open_empty, DBus_Object **ptr_obj)
{
   Efl_Bool r;
...
static Efl_Bool
open_interface(const char *content, unsigned length, Efl_Bool is_open_empty, DBus_Object *obj)
{
   Efl_Bool r;
   char *tmp_name;
   ...
   EINA_INLIST_FOREACH(method->args, arg)
     {
        if ((arg->type[1]) || (arg->type[0] == 'v'))
          {
             if (arg->direction == 'o')
               method->out_complex = EFL_TRUE;
             else
               method->in_complex = EFL_TRUE;
          }
     }
   if (method->no_reply)
     {
...
static Efl_Bool
open_tag(const char *content, unsigned length, Efl_Bool is_open_empty, DBus_Object **obj)
{
   unsigned int i;
...
Efl_Bool
parser(void *data, Eina_Simple_XML_Type type, const char *content, unsigned offset EFL_UNUSED, unsigned length)
{
   Efl_Bool r = EFL_TRUE;
   DBus_Object **obj = data;
