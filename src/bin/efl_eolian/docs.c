...
_append_section(const Eolian_State *state, const char *desc, int ind, int curl,
                Efl_Strbuf *buf, Efl_Strbuf *wbuf)
{
...
static int
_append_extra(const char *el, int indent, int curl, Efl_Bool nl, Efl_Strbuf *buf)
{
   if (el)
     {
        eina_strbuf_append_char(buf, '\n');
...
_gen_doc_brief(const Eolian_State *state, const char *summary, const char *since,
               const char *group, const char *el, int indent, Efl_Strbuf *buf)
{
   int curl = 4 + indent;
