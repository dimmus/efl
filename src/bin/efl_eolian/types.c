               ...
               Efl_Bool full)
{
   ...
   char *p = strrchr(grp, '.');
   if (p) *p = '\0';
   Efl_Strbuf *buf = eo_gen_docs_full_gen(state, eolian_typedecl_documentation_get(tp),
                                           grp, 0);
   free(grp);
                 ...
                 if (fdoc)
                   {
                      ...
                      if (nl)
                        {
                           Efl_Strbuf *fbuf = eo_gen_docs_full_gen(state, fdoc, NULL,
                              strlen(nl));
                           if (fbuf)
...
_const_generate(const Eolian_State *state, const Eolian_Constant *vr)
{
   ...
   char *p = strrchr(fn, '.');
   if (p) *p = '\0';
   Efl_Strbuf *buf = eo_gen_docs_full_gen(state, eolian_constant_documentation_get(vr),
                                           fn, 0);
   if (p)
     {
        *p = '_';
...
_err_generate(const Eolian_State *state, const Eolian_Error *err)
{
   ...
   char *p = strrchr(fn, '.');
   if (p) *p = '\0';
   Efl_Strbuf *buf = eo_gen_docs_full_gen(state, eolian_error_documentation_get(err),
                                           fn, 0);
   if (p)
     {
        *p = '_';
...
void eo_gen_types_header_gen(const Eolian_State *state,
                             Eina_Iterator *itr, Efl_Strbuf *buf,
                             Efl_Bool full)
{
