...
static const char *_dexts[6] = { ".h", ".stub.h", ".c", ".c", ".d", ".d" };
...
{
    fprintf(outf, "Eolian C generator version: " EFL_VERSION "\n");
}
...
_try_set_out(char t, char **outs, const char *val, int *what)
{
    ...
    if (pos < 0) return EFL_FALSE;
    if (outs[pos]) free(outs[pos]);
    outs[pos] = strdup(val);
    return EFL_TRUE;
}

static void
_fill_all_outs(char **outs, const char *val, char *base)
{
    ...
    size_t blen = strlen(basen), elen = strlen(ext);
...
static Efl_Strbuf *
_include_guard(const char *fname, const char *gname, Efl_Strbuf *buf)
{
    if (!buf) return NULL;
...
_get_filename(const char *path)
{
    ...
    if (!ret1 && !ret2) return path;
    if (ret1 && ret2)
    {
        if (ret1 > ret2) return ret1 + 1;
        else return ret2 + 1;
    }
    if (ret1) return ret1 + 1;
    return ret2 + 1;
}

static Efl_Bool
_write_file(const char *fname, const Efl_Strbuf *buf)
{
...
static Efl_Bool
_read_file(const char *fname, Efl_Strbuf **buf)
{
...
char *
eo_gen_c_full_name_get(const char *nm)
{
    if (!nm) return NULL;
    char *buf = strdup(nm);
    if (!buf) abort();
...
void
eo_gen_class_names_get(const Eolian_Class *cl,
                       char              **cname,
                       char              **cnameu,
                       char              **cnamel)
{
...
static Efl_Bool
_write_header(const Eolian_State *eos,
              ...
              const char         *ifname)
{
    ...
    eo_gen_types_header_gen(state,
                            eolian_state_objects_by_file_get(eos, ifname),
                            buf,
                            EFL_TRUE);
    buf = _include_guard(ifname, "TYPES", buf);

    Efl_Strbuf *cltd = eo_gen_class_typedef_gen(eos, ifname);
    if (cltd)
    {
...
static Efl_Bool
_write_stub_header(const Eolian_State *eos,
                   const Eolian_State *state,
                   const char         *ofname,
                   const char         *ifname)
{
    ...
    Efl_Bool ret = _write_file(ofname, buf);
    eina_strbuf_free(buf);
    return ret;
}

static Efl_Bool
_write_source(const Eolian_State *eos,
              const char         *ofname,
              const char         *ifname,
              Efl_Bool            eot)
{
    ...
    if (cl || eot)
    {
        ...
        if (!_write_file(ofname, buf)) goto done;
        ret = EFL_TRUE;
    }
...
_write_impl(const Eolian_State *eos, const char *ofname, const char *ifname)
{
    ...
    const Eolian_Class *cl = eolian_state_class_by_file_get(eos, ifname);
    if (!cl) return EFL_FALSE;

    Efl_Strbuf *buf;
    if (!_read_file(ofname, &buf)) return EFL_FALSE;
...
static void
_append_dep_line(Efl_Strbuf *buf,
                 Efl_Strbuf *dbuf,
                 char      **outs,
                 int         gen_what,
                 int         what)
{
    if (!(gen_what & what)) return;
    eina_strbuf_append(buf, outs[_get_bit_pos(what)]);
    eina_strbuf_append_buffer(buf, dbuf);
}

static Efl_Bool
_write_deps(const Eolian_State *eos,
            const char         *ofname,
            const char         *ifname,
            char              **outs,
            int                 gen_what)
{
    ...
    Efl_Strbuf *buf  = eina_strbuf_new();
    Efl_Strbuf *dbuf = eina_strbuf_new();
...
main(int argc, char **argv)
{
    ...
    char *outs[sizeof(_dexts) / sizeof(void *)] = { NULL, NULL, NULL,
                                                    NULL, NULL, NULL };
    char *basen                                 = NULL;
    ...
    if (!gen_what) gen_what = GEN_H | GEN_C;
    ...
    if (succ && (gen_what & GEN_H_STUB))
        succ =
            _write_stub_header(eos, eos, outs[_get_bit_pos(GEN_H_STUB)], eobn);
    if (succ && (gen_what & GEN_C))
        succ = _write_source(eos,
    ...
    if (succ && (gen_what & GEN_D_FULL))
        succ = _write_deps(eos,
                           outs[_get_bit_pos(GEN_D_FULL)],
                           eobn,
                           outs,
                           gen_what);
    else if (succ && (gen_what & GEN_D))
        succ =
            _write_deps(eos, outs[_get_bit_pos(GEN_D)], eobn, outs, gen_what);
