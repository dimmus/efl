...
_read_file(const char *fname, Efl_Strbuf **buf)
{
  ...
  if (!cont)
  {
    fprintf(stderr, "efl_eo_gen: could not allocate memory for '%s'\n", fname);
    fclose(f);
    return EFL_FALSE;
  }
...
main(int argc, char **argv)
{
  ...
  if (succ && (gen_what & GEN_C))
    succ =
      _write_source(eos, outs[_get_bit_pos(GEN_C)], eobn, !strcmp(ext, ".eot"));
  if (succ && (gen_what & GEN_C_IMPL))
    succ = _write_impl(eos, outs[_get_bit_pos(GEN_C_IMPL)], eobn);

  if (succ && (gen_what & GEN_D_FULL))
    succ =
      _write_deps(eos, outs[_get_bit_pos(GEN_D_FULL)], eobn, outs, gen_what);
