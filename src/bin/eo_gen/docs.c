...
static int
_indent_line(Efl_Strbuf *buf, int ind)
{
  int i;
  for (i = 0; i < ind; ++i)
...
#define DOC_LIMIT(ind) \
    ((ind > DOC_LINE_TEST) ? (ind + DOC_LINE_OVER) : DOC_LINE_LIMIT)

#define SUMMARY_OR_DEFAULT(sum) (sum ? sum : "No description supplied.")

static void
_generate_ref(const Eolian_State *state, const char *refn, Efl_Strbuf *wbuf)
{
  ...
  if (tp)
  {
    const Eolian_Enum_Type_Field *efl =
      eolian_typedecl_enum_field_get(tp, sfx + 1);
    if (!efl)
    {
      eina_stringshare_del(bname);
      goto noref;
  ...
  if (!cl)
  {
    ...
    if (cl)
    {
      ...
      if (ftype == EOLIAN_UNRESOLVED) ftype = eolian_function_type_get(fn);
      free(meth);
    }
  }
  else
...
static int
_append_section(const Eolian_State *state,
                const char         *desc,
                int                 ind,
                int                 curl,
                Efl_Strbuf         *buf,
...
#undef CHECK_NOTE
    ...
    {
      desc++;
      if ((*desc != '@') && (*desc != '$')) eina_strbuf_append_char(wbuf, '\\');
      eina_strbuf_append_char(wbuf, *desc++);
    }
    else if (*desc == '@')
    {
      ...
      else eina_strbuf_append_char(wbuf, '@');
    }
    else if (*desc == '$')
    {
      if (*++desc == '[')
      ...
      if (isalpha(*desc)) eina_strbuf_append(wbuf, "@c ");
      else eina_strbuf_append_char(wbuf, '$');
    }
...
split:
    ...
    if ((int)(curl + wlen) > limit)
    {
      ...
      curl += _indent_line(buf, ind);
      eina_strbuf_append(buf, " * ");
      if (*eina_strbuf_string_get(wbuf) == ' ') eina_strbuf_remove(wbuf, 0, 1);
    }
    curl += eina_strbuf_length_get(wbuf);
    eina_strbuf_append(buf, eina_strbuf_string_get(wbuf));
    eina_strbuf_reset(wbuf);
...
static int
_append_since(const char *since, int indent, int curl, Efl_Strbuf *buf)
{
  if (since)
  {
    eina_strbuf_append_char(buf, '\n');
...
static void
_append_group(Efl_Strbuf *buf, char *sgrp, int indent)
{
  if (!sgrp) return;
...
static void
_gen_doc_brief(const Eolian_State *state,
               const char         *summary,
               const char         *since,
               const char         *group,
               const char         *el,
...
{
  int         curl = 4 + indent;
  Efl_Strbuf *wbuf = eina_strbuf_new();
  if (indent) eina_strbuf_append(buf, "/**< ");
  else eina_strbuf_append(buf, "/** ");
  curl = _append_section(state, summary, indent, curl, buf, wbuf);
  eina_strbuf_free(wbuf);
  curl       = _append_extra(el, indent, curl, EFL_FALSE, buf);
...
static void
_gen_doc_full(const Eolian_State *state,
              const char         *summary,
              ...
              Efl_Strbuf         *buf)
{
  ...
  curl = _append_extra(el, indent, curl, EFL_TRUE, buf);
  curl = _append_since(since, indent, curl, buf);
  eina_strbuf_append_char(buf, '\n');
  _indent_line(buf, indent);
  char *sgrp = _sanitize_group(group);
...
static Efl_Strbuf *
_gen_doc_buf(const Eolian_State         *state,
             ...
             int                         indent)
{
  ...
  if (!desc) _gen_doc_brief(state, sum, since, group, el, indent, buf);
  else _gen_doc_full(state, sum, desc, since, group, el, indent, buf);
  return buf;
}

Efl_Strbuf *
eo_gen_docs_full_gen(const Eolian_State         *state,
                     const Eolian_Documentation *doc,
                     const char                 *group,
                     int                         indent)
{
...
Efl_Strbuf *
eo_gen_docs_event_gen(const Eolian_State *state,
                      ...
                      const char         *group)
{
  ...
  if (!doc)
  {
    Efl_Strbuf *bufs = eina_strbuf_new();
...
Efl_Strbuf *
eo_gen_docs_func_gen(const Eolian_State    *state,
                     ...
                     int                    indent)
{
  ...
  Efl_Bool       force_out = EFL_FALSE;

  Efl_Strbuf *buf  = eina_strbuf_new();
  Efl_Strbuf *wbuf = NULL;
  ...
  eina_strbuf_append(buf, " * @brief ");
  curl += sizeof(" * @brief ") - 1;
  _append_section(state, SUMMARY_OR_DEFAULT(sum), indent, curl, buf, wbuf);
  ...
  while (par)
  {
    ...
    if (!force_out) dir = eolian_parameter_direction_get(par);
    ...
    if (adoc)
    {
      ...
      _append_section(state, SUMMARY_OR_DEFAULT(asum), indent, curl, buf, wbuf);
    }
  ...
  if (rdoc)
  {
    ...
    _append_section(state, SUMMARY_OR_DEFAULT(rsum), indent, curl, buf, wbuf);
    eina_strbuf_append_char(buf, '\n');
    if (since)
    {
      _indent_line(buf, indent);
