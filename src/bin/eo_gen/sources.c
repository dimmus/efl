...
{
  if (ftype == EOLIAN_PROP_GET) return "*";
...
static Efl_Bool
_function_exists(const char *fname, Efl_Strbuf *buf)
{
...
static Efl_Bool
_type_exists(const char *tname, Efl_Strbuf *buf)
{
  const char *ptr  = eina_strbuf_string_get(buf);
                        ...
                        Efl_Bool is_empty               EFL_UNUSED)
{
  ...
  eina_strbuf_append_printf(params,
                            "%s, %s, %s",
                            eina_strbuf_string_get(dataname),
                            name,
                            eina_strbuf_string_get(freename));

  eina_strbuf_append_printf(params_full_imp,
                            ", void *%s, %s %s, Eina_Free_Cb %s",
                            eina_strbuf_string_get(dataname),
                            c_type,
                            name,
                            eina_strbuf_string_get(freename));
  eina_strbuf_append_printf(params_full,
                            ", void *%s, %s %s, Eina_Free_Cb %s",
                            eina_strbuf_string_get(dataname),
                            c_type,
                            name,
               ...
               const char              *ctp)
{
  ...
  if (tdcl && (eolian_typedecl_type_get(tdcl) == EOLIAN_TYPEDECL_STRUCT))
  {
    ...
    else eina_strbuf_append_printf(buf, "((%s){0})", sn);
    free(sn);
    return;
  }
...
_free_func_get(const Eolian_Type *type)
{
  ...
  switch (eolian_type_builtin_type_get(ab))
  {
    ...
    case EOLIAN_TYPE_BUILTIN_INVALID:
      if (eolian_type_type_get(ab) == EOLIAN_TYPE_CLASS) return "efl_del";
      else return eolian_typedecl_free_func_get(eolian_type_typedecl_get(ab));
      /* no free func */
    default:
      return NULL;
  }
...
static void
_generate_normal_free(Efl_Strbuf       **buf,
                      const Eolian_Type *type,
                      const Efl_Strbuf  *parameter,
                      const char        *additional_intention)
{
...
static void
_generate_loop_content(Efl_Strbuf       **buf,
                       const Eolian_Type *inner_type,
                       const Efl_Strbuf  *iter_param)
{
  eina_strbuf_append(*buf, "     {\n");
...
static void
_generate_iterative_free(Efl_Strbuf               **buf,
                         const Eolian_Type         *type,
                         const Eolian_Type         *inner_type,
                         Eolian_Function_Parameter *parameter,
                         Efl_Strbuf                *param)
{
  Efl_Strbuf *iterator_header, *iter_param;
  ...
  eina_strbuf_append_printf(iter_param,
                            "%s_iter",
...
static int
_gen_function_param_fallback(Eina_Iterator *itr,
                             Efl_Strbuf    *fallback_free_ownership,
                             Efl_Strbuf    *param_call)
{
...
_get_reflect_initf(const Eolian_Type *abtp)
{
  ...
  switch (btp)
  {
    ...
    case EOLIAN_TYPE_BUILTIN_STRINGSHARE:
      initf = eolian_type_name_get(abtp);
      break;
    default:
      break;
  }
...
static void
_gen_reflect_get(Efl_Strbuf                      *buf,
                 const char                      *cnamel,
                 ...
                 Eina_Hash                       *refh)
{
  ...
  if (eolian_type_is_ptr(valt)) return;
  ...
  if (et == EOLIAN_PROP_SET) eina_hash_set(refh, &fid, (void *)EOLIAN_PROPERTY);
  else eina_hash_set(refh, &fid, (void *)EOLIAN_PROP_GET);

  eina_strbuf_append(buf, "\nstatic Eina_Value\n");
  eina_strbuf_append_printf(buf,
                            "__eolian_%s_%s_get_reflect(const Eo *obj)\n",
                            cnamel,
                            eolian_function_name_get(fid));
  eina_strbuf_append(buf, "{\n");

  Eina_Stringshare *ct     = eolian_parameter_c_type_get(pr, EFL_TRUE);
...
static void
_gen_reflect_set(Efl_Strbuf                      *buf,
                 const char                      *cnamel,
                 const Eolian_Function_Parameter *pr,
                 const Eolian_Function           *fid,
                 Eina_Hash                       *refh)
{
  ...
  Eolian_Function_Type et = (Eolian_Function_Type)eina_hash_find(refh, &fid);
  if (et == EOLIAN_PROP_GET) eina_hash_set(refh, &fid, (void *)EOLIAN_PROPERTY);
  else eina_hash_set(refh, &fid, (void *)EOLIAN_PROP_SET);

  eina_strbuf_append(buf, "\nstatic Eina_Error\n");
  eina_strbuf_append_printf(
    buf,
    "__eolian_%s_%s_set_reflect(Eo *obj, Eina_Value val)\n",
    cnamel,
    eolian_function_name_get(fid));
  eina_strbuf_append(buf, "{\n");
  eina_strbuf_append(buf, "   Eina_Error r = 0;");

  Eina_Stringshare *ct     = eolian_parameter_c_type_get(pr, EFL_FALSE);
  const char       *starsp = (ct[strlen(ct) - 1] != '*') ? " " : "";
  eina_strbuf_append_printf(buf, "   %s%scval;\n", ct, starsp);
  eina_stringshare_del(ct);

  eina_strbuf_append_printf(buf,
                            "   if (!eina_value_%s_convert(&val, &cval))\n",
                            initf);
  eina_strbuf_append(buf, "      {\n");
...
static void
_emit_class_function(Efl_Strbuf                *buf,
                     const Eolian_Function     *fid,
                     const Eolian_Function_Type ftype,
                     const Eolian_Type         *rtp,
                     const char                *rtpn,
                     Efl_Strbuf                *params_full,
                     const char                *ocnamel,
                     ...
                     const char                *function_name)
{
  ...
  eina_strbuf_append(buf, "(");
  if (eina_strbuf_length_get(params_full) == 0) eina_strbuf_append(buf, "void");
  else eina_strbuf_append_buffer(buf, params_full);
  eina_strbuf_append(buf, ")\n");
  eina_strbuf_append(buf, "{\n");
  eina_strbuf_append_printf(
    buf,
    "   const Efl_Class *klass = %s();\n",
...
static void
_gen_func(const Eolian_Class     *cl,
          const Eolian_Function  *fid,
          ...
          Eina_Hash              *refh)
{
  Efl_Bool is_empty = eolian_implement_is_empty(impl, ftype);
  Efl_Bool is_auto  = eolian_implement_is_auto(impl, ftype);

  if ((ftype != EOLIAN_PROP_GET) && (ftype != EOLIAN_PROP_SET))
    ftype = eolian_function_type_get(fid);

  Efl_Bool is_prop    = (ftype == EOLIAN_PROP_GET || ftype == EOLIAN_PROP_SET);
  Efl_Bool var_as_ret = EFL_FALSE;
  ...
  if (rtp)
  {
    is_auto = EFL_FALSE; /* can't do auto if func returns */
  ...
  if (ftype == EOLIAN_PROP_GET)
  {
    ...
    if (!rtp)
    {
      ...
      if (eina_iterator_next(itr, &d1) && !eina_iterator_next(itr, &d2))
      {
        ...
        if (!eolian_parameter_is_by_ref(pr)) reflect_param = pr;
        var_as_ret = EFL_TRUE;
        def_ret    = eolian_parameter_default_value_get(pr);
        rtpn       = eolian_parameter_c_type_get(pr, EFL_TRUE);
      }
      eina_iterator_free(itr);
    }
  }
  ...
  Efl_Strbuf *params = eina_strbuf_new(); /* par1, par2, par3, ... */
  Efl_Strbuf *params_full =
    eina_strbuf_new(); /* T par1, U par2, ... for decl */
  Efl_Strbuf *params_full_imp = eina_strbuf_new(); /* as above, for impl */
  Efl_Strbuf *params_init     = eina_strbuf_new(); /* default value inits */
  Efl_Strbuf *fallback_free_ownership =
    eina_strbuf_new(); /* list of function calls that are freeing the owned parameters, or doing nothing on the normal parameters, NULL if there is nothing owned*/
  ...
  /* property keys */
  {
    ...
    EINA_ITERATOR_FOREACH(itr, pr)
    {
      ...
      Eina_Stringshare *ptn = eolian_parameter_c_type_get(pr, EFL_FALSE);
  ...
  else
  {
    ...
    if (is_prop)
    {
      ...
      owners +=
        _gen_function_param_fallback(itr, fallback_free_ownership, param_call);
  ...
  {
    Eina_Iterator *itr;
    if (is_prop) itr = eolian_property_values_get(fid, ftype);
    else itr = eolian_function_parameters_get(fid);
    Eolian_Function_Parameter *pr;
    EINA_ITERATOR_FOREACH(itr, pr)
    {
      ...
      Efl_Bool    had_star = ptn[strlen(ptn) - 1] == '*';
      const char *add_star = _get_add_star(ftype, pd);

      if (ptd &&
          eolian_typedecl_type_get(ptd) == EOLIAN_TYPEDECL_FUNCTION_POINTER)
      {
        _gen_func_pointer_param(prn,
                                ptn,
                                ptd,
                                params,
                                params_full,
      ...
      if (!had_star) eina_strbuf_append_char(params_full_imp, ' ');
      ...
      if (!had_star) eina_strbuf_append_char(params_full, ' ');
  ...
  Efl_Bool impl_same_class = (eolian_implement_class_get(impl) == cl);
  Efl_Bool impl_need       = EFL_TRUE;
  if (impl_same_class && eolian_implement_is_pure_virtual(impl, ftype))
    impl_need = EFL_FALSE;

  if (!rtpn) rtpn = eina_stringshare_add("void");
  ...
  if (impl_need)
  {
    ...
    if (eolian_function_is_static(fid)) dt = eina_stringshare_add("void");
    else dt = eolian_class_c_data_type_get(cl);
    ...
    if (!is_empty && !is_auto)
    {
      ...
      eina_strbuf_append(buf, rtpn);
      if (rtpn[strlen(rtpn) - 1] != '*') eina_strbuf_append_char(buf, ' ');
      eina_strbuf_append_char(buf, '_');
      eina_strbuf_append(buf, cnamel);
      if (!impl_same_class) eina_strbuf_append_printf(buf, "_%s", ocnamel);
      eina_strbuf_append_char(buf, '_');
      eina_strbuf_append(buf, eolian_function_name_get(fid));
      eina_strbuf_append(buf, func_suffix);
      ...
      if (eina_strbuf_length_get(params_full_imp) == 0 &&
          eolian_function_is_static(fid))
    ...
    if (is_empty || is_auto || eina_strbuf_length_get(params_init))
    {
      ...
      else eina_hash_add(_funcs_params_init_get, &impl, impl);
      ...
      if ((ftype == EOLIAN_PROP_GET) || eolian_function_object_is_const(fid))
        eina_strbuf_append(buf, "const ");
      eina_strbuf_append(buf, "Eo *obj");
      if (is_empty || is_auto) eina_strbuf_append(buf, " EFL_UNUSED");
      eina_strbuf_append(buf, ", ");
      eina_strbuf_append(buf, dt);
      eina_strbuf_append(buf, " *pd");
      if (is_empty || (is_auto && !eina_strbuf_length_get(params_init)))
    ...
    {
      eina_strbuf_append(buf, "   ");
      if (rtp) eina_strbuf_append(buf, "return ");
      eina_strbuf_append_char(buf, '_');
      eina_strbuf_append(buf, cnamel);
      if (!impl_same_class) eina_strbuf_append_printf(buf, "_%s", ocnamel);
      eina_strbuf_append_char(buf, '_');
  ...
  if (impl_same_class && !eolian_function_is_static(fid))
  {
    ...
    if (reflect_param)
    {
      ...
      else _gen_reflect_set(buf, cnamel, reflect_param, fid, refh);
    }

    void          *data;
    Eina_Iterator *itr        = eolian_property_keys_get(fid, ftype);
    Efl_Bool       has_params = eina_iterator_next(itr, &data);
    eina_iterator_free(itr);
    ...
    if (fallback_free_ownership)
    {
      ...
      eina_strbuf_append_printf(buf,
                                "_%s_ownership_fallback(%s)\n{\n",
                                eolian_function_full_c_name_get(fid, ftype),
                                eina_strbuf_string_get(params_full) + 2);
    ...
    if (!strcmp(rtpn, "void")) eina_strbuf_append(buf, "VOID_");
    eina_strbuf_append(buf, "FUNC_BODY");
    if (has_params) eina_strbuf_append_char(buf, 'V');
    if ((ftype == EOLIAN_PROP_GET) || eolian_function_object_is_const(fid) ||
        eolian_function_is_static(fid))
    {
    ...
    if (fallback_free_ownership) eina_strbuf_append(buf, "_FALLBACK");
  ...
  if (fallback_free_ownership) eina_strbuf_free(fallback_free_ownership);
}

static void
_gen_opfunc(const Eolian_Function  *fid,
            Eolian_Function_Type    ftype,
            Efl_Strbuf             *buf,
            const Eolian_Implement *impl,
            Efl_Bool                pinit,
            ...
            const char             *ocnamel)
{
  ...
  {
    if (pinit) eina_strbuf_append(buf, "__eolian");
    eina_strbuf_append_printf(buf, "_%s_", cnamel);
    if (ocnamel) eina_strbuf_append_printf(buf, "%s_", ocnamel);
    eina_strbuf_append(buf, eolian_function_name_get(fid));
    if (ftype == EOLIAN_PROP_GET) eina_strbuf_append(buf, "_get");
    else if (ftype == EOLIAN_PROP_SET) eina_strbuf_append(buf, "_set");
    eina_strbuf_append(buf, "),\n");
  }
}

static void
_gen_reflop(const Eolian_Function *fid,
            Efl_Strbuf            *buf,
            const char            *cnamel,
            Eina_Hash             *refh)
{
  Eolian_Function_Type aftype =
    (Eolian_Function_Type)eina_hash_find(refh, &fid);
  if (aftype == EOLIAN_UNRESOLVED) return;
...
static void
_gen_initializer(const Eolian_Class *cl, Efl_Strbuf *buf, Eina_Hash *refh)
{
  ...
  Efl_Strbuf *ops   = eina_strbuf_new();
  Efl_Strbuf *refls = eina_strbuf_new();
  ...
  EINA_ITERATOR_FOREACH(itr, imp)
  {
    ...
    Efl_Bool found_get = !!eina_hash_find(_funcs_params_init_get, &imp);
    Efl_Bool found_set = !!eina_hash_find(_funcs_params_init_set, &imp);
    char    *ocnamel   = NULL;
    if (cl != icl) eo_gen_class_names_get(icl, NULL, NULL, &ocnamel);
  ...
  else
  {
    ...
    eina_strbuf_append_printf(buf,
                              "   EFL_OPS_DEFINE(ops, %s_EXTRA_OPS);\n",
                              cnameu);
    eina_strbuf_append(buf, "   opsp = &ops;\n");
    eina_strbuf_append(buf, "#endif\n\n");
  }
...
void
eo_gen_source_gen(const Eolian_Class *cl, Efl_Strbuf *buf)
{
  if (!cl) return;
  ...
  /* event section, they come first */
  {
    ...
    EINA_ITERATOR_FOREACH(itr, ev)
    {
      ...
      if (eolian_event_is_hot(ev)) eina_strbuf_append(buf, "_HOT");
      if (eolian_event_is_restart(ev)) eina_strbuf_append(buf, "_RESTART");
  ...
  if (!strcmp(dt, "void")) eina_strbuf_append(buf, "   0,\n");
  ...
  /* inherits in EFL_DEFINE_CLASS */
  {
    ...
    if (!icl) eina_strbuf_append(buf, ", NULL");
    else
    {
...
static void
_gen_params(const Eolian_Function *fid,
            Eolian_Function_Type   ftype,
            Efl_Bool               var_as_ret,
            Efl_Strbuf            *params,
            Efl_Strbuf            *params_full)
  ...
  if (!var_as_ret)
  {
    ...
    EINA_ITERATOR_FOREACH(itr, pr)
    {
          ...
          eolian_typedecl_type_get(ptd) == EOLIAN_TYPEDECL_FUNCTION_POINTER)
      {
        ...
        eina_strbuf_append_printf(
          params_full,
          ", void *%s_data, %s %s, Eina_Free_Cb %s_free_cb",
          prn,
          ptn,
          prn,
          prn);
...
static void
_gen_proto(const Eolian_Class     *cl,
           const Eolian_Function  *fid,
           Eolian_Function_Type    ftype,
           Efl_Strbuf             *buf,
           const Eolian_Implement *impl,
...
{
  Efl_Bool impl_same_class = (eolian_implement_class_get(impl) == cl);
  if (impl_same_class && eolian_implement_is_pure_virtual(impl, ftype)) return;
  ...
  else snprintf(iname, sizeof(iname), "%s", cnamel);
  ...
  if ((ftype == EOLIAN_PROP_GET) && !rtp)
  {
    ...
    if (eina_iterator_next(itr, &d1) && !eina_iterator_next(itr, &d2))
    {
      Eolian_Function_Parameter *pr = d1;
      rtp                           = eolian_parameter_type_get(pr);
      rtpn       = eolian_parameter_c_type_get(pr, EFL_FALSE);
      var_as_ret = EFL_TRUE;
    }
    eina_iterator_free(itr);
  }
  ...
  if (rtp)
  {
    if (!rtpn) rtpn = eolian_function_return_c_type_get(fid, ftype);
    eina_strbuf_append(buf, rtpn);
    eina_stringshare_del(rtpn);
  }
  else eina_strbuf_append(buf, "void");
  ...
  eina_strbuf_append(buf, "Eo *obj, ");
  if (strcmp(dtype, "void")) eina_strbuf_append_printf(buf, "%s *pd", dtype);
  else eina_strbuf_append(buf, "void *pd EFL_UNUSED");

  /* gen params here */
  Efl_Strbuf *params      = eina_strbuf_new();
  Efl_Strbuf *params_full = eina_strbuf_new();
...
void
eo_gen_impl_gen(const Eolian_Class *cl, Efl_Strbuf *buf)
{
  if (!cl) return;
  ...
  Efl_Strbuf *beg = eina_strbuf_new();
