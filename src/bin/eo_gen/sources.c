      ...
      /* class and user types */
        case EOLIAN_TYPE_BUILTIN_INVALID:
            if (eolian_type_type_get(ab) == EOLIAN_TYPE_CLASS) return "efl_del";
                ...
                return eolian_typedecl_free_func_get(
                    eolian_type_typedecl_get(ab));
      /* no free func */
        default:
            return NULL;
    }
                             ...
                             Efl_Strbuf    *param_call)
{
    ...
    EINA_ITERATOR_FOREACH(itr, pr)
    {
        ...
        {
            _generate_normal_free(&fallback_free_ownership,
                                  type,
                                  param_call,
                                  "");
        }
                 ...
                 Eina_Hash                       *refh)
{
    ...
    Eina_Stringshare *fcn =
        eolian_function_full_c_name_get(fid, EOLIAN_PROP_GET);
    eina_strbuf_append_printf(buf, "   %s%sval = %s(obj);\n", ct, starsp, fcn);
    eina_stringshare_del(fcn);
    eina_stringshare_del(ct);

    eina_strbuf_append_printf(buf,
                              "   return eina_value_%s_init(val);\n",
                              initf);
    eina_strbuf_append(buf, "}\n\n");
}
                 ...
                 Eina_Hash                       *refh)
{
    ...
    Eina_Stringshare *fcn =
        eolian_function_full_c_name_get(fid, EOLIAN_PROP_SET);
            ...
            else if ((ftype != EOLIAN_PROP_SET) && dfv)
            {
                ...
                if (val.type)
                {
                    Eina_Stringshare *vals =
                        eolian_expression_value_to_literal(&val);
                    eina_strbuf_append_printf(params_init,
                                              "   if (%s) *%s = %s;",
                                              prn,
                                              prn,
    ...
    */
    ...
    /* method section */
    {
        ...
        EINA_ITERATOR_FOREACH(itr, imp)
        {
            ...
            const Eolian_Function *fid =
                eolian_implement_function_get(imp, &ftype);
            switch (ftype)
            {
                case EOLIAN_PROP_GET:
                case EOLIAN_PROP_SET:
