...
static Efl_Strbuf *
_type_generate(const Eolian_State    *state,
               ...
               Efl_Bool               full)
{
  ...
  if (p) *p = '\0';
  Efl_Strbuf *buf =
    eo_gen_docs_full_gen(state, eolian_typedecl_documentation_get(tp), grp, 0);
  free(grp);
    ...
    case EOLIAN_TYPEDECL_STRUCT_OPAQUE:
      {
        ...
        EINA_ITERATOR_FOREACH(membs, memb)
        {
          ...
          if (fdoc)
          {
            ...
            {
              Efl_Strbuf *fbuf =
                eo_gen_docs_full_gen(state, fdoc, NULL, strlen(nl));
              if (fbuf)
    ...
    case EOLIAN_TYPEDECL_ENUM:
      {
        ...
        Efl_Bool       next  = eina_iterator_next(membs, (void **)&memb);
        while (next)
        {
          const Eolian_Expression *vale =
            eolian_typedecl_enum_field_value_get(memb, EFL_FALSE);
          Eina_Stringshare *membn =
            eolian_typedecl_enum_field_c_constant_get(memb);
          if (!vale) eina_strbuf_append_printf(buf, "  %s", membn);
          else
          {
            Eolian_Value val = eolian_expression_eval(vale, EOLIAN_MASK_INT);
          ...
          if (next) eina_strbuf_append(buf, ",");
          if (fdoc)
          {
            const char *nl = strrchr(eina_strbuf_string_get(buf), '\n');
            if (nl)
    ...
    case EOLIAN_TYPEDECL_FUNCTION_POINTER:
      {
        ...
        const Eolian_Type *rtp =
          eolian_function_return_type_get(fid, EOLIAN_FUNCTION_POINTER);
        if (!rtp) eina_strbuf_append(buf, "void ");
        else
        {
          Eina_Stringshare *ct =
        ...
        int nidx = 1;
        eo_gen_params(eolian_function_parameters_get(fid),
                      buf,
                      NULL,
                      &nidx,
...
_const_generate(const Eolian_State *state, const Eolian_Constant *vr)
{
  ...
  if (p) *p = '\0';
  Efl_Strbuf *buf =
    eo_gen_docs_full_gen(state, eolian_constant_documentation_get(vr), fn, 0);
  if (p)
  {
    *p = '_';
    while ((p = strchr(fn, '.')))
  ...
  if (exp && strcmp(lit, exp)) eina_strbuf_append_printf(buf, " /* %s */", exp);
  eina_stringshare_del(lit);
  eina_stringshare_del(exp);
  eina_strbuf_append(buf, "\n#endif");
...
_err_generate(const Eolian_State *state, const Eolian_Error *err)
{
  ...
  if (p) *p = '\0';
  Efl_Strbuf *buf =
    eo_gen_docs_full_gen(state, eolian_error_documentation_get(err), fn, 0);
  if (p)
  {
    *p = '_';
    while ((p = strchr(fn, '.')))
  ...
  eina_strbuf_prepend_printf(buf,
                             "%s %s_WEAK Eina_Error %s_get(void);\n\n",
                             _eolian_api_symbol,
                             _eolian_api_symbol,
                             fn);
...
void
eo_gen_types_header_gen(const Eolian_State *state,
                        Eina_Iterator      *itr,
                        Efl_Strbuf         *buf,
                        Efl_Bool            full)
{
  ...
  EINA_ITERATOR_FOREACH(itr, decl)
  {
    ...
    if (dt == EOLIAN_OBJECT_CONSTANT)
    {
      ...
      if (!vr || eolian_constant_is_extern(vr)) continue;

      Efl_Strbuf *vbuf = _const_generate(state, vr);
      if (vbuf)
      {
    ...
    else if (dt == EOLIAN_OBJECT_ERROR)
    {
      ...
      if (!err || eolian_error_is_extern(err)) continue;

      Efl_Strbuf *ebuf = _err_generate(state, err);
      if (ebuf)
      {
    ...
    if (dt != EOLIAN_OBJECT_TYPEDECL) continue;

    const Eolian_Typedecl *tp = (const Eolian_Typedecl *)decl;

    if (eolian_typedecl_is_extern(tp)) continue;

    Eolian_Typedecl_Type tpt = eolian_typedecl_type_get(tp);

    if (tpt == EOLIAN_TYPEDECL_ENUM && !full) continue;

    if (tpt == EOLIAN_TYPEDECL_ALIAS)
    {
      const Eolian_Type *btp = eolian_typedecl_base_type_get(tp);
      if (eolian_type_type_get(btp) == EOLIAN_TYPE_UNDEFINED) continue;
    }

    Efl_Strbuf *tbuf = _type_generate(state, tp, full);
    if (tbuf)
    {
...
static void
_source_gen_error(Efl_Strbuf *buf, const Eolian_Error *err)
{
  if (eolian_error_is_extern(err)) return;
  ...
  for (const char *p = msg; *p; ++p)
    switch (*p)
    {
      ...
      default:
        {
          if (isprint(*p)) eina_strbuf_append_char(buf, *p);
          else eina_strbuf_append_printf(buf, "\\x%2X", (unsigned int)*p);
          continue;
        }
    }
  eina_strbuf_append(buf, "\");\n");
...
void
eo_gen_types_source_gen(Eina_Iterator *itr, Efl_Strbuf *buf)
{
  const Eolian_Object *decl;
  EINA_ITERATOR_FOREACH(itr, decl)
  {
