           ...
           int                       *rpid)
{
  ...
  Eina_Stringshare      *prtn = eolian_parameter_c_type_get(pr, EFL_FALSE);
  ...
  if (prtn[strlen(prtn) - 1] != '*') eina_strbuf_append_char(buf, ' ');
...
void
eo_gen_params(Eina_Iterator       *itr,
              Efl_Strbuf          *buf,
              Efl_Strbuf         **flagbuf,
              int                 *nidx,
              Eolian_Function_Type ftype)
{
  ...
  {
    int rpid = 0;
    if (*nidx) eina_strbuf_append(buf, ", ");
...
static void
_gen_func(const Eolian_State    *state,
          ...
          char                  *cnameu)
{
  ...
  Efl_Bool           return_move = eolian_function_return_is_move(fid, ftype);
  ...
  if (ftype == EOLIAN_PROP_GET && !rtp)
  {
    ...
    if (eina_iterator_next(itr, &d1) && !eina_iterator_next(itr, &d2))
    {
      ...
      rtps                          = eolian_parameter_c_type_get(pr, EFL_TRUE);
      var_as_ret                    = EFL_TRUE;
    }
    eina_iterator_free(itr);
  }
  ...
  Efl_Bool hasdoc = !!eolian_implement_documentation_get(fimp, ftype);
  if (!hasdoc && ((ftype == EOLIAN_PROP_GET) || (ftype == EOLIAN_PROP_SET)))
    hasdoc = !!eolian_implement_documentation_get(fimp, EOLIAN_PROPERTY);
  if (hasdoc)
  {
    Efl_Strbuf *dbuf = eo_gen_docs_func_gen(state, fid, ftype, 0);
  ...
  if (rtp)
  {
    if (!rtps) rtps = eolian_function_return_c_type_get(fid, ftype);
    eina_strbuf_append(buf, rtps);
    if (rtps[strlen(rtps) - 1] != '*') eina_strbuf_append_char(buf, ' ');
  }
  ...
  eo_gen_params(eolian_property_keys_get(fid, ftype),
                buf,
                &flagbuf,
                &nidx,
                EOLIAN_PROPERTY);
  ...
  if (flagbuf) eina_strbuf_append_char(flagbuf, ')');
  ...
  {
    if (!flagbuf) flagbuf = eina_strbuf_new();
    eina_strbuf_prepend(flagbuf, " EFL_WARN_UNUSED_RESULT");
  }
  if (return_move) eina_strbuf_append(buf, " EFL_TRANSFER_OWNERSHIP");
  if (flagbuf)
  {
...
void
eo_gen_header_gen(const Eolian_State *state,
                  ...
                  Efl_Strbuf         *buf)
{
  ...
  {
    Efl_Strbuf *cdoc =
      eo_gen_docs_full_gen(state, doc, eolian_class_c_name_get(cl), 0);
    if (cdoc)
    {
  ...
  eina_strbuf_append_printf(
    buf,
    "%s %s_WEAK const Efl_Class *%s(void) EINA_CONST;\n",
    _eolian_api_symbol,
    _eolian_api_symbol,
    gname);
  ...
  {
    if (eolian_implement_class_get(imp) != cl) continue;
...
events:
  ...
  EINA_ITERATOR_FOREACH(itr, ev)
  {
    ...
    if (evs == EOLIAN_SCOPE_PRIVATE) continue;
    ...
    {
      if (!eolian_event_is_beta(ev)) eina_strbuf_append_char(buf, '\n');
      eina_strbuf_append_printf(buf, "#ifdef %s_PROTECTED\n", cnameu);
    }
