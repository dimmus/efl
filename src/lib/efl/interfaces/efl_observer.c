...
EOLIAN static void
_efl_observable_observer_add(Eo *obj              EFL_UNUSED,
...
EOLIAN static void
_efl_observable_observer_del(Eo *obj              EFL_UNUSED,
                             Efl_Observable_Data *pd,
                             const char          *key,
                             Efl_Observer        *obs)
{
...
EOLIAN static void
_efl_observable_observer_clean(Eo *obj              EFL_UNUSED,
                               Efl_Observable_Data *pd,
                               Efl_Observer        *obs)
{
    Eina_Iterator *it;
...
_efl_observable_observers_iterator_next(Eina_Iterator *it, void **data)
{
    ...
    if (!eina_iterator_next(et->classes, (void **)& or)) return EFL_FALSE;
    if (! or) return EFL_FALSE;
...
_efl_observable_observers_iterator_new(Eo *obj              EFL_UNUSED,
                                       Efl_Observable_Data *pd,
                                       const char          *key)
{
    Eina_Hash             *observers;
...
EOLIAN static void
_efl_observable_observers_update(Eo                     *obj,
                                 Efl_Observable_Data *pd EFL_UNUSED,
                                 const char             *key,
                                 void                   *data)
{
    Eina_Iterator *it;
...
_efl_observable_iterator_tuple_next(Eina_Iterator *it, void **data)
{
    ...
    if (!eina_iterator_next(et->classes, (void **)&key)) return EFL_FALSE;
    if (!key) return EFL_FALSE;

    tuple = calloc(1, sizeof(Efl_Observable_Tuple));
    if (!tuple) return EFL_FALSE;
...
_efl_observable_iterator_tuple_free(Eina_Iterator *it)
{
    ...
    {
        if (tuple->data) eina_iterator_free(tuple->data);
        free(tuple);
    }
    EINA_MAGIC_SET(&et->iterator, 0);
    free(et);
