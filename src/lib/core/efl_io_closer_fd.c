...
EOLIAN static void
_efl_io_closer_fd_closer_fd_set(Eo *o                  EFL_UNUSED,
...
EOLIAN static int
_efl_io_closer_fd_closer_fd_get(const Eo *o            EFL_UNUSED,
                                Efl_Io_Closer_Fd_Data *pd)
{
  return pd->fd;
}
...
_efl_io_closer_fd_efl_io_closer_close(Eo                       *o,
                                      Efl_Io_Closer_Fd_Data *pd EFL_UNUSED)
{
  int        fd;
  Eina_Error err = 0;
...
_efl_io_closer_fd_efl_io_closer_closed_get(const Eo                 *o,
                                           Efl_Io_Closer_Fd_Data *pd EFL_UNUSED)
{
...
#else
  int      fd;
  Efl_Bool old = pd->close_on_exec;

  pd->close_on_exec = close_on_exec;

  fd = efl_io_closer_fd_get(o);
  if (fd < 0)
    return EFL_TRUE; /* postpone until fd_set(), users
...
EOLIAN static Efl_Bool
_efl_io_closer_fd_efl_io_closer_close_on_exec_get(const Eo              *o,
                                                  Efl_Io_Closer_Fd_Data *pd)
{
#ifdef _WIN32
...
EOLIAN static Efl_Bool
_efl_io_closer_fd_efl_io_closer_close_on_invalidate_get(
