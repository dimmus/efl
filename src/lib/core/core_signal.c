  ...
  for (unsigned int i = 0; i < NUM_PIPES; i++)
  {
    while (1)
    {
      ...
      switch (sdata.sig)
      {
        ...
        case SIGUSR2:
          {
            Core_Event_Signal_User *e = _core_event_signal_user_new();
            if (e)
            {
        ...
        case SIGTERM:
          {
            Core_Event_Signal_Exit *e = _core_event_signal_exit_new();
            if (e)
            {
            ...
            Eo *loop = efl_provider_find(obj, EFL_LOOP_CLASS);
            if (loop) efl_event_callback_call(loop, EFL_LOOP_EVENT_QUIT, NULL);
          }
          break;
#ifdef SIGPWR
        case SIGPWR:
          {
            Core_Event_Signal_Power *e = _core_event_signal_power_new();
            if (e)
            {
              e->data = sdata.info;
  ...
  while ((pid = waitpid(-1, &status, WNOHANG)) > 0)
  {
    ...
    EINA_LIST_FOREACH_SAFE(sig_pid_info_list, l, ll, pi)
    {
      if (pi->pid == pid)
      {
        ...
        sig_pid_info_list = eina_list_remove_list(sig_pid_info_list, ll);
        pinfo.pid         = pid;
        pinfo.info        = info;
        if (WIFEXITED(status))
        {
