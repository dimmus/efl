...
_cb_read(void *data EFL_UNUSED, const Efl_Event *event)
{
   ...
   // XXX: ecore event needs to be eo loop api's
    ...
    _core_main_timechanges_stop(loop);
    _core_main_timechanges_start(loop);
}
...
void
_core_main_timechanges_start(Eo *obj)
{
#ifdef HAVE_SYS_TIMERFD_H
...
void
_core_main_timechanges_stop(Eo *obj EFL_UNUSED)
{
#ifdef HAVE_SYS_TIMERFD_H
    Eo *o = realtime_obj;
    if (!o) return;
