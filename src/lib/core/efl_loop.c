...
_efl_loop_begin(Eo *obj, Efl_Loop_Data *pd)
{
  ...
  if (pd->thread_children)
  {
    ...
    pd->quit_on_last_thread_child_del = EFL_TRUE;
    EINA_LIST_FOREACH_SAFE(pd->thread_children, l, ll, child)
    {
      Efl_Task_Flags task_flags = efl_task_flags_get(child);

      if (task_flags & EFL_TASK_FLAGS_EXIT_WITH_PARENT) efl_task_end(child);
...
efl_loop_exit_code_process(Eina_Value *value)
{
  ...
  if (t == EINA_VALUE_TYPE_UCHAR || t == EINA_VALUE_TYPE_USHORT ||
      t == EINA_VALUE_TYPE_UINT || t == EINA_VALUE_TYPE_ULONG ||
      t == EINA_VALUE_TYPE_UINT64 || t == EINA_VALUE_TYPE_CHAR ||
      t == EINA_VALUE_TYPE_SHORT || t == EINA_VALUE_TYPE_INT ||
      t == EINA_VALUE_TYPE_LONG || t == EINA_VALUE_TYPE_INT64 ||
      t == EINA_VALUE_TYPE_FLOAT || t == EINA_VALUE_TYPE_DOUBLE)
  {
    Eina_Value v = EINA_VALUE_EMPTY;
...
_efl_loop_efl_object_constructor(Eo *obj, Efl_Loop_Data *pd)
{
  ...
  pd->future_message_handler =
    efl_add(EFL_LOOP_MESSAGE_FUTURE_HANDLER_CLASS, obj);
  efl_provider_register(obj,
                        EFL_LOOP_MESSAGE_FUTURE_HANDLER_CLASS,
                        pd->future_message_handler);
...

{
  ...
  initialization      = EFL_FALSE;

  efl_event_callback_call(efl_main_loop_get(), EFL_LOOP_EVENT_ARGUMENTS, &arge);
  return v;
}

static void
_efl_loop_arguments_cleanup(Eo *o                          EFL_UNUSED,
                            void                          *data,
...
core_loop_arguments_send(int argc, const char **argv)
{
  ...
  efl_future_then(efl_main_loop_get(),
                  efl_loop_job(efl_main_loop_get()),
...
_efl_loop_timeout(Eo *obj, Efl_Loop_Data *pd EFL_UNUSED, double tim)
{
  ...
  d->timer = efl_add(
    ...
    efl_event_callback_add(efl_added,
                           EFL_LOOP_TIMER_EVENT_TIMER_TICK,
                           _efl_loop_timeout_done,
                           d),
    efl_event_callback_add(efl_added, EFL_EVENT_DEL, _efl_loop_timeout_del, d));
...
EFL_FUNC_BODYV(efl_loop_register,
               Efl_Bool,
               EFL_FALSE,
               EFL_FUNC_CALL(klass, provider),
               const Efl_Class  *klass,
               const Efl_Object *provider);

static Efl_Bool
_efl_loop_unregister(Eo               *obj,
                     Efl_Loop_Data *pd EFL_UNUSED,
                     const Efl_Class  *klass,
                     const Efl_Object *provider)
{
...
EFL_FUNC_BODYV(efl_loop_unregister,
               Efl_Bool,
               EFL_FALSE,
               EFL_FUNC_CALL(klass, provider),
               const Efl_Class  *klass,
...
void
_efl_loop_messages_filter(Eo *obj        EFL_UNUSED,
                          Efl_Loop_Data *pd,
                          void          *handler_pd)
{
  Message *msg;
...
void
_efl_loop_messages_call(Eo *obj        EFL_UNUSED,
                        Efl_Loop_Data *pd,
                        void          *func,
                        void          *data)
{
  ...
  EINA_INLIST_FOREACH(pd->message_queue, msg)
  {
    if ((msg->handler) && (msg->message) && (!msg->delete_me))
    {
      Efl_Bool (*fn)(void *data, void *handler, void *msg);
...
_efl_loop_message_process(Eo *obj, Efl_Loop_Data *pd)
{
  ...
  while (pd->message_queue)
  {
    ...
    else
    {
      ...
      pd->message_queue =
        eina_inlist_remove(pd->message_queue, pd->message_queue);
      free(msg);
    }
  }
  pd->message_walking--;
  if (pd->message_walking == 0)
  {
    ...
    while (pd->message_pending_queue)
    {
      ...
      pd->message_queue =
        eina_inlist_append(pd->message_queue, EINA_INLIST_GET(msg));
    }
  }
  return EFL_TRUE;
}

EOAPI EFL_FUNC_BODY(efl_loop_message_process, Efl_Bool, 0);

EWAPI void
efl_build_version_set(int         vmaj,
                      int         vmin,
                      int         vmic,
                      int         revision,
                      const char *flavor,
...
efl_loop_future_scheduler_get(const Eo *obj)
{
  ...
  if (loop) return efl_loop_future_scheduler_get(loop);
