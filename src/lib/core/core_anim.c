...
#  undef ERR
#endif
#define ERR(...) EINA_LOG_DOM_ERR(_Core_anim_log_dom, __VA_ARGS__)

#ifdef DBG
#  undef DBG
#endif
#define DBG(...) EINA_LOG_DOM_DBG(_Core_anim_log_dom, __VA_ARGS__)

#ifdef INF
#  undef INF
#endif
#define INF(...) EINA_LOG_DOM_INFO(_Core_anim_log_dom, __VA_ARGS__)

#ifdef WRN
#  undef WRN
#endif
#define WRN(...) EINA_LOG_DOM_WARN(_Core_anim_log_dom, __VA_ARGS__)

#ifdef CRI
#  undef CRI
#endif
#define CRI(...) EINA_LOG_DOM_CRIT(_Core_anim_log_dom, __VA_ARGS__)
...
static Efl_Bool      tick_skip        = EFL_FALSE;

#ifndef _WIN32
extern volatile int exit_signal_received;
#endif

static Core_Evas_Object_Animator_Interface _anim_iface;
...
static void
_timer_send_time(double t, Core_Thread *thread)
{
    ...
    if (tim)
    {
        *tim = t;
        ...
        core_thread_feedback(thread, tim);
    }
}

static void
_timer_tick_core(void *data EFL_UNUSED, Core_Thread *thread)
{
#ifdef HAVE_EPOLL
    ...
    struct timeval tv;
    Efl_Bool       data_control;
    Efl_Bool       data_timeout;
    unsigned int   t;
    signed char    tick = 0;
    double         t0, d, ft;
    int            ret;
    ...
    if (pollfd >= 0) eina_file_close_on_exec(pollfd, EFL_TRUE);
...
#  endif
    if (timerfd < 0)
    {
        ...
        if (timerfd >= 0) eina_file_close_on_exec(timerfd, EFL_TRUE);
    }
...
#  define INPUT_TIMER_TIMERFD (&(pollincoming[1]))
    ...
    if (pollfd >= 0)
    {
        while (!core_thread_check(thread))
        {
            data_control = EFL_FALSE;
            data_timeout = EFL_FALSE;
...
#endif
    {
        while (!core_thread_check(thread))
        {
            ...
            if ((ret == 1) && (FD_ISSET(timer_fd_read, &rfds)))
                data_control = EFL_TRUE;
            else if (ret == 0) data_timeout = EFL_TRUE;
            if (data_control)
            {
                if (pipe_read(timer_fd_read, &tick, sizeof(tick)) != 1)
                {
...
static void
_timer_tick_finished(void *data EFL_UNUSED, Core_Thread *thread EFL_UNUSED)
{
...
_timer_tick_begin(void)
{
    if (timer_fd_read < 0)
    {
        ...
        eina_file_close_on_exec(fds[0], EFL_TRUE);
        eina_file_close_on_exec(fds[1], EFL_TRUE);
        timer_fd_read  = fds[0];
        timer_fd_write = fds[1];
        if (getenv("CORE_ANIMATOR_SKIP")) tick_skip = EFL_TRUE;
        tick_queue_count = 0;
...
{
    if (timer_fd_read < 0) return;
    core_thread_cancel(timer_thread);
    _tick_send(-1);
    if (timer_thread) core_thread_wait(timer_thread, 0.5);
}
...
_begin_tick(void)
{
    ...
    switch (src)
    {
        case CORE_ANIMATOR_SOURCE_TIMER:
            DBG("General animator registered with timer source.");
            _timer_tick_begin();
            break;

        case CORE_ANIMATOR_SOURCE_CUSTOM:
...
_end_tick(void)
{
    ...
    if ((src == CORE_ANIMATOR_SOURCE_CUSTOM) && end_tick_cb)
...
_do_tick(void)
{
    ...
    {
        animator->just_added = EFL_FALSE;
    }
    if (animators) eina_evlog("!FRAME", NULL, core_loop_time_get(), NULL);
    EINA_INLIST_FOREACH_SAFE(animators, tmp, animator)
    {
        if ((!animator->delete_me) && (!animator->suspended) &&
            (!animator->just_added))
        {
            animator_ran = EFL_TRUE;
            eina_evlog("+animator", animator, 0.0, NULL);
            if (!_core_call_task_cb(animator->func, animator->data))
            {
                animator->delete_me = EFL_TRUE;
                animators_delete_me++;
            }
            eina_evlog("-animator", animator, 0.0, NULL);
        }
        else animator->just_added = EFL_FALSE;
    }
}

static Core_Animator *
_core_animator_add(Core_Task_Cb func, const void *data)
{
    Core_Animator *animator;
    ...
    if (!func)
    {
        ERR("callback function must be set up for an Core_Animator object.");
        return NULL;
    }

    animator = calloc(1, sizeof(Core_Animator));
...
EAPI Core_Animator *
core_animator_add(Core_Task_Cb func, const void *data)
{
    return _core_animator_add(func, data);
}
                            ...
                            const void       *data)
{
    ...
    animator = _core_animator_add(_core_animator_run, NULL);
    if (!animator) return NULL;

    animator->data     = animator;
    animator->run_func = func;
    animator->run_data = (void *)data;
    animator->start    = core_loop_time_get();
...
cuberoot(double v)
{
    if (v < 0.0) return -pow(-v, 1. / 3.);
    else return pow(v, 1. / 3.);
}
...
_bezier_t_get(double _x, double _x1, double _x2)
{
   ...
   // Cardano's algorithm
    double pa = _x - 0.0, pb = _x - _x1, pc = _x - _x2, pd = _x - 1.0;

    double a = 3 * pa - 6 * pb + 3 * pc, b = -3 * pa + 3 * pb, c = pa,
           d = -pa + 3 * pb - 3 * pc + pd;
    ...
    double p = (3 * b - a * a) / 3.0, p3 = p / 3.0,
           q = (2 * a * a * a - 9 * a * b + 27 * c) / 27.0, q2 = q / 2.0,
    ...
    if (discriminant < 0)
    {
        double mp3 = -p / 3.0, mp33 = mp3 * mp3 * mp3, r = sqrt(mp33),
               t      = -q / (2 * r),
               cosphi = t < -1.0  ? -1.0
                        : t > 1.0 ? 1.0
                                  : t,
               phi = acos(cosphi), crtr = cuberoot(r), t1 = 2 * crtr;
    ...
    else if (EINA_DBL_EQ(discriminant, 0))
    {
...
_bezier_calc(double t, double y1, double y2)
{
    ...
    return u3 * y0 + 3.0 * u2 * t * y1 + 3.0 * u * t2 * y2 + t3 * y3;
}

static double
_pos_map_cubic_bezier(double pos, double x1, double y1, double x2, double y2)
{
    if (EINA_DBL_EQ(x1, y1) && EINA_DBL_EQ(x2, y2)) return pos;
                         ...
                         const double *v)
{
   ...
   /* purely functional - locking not required */
    ...
    else if (pos <= 0.0) return 0.0;
    switch (map)
    {
        case CORE_POS_MAP_LINEAR:
            return pos;

        case CORE_POS_MAP_ACCELERATE:
  /* pos = 1 - sin(Pi / 2 + pos * Pi / 2); */
            pos = DBL_TO(
                SUB(INT_FROM(1),
                    SIN(ADD((EINA_F32P32_PI >> 1),
                            MUL(DBL_FROM(pos), (EINA_F32P32_PI >> 1))))));
            return pos;

        case CORE_POS_MAP_DECELERATE:
  /* pos = sin(pos * Pi / 2); */
            pos = DBL_TO(SIN(MUL(DBL_FROM(pos), (EINA_F32P32_PI >> 1))));
            return pos;

        case CORE_POS_MAP_SINUSOIDAL:
  /* pos = (1 - cos(pos * Pi)) / 2 */
            pos = DBL_TO(
                (SUB(INT_FROM(1), COS(MUL(DBL_FROM(pos), EINA_F32P32_PI)))) >>
                1);
            return pos;

        case CORE_POS_MAP_ACCELERATE_FACTOR:
            if (v_size > 0) v0 = v[0];
            pos = _pos_map_accel_factor(pos, v0);
            return pos;

        case CORE_POS_MAP_DECELERATE_FACTOR:
            if (v_size > 0) v0 = v[0];
            pos = 1.0 - _pos_map_accel_factor(1.0 - pos, v0);
            return pos;

        case CORE_POS_MAP_SINUSOIDAL_FACTOR:
            if (v_size > 0) v0 = v[0];
            if (pos < 0.5) pos = _pos_map_accel_factor(pos * 2.0, v0) / 2.0;
            else
                pos =
                    1.0 - (_pos_map_accel_factor((1.0 - pos) * 2.0, v0) / 2.0);
            return pos;

        case CORE_POS_MAP_DIVISOR_INTERP:
            if (v_size > 0) v0 = v[0];
            if (v_size > 1) v1 = v[1];
            pos = _pos_map_pow(pos, v0, (int)v1);
            return pos;

        case CORE_POS_MAP_BOUNCE:
            if (v_size > 0) v0 = v[0];
            if (v_size > 1) v1 = v[1];
        ...
        case CORE_POS_MAP_SPRING:
            if (v_size > 0) v0 = v[0];
            if (v_size > 1) v1 = v[1];
            pos = 1.0 - _pos_map_spring(pos, (int)v1, v0);
            return pos;

        case CORE_POS_MAP_CUBIC_BEZIER:
            if (v_size > 0) v0 = v[0];
            if (v_size > 1) v1 = v[1];
            if (v_size > 2) v2 = v[2];
            if (v_size > 3) v3 = v[3];
...
core_animator_pos_map(double pos, Core_Pos_Map map, double v1, double v2)
{
    ...
    v[0] = v1;
    v[1] = v2;
    return core_animator_pos_map_n(pos, map, 2, v);
}

EAPI void *
core_animator_del(Core_Animator *animator)
{
    void *data = NULL;
    ...
    animator->delete_me = EFL_TRUE;
    animators_delete_me++;
    if (animator->run_func) data = animator->run_data;
    else data = animator->data;

end:
    if (!in_main_loop) _core_animator_flush();
    return data;
}

EAPI void
core_animator_frametime_set(double frametime)
{
...
EAPI double
core_animator_frametime_get(void)
{
...
EAPI void
core_animator_freeze(Core_Animator *animator)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    if (!animator) return;
    ...
    animator->suspended = EFL_TRUE;
    animators_suspended++;
    if (!_have_animators()) _end_tick();
}

EAPI void
core_animator_thaw(Core_Animator *animator)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    if (!animator) return;
    if (animator->delete_me) return;
    ...
    animator->suspended = EFL_FALSE;
    animators_suspended--;
    if (_have_animators()) _begin_tick();
}

EAPI void
core_animator_source_set(Core_Animator_Source source)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    _end_tick();
    src = source;
...
EAPI Core_Animator_Source
core_animator_source_get(void)
{
...
EAPI void
core_animator_custom_tick(void)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    if (src != CORE_ANIMATOR_SOURCE_CUSTOM) return;
#ifndef _WIN32
    if (!exit_signal_received)
#endif
        _do_tick();
...
_core_animator_shutdown(void)
{
    ...
    eina_log_domain_unregister(_Core_anim_log_dom);
    _Core_anim_log_dom = -1;
}

void
_core_animator_run_reset(void)
{
    animator_ran = EFL_FALSE;
}

Efl_Bool
_core_animator_run_get(void)
{
    return animator_ran;
}
...
_core_animator_run(void *data)
{
    ...
    if (eina_dbl_exact(pos, 1.0)) run_ret = EFL_FALSE;
    return run_ret;
}

Efl_Bool
_core_animator_flush(void)
{
    Core_Animator *animator;
    ...
    if (animators_delete_me)
    {
        ...
        {
            animator = l;
            l        = (Core_Animator *)EINA_INLIST_GET(l)->next;
            if (animator->delete_me)
            {
                if (animator->suspended) animators_suspended--;
                animators = (Core_Animator *)eina_inlist_remove(
                                 ...
                                 const void       *data)
{
    ...
    return core_animator_timeline_add(runtime, func, data);
}
...
Core_evas_animator_add(void *evo, Core_Task_Cb func, const void *data)
{
    ...
    if (_ee_animators_setup) anim = _anim_iface.add(evo, func, data);

    if (anim) return anim;

    return core_animator_add(func, data);
}
