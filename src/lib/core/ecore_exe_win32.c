...
static Efl_Bool
_ecore_exe_close_cb(void *data, Ecore_Win32_Handler *wh EFL_UNUSED)
{
                                ...
                                void            *msg)
{
    ...
    if (exe)
    {
        ...
        if (trep->read)
        {
            ...
            if (event_data)
            {
                ecore_event_add(ECORE_EXE_EVENT_DATA,
                                event_data,
        ...
        else if (trep->error)
        {
            ...
            if (event_data)
            {
                ecore_event_add(ECORE_EXE_EVENT_ERROR,
                                event_data,
                                _ecore_exe_event_exe_data_free,
                                NULL);
                efl_event_callback_call(obj,
...
static DWORD __stdcall _ecore_exe_exit_process(void *data)
{
    UINT *code;
    code = (UINT *)data;
    ExitProcess(*code);
...
_ecore_exe_enum_windows_procedure(HWND window, LPARAM data)
{
    ...
    if (thread_id == exe->thread_id)
    {
        ...
        if (CreateRemoteThread(
                ...
                NULL,
                0,
                (LPTHREAD_START_ROUTINE)_ecore_exe_thread_procedure,
                NULL,
                0,
                NULL))
            return EFL_FALSE;
        ...
        if (WaitForSingleObject(exe->process, ECORE_EXE_WIN32_TIMEOUT) ==
            WAIT_OBJECT_0)
            return EFL_FALSE;
        ...
        if (exe->sig == ECORE_EXE_WIN32_SIGTERM) return EFL_FALSE;
...
_impl_ecore_exe_efl_object_finalize(Eo *obj, Ecore_Exe_Data *exe)
{
    ...
    if ((flags & ECORE_EXE_USE_SH)) use_sh = EFL_TRUE;
        ...
        (exe->flags & ECORE_EXE_PIPE_ERROR))
    {
        ...
        if (tdat)
        {
            ...
            exe->th = ecore_thread_feedback_run(_ecore_exe_win32_io_poll_thread,
    ...
    */
    ...
    exe->h_close =
        ecore_main_win32_handler_add(exe->process, _ecore_exe_close_cb, obj);
        ...
        if (is_buffered) /* Deal with line buffering. */
        {
            ...
            if (i > last) /* Partial line left over, save it for next time. */
            {
                ...
                else
                {
                    ...
                    if (exe->pipe_error.data_buf)
                        memcpy(exe->pipe_error.data_buf,
                               c,
...
void
_impl_ecore_exe_hup(Ecore_Exe *obj EFL_UNUSED, Ecore_Exe_Data *exe EFL_UNUSED)
{
    ERR("Not implemented on windows!");
}
