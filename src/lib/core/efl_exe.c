...
_efl_exe_env_set(Eo *obj EFL_UNUSED, Efl_Exe_Data *pd, Efl_Core_Env *env)
{
  ...
  if (!pd->env) efl_unref(pd->env);
  pd->env = env;
  if (pd->env) efl_ref(pd->env);
}
...
_efl_exe_signal(Eo *obj EFL_UNUSED, Efl_Exe_Data *pd, Efl_Exe_Signal sig)
{
  ...
  switch (sig)
  {
    case EFL_EXE_SIGNAL_INT:
      s                  = SIGINT;
      pd->exit_signalled = EFL_TRUE;
      break;
    case EFL_EXE_SIGNAL_QUIT:
      s                  = SIGQUIT;
      pd->exit_signalled = EFL_TRUE;
      break;
    case EFL_EXE_SIGNAL_TERM:
      s                  = SIGTERM;
      pd->exit_signalled = EFL_TRUE;
      break;
    case EFL_EXE_SIGNAL_KILL:
      s                  = SIGKILL;
      pd->exit_signalled = EFL_TRUE;
      break;
    case EFL_EXE_SIGNAL_CONT:
      s = SIGCONT;
      break;
    case EFL_EXE_SIGNAL_STOP:
      s = SIGSTOP;
      break;
    case EFL_EXE_SIGNAL_HUP:
      s = SIGHUP;
      break;
    case EFL_EXE_SIGNAL_USR1:
      s = SIGUSR1;
      break;
    case EFL_EXE_SIGNAL_USR2:
      s = SIGUSR2;
      break;
    default:
      return;
...
EOLIAN static void
_efl_exe_exe_flags_set(Eo *obj       EFL_UNUSED,
                       Efl_Exe_Data *pd,
                       Efl_Exe_Flags flags)
{
  pd->flags = flags;
...
EOLIAN static void
_efl_exe_efl_task_priority_set(Eo *obj           EFL_UNUSED,
                               Efl_Exe_Data     *pd,
...
_efl_exe_efl_task_priority_get(const Eo *obj EFL_UNUSED, Efl_Exe_Data *pd)
{
  ...
  if (pd->pid == -1) return efl_task_priority_get(efl_super(obj, MY_CLASS));
  // p is -20 -> 19
  errno = 0;
  p     = getpriority(PRIO_PROCESS, pd->pid);
  if (errno != 0) return efl_task_priority_get(efl_super(obj, MY_CLASS));
...
_efl_exe_efl_task_run(Eo *obj, Efl_Exe_Data *pd)
{
  ...
  eina_file_close_on_exec(pd->fd.exited_write, EFL_TRUE);
  pd->fd.exited_write = pipe_exited[1];
  eina_file_close_on_exec(pd->fd.exited_read, EFL_TRUE);
...
#  endif
  ...
  if ((td->flags & EFL_TASK_FLAGS_USE_STDOUT) ||
      (pd->flags & EFL_EXE_FLAGS_HIDE_IO))
    close(STDOUT_FILENO);
  if ((pd->flags & EFL_EXE_FLAGS_HIDE_IO)) close(STDERR_FILENO);
...
EOLIAN static void
_efl_exe_efl_io_reader_can_read_set(Eo           *obj,
                                    Efl_Exe_Data *pd,
                                    Efl_Bool      can_read)
{
...
EOLIAN static void
_efl_exe_efl_io_reader_eos_set(Eo *obj, Efl_Exe_Data *pd, Efl_Bool is_eos)
{
...
EOLIAN static Eina_Error
_efl_exe_efl_io_writer_write(Eo           *obj,
                             Efl_Exe_Data *pd,
                             Eina_Slice   *slice,
...
EOLIAN static void
_efl_exe_efl_io_writer_can_write_set(Eo           *obj,
                                     Efl_Exe_Data *pd,
                                     Efl_Bool      can_write)
{
