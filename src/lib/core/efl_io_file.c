...
_efl_io_file_state_update(Eo *o, Efl_Io_File_Data *pd)
{
    ...
    if ((flags == O_RDWR) || (flags == O_WRONLY))
        efl_io_writer_can_write_set(o, EFL_TRUE);
...
_efl_io_file_efl_object_constructor(Eo *o, Efl_Io_File_Data *pd)
{
    ...
    efl_io_closer_close_on_exec_set(o, EFL_TRUE);
    efl_io_closer_close_on_invalidate_set(o, EFL_TRUE);
...
_efl_io_file_efl_object_finalize(Eo *o, Efl_Io_File_Data *pd)
{
    ...
    if (fd < 0)
    {
        ...
        if (pd->mode) fd = open(path, pd->flags, pd->mode);
        else fd = open(path, pd->flags);
...
_efl_io_file_efl_io_reader_read(Eo               *o,
                                Efl_Io_File_Data *pd,
...
EOLIAN static Eina_Error
_efl_io_file_efl_io_writer_write(Eo               *o,
                                 Efl_Io_File_Data *pd,
                                 Eina_Slice       *slice,
                                 Eina_Slice       *remaining)
{
    Eina_Error err =
        efl_io_writer_write(efl_super(o, MY_CLASS), slice, remaining);
    if (err) return err;
    _efl_io_file_state_update(o, pd);
    return 0;
}
...
EOLIAN static Eina_Error
_efl_io_file_efl_io_positioner_seek(Eo                      *o,
                                    Efl_Io_File_Data        *pd,
                                    int64_t                  offset,
                                    Efl_Io_Positioner_Whence whence)
{
    Eina_Error err =
        efl_io_positioner_seek(efl_super(o, MY_CLASS), offset, whence);
    if (err) return err;
    _efl_io_file_state_update(o, pd);
    return 0;
}
...
_efl_io_file_efl_io_closer_close_on_exec_set(Eo               *o,
                                             Efl_Io_File_Data *pd,
                                             Efl_Bool          close_on_exec)
{
    if (close_on_exec) pd->flags |= O_CLOEXEC;
    else pd->flags &= (~O_CLOEXEC);

    return efl_io_closer_close_on_exec_set(efl_super(o, MY_CLASS),
                                           close_on_exec);
}
