...
static Efl_Bool
_event_references_update(Efl_Loop_Handler_Data *pd,
                         const Efl_Event       *event,
                         int                    increment)
{
  const Efl_Callback_Array_Item_Full *array = event->info;
  int                                 i;
  Efl_Bool                            need_reset = EFL_FALSE;
...
_cb_event_callback_add(void *data, const Efl_Event *event)
{
  ...
  if (_event_references_update(pd, event, 1)) _handler_reset(event->object, pd);
}
...
static void
_efl_loop_handler_active_set(Eo                    *obj,
                             Efl_Loop_Handler_Data *pd,
                             Efl_Loop_Handler_Flags flags)
{
  pd->flags = flags;
...
_efl_loop_handler_active_get(const Eo *obj          EFL_UNUSED,
                             Efl_Loop_Handler_Data *pd)
{
  return pd->flags;
}
...
static int
_efl_loop_handler_fd_get(const Eo *obj EFL_UNUSED, Efl_Loop_Handler_Data *pd)
{
...
static int
_efl_loop_handler_fd_file_get(const Eo *obj          EFL_UNUSED,
                              Efl_Loop_Handler_Data *pd)
{
  if (pd->win32) return -1;
...
static void *
_efl_loop_handler_win32_get(const Eo *obj EFL_UNUSED, Efl_Loop_Handler_Data *pd)
{
  return pd->win32;
}

static void
_efl_loop_handler_efl_object_parent_set(Eo                    *obj,
                                        Efl_Loop_Handler_Data *pd,
                                        Efl_Object            *parent)
{
