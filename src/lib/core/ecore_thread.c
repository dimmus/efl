...
_ecore_notify_handler(void *data)
{
   ...
   /* Force reading all notify event before killing the thread */
    if (work->kill &&
        work->u.feedback_run.send == work->u.feedback_run.received)
    {
        _ecore_thread_kill(work);
    }
...
_ecore_message_notify_handler(void *data)
{
    ...
    else
    {
        if (user_data->sync)
        {
            ...
            ecore_pipe_write(work->u.message_run.send,
                             &user_data,
                             sizeof(Ecore_Pthread_Message *));
...
_ecore_thread_init(void)
{
    ...
    if (_ecore_thread_count_max <= 0) _ecore_thread_count_max = 1;
...
_ecore_thread_shutdown(void)
{
    ...
    if (_ecore_thread_global_hash) eina_hash_free(_ecore_thread_global_hash);
...
retry:
    ...
    {
        _ecore_pending_job_threads =
            eina_list_remove(_ecore_pending_job_threads, work);
    ...
    if ((have_main_loop_thread) && (PHE(get_main_loop_thread(), PHS())))
    {
        if (!work->feedback_run)
        ...
        else EINA_LIST_FOREACH(_ecore_pending_job_threads_feedback, l, work)
            {
                if ((void *)work == (void *)thread)
                {
                    _ecore_pending_job_threads_feedback = eina_list_remove_list(
                        _ecore_pending_job_threads_feedback,
                        l);
...
on_exit:
    eina_thread_cancel(
        work->self); /* noop unless eina_thread_cancellable_set() was used by user */
    SLKL(work->cancel_mutex);
...
on_error:
    ...
    {
        _ecore_pending_job_threads_feedback =
            eina_list_remove(_ecore_pending_job_threads_feedback, worker);
...
ecore_thread_feedback(Ecore_Thread *thread, const void *data)
{
    ...
    else if (worker->message_run)
    {
        ...
        ecore_main_loop_thread_safe_call_async(_ecore_message_notify_handler,
                                               notify);
    }
                            ...
                            Efl_Bool      direct)
{
    ...
    if (!(d = malloc(sizeof(Ecore_Thread_Data)))) return EFL_FALSE;
                            ...
                            Eina_Free_Cb  cb)
{
    ...
    if (!(d = malloc(sizeof(Ecore_Thread_Data)))) return NULL;
                             ...
                             Efl_Bool     direct)
{
    ...
    LRWKWL(_ecore_thread_global_hash_lock);
    if (direct) ret = eina_hash_direct_add(_ecore_thread_global_hash, key, d);
    else ret = eina_hash_add(_ecore_thread_global_hash, key, d);
    LRWKU(_ecore_thread_global_hash_lock);
...
ecore_thread_global_data_del(const char *key)
{
    ...
    if (!_ecore_thread_global_hash) return EFL_FALSE;
...
ecore_thread_global_data_wait(const char *key, double seconds)
{
    ...
    if (seconds > 0) tm = ecore_time_get() + seconds;
