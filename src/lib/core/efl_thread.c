...
struct _Efl_Thread_Data
{
  ...
  Eina_Thread                   thread;
  Efl_Bool                      end_sent    : 1;
  Efl_Bool                      exit_read   : 1;
  Efl_Bool                      exit_called : 1;
  Efl_Bool                      run         : 1;
};
...

{
  ...
  arge.initialization = EFL_TRUE;
  efl_event_callback_call(obj, EFL_LOOP_EVENT_ARGUMENTS, &arge);
  if (accessor)
  {
...
_appthread_parent_read_listeners_modify(Efl_Appthread_Data *ad, int mod)
{
  ...
  if (ad->fd.out_handler)
  {
    if ((ad->read_listeners == 0) && (mod < 0))
      efl_loop_handler_active_set(ad->fd.out_handler,
                                  EFL_LOOP_HANDLER_FLAGS_NONE);
...
_efl_thread_pipe_write(int fd, const void *buf, size_t count)
{
  ...
  for (;;)
  {
    ...
    if (ret < 0)
    {
      ...
      if (errno == EINTR) continue;
      ...
      else if (errno == EBADF) ERR("Error writing to control pipe fd: Bad fd");
      else if (errno == EFAULT)
        ERR("Error writing to control pipe fd: Bad buf outside address "
            "space");
        ...
        ERR("Error writing to control pipe fd: Something else "
            "unexpected");
      return;
    }
...
_efl_thread_main(void *data, Eina_Thread t)
{
  ...
  thdat->ctrl.out_handler = efl_add(
  ...
  thdat->argv = NULL;
  efl_future_then(obj, efl_loop_job(obj), .success = _efl_loop_arguments_send);
  free(thdat->event_cb);
  thdat->event_cb = NULL;
...
_cb_event_callback_add(void *data, const Efl_Event *event)
{
  ...
  for (i = 0; array[i].desc != NULL; i++)
  {
    if (array[i].desc == EFL_LOOP_EVENT_ARGUMENTS)
    {
      ...
      num++;
      event_cb =
        realloc(pd->event_cb, (num + 1) * sizeof(Efl_Callback_Array_Item_Full));
      if (event_cb)
      {
...
_cb_event_callback_del(void *data, const Efl_Event *event)
{
  ...
  for (i = 0; array[i].desc != NULL; i++)
  {
    if (array[i].desc == EFL_LOOP_EVENT_ARGUMENTS)
    {
      ...
      if (pd->event_cb)
      {
        Efl_Bool shuffle_down = EFL_FALSE;
...
EOLIAN static void
_efl_thread_efl_object_parent_set(Eo              *obj,
                                  Efl_Thread_Data *pd,
                                  Efl_Object      *parent)
{
...
_efl_thread_efl_task_run(Eo *obj, Efl_Thread_Data *pd)
{
  ...
  if (pd->run) return EFL_FALSE;
  if (!td) return EFL_FALSE;
  thdat = calloc(1, sizeof(Thread_Data));
  if (!thdat) return EFL_FALSE;
  thdat->fd.in    = -1;
  thdat->fd.out   = -1;
  thdat->ctrl.in  = -1;
  thdat->ctrl.out = -1;
  ...
  if (td->flags & EFL_TASK_FLAGS_USE_STDIN)
  {
    ...
    eina_file_close_on_exec(thdat->fd.in, EFL_TRUE);
    eina_file_close_on_exec(pd->fd.out, EFL_TRUE);
  ...
  if (td->flags & EFL_TASK_FLAGS_USE_STDOUT)
  {
    ...
    eina_file_close_on_exec(pd->fd.in, EFL_TRUE);
    eina_file_close_on_exec(thdat->fd.out, EFL_TRUE);
  ...
  if (fcntl(pd->ctrl.out, F_SETFL, O_NONBLOCK) < 0)
    ERR("can't set pipe to NONBLOCK");
  eina_file_close_on_exec(pd->ctrl.in, EFL_TRUE);
  eina_file_close_on_exec(pd->ctrl.out, EFL_TRUE);
  eina_file_close_on_exec(thdat->ctrl.in, EFL_TRUE);
  eina_file_close_on_exec(thdat->ctrl.out, EFL_TRUE);
  pd->ctrl.in_handler =
    efl_add(EFL_LOOP_HANDLER_CLASS,
            obj,
            efl_loop_handler_fd_set(efl_added, pd->ctrl.in));
  pd->ctrl.out_handler = efl_add(
    EFL_LOOP_HANDLER_CLASS,
    obj,
    efl_loop_handler_fd_set(efl_added, pd->ctrl.out),
    efl_event_callback_add(efl_added,
                           EFL_LOOP_HANDLER_EVENT_READ,
                           _cb_thread_parent_ctrl_out,
                           obj),
    efl_loop_handler_active_set(efl_added, EFL_LOOP_HANDLER_FLAGS_READ));
  ...
  {
    while (eina_array_count(thdat->argv))
      eina_stringshare_del(eina_array_pop(thdat->argv));
...
_efl_thread_efl_io_closer_close(Eo *obj, Efl_Thread_Data *pd)
{
  ...
  efl_io_writer_can_write_set(obj, EFL_FALSE);
  efl_io_reader_can_read_set(obj, EFL_FALSE);
  efl_io_reader_eos_set(obj, EFL_TRUE);
...
EOLIAN static Efl_Bool
_efl_thread_efl_io_closer_closed_get(const Eo *obj    EFL_UNUSED,
                                     Efl_Thread_Data *pd)
{
...
EOLIAN static Eina_Error
_efl_thread_efl_io_reader_read(Eo              *obj,
                               Efl_Thread_Data *pd,
                               Eina_Rw_Slice   *rw_slice)
{
  ...
  return 0;
err:
  if ((pd->fd.out != -1) && (errno != EAGAIN)) _io_reader_read_fd_out_clear(pd);
  rw_slice->len = 0;
  rw_slice->mem = NULL;
  efl_io_reader_can_read_set(obj, EFL_FALSE);
  _thread_exit_eval(obj, pd);
...
EOLIAN static void
_efl_thread_efl_io_reader_can_read_set(Eo              *obj,
                                       Efl_Thread_Data *pd,
                                       Efl_Bool         can_read)
{
  Efl_Bool old = efl_io_reader_can_read_get(obj);
  if (old == can_read) return;
  pd->fd.can_read = can_read;
  if (!pd->fd.out_handler) return;
  if (can_read) efl_loop_handler_active_set(pd->fd.out_handler, 0);
  else
    efl_loop_handler_active_set(pd->fd.out_handler,
                                EFL_LOOP_HANDLER_FLAGS_READ);
...
EOLIAN static Efl_Bool
_efl_thread_efl_io_reader_can_read_get(const Eo *obj    EFL_UNUSED,
                                       Efl_Thread_Data *pd)
{
...
EOLIAN static void
_efl_thread_efl_io_reader_eos_set(Eo *obj, Efl_Thread_Data *pd, Efl_Bool is_eos)
{
  Efl_Bool old = efl_io_reader_eos_get(obj);
  if (old == is_eos) return;

  pd->fd.eos_read = is_eos;
  if (!is_eos) return;
  if (pd->fd.out_handler) efl_loop_handler_active_set(pd->fd.out_handler, 0);
...
EOLIAN static Eina_Error
_efl_thread_efl_io_writer_write(Eo              *obj,
                                Efl_Thread_Data *pd,
                                Eina_Slice      *slice,
                                Eina_Slice      *remaining)
{
  ...
  if (slice->len > 0) efl_io_writer_can_write_set(obj, EFL_FALSE);
  if (r == 0)
  {
  ...
  return 0;
err:
  if ((pd->fd.in != -1) && (errno != EAGAIN)) _io_writer_write_fd_in_clear(pd);
...
EOLIAN static void
_efl_thread_efl_io_writer_can_write_set(Eo              *obj,
                                        Efl_Thread_Data *pd,
                                        Efl_Bool         can_write)
{
  Efl_Bool old = efl_io_writer_can_write_get(obj);
  if (old == can_write) return;
  pd->fd.can_write = can_write;
  if (can_write) efl_loop_handler_active_set(pd->fd.in_handler, 0);
  else
...
EOLIAN static Efl_Bool
_efl_thread_efl_io_writer_can_write_get(const Eo *obj    EFL_UNUSED,
                                        Efl_Thread_Data *pd)
{
...
static void
_threadio_call(int             fd,
               void           *func_data,
               EflThreadIOCall func,
...
void
_appthread_threadio_call(Eo *obj             EFL_UNUSED,
...
EOLIAN static void
_efl_thread_efl_threadio_call(Eo *obj          EFL_UNUSED,
                              Efl_Thread_Data *pd,
                              void            *func_data,
                              EflThreadIOCall  func,
                              Eina_Free_Cb     func_free_cb)
...
static void *
_threadio_call_sync(int                 fd,
                    void               *func_data,
                    EflThreadIOCallSync func,
                    Eina_Free_Cb        func_free_cb)
{
  Control_Data  cmd;
  Control_Reply rep;
...
void *
_appthread_threadio_call_sync(Eo *obj             EFL_UNUSED,
                              Efl_Appthread_Data *pd,
                              void               *func_data,
                              EflThreadIOCallSync func,
                              Eina_Free_Cb        func_free_cb)
...
EOLIAN static void *
_efl_thread_efl_threadio_call_sync(Eo *obj             EFL_UNUSED,
                                   Efl_Thread_Data    *pd,
                                   void               *func_data,
                                   EflThreadIOCallSync func,
                                   Eina_Free_Cb        func_free_cb)
