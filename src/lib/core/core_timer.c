...
struct _Core_Timer_Legacy
{
    Core_Task_Cb func;

    const void *data;

    Efl_Bool inside_call : 1;
    Efl_Bool delete_me   : 1;
};
typedef struct _Core_Timer_Legacy Core_Timer_Legacy;
...
struct _Efl_Loop_Timer_Data
{
    ...
    Efl_Bool frozen      : 1;
    Efl_Bool initialized : 1;
    Efl_Bool noparent    : 1;
    Efl_Bool constructed : 1;
    Efl_Bool finalized   : 1;
};
...
EAPI void
core_timer_precision_set(double value)
{
...
_efl_loop_timer_efl_object_finalize(Eo *obj, Efl_Loop_Timer_Data *pd)
{
    ...
    pd->initialized = EFL_TRUE;
    pd->finalized   = EFL_TRUE;
    _efl_loop_timer_set(pd, pd->at, pd->in);
    return efl_finalize(efl_super(obj, MY_CLASS));
}

static void
_core_timer_legacy_del(void *data, const Efl_Event *event EFL_UNUSED)
{
    free(data);
}
...
_core_timer_legacy_tick(void *data, const Efl_Event *event)
{
    ...
    if (!_core_call_task_cb(legacy->func, (void *)legacy->data) ||
        legacy->delete_me)
    {
        legacy->delete_me = EFL_TRUE;
        /* cannot destroy timer if recursing */
        if (!inside_call) efl_del(event->object);
    }
    /* only unset flag if not currently recursing */
    else if (!inside_call) legacy->inside_call = 0;
}

EFL_CALLBACKS_ARRAY_DEFINE(legacy_timer,
                           { EFL_LOOP_TIMER_EVENT_TIMER_TICK,
                             _core_timer_legacy_tick },
                           { EFL_EVENT_DEL, _core_timer_legacy_del });
...
core_timer_add(double in, Core_Task_Cb func, const void *data)
{
    ...
    legacy = calloc(1, sizeof(Core_Timer_Legacy));
...
EAPI void *
core_timer_del(Core_Timer *timer)
{
    void *data;
    ...
    if (td->legacy->inside_call) td->legacy->delete_me = EFL_TRUE;
    else efl_del(timer);
    return data;
}

EOLIAN static void
_efl_loop_timer_timer_interval_set(Eo *obj              EFL_UNUSED,
                                   Efl_Loop_Timer_Data *timer,
                                   double               in)
{
    if (in < 0.0) in = 0.0;
...
_efl_loop_timer_timer_interval_get(const Eo *obj        EFL_UNUSED,
                                   Efl_Loop_Timer_Data *timer)
{
    return timer->in;
}
...
EOLIAN static void
_efl_loop_timer_timer_delay(Eo *obj              EFL_UNUSED,
...
EOLIAN static double
_efl_loop_timer_time_pending_get(const Eo *obj        EFL_UNUSED,
                                 Efl_Loop_Timer_Data *timer)
{
    double now, ret = 0.0;
...
EAPI void
core_timer_freeze(Core_Timer *timer)
{
    Core_TIMER_CHECK(timer);
...
_efl_loop_timer_efl_object_event_freeze(Eo *obj, Efl_Loop_Timer_Data *timer)
{
    ...
    if (timer->loop) now = efl_loop_time_get(timer->loop);
    /* only if timer interval has been set */
    if (timer->initialized) timer->pending = timer->at - now;
    else timer->pending = 0.0;
    timer->at     = 0.0;
    timer->frozen = 1;
...
EAPI Efl_Bool
core_timer_freeze_get(Core_Timer *timer)
{
...
EOLIAN static int
_efl_loop_timer_efl_object_event_freeze_count_get(const Eo *obj EFL_UNUSED,
                                                  Efl_Loop_Timer_Data *timer)
{
    return timer->frozen;
}
...
EAPI void
core_timer_thaw(Core_Timer *timer)
{
    Core_TIMER_CHECK(timer);
...
_efl_loop_timer_efl_object_event_thaw(Eo *obj, Efl_Loop_Timer_Data *timer)
{
    ...
    if (timer->loop_data)
    {
        timer->loop_data->suspended =
            eina_inlist_remove(timer->loop_data->suspended,
...
_efl_loop_timer_util_loop_clear(Efl_Loop_Timer_Data *pd)
{
    ...
    if (pd->loop_data->timer_current == pd)
        pd->loop_data->timer_current =
            (Efl_Loop_Timer_Data *)EINA_INLIST_GET(pd)->next;

    // Remove the timer from all possible pending list
    first = eina_inlist_first(EINA_INLIST_GET(pd));
    if (first == pd->loop_data->timers)
        pd->loop_data->timers =
            eina_inlist_remove(pd->loop_data->timers, EINA_INLIST_GET(pd));
    else if (first == pd->loop_data->suspended)
        pd->loop_data->suspended =
            eina_inlist_remove(pd->loop_data->suspended, EINA_INLIST_GET(pd));
}

static void
_efl_loop_timer_util_instanciate(Efl_Loop_Data       *loop,
                                 Efl_Loop_Timer_Data *timer)
{
    ...
    if ((!timer->listening) || (timer->frozen) || (timer->at <= 0.0) ||
        (timer->in < 0.0))
    {
        loop->suspended =
            eina_inlist_prepend(loop->suspended, EINA_INLIST_GET(timer));
        return;
    }
...
EOLIAN static void
_efl_loop_timer_efl_object_parent_set(Eo                  *obj,
                                      Efl_Loop_Timer_Data *pd,
                                      Efl_Object          *parent)
{
    Eina_Inlist *first;
    ...
    pd->loop = efl_provider_find(obj, EFL_LOOP_CLASS);
    if (pd->loop) pd->loop_data = efl_data_scope_get(pd->loop, EFL_LOOP_CLASS);
    else pd->loop_data = NULL;

    if (efl_parent_get(obj) != parent) return;

    _efl_loop_timer_util_instanciate(pd->loop_data, pd);

    if (parent != NULL) pd->noparent = EFL_FALSE;
    else pd->noparent = EFL_TRUE;
}
...
_efl_loop_timer_reschedule(Efl_Loop_Timer_Data *timer, double when)
{
    if (timer->frozen || efl_invalidated_get(timer->object) ||
        (timer->legacy && timer->legacy->delete_me))
        return;
        ...
        (EINA_INLIST_GET(timer)->next || EINA_INLIST_GET(timer)->prev))
    {
        if (timer->loop_data->timers && (!timer->noparent))
            timer->loop_data->timers =
                eina_inlist_remove(timer->loop_data->timers,
                                   EINA_INLIST_GET(timer));
    }
    ...
    */
    ...
    else _efl_loop_timer_set(timer, timer->at + timer->in, timer->in);
}
...
_efl_loop_timer_expired_call(Eo *obj EFL_UNUSED, Efl_Loop_Data *pd, double when)
{
    ...
    else
    {
        ...
        pd->timer_current =
            (Efl_Loop_Timer_Data *)EINA_INLIST_GET(pd->timer_current)->next;
    ...
    while (pd->timer_current)
    {
        ...
        efl_event_callback_call(timer->object,
                                EFL_LOOP_TIMER_EVENT_TIMER_TICK,
                                NULL);
        eina_evlog("-timer", timer, 0.0, NULL);
        ...
        if (pd->timer_current)
        {
            if (pd->timer_current == timer)
                pd->timer_current =
                    (Efl_Loop_Timer_Data *)EINA_INLIST_GET(pd->timer_current)
                        ->next;
