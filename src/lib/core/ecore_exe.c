...
ecore_exe_pipe_run(const char *exe_cmd, Ecore_Exe_Flags flags, const void *data)
{
    ...
    Ecore_Exe *ret =
        efl_add(MY_CLASS,
                efl_main_loop_get(),
                ecore_obj_exe_command_set(efl_added, exe_cmd, flags));
    if (ret)
    {
...
EOLIAN static void
_ecore_exe_command_set(Eo *obj         EFL_UNUSED,
                       Ecore_Exe_Data *pd,
                       const char     *cmd,
...
EOLIAN static void
_ecore_exe_command_get(const Eo *obj    EFL_UNUSED,
                       Ecore_Exe_Data  *pd,
                       const char     **cmd,
                       Ecore_Exe_Flags *flags)
{
...
EAPI void
ecore_exe_callback_pre_free_set(Ecore_Exe *obj, Ecore_Exe_Cb func)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
...
EAPI Efl_Bool
ecore_exe_send(Ecore_Exe *obj, const void *data, int size)
{
                          ...
                          int        end_lines)
{
    ...
    Ecore_Exe_Data *exe = efl_data_scope_get(obj, MY_CLASS);
    if (!efl_isa(obj, MY_CLASS)) return;
    _impl_ecore_exe_auto_limits_set(obj,
                                    exe,
                                    start_bytes,
                                    end_bytes,
                                    start_lines,
...
ecore_exe_event_data_get(Ecore_Exe *obj, Ecore_Exe_Flags flags)
{
    EINA_MAIN_LOOP_CHECK_RETURN_VAL(NULL);
    Ecore_Exe_Data *exe = efl_data_scope_get(obj, MY_CLASS);
...
EAPI void
ecore_exe_tag_set(Ecore_Exe *obj, const char *tag)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    Ecore_Exe_Data *exe = efl_data_scope_get(obj, MY_CLASS);
    if (!efl_isa(obj, MY_CLASS)) return;
...
EAPI void *
ecore_exe_data_set(Ecore_Exe *obj, void *data)
{
    void *ret;
    EINA_MAIN_LOOP_CHECK_RETURN_VAL(NULL);
    Ecore_Exe_Data *exe = efl_data_scope_get(obj, MY_CLASS);
...
EOLIAN static void
_ecore_exe_efl_control_suspend_set(Eo *obj         EFL_UNUSED,
                                   Ecore_Exe_Data *exe,
                                   Efl_Bool        suspend)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
...
EAPI void
ecore_exe_signal(Ecore_Exe *obj, int num)
{
    EINA_MAIN_LOOP_CHECK_RETURN;
    Ecore_Exe_Data *exe = efl_data_scope_get(obj, MY_CLASS);
    if (!efl_isa(obj, MY_CLASS)) return;
...
_ecore_exe_init(void)
{
    ...
    if (_ecore_exe_win32_job)
    {
        ...
        if (!SetInformationJobObject(_ecore_exe_win32_job,
...
#ifdef _WIN32
    if (_ecore_exe_win32_job) CloseHandle(_ecore_exe_win32_job);
#endif
}
...
void
_ecore_exe_event_del_free(void *data EFL_UNUSED, void *ev)
{
...
void
_ecore_exe_event_exe_data_free(void *data EFL_UNUSED, void *ev)
{
...
void
_ecore_exe_event_add_free(void *data EFL_UNUSED, void *ev)
{
