                             ...
                             const Core_Getopt_Desc *desc)
{
  ...
  used = _core_getopt_help_line(fp, base, total, used, ". ", sizeof(". ") - 1);
                               ...
                               const Core_Getopt_Desc *desc)
{
  ...
  {
    used = _core_getopt_help_line(fp, base, total, used, *itr, strlen(*itr));
    if (itr[1])
                       ...
                       const Core_Getopt      *parser)
{
  ...
  switch (desc->action)
  {
    ...
    case CORE_GETOPT_ACTION_HELP:
      _core_getopt_help_desc_categories(fp, helpcol, cols, used, parser->descs);
      break;
...
error:
  _core_getopt_desc_print_error(desc, _("invalid number format %s\n"), arg_val);
  return EFL_FALSE;
                          ...
                          const char               *arg_val)
{
  ...
  if (!arg_val)
  {
    _core_getopt_desc_print_error(desc, _("missing parameter to append.\n"));
    return EFL_FALSE;
  }
  ...
  switch (desc->action_param.append_type)
  {
    ...
    default:
      {
        _core_getopt_desc_print_error(desc, _("could not parse value.\n"));
        return EFL_FALSE;
      }
  }
...
error:
  {
    ...
    if (!help) fputc('\n', stderr);
    else if (help->longname) fprintf(stderr, _(" See --%s.\n"), help->longname);
