...
static Efl_Bool     _ecore_main_uv_idling;
...
EAPI double
_ecore_main_loop_wakeup_time_get(void)
{
...
static inline void
_ecore_try_add_to_call_list(Eo *obj           EFL_UNUSED,
                            Efl_Loop_Data    *pd,
                            Ecore_Fd_Handler *fdh)
{
        ...
        (fdh->flags & ECORE_FD_ALWAYS))
    {
        ...
        fdh->next_ready =
            pd->fd_handlers_to_call ? pd->fd_handlers_to_call : fdh;
...
_ecore_main_uv_poll_cb(uv_poll_t *handle, int status, int events)
{
    ...
    if (_ecore_main_uv_idling)
    {
        ...
        _ecore_main_uv_idling = EFL_FALSE;
...
#  endif
    {
        if ((!fdh->file) && (pd->epoll_fd >= 0))
            r = _ecore_epoll_add(_ecore_get_epoll_fd(fdh->loop, pd),
                                 fdh->fd,
                                 _ecore_poll_events_from_fdh(fdh),
                                 fdh);
    }
#  ifdef HAVE_LIBUV
    else
#  endif
#endif
    {
        ...
        if (ecore_glib_source) g_source_add_poll(ecore_glib_source, &fdh->gfd);
        else
        ...
        if ((!fdh->file) && (pd->epoll_fd >= 0))
        {
            ...
            if ((epoll_ctl(efd, EPOLL_CTL_DEL, fdh->fd, &ev) < 0))
            {
                if (errno == EBADF)
                {
                    ...
                    ERR("Failed to delete epoll fd %d! (errno=%d)",
                        fdh->fd,
                        errno);
            }
        }
...
_ecore_main_fdh_epoll_mark_active(Eo *obj, Efl_Loop_Data *pd)
{
    ...
    for (i = 0; i < ret; i++)
    {
        ...
        {
            ECORE_MAGIC_FAIL(fdh,
                             ECORE_MAGIC_FD_HANDLER,
        ...
        /* We'll add this one anyway outside this function,
           don't want it twice */
        if (fdh->flags & ECORE_FD_ALWAYS) continue;
...
static gboolean
_ecore_main_gsource_prepare(GSource *source EFL_UNUSED, gint *next_time)
{
    Eo            *obj   = ML_OBJ;
    Efl_Loop_Data *pd    = ML_DAT;
...
_ecore_main_gsource_check(GSource *source EFL_UNUSED)
{
    ...
    if (ecore_idling && (!_ecore_main_idlers_exist(pd)) && (!pd->message_queue))
    {
        if (pd->timer_fd >= 0)
        {
            uint64_t count = 0;
...
#  endif
        ...
        if (lib && dlsym(lib, "uv_run"))
        {
            ...
            _dl_uv_prepare_start(&_ecore_main_uv_prepare,
...
_ecore_main_loop_begin(Eo *obj, Efl_Loop_Data *pd)
{
    ...
    else
    {
        ...
        if (!pd->do_quit)
        {
            if (!ecore_main_loop) ecore_main_loop = g_main_loop_new(NULL, 1);
            g_main_loop_run(ecore_main_loop);
        }
        pd->do_quit = 0;
#endif
                           ...
                           Efl_Bool               is_file)
{
    ...
    pd->fd_handlers =
        (Ecore_Fd_Handler *)eina_inlist_append(EINA_INLIST_GET(pd->fd_handlers),
                                               EINA_INLIST_GET(fdh));
    return fdh;
}
                              ...
                              const void           *data)
{
    ...
    wh->func           = func;
    wh->data           = (void *)data;
    pd->win32_handlers = (Ecore_Win32_Handler *)eina_inlist_append(
...
ecore_main_win32_handler_del(Ecore_Win32_Handler *win32_handler)
{
    ...
    {
        ECORE_MAGIC_FAIL(win32_handler,
                                 ...
                                 Ecore_Fd_Handler_Flags flags)
{
    ...
    if ((flags & ECORE_FD_READ) && (fd_handler->read_active)) ret = EFL_TRUE;
    if ((flags & ECORE_FD_WRITE) && (fd_handler->write_active)) ret = EFL_TRUE;
    if ((flags & ECORE_FD_ERROR) && (fd_handler->error_active)) ret = EFL_TRUE;
    return ret;
}
...
_ecore_main_content_clear(Eo *obj, Efl_Loop_Data *pd)
{
    ...
    while (pd->fd_handlers)
    {
        ...
        pd->fd_handlers = (Ecore_Fd_Handler *)eina_inlist_remove(
    ...
    while (pd->win32_handlers)
    {
        ...
        pd->win32_handlers = (Ecore_Win32_Handler *)eina_inlist_remove(
            EINA_INLIST_GET(pd->win32_handlers),
            EINA_INLIST_GET(wh));
...
#  endif
    ...
    {
#  ifdef HAVE_SYS_EPOLL_H
        if (pd->epoll_fd >= 0) _ecore_main_fdh_epoll_mark_active(obj, pd);
        else
#  endif
        {
            EINA_INLIST_FOREACH(pd->fd_handlers, fdh)
    ...
    while (fdh)
    {
        if (fdh->delete_me)
        {
            if (!last)
                ...
                last->next_ready =
                    fdh == fdh->next_ready ? last : fdh->next_ready;
        }
        else last = fdh;
    ...
    EINA_LIST_FOREACH_SAFE(pd->fd_handlers_to_delete, l, l2, fdh)
    {
        ...
        if (fdh->flags & ECORE_FD_ALWAYS)
            pd->always_fd_handlers =
                eina_list_remove(pd->always_fd_handlers, fdh);
        ECORE_MAGIC_SET(fdh, ECORE_MAGIC_NONE);
        ecore_fd_handler_mp_free(fdh);
        pd->fd_handlers_to_delete =
    ...
    while (pd->fd_handlers_to_call_current)
    {
        ...
        if (!fdh->delete_me)
        {
            if ((fdh->read_active) || (fdh->write_active) ||
                (fdh->error_active) || (fdh->flags & ECORE_FD_ALWAYS))
            {
                fdh->references++;
                if (!_ecore_call_fd_cb(fdh->func, fdh->data, fdh))
                {
...
_ecore_main_loop_uv_prepare(uv_prepare_t *handle EFL_UNUSED)
{
    ...
    if (!_ecore_main_uv_idling)
    {
        _ecore_main_uv_idling = EFL_TRUE;
        efl_event_callback_call(obj, EFL_LOOP_EVENT_IDLE_ENTER, NULL);
        _ecore_throttle();
        _throttle_do(pd);
    }
...
_ecore_main_loop_iterate_internal(Eo *obj, Efl_Loop_Data *pd, int once_only)
{
    ...
    if (once_only)
    {
        ...
        // merged together
        if (_ecore_main_select(obj, pd, 0.0) | _ecore_signal_count_get(obj, pd))
        {
            _efl_loop_timer_enable_new(obj, pd);
            goto process_all;
        }
...
start_loop:  //-*************************************************************
    ...
    if (!pd->message_queue)
    {
        ...
        else
        {
            ...
            // no timers - spin
            if (next_time < 0)
                action = _ecore_main_loop_spin_no_timers(obj, pd);
            // timers - spin
...
static unsigned int __stdcall _ecore_main_win32_objects_wait_thread(void *data)
{
                               ...
                               DWORD         timeout)
{
    ...
    threads_data = (Ecore_Main_Win32_Thread_Data *)malloc(
        threads_nbr * sizeof(Ecore_Main_Win32_Thread_Data));
    if (!threads_data)
    {
        ERR("Can not allocate memory for the waiting thread.");
    ...
    if (result < (WAIT_OBJECT_0 + threads_nbr))
    {
        ...
        if (GetExitCodeThread(threads_handles[result - WAIT_OBJECT_0],
                              &wait_res))
        {
            WaitForMultipleObjects(threads_nbr,
                                   threads_handles,
                                   TRUE,
                                   INFINITE);
            for (i = 0; i < threads_nbr; i++)
                         ...
                         struct timeval *tv)
{
    ...
    EINA_INLIST_FOREACH(pd->win32_handlers, wh)
    {
        ...
        if (wh->h == stdin_handle)
        {
            if (stdin_wait_thread == INVALID_HANDLE_VALUE)
                stdin_wait_thread = (HANDLE)
                    _beginthreadex(NULL, 0, _stdin_wait_thread, NULL, 0, NULL);
             ...
             (result < (WAIT_OBJECT_0 + objects_nbr)))
    {
        if (!pd->win32_handler_current)
            ...
            pd->win32_handler_current = (Ecore_Win32_Handler *)EINA_INLIST_GET(
        ...
        while (pd->win32_handler_current)
        {
            ...
            if (pd->win32_handler_current)
                ...
                pd->win32_handler_current =
                    (Ecore_Win32_Handler *)EINA_INLIST_GET(
                        pd->win32_handler_current)
                        ->next;
        }
        res = 1;
