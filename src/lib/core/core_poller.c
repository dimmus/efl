...
#define MY_CLASS Core_POLLER_CLASS

#define MY_CLASS_NAME "Core_Poller"

#define Core_POLLER_CHECK(obj) \
    if (!efl_isa((obj), Core_POLLER_CLASS)) return
...
_core_poller_cleanup(Core_Poller *poller)
{
  ...
  data = poller->data;
  pollers[poller->ibit] =
    (Core_Poller *)eina_inlist_remove(EINA_INLIST_GET(pollers[poller->ibit]),
...
_core_poller_next_tick_eval(void)
{
  ...
  if (at_tick)
  {
    if (!timer) timer = core_timer_add(interval, _core_poller_cb_timer, NULL);
  }
  else
  {
    double t;
    ...
    else
    {
      ...
      * time interval. at the tick this will be adjusted */
        core_timer_del(timer);
        timer = core_timer_loop_add(interval - t, _core_poller_cb_timer, NULL);
      }
    }
  }
...
_core_poller_cb_timer(void *data EFL_UNUSED)
{
  ...
  /* if the timer was deleted then there is no point returning 1 - ambiguous
    ...
    * anyway */
  if (!timer) return CORE_CALLBACK_CANCEL;

  /* adjust interval */
  core_timer_interval_set(timer, poll_cur_interval);
                ...
                const void           *data)
{
  ...
  pollers[poller->ibit] =
    (Core_Poller *)eina_inlist_prepend(EINA_INLIST_GET(pollers[poller->ibit]),
                                       EINA_INLIST_GET(poller));
...
core_poller_poller_interval_set(Core_Poller *poller, int interval)
{
  ...
  if (poller->ibit == ibit) return EFL_TRUE;
