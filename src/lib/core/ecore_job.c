...
ecore_job_add(Ecore_Cb func, const void *data)
{
    ...
    job->event =
        ecore_event_add(ecore_event_job_type, job, _ecore_job_event_free, job);
    if (!job->event)
    {
...
static Efl_Bool
_ecore_job_event_handler(void *data EFL_UNUSED, int type EFL_UNUSED, void *ev)
{
    Ecore_Job *job;
...
static void
_ecore_job_event_free(void *data, void *job EFL_UNUSED)
{
    free(data);
}
