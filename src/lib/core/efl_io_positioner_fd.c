...
EOLIAN static void
_efl_io_positioner_fd_positioner_fd_set(Eo *o                      EFL_UNUSED,
                                        Efl_Io_Positioner_Fd_Data *pd,
                                        int                        fd)
{
  pd->fd = fd;
}
...
EOLIAN static int
_efl_io_positioner_fd_positioner_fd_get(const Eo *o                EFL_UNUSED,
                                        Efl_Io_Positioner_Fd_Data *pd)
{
  return pd->fd;
}
...
_efl_io_positioner_whence_convert(Efl_Io_Positioner_Whence whence)
{
  switch (whence)
  {
    case EFL_IO_POSITIONER_WHENCE_START:
      return SEEK_SET;
    case EFL_IO_POSITIONER_WHENCE_CURRENT:
      return SEEK_CUR;
    case EFL_IO_POSITIONER_WHENCE_END:
      return SEEK_END;
  }
  return SEEK_SET;
}
...
_efl_io_positioner_fd_efl_io_positioner_seek(Eo *o,
                                             Efl_Io_Positioner_Fd_Data *pd
                                                     EFL_UNUSED,
                                             int64_t offset,
...
_efl_io_positioner_fd_efl_io_positioner_position_get(
  const Eo                     *o,
  Efl_Io_Positioner_Fd_Data *pd EFL_UNUSED)
{
  int   fd = efl_io_positioner_fd_get(o);
  off_t offset;
