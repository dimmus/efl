...
void
_efl_loop_message_send_info_set(Eo            *obj,
                                Eina_Inlist   *node,
                                Eo            *loop,
                                Efl_Loop_Data *loop_data)
{
...
_efl_loop_message_efl_object_constructor(Eo                       *obj,
                                         Efl_Loop_Message_Data *pd EFL_UNUSED)
{
  ...
  if ((pd->send_list_node) && (pd->loop_data))
  {
    ...
    if (pd->loop_data->message_walking == 0)
    {
      pd->loop_data->message_queue =
        eina_inlist_remove(pd->loop_data->message_queue, pd->send_list_node);
    }
