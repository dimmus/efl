...
static Efl_Bool _ecore_pipe_read(void *data, Ecore_Fd_Handler *fd_handler);
...
// Private functions
Ecore_Pipe *
_ecore_pipe_add(Ecore_Pipe_Cb handler, const void *data)
{
    return ecore_pipe_full_add(handler, data, -1, -1, EFL_FALSE, EFL_FALSE);
}
...
#if !defined(HAVE_SYS_EPOLL_H) || !defined(HAVE_SYS_TIMERFD_H)
int
_ecore_pipe_wait(Ecore_Pipe *p, int message_count, double wait)
{
    struct timeval tv, *t;
           ...
           (ret = epoll_wait(p->pollfd, pollincoming, 2, time_exit)) > 0)
    {
        ...
        for (int i = 0; i < ret; i++)
        {
            if ((&p->fd_read == pollincoming[i].data.ptr))
                fd_read_found = EFL_TRUE;
            if ((&p->timerfd == pollincoming[i].data.ptr))
                fd_timer_found = EFL_TRUE;
        }
...
#endif
static void
_ecore_pipe_handler_call(Ecore_Pipe *p, unsigned char *buf, size_t len)
{
...
static Efl_Bool
_ecore_pipe_read(void *data, Ecore_Fd_Handler *fd_handler EFL_UNUSED)
{
