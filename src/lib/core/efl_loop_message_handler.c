...
_efl_loop_message_handler_message_add(Eo *obj,
                                      Efl_Loop_Message_Handler_Data *pd
                                        EFL_UNUSED)
{
...
EOLIAN static void
_efl_loop_message_handler_message_send(Eo                            *obj,
                                       Efl_Loop_Message_Handler_Data *pd,
                                       Efl_Loop_Message              *message)
{
  ...
  if (msg)
  {
    ...
    {
      pd->loop_data->message_pending_queue =
        eina_inlist_append(pd->loop_data->message_pending_queue,
                           EINA_INLIST_GET(msg));
    }
    else
    {
      pd->loop_data->message_queue =
        eina_inlist_append(pd->loop_data->message_queue, EINA_INLIST_GET(msg));
    }
    _efl_loop_message_send_info_set(message,
                                    EINA_INLIST_GET(msg),
                                    pd->loop,
                                    pd->loop_data);
    return;
...
EOLIAN static void
_efl_loop_message_handler_message_call(Eo                            *obj,
                                       Efl_Loop_Message_Handler_Data *pd,
                                       Efl_Loop_Message              *message)
{
  Message     *msg;
  ...
  EINA_INLIST_FOREACH(pd->loop_data->message_queue, msg)
  {
    ...
    {
      pd->loop_data->message_queue =
        eina_inlist_remove(pd->loop_data->message_queue, EINA_INLIST_GET(msg));
      free(msg);
    }
    else msg->delete_me = EFL_TRUE;
    break;
  }
  efl_event_callback_call(message, EFL_LOOP_MESSAGE_EVENT_MESSAGE, message);
  efl_event_callback_call(obj, EFL_LOOP_MESSAGE_HANDLER_EVENT_MESSAGE, message);
   // XXX: implement message object cache...
...
_efl_loop_message_handler_message_clear(Eo                            *obj,
                                        Efl_Loop_Message_Handler_Data *pd)
{
  Eina_Inlist *tmp;
  Message     *msg;

  if (!pd->loop) return EFL_FALSE;
  if (!pd->loop_data->message_queue) return EFL_FALSE;
...
_efl_loop_message_handler_efl_object_constructor(
  Eo                               *obj,
  Efl_Loop_Message_Handler_Data *pd EFL_UNUSED)
{
...
EOLIAN static void
_efl_loop_message_handler_efl_object_destructor(
  Eo                               *obj,
  Efl_Loop_Message_Handler_Data *pd EFL_UNUSED)
{
