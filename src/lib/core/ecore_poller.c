...
#define ECORE_POLLER_CHECK(obj) \
    if (!efl_isa((obj), ECORE_POLLER_CLASS)) return
...
static Ecore_Poller  *pollers[16] = { NULL, NULL, NULL, NULL, NULL, NULL,
...
static Efl_Bool _ecore_poller_cb_timer(void *data);
...
_ecore_poller_cleanup(Ecore_Poller *poller)
{
    ...
    data                  = poller->data;
    pollers[poller->ibit] = (Ecore_Poller *)eina_inlist_remove(
...
static Efl_Bool
_ecore_poller_cb_timer(void *data EFL_UNUSED)
{
    Ecore_Poller *poller;
    int           i;
    int           changes = 0;
...
EAPI void
ecore_poller_poll_interval_set(Ecore_Poller_Type type EFL_UNUSED,
                               double                 poll_time)
{
...
EAPI double
ecore_poller_poll_interval_get(Ecore_Poller_Type type EFL_UNUSED)
{
                 ...
                 const void            *data)
{
    ...
    pollers[poller->ibit] = (Ecore_Poller *)eina_inlist_prepend(
        EINA_INLIST_GET(pollers[poller->ibit]),
        EINA_INLIST_GET(poller));
    if (poller_walking) just_added_poller++;
    else _ecore_poller_next_tick_eval();
