...
static Efl_Object *
_efl_container_model_efl_object_finalize(Eo *obj,
                                         Efl_Container_Model_Data *sd
                                             EFL_UNUSED)
{
                                        ...
                                        Eina_Iterator            *values)
{
    ...
    EINA_ITERATOR_FOREACH(values, original)
    {
        ...
        else r = eina_value_pset(copy, original);
    ...
    if (!cpd)
    {
        ...
        if (!cpd) goto on_error;

        cpd->name   = eina_stringshare_ref(name);
        cpd->type   = type;
        cpd->values = arr;

        if (!eina_hash_direct_add(sd->properties, name, cpd)) goto on_error;
    }
    else
    {
        _property_values_free(cpd->values);
...
_efl_container_model_efl_model_properties_get(const Eo *obj EFL_UNUSED,
                                              Efl_Container_Model_Data *sd)
{
    EFL_COMPOSITE_MODEL_PROPERTIES_SUPER(
        props,
        obj,
        EFL_CONTAINER_MODEL_CLASS,
    ...
    if (pd->parent)
    {
        ...
        if (eina_value_type_get(value) != cpd->type)
        {
            ...
            if (!eina_value_convert(value, &r))
                return efl_loop_future_rejected(
                    obj,
                    EFL_MODEL_ERROR_INCORRECT_VALUE);
        }
        else
        ...
        while (index >= eina_array_count(cpd->values))
            eina_array_push(
                cpd->values,
                eina_value_error_new(EFL_MODEL_ERROR_INCORRECT_VALUE));
...
not_mine:
    return efl_model_property_set(efl_super(obj, EFL_CONTAINER_MODEL_CLASS),
                                  property,
                                  value);
}
...
not_mine:
    return efl_model_property_get(efl_super(obj, EFL_CONTAINER_MODEL_CLASS),
                                  property);
}

static unsigned int
_efl_container_model_efl_model_children_count_get(const Eo *obj EFL_UNUSED,
                                                  Efl_Container_Model_Data *sd)
{
    unsigned int pcount;

    pcount =
        efl_model_children_count_get(efl_super(obj, EFL_CONTAINER_MODEL_CLASS));
