...
static Eolian_Type *parse_type_void(Eo_Lexer *ls, Efl_Bool allow_ptr, Efl_Bool allow_const);
...
parse_struct(Eo_Lexer *ls, const char *name, Efl_Bool is_extern,
             Efl_Bool is_beta, int line, int column, const char *freefunc,
             const char *cname)
{
...
static Eolian_Typedecl *
parse_enum(Eo_Lexer *ls, const char *name, Efl_Bool is_extern,
           Efl_Bool is_beta, int line, int column, const char *cname)
{
   int bline = ls->line_number, bcolumn = ls->column;
   Eolian_Typedecl *def = eo_lexer_typedecl_new(ls);
   def->is_extern = is_extern;
        ...
        if (eo_lexer_is_type_keyword(ls->t.kw))
          {
             ...
             if ((tpid >= KW_accessor && tpid <= KW_list) ||
                 (tpid >= KW_slice && tpid <= KW_rw_slice) || (tpid == KW_hash))
               {
                  ...
                  check_next(ls, '<');
                  if (tpid == KW_future)
                    def->base_type = eo_lexer_type_release(ls, parse_type_void(ls, EFL_TRUE, EFL_TRUE));
                  else
                    def->base_type = eo_lexer_type_release(ls, parse_type(ls, EFL_TRUE, EFL_TRUE));
...
tags_done:
   ...
   eo_lexer_context_pop(ls);
   check_next(ls, ':');
   def->base_type = eo_lexer_type_release(ls, parse_type(ls, EFL_FALSE, EFL_FALSE));
   check_next(ls, ';');
   FILL_DOC(ls, def, doc);
   eo_lexer_dtor_pop(ls);
   return def;
...
static void
parse_return(Eo_Lexer *ls, Eo_Ret_Def *ret, Efl_Bool allow_void,
             Efl_Bool allow_def, Efl_Bool is_funcptr)
{
   eo_lexer_get(ls);
   check_next(ls, ':');
   if (allow_void)
     ret->type = parse_type_void(ls, EFL_TRUE, EFL_TRUE);
   else
     ret->type = parse_type(ls, EFL_TRUE, EFL_TRUE);
   ret->doc = NULL;
...
static void
parse_param(Eo_Lexer *ls, Eina_List **params, Efl_Bool allow_inout,
            Efl_Bool is_vals, const Eolian_Function *func)
{
   Efl_Bool has_optional = EFL_FALSE,
             has_move     = EFL_FALSE,
             has_by_ref   = EFL_FALSE;
       ...
       if (par->param_dir == EOLIAN_PARAMETER_OUT || par->param_dir == EOLIAN_PARAMETER_INOUT)
         {
            ...
            par->type = eo_lexer_type_release(ls, parse_type_void(ls, EFL_TRUE, EFL_TRUE));
            goto type_done;
         }
     }
...
static void
parse_params(Eo_Lexer *ls, Eina_List **params, Efl_Bool allow_inout,
             Efl_Bool is_vals, const Eolian_Function *func)
{
   int line, col;
   eo_lexer_get(ls);
...
parse_property(Eo_Lexer *ls)
{
   ...
   Efl_Bool has_get       = EFL_FALSE, has_set    = EFL_FALSE,
             has_keys      = EFL_FALSE, has_values = EFL_FALSE,
             has_protected = EFL_FALSE, has_class  = EFL_FALSE,
             has_beta      = EFL_FALSE, has_virtp  = EFL_FALSE;
...
parse_function_pointer(Eo_Lexer *ls)
{
   ...
   Eina_Stringshare *cname = NULL;
   Efl_Bool has_params = EFL_FALSE,
             has_return = EFL_FALSE,
...
parse_method(Eo_Lexer *ls)
{
   ...
   Efl_Bool has_const       = EFL_FALSE, has_params = EFL_FALSE,
             ...
             has_class       = EFL_FALSE, has_beta   = EFL_FALSE,
             has_virtp       = EFL_FALSE;
   meth = calloc(1, sizeof(Eolian_Function));
   meth->klass = ls->klass;
...
propend:
     ...
     {
        if (glob_auto)
          impl->get_auto = impl->set_auto = EFL_TRUE;
        if (glob_empty)
          impl->get_empty = impl->set_empty = EFL_TRUE;
        check_next(ls, ';');
        FILL_DOC(ls, impl, common_doc);
     }
   if (buf)
...
parse_event(Eo_Lexer *ls)
{
   ...
   eo_lexer_dtor_pop(ls);
   Efl_Bool has_scope = EFL_FALSE, has_beta = EFL_FALSE,
             has_hot   = EFL_FALSE, has_restart = EFL_FALSE;
   for (;;) switch (ls->t.kw)
     {
      case KW_at_private:
      case KW_at_protected:
...
{
   Efl_Bool has_c_prefix     = EFL_FALSE,
             has_event_c_prefix = EFL_FALSE,
             has_data          = EFL_FALSE,
             has_methods       = EFL_FALSE,
             has_parts         = EFL_FALSE,
             has_implements    = EFL_FALSE,
             has_constructors  = EFL_FALSE,
...
tags_done:
   ...
   Efl_Bool is_reg = (type == EOLIAN_CLASS_REGULAR) || (type == EOLIAN_CLASS_ABSTRACT);
   if (ls->t.token != '{')
     {
        line = ls->line_number;
        col = ls->column;
