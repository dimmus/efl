...
static Efl_Bool _validate_type(Validate_State *vals, Eolian_Type *tp,
                                Efl_Bool by_ref, Efl_Bool is_ret);
static Efl_Bool _validate_type_by_ref(Validate_State *vals, Eolian_Type *tp,
                                       Efl_Bool by_ref, Efl_Bool move,
                                       Efl_Bool is_ret);
...
static Efl_Bool
_validate_by_ref(Validate_State *vals, Eolian_Type *tp, Efl_Bool by_ref,
                 Efl_Bool move)
{
...
_validate_type_by_ref(Validate_State *vals, Eolian_Type *tp,
                      Efl_Bool by_ref, Efl_Bool move, Efl_Bool is_ret)
{
...
static Efl_Bool
_validate_type(Validate_State *vals, Eolian_Type *tp, Efl_Bool by_ref,
               Efl_Bool is_ret)
{
                ...
                while (itp)
                  {
                     ...
                     if (!_validate_type_by_ref(vals, itp, EFL_FALSE, itp->move, EFL_FALSE))
                       return EFL_FALSE;
                     itp = itp->next_type;
                  }
                return _validate(&tp->base);
      ...
      case EOLIAN_TYPE_ERROR:
        {
           ...
           if (tp->next_type && !_validate_type(vals, tp->next_type, EFL_FALSE, EFL_FALSE))
             return EFL_FALSE;
           return _validate(&tp->base);
        }
...
_validate_expr(Eolian_Expression *expr, const Eolian_Type *tp,
               Eolian_Expression_Mask msk, Efl_Bool by_ref)
{
   Eolian_Value val;
   if (by_ref)
...
{
   if (!_validate_type_by_ref(vals, param->type, param->by_ref, param->move, EFL_FALSE))
     return EFL_FALSE;
...
_validate_function(Validate_State *vals, Eolian_Function *func, Eina_Hash *nhash)
{
   ...
   if (func->get_ret_type && !_validate_type_by_ref(vals, func->get_ret_type,
       func->get_return_by_ref, func->get_return_move, EFL_TRUE))
     return _reset_stable(vals, was_stable, EFL_FALSE);

   if (func->set_ret_type && !_validate_type_by_ref(vals, func->set_ret_type,
       func->set_return_by_ref, func->set_return_move, EFL_TRUE))
     return _reset_stable(vals, was_stable, EFL_FALSE);
...
static void
_db_fill_callables(Eolian_Class *cl, Eolian_Class *icl, Eina_Hash *fs, Efl_Bool parent)
{
   Eina_List *l;
...
static Efl_Bool
_db_swap_inherit(Eolian_Class *cl, Efl_Bool succ, Eina_Stringshare *in_cl,
                 Eolian_Class **out_cl, Efl_Bool iface_only)
{
   if (!succ)
     {
                  ...
                  Eina_Hash *errh)
{
    ...
    */
   if (cl->parent)
     _db_fill_callables(cl, cl->parent, fh, EFL_TRUE);
