...
typedef struct _Validate_State
{
    ...
    Efl_Bool    stable;
    Efl_Bool    in_tree;
    Efl_Bool    unimplemented_beta;
    const char *since_ver;
} Validate_State;

static Efl_Bool
_set_stable(Validate_State *vals, Efl_Bool newval)
{
    Efl_Bool ret = vals->stable;
    vals->stable = newval;
    return ret;
}

static Efl_Bool
_reset_stable(Validate_State *vals, Efl_Bool oldval, Efl_Bool ret)
{
    vals->stable = oldval;
    return ret;
}
    ...
    EINA_LIST_FREE(pl, par)
    {
        ...
        while (ret && (doc = eolian_documentation_tokenize(doc, &tok)))
        {
            if (eolian_doc_token_type_get(&tok) == EOLIAN_DOC_TOKEN_REF)
            {
                ...
                Eolian_Object_Type tp = database_doc_token_ref_resolve(
                    &tok,
                ...
                if (tp == EOLIAN_OBJECT_UNKNOWN)
                {
                    ...
                    eolian_state_log_obj(info->unit->state,
                                         &tmp,
...
{
    if (!doc || !vals->stable) return EFL_TRUE;
    ...
    if (doc->since)
    {
        ...
        if (vals->in_tree)
        {
            ...
            if (min < 22)
            {
                _eo_parser_log(&doc->base,
                               "stable APIs must be 1.22 or higher");
                return EFL_FALSE;
            }
        }
...
{
    if (!doc || !doc->since) return EFL_TRUE;

    const char *old_since = vals->since_ver;
    Efl_Bool    ret       = _validate_doc_since(vals, doc);
...
{
    if (!doc) return EFL_TRUE;

    Eina_List *rdbg = doc->ref_dbg;

    if (!_validate_docstr(doc->summary, &doc->base, &rdbg)) return EFL_FALSE;
...
static Efl_Bool _validate_type(Validate_State *vals,
                               Eolian_Type    *tp,
                               Efl_Bool        by_ref,
                               Efl_Bool        is_ret);
static Efl_Bool _validate_type_by_ref(Validate_State *vals,
                                      Eolian_Type    *tp,
                                      Efl_Bool        by_ref,
                                      Efl_Bool        move,
                                      Efl_Bool        is_ret);
static Efl_Bool _validate_expr(Eolian_Expression     *expr,
                               const Eolian_Type     *tp,
                               Eolian_Expression_Mask msk,
                               Efl_Bool               by_ref);
static Efl_Bool _validate_function(Validate_State  *vals,
...
static Efl_Bool
_sf_map_cb(const Eina_Hash *hash           EFL_UNUSED,
           const void *key                 EFL_UNUSED,
           const Eolian_Struct_Type_Field *sf,
           Cb_Ret                         *sc)
{
    sc->succ = _validate_type_by_ref(sc->vals,
                                     sf->type,
                                     sf->by_ref,
                                     sf->move,
                                     EFL_FALSE);

    if (!sc->succ) return EFL_FALSE;
...
static Efl_Bool
_ef_map_cb(const Eina_Hash *hash         EFL_UNUSED,
           const void *key               EFL_UNUSED,
           const Eolian_Enum_Type_Field *ef,
           Cb_Ret                       *sc)
{
    if (ef->value)
        sc->succ = _validate_expr(ef->value, NULL, EOLIAN_MASK_INT, EFL_FALSE);
    else sc->succ = EFL_TRUE;
...
{
    if (tp->base.validated) return EFL_TRUE;

    const char *old_since = vals->since_ver;
    vals->since_ver       = NULL;

   /* for the time being assume all typedecls are beta unless overridden */
    Efl_Bool was_stable = _set_stable(vals, !tp->base.is_beta);

    if (!_validate_doc(tp->doc)) return EFL_FALSE;

    if (!_validate_doc_since(vals, tp->doc)) return EFL_FALSE;

    switch (tp->type)
    {
        case EOLIAN_TYPEDECL_ALIAS:
            if (!_validate_type(vals, tp->base_type, EFL_FALSE, EFL_FALSE))
                return _reset_stable(vals, was_stable, EFL_FALSE);
            if (tp->base_type->ownable) tp->ownable = EFL_TRUE;
            _reset_stable(vals, was_stable, EFL_TRUE);
            vals->since_ver = old_since;
            return _validate(&tp->base);
        case EOLIAN_TYPEDECL_STRUCT:
            {
                Cb_Ret rt = { vals, EFL_TRUE };
                eina_hash_foreach(tp->fields,
                                  (Eina_Hash_Foreach)_sf_map_cb,
                                  &rt);
                if (!rt.succ) return _reset_stable(vals, was_stable, EFL_FALSE);
                _reset_stable(vals, was_stable, EFL_TRUE);
                vals->since_ver = old_since;
                return _validate(&tp->base);
            }
        ...
        case EOLIAN_TYPEDECL_ENUM:
            {
                if (vals->stable && tp->legacy)
                {
                    _eo_parser_log(&tp->base,
                                   "legacy field not allowed in stable enums");
                    return EFL_FALSE;
                }
                Cb_Ret rt = { vals, EFL_TRUE };
                eina_hash_foreach(tp->fields,
                                  (Eina_Hash_Foreach)_ef_map_cb,
                                  &rt);
                if (!rt.succ) return _reset_stable(vals, was_stable, EFL_FALSE);
                _reset_stable(vals, was_stable, EFL_TRUE);
                vals->since_ver = old_since;
...
static Efl_Bool
_validate_by_ref(Validate_State *vals,
                 Eolian_Type    *tp,
                 Efl_Bool        by_ref,
                 Efl_Bool        move)
{
    Efl_Bool maybe_ownable =
        database_type_is_ownable(tp->base.unit, tp, EFL_FALSE, NULL);
   ...
   /* futures can be whatever... */
    if (tp->btype == EOLIAN_TYPE_BUILTIN_FUTURE) return EFL_TRUE;
   ...
   /* not marked @move, or marked @by_ref; just validate */
    if (!move || by_ref) return EFL_TRUE;
...
static Efl_Bool
_validate_type_by_ref(Validate_State *vals,
                      Eolian_Type    *tp,
                      Efl_Bool        by_ref,
                      Efl_Bool        move,
                      Efl_Bool        is_ret)
{
    if (!_validate_type(vals, tp, by_ref, is_ret)) return EFL_FALSE;

    return _validate_by_ref(vals, tp, by_ref, move);
}

static Efl_Bool
_validate_type(Validate_State *vals,
               Eolian_Type    *tp,
               Efl_Bool        by_ref,
               Efl_Bool        is_ret)
{
    ...
    if (tp->is_ptr)
    {
        ...
        tp->is_ptr = EFL_FALSE;
        Efl_Bool still_ownable =
            database_type_is_ownable(src, tp, EFL_FALSE, NULL);
        tp->is_ptr = EFL_TRUE;
        if (still_ownable)
        {
    ...
    {
        if (database_type_is_ownable(src, tp, EFL_FALSE, NULL))
        {
            ...
            switch (kw)
            {
                ...
                case KW_stringshare:
                    _eo_parser_log(&tp->base,
                                   "spurious explicit const on string type");
                    return EFL_FALSE;
                default:
                    {
                        if (!is_ret) break;
                        ...
                        {
                            _eo_parser_log(&tp->base,
                                           "return const requires a C pointer");
                            return EFL_FALSE;
                        }
                    }
            }
    ...
    switch (tp->type)
    {
        ...
        case EOLIAN_TYPE_REGULAR:
            {
                if (tp->base_type)
                {
                    ...
                    if (kwid > KW_void) tp->ownable = EFL_TRUE;
                    if ((kwid == KW_hash || kwid == KW_list) && vals->stable)
                    {
                        _eo_parser_log(
                            &tp->base,
                            "hashes and lists not allowed in stable context");
                        return EFL_FALSE;
                    }
                    ...
                    while (itp)
                    {
                        if (vals->stable && itp->is_ptr)
                        {
                            _eo_parser_log(&itp->base,
                                           "pointer types not allowed in '%s' "
                                           "in stable context",
                                           tp->base.name);
                            return EFL_FALSE;
                        }
                        if (!_validate_type_by_ref(vals,
                                                   itp,
                                                   EFL_FALSE,
                                                   itp->move,
                                                   EFL_FALSE))
                ...
                if (id)
                {
                    if (!eo_lexer_is_type_keyword(id)) return EFL_FALSE;
                    switch (id)
                    {
                        case KW_mstring:
                        case KW_stringshare:
                ...
                else if (vals->stable && tp->tdecl->base.is_beta)
                {
                    ...
                    _eo_parser_log(
                        &tp->base,
                        "beta type declaration '%s' used in stable context",
                        tp->tdecl->base.name);
                    return EFL_FALSE;
                }
                if (!_validate_typedecl(vals, tp->tdecl)) return EFL_FALSE;
                if (tp->tdecl->ownable) tp->ownable = EFL_TRUE;
                tp->base.c_name = eina_stringshare_ref(tp->tdecl->base.c_name);
                return _validate(&tp->base);
            }
        case EOLIAN_TYPE_CLASS:
            {
                tp->klass = (Eolian_Class *)eolian_unit_class_by_name_get(
                    src,
                    tp->base.name);
                if (!tp->klass)
                {
                ...
                {
                    _eo_parser_log(&tp->base,
                                   "beta class '%s' used in stable context",
                                   tp->klass->base.name);
                    return EFL_FALSE;
                }
                tp->ownable     = EFL_TRUE;
        ...
        case EOLIAN_TYPE_ERROR:
            {
                tp->error = (Eolian_Error *)eolian_unit_error_by_name_get(
                    src,
                    tp->base.name);
                if (!tp->error)
                {
                ...
                {
                    _eo_parser_log(&tp->base,
                                   "beta error '%s' used in stable context",
                                   tp->error->base.name);
                    return EFL_FALSE;
                }
                tp->base.c_name = eina_stringshare_ref(tp->error->base.c_name);
                if (tp->next_type &&
                    !_validate_type(vals, tp->next_type, EFL_FALSE, EFL_FALSE))
                    return EFL_FALSE;
                return _validate(&tp->base);
            }
               ...
               Efl_Bool               by_ref)
{
    Eolian_Value val;
    if (by_ref)
        val = database_expr_eval(expr->base.unit,
                                 expr,
                                 EOLIAN_MASK_NULL,
                                 NULL,
                                 NULL);
    ...
    else val = database_expr_eval(expr->base.unit, expr, msk, NULL, NULL);

    if (val.type == EOLIAN_EXPR_UNKNOWN) return EFL_FALSE;
...
_validate_param(Validate_State *vals, Eolian_Function_Parameter *param)
{
    ...
    if (param->value &&
        !_validate_expr(param->value, param->type, 0, param->by_ref))
        return EFL_FALSE;

    if (!_validate_doc(param->doc)) return EFL_FALSE;

    return _validate(&param->base);
}

static Efl_Bool
_validate_function(Validate_State  *vals,
                   Eolian_Function *func,
                   Eina_Hash       *nhash)
{
    Eina_List                 *l;
    ...
    const Eolian_Object *oobj =
        nhash ? eina_hash_find(nhash, &func->base.name) : NULL;
    if (EINA_UNLIKELY(oobj && (oobj != &func->base)))
    {
        _eo_parser_log(
            &func->base,
            "%sfunction '%s' conflicts with another symbol (at %s:%d:%d)",
            func->base.is_beta ? "beta " : "",
            func->base.name,
            oobj->file,
            oobj->line,
            oobj->column);
    ...
    */
    if (func->base.validated)
    {
        ...
        if (!oobj && nhash) eina_hash_add(nhash, &func->base.name, &func->base);
        return EFL_TRUE;
    }

    /* need to preserve stable flag set from the class */
    Efl_Bool was_stable =
        _set_stable(vals, !func->base.is_beta && vals->stable);

    if (func->get_ret_type && !_validate_type_by_ref(vals,
                                                     func->get_ret_type,
                                                     func->get_return_by_ref,
                                                     func->get_return_move,
                                                     EFL_TRUE))
        return _reset_stable(vals, was_stable, EFL_FALSE);

    if (func->set_ret_type && !_validate_type_by_ref(vals,
                                                     func->set_ret_type,
                                                     func->set_return_by_ref,
                                                     func->set_return_move,
                                                     EFL_TRUE))
        return _reset_stable(vals, was_stable, EFL_FALSE);
...
_validate_part(Validate_State *vals, Eolian_Part *part, Eina_Hash *phash)
{
    ...
    {
        if (!oobj) eina_hash_add(phash, &part->base.name, &part->base);
        return EFL_TRUE;
    }

    Efl_Bool was_stable =
        _set_stable(vals, !part->base.is_beta && vals->stable);
    ...
    Eolian_Class *pcl =
        eina_hash_find(part->base.unit->classes, part->klass_name);
    if (!pcl)
    {
        _eo_parser_log(&part->base,
                       "unknown part class '%s' (incorrect case?)",
                       part->klass_name);
        return _reset_stable(vals, was_stable, EFL_FALSE);
    }
    ...
    {
        _eo_parser_log(&part->base,
                       "beta part class '%s' used in stable context",
                       pcl->base.name);
        return _reset_stable(vals, was_stable, EFL_FALSE);
    }
...
_validate_event(Validate_State *vals, Eolian_Event *event, Eina_Hash *nhash)
{
    ...
    Efl_Bool was_stable =
        _set_stable(vals, !event->base.is_beta && vals->stable);

    if (!_validate_type(vals, event->type, EFL_FALSE, EFL_FALSE))
        return _reset_stable(vals, was_stable, EFL_FALSE);
    ...
    */
    if (tp->type == EOLIAN_TYPE_REGULAR && vals->stable)
    {
         ...
         */
        if (database_type_is_ownable(tp->base.unit, tp, EFL_FALSE, NULL))
        {
            switch (kwid)
            {
                case KW_string:
                ...
                default:
                    ...
                    {
                        _eo_parser_log(&tp->base,
                                       "event container types must be const");
                        return _reset_stable(vals, was_stable, EFL_FALSE);
                    }
            }
        }
        ...
        {
            _eo_parser_log(&tp->base,
                           "sequence containers not allowed in events");
            return _reset_stable(vals, was_stable, EFL_FALSE);
        }
...
{
    if (!cl || !strcmp(cl->base.name, cln)) return cl;
    Eina_List    *l;
...
_db_fill_implement(Eolian_Class *cl, Eolian_Implement *impl)
{
    ...
    if (impl->is_prop_get && impl->is_prop_set) ftype = EOLIAN_PROPERTY;
    else if (impl->is_prop_get) ftype = EOLIAN_PROP_GET;
    else if (impl->is_prop_set) ftype = EOLIAN_PROP_SET;
    ...
    if (!ldot)
        return EFL_FALSE; /* unreachable in practice, for static analysis */
    ...
    *ldot              = '\0'; /* split between class name and func name */
    ...
    if (!tcl)
    {
        _eo_parser_log(
            &impl->base,
            "class '%s' not found within the inheritance tree of '%s'",
            clname,
            cl->base.name);
        return EFL_FALSE;
    }
    ...
    const Eolian_Function *fid =
        eolian_class_function_by_name_get(tcl, fnname, EOLIAN_UNRESOLVED);
    if (!fid)
    {
        _eo_parser_log(&impl->base,
                       "function '%s' not known in class '%s'",
                       fnname,
                       clname);
        return EFL_FALSE;
    }
    ...
    Efl_Bool auto_empty = (impl->get_auto || impl->get_empty);
    ...
    if (ftype == EOLIAN_PROPERTY)
    {
        ...
        {
            _eo_parser_log(&impl->base,
                           "function '%s' is not a complete property",
                           fnname);
            return EFL_FALSE;
        }
    ...
    else if (ftype == EOLIAN_PROP_SET)
    {
        ...
        {
            _eo_parser_log(&impl->base,
                           "function '%s' doesn't have a setter",
                           fnname);
            return EFL_FALSE;
        }
    ...
    else if (ftype == EOLIAN_PROP_GET)
    {
        ...
        {
            _eo_parser_log(&impl->base,
                           "function '%s' doesn't have a getter",
                           fnname);
            return EFL_FALSE;
        }
    }
...
static Efl_Bool
_extend_impl(Eina_Hash *fs, Eolian_Implement *impl, Efl_Bool as_iface)
{
    const Eolian_Function *fid = impl->foo_id;
    Impl_Status            st  = (Impl_Status)eina_hash_find(fs, &fid);
    if (st == IMPL_STATUS_FULL) return EFL_FALSE;
    if (!st) eina_hash_set(fs, &fid, (void *)IMPL_STATUS_NONE);
    if (as_iface || (impl->implklass->type == EOLIAN_CLASS_INTERFACE))
        return !st;
    /* impl covers entire declaration */
    if (fid->type == EOLIAN_METHOD ||
        ((st == IMPL_STATUS_GET || fid->type == EOLIAN_PROP_SET) &&
         impl->is_prop_set) ||
        ((st == IMPL_STATUS_SET || fid->type == EOLIAN_PROP_GET) &&
         impl->is_prop_get) ||
        (impl->is_prop_get && impl->is_prop_set))
    {
        ...
        if (impl->get_pure_virtual && impl->set_pure_virtual) return !st;
        ...
        {
            if (fid->type == EOLIAN_PROP_SET) return !st;
...
{
    if (icl->parent) _db_fill_ihash(icl->parent, icls);
    Eina_List    *l;
    Eolian_Class *sicl;
...
static void
_db_fill_callables(Eolian_Class *cl,
                   Eolian_Class *icl,
                   Eina_Hash    *fs,
                   Efl_Bool      parent)
{
    ...
    Efl_Bool          allow_impl = parent || (icl->type == EOLIAN_CLASS_MIXIN);
    EINA_LIST_FOREACH(icl->callables, l, impl)
    {
        ...
        if (_extend_impl(fs, impl, !allow_impl))
        {
            ...
            else cl->callables = eina_list_append(cl->callables, impl);
        }
    }
}

static Efl_Bool
_db_check_implemented(Validate_State *vals,
                      Eolian_Class   *cl,
                      Eina_Hash      *fs,
                      Eina_Hash      *cs,
                      Eina_Hash      *errh)
{
    if (cl->type != EOLIAN_CLASS_REGULAR) return EFL_TRUE;

    Efl_Bool succ = EFL_TRUE;

    /* class is beta and we didn't enable unimplemented checking for those */
    if (!vals->unimplemented_beta && cl->base.is_beta) return EFL_TRUE;
    ...
    EINA_LIST_FOREACH(cl->callables, l, impl)
    {
        ...
        if (!vals->unimplemented_beta && fid->base.is_beta) continue;
        /* not checking beta and the function's class is beta: skip */
        if (!vals->unimplemented_beta && fid->klass->base.is_beta) continue;
        Impl_Status st = (Impl_Status)eina_hash_find(fs, &fid);
         ...
         */
        if (eina_hash_find(cs, &fid->klass)) continue;
         ...
         */
        if (eina_hash_find(errh, &impl)) continue;
        switch (st)
        {
            case IMPL_STATUS_NONE:
                _eo_parser_log(&cl->base,
                               "unimplemented function '%s' (originally "
                               "defined at %s:%d:%d)",
                               fid->base.name,
                               fid->base.file,
                               fid->base.line,
            ...
            case IMPL_STATUS_SET:
                _eo_parser_log(&cl->base,
                               "partially implemented function '%s' "
            ...
            default:
                _eo_parser_log(&cl->base,
                               "internal error, unregistered function '%s' "
                               "(originally defined at %s:%d:%d)",
                               fid->base.name,
                               fid->base.file,
                               fid->base.line,
...
_db_fill_implements(Eolian_Class *cl, Eina_Hash *fs)
{
    ...
    {
        Efl_Bool prop = (impl->is_prop_get || impl->is_prop_set);
        if (eina_hash_find(prop ? pth : th, impl->base.name))
        {
            _eo_parser_log(&impl->base,
                           "duplicate implement '%s'",
                           impl->base.name);
            ret = EFL_FALSE;
            goto end;
        }
        if (impl->klass != cl)
        {
            ...
            if (eolian_function_is_constructor(impl->foo_id, impl->klass))
                database_function_constructor_add(
                    (Eolian_Function *)impl->foo_id,
                    cl);
        }
...
static Efl_Bool
_db_swap_inherit(Eolian_Class     *cl,
                 Efl_Bool          succ,
                 ...
                 Efl_Bool          iface_only)
{
    ...
    if (!icl)
    {
        succ = EFL_FALSE;
        _eo_parser_log(&cl->base,
                       "unknown inherit '%s' (incorrect case?)",
                       in_cl);
    }
    else if (iface_only && (icl->type != EOLIAN_CLASS_INTERFACE))
    {
        succ = EFL_FALSE;
        _eo_parser_log(&cl->base,
                       "non-interface class '%s' in composite list",
                       icl->base.name);
    }
    else *out_cl = icl;
    eina_stringshare_del(in_cl);
...
_add_composite(Eolian_Class *cl, const Eolian_Class *icl, Eina_Hash *ch)
{
    ...
    {
        if (eina_hash_find(ch, &ccl)) continue;
...
static void
_add_implicit_composite(Eolian_Class *icl, Eina_Hash *ch, Efl_Bool try_tree)
{
...
static Efl_Bool
_db_fill_inherits(Validate_State *vals,
                  Eolian_Class   *cl,
                  Eina_Hash      *fhash,
                  Eina_Hash      *errh)
{
    if (eina_hash_find(fhash, &cl->base.name)) return EFL_TRUE;
    ...
    {
        succ =
            _db_swap_inherit(cl, succ, cl->parent_name, &cl->parent, EFL_FALSE);
        if (succ)
        {
    ...
    EINA_LIST_FREE(il, inn)
    {
        ...
        succ = _db_swap_inherit(cl, succ, inn, &out_cl, EFL_FALSE);
        if (!succ) continue;
    ...
    if (succ && cl->type == EOLIAN_CLASS_MIXIN)
    {
        EINA_LIST_FREE(rl, inn)
        {
            Eolian_Class *out_cl = NULL;
            succ = _db_swap_inherit(cl, succ, inn, &out_cl, EFL_FALSE);
            if (succ && !(out_cl->type == EOLIAN_CLASS_REGULAR ||
                          out_cl->type == EOLIAN_CLASS_ABSTRACT))
            {
                _eo_parser_log(&cl->base,
                               "requires only allows regulars or abstracts");
                succ = EFL_FALSE;
            }
            if (succ)
            {
            ...
            cl->
                requires
            = eina_list_append(cl->requires, out_cl);
        }
    }
    ...
    {
        Eolian_Class *out_cl = NULL;
        succ = _db_swap_inherit(cl, succ, inn, &out_cl, EFL_TRUE);
        if (!succ) continue;
         ...
         */
        _add_implicit_composite(out_cl,
                                ch,
                                out_cl->type == EOLIAN_CLASS_INTERFACE);
    }
    ...
    */
    if (cl->parent) _add_composite(cl, cl->parent, ch);
    ...
    */
    ...
    if (ncomp > 0)
    {
        ...
        EINA_LIST_FOREACH(cl->composite, il, icl)
        {
            ...
            if (eina_hash_find(ih, &icl)) continue;
...
{
    if (impl->base.validated) return EFL_TRUE;

    if (!_validate_doc(impl->common_doc)) return EFL_FALSE;
    if (!_validate_doc(impl->get_doc)) return EFL_FALSE;
    if (!_validate_doc(impl->set_doc)) return EFL_FALSE;

    /* common doc inherits @since into get/set doc */
    const char *old_since = vals->since_ver;
    if (impl->common_doc && !_validate_doc_since(vals, impl->common_doc))
        return EFL_FALSE;
    if (!_validate_doc_since_reset(vals, impl->get_doc)) return EFL_FALSE;
    if (!_validate_doc_since_reset(vals, impl->set_doc)) return EFL_FALSE;
    vals->since_ver = old_since;
...
static Efl_Bool
_validate_class(Validate_State *vals,
                Eolian_Class   *cl,
                Eina_Hash      *nhash,
                Eina_Hash      *ehash,
                Eina_Hash      *phash,
                Eina_Hash      *chash)
{
    ...
    if (!cl)
        return EFL_FALSE; /* if this happens something is very wrong though */

    /* we've gone through this part */
    if (eina_hash_find(chash, &cl)) return EFL_TRUE;

    Efl_Bool valid = cl->base.validated;
        ...
        if (!valid) switch (cl->type)
            {
                ...
                case EOLIAN_CLASS_ABSTRACT:
                    if (cl->parent->type != EOLIAN_CLASS_REGULAR &&
                        cl->parent->type != EOLIAN_CLASS_ABSTRACT)
                    {
                        _eo_parser_log(&cl->base,
                                       "regular classes ('%s') cannot inherit "
                                       "from non-regular classes ('%s')",
                                       cl->base.name,
                                       cl->parent->base.name);
                        return EFL_FALSE;
                    }
        ...
        if (icl->type == EOLIAN_CLASS_MIXIN)
        {
            ...
            EINA_LIST_FOREACH(res, n, required_class)
            {
                if (!eina_list_data_find(required_classes, required_class))
                    required_classes =
                        eina_list_append(required_classes, required_class);
            }
        }
        if (!valid) switch (icl->type)
            {
                ...
                case EOLIAN_CLASS_ABSTRACT:
                    ...
                    {
                        _eo_parser_log(&cl->base,
                                       "regular classes ('%s') cannot appear "
                                       "in extensions list of '%s'",
                                       icl->base.name,
                                       cl->base.name);
                        vals->warned = EFL_TRUE;
                        break;
    ...
    if (cl->type == EOLIAN_CLASS_ABSTRACT || cl->type == EOLIAN_CLASS_REGULAR)
    {
        ...
        if (required_classes)
        {
            Efl_Strbuf   *classes = eina_strbuf_new();
            Eolian_Class *required_class;
            Eina_List    *n;
            ...
            _eo_parser_log(
                &cl->base,
                "required classes %sare not in the inherit chain of %s",
                eina_strbuf_string_get(classes),
                cl->base.name);
    ...
    if (!_validate_doc(cl->doc)) return EFL_FALSE;
...
{
    if (var->base.validated) return EFL_TRUE;

    const char *old_since = vals->since_ver;
    vals->since_ver       = NULL;

    Efl_Bool was_stable = _set_stable(vals, !var->base.is_beta && vals->stable);

    if (!_validate_doc_since(vals, var->doc)) return EFL_FALSE;

    if (!_validate_type(vals, var->base_type, EFL_FALSE, EFL_FALSE))
        return _reset_stable(vals, was_stable, EFL_FALSE);

    if (!_validate_expr(var->value, var->base_type, 0, EFL_FALSE))
        return _reset_stable(vals, was_stable, EFL_FALSE);
...
static Efl_Bool
_typedecl_map_cb(const Eina_Hash *hash EFL_UNUSED,
                 const void *key       EFL_UNUSED,
                 Eolian_Typedecl      *tp,
                 Cb_Ret               *sc)
{
...
static Efl_Bool
_constant_map_cb(const Eina_Hash *hash EFL_UNUSED,
                 const void *key       EFL_UNUSED,
                 Eolian_Constant      *var,
                 Cb_Ret               *sc)
{
...
database_validate(const Eolian_Unit *src)
{
    ...
    if (vals.warned) return EFL_FALSE;
