    ...
    *since = EFL_FALSE;
    for (;;)
       ...
       */
            ...
            case '@':
                if ((size_t)(ls->stream_end - ls->stream) >=
                        (sizeof("since")) &&
            ...
            case '/':
                {
                    ...
                    if (ls->current == '*')
                    {
                        ...
                        if (ls->current == '@')
                        {
                            eo_lexer_lex_error(
                                ls,
                                "old style documentation comment",
                                -1);
            ...
            default:
                {
                         ...
                         ls->current == '#' || ls->current == '_'))
                    {
                        ...
                        while (ls->current &&
                               (isalnum(ls->current) || ls->current == '_'));
                        str = eina_strbuf_string_get(ls->buff);
                        tok->kw =
                            (int)(uintptr_t)eina_hash_find(keyword_map, str);
                        ls->column   = col + 1;
