...
_list_sorted_insert_no_dup(Eina_List *l, Eina_Compare_Cb func, const void *data)
{
    ...
    if (!l) return eina_list_append(NULL, data);
    else lnear = eina_list_search_sorted_near_list(l, func, data, &cmp);

    if (cmp < 0) return eina_list_append_relative_list(l, data, lnear);
    else if (cmp > 0) return eina_list_prepend_relative_list(l, data, lnear);
    return l;
}

void
database_function_constructor_add(Eolian_Function    *func,
                                  const Eolian_Class *cls)
{
    func->ctor_of =
        _list_sorted_insert_no_dup(func->ctor_of,
...
{
    if (ftype == EOLIAN_UNRESOLVED) return EFL_TRUE;
