                          ...
                          Eolian_Function_Type   ftype)
{
    ...
    {
        case EOLIAN_METHOD:
            if (fid->type != EOLIAN_METHOD) return EOLIAN_SCOPE_UNKNOWN;
            return fid->get_scope;
        case EOLIAN_PROP_GET:
            if ((fid->type != EOLIAN_PROP_GET) &&
                (fid->type != EOLIAN_PROPERTY))
...
{
    if (foo_id->klass->c_prefix) return foo_id->klass->c_prefix;
                                ...
                                Eolian_Function_Type   ftype)
{
    ...
    tbuf[0] = '\0';
    const char *prefix =
        (ftype != EOLIAN_FUNCTION_POINTER) ? _get_c_prefix(foo_id, tbuf) : tbuf;
...
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, EFL_FALSE);
    return fid->is_static;
}

EOLIAN_API Efl_Bool
eolian_function_is_constructor(const Eolian_Function *fid,
                               const Eolian_Class    *klass)
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, EFL_FALSE);
    Eina_Stringshare *s = eina_stringshare_ref(klass->base.name);
    Efl_Bool          r = !!eina_list_search_sorted_list(fid->ctor_of,
...
eolian_property_keys_get(const Eolian_Function *fid, Eolian_Function_Type ftype)
{
    Eina_List *l = NULL;
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, NULL);
    if (ftype != EOLIAN_PROP_GET && ftype != EOLIAN_PROP_SET) return NULL;
...
eolian_function_return_default_value_get(const Eolian_Function *fid,
                                         Eolian_Function_Type   ftype)
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, NULL);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(ftype != EOLIAN_UNRESOLVED, NULL);
...
eolian_function_return_documentation_get(const Eolian_Function *fid,
                                         Eolian_Function_Type   ftype)
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, NULL);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(ftype != EOLIAN_UNRESOLVED, NULL);
...
eolian_function_return_allow_unused(const Eolian_Function *fid,
                                    Eolian_Function_Type   ftype)
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(fid, EFL_TRUE);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(ftype != EOLIAN_UNRESOLVED, EFL_TRUE);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(ftype != EOLIAN_PROPERTY, EFL_TRUE);
    switch (ftype)
    {
        case EOLIAN_METHOD:
        case EOLIAN_FUNCTION_POINTER:
            if (fid->type != ftype) return EFL_TRUE;
                                 ...
                                 Eolian_Function_Type   ftype)
{
    ...
    switch (ftype)
    {
        case EOLIAN_METHOD:
        case EOLIAN_FUNCTION_POINTER:
            if (fid->type != ftype) return EFL_FALSE;
