...
_eina_xattr_value_ls_fd_iterator_next(Eina_Xattr_Iterator *it, void **data)
{
    ...
    *data          = it->attr;
    ...
    if (it->attr->length > 0)
    {
        ...
        {
            it->attr->value       = tmp;
            it->attr->length      = fgetxattr(it->fd,
                                         it->attr->name,
                                         (void *)it->attr->value,
                                         it->attr->length);
...
_eina_xattr_value_ls_iterator_next(Eina_Xattr_Iterator *it, void **data)
{
    ...
    *data          = it->attr;
    ...
    if (it->attr->length > 0)
    {
        ...
        {
            it->attr->value       = tmp;
            it->attr->length      = getxattr(it->file,
                                        it->attr->name,
                                        (void *)it->attr->value,
                                        it->attr->length);
            tmp[it->attr->length] = '\0';
...
eina_xattr_value_fd_ls(int fd)
{
    ...
    it->iterator.next =
        FUNC_ITERATOR_NEXT(_eina_xattr_value_ls_fd_iterator_next);
...
eina_xattr_get(const char *file, const char *attribute, ssize_t *size)
{
   ...
   /* Size should be less than 2MB (already huge in my opinion) */
    if (!(*size > 0 && *size < 2 * 1024 * 1024)) goto on_error;

    ret = malloc(*size + 1);
    if (!ret) goto on_error;

    tmp = getxattr(file, attribute, ret, *size);
    if (tmp != *size) goto on_error;
    ret[tmp] = '\0';
...
EINA_API Efl_Bool
eina_xattr_set(const char      *file,
               ...
               Eina_Xattr_Flags flags)
{
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(attribute, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(data, EFL_FALSE);
    EINA_SAFETY_ON_TRUE_RETURN_VAL(!(length > 0 && length < 2 * 1024 * 1024),
                                   EFL_FALSE);

    switch (flags)
    {
        case EINA_XATTR_INSERT:
            iflags = 0;
            break;
        case EINA_XATTR_REPLACE:
            iflags = XATTR_REPLACE;
            break;
        case EINA_XATTR_CREATED:
            iflags = XATTR_CREATE;
            break;
        default:
            return EFL_FALSE;
    ...
    if (setxattr(file, attribute, data, length, iflags)) return EFL_FALSE;
    return EFL_TRUE;
#else
    return EFL_FALSE;
    (void)file;
                  ...
                  Eina_Xattr_Flags flags)
{
    ...
    if (fsetxattr(fd, attribute, data, length, iflags)) return EFL_FALSE;
    return EFL_TRUE;
#else
    return EFL_FALSE;
    (void)fd;
...
EINA_API Efl_Bool
eina_xattr_string_set(const char      *file,
                      const char      *attribute,
                      const char      *data,
                      Eina_Xattr_Flags flags)
{
...
EINA_API Efl_Bool
eina_xattr_double_set(const char      *file,
                      const char      *attribute,
                      double           value,
                      Eina_Xattr_Flags flags)
{
...
EINA_API Efl_Bool
eina_xattr_int_set(const char      *file,
                   const char      *attribute,
                   int              value,
                   Eina_Xattr_Flags flags)
{
...
eina_xattr_fd_copy(int src, int dst)
{
    ...
    if (length == 0) return EFL_TRUE;
    EINA_SAFETY_ON_TRUE_RETURN_VAL(length < 0, EFL_FALSE);
