...
#if defined(HAVE_SYS_AUXV_H) && defined(HAVE_ASM_HWCAP_H) && \
...
static inline void
_x86_cpuid(int op, int *a, int *b, int *c, int *d)
{
    __asm__ volatile(
...
static void
_x86_simd(Eina_Cpu_Features *features)
{
    int a, b, c, d;
    ...
    * 20 = SSE4.2
    */
    if ((d >> 23) & 1) *features |= EINA_CPU_MMX;

    if ((d >> 25) & 1) *features |= EINA_CPU_SSE;

    if ((d >> 26) & 1) *features |= EINA_CPU_SSE2;

    if (c & 1) *features |= EINA_CPU_SSE3;

    if ((c >> 9) & 1) *features |= EINA_CPU_SSSE3;

    if ((c >> 19) & 1) *features |= EINA_CPU_SSE41;

    if ((c >> 20) & 1) *features |= EINA_CPU_SSE42;
}
#endif
...
#if defined(HAVE_SYS_AUXV_H) && defined(HAVE_ASM_HWCAP_H) && \
    (defined(__POWERPC__) && defined(__VEC__)) && defined(__linux__)
static void
 ...
 * @return
 */
EINA_API Eina_Cpu_Features
eina_cpu_features_get(void)
{
...
#  elif defined(__linux__) || defined(__GLIBC__)
    ...
    {
        fprintf(stderr,
                "[Eina] could not get cpu affinity: %s\n",
                strerror(errno));
        return 1;
    }
...
#  warn "Falling back to a safe default page size (4K) !"
    ...
    {
        EINA_LOG_ERR(
            "system reported weird value for PAGESIZE, assuming 4096.");
        _page_size = 4096;
    }
}

EINA_API int
eina_cpu_page_size(void)
{
...
void
eina_cpu_count_internal(void)
{
    if (getenv("EINA_CPU_FAKE")) _cpu_count = atoi(getenv("EINA_CPU_FAKE"));
    else _cpu_count = _eina_cpu_count_internal();
}
