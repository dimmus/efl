        ...
        if (sscanf(buf, "%lx-%lx %4s", &ptr1, &ptr2, mode) == 3)
        {
            if (!strcmp(mode, "r-xp"))
            {
                if (((void *)ptr1 <= symbol) && (symbol < (void *)ptr2))
                {
                    ...
                    {
                        DBG("Found %p in /proc/self/maps but not a file (%s)",
                            symbol,
                            buf);
                        break;
                    }
   ...
   /* 3. argv0 no path - look in PATH */
    ...
    while ((p = strchr(cp, PSEP_C)))
    {
        ...
        if (realpath(buf2, buf))
        {
            ...
            DBG("No realpath for argv0=%s in %.*s",
                argv0,
                (unsigned int)len,
                cp);
        cp = p + 1;
              ...
              const char  *localedir)
{
...
#endif
    {
        ...
        ret += _get_env_var(&pfx->prefix_path_bin,
                            envprefix,
                            "BIN",
                            prefix,
                            bindir);
        ret += _get_env_var(&pfx->prefix_path_lib,
                            envprefix,
                            "LIB",
                            prefix,
                            libdir);
        ret += _get_env_var(&pfx->prefix_path_data,
                            envprefix,
                            "DATA",
                            prefix,
                            datadir);
