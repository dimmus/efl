 ...
 * @param[in] status EFL_TRUE if opcodes have been received from the daemon, EFL_FALSE otherwise.
 */
typedef void (*Eina_Debug_Opcode_Status_Cb)(void *data, Efl_Bool status);
 ...
 */
typedef Efl_Bool (*Eina_Debug_Timer_Cb)(void *);
 ...
 *
 * @param[in] is_master true if the application is a debugger. EFL_FALSE otherwise.
 *
 * @return The session on success or NULL otherwise.
 */
EINA_API Eina_Debug_Session *eina_debug_local_connect(Efl_Bool is_master);
