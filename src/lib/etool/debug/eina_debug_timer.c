...
static Efl_Bool  _thread_runs = EFL_FALSE;
...
eina_debug_timer_add(unsigned int        timeout_ms,
                     Eina_Debug_Timer_Cb cb,
                     void               *data)
{
...
#ifndef _WIN32
    if (pipe(pipeToThread) == -1) return EFL_FALSE;
#endif
    return EFL_TRUE;
}
...
_eina_debug_timer_shutdown(void)
{
    ...
    close(pipeToThread[0]);
    close(pipeToThread[1]);
    if (_thread_runs) pthread_cancel(_thread);
    _thread_runs = 0;
    eina_spinlock_release(&_lock);
