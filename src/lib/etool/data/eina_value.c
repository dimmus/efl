                             ...
                             va_list                     args)
{
    unsigned short *tmem = mem;
    *tmem =
        va_arg(args, unsigned int); /* short is promoted to int for va_arg */
    return EFL_TRUE;
}
...
{
    struct _eina_value_type_hash_flush_each_ctx ctx = { tmem->subtype,
                                                        EFL_TRUE };
                           ...
                           void                       *dst)
{
    ...
    struct _eina_value_type_hash_copy_each_ctx ctx = { s->subtype,
                                                       d,
                                                       EFL_TRUE };
                                 ...
                                 void                       *convert_mem)
{
    ...
    else if ((tmem->hash) && (eina_hash_population(tmem->hash) == 1))
    {
        ...
        else
        {
            ...
            if ((!ret) && (convert->convert_from))
                ret =
                    convert->convert_from(convert, subtype, convert_mem, imem);
        }
                               ...
                               void                  *convert_mem)
{
        ...
        convert == EINA_VALUE_TYPE_STRING)
    {
        ...
        return _eina_value_type_timeval_convert_to(type,
                                                   convert,
                                                   &v,
                                                   convert_mem);
}
...
{
    const Eina_Value_Struct_Operations *ops =
        _eina_value_type_struct_ops_get(a);
        ...
        convert == EINA_VALUE_TYPE_STRING)
    {
        ...
        else
        {
            ...
            if (tmem->desc->member_count > 0)
            {
                ...
                itr_end = itr + tmem->desc->member_count;
                for (; itr < itr_end; itr++)
                    _eina_value_type_struct_convert_to_string_member(tmem,
                                                                     itr,
                                                                     str);
            }
            else
    ...
    if (desc->member_count > 0)
    {
        ...
        for (; itr < itr_end; itr++)
        {
            ...
            EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(itr->type),
                                            EFL_FALSE);
            EINA_SAFETY_ON_FALSE_RETURN_VAL(itr->type->value_size > 0,
                                            EFL_FALSE);
                               ...
                               void                       *dst_raw)
{
    ...
    else if (src_raw)
    {
        ...
        Eina_Value_Optional_Inner *dst = *(void **)dst_raw = malloc(
            sizeof(Eina_Value_Optional_Inner) + src->subtype->value_size);
        dst->subtype = src->subtype;
                                  ...
                                  const void                 *rhs_raw)
{
    if (sizeof(Eina_Value_Optional_Outer) <= sizeof(Eina_Value_Union))
    {
        ...
        if (!lhs->subtype) return !rhs->subtype ? 0 : -1;
            ...
            return eina_value_type_compare(lhs->subtype,
                                           lhs->value,
                                           rhs->value);
    }
    else
                                       ...
                                       void                  *convert_mem)
{
    ...
    return _eina_value_type_int_convert_to(type,
                                           convert,
                                           type_mem,
                                           convert_mem);
}
...
eina_value_convert(const Eina_Value *value, Eina_Value *convert)
{
    ...
    if ((!ret) && (convert_type->convert_from))
        ret = convert_type->convert_from(convert_type,
                                         type,
                                         convert_mem,
                                         type_mem);
