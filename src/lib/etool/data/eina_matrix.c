...
{
    if (!EINA_DBL_EQ(MATRIX_ZX(m), 0.0) || !EINA_DBL_EQ(MATRIX_ZY(m), 0.0) ||
        !EINA_DBL_EQ(MATRIX_ZZ(m), 1.0))
        return EINA_MATRIX_TYPE_PROJECTIVE;
    else
    {
        if (EINA_DBL_EQ(MATRIX_XX(m), 1.0) && EINA_DBL_EQ(MATRIX_XY(m), 0.0) &&
            EINA_DBL_EQ(MATRIX_XZ(m), 0.0) && EINA_DBL_EQ(MATRIX_YX(m), 0.0) &&
            EINA_DBL_EQ(MATRIX_YY(m), 1.0) && EINA_DBL_EQ(MATRIX_YZ(m), 0.0))
            return EINA_MATRIX_TYPE_IDENTITY;
        else return EINA_MATRIX_TYPE_AFFINE;
    }
}
...
{
    if (EINA_DBL_EQ(MATRIX_XX(m), 1.0) && EINA_DBL_EQ(MATRIX_XY(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_XZ(m), 0.0) && EINA_DBL_EQ(MATRIX_XW(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_YX(m), 0.0) && EINA_DBL_EQ(MATRIX_YY(m), 1.0) &&
        EINA_DBL_EQ(MATRIX_YZ(m), 0.0) && EINA_DBL_EQ(MATRIX_YW(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_ZX(m), 0.0) && EINA_DBL_EQ(MATRIX_ZY(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_ZZ(m), 1.0) && EINA_DBL_EQ(MATRIX_ZW(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_WX(m), 0.0) && EINA_DBL_EQ(MATRIX_WY(m), 0.0) &&
        EINA_DBL_EQ(MATRIX_WZ(m), 0.0) && EINA_DBL_EQ(MATRIX_WW(m), 1.0))
        return EINA_MATRIX_TYPE_IDENTITY;
...
eina_matrix3_f16p16_type_get(const Eina_Matrix3_F16p16 *m)
{
    ...
    {
        if ((MATRIX_XX(m) == 65536) && (MATRIX_XY(m) == 0) &&
            (MATRIX_XZ(m) == 0) && (MATRIX_YX(m) == 0) &&
            (MATRIX_YY(m) == 65536) && (MATRIX_YZ(m) == 0))
            return EINA_MATRIX_TYPE_IDENTITY;
        else return EINA_MATRIX_TYPE_AFFINE;
    }
}
...
EINA_API void
eina_matrix3_matrix3_f16p16_to(const Eina_Matrix3 *m, Eina_Matrix3_F16p16 *fm)
{
...
eina_matrix3_determinant(const Eina_Matrix3 *m)
{
    ...
    det = MATRIX_XX(m) *
          ((MATRIX_YY(m) * MATRIX_ZZ(m)) - (MATRIX_YZ(m) * MATRIX_ZY(m)));
    det -= MATRIX_XY(m) *
           ((MATRIX_YX(m) * MATRIX_ZZ(m)) - (MATRIX_YZ(m) * MATRIX_ZX(m)));
    det += MATRIX_XZ(m) *
           ((MATRIX_YX(m) * MATRIX_ZY(m)) - (MATRIX_YY(m) * MATRIX_ZX(m)));
                     ...
                     Eina_Matrix3       *dst)
{
    ...
    a11 = (MATRIX_XX(m1) * MATRIX_XX(m2)) + (MATRIX_XY(m1) * MATRIX_YX(m2)) +
          (MATRIX_XZ(m1) * MATRIX_ZX(m2));
    a12 = (MATRIX_XX(m1) * MATRIX_XY(m2)) + (MATRIX_XY(m1) * MATRIX_YY(m2)) +
          (MATRIX_XZ(m1) * MATRIX_ZY(m2));
    a13 = (MATRIX_XX(m1) * MATRIX_XZ(m2)) + (MATRIX_XY(m1) * MATRIX_YZ(m2)) +
          (MATRIX_XZ(m1) * MATRIX_ZZ(m2));

    a21 = (MATRIX_YX(m1) * MATRIX_XX(m2)) + (MATRIX_YY(m1) * MATRIX_YX(m2)) +
          (MATRIX_YZ(m1) * MATRIX_ZX(m2));
    a22 = (MATRIX_YX(m1) * MATRIX_XY(m2)) + (MATRIX_YY(m1) * MATRIX_YY(m2)) +
          (MATRIX_YZ(m1) * MATRIX_ZY(m2));
    a23 = (MATRIX_YX(m1) * MATRIX_XZ(m2)) + (MATRIX_YY(m1) * MATRIX_YZ(m2)) +
          (MATRIX_YZ(m1) * MATRIX_ZZ(m2));

    a31 = (MATRIX_ZX(m1) * MATRIX_XX(m2)) + (MATRIX_ZY(m1) * MATRIX_YX(m2)) +
          (MATRIX_ZZ(m1) * MATRIX_ZX(m2));
    a32 = (MATRIX_ZX(m1) * MATRIX_XY(m2)) + (MATRIX_ZY(m1) * MATRIX_YY(m2)) +
          (MATRIX_ZZ(m1) * MATRIX_ZY(m2));
    a33 = (MATRIX_ZX(m1) * MATRIX_XZ(m2)) + (MATRIX_ZY(m1) * MATRIX_YZ(m2)) +
          (MATRIX_ZZ(m1) * MATRIX_ZZ(m2));
...
{
    if (!EINA_DBL_EQ(m1->xx, m2->xx) || !EINA_DBL_EQ(m1->xy, m2->xy) ||
        !EINA_DBL_EQ(m1->xz, m2->xz) || !EINA_DBL_EQ(m1->yx, m2->yx) ||
        !EINA_DBL_EQ(m1->yy, m2->yy) || !EINA_DBL_EQ(m1->yz, m2->yz) ||
        !EINA_DBL_EQ(m1->zx, m2->zx) || !EINA_DBL_EQ(m1->zy, m2->zy) ||
...
eina_matrix3_square_quad_map(Eina_Matrix3 *m, const Eina_Quad *q)
{
   ...
   /* parallelogram */
    ...
    else
    {
        ...
        if (EINA_DBL_EQ(den, 0.0)) return EFL_FALSE;
...
EINA_API Efl_Bool
eina_matrix3_quad_square_map(Eina_Matrix3 *m, const Eina_Quad *q)
{
    Eina_Matrix3 tmp;

   /* compute square to quad */
    if (!eina_matrix3_square_quad_map(&tmp, q)) return EFL_FALSE;
                           ...
                           const Eina_Quad *dst)
{
   ...
   /* TODO check that both are actually quadrangles */
    if (!eina_matrix3_quad_square_map(m, src)) return EFL_FALSE;
    if (!eina_matrix3_square_quad_map(&tmp, dst)) return EFL_FALSE;
...
{
    return MATRIX_XW(m) * MATRIX_YZ(m) * MATRIX_ZY(m) * MATRIX_WX(m) -
...
eina_matrix4_normalized(Eina_Matrix4 *out, const Eina_Matrix4 *in)
{
    ...
    if (fabs(det) < DBL_EPSILON) return EFL_FALSE;
...
eina_matrix4_inverse(Eina_Matrix4 *out, const Eina_Matrix4 *in)
{
    ...
    MATRIX_XX(out) = MATRIX_YY(in) * MATRIX_ZZ(in) * MATRIX_WW(in) -
    ...
    MATRIX_YX(out) = -MATRIX_YX(in) * MATRIX_ZZ(in) * MATRIX_WW(in) +
    ...
    MATRIX_ZX(out) = MATRIX_YX(in) * MATRIX_ZY(in) * MATRIX_WW(in) -
    ...
    MATRIX_WX(out) = -MATRIX_YX(in) * MATRIX_ZY(in) * MATRIX_WZ(in) +
    ...
    MATRIX_XY(out) = -MATRIX_XY(in) * MATRIX_ZZ(in) * MATRIX_WW(in) +
    ...
    MATRIX_YY(out) = MATRIX_XX(in) * MATRIX_ZZ(in) * MATRIX_WW(in) -
    ...
    MATRIX_ZY(out) = -MATRIX_XX(in) * MATRIX_ZY(in) * MATRIX_WW(in) +
    ...
    MATRIX_WY(out) = MATRIX_XX(in) * MATRIX_ZY(in) * MATRIX_WZ(in) -
    ...
    MATRIX_XZ(out) = MATRIX_XY(in) * MATRIX_YZ(in) * MATRIX_WW(in) -
    ...
    MATRIX_YZ(out) = -MATRIX_XX(in) * MATRIX_YZ(in) * MATRIX_WW(in) +
    ...
    MATRIX_ZZ(out) = MATRIX_XX(in) * MATRIX_YY(in) * MATRIX_WW(in) -
    ...
    MATRIX_WZ(out) = -MATRIX_XX(in) * MATRIX_YY(in) * MATRIX_WZ(in) +
    ...
    MATRIX_XW(out) = -MATRIX_XY(in) * MATRIX_YZ(in) * MATRIX_ZW(in) +
    ...
    MATRIX_YW(out) = MATRIX_XX(in) * MATRIX_YZ(in) * MATRIX_ZW(in) -
    ...
    MATRIX_ZW(out) = -MATRIX_XX(in) * MATRIX_YY(in) * MATRIX_ZW(in) +
    ...
    MATRIX_WW(out) = MATRIX_XX(in) * MATRIX_YY(in) * MATRIX_ZZ(in) -
    ...
    det = MATRIX_XX(in) * MATRIX_XX(out) + MATRIX_XY(in) * MATRIX_YX(out) +
          MATRIX_XZ(in) * MATRIX_ZX(out) + MATRIX_XW(in) * MATRIX_WX(out);
                     ...
                     Eina_Matrix4       *out)
{
    ...
    xx = MATRIX_XX(mat_a) * MATRIX_XX(mat_b) +
         MATRIX_XY(mat_a) * MATRIX_YX(mat_b) +
         MATRIX_XZ(mat_a) * MATRIX_ZX(mat_b) +
         MATRIX_XW(mat_a) * MATRIX_WX(mat_b);
    xy = MATRIX_XX(mat_a) * MATRIX_XY(mat_b) +
         MATRIX_XY(mat_a) * MATRIX_YY(mat_b) +
         MATRIX_XZ(mat_a) * MATRIX_ZY(mat_b) +
         MATRIX_XW(mat_a) * MATRIX_WY(mat_b);
    xz = MATRIX_XX(mat_a) * MATRIX_XZ(mat_b) +
         MATRIX_XY(mat_a) * MATRIX_YZ(mat_b) +
         MATRIX_XZ(mat_a) * MATRIX_ZZ(mat_b) +
         MATRIX_XW(mat_a) * MATRIX_WZ(mat_b);
    xw = MATRIX_XX(mat_a) * MATRIX_XW(mat_b) +
         MATRIX_XY(mat_a) * MATRIX_YW(mat_b) +
         MATRIX_XZ(mat_a) * MATRIX_ZW(mat_b) +
         MATRIX_XW(mat_a) * MATRIX_WW(mat_b);

    yx = MATRIX_YX(mat_a) * MATRIX_XX(mat_b) +
         MATRIX_YY(mat_a) * MATRIX_YX(mat_b) +
         MATRIX_YZ(mat_a) * MATRIX_ZX(mat_b) +
         MATRIX_YW(mat_a) * MATRIX_WX(mat_b);
    yy = MATRIX_YX(mat_a) * MATRIX_XY(mat_b) +
         MATRIX_YY(mat_a) * MATRIX_YY(mat_b) +
         MATRIX_YZ(mat_a) * MATRIX_ZY(mat_b) +
         MATRIX_YW(mat_a) * MATRIX_WY(mat_b);
    yz = MATRIX_YX(mat_a) * MATRIX_XZ(mat_b) +
         MATRIX_YY(mat_a) * MATRIX_YZ(mat_b) +
         MATRIX_YZ(mat_a) * MATRIX_ZZ(mat_b) +
         MATRIX_YW(mat_a) * MATRIX_WZ(mat_b);
    yw = MATRIX_YX(mat_a) * MATRIX_XW(mat_b) +
         MATRIX_YY(mat_a) * MATRIX_YW(mat_b) +
         MATRIX_YZ(mat_a) * MATRIX_ZW(mat_b) +
         MATRIX_YW(mat_a) * MATRIX_WW(mat_b);

    zx = MATRIX_ZX(mat_a) * MATRIX_XX(mat_b) +
         MATRIX_ZY(mat_a) * MATRIX_YX(mat_b) +
         MATRIX_ZZ(mat_a) * MATRIX_ZX(mat_b) +
         MATRIX_ZW(mat_a) * MATRIX_WX(mat_b);
    zy = MATRIX_ZX(mat_a) * MATRIX_XY(mat_b) +
         MATRIX_ZY(mat_a) * MATRIX_YY(mat_b) +
         MATRIX_ZZ(mat_a) * MATRIX_ZY(mat_b) +
         MATRIX_ZW(mat_a) * MATRIX_WY(mat_b);
    zz = MATRIX_ZX(mat_a) * MATRIX_XZ(mat_b) +
         MATRIX_ZY(mat_a) * MATRIX_YZ(mat_b) +
         MATRIX_ZZ(mat_a) * MATRIX_ZZ(mat_b) +
         MATRIX_ZW(mat_a) * MATRIX_WZ(mat_b);
    zw = MATRIX_ZX(mat_a) * MATRIX_XW(mat_b) +
         MATRIX_ZY(mat_a) * MATRIX_YW(mat_b) +
         MATRIX_ZZ(mat_a) * MATRIX_ZW(mat_b) +
         MATRIX_ZW(mat_a) * MATRIX_WW(mat_b);

    wx = MATRIX_WX(mat_a) * MATRIX_XX(mat_b) +
         MATRIX_WY(mat_a) * MATRIX_YX(mat_b) +
         MATRIX_WZ(mat_a) * MATRIX_ZX(mat_b) +
         MATRIX_WW(mat_a) * MATRIX_WX(mat_b);
    wy = MATRIX_WX(mat_a) * MATRIX_XY(mat_b) +
         MATRIX_WY(mat_a) * MATRIX_YY(mat_b) +
         MATRIX_WZ(mat_a) * MATRIX_ZY(mat_b) +
         MATRIX_WW(mat_a) * MATRIX_WY(mat_b);
    wz = MATRIX_WX(mat_a) * MATRIX_XZ(mat_b) +
         MATRIX_WY(mat_a) * MATRIX_YZ(mat_b) +
         MATRIX_WZ(mat_a) * MATRIX_ZZ(mat_b) +
         MATRIX_WW(mat_a) * MATRIX_WZ(mat_b);
    ww = MATRIX_WX(mat_a) * MATRIX_XW(mat_b) +
         MATRIX_WY(mat_a) * MATRIX_YW(mat_b) +
         MATRIX_WZ(mat_a) * MATRIX_ZW(mat_b) +
         MATRIX_WW(mat_a) * MATRIX_WW(mat_b);
...
EINA_API void
eina_matrix3_multiply(Eina_Matrix3       *out,
...
EINA_API void
eina_matrix3_multiply_copy(Eina_Matrix3       *out,
                           const Eina_Matrix3 *mat_a,
                           const Eina_Matrix3 *mat_b)
{
    if (out != mat_a && out != mat_b)
...
EINA_API void
eina_matrix3_position_transform_set(Eina_Matrix3 *out,
                                    const double  p_x,
                                    const double  p_y)
{
...
EINA_API void
eina_matrix2_multiply(Eina_Matrix2       *out,
                      ...
                      const Eina_Matrix2 *mat_b)
{
    ...
    MATRIX_XX(out) = MATRIX_XX(mat_a) * MATRIX_XX(mat_b) +
                     MATRIX_YX(mat_a) * MATRIX_XY(mat_b);
    MATRIX_YX(out) = MATRIX_XX(mat_a) * MATRIX_YX(mat_b) +
                     MATRIX_YX(mat_a) * MATRIX_YY(mat_b);

    MATRIX_XY(out) = MATRIX_XY(mat_a) * MATRIX_XX(mat_b) +
                     MATRIX_YY(mat_a) * MATRIX_XY(mat_b);
    MATRIX_YY(out) = MATRIX_XY(mat_a) * MATRIX_YX(mat_b) +
                     MATRIX_YY(mat_a) * MATRIX_YY(mat_b);
}

EINA_API void
eina_matrix2_multiply_copy(Eina_Matrix2       *out,
                           const Eina_Matrix2 *mat_a,
                           const Eina_Matrix2 *mat_b)
{
    if (out != mat_a && out != mat_b)
