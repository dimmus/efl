...
static Eina_List *
_eina_rectangle_merge_list(Eina_List             *empty,
                           Eina_Rectangle_Packing type,
                           Eina_Rectangle        *r)
{
...
start_again:
    ...
    EINA_LIST_FOREACH(empty, l, match)
    {
                 ...
                 (match->x + match->w == r->x || r->x + r->w == match->x))
        {
            ...
            if (r1) empty = eina_list_append(empty, r1);
...
static Eina_List *
_eina_rectangle_empty_space_find(Eina_List             *empty,
                                 Eina_Rectangle_Packing type,
                                 int                    w,
                                 int                    h,
                                 int                   *x,
        ...
        if (r->w >= w && r->h >= h)
        {
            ...
            *y = r->y;
            ...
            else
            {
                ...
                if ((rw1 * r->h > h2 * r->w) ||
                    type == Eina_Packing_Bottom_Left ||
                    type == Eina_Packing_Bottom_Left_Skyline)
                {
                    rh1 = r->h;
                    w2  = w;
                }
            ...
            if (r)
            {
                empty = _eina_rectangle_merge_list(empty,
                                                   type,
                                                   r); /* Return empty */
            }
...
eina_rectangle_init(void)
{
    ...
    _eina_rectangle_log_dom =
        eina_log_domain_register("eina_rectangle", EINA_LOG_COLOR_DEFAULT);
    ...
    _eina_rectangle_mp =
        eina_mempool_add(choice, "rectangle", NULL, sizeof(Eina_Rectangle), 32);
...
eina_rectangle_pool_request(Eina_Rectangle_Pool *pool, int w, int h)
{
    ...
    if (!pool->sorted)
    {
        ...
        pool->sorted = EFL_TRUE;
    }

    if (pool->type == Eina_Packing_Bottom_Left_Skyline_Improved)
        pool->empty =
            _eina_rectangle_skyline_list_update_duplicate(pool->empty);
    pool->empty =
        _eina_rectangle_empty_space_find(pool->empty, pool->type, w, h, &x, &y);
    if (x == -1) return NULL;

    pool->sorted = EFL_FALSE;
...
eina_rectangle_pool_release(Eina_Rectangle *rect)
{
    ...
    if (r)
    {
        ...
        {
            era->pool->empty =
                _eina_rectangle_skyline_merge_list(era->pool->empty, r);
            era->pool->empty =
                _eina_rectangle_skyline_list_update_duplicate(era->pool->empty);
            EINA_INLIST_FOREACH(era->pool->head, new)
            {
                match = (Eina_Rectangle *)(new + 1);
                if (match)
                    era->pool->empty =
                        _eina_rectangle_skyline_list_update(era->pool->empty,
                                                            match);
            }
        }
        else
            era->pool->empty = _eina_rectangle_merge_list(era->pool->empty,
                                                          era->pool->type,
                                                          r);
        era->pool->sorted = EFL_FALSE;
    }
...
EINA_API void
eina_rectangle_pool_packing_set(Eina_Rectangle_Pool   *pool,
                                Eina_Rectangle_Packing type)
{
...
{
    if (!pool) return EFL_FALSE;

    EINA_MAGIC_CHECK_RECTANGLE_POOL(pool);
    EINA_SAFETY_ON_NULL_RETURN_VAL(pool, EFL_FALSE);
...
eina_rectangle_outside_position(Eina_Rectangle *rect1, Eina_Rectangle *rect2)
{
    ...
    if (rect1->y > rect2->y) ret |= EINA_RECTANGLE_OUTSIDE_TOP;
    if (rect1->x > rect2->x) ret |= EINA_RECTANGLE_OUTSIDE_LEFT;
