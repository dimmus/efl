...
eina_value_setup(Eina_Value *value, const Eina_Value_Type *type)
{
   ...
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   EINA_SAFETY_ON_FALSE_RETURN_VAL(type->value_size > 0, EFL_FALSE);
   ...
   if (type->value_size <= 8) mem = &value->value;
   else
     {
        ...
        EINA_SAFETY_ON_NULL_RETURN_VAL(mem, EFL_FALSE);
     }
...
eina_value_vset(Eina_Value *value, va_list args)
{
   ...
   EINA_VALUE_TYPE_CHECK_RETURN_VAL(value, EFL_FALSE);
   ...
   if (type == EINA_VALUE_TYPE_UCHAR)
     ...
     {
        const char *str = (const char *) va_arg(args, const char *);
        if (value->value.ptr == str) return EFL_TRUE;
        if (!str)
          {
             free(value->value.ptr);
             value->value.ptr = NULL;
...
eina_value_pset(Eina_Value *value, const void *ptr)
{
   ...
   EINA_VALUE_TYPE_CHECK_RETURN_VAL(value, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(ptr, EFL_FALSE);
...
eina_value_array_value_get(const Eina_Value *src, unsigned int position, Eina_Value *dst)
{
   ...
   EINA_VALUE_TYPE_ARRAY_CHECK_RETURN_VAL(src, EFL_FALSE);
...
eina_value_list_vinsert(Eina_Value *value, unsigned int position, va_list args)
{
   ...
   if (!desc->list && position == 0)
   ...
   else
     {
        ...
        EINA_SAFETY_ON_FALSE_RETURN_VAL(rel, EFL_FALSE);
...
eina_value_hash_del(Eina_Value *value, const char *key)
{
   ...
   EINA_VALUE_TYPE_HASH_CHECK_RETURN_VAL(value, EFL_FALSE);
...
eina_value_struct_vset(Eina_Value *value, const char *name, va_list args)
{
   ...
   EINA_VALUE_TYPE_STRUCT_CHECK_RETURN_VAL(value, EFL_FALSE);
...
eina_value_struct_value_get(const Eina_Value *src, const char *name, Eina_Value *dst)
{
   ...
   EINA_VALUE_TYPE_STRUCT_CHECK_RETURN_VAL(src, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(name, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(dst, EFL_FALSE);
...
eina_value_struct_value_set(Eina_Value *dst, const char *name, const Eina_Value *src)
{
   ...
   EINA_VALUE_TYPE_STRUCT_CHECK_RETURN_VAL(dst, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(name, EFL_FALSE);
   ...
   if (!member)
     return EFL_FALSE;
   EINA_SAFETY_ON_FALSE_RETURN_VAL(src->type == member->type, EFL_FALSE);
...
eina_value_struct_member_value_get(const Eina_Value *src, const Eina_Value_Struct_Member *member, Eina_Value *dst)
{
   ...
   EINA_VALUE_TYPE_STRUCT_CHECK_RETURN_VAL(src, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(member, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(dst, EFL_FALSE);
   st = (const Eina_Value_Struct *)eina_value_memory_get(src);
   if (!st)
     return EFL_FALSE;
...
static inline Efl_Bool
eina_value_optional_empty_is(const Eina_Value *value, Efl_Bool *is_empty)
{
   void *mem;

   EINA_VALUE_TYPE_OPTIONAL_CHECK_RETURN_VAL(value, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(is_empty, EFL_FALSE);
...
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   if (!type->setup) return EFL_FALSE;
...
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   if (!type->flush) return EFL_FALSE;
...
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   if (!type->compare) return EFL_FALSE;
...
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   if (!type->convert_to) return EFL_FALSE;
...
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_type_check(type), EFL_FALSE);
   if (!type->convert_from) return EFL_FALSE;
