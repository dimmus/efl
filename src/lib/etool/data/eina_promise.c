...
#define _eina_promise_value_dbg(_msg, _p, _v) \
    __eina_promise_value_dbg(_msg, _p, _v, __LINE__, __func__)
...
{
    if (EINA_UNLIKELY(
            eina_log_domain_level_check(_promise_log_dom, EINA_LOG_LEVEL_DBG)))
    {
        if (!v.type)
        {
            eina_log_print(_promise_log_dom,
                           EINA_LOG_LEVEL_DBG,
                           __FILE__,
                           fname,
                           line,
...
static Efl_Bool
_promise_convert_to(const Eina_Value_Type *type EFL_UNUSED,
...
_eina_future_cb_dispatch(Eina_Future *f, const Eina_Value value)
{
   ...
   // Cleaning up prev and past to avoid recursion
    ...
    {
        if (!value.type)
            DBG("Distach cb: %p, data: %p with no value", cb, f->data);
        else
        {
            ...
            DBG("Dispatch cb: %p, data: %p, value type: %s content: '%s'",
                cb,
                f->data,
                eina_value_type_name_get(eina_value_type_get(&value)),
                str);
            free(str);
        }
    }
...
static Eina_Value
_eina_future_dispatch_internal(Eina_Future **f, const Eina_Value value)
{
...
{
    if (v1.type != v2.type) return EFL_FALSE;
   //Both types are NULL at this point... so they are equal...
    if (!v1.type) return EFL_TRUE;
...
static void
_eina_future_dispatch(Eina_Future_Scheduler *scheduler,
                      Eina_Future           *f,
                      Eina_Value             value)
{
    ...
    if (!f)
    {
        ...
        {
            DBG("There are no more futures, but next_value is a promise "
                "setting p->future to NULL.");
...
_eina_future_cancel(Eina_Future *f, int err)
{
    ...
    {
        assert(f->promise ==
               NULL); /* intermediate futures shouldn't have a promise */
        assert(f->scheduled_entry ==
               NULL); /* intermediate futures shouldn't have pending dispatch */
    }
...
static void
_eina_future_schedule(Eina_Promise *p, Eina_Future *f, Eina_Value value)
{
    if (!p->scheduler)
    {
        ERR("Trying to resolve a continue promise during a future callback. "
            "Directly return Eina_Value instead.");
        goto err;
    }
    f->scheduled_entry =
        p->scheduler->schedule(p->scheduler, _scheduled_entry_cb, f, value);
...
static void
_eina_promise_deliver(Eina_Promise *p, Eina_Value value)
{
    if (p->future)
    {
...
eina_promise_init(void)
{
    ...
    _promise_log_dom =
        eina_log_domain_register("eina_promise", EINA_COLOR_CYAN);
    ...
    _promise_mp = eina_mempool_add(choice,
                                   "Eina_Promise",
                                   NULL,
                                   sizeof(Eina_Promise),
                                   512);
    ...
    _future_mp =
        eina_mempool_add(choice, "Eina_Future", NULL, sizeof(Eina_Future), 512);
...
_scheduler_get(Eina_Future *f)
{
    ...
    while (f);
    assert(EFL_FALSE && "no scheduler for future!");
    return NULL;
}
                 ...
                 const void            *data)
{
    ...
    DBG("Creating new promise - Promise:%p, cb: %p, data:%p",
        p,
        p->cancel,
        p->data);
    return p;
                          ...
                          const void            *data)
{
    ...
    p->data      = data;
    p->scheduler = NULL;
    DBG("Creating continuing new promise - Promise:%p, cb: %p, data:%p",
        p,
        p->cancel,
        p->data);
    return p;
...
eina_promise_reject(Eina_Promise *p, Eina_Error err)
{
    ...
    DBG("Reject promise %p - Error msg: '%s' - Error code: %d",
        p,
        eina_error_msg_get(err),
        err);
    _eina_promise_deliver(p, value);
...
{
    static const Eina_Future_Desc desc = { .cb = NULL, .data = NULL };
...
eina_future_cb_console_from_desc(const Eina_Future_Cb_Console_Desc desc)
{
    ...
    Eina_Future_Desc fdesc = { .cb = _eina_future_cb_console, .data = NULL };
...
static Eina_Value
_eina_future_cb_convert_to(void                          *data,
...
static Eina_Value
_eina_future_cb_easy(void              *data,
                     const Eina_Value   value,
                     const Eina_Future *dead_future)
{
    ...
    if (!d)
    {
        if (eina_value_setup(&ret, EINA_VALUE_TYPE_ERROR))
...
static Eina_Value
_race_then_cb(void *data, const Eina_Value v, const Eina_Future *dead_ptr)
{
    Race_Promise_Ctx *ctx = data;
...
static Eina_Value
_all_then_cb(void *data, const Eina_Value v, const Eina_Future *dead_ptr)
{
    All_Promise_Ctx *ctx = data;
    unsigned int     i   = 0;
    Efl_Bool         found;
    ...
    if (ctx->processed == ctx->base.futures_len)
    {
        ...
        ctx->values =
            EINA_VALUE_EMPTY; /* flushed in _eina_promise_clean_dispatch() */
        if (!ctx->base.cancelled) _all_promise_ctx_free(ctx);
    }
    return v;
}
...
_future_array_cancel(Eina_Future *array[])
{
    size_t i;
    for (i = 0; array[i]; i++)
        _eina_future_cancel(array[i], ENOMEM);
}
                                     ...
                                     Eina_Future_Cb         future_cb)
{
    ...
    for (i = 0; array[i] != EINA_FUTURE_SENTINEL; i++)
        ...
        EINA_SAFETY_ON_NULL_RETURN_VAL(array[i], EFL_FALSE);

    EINA_SAFETY_ON_FALSE_RETURN_VAL(i > 0, EFL_FALSE);

    ctx->promise = eina_promise_new(_scheduler_get(array[0]), cancel_cb, ctx);
    EINA_SAFETY_ON_NULL_RETURN_VAL(ctx->promise, EFL_FALSE);
...
err_then:
    ...
    while (i >= 1)
        _eina_future_free(ctx->futures[--i]);
    free(ctx->futures);
    ctx->futures = NULL;
err_futures:
    ctx->futures_len = 0;
...
eina_promise_all_iterator(Eina_Iterator *it)
{
    ...
    ctx->base.promise =
        eina_promise_new(_scheduler_get(f), _all_promise_cancel, ctx);
    ...
    EINA_ITERATOR_FOREACH(it, f)
    {
        ...
        tmp = realloc(ctx->base.futures,
                      ctx->base.futures_len * sizeof(Eina_Future *));
...
err_futures:
    while (i >= 1)
        _eina_future_free(ctx->base.futures[--i]);
...
eina_promise_all_array(Eina_Future *array[])
{
    ...
    r = eina_value_array_setup(&ctx->values, EINA_VALUE_TYPE_VALUE, 0);
    EINA_SAFETY_ON_FALSE_GOTO(r, err_array);
    r = promise_proxy_of_future_array_create(array,
                                             &ctx->base,
                                             _all_promise_cancel,
                                             _all_then_cb);
...
static Eina_Value
_eina_future_cb_ignore_error(void                          *data,
                             const Eina_Value               value,
                             const Eina_Future *dead_future EFL_UNUSED)
{
    ...
    if (value.type == EINA_VALUE_TYPE_ERROR)
    {
        ...
        if (!eina_value_get(&value, &err)) return EINA_VALUE_EMPTY;
