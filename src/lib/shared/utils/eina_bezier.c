                            ...
                            double            *py)
{
    ...
    *px = 3 * (a * bz->start.x + b * bz->ctrl_start.x + c * bz->ctrl_end.x +
               d * bz->end.x);
    *py = 3 * (a * bz->start.y + b * bz->ctrl_start.y + c * bz->ctrl_end.y +
               d * bz->end.y);
}
                   ...
                   Eina_Bezier       *second)
{
    ...
    first->end.x         = second->start.x =
        (first->ctrl_end.x + second->ctrl_start.x) * 0.5;
    ...
    first->end.y         = second->start.y =
        (first->ctrl_end.y + second->ctrl_start.y) * 0.5;
}
...
_eina_bezier_length_helper(const Eina_Bezier *b)
{
    ...
    len =
        len +
        _line_length(b->start.x, b->start.y, b->ctrl_start.x, b->ctrl_start.y);
    len = len + _line_length(b->ctrl_start.x,
                             b->ctrl_start.y,
                             b->ctrl_end.x,
                             b->ctrl_end.y);
    ...
    if (!EINA_FLT_EQ(len, chord))
    {
        ...
        length = _eina_bezier_length_helper(&left) + /* try left side */
...
_eina_bezier_split_left(Eina_Bezier *b, double t, Eina_Bezier *left)
{
    ...
    left->ctrl_end.x =
        b->ctrl_start.x +
        t * (b->ctrl_end.x - b->ctrl_start.x);  // temporary holding spot
    left->ctrl_end.y =
        b->ctrl_start.y +
        t * (b->ctrl_end.y - b->ctrl_start.y);  // temporary holding spot
    ...
    left->ctrl_end.x =
        left->ctrl_start.x + t * (left->ctrl_end.x - left->ctrl_start.x);
    left->ctrl_end.y =
        left->ctrl_start.y + t * (left->ctrl_end.y - left->ctrl_start.y);

    left->end.x = b->start.x =
        left->ctrl_end.x + t * (b->ctrl_start.x - left->ctrl_end.x);
    left->end.y = b->start.y =
        left->ctrl_end.y + t * (b->ctrl_start.y - left->ctrl_end.y);
}
...
EINA_API void
eina_bezier_split_at_length(const Eina_Bezier *b,
                            double             len,
                            Eina_Bezier       *left,
                            Eina_Bezier       *right)
{
...
EINA_API void
eina_bezier_bounds_get(const Eina_Bezier *b,
                       double            *x,
                       double            *y,
                       double            *w,
                       double            *h)
{
    ...
    if (b->ctrl_start.x < xmin) xmin = b->ctrl_start.x;
    else if (b->ctrl_start.x > xmax) xmax = b->ctrl_start.x;
    if (b->ctrl_end.x < xmin) xmin = b->ctrl_end.x;
    else if (b->ctrl_end.x > xmax) xmax = b->ctrl_end.x;
    if (b->end.x < xmin) xmin = b->end.x;
    else if (b->end.x > xmax) xmax = b->end.x;

    if (b->ctrl_start.y < ymin) ymin = b->ctrl_start.y;
    else if (b->ctrl_start.y > ymax) ymax = b->ctrl_start.y;
    if (b->ctrl_end.y < ymin) ymin = b->ctrl_end.y;
    else if (b->ctrl_end.y > ymax) ymax = b->ctrl_end.y;
    if (b->end.y < ymin) ymin = b->end.y;
    else if (b->end.y > ymax) ymax = b->end.y;
...
EINA_API void
eina_bezier_on_interval(Eina_Bezier *b,
                        double       t0,
                        double       t1,
                        Eina_Bezier *result)
{
    ...
    if (EINA_DBL_EQ(t0, 0.0) && EINA_DBL_EQ(t1, 1.0))
    {
        *result = *b;
        return;
    }
