...
#ifdef HAVE_DIRENT_H
static long
_eina_name_max(DIR *dirp EFL_UNUSED)
{
  long name_max;
...
#      ifdef _PC_NAME_MAX
#        warning \
          "buffer size for readdir_r cannot be determined safely, best effort, but racy"
...
#  else
    length = strlen(dp->d_name);
#  endif
    if (it->info.name_start + length + 1 >= EINA_PATH_MAX) continue;
  }
  while ((dp->d_name[0] == '.') &&
         ((dp->d_name[1] == '\0') ||
          ((dp->d_name[1] == '.') && (dp->d_name[2] == '\0'))));
...
_eina_file_stat_ls_iterator_next(Eina_File_Direct_Iterator *it, void **data)
{
  ...
  if (!_eina_file_direct_ls_iterator_next(it, data)) return EFL_FALSE;
...
#ifndef MAP_POPULATE
static unsigned int
_eina_file_map_populate(char *map, unsigned long int size, Efl_Bool hugetlb)
{
...
static char *
_page_aligned_address(const char       *map,
...
static int
_eina_file_map_rule_apply(Eina_File_Populate rule,
                          const void        *map,
                          unsigned long int  offset,
                          ...
                          Efl_Bool           hugetlb)
{
  ...
  switch (rule)
  {
    case EINA_FILE_RANDOM:
      flag = MADV_RANDOM;
      break;
    case EINA_FILE_SEQUENTIAL:
    ...
    case EINA_FILE_WILLNEED:
      flag = MADV_WILLNEED;
      break;
    case EINA_FILE_DONTNEED:
      flag = MADV_DONTNEED;
      break;
#ifdef MADV_REMOVE
    case EINA_FILE_REMOVE:
      flag = MADV_REMOVE;
      break;
...
{
  if (f->mtime != st->st_mtime) return EFL_FALSE;
  if (f->length != (unsigned long long)st->st_size) return EFL_FALSE;
  if (f->inode != st->st_ino) return EFL_FALSE;
#ifdef _STAT_VER_LINUX
#  if (defined __USE_MISC && defined st_mtime)
  if (f->mtime_nsec != (unsigned long int)st->st_mtim.tv_nsec) return EFL_FALSE;
#  else
  if (f->mtime_nsec != (unsigned long int)st->st_mtimensec) return EFL_FALSE;
#  endif
#endif
  return EFL_TRUE;
}
...
static Efl_Bool
_eina_file_mmap_faulty_one(void *addr, long page_size, Eina_File_Map *m)
{
                   ...
                   void                 *data)
{
  ...
  if (!it) return EFL_FALSE;

  EINA_ITERATOR_FOREACH(it, info)
  {
    cb(info->path + info->name_start, dir, data);

    if (recursive == EFL_TRUE && info->type == EINA_FILE_DIR)
    {
...
eina_file_split(char *path)
{
  ...
  for (current = strchr(path, PATH_DELIM); current;
...
eina_file_ls(const char *dir)
{
  ...
  if (dir[length - 1] != '/') it->length = length;
...
eina_file_direct_ls(const char *dir)
{
  ...
  if (dir[length - 1] == '/') it->info.name_start = length;
  else
  {
    it->info.path[length] = '/';
  ...
  it->iterator.get_container =
    FUNC_ITERATOR_GET_CONTAINER(_eina_file_direct_ls_iterator_container);
...
eina_file_open(const char *path, Efl_Bool shared)
{
  ...
  if ((!file) || (file->statgen != statgen) || (statgen == 0))
  {
    if (shared)
    {
      ...
      if ((fd != -1) && (!eina_file_close_on_exec(fd, EFL_TRUE))) goto on_error;
#else
      goto on_error;
#endif
    }
    ...
    if (fstat(fd, &file_stat)) goto on_error;
...
eina_file_refresh(Eina_File *file)
{
  ...
  if (fstat(file->fd, &file_stat)) return EFL_FALSE;
...
eina_file_map_all(Eina_File *file, Eina_File_Populate rule)
{
  ...
  eina_mmap_safety_enabled_set(EFL_TRUE);
  ...
  if (file->global_map != MAP_FAILED)
  {
    Efl_Bool hugetlb = EFL_FALSE;

#ifdef MAP_HUGETLB
    hugetlb = !!(flags & MAP_HUGETLB);
#endif
    if (!file->global_refcount) file->global_hugetlb = hugetlb;
    else hugetlb = file->global_hugetlb;

    _eina_file_map_rule_apply(rule,
                              file->global_map,
                              0,
                              file->length,
                              file->length,
...
EINA_API void *
eina_file_map_new(Eina_File         *file,
...
EINA_API void
eina_file_map_populate(Eina_File         *file,
                       Eina_File_Populate rule,
                       const void        *map,
                       unsigned long int  offset,
                       unsigned long int  length)
...
eina_file_statat(void *container, Eina_File_Direct_Info *info, Eina_Stat *st)
{
...
#endif
  {
    if (info->type != EINA_FILE_LNK) info->type = EINA_FILE_UNKNOWN;
    return -1;
  }
  ...
  if (info->type == EINA_FILE_UNKNOWN)
  {
    ...
    else if (S_ISCHR(buf.st_mode)) info->type = EINA_FILE_CHR;
    else if (S_ISBLK(buf.st_mode)) info->type = EINA_FILE_BLK;
    else if (S_ISFIFO(buf.st_mode)) info->type = EINA_FILE_FIFO;
    else if (S_ISLNK(buf.st_mode)) info->type = EINA_FILE_LNK;
    else if (S_ISSOCK(buf.st_mode)) info->type = EINA_FILE_SOCK;
