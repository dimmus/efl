...
static Efl_Bool           _eina_magic_strings_dirty     = 0;
...
_eina_magic_strings_alloc(void)
{
  ...
  if (_eina_magic_strings_count == _eina_magic_strings_allocated)
  {
    ...
    if (EINA_UNLIKELY(_eina_magic_strings_allocated == 0)) size = 48;
    else size = _eina_magic_strings_allocated + 16;

    tmp = realloc(_eina_magic_strings, sizeof(Eina_Magic_String) * size);
    if (!tmp)
    {
      ERR("could not realloc magic_strings from %zu to %zu buckets.",
          _eina_magic_strings_allocated,
          size);
      return NULL;
    }
...
{
  _eina_magic_string_log_dom =
    eina_log_domain_register("eina_magic_string", EINA_LOG_COLOR_DEFAULT);
  ...
  EINA_ERROR_MAGIC_FAILED =
    eina_error_msg_static_register("Magic check failed.");
...
eina_magic_string_shutdown(void)
{
  ...
  for (; ems < ems_end; ems++)
    if (ems->string_allocated) free((char *)ems->string);
...
eina_magic_string_get(Eina_Magic magic)
{
  ...
  ems = bsearch((void *)(size_t)magic,
                _eina_magic_strings,
                _eina_magic_strings_count,
                sizeof(Eina_Magic_String),
                _eina_magic_strings_find_cmp);
  if (ems) return ems->string ? ems->string : "(undefined)";
...
eina_magic_string_set(Eina_Magic magic, const char *magic_name)
{
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(magic_name, EFL_FALSE);

  ems = _eina_magic_strings_alloc();
  if (!ems) return EFL_FALSE;

  ems->magic            = magic;
  ems->string_allocated = EFL_TRUE;
...
eina_magic_string_static_set(Eina_Magic magic, const char *magic_name)
{
  ...
  ems->magic            = magic;
  ems->string_allocated = EFL_FALSE;
                ...
                int         line)
{
  if (!d)
    ...
    eina_log_print(EINA_LOG_DOMAIN_GLOBAL,
                   ...
                   d,
                   req_m,
                   eina_magic_string_get(req_m),
                   m,
                   eina_magic_string_get(m));
  else
    eina_log_print(EINA_LOG_DOMAIN_GLOBAL,
                   EINA_LOG_LEVEL_CRITICAL,
                   file,
