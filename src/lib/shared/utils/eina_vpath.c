...
_fallback_runtime_dir(const char *home)
{
  ...
  if (setuid(geteuid()) != 0)
  {
    fprintf(stderr, "FATAL: Cannot setuid - errno=%i\n", errno);
    abort();
  }
#endif
   // fallback - make ~/.run
    ...
    if (errno == EEXIST)
    {
      if (stat(buf, &st) == 0)
      {
        ...
        if (!(st.st_mode & S_IFDIR))
        {
          ...
          fprintf(stderr, "FATAL: run dir '%s' exists but not a dir\n", buf);
          abort();
        }
#if defined(HAVE_GETUID) && defined(HAVE_GETEUID)
  ...
  {
    fprintf(stderr, "FATAL: Cannot setreuid - errno=%i\n", errno);
    abort();
  }
#endif
...
_fallback_home_dir(void)
{
  ...
  if (mkdir(buf, S_IRUSR | S_IWUSR | S_IXUSR) < 0)
  {
    if (errno != EEXIST)
    {
      if (stat("/tmp", &st) == 0) snprintf(buf, sizeof(buf), "/tmp");
...
_eina_vpath_interface_sys_init(void)
{
  ...
  else _eina_vpath_data_add("home", home);
...
_eina_vpath_resolve(const char *path, char *str, size_t size)
{
  if (path[0] == '~')
  {
    ...
    else
    {
      ...
      if (!_fetch_user_homedir(&home, name, path)) return 0;
      path = p;
#endif
    }
    if (home)
...
eina_vpath_resolve(const char *path)
{
  ...
  if (_eina_vpath_resolve(path, buf, sizeof(buf)) > 0) return strdup(buf);
  return NULL;
}
...
eina_vpath_interface_app_set(const char *app_domain, Eina_Prefix *app_pfx)
{
  ...
  snprintf(buf,
           sizeof(buf),
           "%s/%s",
           _eina_vpath_data_get("usr.config"),
           app_domain);
  _eina_vpath_data_add("app.config", buf);
  snprintf(buf,
           sizeof(buf),
           "%s/%s",
           _eina_vpath_data_get("usr.cache"),
           app_domain);
  _eina_vpath_data_add("app.cache", buf);
  snprintf(buf,
           sizeof(buf),
           "%s/%s",
           _eina_vpath_data_get("usr.data"),
           app_domain);
  _eina_vpath_data_add("app.local", buf);
  snprintf(buf,
           sizeof(buf),
           "%s/%s",
           _eina_vpath_data_get("usr.tmp"),
           app_domain);
...
#undef ADD
  ...
  if (free_run) free((char *)user->run);
}
