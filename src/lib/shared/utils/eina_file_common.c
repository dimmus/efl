...
#  define EINA_FILE_MAGIC_CHECK(f, ...)                                 \
      do                                                                \
      {                                                                 \
...
int
eina_file_map_key_cmp(const unsigned long long int *key1,
                      int key1_length               EFL_UNUSED,
                      const unsigned long long int *key2,
                      int key2_length               EFL_UNUSED)
{
  if (key1[0] == key2[0])
  {
...
int
eina_file_map_key_hash(const unsigned long long int *key,
                       int key_length                EFL_UNUSED)
{
...
void
eina_file_common_map_free(Eina_File *file,
                          void      *map,
...
EINA_API Eina_File *
eina_file_virtualize(const char        *virtual_name,
                     const void        *data,
                     ...
                     Efl_Bool           copy)
{
  ...
  if (virtual_name) file->filename = eina_stringshare_add(virtual_name);
  else file->filename = eina_stringshare_printf(tmpname, ti);
...
{
  if (!file) return EFL_FALSE;
  EINA_FILE_MAGIC_CHECK(file, EFL_FALSE);
  return file->virtual;
}
...
eina_file_dup(const Eina_File *f)
{
  ...
  if (file)
  {
    ...
    if (file->virtual && !file->copied)
    {
      ...
      r = eina_file_virtualize(file->filename,
                               file->global_map,
                               file->length,
                               EFL_TRUE);
      eina_lock_release(&file->lock);
...
eina_file_close(Eina_File *file)
{
  ...
  eina_lock_take(&file->lock);
  file->refcount--;
  if (file->refcount == 0) leave = EFL_FALSE;
  eina_lock_release(&file->lock);
  if (leave) goto end;

  key = eina_hash_superfast((void *)&file->filename, sizeof(void *));
  if (eina_hash_find_by_hash(_eina_file_cache, file->filename, 0, key) == file)
  {
    eina_hash_del_by_key_hash(_eina_file_cache, file->filename, 0, key);
  }
...
_eina_file_map_lines_iterator_next(Eina_Lines_Iterator *it, void **data)
{
  ...
  if (it->current.end >= it->end) return EFL_FALSE;

  match = *it->current.end;
  if (it->current.index > 0) it->current.end++;
  while (it->current.end < it->end &&
         (*it->current.end == '\n' || *it->current.end == '\r'))
  {
    if (match == *it->current.end) break;
    it->current.end++;
  }
  it->current.index++;
  ...
  eol          = _eina_find_eol(it->current.end, it->boundary, it->end);
...
_eina_file_copy_write_internal(int fd, char *buf, size_t size)
{
  ...
  while (done < size)
  {
    ...
    {
      ERR("Error writing destination file during copy: %s", strerror(errno));
      return EFL_FALSE;
    }
  }
  return EFL_TRUE;
...
static Efl_Bool
_eina_file_copy_read_internal(int      fd,
                              char    *buf,
                              off_t    bufsize,
                              ssize_t *readsize)
{
  while (1)
  {
    ...
    else if (r < 0)
    {
      ...
      {
        ERR("Error reading source file during copy: %s", strerror(errno));
        return EFL_FALSE;
      }
    }
    else
...
static Efl_Bool
_eina_file_copy_read_splice_internal(int      fd,
                                     int      pipefd,
                                     off_t    bufsize,
                                     ssize_t *readsize)
{
  while (1)
  {
    ...
    else if (r < 0)
    {
      ...
      {
        ERR("Error splicing from source file during copy: %s", strerror(errno));
        return EFL_FALSE;
      }
    }
    else
...
static Efl_Bool
_eina_file_copy_splice_internal(int                     s,
                                int                     d,
                                off_t                   total,
                                Eina_File_Copy_Progress cb,
                                const void             *cb_data,
                                Efl_Bool               *splice_unsupported)
{
#ifdef HAVE_SPLICE
  ...
  *splice_unsupported = EFL_TRUE;

  if (pipe(pipefd) < 0) return EFL_FALSE;
  ...
  while (done < total)
  {
    ...
    if (done + bufsize < total) todo = bufsize;
    else todo = total - done;
...
static Efl_Bool
_eina_file_copy_internal(int                     s,
                         int                     d,
                         off_t                   total,
                         Eina_File_Copy_Progress cb,
                         const void             *cb_data)
{
  ...
  off_t    bufsize = COPY_BLOCKSIZE;
  off_t    done;
  Efl_Bool ret, splice_unsupported;

  ret = _eina_file_copy_splice_internal(s,
                                        d,
                                        total,
                                        cb,
                                        cb_data,
                                        &splice_unsupported);
  if (ret) return EFL_TRUE;
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(buf, EFL_FALSE);
               ...
               const void             *cb_data)
{
  ...
  EINA_SAFETY_ON_NULL_RETURN_VAL(dst, EFL_FALSE);

  s = open(src, O_RDONLY | O_BINARY);
  EINA_SAFETY_ON_TRUE_RETURN_VAL(s < 0, EFL_FALSE);
  ...
  {
#ifdef HAVE_FCHMOD
    if (flags & EINA_FILE_COPY_PERMISSION) fchmod(d, st.st_mode);
#endif
    if (flags & EINA_FILE_COPY_XATTR) eina_xattr_fd_copy(s, d);
  }

end:
  if (d >= 0) close(d);
  else success = EFL_FALSE;
  close(s);
...
{
  _eina_file_log_dom =
    eina_log_domain_register("eina_file", EINA_LOG_COLOR_DEFAULT);
...
EINA_API Efl_Bool
eina_file_close_on_exec(int fd, Efl_Bool on)
{
#ifdef _WIN32
  return EFL_TRUE;
  (void)fd;
  ...
  return EFL_TRUE;
#else
  static Efl_Bool statement = EFL_FALSE;
