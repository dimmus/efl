 ...
 * dlopen() and if #EFL_FALSE is returned, load will fail, #EFL_TRUE
 * means the module was successfully initialized.
 * @see Eina_Module_Shutdown
 */
typedef Efl_Bool (*Eina_Module_Init)(void);
 ...
 * be loaded, #EFL_FALSE is returned. If it is an internal Eina module and the
 * module cannot be initialized, #EFL_FALSE is returned. If the module has
 * already been loaded, its reference counter is increased by one and
 * #EFL_TRUE is returned. If @p module is @c NULL, the function returns
 * immediately #EFL_FALSE.
 *
 ...
 * strictly greater than @c 1, #EFL_FALSE is returned. Otherwise, the
 * shared object file is closed and if it is a internal Eina module, it
 * is shut down just before. In that case, #EFL_TRUE is
 ...
 * @p path. If @p recursive is #EFL_TRUE, then recursive search is
