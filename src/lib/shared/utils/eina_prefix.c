...
_path_join_multiple(char *buf, int bufsize, ...)
{
  ...
  while (used < bufsize - 1)
  {
    ...
    if (used > 0) buf[used] = EINA_PATH_SEP_C;
      ...
      if (!strcmp(mode, "r-xp"))
      {
        if (((void *)ptr1 <= symbol) && (symbol < (void *)ptr2))
        {
          ...
          if (p)
          {
            if (len > 10)
            {
              if (!strcmp(buf + len - 10, " (deleted)")) buf[len - 10] = 0;
            }
            STRDUP_REP(pfx->exe_path, p);
            INF("Found %p in /proc/self/maps: %s (%s)",
                symbol,
                pfx->exe_path,
                buf);
            fclose(f);
...
_try_argv(Eina_Prefix *pfx, const char *argv0)
{
  ...
  if (strchr(argv0, EINA_PATH_SEP_C))
  {
    if (getcwd(buf2, sizeof(buf2)))
    {
      ...
      else WRN("No realpath for argv0=%s, cwd=%s", argv0, buf2);
    }
    else WRN("Couldn't get current directory to lookup argv0=%s", argv0);
  }
  ...
  while ((p = strchr(cp, PSEP_C)))
  {
    ...
    if (realpath(buf2, buf))
    {
      ...
      else DBG("Path not executable %s", buf);
    }
    else DBG("No realpath for argv0=%s in %.*s", argv0, (unsigned int)len, cp);
    cp = p + 1;
  }
              ...
              const char  *localedir)
{
...
#endif
  {
    ...
    ret +=
      _get_env_var(&pfx->prefix_path_bin, envprefix, "BIN", prefix, bindir);
    ret +=
      _get_env_var(&pfx->prefix_path_lib, envprefix, "LIB", prefix, libdir);
    ret +=
      _get_env_var(&pfx->prefix_path_data, envprefix, "DATA", prefix, datadir);
    ret += _get_env_var(&pfx->prefix_path_locale,
                        envprefix,
                        "LOCALE",
                        prefix,
                        localedir);
...
static int
_common_prefix_find(const char *bin,
                    const char *lib,
                    const char *data,
                    const char *locale)
{
...
eina_prefix_new(const char *argv0,
                void       *symbol,
                const char *envprefix,
                const char *sharedir,
                ...
                const char *pkg_locale)
{
  ...
  Efl_Bool     from_lib = EFL_FALSE, from_bin = EFL_FALSE;
    ...
    if (dladdr(symbol, &info_dl))
    {
      if (info_dl.dli_fname)
      {
        if (_path_absolute_check(info_dl.dli_fname))
        {
          ...
          WRN("dladdr for symbol=%p: %s is relative",
              symbol,
              info_dl.dli_fname);
      }
      else WRN("no dladdr filename for symbol=%p", symbol);
    }
    else WRN("no dladdr for symbol=%p", symbol);
#endif

    if (!pfx->exe_path) _try_proc(pfx, symbol);
    ...
    while (p >= pfx->exe_path)
    {
      if (*p == EINA_PATH_SEP_C)
      {
        ...
        if (pfx->prefix_path)
        {
          ...
          eina_strlcpy(pfx->prefix_path, pfx->exe_path, p - pfx->exe_path);
          ...
          if ((!from_bin) && (stat(buf, &st) == 0)) checks_passed++;
          ...
          if ((!from_lib) && (stat(buf, &st) == 0)) checks_passed++;
          ...
          if (stat(buf, &st) == 0) checks_passed++;

          /* check if magic file is there - then our guess is right */
          if (!magic) DBG("No magic file");
          else
          {
            DBG("Magic = %s", magic);
            _path_join_multiple(buf,
                                sizeof(buf),
                                pfx->prefix_path,
                                datadir,
                                magic,
            ...
            {
              checks_passed++;
              magic_found = EFL_TRUE;
              DBG("Magic path %s stat passed", buf);
            }
            else WRN("Missing magic path %s", buf);
          }

          if (((!magic) && (checks_passed > 0)) || ((magic) && (magic_found)))
          {
