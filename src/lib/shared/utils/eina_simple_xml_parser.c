        ...
        if (itr[0] == '<')
        {
            ...
            else
            {
                ...
                else if (itr[1] == '!')
                {
                    if ((itr + sizeof("<!DOCTYPE>") - 1 < itr_end) &&
                        ...
                        ((itr[2 + sizeof("DOCTYPE") - 1] == '>') ||
                         (isspace(
                             (unsigned char)itr[2 + sizeof("DOCTYPE") - 1]))))
                    {
                ...
                if (type == EINA_SIMPLE_XML_CDATA)
                    p = _eina_simple_xml_tag_cdata_end_find(itr + 1 + toff,
                                                            itr_end);
                else if (type == EINA_SIMPLE_XML_DOCTYPE_CHILD)
                    p = _eina_simple_xml_tag_doctype_child_end_find(itr + 1 +
                                                                        toff,
                                                                    itr_end);
                else if (type == EINA_SIMPLE_XML_COMMENT)
                    p = _eina_simple_xml_tag_comment_end_find(itr + 1 + toff,
                                                              itr_end);
                ...
                if (p)
                {
                        ...
                        (type != EINA_SIMPLE_XML_CDATA))
                    {
                        start = _eina_simple_xml_whitespace_skip(start, end);
                            ...
                            _eina_simple_xml_whitespace_unskip(end, start + 1);
                    }
...
_eina_simple_xml_node_tag_free(Eina_Simple_XML_Node_Tag *tag)
{
    ...
    while (tag->attributes)
    {
        Eina_Simple_XML_Attribute *a =
            EINA_INLIST_CONTAINER_GET(tag->attributes,
                                      Eina_Simple_XML_Attribute);
                            ...
                            unsigned             length)
{
    ...
    switch (type)
    {
        ...
        case EINA_SIMPLE_XML_OPEN_EMPTY:
            {
                ...
                name_end =
                    _eina_simple_xml_whitespace_unskip(name_end, content);
        ...
        case EINA_SIMPLE_XML_CLOSE:
            ...
            {
                const char *end =
                    _eina_simple_xml_whitespace_unskip(content + length,
                                                       content);
                int len;
                len = end - content;
        ...
        case EINA_SIMPLE_XML_DATA:
            return !!eina_simple_xml_node_data_new(ctx->current,
                                                   content,
                                                   length);
        case EINA_SIMPLE_XML_CDATA:
            return !!eina_simple_xml_node_cdata_new(ctx->current,
                                                    content,
                                                    length);
        case EINA_SIMPLE_XML_PROCESSING:
            return !!eina_simple_xml_node_processing_new(ctx->current,
                                                         ...
                                                         length);
        case EINA_SIMPLE_XML_DOCTYPE:
            return !!eina_simple_xml_node_doctype_new(ctx->current,
                                                      content,
                                                      length);
        case EINA_SIMPLE_XML_DOCTYPE_CHILD:
            return !!eina_simple_xml_node_doctype_child_new(ctx->current,
                                                            ...
                                                            length);
        case EINA_SIMPLE_XML_COMMENT:
            return !!eina_simple_xml_node_comment_new(ctx->current,
                                                      content,
                                                      length);
                           ...
                           unsigned              level)
{
    switch (node->type)
    {
        ...
        case EINA_SIMPLE_XML_NODE_DATA:
            {
                Eina_Simple_XML_Node_Data *n =
                    (Eina_Simple_XML_Node_Data *)node;
