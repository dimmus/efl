...
static const char EINA_MAGIC_SIMPLE_XML_ATTRIBUTE_STR[] =
  "Eina Simple XML Attribute";
...
{
  Efl_Bool inside_quote = EFL_FALSE;
  for (; itr < itr_end; itr++)
  {
...
static inline const char *
_eina_simple_xml_tag_doctype_child_end_find(const char *itr,
                                            const char *itr_end)
{
  for (; itr < itr_end; itr++)
    if (*itr == '>') return itr;
...
eina_simple_xml_init(void)
{
  ...
  _eina_simple_xml_log_dom =
    eina_log_domain_register("eina_simple_xml", EINA_LOG_COLOR_DEFAULT);
  ...
  _eina_simple_xml_tag_mp = eina_mempool_add(choice,
                                             "simple_xml_tag",
                                             NULL,
                                             sizeof(Eina_Simple_XML_Node_Tag),
                                             32);
  ...
  _eina_simple_xml_attribute_mp =
    eina_mempool_add(choice,
                     "simple_xml_attribute",
                     NULL,
                     sizeof(Eina_Simple_XML_Attribute),
                     8);
...
EINA_API Efl_Bool
eina_simple_xml_parse(const char        *buf,
                      unsigned           buflen,
                      Efl_Bool           strip,
...
#define CB(type, start, end)                                      \
  ...
  while (itr < itr_end)
  {
    if (itr[0] == '<')
    {
      ...
      else
      {
        ...
        if (type == EINA_SIMPLE_XML_CDATA)
          ...
          p = _eina_simple_xml_tag_doctype_child_end_find(itr + 1 + toff,
                                                          itr_end);
        else if (type == EINA_SIMPLE_XML_COMMENT)
          p = _eina_simple_xml_tag_comment_end_find(itr + 1 + toff, itr_end);
        else p = _eina_simple_xml_tag_end_find(itr + 1 + toff, itr_end);
        ...
        if (p)
        {
          ...
          if ((strip) && (type != EINA_SIMPLE_XML_ERROR) &&
              (type != EINA_SIMPLE_XML_CDATA))
          {
            start = _eina_simple_xml_whitespace_skip(start, end);
            end   = _eina_simple_xml_whitespace_unskip(end, start + 1);
          }

          CB(type, start, end);

          if (type != EINA_SIMPLE_XML_ERROR) itr = p + 1;
          else itr = p;
        }
        else
        {
    ...
    else
    {
      ...
      if (strip) end = _eina_simple_xml_whitespace_unskip(end, itr);

      if (itr != end) CB(EINA_SIMPLE_XML_DATA, itr, end);

      if ((strip) && (end < p)) CB(EINA_SIMPLE_XML_IGNORED, end, p);
...
EINA_API Efl_Bool
eina_simple_xml_attributes_parse(const char                  *buf,
                                 unsigned                     buflen,
                                 ...
                                 const void                  *data)
{
  ...
  while (itr < itr_end)
  {
    ...
    if (!func((void *)data, tmpbuf, tval)) return EFL_FALSE;
                                    ...
                                    const void                  *data)
{
  ...
  do
  {
    ...
    if (key[0])
      if (!func((void *)data, key, val)) return EFL_FALSE;
...
eina_simple_xml_attribute_new(Eina_Simple_XML_Node_Tag *parent,
                              const char               *key,
                              const char               *value)
{
  ...
  if (parent)
    parent->attributes =
      eina_inlist_append(parent->attributes, EINA_INLIST_GET(attr));
...
eina_simple_xml_attribute_free(Eina_Simple_XML_Attribute *attr)
{
  ...
  if (attr->parent)
    attr->parent->attributes =
      eina_inlist_remove(attr->parent->attributes, EINA_INLIST_GET(attr));
...
{
  if (node->base.parent)
    node->base.parent->children =
      eina_inlist_remove(node->base.parent->children,
                         EINA_INLIST_GET(&node->base));
...
eina_simple_xml_node_tag_new(Eina_Simple_XML_Node_Tag *parent, const char *name)
{
  ...
  if (parent)
    parent->children =
      eina_inlist_append(parent->children, EINA_INLIST_GET(&n->base));
...
_eina_simple_xml_node_tag_free(Eina_Simple_XML_Node_Tag *tag)
{
  while (tag->children)
  {
    Eina_Simple_XML_Node *n =
      EINA_INLIST_CONTAINER_GET(tag->children, Eina_Simple_XML_Node);
    if (n->type == EINA_SIMPLE_XML_NODE_TAG)
      _eina_simple_xml_node_tag_free((Eina_Simple_XML_Node_Tag *)n);
    else _eina_simple_xml_node_data_free((Eina_Simple_XML_Node_Data *)n);
  }

  while (tag->attributes)
  {
    Eina_Simple_XML_Attribute *a =
      EINA_INLIST_CONTAINER_GET(tag->attributes, Eina_Simple_XML_Attribute);
    eina_simple_xml_attribute_free(a);
  }

  if (tag->base.parent)
    tag->base.parent->children =
      eina_inlist_remove(tag->base.parent->children,
                         EINA_INLIST_GET(&tag->base));
...
_eina_simple_xml_node_data_new(Eina_Simple_XML_Node_Tag *parent,
...
eina_simple_xml_node_data_new(Eina_Simple_XML_Node_Tag *parent,
                              const char               *contents,
                              size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
eina_simple_xml_node_cdata_new(Eina_Simple_XML_Node_Tag *parent,
                               const char               *contents,
                               size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
eina_simple_xml_node_doctype_child_new(Eina_Simple_XML_Node_Tag *parent,
                                       const char               *contents,
                                       size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
eina_simple_xml_node_processing_new(Eina_Simple_XML_Node_Tag *parent,
                                    const char               *contents,
                                    size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
eina_simple_xml_node_doctype_new(Eina_Simple_XML_Node_Tag *parent,
                                 const char               *contents,
                                 size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
eina_simple_xml_node_comment_new(Eina_Simple_XML_Node_Tag *parent,
                                 const char               *contents,
                                 size_t                    length)
{
  return _eina_simple_xml_node_data_new(parent,
...
static Efl_Bool
_eina_simple_xml_node_parse(void                *data,
                            Eina_Simple_XML_Type type,
                            const char          *content,
                            unsigned             offset,
                            unsigned             length)
{
  ...
  switch (type)
  {
    ...
    case EINA_SIMPLE_XML_OPEN_EMPTY:
      {
        ...
        if (!attrs) name_end = content + length;
        else name_end = attrs;
        ...
        if (!n) return EFL_FALSE;

        if (attrs)
          eina_simple_xml_attributes_parse(attrs,
                                           length - (attrs - content),
                                           _eina_simple_xml_attrs_parse,
                                           n);

        if (type == EINA_SIMPLE_XML_OPEN) ctx->current = n;
      }
      break;
    ...
    case EINA_SIMPLE_XML_CLOSE:
      ...
      {
        const char *end =
          _eina_simple_xml_whitespace_unskip(content + length, content);
        int len;
        len = end - content;
        ...
        WRN("closed tag '%.*s' but already at document root!", length, content);
      break;

    case EINA_SIMPLE_XML_DATA:
      return !!eina_simple_xml_node_data_new(ctx->current, content, length);
    case EINA_SIMPLE_XML_CDATA:
      return !!eina_simple_xml_node_cdata_new(ctx->current, content, length);
    case EINA_SIMPLE_XML_PROCESSING:
      return !!eina_simple_xml_node_processing_new(ctx->current,
                                                   content,
                                                   length);
    case EINA_SIMPLE_XML_DOCTYPE:
      return !!eina_simple_xml_node_doctype_new(ctx->current, content, length);
    case EINA_SIMPLE_XML_DOCTYPE_CHILD:
      return !!eina_simple_xml_node_doctype_child_new(ctx->current,
                                                      content,
                                                      length);
    case EINA_SIMPLE_XML_COMMENT:
      return !!eina_simple_xml_node_comment_new(ctx->current, content, length);
...
eina_simple_xml_node_load(const char *buf, unsigned buflen, Efl_Bool strip)
{
  Eina_Simple_XML_Node_Root            *root;
...
static inline void
_eina_simple_xml_node_dump_indent(Efl_Strbuf *buf,
                                  const char *indent,
                                  unsigned    level)
{
...
static void
_eina_simple_xml_node_tag_attributes_append(Efl_Strbuf               *buf,
...
static void
_eina_simple_xml_node_children_dump(Efl_Strbuf               *buf,
                                    Eina_Simple_XML_Node_Tag *tag,
                                    const char               *indent,
                                    unsigned                  level)
{
...
static void
_eina_simple_xml_node_dump(Efl_Strbuf           *buf,
                           Eina_Simple_XML_Node *node,
                           const char           *indent,
                           unsigned              level)
{
  switch (node->type)
  {
    case EINA_SIMPLE_XML_NODE_ROOT:
      _eina_simple_xml_node_children_dump(buf,
    ...
    case EINA_SIMPLE_XML_NODE_TAG:
      {
        ...
        eina_strbuf_append_length(buf,
                                  n->name,
                                  eina_stringshare_strlen(n->name));

        if (n->attributes) _eina_simple_xml_node_tag_attributes_append(buf, n);

        if (n->children) eina_strbuf_append_char(buf, '>');
        else eina_strbuf_append_length(buf, "/>", sizeof("/>") - 1);
    ...
    case EINA_SIMPLE_XML_NODE_DOCTYPE:
      {
        ...
        if (indent) _eina_simple_xml_node_dump_indent(buf, indent, level);
        eina_strbuf_append_length(buf, "<!DOCTYPE ", sizeof("<!DOCTYPE ") - 1);
        eina_strbuf_append_length(buf, n->data, n->length);
        eina_strbuf_append_char(buf, '>');
        if (indent) eina_strbuf_append_char(buf, '\n');
      }
