...
#define SP_COMPOSE_FINAL_ID(PARTIAL_ID, ENTRY, GENERATION)                \
    (PARTIAL_ID | ((ENTRY & EINA_MASK_ENTRY_ID) << EINA_SHIFT_ENTRY_ID) | \
...
EINA_API Eina_Memory_Table **_eina_sp_ids_tables[EINA_MAX_MID_TABLE_ID] = {
  NULL
};
...
#  endif
  {
    ...
    newsize =
      ((size / MEM_PAGE_SIZE) + (size % MEM_PAGE_SIZE ? 1 : 0)) * MEM_PAGE_SIZE;
...
static void
_eina_safepointer_protect(void *pointer, Efl_Bool may_not_write)
{
#  ifdef HAVE_MMAP
#    ifdef HAVE_VALGRIND
...
#    endif
  {
    ...
    mprotect(header,
             header->size,
             PROT_READ | (may_not_write ? 0 : PROT_WRITE));
  }
#  else
  (void)pointer;
#  endif
...
#  define PROTECT(Ptr)   _eina_safepointer_protect(Ptr, EFL_TRUE)
#  define UNPROTECT(Ptr) _eina_safepointer_protect(Ptr, EFL_FALSE)
                            ...
                            Eina_Table_Index table_id)
{
  ...
  table->partial_id = SP_COMPOSE_PARTIAL_ID(mid_table_id, table_id);
  PROTECT(table);
  ...
  for (mid_table_id = 0; mid_table_id < EINA_MAX_MID_TABLE_ID; mid_table_id++)
  {
    ...
    if (!_eina_sp_ids_tables[mid_table_id])
    {
      _eina_sp_ids_tables[mid_table_id] =
        _eina_safepointer_calloc(EINA_MAX_TABLE_ID,
                                 sizeof(Eina_Memory_Table *));
    }
    if (!_eina_sp_ids_tables[mid_table_id])
    {
    ...
    for (table_id = 0; table_id < EINA_MAX_TABLE_ID; table_id++)
    {
      ...
      if (!table) table = _eina_safepointer_table_new(mid_table_id, table_id);

      if (!table) return NULL;

      if (table->trash || table->start < EINA_MAX_ENTRY_ID) return table;
    }
  }
...
eina_safepointer_unregister(const Eina_Safepointer *safe)
{
  ...
  if (entry_id == EINA_MAX_ENTRY_ID - 1)
  {
    ...
    {
      if (table->entries[i].active) break;
    }
    ...
    if (i == -1)
    {
      ...
      mid_table_id =
        (table->partial_id >> EINA_SHIFT_MID_TABLE_ID) & EINA_MASK_MID_TABLE_ID;
      table_id =
        (table->partial_id >> EINA_SHIFT_TABLE_ID) & EINA_MASK_TABLE_ID;
      UNPROTECT(_eina_sp_ids_tables[mid_table_id]);
      _eina_sp_ids_tables[mid_table_id][table_id] = NULL;
      PROTECT(_eina_sp_ids_tables[mid_table_id]);
      if (!empty_table) empty_table = table;
      else _eina_safepointer_free(table);
    }
  }
...
eina_safepointer_init(void)
{
  ...
  _eina_sp_log_dom =
    eina_log_domain_register("eina_safepointer", EINA_LOG_COLOR_DEFAULT);
