...
{
   if ((r->w > 0) && (r->h > 0)) return EFL_TRUE;
   return EFL_FALSE;
}
...
{
	return (eina_spans_intersect(r1->x, r1->w, r2->x, r2->w) && eina_spans_intersect(r1->y, r1->h, r2->y, r2->h)) ? EFL_TRUE : EFL_FALSE;
}

static inline Efl_Bool
eina_rectangle_xcoord_inside(const Eina_Rectangle *r, int x)
{
	return ((x >= r->x) && (x < (r->x + r->w))) ? EFL_TRUE : EFL_FALSE;
}

static inline Efl_Bool
eina_rectangle_ycoord_inside(const Eina_Rectangle *r, int y)
{
	return ((y >= r->y) && (y < (r->y + r->h))) ? EFL_TRUE : EFL_FALSE;
}

static inline Efl_Bool
eina_rectangle_coords_inside(const Eina_Rectangle *r, int x, int y)
{
	return (eina_rectangle_xcoord_inside(r, x) && eina_rectangle_ycoord_inside(r, y)) ? EFL_TRUE : EFL_FALSE;
}
...
eina_rectangle_intersection(Eina_Rectangle *dst, const Eina_Rectangle *src)
{
   if (eina_rectangle_is_valid(dst) && eina_rectangle_is_valid(src) &&
       eina_rectangles_intersect(dst, src))
     {
        ...
        if ((dst->w == 0) || (dst->h == 0)) return EFL_FALSE;
        return EFL_TRUE;
     }
   return EFL_FALSE;
}
