 ...
 * some data:
 * @until }
 * @note Returning EFL_TRUE is important so we don't stop iterating over the
 * container.
 *
 ...
 */
typedef Efl_Bool (*Eina_Iterator_Next_Callback)(Eina_Iterator *it, void **data);
 ...
 */
typedef Efl_Bool (*Eina_Iterator_Lock_Callback)(Eina_Iterator *it);
...
#define EINA_ITERATOR_VERSION 1
  ...
  Eina_Iterator_Next_Callback next EINA_ARG_NONNULL(1, 2)
    EFL_WARN_UNUSED_RESULT; /**< Callback called when a next element is requested. */
 ...
 * iterator is @c NULL or if a problem occurred, #EFL_FALSE is
 * returned, otherwise #EFL_TRUE is returned.
 */
EINA_API Efl_Bool eina_iterator_next(Eina_Iterator *iterator, void **data)
 ...
 * the function returns immediately. Also, if @p cb returns #EFL_FALSE,
 * the iteration stops at that point, if @p cb returns #EFL_TRUE
 ...
 * immediately. If @p iterator is @c NULL or if a problem occurred, #EFL_FALSE
 * is returned, otherwise #EFL_TRUE is returned. If the container isn't
 * lockable, it will return #EFL_TRUE.
 *
 * @warning None of the existing eina data structures are lockable.
 */
EINA_API Efl_Bool eina_iterator_lock(Eina_Iterator *iterator)
  EINA_ARG_NONNULL(1);
 ...
 * problem occurred, #EFL_FALSE is returned, otherwise #EFL_TRUE
 * is returned. If the container is not lockable, it will
 * return #EFL_TRUE.
 *
 * @warning None of the existing eina data structures are lockable.
 */
EINA_API Efl_Bool eina_iterator_unlock(Eina_Iterator *iterator)
  EINA_ARG_NONNULL(1);
...
eina_iterator_processed_new(Eina_Iterator  *iterator,
                            ...
                            Eina_Free_Cb    free_cb,
                            void           *fdata) EFL_WARN_UNUSED_RESULT;
 ...
 */
#define EINA_ITERATOR_FOREACH(itr, data) while (eina_iterator_next((itr), \
                                                              (void **)(void *)&(data)))
