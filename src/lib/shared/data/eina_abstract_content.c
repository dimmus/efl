...
#endif
#define ERR(...) \
    EINA_LOG_DOM_ERR(_eina_abstract_content_log_domain, __VA_ARGS__)

#ifdef DBG
#  undef DBG
#endif
#define DBG(...) \
    EINA_LOG_DOM_DBG(_eina_abstract_content_log_domain, __VA_ARGS__)
...
EINA_API Efl_Bool
eina_content_converter_conversion_register(
  const char                      *from,
  const char                      *to,
  Eina_Content_Conversion_Callback conversion)
{
  Eina_Content_Conversion_Node *node =
    calloc(1, sizeof(Eina_Content_Conversion_Node));
...
static const void *
_process_cb(const void *container EFL_UNUSED,
            void                 *data,
...
eina_content_converter_possible_conversions(const char *from)
{
  ...
  return eina_iterator_processed_new(eina_list_iterator_new(possibilities),
                                     EINA_PROCESS_CB(_process_cb),
                                     NULL,
                                     possibilities);
}
...
eina_content_free(Eina_Content *content)
{
  ...
  {
    if (content->file) eina_tmpstr_del(content->file);
    free(content->data.mem);
    free(content);
  }
}
...
{
  Eina_Content_Conversion_Callback callback =
    _conversion_callback_fetch(content->type, new_type);
...
static Efl_Bool
_eina_value_type_content_setup(const Eina_Value_Type *type EFL_UNUSED,
                               void                       *mem)
{
...
static Efl_Bool
_eina_value_type_content_copy(const Eina_Value_Type *type EFL_UNUSED,
                              const void                 *src,
                              void                       *dst)
{
...
static int
_eina_value_type_content_compare(const Eina_Value_Type *type EFL_UNUSED,
                                 const void                 *a,
                                 const void                 *b)
{
...
static Efl_Bool
_eina_value_type_content_convert_to(const Eina_Value_Type *type    EFL_UNUSED,
                                    const Eina_Value_Type *convert EFL_UNUSED,
                                    const void *type_mem           EFL_UNUSED,
                                    void *convert_mem              EFL_UNUSED)
{
    ...
    else
    {
      ...
      EINA_ITERATOR_FOREACH(iter, content_type)
      {
        if (eina_streq(content_type, "text/plain;charset=utf-8"))
        {
          ...
          Efl_Bool   success =
            eina_value_type_pset(convert, convert_mem, &data.mem);
             ...
             //create some fallback
      {
        ...
        snprintf(buf,
                 sizeof(buf),
                 "Content %p cannot be converted to "
...
static Efl_Bool
_eina_value_type_content_convert_from(const Eina_Value_Type *type    EFL_UNUSED,
                                      const Eina_Value_Type *convert EFL_UNUSED,
                                      void *type_mem                 EFL_UNUSED,
                                      const void *convert_mem        EFL_UNUSED)
{
  return EFL_FALSE;
}

static Efl_Bool
_eina_value_type_content_pset(const Eina_Value_Type *type EFL_UNUSED,
                              void                       *mem,
                              const void                 *ptr)
{
...
static Efl_Bool
_eina_value_type_content_vset(const Eina_Value_Type *type EFL_UNUSED,
                              void                       *mem,
                              va_list                     args)
{
...
static Efl_Bool
_eina_value_type_content_pget(const Eina_Value_Type *type EFL_UNUSED,
                              const void                 *mem,
                              void                       *ptr)
{
...
{
  _eina_abstract_content_log_domain =
    eina_log_domain_register("eina_abstract_content", "white");
  ...
  eina_content_converter_conversion_register("text/plain",
                                             "text/plain;charset=utf-8",
                                             _copy_converter);
  eina_content_converter_conversion_register("text/plain",
                                             "text/plain;charset=iso-8859-1",
                                             _copy_converter);
  eina_content_converter_conversion_register("text/plain;charset=iso-8859-1",
                                             "text/plain;charset=utf-8",
                                             _latin1_to_utf8_converter);
  eina_content_converter_conversion_register("text/plain;charset=iso-8859-1",
                                             "text/plain",
                                             _copy_converter);
...
eina_value_content_new(Eina_Content *content)
{
  ...
  if (!eina_value_pset(v, &content)) return NULL;
  return v;
}
...
eina_value_content_init(Eina_Content *content)
{
  ...
  EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_setup(&v, EINA_VALUE_TYPE_CONTENT),
                                  EINA_VALUE_EMPTY);
  EINA_SAFETY_ON_FALSE_RETURN_VAL(eina_value_pset(&v, &content),
                                  EINA_VALUE_EMPTY);
