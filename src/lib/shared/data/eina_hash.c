                                ...
                                EFL_UNUSED void      *data)
{
    ...
    if (hash_diff < 0) return EINA_RBTREE_LEFT;
                               ...
                               Eina_Key_Cmp             cmp)
{
    ...
    result = cmp(left->tuple.key,
                 left->tuple.key_length,
                 right->tuple.key,
                 right->tuple.key_length);

    if (result < 0) return EINA_RBTREE_LEFT;
                            ...
                            const void *data)
{
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(hash, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(key, EFL_FALSE);
   ...
   /* Apply eina mask to hash. */
    ...
    else     /* Look up for head node. */
        hash_head = (Eina_Hash_Head *)eina_rbtree_inline_lookup(
    ...
    if (!hash_head)
    {
        ...
        if (!hash_head) goto on_error;

        hash_head->hash = key_hash;
        hash_head->head = NULL;

        hash->buckets[hash_num] = eina_rbtree_inline_insert(
            hash->buckets[hash_num],
            EINA_RBTREE_GET(hash_head),
            EINA_RBTREE_CMP_NODE_CB(_eina_hash_hash_rbtree_cmp_node),
            NULL);
    }
   ...
   */
    ...
    if (!new_hash_element) goto on_error;
   ...
   /* Setup the element */
    ...
    else new_hash_element->tuple.key = key;

   /* add the new element to the hash. */
    hash_head->head = eina_rbtree_inline_insert(
        hash_head->head,
        EINA_RBTREE_GET(new_hash_element),
        EINA_RBTREE_CMP_NODE_CB(_eina_hash_key_rbtree_cmp_node),
                        ...
                        Eina_Hash_Head **hash_head)
{
    Eina_Hash_Element *hash_element;
    int                rb_hash =
        (key_hash >> hash->buckets_power_size) & EINA_HASH_RBTREE_MASK;
    ...
    *hash_head = (Eina_Hash_Head *)eina_rbtree_inline_lookup(
        ...
        EINA_RBTREE_CMP_KEY_CB(_eina_hash_hash_rbtree_cmp_hash),
        NULL);
    if (!*hash_head) return NULL;

    hash_element = (Eina_Hash_Element *)eina_rbtree_inline_lookup(
        (*hash_head)->head,
        tuple,
        0,
        EINA_RBTREE_CMP_KEY_CB(_eina_hash_key_rbtree_cmp_key_data),
        (const void *)hash->key_cmp_cb);
                        ...
                        Eina_Hash_Head **hash_head)
{
    ...
    {
        if (!hash->buckets[hash_num]) continue;
...
{
    if (hash->data_free_cb) hash->data_free_cb(hash_element->tuple.data);
...
_eina_hash_head_free(Eina_Hash_Head *hash_head, Eina_Hash *hash)
{
    eina_rbtree_delete(hash_head->head,
                       EINA_RBTREE_FREE_CB(_eina_hash_el_free),
                       hash);
    free(hash_head);
}
...
_eina_hash_del_by_hash_el(Eina_Hash         *hash,
                          ...
                          int                key_hash)
{
    hash_head->head = eina_rbtree_inline_remove(
        hash_head->head,
    ...
    if (!hash_head->head)
    {
        ...
        hash->buckets[key_hash] = eina_rbtree_inline_remove(
            hash->buckets[key_hash],
            EINA_RBTREE_GET(hash_head),
            EINA_RBTREE_CMP_NODE_CB(_eina_hash_hash_rbtree_cmp_node),
            NULL);
                           ...
                           const void *data)
{
    ...
    if (!hash->buckets) return EFL_FALSE;
    ...
    if (!hash_element) return EFL_FALSE;

    return _eina_hash_del_by_hash_el(hash, hash_element, hash_head, key_hash);
}

static void
_eina_hash_compute(const Eina_Hash *hash,
                   const void      *key,
                   int             *key_length,
                   int             *key_hash)
{
...
static int
_eina_stringshared_key_cmp(const char    *key1,
                           EFL_UNUSED int key1_length,
                           const char    *key2,
                           EFL_UNUSED int key2_length)
{
...
static int
_eina_int32_key_cmp(const uint32_t *key1,
                    EFL_UNUSED int  key1_length,
                    const uint32_t *key2,
                    EFL_UNUSED int  key2_length)
{
...
static int
_eina_int64_key_cmp(const uint64_t *key1,
                    EFL_UNUSED int  key1_length,
                    const uint64_t *key2,
                    EFL_UNUSED int  key2_length)
{
...
_eina_hash_iterator_next(Eina_Iterator_Hash *it, void **data)
{
    ...
    if (!(it->index < it->hash->population)) return EFL_FALSE;
    ...
    if (ok == EFL_FALSE)
    {
        ...
        it->list = eina_rbtree_iterator_prefix(it->hash_head->head);
        ok = eina_iterator_next(it->list, (void **)(void *)&it->hash_element);
        if (bucket == it->hash->size) ok = EFL_FALSE;
    }

    it->index++;
    it->bucket = bucket;

    if (ok) *data = it->get_content(it);
    ...
    if (hash->buckets)
    {
        for (i = 0; i < hash->size; i++)
            eina_rbtree_delete(hash->buckets[i],
                               EINA_RBTREE_FREE_CB(_eina_hash_head_free),
                               hash);
        free(hash->buckets);
    }
    free(hash);
...
eina_hash_add(Eina_Hash *hash, const void *key, const void *data)
{
    ...
    EINA_MAGIC_CHECK_HASH(hash);
    EINA_SAFETY_ON_NULL_RETURN_VAL(hash, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(hash->key_hash_cb, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(key, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(data, EFL_FALSE);
...
eina_hash_del_by_data(Eina_Hash *hash, const void *data)
{
    ...
    if (!hash_element) goto error;

    if (hash_element->tuple.data != data) goto error;
                      ...
                      const void *data)
{
    ...
    else ret = eina_hash_del_by_data(hash, data);
...
eina_hash_del(Eina_Hash *hash, const void *key, const void *data)
{
    ...
    if (!key) return eina_hash_del_by_data(hash, data);
                       ...
                       int              key_hash)
{
    ...
    hash_element = _eina_hash_find_by_hash(hash, &tuple, key_hash, &hash_head);
    if (hash_element) return hash_element->tuple.data;
...
eina_hash_move(Eina_Hash *hash, const void *old_key, const void *new_key)
{
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(hash, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(hash->key_hash_cb, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(old_key, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(new_key, EFL_FALSE);
    EINA_MAGIC_CHECK_HASH(hash);
...
eina_hash_list_append(Eina_Hash *hash, const void *key, const void *data)
{
    ...
    hash_element = _eina_hash_find_by_hash(hash, &tuple, key_hash, &hash_head);
    if (hash_element)
        hash_element->tuple.data =
            eina_list_append(hash_element->tuple.data, data);
    else
        eina_hash_add_alloc_by_hash(hash,
                                    key,
                                    key_length,
...
eina_hash_list_prepend(Eina_Hash *hash, const void *key, const void *data)
{
    ...
    if (hash_element)
        hash_element->tuple.data =
            eina_list_prepend(hash_element->tuple.data, data);
    else
        eina_hash_add_alloc_by_hash(hash,
                                    key,
                                    key_length,
...
EINA_API void
eina_hash_list_direct_prepend(Eina_Hash  *hash,
                              const void *key,
                              const void *data)
{
    Eina_Hash_Tuple    tuple;
