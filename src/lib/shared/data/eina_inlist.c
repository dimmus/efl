                            ...
                            void                **data)
{
    ...
    else if (idx > it->index)     /* Looking after current. */
    ...
    else
    {
        ...
        if (idx > middle)          /* Looking backward from current. */
            for (i = it->index, over = it->current; i > idx && over;
                 --i, over           = over->prev)
                ;
        else          /* Looking from the start. */
...
eina_inlist_sort_merge(Eina_Inlist *a, Eina_Inlist *b, Eina_Compare_Cb func)
{
    ...
    if (func(a, b) < 0) a = (last = first = a)->next;
    else b = (last = first = b)->next;

    while (a && b)
        if (func(a, b) < 0) a = (last = last->next = a)->next;
...
eina_inlist_remove(Eina_Inlist *list, Eina_Inlist *item)
{
    ...
    {
        EINA_LOG_ERR("safety check failed: item %p does not appear to be part "
                     "of an inlist!",
                     item);
        return list;
    }
...
EINA_API int
eina_inlist_sorted_state_init(Eina_Inlist_Sorted_State *state,
                              Eina_Inlist              *list)
{
    Eina_Inlist *ct         = NULL;
    int          count      = 0;
                          ...
                          Eina_Compare_Cb func)
{
    ...
    if (!list->next)
    {
        ...
        if (cmp < 0) return eina_inlist_append(list, item);
    ...
    * just append the item to the list */
    if (cmp < 0 && ct == list->last) return eina_inlist_append(list, item);
    ...
    */
    ...
    if (cmp <= 0) return eina_inlist_append_relative(list, item, ct);
    return eina_inlist_prepend_relative(list, item, ct);
}
...
eina_inlist_sort(Eina_Inlist *head, Eina_Compare_Cb func)
{
    ...
    while (tail)
    {
        ...
        if (func(a, b) < 0) ((stack[i++] = a)->next = b)->next = 0;
        else ((stack[i++] = b)->next = a)->next = 0;

        tmp = n++;
        for (idx = n ^ tmp; idx &= idx - 1; i--)
            stack[i - 2] =
                eina_inlist_sort_merge(stack[i - 2], stack[i - 1], func);
    }
