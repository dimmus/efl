    ...
    if (r->w >= w && r->h >= h)
    {
      ...
      *y = r->y;
      ...
      else
      {
        ...
        if ((rw1 * r->h > h2 * r->w) || type == Eina_Packing_Bottom_Left ||
  ...
  EINA_LIST_FOREACH(empty, l, r)
  {
    ...
    EINA_LIST_FOREACH(l1, l2, r1)
    {
      if ((r->x <= r1->x) && (r->y <= r1->y) &&
          (r->x + r->w >= r1->x + r1->w) && (r->y + r->h >= r1->y + r1->h))
      {
  ...
  if (r)
  {
    if (era->pool->type == Eina_Packing_Bottom_Left_Skyline_Improved)
    {
      ...
      EINA_INLIST_FOREACH(era->pool->head, new)
      {
        ...
        if (match)
          era->pool->empty =
            _eina_rectangle_skyline_list_update(era->pool->empty, match);
      }
    }
    else
      era->pool->empty =
        _eina_rectangle_merge_list(era->pool->empty, era->pool->type, r);
