...
_eina_rbtree_iterator_list_new(Eina_Iterator_Rbtree *it,
...
_eina_rbtree_iterator_next(Eina_Iterator_Rbtree *it, void **data)
{
  ...
  if (eina_array_count(it->stack) <= 0) return EFL_FALSE;

  last = eina_array_data_get(it->stack, eina_array_count(it->stack) - 1);
  tree = last->tree;

  if (!last->tree || last->up == EFL_TRUE)
  {
    ...
    while (last->dir == EINA_RBTREE_LEFT || !last->tree)
    {
      if (tree)
            ...
            EINA_RBTREE_ITERATOR_POSTFIX_MASK)
        {
          ...
          {
            last =
              eina_array_data_get(it->stack, eina_array_count(it->stack) - 1);
            last->up = EFL_TRUE;
          }
...
{
  node->son[dir ^ 1] =
    _eina_rbtree_inline_single_rotation(node->son[dir ^ 1], dir ^ 1);
                          ...
                          const void             *data)
{
  ...
  *r = node;
  ...
  while (s > 0)
  {
    ...
    a = q->son[dir];
    /* Rebalance done ? */
    if (a == NULL || a->color == EINA_RBTREE_BLACK) break;
...
found:
  ...
  if (l0 != NULL && l1 != NULL) /* two links case */
  {
    ...
    *rt = q;
    ...
    if (c == EINA_RBTREE_RED) goto end;

    /* fix stack for replaced entry */
    if (s > ss) stack[ss] = (uintptr_t)(q->son + 1) | 0;
  }
  else /* single link case */
  {
    ...
    *rt = l0;
    ...
    if (node->color == EINA_RBTREE_RED) goto end; /* removed red */
    ...
    if (q != NULL)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          q->color = EINA_RBTREE_RED;
          if (r_color == EINA_RBTREE_RED) break;
          continue;
        }
      }
  ...
  if (root != NULL) root->color = EINA_RBTREE_BLACK;
  return root;
}
