...
_split_strict(list_t *dirty, const rect_t current, rect_t r)
{
  ...
  if (w_1 > 0)
       ...
       *        `----'
       */
    rect_list_append_xywh(
      dirty,
      r.left,
      r.top,
      w_1,
      r.height); /* not necessary to keep these, r (b) will be destroyed */
...
static inline void
_calc_intra_outer_rect_area(const rect_t a,
                            const rect_t b,
                            rect_t      *intra,
                            rect_t      *outer)
{
...
rect_list_del_split_strict(list_t *rects, const rect_t del_r)
{
  ...
  while (cur_node)
  {
    ...
    else if ((intra_width == current.width) && (intra_height == current.height))
    {
            /*  .-------.del_r
              *  | .---. |
              *  | |   | |
    ...
    while (cur_node)
    {
               ...
               (intra.height == current.height))
      {
                /* .-------.r
                   ...
                   * `-------'
                   */
        if (old_last == cur_node) old_last = prev_cur_node;
 ...
 */
    ...
    if (EINA_UNLIKELY(keep_dirty)) rect_list_append_node(rects, d_node);
...
rect_list_merge_rects(list_t *rects, list_t *to_merge, int accepted_error)
{
  while (to_merge->head)
  {
    ...
    else rect_list_del_next(to_merge, NULL);
  }
}
...
static inline void
_splitter_new(Eina_Tiler *t)
{
  t->splitter.rects      = list_zeroed;
  t->splitter.need_merge = EFL_FALSE;
}

static inline void
_splitter_del(Eina_Tiler *t)
{
...
static inline void
_splitter_tile_size_set(Eina_Tiler *t, int w EFL_UNUSED, int h EFL_UNUSED)
{
...
_splitter_rect_add(Eina_Tiler *t, Eina_Rectangle *rect)
{
  rect_node_t *rn;
...
static inline void
_splitter_rect_del(Eina_Tiler *t, Eina_Rectangle *rect)
{
  rect_t r;

  if (!t->splitter.rects.head) return;
  ...
  t->splitter.need_merge = EFL_TRUE;
  return;
}

static inline void
_splitter_clear(Eina_Tiler *t)
{
  rect_list_clear(&t->splitter.rects);
  t->splitter.need_merge = EFL_FALSE;
}
...
static Efl_Bool
_iterator_next(Eina_Iterator_Tiler *it, void **data)
{
  list_node_t *n;
  ...
  for (n = it->curr; n; n = n->next)
  {
    ...
    if (eina_rectangle_intersection(&it->r, &it->tiler->area) == EFL_FALSE)
      continue;

    if ((it->r.w <= 0) || (it->r.h <= 0)) continue;
...
static void *
_iterator_get_container(Eina_Iterator_Tiler *it)
{
...
static void
_iterator_free(Eina_Iterator_Tiler *it)
{
...
EINA_API void
eina_tiler_area_size_set(Eina_Tiler *t, int w, int h)
{
  EINA_MAGIC_CHECK_TILER(t);
  if ((w <= 0) || (h <= 0)) return;
...
EINA_API void
eina_tiler_area_size_get(const Eina_Tiler *t, int *w, int *h)
{
  EINA_MAGIC_CHECK_TILER(t);
  if (w) *w = t->area.w;
  if (h) *h = t->area.h;
...
eina_tiler_tile_size_set(Eina_Tiler *t, int w, int h)
{
  ...
  if (w == 1 || h == 1) t->rounding = EFL_FALSE;
  t->tile.w = w;
  t->tile.h = h;
...
{
  EINA_MAGIC_CHECK_TILER(t, EFL_TRUE);
...
EINA_API Efl_Bool
eina_tiler_rect_add(Eina_Tiler *t, const Eina_Rectangle *r)
{
  Eina_Rectangle tmp __attribute__((aligned(8)));

  EINA_MAGIC_CHECK_TILER(t, EFL_FALSE);
  EINA_SAFETY_ON_NULL_RETURN_VAL(r, EFL_FALSE);

  if ((r->w <= 0) || (r->h <= 0)) return EFL_FALSE;

  tmp = *r;
  if (eina_rectangle_intersection(&tmp, &t->area) == EFL_FALSE)
    return EFL_FALSE;

  if ((tmp.w <= 0) || (tmp.h <= 0)) return EFL_FALSE;

  if (_rect_same(&tmp, &t->last.add)) return EFL_TRUE;
...
EINA_API void
eina_tiler_rect_del(Eina_Tiler *t, const Eina_Rectangle *r)
{
  Eina_Rectangle tmp __attribute__((aligned(8)));
  ...
  tmp = *r;
  if (eina_rectangle_intersection(&tmp, &t->area) == EFL_FALSE) return;

  if ((tmp.w <= 0) || (tmp.h <= 0)) return;

  if (_rect_same(&tmp, &t->last.del)) return;
...
EINA_API void
eina_tiler_strict_set(Eina_Tiler *t, Efl_Bool strict)
{
  EINA_MAGIC_CHECK_TILER(t);
  t->strict = strict;
}
...
eina_tiler_iterator_new(const Eina_Tiler *t)
{
  ...
  if (t->splitter.need_merge == EFL_TRUE)
  {
    splitter_t *sp;
    list_t      to_merge;
...
EINA_API Efl_Bool
eina_tiler_union(Eina_Tiler *dst, Eina_Tiler *src)
{
  Eina_Iterator  *itr;
  ...
  EINA_MAGIC_CHECK_TILER(src, EFL_FALSE);
  EINA_MAGIC_CHECK_TILER(dst, EFL_FALSE);

  if ((src->area.w <= 0) || (src->area.h <= 0)) return EFL_FALSE;

  dst->area.w = MAX(src->area.w, dst->area.w);
  dst->area.h = MAX(src->area.h, dst->area.h);

  itr = eina_tiler_iterator_new(src);
  if (!itr) return EFL_FALSE;
...
EINA_API Efl_Bool
eina_tiler_subtract(Eina_Tiler *dst, Eina_Tiler *src)
{
  Eina_Iterator  *itr;
  Eina_Rectangle *rect, _rect;
...
EINA_API Eina_Tiler *
eina_tiler_intersection(Eina_Tiler *t1, Eina_Tiler *t2)
{
  Eina_Tiler *t = NULL;
  int         w, h;
  ...
  if (!(eina_rectangles_intersect(&t1->area, &t2->area))) return NULL;

  itr1 = eina_tiler_iterator_new(t1);
  itr2 = eina_tiler_iterator_new(t2);

  if (!((itr1) && (itr2))) goto cleanup;
  ...
  while ((rect1) && (rect2))
  {
    ...
    {
      if (eina_iterator_next(itr1, (void **)&rect1)) continue;
    }
    else if (rect1->x + rect1->w > rect2->x + rect2->w)
    {
      if (eina_iterator_next(itr2, (void **)&rect2)) continue;
    }
...
EINA_API Efl_Bool
eina_tiler_equal(const Eina_Tiler *t1, const Eina_Tiler *t2)
{
  Eina_Iterator  *itr1 = NULL, *itr2 = NULL;
  Eina_Rectangle *rect1 = NULL, *rect2 = NULL;
  Efl_Bool        next_t1 = EFL_FALSE, next_t2 = EFL_FALSE;
  Efl_Bool        r = EFL_FALSE;

  EINA_MAGIC_CHECK_TILER(t1, EFL_FALSE);
  EINA_MAGIC_CHECK_TILER(t2, EFL_FALSE);

  if (!(eina_rectangles_intersect(&t1->area, &t2->area))) return EFL_FALSE;
  ...
  while ((rect1) && (rect2))
  {
    if (!eina_rectangles_intersect(rect1, rect2)) goto cleanup;
    ...
    if (next_t1 != next_t2) goto cleanup;

    if (!next_t1 && !next_t2) break;
  }
...
eina_tile_grid_slicer_iterator_next(Eina_Tile_Grid_Slicer_Iterator *it,
                                    void                          **data)
{
  return eina_tile_grid_slicer_next(&it->priv,
                                    (const Eina_Tile_Grid_Info **)data);
}
