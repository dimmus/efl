...
/* EINA_VALUE_NEW(float, FLOAT, float) */ /* implicit conversion from ‘float’ to ‘double’ when passing argument to function */
EINA_VALUE_NEW(double, DOUBLE, double)
EINA_VALUE_NEW(bool, BOOL, Efl_Bool)
...
/**
 ...
 @return #EFL_TRUE if type matches and fetched contents,
 #EFL_FALSE on different type or failures.
 @since 1.21
 */                                                                     \
static inline Efl_Bool                                                 \
eina_value_##Compress_Type##_get(const Eina_Value *v, Type *c)          \
{                                                                       \
   EINA_SAFETY_ON_NULL_RETURN_VAL(c, EFL_FALSE);                       \
   *c = 0;                                                              \
...
EINA_VALUE_GET(bool, BOOL, Efl_Bool)
...
{                                                                       \
   ...
   Efl_Bool r = EFL_FALSE;                                            \
                                                                        \
   EINA_SAFETY_ON_NULL_RETURN_VAL(c, EFL_FALSE);                       \
   EINA_SAFETY_ON_NULL_RETURN_VAL(v, EFL_FALSE);                       \
                                                                        \
   /* Try no conversion first */                                        \
   if (eina_value_##Compress_Type##_get(v, c)) return EFL_TRUE;        \
                                                                        \
   if (!eina_value_setup(&dst, EINA_VALUE_TYPE_##Uppercase_Compress_Type)) return EFL_FALSE; \
...
EINA_VALUE_CONVERT(bool, BOOL, Efl_Bool)
 ...
 * @return @c EFL_TRUE on success
 * @since 1.21
 */
static inline Efl_Bool
eina_value_stringshare_copy(const Eina_Value *val, Eina_Stringshare **str)
{
   EINA_SAFETY_ON_NULL_RETURN_VAL(val, EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(str, EFL_FALSE);
   *str = NULL;
   EINA_SAFETY_ON_FALSE_RETURN_VAL(val->type == EINA_VALUE_TYPE_STRINGSHARE, EFL_FALSE);
   if (!eina_value_get(val, str)) return EFL_FALSE;
...
eina_value_string_copy(const Eina_Value *val, char **str)
{
   ...
   EINA_SAFETY_ON_NULL_RETURN_VAL(str, EFL_FALSE);
   *str = NULL;
   EINA_SAFETY_ON_FALSE_RETURN_VAL((val->type == EINA_VALUE_TYPE_STRINGSHARE || val->type == EINA_VALUE_TYPE_STRING), EFL_FALSE);
   if (!eina_value_get(val, &s)) return EFL_FALSE;
   *str = s ? strdup(s) : NULL;
   return EFL_TRUE;
}
