...
_eina_matrixsparse_cell_free(Eina_Matrixsparse_Cell *c,
                             ...
                             void *user_data)
{
  if (free_func) free_func(user_data, c->data);
...
_eina_matrixsparse_cell_unlink(Eina_Matrixsparse_Cell *c)
{
  ...
  if (r->last_used == c)
  {
    if (c->next) r->last_used = c->next;
    else r->last_used = c->prev;
  }

  if (r->last_col == c) r->last_col = c->prev;

  if (r->cols == c) r->cols = c->next;
  ...
  else if (c->next) c->next->prev = NULL;
  else if (c->prev) c->prev->next = NULL;
}
...
_eina_matrixsparse_row_unlink(Eina_Matrixsparse_Row *r)
{
  ...
  if (m->last_used == r)
  {
    if (r->next) m->last_used = r->next;
    else m->last_used = r->prev;
  }

  if (m->last_row == r) m->last_row = r->prev;

  if (m->rows == r) m->rows = r->next;
  ...
  else if (r->next) r->next->prev = NULL;
  else if (r->prev) r->prev->next = NULL;
}
...
{
  Eina_Matrixsparse_Row *r = eina_mempool_malloc(_eina_matrixsparse_row_mp,
                                                 sizeof(Eina_Matrixsparse_Row));
  if (!r) return NULL;
...
{
  Eina_Matrixsparse_Cell *c =
    eina_mempool_malloc(_eina_matrixsparse_cell_mp,
...
{
  Eina_Matrixsparse_Row *r = _eina_matrixsparse_row_idx_get(m, row);
  if (!r) r = _eina_matrixsparse_row_idx_add(m, row);

  if (!r) return 0;

  if (_eina_matrixsparse_row_cell_idx_add(r, col, data)) return 1;
...
{
  EINA_MAGIC_CHECK_MATRIXSPARSE_ITERATOR(it, EFL_FALSE);
  ...
  *data = (Eina_Matrixsparse_Cell *)it->ref.col;
  ...
  if (!it->ref.col)
  {
    ...
    if (it->ref.row) it->ref.col = it->ref.row->cols;
  }
  ...
  void                               **data)
{
  ...
  if (it->idx.row >= it->m->size.rows) return 0;

  if (it->dummy.col.data) ERR("Last iterator call changed dummy cell!");

  if ((it->ref.col) && (it->ref.col->col == it->idx.col) &&
...
eina_matrixsparse_init(void)
{
  ...
  _eina_matrixsparse_log_dom =
    eina_log_domain_register("eina_matrixsparse", EINA_LOG_COLOR_DEFAULT);
  ...
  _eina_matrixsparse_cell_mp = eina_mempool_add(choice,
                                                "matrixsparse_cell",
                                                NULL,
                                                sizeof(Eina_Matrixsparse_Cell),
                                                32);
  ...
  {
    ERR("Mempool for matrixsparse_cell cannot be allocated in matrixsparse "
        "init.");
    goto on_init_fail;
  }

  _eina_matrixsparse_row_mp = eina_mempool_add(choice,
                                               "matrixsparse_row",
                                               NULL,
                                               sizeof(Eina_Matrixsparse_Row),
                                               32);
  ...
  {
    ERR("Mempool for matrixsparse_row cannot be allocated in matrixsparse "
        "init.");
    goto on_init_fail;
  }
                           ...
                           unsigned long      cols)
{
  ...
  if (update_last_used_row) m->last_used = m->last_row;

  r = m->rows;
  while (r)
  {
    Eina_Matrixsparse_Cell *c = r->last_col;
    Efl_Bool                update_last_used_col;
    ...
    if (!c)
    {
      ...
      r = r->next;
      _eina_matrixsparse_row_free(r_aux, free_func, user_data);
      if ((update_last_used_row) && (m->last_used == r_aux)) m->last_used = r;
    }
    else
    {
      if (c != r->last_col)
      ...
      if (update_last_used_col) r->last_used = r->last_col;
...
eina_matrixsparse_cell_data_set(Eina_Matrixsparse_Cell *cell, const void *data)
{
  ...
  if (m->free.func) m->free.func(m->free.user_data, cell->data);
...
eina_matrixsparse_iterator_new(const Eina_Matrixsparse *m)
{
  ...
  it->iterator.get_container =
    FUNC_ITERATOR_GET_CONTAINER(_eina_matrixsparse_iterator_get_container);
