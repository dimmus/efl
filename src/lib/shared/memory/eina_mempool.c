...
eina_mempool_init(void)
{
  ...
  path =
    eina_module_symbol_path_get((const void *)eina_init, "/eina/modules/mp");
...
eina_mempool_shutdown(void)
{
  ...
  if (_modules) eina_array_free(_modules);

  if (_backends) eina_hash_free(_backends);
...
EINA_API void
eina_mempool_del(Eina_Mempool *mp)
{
...
EINA_API void
eina_mempool_gc(Eina_Mempool *mp)
{
  EINA_SAFETY_ON_NULL_RETURN(mp);
...
EINA_API void
eina_mempool_statistics(Eina_Mempool *mp)
{
  EINA_SAFETY_ON_NULL_RETURN(mp);
