...
static Efl_Bool _disable_color           = EFL_FALSE;
static Efl_Bool _disable_file            = EFL_FALSE;
static Efl_Bool _disable_function        = EFL_FALSE;
static Efl_Bool _abort_on_critical       = EFL_FALSE;
static Efl_Bool _disable_timing          = EFL_TRUE;
...
static Efl_Bool _threads_enabled = EFL_FALSE;
static Efl_Bool _threads_inited  = EFL_FALSE;
...
static Efl_Bool _eina_log_win32_is_console = EFL_FALSE;
  ...
  while (1)
  {
    ...
    if (code)
    {
      ...
      else if ((code >= 30) && (code <= 37))
      {
        ...
        if (code == 31) attr |= FOREGROUND_RED;
        else if (code == 32) attr |= FOREGROUND_GREEN;
        else if (code == 33) attr |= FOREGROUND_RED | FOREGROUND_GREEN;
        else if (code == 34) attr |= FOREGROUND_BLUE;
        else if (code == 35) attr |= FOREGROUND_RED | FOREGROUND_BLUE;
        else if (code == 36) attr |= FOREGROUND_GREEN | FOREGROUND_BLUE;
        else if (code == 37)
          attr |= FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE;
      }
      else if ((code >= 40) && (code <= 47))
      {
        ...
        if (code == 41) attr |= BACKGROUND_RED;
        else if (code == 42) attr |= BACKGROUND_GREEN;
        else if (code == 43) attr |= BACKGROUND_RED | BACKGROUND_GREEN;
        else if (code == 44) attr |= BACKGROUND_BLUE;
        else if (code == 45) attr |= BACKGROUND_RED | BACKGROUND_BLUE;
        else if (code == 46) attr |= BACKGROUND_GREEN | BACKGROUND_BLUE;
        else if (code == 47)
          attr |= BACKGROUND_RED | BACKGROUND_GREEN | BACKGROUND_BLUE;
      }
    }
...
#  define DECLARE_LEVEL_NAME(level) \
      const char *name;             \
      eina_log_print_level_name_get(level, &name)
#  ifdef _WIN32
#    define DECLARE_LEVEL_NAME_COLOR_WIN32_CONSOLE(level) \
        const char *name;                                 \
        int         color;                                \
        eina_log_print_level_name_color_get_win32_console(level, &name, &color)
#  endif
#  define DECLARE_LEVEL_NAME_COLOR_POSIX(level) \
...
{
  DECLARE_LEVEL_NAME(level);
  fprintf(fp, "%s<%u>:%s %s() ", name, eina_log_pid_get(), d->domain_str, fnc);
}
...
#  ifdef _WIN32
static void
eina_log_print_prefix_NOthreads_color_file_func_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *file,
                                                      ...
                                                      int            line)
{
  ...
  fprintf(fp,
          "%s%s<%u>" EINA_COLOR_RESET ":%s %s:%d " EINA_COLOR_HIGH
          "%s()" EINA_COLOR_RESET " ",
          color,
          name,
          eina_log_pid_get(),
...
#  ifdef _WIN32
static void
eina_log_print_prefix_NOthreads_color_NOfile_func_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *fnc)
...
static void
eina_log_print_prefix_NOthreads_color_NOfile_func_posix(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *fnc)
{
  DECLARE_LEVEL_NAME_COLOR_POSIX(level);
  fprintf(fp,
          "%s%s<%u>" EINA_COLOR_RESET ":%s " EINA_COLOR_HIGH
          "%s()" EINA_COLOR_RESET " ",
          color,
          name,
          eina_log_pid_get(),
...
#  endif
    eina_log_print_prefix_NOthreads_color_NOfile_func_posix(fp, d, level, fnc);
}

#  ifdef _WIN32
static void
eina_log_print_prefix_NOthreads_color_file_NOfunc_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *file,
...
static void
eina_log_print_prefix_NOthreads_color_file_NOfunc_posix(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *file,
...
#  ifdef _WIN32
static void
eina_log_print_prefix_threads_color_file_func_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *file,
                                                    ...
                                                    Eina_Thread    cur)
{
  DECLARE_LEVEL_NAME_COLOR_POSIX(level);
  fprintf(fp,
          "%s%s<%u>" EINA_COLOR_RESET ":%s[T:" EINA_COLOR_ORANGE
          "%lu" EINA_COLOR_RESET "] %s:%d " EINA_COLOR_HIGH
          "%s()" EINA_COLOR_RESET " ",
          color,
          name,
          eina_log_pid_get(),
...
#  ifdef _WIN32
static void
eina_log_print_prefix_threads_color_NOfile_func_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *fnc,
                                                      ...
                                                      Eina_Thread    cur)
{
  ...
  fprintf(fp,
          "%s%s<%u>" EINA_COLOR_RESET ":%s[T:" EINA_COLOR_ORANGE
          "%lu" EINA_COLOR_RESET "] " EINA_COLOR_HIGH "%s()" EINA_COLOR_RESET
          " ",
          color,
          name,
          eina_log_pid_get(),
...
#  ifdef _WIN32
static void
eina_log_print_prefix_threads_color_file_NOfunc_win32_console(
  FILE                  *fp,
  const Eina_Log_Domain *d,
  Eina_Log_Level         level,
  const char            *file,
...
eina_log_print_prefix_update(void)
{
  if (_disable_file && _disable_function)
  {
    fprintf(stderr,
            ...
            " and " EINA_LOG_ENV_FUNCTION_DISABLE " set at the same time, will "
...
#  define S(NOthread, NOcolor, NOfile, NOfunc) \
      _eina_log_print_prefix =                 \
          eina_log_print_prefix_##NOthread##threads_##NOcolor##color_##NOfile##file_##NOfunc##func

  if (_threads_enabled)
  {
    if (_disable_color)
    {
      if (_disable_file) S(, NO, NO, );
      else if (_disable_function) S(, NO, , NO);
      else S(, NO, , );
    }
    else
    {
      if (_disable_file) S(, , NO, );
      else if (_disable_function) S(, , , NO);
      else S(, , , );
    }
  ...
  if (_disable_color)
  {
    if (_disable_file) S(NO, NO, NO, );
    else if (_disable_function) S(NO, NO, , NO);
    else S(NO, NO, , );
  }
  else
  {
...
eina_log_domain_new(Eina_Log_Domain *d,
                    Eina_Log_Timing *t,
                    const char      *name,
                    const char      *color)
{
  ...
  else d->domain_str = eina_log_domain_str_get(name, NULL);
...
#  ifdef EINA_SAFETY_CHECKS
      ...
      EINA_UNLIKELY(domain < 0))
  {
    ...
    {
      fprintf(stderr,
              "CRI<%u>:eina_log %s:%d %s() unknown log domain %d, "
    ...
    else
    {
      ...
      {
        fprintf(stderr,
                "CRI<%u>:eina_log unknown log domain %d, original "
                "message was: %s: '",
                eina_log_pid_get(),
                domain,
                name);
    ...
    if (EINA_UNLIKELY(_abort_on_critical)) abort();
...
#  ifdef EINA_SAFETY_CHECKS
  if (EINA_UNLIKELY(d->deleted))
  {
    if ((!d->level) || (level > d->level))
      fprintf(stderr,
              "ERR<%u>:eina_log %s:%d %s() log domain %d was deleted\n",
              eina_log_pid_get(),
              file,
              line,
              fnc,
    ...
    else
    {
      DECLARE_LEVEL_NAME(level);
      fprintf(stderr,
              "ERR<%u>:eina_log %s:%d %s() log domain %d was "
...
#  ifdef EINA_LOG_BACKTRACE
  if ((tmp = getenv(EINA_LOG_ENV_BACKTRACE))) _backtrace_level = atoi(tmp);
#  endif

  if ((tmp = getenv(EINA_LOG_ENV_COLOR_DISABLE))) color_disable = atoi(tmp);
  else color_disable = -1;

  /* Check if color is explicitly disabled */
  if (color_disable == 1) _disable_color = EFL_TRUE;
...
#  ifndef _WIN32
  ...
  else if (color_disable == -1)
  {
    if (!eina_log_term_color_supported(getenv("TERM")))
      _disable_color = EFL_TRUE;
    else
    {
      /* if not a terminal, but redirected to a file, disable color */
      int fd;

      if (_print_cb == eina_log_print_cb_stderr) fd = STDERR_FILENO;
      else if (_print_cb == eina_log_print_cb_stdout) fd = STDOUT_FILENO;
      else fd = -1;

      if ((fd >= 0) && (!isatty(fd))) _disable_color = EFL_TRUE;
    }
  }
#  else
  if (GetConsoleMode(GetStdHandle(STD_OUTPUT_HANDLE), &mode))
    _eina_log_win32_is_console = EFL_TRUE;
#  endif
...
#  endif
  ...
  // Global log level
  if ((level = getenv(EINA_LOG_ENV_LEVEL))) _log_level = atoi(level);
...
eina_log_shutdown(void)
{
  ...
  {
    if (_log_domains[_log_domains_count].deleted) continue;
...
eina_log_threads_shutdown(void)
{
  ...
  CHECK_MAIN();
  SHUTDOWN();
  _threads_enabled = EFL_FALSE;
  _threads_inited  = EFL_FALSE;
#endif
}
...
eina_log_print_cb_set(Eina_Log_Print_Cb cb, void *data)
{
  ...
  LOG_LOCK();
  if (cb) _print_cb = cb;
  else _print_cb = eina_log_print_cb_stderr;
  _print_cb_data = data;
  eina_log_print_prefix_update();
  LOG_UNLOCK();
#else
...
EINA_API void
eina_log_color_disable_set(Efl_Bool disabled)
{
#ifdef EINA_ENABLE_LOG
  ...
  for (i = 0; i < _log_domains_count; i++)
  {
    if (_log_domains[i].deleted) continue;
    ...
    else domain->domain_str = eina_log_domain_str_get(domain->name, NULL);
  }
...
EINA_API void
eina_log_file_disable_set(Efl_Bool disabled)
{
#ifdef EINA_ENABLE_LOG
...
EINA_API void
eina_log_function_disable_set(Efl_Bool disabled)
{
#ifdef EINA_ENABLE_LOG
...
EINA_API void
eina_log_abort_on_critical_set(Efl_Bool abort_on_critical)
{
#ifdef EINA_ENABLE_LOG
...
eina_log_domain_level_get(const char *domain_name)
{
  ...
  EINA_INLIST_FOREACH(_glob_list, pending)
  {
    if (eina_fnmatch(pending->name, domain_name, 0)) return pending->level;
  }
...
#  ifdef _WIN32
  /*
    ...
    */
  if (!_eina_log_win32_is_console) fflush(stderr);
#  endif
#else
  (void)d;
  (void)level;
...
static int (*_eina_sd_journal_send_with_location)(const char *file,
                                                  const char *line,
                                                  const char *func,
                                                  const char *format,
                                                  ...) = NULL;
...
#  endif
    _eina_sd_journal_send_with_location(file_prefixed,
                                        line_str,
                                        fnc,
                                        "PRIORITY=%i",
                                        level,
...
#  ifdef EINA_LOG_BACKTRACE
  else
  {
    ...
    bts = eina_strbuf_new();
    for (i = 0; i < btlen; i++)
      if (i + 1 == btlen) eina_strbuf_append_printf(bts, "[%s]", strings[i]);
      else eina_strbuf_append_printf(bts, "[%s], ", strings[i]);
...
#  ifdef _WIN32
  ...
  if (fp == stderr) handle = GetStdHandle(STD_ERROR_HANDLE);
  else if (fp == stdout) handle = GetStdHandle(STD_OUTPUT_HANDLE);
  else
  {
...
EINA_API void
eina_log_timing(int domain, Eina_Log_State state, const char *phase)
{
#ifdef EINA_SAFETY_CHECKS
  Eina_Log_Domain *d;
#endif
  ...
  if (!t->phase && state == EINA_LOG_STATE_STOP) return;

  if (t->phase == EINA_LOG_STATE_INIT && phase == EINA_LOG_STATE_SHUTDOWN)
    return;

  if (state == EINA_LOG_STATE_START && t->phase &&
