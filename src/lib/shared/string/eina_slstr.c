...
{
  if (_slstr_init++) return EFL_TRUE;
...
{
  if (_slstr_init == 0) return EFL_FALSE;
  if (--_slstr_init) return EFL_TRUE;
...
_slstr_freeq_get(Efl_Bool nocreate)
{
  if (eina_main_loop_is()) return _slstr_main_fq;
  else
  {
    Eina_FreeQ *fq;
