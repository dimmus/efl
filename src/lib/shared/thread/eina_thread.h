 ...
 * @return EFL_TRUE if it succeeds in setting the name or EFL_FALSE
 *         otherwise.
 * @since 1.16
 */
EINA_API Efl_Bool eina_thread_name_set(Eina_Thread t, const char *name);
 ...
 * (eina_thread_cancellable_set() with EFL_TRUE as first parameter)
 ...
 *
 * @return EFL_FALSE if thread was not running, EFL_TRUE
 *         otherwise. Note that if a thread is not cancellable and it
 *         is running, this function will return EFL_TRUE!
 *
 * @since 1.19
 */
EINA_API Efl_Bool eina_thread_cancel(Eina_Thread t);
 ...
 * pair for you.
 *
 * @param[in] cancellable If EFL_TRUE, this thread will be accept
 *        cancellation requests. If EFL_FALSE -- the default, it will
 *        ignore cancellation requests.
 * @param[in] was_cancellable If non-NULL, will return the previous state,
 *        shall you want to restore.
 *
 * @return EFL_TRUE if it succeeds in setting the cancellable state
 *        or EFL_FALSE otherwise.
 *
 ...
 * EINA_THREAD_CLEANUP_POP(EFL_TRUE) is used.
 *
 ...
 * @note If the block within EINA_THREAD_CLEANUP_PUSH() and
 *       EINA_THREAD_CLEANUP_POP() returns, the cleanup callback will
 *       @b not be executed even if exec_cleanup is EFL_TRUE! To
 *       avoid problems prefer to use eina_thread_cancellable_run()!
 *
 * @param[in] exec_cleanup if EFL_TRUE, the function registered with
