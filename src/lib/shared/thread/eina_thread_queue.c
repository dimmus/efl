...
struct _Eina_Thread_Queue_Msg_Block
{
  ...
  Efl_Bool              full : 1; // is this block full yet?
...
#define MIN_SIZE                                        \
    ((int)(4096 - sizeof(Eina_Thread_Queue_Msg_Block) + \
...
_eina_thread_queue_wake(Eina_Thread_Queue *thq)
{
  if (!eina_semaphore_release(&(thq->sem), 1))
    ERR("Thread queue semaphore release/wakeup faile - bad things will "
        "happen");
}
...
_eina_thread_queue_msg_alloc(Eina_Thread_Queue            *thq,
                             int                           size,
                             Eina_Thread_Queue_Msg_Block **blkret)
{
  Eina_Thread_Queue_Msg_Block *blk;
  ...
  if (!thq->data)
  {
    if (size < MIN_SIZE) thq->data = _eina_thread_queue_msg_block_new(MIN_SIZE);
    else thq->data = _eina_thread_queue_msg_block_new(size);
    thq->last = thq->data;
  }
  blk = thq->last;
  if (blk->full)
  {
    if (size < MIN_SIZE) blk->next = _eina_thread_queue_msg_block_new(MIN_SIZE);
  ...
  if ((blk->size - blk->last) >= size)
  {
    ...
    msg =
      (Eina_Thread_Queue_Msg *)((char *)(&(blk->data[0])) + (blk->last - size));
  }
  else
  {
    if (size < MIN_SIZE) blk->next = _eina_thread_queue_msg_block_new(MIN_SIZE);
...
static Eina_Thread_Queue_Msg *
_eina_thread_queue_msg_fetch(Eina_Thread_Queue            *thq,
                             Eina_Thread_Queue_Msg_Block **blkret)
{
  Eina_Thread_Queue_Msg_Block *blk;
...
eina_thread_queue_init(void)
{
  _eina_thread_queue_log_dom =
    eina_log_domain_register("eina_thread_queue", EINA_LOG_COLOR_DEFAULT);
...
eina_thread_queue_send_done(Eina_Thread_Queue *thq, void *allocref)
{
  ...
  if (thq->parent)
  {
    ...
    msg = eina_thread_queue_send(thq->parent,
                                 sizeof(Eina_Thread_Queue_Msg_Sub),
                                 &ref);
    if (msg)
    {
      msg->queue = thq;
...
EINA_API void
eina_thread_queue_parent_set(Eina_Thread_Queue *thq,
