...
Efl_Bool fork_resetting;

EINA_API void
_eina_lock_debug_abort(int err, const char *fn, volatile const void *ptr)
{
...
EINA_API void
_eina_lock_debug_deadlock(const char *fn, volatile const void *ptr)
{
...
#ifdef EINA_HAVE_DEBUG_THREADS
  fprintf(stderr,
          "EINA ERROR: DEADLOCK on lock %p, locked: %i, by %ti\n",
          mutex,
          (int)mutex->locked,
          (ptrdiff_t)mutex->lock_thread_id);
...
EINA_API Efl_Bool
_eina_lock_new(Eina_Lock *mutex, Efl_Bool recursive)
{
  pthread_mutexattr_t attr;
  Efl_Bool            ok = EFL_FALSE;

  if (pthread_mutexattr_init(&attr) != 0) return EFL_FALSE;
  if (recursive)
  {
...
#ifdef EINA_HAVE_DEBUG_THREADS
  else if (pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_ERRORCHECK) != 0)
    goto fail_release;
#endif
...
#  endif
  ...
  if (ok != 0)
  {
    ...
    if (ok == EBUSY)
      fprintf(stderr,
              "EINA ERROR: eina_condition_new on already initialized "
              "Eina_Condition\n");
#endif
    return EFL_FALSE;
  }
...
_eina_rwlock_new(Eina_RWLock *mutex)
{
  ...
  else if ((ok == EAGAIN) || (ok == ENOMEM)) return EFL_FALSE;
...
_eina_semaphore_new(Eina_Semaphore *sem, int count_init)
{
  if (sem && (count_init >= 0))
  {
    ...
    kern_return_t kr =
      semaphore_create(mach_task_self(), sem, SYNC_POLICY_FIFO, count_init);
    return (kr == KERN_SUCCESS) ? EFL_TRUE : EFL_FALSE;
#else
    return (sem_init(sem, 0, count_init) == 0) ? EFL_TRUE : EFL_FALSE;
#endif
  }
  return EFL_FALSE;
}
...
_eina_semaphore_free(Eina_Semaphore *sem)
{
  if (sem)
  {
    ...
    return (semaphore_destroy(mach_task_self(), *sem) == KERN_SUCCESS)
             ...
             : EFL_FALSE;
#else
    return (sem_destroy(sem) == 0) ? EFL_TRUE : EFL_FALSE;
#endif
  }
  return EFL_FALSE;
}
