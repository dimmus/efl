...
struct Thread
{
  ...
  void *
    data; /* on entry, the thread function argument, on exit, the return value */
  Eina_Thread_Cb    fn; /* the thread function */
  Eina_Array       *cleanup_fn;
  Eina_Array       *cleanup_arg;
  unsigned          id; /* thread id */
  Efl_Bool          free_on_exit; /* free the structure when thread exit */
  volatile Efl_Bool cancel; /* the cancel event handle */
  volatile Efl_Bool cancellable; /* is cancel enabled? */
};
...
eina_thread_self(void)
{
  ...
  if (!self)
  {
    ...
    self->free_on_exit = EFL_TRUE;
    self->cleanup_fn   = eina_array_new(4);
    self->cleanup_arg  = eina_array_new(4);
    if (tls_thread_self) TlsSetValue(tls_thread_self, self);
  }
                   ...
                   const void          *data)
{
  ...
  if (!thr) return EFL_FALSE;

  thr->data = (void *)data;
  thr->fn   = func;

  thr->handle =
    (HANDLE)_beginthreadex(NULL, 0, thread_fn, thr, CREATE_SUSPENDED, &thr->id);
  if (!thr->handle) goto fail;
  ...
  if (!SetThreadPriority(thr->handle, priority)) goto fail;
  ...
  if ((!thr->cleanup_fn) || (!thr->cleanup_arg)) goto fail;

  InitializeCriticalSection(&thr->cancel_lock);

  if (!ResumeThread(thr->handle)) goto cs_fail;
...
eina_thread_cancel_checkpoint(void)
{
  ...
  if (cancel)
  {
    ...
    {
      Eina_Thread_Cleanup_Cb fn =
        (Eina_Thread_Cleanup_Cb)eina_array_pop(t->cleanup_fn);
...
eina_thread_cleanup_push(Eina_Thread_Cleanup_Cb fn, void *data)
{
  ...
  if (!eina_array_push(t->cleanup_fn, fn)) return EFL_FALSE;
...
{
  if (!eina_main_loop_is()) return EFL_FALSE;

  tls_thread_self = TlsAlloc();
  if (TLS_OUT_OF_INDEXES == tls_thread_self) return EFL_FALSE;
  ...
  main_thread.cancellable = EFL_FALSE;
  main_thread.cancel      = EFL_FALSE;
