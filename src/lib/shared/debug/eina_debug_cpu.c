...
#  if defined(__clockid_t_defined)
            ...
            for (i = 0; i < _eina_debug_thread_active_num; i++)
            {
                ...
                pthread_getcpuclockid(_eina_debug_thread_active[i].thread,
                                      &cid);
...
#  if defined(__clockid_t_defined)
        ...
        if (tim_span > 0)
        {
            ...
            for (i = 0; i < _eina_debug_thread_active_num; i++)
            {
                ...
                tim1 =
                    (_eina_debug_thread_active[i].clok.tv_sec * 1000000000LL) +
                ...
                if (cpu != _eina_debug_thread_active[i].val)
                {
                    ...
                    snprintf(path,
                             sizeof(path),
                             "%i",
                             _eina_debug_thread_active[i].val);
            ...
            if (prev_threads_redo)
            {
                ...
                if (prev_threads_num != _eina_debug_thread_active_num)
                {
                    ...
                    prev_threads =
                        malloc(prev_threads_num * sizeof(Eina_Debug_Thread));
                }
               ...
               int size                    EFL_UNUSED)
{
    ...
    err = eina_thread_create(&_sysmon_thread,
                             EINA_THREAD_NORMAL,
                             -1,
                             _sysmon,
                             NULL);
