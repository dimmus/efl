...
#  if defined(__clockid_t_defined)
      ...
      for (i = 0; i < _eina_debug_thread_active_num; i++)
      {
        ...
        pthread_getcpuclockid(_eina_debug_thread_active[i].thread, &cid);
...
#  if defined(__clockid_t_defined)
    ...
    if (tim_span > 0)
    {
      ...
      if (prev_threads_num != _eina_debug_thread_active_num)
        prev_threads_redo = EFL_TRUE;
      else
      {
        // XXX: isolate this out of hot path
        for (i = 0; i < _eina_debug_thread_active_num; i++)
        {
          if (_eina_debug_thread_active[i].thread != prev_threads[i].thread)
          {
            prev_threads_redo = EFL_TRUE;
            break;
          }
      ...
      if (prev_threads_redo)
      {
        ...
        for (j = 0; j < prev_threads_num; j++)
        {
          for (i = 0; i < _eina_debug_thread_active_num; i++)
          {
            if (prev_threads[j].thread == _eina_debug_thread_active[i].thread)
              break;
          }
          ...
          if (i == _eina_debug_thread_active_num)
          {
            ...
            snprintf(buf,
                     sizeof(buf),
                     "*CPUUSED %llu",
                     (unsigned long long)prev_threads[i].thread);
        ...
        if (prev_threads_num != _eina_debug_thread_active_num)
        {
          ...
          prev_threads = malloc(prev_threads_num * sizeof(Eina_Debug_Thread));
        }
        // store the thread handles/id's
        for (i = 0; i < prev_threads_num; i++)
          prev_threads[i].thread = _eina_debug_thread_active[i].thread;
      }
      eina_spinlock_release(&_eina_debug_thread_lock);
      t_last = t_now;
    }
...
static Efl_Bool
_cpufreq_on_cb(Eina_Debug_Session *session EFL_UNUSED,
               int cid                     EFL_UNUSED,
               ...
               int size                    EFL_UNUSED)
{
  ...
  err =
    eina_thread_create(&_sysmon_thread, EINA_THREAD_NORMAL, -1, _sysmon, NULL);
...
static Efl_Bool
_cpufreq_off_cb(Eina_Debug_Session *session EFL_UNUSED,
                int cid                     EFL_UNUSED,
                void *buffer                EFL_UNUSED,
                int size                    EFL_UNUSED)
{
...
{
#ifndef _WIN32
  if (_eina_debug_sysmon_active) _stop_cpu_thread();
