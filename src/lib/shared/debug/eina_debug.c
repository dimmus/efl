...
extern Efl_Bool eina_module_init(void);
extern Efl_Bool eina_mempool_init(void);
extern Efl_Bool eina_list_init(void);

extern Eina_Spinlock _eina_debug_thread_lock;
static Eina_List    *sessions;

static Efl_Bool _debug_disabled = EFL_FALSE;
...
struct _Eina_Debug_Session
{
    ...
    Efl_Bool               deleted : 1; /* set if session is dead */
};
...
EINA_API int
eina_debug_session_send(Eina_Debug_Session *session,
                        int                 dest,
                        int                 op,
                        void               *data,
                        int                 size)
...
#  endif
    ...
    memcpy(buf + 4, &pid, 4);
    if (app_name) memcpy(buf + 8, app_name, strlen(app_name) + 1);
    else buf[8] = '\0';
...
_packet_receive(Eina_Debug_Session *session, unsigned char **buffer)
{
    ...
    if (rret == 4)
    {
        ...
        {
            e_debug("Packet too big: %d. The maximum allowed is %d",
                    size,
                    EINA_DEBUG_MAX_PACKET_SIZE);
            rret = -1;
            goto end;
        }
        ...
        if (packet_buf)
        {
            ...
            {
                rret = read(session->fd,
                            packet_buf + cur_packet_size,
                            size - cur_packet_size);
                if (rret <= 0)
                {
                        ...
                        Eina_Debug_Cb       cb)
{
    if (session->cbs_length < op_id + 1)
    {
        ...
        session->cbs =
            realloc(session->cbs, session->cbs_length * sizeof(Eina_List *));
 ...
 */
static Efl_Bool
_callbacks_register_cb(Eina_Debug_Session *session,
                       int src_id          EFL_UNUSED,
                       void               *buffer,
                       int                 size)
{
    ...
    EINA_LIST_FOREACH(session->opcode_reply_infos, itr, info2)
    {
        if (info2 == info)
        {
            ...
            if (info->status_cb) info->status_cb(info->status_data, EFL_TRUE);
            return EFL_TRUE;
        }
    }
...
_opcodes_unregister_all(Eina_Debug_Session *session)
{
    ...
    EINA_LIST_FOREACH(session->opcode_reply_infos, l, info)
    {
        ...
        if (info->status_cb) info->status_cb(info->status_data, EFL_FALSE);
    }
}
...
EINA_API Eina_Debug_Session *
eina_debug_local_connect(Efl_Bool is_master)
{
#ifndef _WIN32
    ...
    eina_strlcpy(socket_unix.sun_path, buf, sizeof(socket_unix.sun_path) - 1);
...
#  endif
    ...
    while (1)
    {
        ...
        if (size > 0)
        {
            ...
            if (EFL_TRUE != session->dispatch_cb(session, buffer))
            {
                // something we don't understand
                e_debug("EINA DEBUG ERROR: Unknown command");
            }
            /* Free the buffer only if the default dispatcher is used */
            if (session->dispatch_cb == eina_debug_dispatch) free(buffer);
        }
        else
        {
 ...
 */
EINA_API void
eina_debug_opcodes_register(Eina_Debug_Session         *session,
                            const Eina_Debug_Opcode     ops[],
                            Eina_Debug_Opcode_Status_Cb status_cb,
                            void                       *data)
{
    ...
    session->opcode_reply_infos =
        eina_list_append(session->opcode_reply_infos, info);

    /* Send only if session's fd connected.
    * Otherwise, it will be sent when connected */
    if (session->fd != -1) _opcodes_registration_send(session, info);
}
...
eina_debug_dispatch(Eina_Debug_Session *session, void *buffer)
{
    ...
    {
        if (!cb) continue;
        Efl_Bool ret = cb(session,
                          hdr->cid,
                          (unsigned char *)buffer + sizeof(*hdr),
                          hdr->size - sizeof(*hdr));
        if (ret == EFL_FALSE) return ret;
    }
    return EFL_TRUE;
}
...
eina_debug_init(void)
{
    ...
    if (getuid() != geteuid()) return EFL_TRUE;
#endif
    ...
    {
        eina_debug_local_connect(EFL_FALSE);
    }
    return EFL_TRUE;
}
