 ...
 * @param[in] start_stream Set as #EFL_TRUE for the initial call and
 *            #EFL_FALSE for the remaining stream data.
 ...
 * setting @p start_stream to #EFL_FALSE.  For the initial call, set
 * @p seed to @c 0xffffffff and @p start_stream to #EFL_TRUE.
 *
 ...
 * unsigned int crc = eina_crc(tmp, strlen(tmp), 0xffffffff, EFL_TRUE);
 * @endcode
 *
 ...
 * char *tmp1 = "hello "
 * char *tmp2 = "world"
 * unsigned int crc = eina_crc(tmp1, strlen(tmp1), 0xffffffff, EFL_TRUE);
 * crc = eina_crc(tmp2, strlen(tmp2), crc, EFL_FALSE);
 * @endcode
 *
 * @since 1.15
 */
