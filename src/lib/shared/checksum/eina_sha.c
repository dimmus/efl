...
#define SHSH(n, v) ((((v) << (n)) & 0xffffffff) | ((v) >> (32 - (n))))
  ...
  int                  idx, left, i;
  const unsigned int   magic[4] = { 0x5a827999,
                                    0x6ed9eba1,
                                    0x8f1bbcdc,
                                    0xca62c1d6 };
  ...
  for (left = size, d = data; left > 0; left--, d++)
  {
    ...
    if ((idx == 64) || (left == 1))
    {
      ...
      for (i = 16; i < 80; i++)
        word[i] =
          SHSH(1, word[i - 3] ^ word[i - 8] ^ word[i - 14] ^ word[i - 16]);
      wa = digest[0];
      wb = digest[1];
      wc = digest[2];
      wd = digest[3];
      ...
      for (i = 0; i < 80; i++)
      {
        if (i < 20)
          t =
            SHSH(5, wa) + ((wb & wc) | ((~wb) & wd)) + we + word[i] + magic[0];
        else if (i < 40)
          t = SHSH(5, wa) + (wb ^ wc ^ wd) + we + word[i] + magic[1];
        else if (i < 60)
          t = SHSH(5, wa) + ((wb & wc) | (wb & wd) | (wc & wd)) + we + word[i] +
              magic[2];
        else if (i < 80)
  ...
  t         = eina_htonl(digest[0]);
  digest[0] = t;
  t         = eina_htonl(digest[1]);
  digest[1] = t;
  t         = eina_htonl(digest[2]);
  digest[2] = t;
  t         = eina_htonl(digest[3]);
  digest[3] = t;
  t         = eina_htonl(digest[4]);
  digest[4] = t;
