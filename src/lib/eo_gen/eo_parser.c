        ...
        case TOK_NUMBER:
            {
                ...
                FILL_BASE(expr->base, ls, line, col, EXPRESSION);
                expr->type =
                    ls->t.kw + 1; /* map Numbers from lexer to expr type */
                    ...
                    default:
                        {
                            ...
                            eo_lexer_dtor_push(ls,
                                               EINA_FREE_CB(eina_strbuf_free),
                                               buf);
                            expr       = eo_lexer_expr_new(ls);
                            expr->type = EOLIAN_EXPR_NAME;
                            parse_name(ls, buf);
                            expr->value.s = eina_stringshare_add(
    ...
    else
    {
        ...
        if (eo_lexer_is_type_keyword(ls->t.kw))
        {
                ...
                (tpid >= KW_slice && tpid <= KW_rw_slice) || (tpid == KW_hash))
            {
                ...
                check_next(ls, '<');
                if (tpid == KW_future)
                    def->base_type = eo_lexer_type_release(
                        ls,
                        parse_type_void(ls, EFL_TRUE, EFL_TRUE));
                else
                    def->base_type = eo_lexer_type_release(
                        ls,
                        parse_type(ls, EFL_TRUE, EFL_TRUE));
                ...
                {
                    check_next(ls, ',');
                    def->base_type->next_type = eo_lexer_type_release(
                        ls,
                        parse_type(ls, EFL_TRUE, EFL_TRUE));
                    if ((def->base_type->next_type->move =
                             ls->t.kw == KW_at_move))
                        eo_lexer_get(ls);
                }
            ...
            const Eolian_Function *func)
{
    ...
    Eolian_Function_Parameter *par =
        calloc(1, sizeof(Eolian_Function_Parameter));
...
{
    int      line, col;
    Efl_Bool has_return = EFL_FALSE, has_keys = EFL_FALSE,
            ...
            case KW_values:
                {
                    ...
                    stor = is_get ? &prop->prop_values_get
                                  : &prop->prop_values_set;
...
parse_class_body(Eo_Lexer *ls, Eolian_Class_Type type)
{
    Efl_Bool has_c_prefix = EFL_FALSE, has_event_c_prefix = EFL_FALSE,
             has_data = EFL_FALSE, has_methods = EFL_FALSE,
             has_parts = EFL_FALSE, has_implements = EFL_FALSE,
             has_constructors = EFL_FALSE, has_events = EFL_FALSE;
...
parse_unit(Eo_Lexer *ls, Efl_Bool eot)
{
    switch (ls->t.kw)
    {
        ...
        case KW_error:
            database_error_add(ls->unit,
                               eo_lexer_error_release(ls, parse_error(ls)));
            break;
        case KW_struct:
        case KW_enum:
            {
...
postparams:
                ...
                if (!is_enum && ls->t.token == ';')
                {
                    ...
                    FILL_DOC(ls, def, doc);
                    FILL_BASE(def->base, ls, line, col, TYPEDECL);
                    database_struct_add(ls->unit,
                                        eo_lexer_typedecl_release(ls, def));
                    break;
                }
                if (is_enum)
                    parse_enum(ls,
            ...
            case KW_hash_version:
                {
                    ...
                    if (!parsing_header)
                        eo_lexer_syntax_error(
                            ls,
                            "header keyword outside of unit header");
                    eo_lexer_get(ls);
