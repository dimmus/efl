...
#define CASE_LOCK(ls, var, msg)                              \
    if (has_##var) eo_lexer_syntax_error(ls, "double " msg); \
    has_##var = EFL_TRUE;
...
{
  if (ls->t.token != token) error_expected(ls, token);
}

static void
check_kw(Eo_Lexer *ls, int kw)
{
  if (ls->t.kw != kw) error_expected(ls, TOK_VALUE + kw);
}
...
check_match(Eo_Lexer *ls, int what, int who, int where, int col)
{
  ...
  {
    if (where == ls->line_number) error_expected(ls, what);
    else
    {
...
_eolian_decl_get(Eo_Lexer *ls, const char *name)
{
  ...
  if (!obj) obj = eina_hash_find(ls->state->staging.unit.objects, name);
...
redef_error(Eo_Lexer *ls, Eolian_Object *obj, Eolian_Object *nobj)
{
  ...
  else snprintf(fbuf, sizeof(fbuf), "%d:%d", obj->line, obj->column);

  if (nobj->type != obj->type)
    snprintf(buf,
             sizeof(buf),
             "%s '%s' redefined as %s (originally at %s)",
             _eolian_decl_name_get(obj),
             obj->name,
             _eolian_decl_name_get(nobj),
             fbuf);
  else
    snprintf(buf,
             sizeof(buf),
             "%s '%s' redefined (originally at %s)",
             _eolian_decl_name_get(obj),
             obj->name,
             fbuf);
...
static Efl_Strbuf *
parse_name(Eo_Lexer *ls, Efl_Strbuf *buf)
{
  check(ls, TOK_VALUE);
...
get_binop_id(int tok)
{
  ...
  {
    case '+':
      return EOLIAN_BINOP_ADD;
    case '-':
      return EOLIAN_BINOP_SUB;
    case '*':
      return EOLIAN_BINOP_MUL;
    case '/':
      return EOLIAN_BINOP_DIV;
    case '%':
      return EOLIAN_BINOP_MOD;

    case TOK_EQ:
      return EOLIAN_BINOP_EQ;
    case TOK_NQ:
      return EOLIAN_BINOP_NQ;
    case '>':
      return EOLIAN_BINOP_GT;
    case '<':
      return EOLIAN_BINOP_LT;
    case TOK_GE:
      return EOLIAN_BINOP_GE;
    case TOK_LE:
      return EOLIAN_BINOP_LE;

    case TOK_AND:
      return EOLIAN_BINOP_AND;
    case TOK_OR:
      return EOLIAN_BINOP_OR;

    case '&':
      return EOLIAN_BINOP_BAND;
    case '|':
      return EOLIAN_BINOP_BOR;
    case '^':
      return EOLIAN_BINOP_BXOR;

    case TOK_LSH:
      return EOLIAN_BINOP_LSH;
    case TOK_RSH:
      return EOLIAN_BINOP_RSH;

    default:
      return EOLIAN_BINOP_INVALID;
  }
}
...
get_unop_id(int tok)
{
  ...
  {
    case '-':
      return EOLIAN_UNOP_UNM;
    case '+':
      return EOLIAN_UNOP_UNP;
    case '!':
      return EOLIAN_UNOP_NOT;
    case '~':
      return EOLIAN_UNOP_BNOT;

    default:
      return EOLIAN_UNOP_INVALID;
  }
}
          ...
          default:
            {
              ...
              parse_name(ls, buf);
              expr->value.s = eina_stringshare_add(eina_strbuf_string_get(buf));
...
parse_expr_bin(Eo_Lexer *ls, int min_prec)
{
  ...
  for (;;)
  {
    ...
    int                    prec = binprec[op];
    if ((op == EOLIAN_BINOP_INVALID) || (prec < 0) || (prec < min_prec)) break;
    eo_lexer_get(ls);
...
static Eolian_Type *
parse_type_void(Eo_Lexer *ls, Efl_Bool allow_ptr, Efl_Bool allow_const);

static Eolian_Type *
parse_type(Eo_Lexer *ls, Efl_Bool allow_ptr, Efl_Bool allow_const)
{
  Eolian_Type *ret;
             ...
             const char *cname)
{
  ...
  if (freefunc)
  {
    ...
    def->ownable  = EFL_TRUE;
  }
  if (cname) def->base.c_name = eina_stringshare_ref(cname);
  else def->base.c_name = make_c_name(name);
  ...
  else if (freefunc || cname) eo_lexer_dtor_pop(ls);
  check_next(ls, '{');
  FILL_DOC(ls, def, doc);
  while (ls->t.token != '}')
  {
    ...
    eo_lexer_get(ls);
    check_next(ls, ':');
    tp = parse_type(ls, EFL_TRUE, EFL_TRUE);
    ...
    Efl_Bool has_move = EFL_FALSE, has_by_ref = EFL_FALSE;
    for (;;)
      switch (ls->t.kw)
      {
        case KW_at_by_ref:
          CASE_LOCK(ls, by_ref, "by_ref qualifier");
          fdef->by_ref = EFL_TRUE;
          eo_lexer_get(ls);
          break;
        case KW_at_move:
           ...
           const char *cname)
{
  ...
  for (;;)
  {
    ...
    {
      ls->expr_mode = EFL_TRUE;
      eo_lexer_get(ls);
      fdef->value           = parse_expr(ls);
      fdef->is_public_value = EFL_TRUE;
      ls->expr_mode         = EFL_FALSE;
      prev_fl               = fdef;
      fl_nadd               = 0;
      eo_lexer_expr_release_ref(ls, fdef->value);
    }
    Efl_Bool want_next = (ls->t.token == ',');
    if (want_next) eo_lexer_get(ls);
    FILL_DOC(ls, fdef, doc);
    if (def->doc && fdef->doc && def->doc->since && !fdef->doc->since)
      fdef->doc->since = eina_stringshare_ref(def->doc->since);
    if (!want_next || ls->t.token == '}') break;
  }
  check_match(ls, '}', '{', bline, bcolumn);
  FILL_BASE(def->base, ls, line, column, TYPEDECL);
...
parse_type_void(Eo_Lexer *ls, Efl_Bool allow_ptr, Efl_Bool allow_const)
{
  ...
  switch (ls->t.kw)
  {
    case KW_const:
      {
        if (!allow_const) break;
        int pline, pcol;
        eo_lexer_get(ls);
        pline = ls->line_number;
        pcol  = ls->column;
        check_next(ls, '(');
        def = parse_type_void(ls, allow_ptr, EFL_FALSE);
        FILL_BASE(def->base, ls, line, col, TYPE);
        def->is_const = EFL_TRUE;
        check_match(ls, ')', '(', pline, pcol);
        return def;
      }
    case KW_ptr:
      {
        ...
        pcol  = ls->column;
        check_next(ls, '(');
        def = parse_type_void(ls, EFL_FALSE, allow_const);
        FILL_BASE(def->base, ls, line, col, TYPE);
        def->is_ptr = EFL_TRUE;
        check_match(ls, ')', '(', pline, pcol);
        return def;
  ...
  else
  {
    ...
    if (eo_lexer_is_type_keyword(ls->t.kw))
    {
          ...
          (tpid >= KW_slice && tpid <= KW_rw_slice) || (tpid == KW_hash))
      {
        ...
        if (tpid == KW_future)
          def->base_type =
            eo_lexer_type_release(ls, parse_type_void(ls, EFL_TRUE, EFL_TRUE));
        else
          def->base_type =
            eo_lexer_type_release(ls, parse_type(ls, EFL_TRUE, EFL_TRUE));
        /* view-only types are not allowed to own the contents */
        if (tpid == KW_array || tpid == KW_hash || tpid == KW_list ||
            tpid == KW_future)
          if ((def->base_type->move = ls->t.kw == KW_at_move)) eo_lexer_get(ls);
        if (tpid == KW_hash)
        {
          check_next(ls, ',');
    ...
    else
    {
      ...
      if (!compare_class_file(bnm, fnm))
      {
        ...
        {
          database_defer(ls->state, fnm, EFL_TRUE);
...
parse_typedef(Eo_Lexer *ls)
{
  ...
  Efl_Bool has_extern = EFL_FALSE, has_beta = EFL_FALSE, has_c_name = EFL_FALSE;
  for (;;)
    switch (ls->t.kw)
    {
      case KW_at_extern:
        CASE_LOCK(ls, extern, "extern qualifier");
        def->is_extern = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_beta:
        CASE_LOCK(ls, beta, "beta qualifier");
        def->base.is_beta = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_c_name:
...
tags_done:
  ...
  else def->base.c_name = make_c_name(def->base.name);
  ...
  check_next(ls, ':');
  def->base_type =
    eo_lexer_type_release(ls, parse_type(ls, EFL_FALSE, EFL_FALSE));
  check_next(ls, ';');
  FILL_DOC(ls, def, doc);
  eo_lexer_dtor_pop(ls);
  return def;
             ...
             Efl_Bool    is_funcptr)
{
  eo_lexer_get(ls);
  check_next(ls, ':');
  if (allow_void) ret->type = parse_type_void(ls, EFL_TRUE, EFL_TRUE);
  else ret->type = parse_type(ls, EFL_TRUE, EFL_TRUE);
  ret->doc             = NULL;
  ret->default_ret_val = NULL;
  ret->no_unused       = EFL_FALSE;
  ret->move            = EFL_FALSE;
  ret->by_ref          = EFL_FALSE;
  ...
  Efl_Bool has_no_unused = EFL_FALSE, has_move = EFL_FALSE,
           has_by_ref = EFL_FALSE;
  if (!is_funcptr)
    for (;;)
      switch (ls->t.kw)
      {
...
static void
parse_param(Eo_Lexer              *ls,
            Eina_List            **params,
            Efl_Bool               allow_inout,
            Efl_Bool               is_vals,
            const Eolian_Function *func)
{
  Efl_Bool has_optional = EFL_FALSE, has_move = EFL_FALSE,
           has_by_ref            = EFL_FALSE;
  ...
  *params = eina_list_append(*params, par);
  ...
  {
    if (par->param_dir == EOLIAN_PARAMETER_OUT ||
        par->param_dir == EOLIAN_PARAMETER_INOUT)
    {
  ...
  par->type = eo_lexer_type_release(ls, parse_type(ls, EFL_TRUE, EFL_TRUE));
type_done:
  if ((is_vals || (par->param_dir == EOLIAN_PARAMETER_OUT)) &&
      (ls->t.token == '('))
  {
    int line = ls->line_number, col = ls->column;
    ls->expr_mode = EFL_TRUE;
...
{
  if ((ls->klass->type != EOLIAN_CLASS_ABSTRACT) &&
      (ls->klass->type != EOLIAN_CLASS_MIXIN))
    eo_lexer_syntax_error(
      ls,
      "@pure_virtual only allowed in abstract classes or mixins");
}
...
{
  int      line, col;
  Efl_Bool has_return = EFL_FALSE, has_keys = EFL_FALSE, has_values = EFL_FALSE,
           has_protected = EFL_FALSE, has_virtp = EFL_FALSE;
  Efl_Bool is_get = (ls->t.kw == KW_get);
  if (is_get)
  {
    if (prop->base.file) eina_stringshare_del(prop->base.file);
    FILL_BASE(prop->base, ls, ls->line_number, ls->column, FUNCTION);
    if (prop->type == EOLIAN_PROP_SET) prop->type = EOLIAN_PROPERTY;
    else prop->type = EOLIAN_PROP_GET;
  }
  else
  {
    FILL_BASE(prop->set_base, ls, ls->line_number, ls->column, FUNCTION);
    if (prop->type == EOLIAN_PROP_GET) prop->type = EOLIAN_PROPERTY;
    else prop->type = EOLIAN_PROP_SET;
  }
  eo_lexer_get(ls);
  for (;;)
    switch (ls->t.kw)
    {
      case KW_at_pure_virtual:
        ...
        if (is_get) prop->impl->get_pure_virtual = EFL_TRUE;
        else prop->impl->set_pure_virtual = EFL_TRUE;
        eo_lexer_get(ls);
        break;
  ...
  for (;;)
    switch (ls->t.kw)
    {
      case KW_return:
        ...
        Eo_Ret_Def ret;
        parse_return(ls, &ret, is_get, EFL_TRUE, EFL_FALSE);
      ...
      case KW_keys:
        {
          ...
          parse_params(ls, stor, EFL_FALSE, EFL_FALSE, prop);
          break;
        }
      case KW_values:
        {
          ...
          parse_params(ls, stor, EFL_FALSE, EFL_TRUE, prop);
          break;
        }
      default:
        goto end;
...
static void
_func_pure_virtual_set(Eo_Lexer *ls, Eolian_Function *foo_id, Efl_Bool virt)
{
  if (ls->klass->type != EOLIAN_CLASS_INTERFACE && !virt) return;

  if (foo_id->type == EOLIAN_PROP_GET || foo_id->type == EOLIAN_METHOD)
    foo_id->impl->get_pure_virtual = EFL_TRUE;
  else if (foo_id->type == EOLIAN_PROP_SET)
    foo_id->impl->set_pure_virtual = EFL_TRUE;
  else if (foo_id->type == EOLIAN_PROPERTY)
    foo_id->impl->get_pure_virtual = foo_id->impl->set_pure_virtual = EFL_TRUE;
}
...
parse_property(Eo_Lexer *ls)
{
  ...
  Efl_Bool has_get = EFL_FALSE, has_set = EFL_FALSE, has_keys = EFL_FALSE,
           has_values = EFL_FALSE, has_protected = EFL_FALSE,
           has_class = EFL_FALSE, has_beta = EFL_FALSE, has_virtp = EFL_FALSE;
  ...
  impl->base.name =
    eina_stringshare_printf("%s.%s", ls->klass->base.name, prop->base.name);
  eo_lexer_get(ls);
  for (;;)
    switch (ls->t.kw)
    {
      ...
      case KW_at_static:
        ...
        prop->is_static = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_beta:
        CASE_LOCK(ls, beta, "beta qualifier");
        prop->base.is_beta = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_pure_virtual:
        check_abstract_pure_virtual(ls);
...
body:
  ...
  for (;;)
    switch (ls->t.kw)
    {
      case KW_get:
        ...
        impl->is_prop_get = EFL_TRUE;
        parse_accessor(ls, prop);
        break;
      case KW_set:
        CASE_LOCK(ls, set, "set definition")
        impl->is_prop_set = EFL_TRUE;
        parse_accessor(ls, prop);
        break;
      case KW_keys:
        CASE_LOCK(ls, keys, "keys definition")
        parse_params(ls, &prop->prop_keys, EFL_FALSE, EFL_FALSE, prop);
        break;
      case KW_values:
        CASE_LOCK(ls, values, "values definition")
        parse_params(ls, &prop->prop_values, EFL_FALSE, EFL_TRUE, prop);
        break;
      default:
        goto end;
    }
  ...
  if (!has_get && !has_set)
  {
    ...
    impl->is_prop_get = impl->is_prop_set = EFL_TRUE;
  }
...
parse_function_pointer(Eo_Lexer *ls)
{
  ...
  Eina_Stringshare *cname      = NULL;
  Efl_Bool          has_params = EFL_FALSE, has_return = EFL_FALSE,
           has_c_name = EFL_FALSE;

  eo_lexer_get(ls);

  def->type           = EOLIAN_TYPEDECL_FUNCTION_POINTER;
  Efl_Bool has_extern = EFL_FALSE, has_beta = EFL_FALSE;
  for (;;)
    switch (ls->t.kw)
    {
      case KW_at_extern:
...
tags_done:
  ...
  if (meth->base.is_beta) eo_lexer_get(ls);
...
parse_method(Eo_Lexer *ls)
{
  ...
  Eolian_Function  *meth      = NULL;
  Eolian_Implement *impl      = NULL;
  Efl_Bool          has_const = EFL_FALSE, has_params = EFL_FALSE,
           has_return = EFL_FALSE, has_protected = EFL_FALSE,
           has_class = EFL_FALSE, has_beta = EFL_FALSE, has_virtp = EFL_FALSE;
  meth            = calloc(1, sizeof(Eolian_Function));
  meth->klass     = ls->klass;
  ...
  impl->base.name =
    eina_stringshare_printf("%s.%s", ls->klass->base.name, meth->base.name);
  eo_lexer_get(ls);
  for (;;)
    switch (ls->t.kw)
    {
      ...
      case KW_at_const:
        ...
        meth->obj_is_const = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_static:
        CASE_LOCK(ls, class, "class qualifier");
        meth->is_static = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_beta:
        CASE_LOCK(ls, beta, "beta qualifier");
        meth->base.is_beta = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_pure_virtual:
        check_abstract_pure_virtual(ls);
  ...
  for (;;)
    switch (ls->t.kw)
    {
      case KW_return:
        CASE_LOCK(ls, return, "return")
        Eo_Ret_Def ret;
        parse_return(ls, &ret, EFL_FALSE, EFL_TRUE, EFL_FALSE);
        if (ret.default_ret_val)
          eo_lexer_expr_release_ref(ls, ret.default_ret_val);
        meth->get_ret_type         = eo_lexer_type_release(ls, ret.type);
        meth->get_return_doc       = ret.doc;
...
static void
parse_implement(Eo_Lexer *ls, Efl_Bool iface)
{
  Efl_Strbuf       *buf   = NULL;
  Eolian_Implement *impl  = NULL;
  int               iline = ls->line_number, icol = ls->column;
  if (iface) check_kw(ls, KW_class);
  if (ls->t.kw == KW_class)
  {
    eo_lexer_get(ls);
    check_next(ls, '.');
    ...
    {
      ls->klass->class_dtor_enable = EFL_TRUE;
      eo_lexer_get(ls);
    }
    else
    {
      check_kw_next(ls, KW_constructor);
      ls->klass->class_ctor_enable = EFL_TRUE;
    }
    check_next(ls, ';');
    return;
  }
  Efl_Bool glob_auto = EFL_FALSE, glob_empty = EFL_FALSE;
  switch (ls->t.kw)
  {
    case KW_at_auto:
      glob_auto = EFL_TRUE;
  ...
  {
    eo_lexer_get(ls);
    if (ls->t.token != TOK_VALUE) eo_lexer_syntax_error(ls, "name expected");
    Eina_Stringshare *iname =
      eina_stringshare_printf("%s.%s", ls->klass->base.name, ls->t.value.s);
    Eina_List        *l;
...
propbeg:
  if (ls->t.token == '{')
  {
    Efl_Bool has_get = EFL_FALSE, has_set = EFL_FALSE;
    eo_lexer_get(ls);
    ...
    for (;;)
      switch (ls->t.kw)
      {
        case KW_get:
          ...
          {
            impl->get_auto = EFL_TRUE;
            eo_lexer_get(ls);
          }
          else if (ls->t.kw == KW_at_empty)
          {
            impl->get_empty = EFL_TRUE;
            eo_lexer_get(ls);
          }
          check_next(ls, ';');
        ...
        case KW_set:
          ...
          if (ls->t.kw == KW_at_auto)
          {
            impl->set_auto = EFL_TRUE;
            eo_lexer_get(ls);
          }
          else if (ls->t.kw == KW_at_empty)
          {
            impl->set_empty = EFL_TRUE;
            eo_lexer_get(ls);
          }
          check_next(ls, ';');
...
propend:
    if (!has_get && !has_set)
      eo_lexer_syntax_error(
        ls,
        "property implements need at least get or set specified");
    check_next(ls, '}');
  }
  else
  {
    if (glob_auto) impl->get_auto = impl->set_auto = EFL_TRUE;
    if (glob_empty) impl->get_empty = impl->set_empty = EFL_TRUE;
    check_next(ls, ';');
    FILL_DOC(ls, impl, common_doc);
  }
  if (buf)
...
parse_event(Eo_Lexer *ls)
{
  ...
  eo_lexer_dtor_pop(ls);
  Efl_Bool has_scope = EFL_FALSE, has_beta = EFL_FALSE, has_hot = EFL_FALSE,
           has_restart = EFL_FALSE;
  for (;;)
    switch (ls->t.kw)
    {
      case KW_at_private:
      case KW_at_protected:
        ...
        ev->scope = (ls->t.kw == KW_at_private) ? EOLIAN_SCOPE_PRIVATE
                                                : EOLIAN_SCOPE_PROTECTED;
        eo_lexer_get(ls);
        break;
      case KW_at_beta:
        CASE_LOCK(ls, beta, "beta qualifier")
        ev->base.is_beta = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_hot:
  ...
  check_next(ls, ':');
  ev->type = eo_lexer_type_release(ls, parse_type_void(ls, EFL_TRUE, EFL_TRUE));
  check(ls, ';');
  eo_lexer_get(ls);
  FILL_DOC(ls, ev, doc);
  ev->klass = ls->klass;
...
static void
parse_implements(Eo_Lexer *ls, Efl_Bool iface)
{
  int line, col;
  eo_lexer_get(ls);
  line = ls->line_number, col = ls->column;
...
_validate_pfx(Eo_Lexer *ls)
{
  ...
  if ((*str != '_') && ((*str < 'a') || (*str > 'z'))) goto error;
  for (++str; *str; ++str)
  {
    if (*str == '_') continue;
...
{
  Efl_Bool has_c_prefix = EFL_FALSE, has_event_c_prefix = EFL_FALSE,
           has_data = EFL_FALSE, has_methods = EFL_FALSE, has_parts = EFL_FALSE,
           has_implements = EFL_FALSE, has_constructors = EFL_FALSE,
           has_events = EFL_FALSE;
...
static void
_inherit_dep(Eo_Lexer *ls, Efl_Strbuf *buf, Efl_Bool parent)
{
  char        ebuf[PATH_MAX];
  const char *iname;
  char       *fnm;
  ...
  {
    Eina_List *l;
    if (inames == ls->klass->parent_name) goto inherit_dup;
    EINA_LIST_FOREACH(ls->klass->extends, l, oiname)
    {
      if (inames == oiname) goto inherit_dup;
    }
  }
  database_defer(ls->state, fnm, EFL_TRUE);
  if (parent) ls->klass->parent_name = inames;
  else ls->klass->extends = eina_list_append(ls->klass->extends, inames);
  free(fnm);
  eo_lexer_context_pop(ls);
  return;
...
static void
_requires_add(Eo_Lexer *ls, Efl_Strbuf *buf)
{
  const char *required;
  char       *fnm;
  Eina_List  *l;
  ...
  ls->klass->
    requires
  = eina_list_append(ls->klass->requires, required);
  database_defer(ls->state, fnm, EFL_TRUE);
  eo_lexer_context_pop(ls);
...
static void
_composite_add(Eo_Lexer *ls, Efl_Strbuf *buf)
{
  const char *oname;
  char        ebuf[PATH_MAX];
  Eina_List  *l;
...
parse_class(Eo_Lexer *ls, Eolian_Class_Type type)
{
  ...
  eo_lexer_context_push(ls);
  Eina_Stringshare *cname    = NULL;
  Efl_Bool          has_beta = EFL_FALSE, has_c_name = EFL_FALSE;
  for (;;)
    switch (ls->t.kw)
    {
      case KW_at_beta:
        CASE_LOCK(ls, beta, "beta qualifier");
        ls->klass->base.is_beta = EFL_TRUE;
        eo_lexer_get(ls);
        break;
      case KW_at_c_name:
        CASE_LOCK(ls, c_name, "@c_name specifier");
...
tags_done:
  ...
  else ls->klass->base.c_name = make_c_name(ls->klass->base.name);
  ...
  if (ls->t.token != '{')
  {
    line             = ls->line_number;
    col              = ls->column;
    Efl_Strbuf *ibuf = eina_strbuf_new();
    ...
    if (ls->t.kw == KW_requires)
    {
      if (type != EOLIAN_CLASS_MIXIN)
      {
        eo_lexer_syntax_error(
          ls,
          "\"requires\" keyword is only needed for mixin classes");
      }
      eo_lexer_get(ls);
      do
    ...
    {
      Efl_Bool ext = (ls->t.kw == KW_extends);
      eo_lexer_get(ls);
      if (is_reg && ext)
      {
        /* regular class can have a parent, but just one */
        _inherit_dep(ls, ibuf, EFL_TRUE);
        /* followed by composites */
        if (ls->t.kw == KW_composites) goto noimp_comp;
      ...
      do
        _inherit_dep(ls, ibuf, EFL_FALSE);
      while (test_next(ls, ','));
    }
...
static Efl_Bool
parse_unit(Eo_Lexer *ls, Efl_Bool eot)
{
  switch (ls->t.kw)
  {
    case KW_abstract:
    ...
    case KW_parse:
      {
        Efl_Bool    isdep = (ls->t.kw == KW_import);
        Efl_Strbuf *buf   = eina_strbuf_new();
        eo_lexer_dtor_push(ls, EINA_FREE_CB(eina_strbuf_free), buf);
    ...
    case KW_const:
      {
        database_constant_add(
          ls->unit,
          eo_lexer_constant_release(ls, parse_constant(ls)));
        break;
      }
    case KW_error:
    ...
    case KW_enum:
      {
        Efl_Bool    is_enum = (ls->t.kw == KW_enum);
        const char *name;
        int         line, col;
        const char *freefunc = NULL, *cname = NULL;
        Efl_Strbuf *buf;
        eo_lexer_get(ls);
        Efl_Bool has_extern = EFL_FALSE, has_free = EFL_FALSE,
                 has_beta = EFL_FALSE, has_c_name = EFL_FALSE;
        for (;;)
          switch (ls->t.kw)
          {
            case KW_at_extern:
...
postparams:
        ...
        if (!is_enum && ls->t.token == ';')
        {
          ...
          if (!def->base.c_name) def->base.c_name = make_c_name(name);
          eo_lexer_get(ls);
          FILL_DOC(ls, def, doc);
          FILL_BASE(def->base, ls, line, col, TYPEDECL);
          database_struct_add(ls->unit, eo_lexer_typedecl_release(ls, def));
...
static void
parse_chunk(Eo_Lexer *ls, Efl_Bool eot)
{
  Efl_Bool parsing_header = EFL_TRUE;
  Efl_Bool has_version    = EFL_FALSE;
  while (ls->t.token >= 0)
    switch (ls->t.kw)
    {
      case KW_hash_version:
        {
          ...
          if (ls->unit->version > EOLIAN_FILE_FORMAT_VERSION)
            eo_lexer_syntax_error(
              ls,
              "file version too new for this version of Eolian");
          eo_lexer_get(ls);
          break;
        }
      default:
        parsing_header = EFL_FALSE;
        /* set eot to EFL_TRUE so that we only allow parsing of one class */
        if (parse_unit(ls, eot)) eot = EFL_TRUE;
        break;
    }
}

Eolian_Unit *
eo_parser_database_fill(Eolian_Unit *parent, const char *filename, Efl_Bool eot)
{
  int         status = 0;
  ...
  else fname = eina_stringshare_add(filename);

  Eolian_Unit *ret = eina_hash_find(parent->state->main.units, fname);
  if (!ret) ret = eina_hash_find(parent->state->staging.units, fname);
  ...
  if (!ls)
  {
    eolian_state_log(parent->state,
                     "unable to create lexer for file '%s'",
                     filename);
    goto error;
  }
  ...
  if ((status = setjmp(ls->err_jmp))) goto error;
