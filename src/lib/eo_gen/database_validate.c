  ...
  EINA_LIST_FREE(pl, par)
  {
    ...
    while (ret && (doc = eolian_documentation_tokenize(doc, &tok)))
    {
      if (eolian_doc_token_type_get(&tok) == EOLIAN_DOC_TOKEN_REF)
      {
        ...
        Eolian_Object_Type tp =
          database_doc_token_ref_resolve(&tok,
...
_validate_doc(Eolian_Documentation *doc)
{
  ...
  if (!_validate_docstr(doc->description, &doc->base, &rdbg)) return EFL_FALSE;
...
{
  sc->succ =
    _validate_type_by_ref(sc->vals, sf->type, sf->by_ref, sf->move, EFL_FALSE);
               ...
               Efl_Bool        is_ret)
{
  ...
  if (tp->is_ptr)
  {
    ...
    tp->is_ptr             = EFL_FALSE;
    Efl_Bool still_ownable = database_type_is_ownable(src, tp, EFL_FALSE, NULL);
    tp->is_ptr             = EFL_TRUE;
    if (still_ownable)
    {
  ...
  switch (tp->type)
  {
    ...
    case EOLIAN_TYPE_CLASS:
      {
        tp->klass =
          (Eolian_Class *)eolian_unit_class_by_name_get(src, tp->base.name);
        if (!tp->klass)
        {
          _eo_parser_log(&tp->base,
                         "undefined class %s "
    ...
    case EOLIAN_TYPE_ERROR:
      {
        tp->error =
          (Eolian_Error *)eolian_unit_error_by_name_get(src, tp->base.name);
        if (!tp->error)
        {
          _eo_parser_log(&tp->base,
                         "undefined error %s "
               ...
               Efl_Bool               by_ref)
{
  ...
  if (by_ref)
    val =
      database_expr_eval(expr->base.unit, expr, EOLIAN_MASK_NULL, NULL, NULL);
  else if (tp)
                   ...
                   Eina_Hash       *nhash)
{
  ...
  Efl_Bool was_stable = _set_stable(vals, !func->base.is_beta && vals->stable);
...
_validate_part(Validate_State *vals, Eolian_Part *part, Eina_Hash *phash)
{
  ...
  Efl_Bool was_stable = _set_stable(vals, !part->base.is_beta && vals->stable);
...
_validate_event(Validate_State *vals, Eolian_Event *event, Eina_Hash *nhash)
{
  ...
  Efl_Bool was_stable = _set_stable(vals, !event->base.is_beta && vals->stable);
...
_extend_impl(Eina_Hash *fs, Eolian_Implement *impl, Efl_Bool as_iface)
{
  ...
  if (as_iface || (impl->implklass->type == EOLIAN_CLASS_INTERFACE)) return !st;
  ...
  EINA_LIST_FOREACH(icl->callables, l, impl)
  {
    ...
    if (_extend_impl(fs, impl, !allow_impl))
    {
      ...
      if (ost == IMPL_STATUS_NONE)
      {
        ...
        {
          if (old->foo_id == impl->foo_id) eina_list_data_set(ll, impl);
        }
      }
                  ...
                  Eina_Hash      *errh)
{
  ...
  if (cl->parent_name)
  {
    succ = _db_swap_inherit(cl, succ, cl->parent_name, &cl->parent, EFL_FALSE);
    if (succ)
    {
