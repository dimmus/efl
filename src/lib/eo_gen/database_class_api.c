...
eolian_class_function_by_name_get(const Eolian_Class  *cl,
                                  ...
                                  Eolian_Function_Type f_type)
{
  ...
  if (f_type == EOLIAN_UNRESOLVED || f_type == EOLIAN_METHOD)
    ...
    {
      if (!strcmp(fid->base.name, func_name)) return fid;
    }
      ...
      f_type == EOLIAN_PROP_SET || f_type == EOLIAN_PROP_GET)
  {
    ...
    {
      if (!database_function_is_type(fid, f_type)) continue;
      if (!strcmp(fid->base.name, func_name)) return fid;
    }
  }

  eolian_state_log(cl->base.unit->state,
...
EOLIAN_API Eina_Iterator *
eolian_class_functions_get(const Eolian_Class  *cl,
...
{
  EINA_SAFETY_ON_NULL_RETURN_VAL(cl, EFL_FALSE);
...
eolian_class_c_data_type_get(const Eolian_Class *cl)
{
  char buf[512];
  EINA_SAFETY_ON_NULL_RETURN_VAL(cl, NULL);
  if (!cl->data_type) snprintf(buf, sizeof(buf), "%s_Data", cl->base.c_name);
  else if (!strcmp(cl->data_type, "null")) return eina_stringshare_add("void");
  else snprintf(buf, sizeof(buf), "%s", cl->data_type);
