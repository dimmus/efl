...
database_object_add(Eolian_Unit *unit, const Eolian_Object *obj)
{
  ...
  eina_hash_set(
    unit->state->staging.objects_f,
    obj->file,
    eina_list_append(
      (Eina_List *)eina_hash_find(unit->state->staging.objects_f, obj->file),
      obj));
}
...
{
  if (!it || !it->curp) return EFL_FALSE;

  char *ndot = strchr(it->curp, '.');
  if (!ndot) return EFL_FALSE;
...
{
  if (!obj) return EFL_FALSE;
  return obj->is_beta;
}

void
database_doc_del(Eolian_Documentation *doc)
{
  if (!doc) return;
...
eolian_documentation_string_split(const char *doc)
{
  ...
  for (;;)
  {
    ...
    if (sep) eina_strbuf_append_length(buf, doc, sep - doc);
    else eina_strbuf_append(buf, doc);
...
{
  if (((*doc)[0] != '_') && !isalpha((*doc)[0])) return EFL_FALSE;
...
_get_ref_token(const char *doc, const char **doc_end)
{
  ...
  if (doc[0] != '@') return EOLIAN_DOC_TOKEN_UNKNOWN;

  ++doc;

  Efl_Bool is_event = (doc[0] == '[');
  if (is_event) ++doc;
  ...
  else return EOLIAN_DOC_TOKEN_UNKNOWN;
...
eolian_documentation_tokenize(const char *doc, Eolian_Doc_Token *ret)
{
  ...
  Efl_Bool cont = (ret->type != EOLIAN_DOC_TOKEN_UNKNOWN);
...
mloop:
  ...
  if ((doc[0] == '$') && (doc[1] == '['))
  {
    ...
    if (ret->text_end[0] == ']') return ret->text_end + 1;
    return ret->text_end;
  }
...
eolian_doc_token_text_get(const Eolian_Doc_Token *tok)
{
  ...
  if (tok->type == EOLIAN_DOC_TOKEN_UNKNOWN) return NULL;
  Efl_Strbuf *buf = eina_strbuf_new();
  for (const char *p = tok->text; p != tok->text_end; ++p)
  {
    if (*p == '\\') ++p;
    if (p != tok->text_end) eina_strbuf_append_char(buf, *p);
  }
               ...
               const Eolian_Object **data2)
{
  ...
  if (name[0] == ',') return EOLIAN_OBJECT_UNKNOWN;

  char *evname = strrchr(name, '.');
  if (!evname) return EOLIAN_OBJECT_UNKNOWN;

  *evname++              = '\0';
  const Eolian_Class *cl = eolian_unit_class_by_name_get(unit1, name);
  if (!cl && unit2) cl = eolian_unit_class_by_name_get(unit2, name);
  if (!cl) return EOLIAN_OBJECT_UNKNOWN;

  const Eolian_Event *ev = eolian_class_event_by_name_get(cl, evname);
  if (!ev) return EOLIAN_OBJECT_UNKNOWN;
...
database_doc_token_ref_resolve(const Eolian_Doc_Token *tok,
                               ...
                               const Eolian_Object   **data2)
{
  if (tok->type != EOLIAN_DOC_TOKEN_REF) return EOLIAN_OBJECT_UNKNOWN;
  ...
  if (!decl && unit2) decl = eolian_unit_object_by_name_get(unit2, name);
  if (decl)
  {
    if (data1) *data1 = decl;
  ...
  if (!suffix) return EOLIAN_OBJECT_UNKNOWN;
  ...
  *suffix++ = '\0';
  ...
  if (!tpd && unit2) tpd = eolian_unit_enum_by_name_get(unit2, name);
  if (tpd)
  {
  ...
  if (!strcmp(suffix, "get")) ftype = EOLIAN_PROP_GET;
  else if (!strcmp(suffix, "set")) ftype = EOLIAN_PROP_SET;
  ...
  const Eolian_Function *fid =
    eolian_class_function_by_name_get(cl, suffix, ftype);
  if (!fid) return EOLIAN_OBJECT_UNKNOWN;
...
eolian_doc_token_ref_resolve(const Eolian_Doc_Token *tok,
                             ...
                             const Eolian_Object   **data2)
{
  return database_doc_token_ref_resolve(tok,
                                        (const Eolian_Unit *)state,
                                        NULL,
                                        data,
                                        data2);
}
...
database_unit_init(Eolian_State *state, Eolian_Unit *unit, const char *file)
{
  ...
  unit->constants =
    eina_hash_stringshared_new(EINA_FREE_CB(database_constant_del));
...
static void
_default_error_cb(const Eolian_Object *obj,
                  const char          *msg,
                  void *data           EFL_UNUSED)
{
  if (obj) _eolian_log_line(obj->file, obj->line, obj->column, msg);
  else _eolian_log(msg);
}
...
{
  Scan_State *sst   = data;
  Efl_Bool    is_eo = eina_str_has_suffix(name, EO_SUFFIX);
  ...
  else eina_hash_add(fh, name, newpath);
}

EOLIAN_API Efl_Bool
eolian_state_directory_add(Eolian_State *state, const char *dir)
{
  if (!dir || !state) return EFL_FALSE;
  Scan_State sst = { state, EFL_TRUE };
  return eina_file_dir_list(dir, EFL_TRUE, _scan_cb, &sst) && sst.succ;
}
...
_eolian_file_parse_nodep(Eolian_Unit *parent, const char *filename)
{
  ...
  const char *eopath = eina_hash_find(is_eo ? parent->state->filenames_eo
                                            : parent->state->filenames_eot,
                                      filename);
  if (!eopath)
  {
    eolian_state_log(parent->state,
...
void
database_defer(Eolian_State *state, const char *fname, Efl_Bool isdep)
{
  ...
  if (!found) eina_hash_add(state->defer, fname, nval);
  else if ((found <= 1) && isdep) eina_hash_set(state->defer, fname, nval);
}

static Efl_Bool _parse_deferred(Eolian_Unit *parent);
...
static Efl_Bool
_defer_hash_cb(const Eina_Hash *hash EFL_UNUSED,
               const void           *key,
               void                 *data,
               void                 *fdata)
{
  Defer_Data  *d      = fdata;
  Efl_Bool     alone  = ((size_t)data <= 1);
  Eolian_Unit *parent = d->parent;
  /* not a dependency; parse standalone */
  if (alone) parent = &parent->state->staging.unit;
  ...
  /* standalone-parsed stuff forms its own dependency trees,
    ...
    */
  if (d->succ && alone) _merge_units(pdep);
  return d->succ;
}
...
{
  Eina_Hash *dest = fdata;
  if (!eina_hash_find(dest, key)) eina_hash_add(dest, key, data);
  return EFL_TRUE;
}
...
eolian_state_file_parse(Eolian_State *state, const char *filename)
{
  ...
  if (!ret) return NULL;
  if (!_parse_deferred(ret)) return NULL;
  _merge_units(ret);
  if (!database_validate(&state->staging.unit)) return NULL;
...
eolian_state_file_path_parse(Eolian_State *state, const char *filepath)
{
  ...
  if (fname && strrchr(fname, '\\')) fname = strrchr(fname, '\\');
...
static Efl_Bool
_tfile_parse(const Eina_Hash *hash EFL_UNUSED,
             const void *key       EFL_UNUSED,
             void                 *data,
             void                 *fdata)
{
  ...
  if (pd->ret)
    unit = eo_parser_database_fill(&pd->state->staging.unit, data, EFL_TRUE);
  pd->ret = !!unit;
...
{
  Parse_Data pd = { state, EFL_TRUE };

  if (!state) return EFL_FALSE;

  _state_clean(state);
  eina_hash_foreach(state->filenames_eot, _tfile_parse, &pd);

  if (pd.ret && !database_validate(&state->staging.unit)) return EFL_FALSE;
...
static Efl_Bool
_file_parse(const Eina_Hash *hash EFL_UNUSED,
            const void *key       EFL_UNUSED,
            void                 *data,
            void                 *fdata)
{
  ...
  Eolian_Unit *unit = NULL;
  if (pd->ret)
    unit = eo_parser_database_fill(&pd->state->staging.unit, data, EFL_FALSE);
  pd->ret = !!unit;
  if (pd->ret) pd->ret = _parse_deferred(unit);
  if (pd->ret) _merge_units(unit);
  return pd->ret;
...
eolian_unit_file_path_get(const Eolian_Unit *unit)
{
  ...
  Efl_Bool is_eo = eina_str_has_suffix(unit->file, EO_SUFFIX);
  return eina_hash_find(is_eo ? unit->state->filenames_eo
                              : unit->state->filenames_eot,
                        unit->file);
}
...
EOLIAN_API Eina_Iterator *
eolian_unit_objects_get(const Eolian_Unit *unit)
{
...
database_error_add(Eolian_Unit *unit, Eolian_Error *err)
{
  ...
  eina_hash_set(
    unit->state->staging.errors_f,
    err->base.file,
    eina_list_append((Eina_List *)eina_hash_find(unit->state->staging.errors_f,
                                                 err->base.file),
                     err));
...
{
  char       *ret;
  Efl_Strbuf *strbuf = eina_strbuf_new();
