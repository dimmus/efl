...
database_object_add(Eolian_Unit *unit, const Eolian_Object *obj)
{
   /* object storage */
    ...
    eina_hash_set(unit->state->staging.objects_f,
                  obj->file,
             ...
             void                 *fdata)
{
    ...
    if (pd->ret)
        unit =
            eo_parser_database_fill(&pd->state->staging.unit, data, EFL_TRUE);
    pd->ret = !!unit;
            ...
            void                 *fdata)
{
    ...
    if (pd->ret)
        unit =
            eo_parser_database_fill(&pd->state->staging.unit, data, EFL_FALSE);
    pd->ret = !!unit;
    if (pd->ret) pd->ret = _parse_deferred(unit);
    if (pd->ret) _merge_units(unit);
    return pd->ret;
...
database_error_add(Eolian_Unit *unit, Eolian_Error *err)
{
    ...
    eina_hash_set(unit->state->staging.errors_f,
                  err->base.file,
                  eina_list_append(
                      (Eina_List *)eina_hash_find(unit->state->staging.errors_f,
                                                  err->base.file),
                      err));
