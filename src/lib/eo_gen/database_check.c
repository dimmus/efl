...
_check_cycle(Eina_Hash *chash, const Eolian_Object *obj)
{
  ...
  if (eina_hash_find(chash, &obj)) return EFL_TRUE;
...
{
  if (!eina_hash_find(depset, &dep)) eina_hash_add(depset, &dep, dep);
}

static void
_check_type(const Eolian_Type *tp, Eina_Hash *depset, Eina_Hash *chash)
{
  if (tp->base_type) _check_type(tp->base_type, depset, chash);
  ...
  if (tp->tdecl) _add_dep(depset, ((const Eolian_Object *)tp->tdecl)->unit);
}
...
{
  database_expr_eval(expr->base.unit,
...
static void
_check_param(const Eolian_Function_Parameter *arg,
             Eina_Hash                       *depset,
             Eina_Hash                       *chash)
{
  if (arg->type) _check_type(arg->type, depset, chash);
  if (arg->value) _check_expr(arg->value, depset);
}

static void
_check_function(const Eolian_Function *f, Eina_Hash *depset, Eina_Hash *chash)
{
  if (f->get_ret_type) _check_type(f->get_ret_type, depset, chash);
  if (f->set_ret_type) _check_type(f->set_ret_type, depset, chash);

  if (f->get_ret_val) _check_expr(f->get_ret_val, depset);
  if (f->set_ret_val) _check_expr(f->set_ret_val, depset);
...
{
  if (_check_cycle(chash, &cl->base)) return;

  _add_dep(depset, cl->base.unit);

  const Eolian_Class *icl = cl->parent;
  if (icl) _add_dep(depset, icl->base.unit);
  ...
  {
    if (ev->type) _check_type(ev->type, depset, chash);
  }
  eina_iterator_free(itr);
...
{
  if (_check_cycle(chash, &tp->base)) return;
    ...
    EINA_LIST_FOREACH(tp->field_list, l, data)
    {
      switch (tp->type)
      {
        ...
        case EOLIAN_TYPEDECL_ENUM:
          _check_expr(((const Eolian_Enum_Type_Field *)data)->value, depset);
          break;
        default:
          break;
      }
...
{
  if (_check_cycle(chash, &v->base)) return;

  _add_dep(depset, v->base.unit);

  _check_type(v->base_type, depset, chash);
  if (v->value) _check_expr(v->value, depset);
}
...
_check_unit(const Eolian_Unit *unit)
{
  ...
  EINA_ITERATOR_FOREACH(citr, cunit)
  {
    ...
    {
      eolian_state_log(unit->state,
                       "%s: unused dependency %s",
                       unit->file,
                       cunit->file);
    }
  }
...
_check_namespaces(const Eolian_Unit *src)
{
  ...
  Efl_Bool check_beta = !!getenv("EOLIAN_CHECK_NAMESPACES_BETA");

  EINA_ITERATOR_FOREACH(itr, obj)
  {
    const char *dot = strrchr(obj->name, '.');
    if (!dot) continue;
    Eina_Stringshare *ssr =
      eina_stringshare_add_length(obj->name, dot - obj->name);
...
database_check(const Eolian_State *state)
{
  ...
  eina_iterator_free(itr);
   ...
   /* namespace checks */
  if (!_check_namespaces(&state->main.unit)) ret = EFL_FALSE;
