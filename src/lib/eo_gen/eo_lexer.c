...
static void throw(Eo_Lexer * ls, const char *fmt, ...)
{
...
{
  if (token == TOK_VALUE) memcpy(buf, ls->t.value.s, strlen(ls->t.value.s) + 1);
  else return eo_lexer_token_to_str(token, buf);
}
...
static Efl_Bool
should_skip_star(Eo_Lexer *ls, int ccol, Efl_Bool *term)
{
  Efl_Bool had_star = EFL_FALSE;
...
{
  Efl_Bool had_star = EFL_FALSE, had_nl = EFL_FALSE;
  eina_strbuf_reset(ls->buff);

  if (is_newline(ls->current))
  {
    Efl_Bool term = EFL_FALSE;
    had_nl        = EFL_TRUE;
    next_line_ws(ls);
  ...
  for (;;)
  {
    if (!ls->current) eo_lexer_lex_error(ls, "unfinished long comment", -1);
    if (ls->current == '*')
    {
      next_char(ls);
      if (ls->current == '/')
...
doc_ref_class(Eo_Lexer *ls, const char *cname)
{
  ...
  if (!eina_hash_find(ls->state->filenames_eo, buf)) return;
   /* ref'd classes do not become dependencies */
  database_defer(ls->state, buf, EFL_FALSE);
}
...
doc_ref(Eo_Lexer *ls, Eolian_Documentation *doc)
{
  ...
  doc_ref_class(ls, buf);
    ...
    */
  doc->ref_dbg =
    eina_list_append(doc->ref_dbg,
                     (void *)(size_t)((ls->line_number & 0xFFFFF) |
                                      (((ls->column + 1) & 0xFFF) << 20)));
  ...
  *end = '\0';
  ...
  if (strcmp(end + 1, "get") && strcmp(end + 1, "set")) return;
  end = strrchr(buf, '.');
  if (!end) return;
  *end = '\0';
  doc_ref_class(ls, buf);
    ...
    switch (ls->current)
    {
      ...
      case '\\':
        ...
        if (ls->current == ']')
        {
          ...
          else eina_strbuf_append(ls->buff, "\\]");
        }
        else eina_strbuf_append_char(ls->buff, '\\');
        continue;
      /* terminating case */
      case ']':
        next_char(ls);
...
read_since(Eo_Lexer *ls)
{
  eina_strbuf_reset(ls->buff);
  while (ls->current &&
         (ls->current == '.' || ls->current == '_' || isalnum(ls->current)))
  {
    eina_strbuf_append_char(ls->buff, ls->current);
    next_char(ls);
  }
  if (!eina_strbuf_length_get(ls->buff)) return DOC_UNFINISHED;
  skip_ws(ls);
  while (is_newline(ls->current))
    next_line_ws(ls);
  if (ls->current != ']') return DOC_MANGLED;
  next_char(ls);
  if (ls->current != ']') return DOC_MANGLED;
  next_char(ls);
  return DOC_SINCE;
...
read_doc(Eo_Lexer *ls, Eo_Token *tok, int line, int column)
{
  ...
  if (!doc) longjmp(ls->err_jmp, EO_LEXER_ERROR_OOM);
  ...
  Efl_Strbuf *rbuf = eina_strbuf_new();

  Efl_Bool term = EFL_FALSE, since = EFL_FALSE;
  while (!term)
  {
    int read;
    if (since)
    ...
    else read = doc_lex(ls, doc, &term, &since);
    switch (read)
    {
      case DOC_MANGLED:
      ...
      case DOC_UNFINISHED:
        ...
        return;
      case DOC_TEXT:
        if (!eina_strbuf_length_get(ls->buff)) continue;
        if (!doc->summary)
          doc->summary = eina_stringshare_add(eina_strbuf_string_get(ls->buff));
        else
        {
          if (eina_strbuf_length_get(rbuf)) eina_strbuf_append(rbuf, "\n\n");
...
read_hex_esc(Eo_Lexer *ls)
{
  ...
  for (i = 1; i < 3; ++i)
  {
    next_char(ls);
    c[i] = ls->current;
    if (!isxdigit(c[i])) esc_error(ls, c, i + 1, "hexadecimal digit expected");
    r = (r << 4) + hex_val(c[i]);
  }
  return r;
}
...
read_dec_esc(Eo_Lexer *ls)
{
  ...
  if (r > UCHAR_MAX) esc_error(ls, c, i, "decimal escape too large");
  return r;
}
...
read_escape(Eo_Lexer *ls)
{
  ...
  {
    case 'a':
      eina_strbuf_append_char(ls->buff, '\a');
      next_char(ls);
      break;
    case 'b':
      eina_strbuf_append_char(ls->buff, '\b');
      next_char(ls);
      break;
    case 'f':
      eina_strbuf_append_char(ls->buff, '\f');
      next_char(ls);
      break;
    case 'n':
      eina_strbuf_append_char(ls->buff, '\n');
      ...
      break;
    case 'r':
      eina_strbuf_append_char(ls->buff, '\r');
      next_char(ls);
      break;
    case 't':
      eina_strbuf_append_char(ls->buff, '\t');
      ...
      break;
    case 'v':
      eina_strbuf_append_char(ls->buff, '\v');
      next_char(ls);
      break;
    case 'x':
...
read_string(Eo_Lexer *ls, Eo_Token *tok)
{
  ...
  eina_strbuf_append_char(ls->buff, ls->current);
  next_char(ls);
  tok->value.s = eina_stringshare_add_length(
    eina_strbuf_string_get(ls->buff) + 1,
    (unsigned int)eina_strbuf_length_get(ls->buff) - 2);
}

static int
get_type(Eo_Lexer *ls, Efl_Bool is_float)
{
  if (is_float)
  {
...
write_val_with_decpoint(Eo_Lexer *ls, Eo_Token *tok, int type)
{
  ...
  if (end && end[0]) eo_lexer_lex_error(ls, "malformed number", TOK_NUMBER);
  tok->kw = type;
}

static void
write_val(Eo_Lexer *ls, Eo_Token *tok, Efl_Bool is_float)
{
...
{
  Efl_Bool is_float = EFL_FALSE;
  while (isxdigit(ls->current) || ls->current == '.')
  {
    eina_strbuf_append_char(ls->buff, ls->current);
    if (ls->current == '.') is_float = EFL_TRUE;
    next_char(ls);
  }
  if (is_float && (ls->current != 'p' && ls->current != 'P'))
  {
    eo_lexer_lex_error(ls,
...
{
  Efl_Bool is_float = eina_strbuf_string_get(ls->buff)[0] == '.';
      ...
      default:
        {
          ...
          if (ls->current && (isalnum(ls->current) || ls->current == '@' ||
                              ls->current == '#' || ls->current == '_'))
          {
            int         col    = ls->column;
            Efl_Bool    pfx_kw = (ls->current == '@') || (ls->current == '#');
            const char *str;
            eina_strbuf_reset(ls->buff);
            do
            {
            ...
            while (ls->current && (isalnum(ls->current) || ls->current == '_'));
...
get_filename(Eo_Lexer *ls)
{
  ...
  if (fslash || bslash)
    return eina_stringshare_add((fslash > bslash) ? (fslash + 1)
                                                  : (bslash + 1));
...
{
  Eina_File *f = eina_file_open(source, EFL_FALSE);
  if (!f)
  {
  ...
  if (ls->current != 0xEF) return;
  next_char(ls);
  if (ls->current != 0xBB) return;
  next_char(ls);
  if (ls->current != 0xBF) return;
  next_char(ls);
}
...
eo_lexer_node_new(Eo_Lexer *ls, size_t objsize)
{
  ...
  if (!obj) longjmp(ls->err_jmp, EO_LEXER_ERROR_OOM);
...
eo_lexer_new(Eolian_State *state, const char *source)
{
  ...
  if (!ls) eolian_state_panic(state, "out of memory");
...
eo_lexer_token_to_str(int token, char *buf)
{
  ...
  else if (token < START_CUSTOM)
  {
    ...
    if (iscntrl(token)) sprintf(buf, "char(%d)", token);
    else sprintf(buf, "%c", token);
  }
  else
  {
    const char *v;
...
eo_lexer_context_push(Eo_Lexer *ls)
{
  ...
  if (!ctx) longjmp(ls->err_jmp, EO_LEXER_ERROR_OOM);
  ctx->line    = ls->line_number;
  ctx->column  = ls->column;
  ctx->linestr = ls->stream_line;
  ctx->token   = ls->t;
  if (_eo_is_tokstr(ctx->token.token)) eina_stringshare_ref(ctx->token.value.s);
...
eo_lexer_context_pop(Eo_Lexer *ls)
{
  ...
  if (_eo_is_tokstr(ctx->token.token)) eina_stringshare_del(ctx->token.value.s);
  free(ctx);
...
eo_lexer_context_restore(Eo_Lexer *ls)
{
  ...
  if (_eo_is_tokstr(ls->t.token)) eina_stringshare_del(ls->t.value.s);
  ls->t = ctx->token;
  if (_eo_is_tokstr(ls->t.token)) eina_stringshare_ref(ls->t.value.s);
}
