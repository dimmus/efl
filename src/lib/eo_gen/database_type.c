...
database_type_add(Eolian_Unit *unit, Eolian_Typedecl *tp)
{
    ...
    eina_hash_set(unit->state->staging.aliases_f,
                  tp->base.file,
...
database_enum_add(Eolian_Unit *unit, Eolian_Typedecl *tp)
{
    ...
    eina_hash_set(unit->state->staging.enums_f,
                  tp->base.file,
                  eina_list_append(
    ...
    if (tp->type == EOLIAN_TYPE_REGULAR)
    {
        ...
        if (!ct)
        {
            ...
            if (tpp->type == EOLIAN_TYPEDECL_ALIAS)
                return database_type_is_ownable(unit,
                                                tpp->base_type,
                                                allow_void,
                                                otp);
            return EFL_FALSE;
...
_etype_to_str(const Eolian_Typedecl *tp, Efl_Strbuf *buf)
{
    ...
    EINA_LIST_FOREACH(tp->field_list, l, ef)
    {
        ...
        {
            Eolian_Value val =
                eolian_expression_eval(ef->value, EOLIAN_MASK_INT);
            const char *ret;
            eina_strbuf_append(buf, " = ");
