...
database_type_add(Eolian_Unit *unit, Eolian_Typedecl *tp)
{
  ...
  eina_hash_set(
    unit->state->staging.aliases_f,
    tp->base.file,
    eina_list_append((Eina_List *)eina_hash_find(unit->state->staging.aliases_f,
                                                 tp->base.file),
                     tp));
...
database_struct_add(Eolian_Unit *unit, Eolian_Typedecl *tp)
{
  ...
  eina_hash_set(
    sh,
    tp->base.file,
    eina_list_append((Eina_List *)eina_hash_find(sh, tp->base.file), tp));
  database_object_add(unit, &tp->base);
}
...
database_enum_add(Eolian_Unit *unit, Eolian_Typedecl *tp)
{
  ...
  eina_hash_set(
    unit->state->staging.enums_f,
    tp->base.file,
    eina_list_append(
      (Eina_List *)eina_hash_find(unit->state->staging.enums_f, tp->base.file),
      tp));
  database_object_add(unit, &tp->base);
}

Efl_Bool
database_type_is_ownable(const Eolian_Unit  *unit,
                         const Eolian_Type  *tp,
                         Efl_Bool            allow_void,
                         const Eolian_Type **otp)
{
  if (otp) *otp = tp;
  if (tp->is_ptr) return EFL_TRUE;
  if (tp->type == EOLIAN_TYPE_REGULAR)
  {
    ...
    if (!ct)
    {
      ...
      if (!tpp) return EFL_FALSE;
      if (tpp->type == EOLIAN_TYPEDECL_FUNCTION_POINTER) return EFL_TRUE;
  ...
  if (allow_void && (tp->type == EOLIAN_TYPE_VOID)) return EFL_TRUE;
  return (tp->type == EOLIAN_TYPE_CLASS);
}

static void
_buf_add_suffix(Efl_Strbuf *buf, const char *suffix)
{
  if (!suffix) return;
                     ...
                     Efl_Bool           by_ref)
{
  if ((tp->type == EOLIAN_TYPE_REGULAR || tp->type == EOLIAN_TYPE_CLASS ||
      ...
      ((ctype != EOLIAN_C_TYPE_RETURN) || by_ref ||
       database_type_is_ownable(NULL, tp, EFL_FALSE, NULL)))
  {
  ...
  if (tp->type == EOLIAN_TYPE_REGULAR || tp->type == EOLIAN_TYPE_CLASS)
  {
    ...
    else eina_strbuf_append(buf, tp->base.c_name);
  }
  else if (tp->type == EOLIAN_TYPE_ERROR) eina_strbuf_append(buf, "Eina_Error");
  else if (tp->type == EOLIAN_TYPE_VOID) eina_strbuf_append(buf, "void");
  ...
  else
  {
    ...
    database_type_to_str(tp->base_type,
                         buf,
                         NULL,
  ...
  if (tp->type == EOLIAN_TYPE_CLASS) _buf_add_suffix(buf, "*");
  if (tp->is_ptr) _buf_add_suffix(buf, "*");
  if (by_ref) _buf_add_suffix(buf, "*");
  _buf_add_suffix(buf, name);
}

static void
_stype_to_str(const Eolian_Typedecl *tp, Efl_Strbuf *buf)
{
  eina_strbuf_append(buf, "struct ");
  eina_strbuf_append(buf, tp->base.c_name);
  if (tp->type == EOLIAN_TYPEDECL_STRUCT_OPAQUE) return;
  eina_strbuf_append(buf, " { ");
  Eina_List                *l;
  Eolian_Struct_Type_Field *sf;
  EINA_LIST_FOREACH(tp->field_list, l, sf)
  {
    database_type_to_str(sf->type,
                         buf,
                         sf->base.name,
                         EOLIAN_C_TYPE_DEFAULT,
                         sf->by_ref);
...
static void
_etype_to_str(const Eolian_Typedecl *tp, Efl_Strbuf *buf)
{
  ...
  EINA_LIST_FOREACH(tp->field_list, l, ef)
  {
    ...
    if (ef->value)
    {
      Eolian_Value val = eolian_expression_eval(ef->value, EOLIAN_MASK_INT);
      const char  *ret;
      eina_strbuf_append(buf, " = ");
    ...
    if (l != eina_list_last(tp->field_list)) eina_strbuf_append(buf, ", ");
  }
  eina_strbuf_append(buf, " }");
}

static void
_atype_to_str(const Eolian_Typedecl *tp, Efl_Strbuf *buf)
{
...
void
database_typedecl_to_str(const Eolian_Typedecl *tp, Efl_Strbuf *buf)
{
  switch (tp->type)
  {
...
Eolian_Typedecl *
database_type_decl_find(const Eolian_Unit *unit, const Eolian_Type *tp)
{
  if (tp->type != EOLIAN_TYPE_REGULAR) return NULL;
  if (tp->tdecl) return tp->tdecl;
