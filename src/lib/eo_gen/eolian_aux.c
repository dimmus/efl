                     ...
                     Eina_Hash          *written)
{
  ...
  if (!funcs) goto justevs;

  iitr = eolian_class_implements_get(pcl);
  EINA_ITERATOR_FOREACH(iitr, imp)
  {
    const Eolian_Function *ifn = eolian_implement_function_get(imp, NULL);
    if (eina_hash_find(written, &ifn)) continue;
...
static size_t
_callables_find(const Eolian_Class *cl,
                Eina_List         **funcs,
                Eina_List         **events,
                Eina_Hash          *written)
{
  size_t total = 0;
  if (!funcs && !events) return total;
...
static void
_all_impls_find(Eina_List            **l,
                const Eolian_Class    *cl,
                const Eolian_Function *func,
                Eina_Hash             *got,
                Eina_Hash             *children)
{
  if (eina_hash_find(got, &cl)) return;
...
{
  if (!class_children) return NULL;

  const Eolian_Class *cl =
    eolian_implement_class_get(eolian_function_implement_get(func));
...
static const Eolian_Implement *_parent_impl_find(const char         *fulln,
                                                 const Eolian_Class *cl);
...
_parent_impl_find(const char *fulln, const Eolian_Class *cl)
{
  ...
  const Eolian_Class     *icl  = eolian_class_parent_get(cl);
  if (icl) iret = _parent_impl_find_body(icl, fulln);
  if (iret) return iret;
  Eina_Iterator *itr = eolian_class_extensions_get(cl);
                           ...
                           Eolian_Function_Type    ftype)
{
  ...
  if (!pdoc) return _parent_documentation_find(pimpl, ftype);
                                       ...
                                       Eolian_Function_Type    ftype)
{
  ...
  if (eolian_implement_class_get(impl) == icl) return NULL;

  const Eolian_Implement *oimp =
    eolian_function_implement_get(eolian_implement_function_get(impl, NULL));
...
{
  Efl_Bool ig = eolian_implement_is_prop_get(impl),
