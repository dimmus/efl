...
Efl_Bool
database_type_is_ownable(const Eolian_Unit *unit, const Eolian_Type *tp, Efl_Bool allow_void, const Eolian_Type **otp)
{
   if (otp) *otp = tp;
   if (tp->is_ptr)
     return EFL_TRUE;
...
database_type_to_str(const Eolian_Type *tp,
                     Efl_Strbuf *buf, const char *name,
                     Eolian_C_Type_Type ctype, Efl_Bool by_ref)
{
   if ((tp->type == EOLIAN_TYPE_REGULAR
     || tp->type == EOLIAN_TYPE_CLASS
     || tp->type == EOLIAN_TYPE_VOID)
     && tp->is_const
     && ((ctype != EOLIAN_C_TYPE_RETURN) || by_ref || database_type_is_ownable(NULL, tp, EFL_FALSE, NULL)))
     {
    ...
    || tp->type == EOLIAN_TYPE_CLASS)
   ...
   else
     {
        ...
        database_type_to_str(tp->base_type, buf, NULL,
                             EOLIAN_C_TYPE_DEFAULT, EFL_FALSE);
        _buf_add_suffix(buf, "*");
