...
typedef void (*Joystick_Mapper)(struct js_event      *event,
...
static void _joystick_xboxone_mapper(struct js_event      *event,
                                     Ecore_Event_Joystick *e);
static void _joystick_ps4_mapper(struct js_event      *event,
                                 Ecore_Event_Joystick *e);
...
struct _Joystick_Mapping_Info
{
    ...
    {"045e",
     "02dd", _joystick_xboxone_mapper}, /* Microsoft X-Box One pad (Covert Forces) */
    {"054c",
     "05c4",     _joystick_ps4_mapper}  /* Sony Computer Entertainment Wireless Controller */
};
...
_joystick_connected_event_add(int index, Eina_Bool connected)
{
    ...
    e->index = index;
    if (connected) e->type = CORE_EVENT_JOYSTICK_EVENT_TYPE_CONNECTED;
    else e->type = CORE_EVENT_JOYSTICK_EVENT_TYPE_DISCONNECTED;

    INF("index: %d, connected: %d", index, connected);
    core_event_add(CORE_EVENT_JOYSTICK, e, NULL, NULL);
}
...
_joystick_ps4_mapper(struct js_event *event, Ecore_Event_Joystick *e)
{
    ...
    {
        e->type         = CORE_EVENT_JOYSTICK_EVENT_TYPE_BUTTON;
        e->button.value = event->value;
        switch (event->number)
        {
            case 0:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_FACE_0;
                break;

            case 1:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_FACE_1;
                break;

            case 2:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_FACE_2;
                break;

            case 3:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_FACE_3;
                break;

            case 4:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_LEFT_SHOULDER;
                break;

            case 5:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_RIGHT_SHOULDER;
                break;

            case 8:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_SELECT;
                break;

            case 9:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_META;
                break;

            case 10:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_LEFT_ANALOG_STICK;
                break;
            ...
            case 12:
                e->button.index = CORE_EVENT_JOYSTICK_BUTTON_START;
                break;
    ...
    {
        e->type       = CORE_EVENT_JOYSTICK_EVENT_TYPE_AXIS;
        e->axis.value = event->value / 32767.0f;
        ;
        switch (event->number)
        {
            case 0:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_LEFT_ANALOG_HOR;
                break;

            case 1:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_LEFT_ANALOG_VER;
                break;

            case 2:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_RIGHT_ANALOG_HOR;
                break;

            case 3:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_LEFT_SHOULDER;
                break;

            case 4:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_RIGHT_SHOULDER;
                break;

            case 5:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_RIGHT_ANALOG_VER;
                break;

            case 6:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_HAT_X;
                break;

            case 7:
                e->axis.index = CORE_EVENT_JOYSTICK_AXIS_HAT_Y;
                break;
...
static Eina_Bool
_fd_handler_cb(void *userData, Core_Fd_Handler *fdHandler)
{
    int             fd;
    ...
    fd = core_main_fd_handler_fd_get(fdHandler);
    if (fd < 0) return CORE_CALLBACK_RENEW;

    len = read(fd, &event, sizeof(event));
    if (len == -1) return CORE_CALLBACK_RENEW;
...
_joystick_mapping_info_get(const char *syspath)
{
    ...
    mapping_info_size =
        (int)(sizeof(Joystick_Mapping_Info) / sizeof(Joystick_Mapping_Info[0]));
...
_joystick_register(const char *syspath)
{
    ...
    ji->fd_handler =
        core_main_fd_handler_add(fd, CORE_FD_READ, _fd_handler_cb, ji, 0, 0);
...
_joystick_unregister(const char *syspath)
{
    ...
    EINA_LIST_FOREACH_SAFE(joystick_list, l, l2, ji)
    {
        ...
        {
            fd = core_main_fd_handler_fd_get(ji->fd_handler);
            if (fd < 0) continue;

            close(fd);
            core_main_fd_handler_del(ji->fd_handler);
...
ecore_input_joystick_init(void)
{
    ...
    if (!eeze_init()) return --_ecore_input_joystick_init_count;
