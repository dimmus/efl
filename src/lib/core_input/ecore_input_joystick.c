...
_joystick_connected_event_add(int index, Eina_Bool connected)
{
    ...
    e->index = index;
    if (connected) e->type = ECORE_EVENT_JOYSTICK_EVENT_TYPE_CONNECTED;
    else e->type = ECORE_EVENT_JOYSTICK_EVENT_TYPE_DISCONNECTED;
...
_joystick_register(const char *syspath)
{
    ...
    ji->fd_handler =
        ecore_main_fd_handler_add(fd, ECORE_FD_READ, _fd_handler_cb, ji, 0, 0);
