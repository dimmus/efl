...
typedef struct _Ecore_Event_Axis_Update
                           Ecore_Event_Axis_Update; /**< @since 1.13 */
...
struct _Ecore_Event_Key
{
  ...
  const char *string;
  const char
    *compose; /**< final string corresponding to the key symbol composed */
  ...
  unsigned int
    modifiers; /**< The combination of modifiers key (SHIFT,CTRL,ALT,..)*/
...
struct _Ecore_Event_Mouse_Button
{
  ...
  {
    int
      device; /**< 0 if normal mouse, 1+ for other mouse-devices (eg multi-touch - other fingers) */
    double radius, radius_x,
      radius_y; /**< radius of press point - radius_x and y if its an ellipse (radius is the average of the 2) */
    double
      pressure; /**< pressure - 1.0 == normal, > 1.0 == more, 0.0 == none */
    double
      angle; /**< angle relative to perpendicular (0.0 == perpendicular), in degrees */
    double x,
      y; /**< same as x, y, but with sub-pixel precision, if available */
...
struct _Ecore_Event_Mouse_Move
{
  ...
  struct
  {
    ...
    double
      angle; /**< angle relative to perpendicular (0.0 == perpendicular), in degrees */
    double x,
      y; /**< same as x, y root.x, root.y, but with sub-pixel precision, if available */
...
struct _Ecore_Event_Joystick
{
  ...
  union {
    ...
    struct
    {
      ...
      double
        value; /* [0.0 .. 1.0] 0.0 == fully unpressed, 1.0 == fully pressed */
    } button;
  };
