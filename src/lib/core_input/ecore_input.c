...
{
    if (++_ecore_event_init_count != 1) return _ecore_event_init_count;
    if (!ecore_init())
    {
...
{
    if (--_ecore_event_init_count != 0) return _ecore_event_init_count;
...
ecore_event_update_modifier(const char            *key,
