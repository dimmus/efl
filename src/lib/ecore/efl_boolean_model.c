...
struct _Efl_Boolean_Model_Storage_Range
{
    ...
    unsigned int offset;
    uint16_t
        length;  // Maximum length of the buffer will be 256, to avoid math error we rely on 16bits here.
                   ...
                   void *data                             EFL_UNUSED)
{
    ...
    if (left->offset < right->offset) return EINA_RBTREE_LEFT;
                ...
                Efl_Bool               *default_value)
{
    ...
    *default_value = !!v->default_value;
    ...
    do
    {
        ...
        lookup->length =
            sizeof(lookup->buffer) * 8;  // Number of bits in the buffer
        // Initialize the buffer to the right default value
        memset(&lookup->buffer[0], *default_value, sizeof(lookup->buffer));

        v->buffers_root = eina_rbtree_inline_insert(
            v->buffers_root,
            EINA_RBTREE_GET(lookup),
            EINA_RBTREE_CMP_NODE_CB(_storage_range_cmp),
            NULL);
        v->last = lookup;
    }
                                            ...
                                            Efl_Boolean_Model_Data *pd)
{
    ...
    if (pd->parent) properties = eina_hash_iterator_key_new(pd->parent->values);
                                          ...
                                          const char             *property)
{
    ...
    Efl_Bool                         default_value = EFL_FALSE;
                                          ...
                                          Eina_Value             *value)
{
    ...
    unsigned int                     index;
    unsigned int                     offset;
    Efl_Bool                         flag          = EFL_FALSE;
    Efl_Bool                         found         = EFL_FALSE;
    Efl_Bool                         convert_fail  = EFL_FALSE;
    Efl_Bool                         default_value = EFL_FALSE;
    ...
    index = efl_composite_model_index_get(obj);
    if (!eina_value_bool_convert(value, &flag)) convert_fail = EFL_TRUE;
    ...
    if (!sr)
        return efl_loop_future_resolved(obj,
                                        eina_value_bool_init(default_value));

    // Calculate the matching offset for the requested index
    offset = index - sr->offset;

    // It is assumed that during slice get the buffer is properly sized
    if (flag) sr->buffer[offset >> 3] |= ((unsigned char)1) << (offset & 0x7);
    else sr->buffer[offset >> 3] &= ~(((unsigned char)1) << (offset & 0x7));
...
static void
_mark_greater(Efl_Boolean_Model_Storage_Range *root,
              Eina_Array                      *mark,
    ...
    EINA_ITERATOR_FOREACH(it, v)
    {
        ...
        if (lookup)
        {
            unsigned char lower_mask =
                (((unsigned char)1) << (ev->index & 0x7)) - 1;
            unsigned char upper_mask =
                (~(((unsigned char)1) << (ev->index & 0x7))) & (~lower_mask);
            ...
            {
                lookup->buffer[offset] =
                    ((lookup->buffer[offset] & upper_mask) >> 1) |
            ...
            if (lookup->length == 0)
            {
                v->buffers_root = eina_rbtree_inline_remove(
                    v->buffers_root,
                    EINA_RBTREE_GET(lookup),
                    EINA_RBTREE_CMP_NODE_CB(_storage_range_cmp),
                    NULL);
...
_efl_boolean_model_iterator_storage_index_find(Eina_Iterator_Boolean *it)
{
    ...
    while (offset < it->sr->length)
    {
        ...
        while (((offset >> 3) == upidx) && (offset < it->sr->length))
        {
            Efl_Bool flag = it->sr->buffer[offset >> 3] &
                            (((unsigned char)1) << (offset & 0x7));

            if (it->request == !!flag) return EFL_TRUE;
    ...
    while (it->index < it->total)
    {
        ...
        if (!it->sr)
        {
            if (!eina_iterator_next(it->infix, (void **)&it->sr))
            {
                ...
                if (it->v->default_value != it->request) return EFL_FALSE;
                return EFL_TRUE;
            }
        }

        if (_efl_boolean_model_iterator_storage_index_find(it)) return EFL_TRUE;
...
static Eina_Iterator *
_efl_boolean_model_boolean_iterator_get(Eo                     *obj,
                                        Efl_Boolean_Model_Data *pd,
                                        const char             *name,
                                        Efl_Bool                request)
{
