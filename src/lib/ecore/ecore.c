...
bs_mod_get(char *path, size_t maxlen, const char *subsystem, const char *mod_name)
{
   ...
   if (!strcmp(subsystem, "ethumb"))
      snprintf(path, maxlen, EFL_BUILD_DIR"/src/modules/%s/%s/lib%s_el"MOD_SUFFIX, subsystem, mod_name, mod_name);
   else if (!strcmp(subsystem, "evas/image_loaders"))
      snprintf(path, maxlen, EFL_BUILD_DIR"/src/modules/%s/libshared_loader_%s"MOD_SUFFIX, subsystem, mod_name);
   else if (!strcmp(subsystem, "evas/image_savers"))
      snprintf(path, maxlen, EFL_BUILD_DIR"/src/modules/%s/libshared_saver_%s"MOD_SUFFIX, subsystem, mod_name);
   else
      snprintf(path, maxlen, EFL_BUILD_DIR"/src/modules/%s/%s/lib%s"MOD_SUFFIX, subsystem, mod_name, mod_name);
...
//    snprintf(path, maxlen, EFL_BUILD_DIR"/src/modules/%s/%s/", subsystem, mod_name);
...
//    snprintf(path, maxlen, EFL_BUILD_DIR"/src/bin/%s/%s"EXE_SUFFIX, subsystem, bin_name);
...
//    snprintf(path, maxlen, EFL_SRC_DIR"/data/%s/%s", subsystem, file);
