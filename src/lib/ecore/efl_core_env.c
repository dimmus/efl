...
{
    if (!key || key[0] == '\0') return EFL_FALSE;

    if (isdigit(key[0])) return EFL_FALSE;

    for (int i = 0; key[i] != '\0'; ++i)
    {
        if (!isalnum(key[i]) && key[i] != '_') return EFL_FALSE;
    }
...
EOLIAN static void
_efl_core_env_env_set(Eo *obj            EFL_UNUSED,
                      Efl_Core_Env_Data *pd,
                      const char        *var,
                      const char        *value)
{
...
_efl_core_env_env_get(const Eo *obj      EFL_UNUSED,
                      Efl_Core_Env_Data *pd,
                      const char        *var)
{
...
_efl_core_env_efl_duplicate_duplicate(const Eo *obj      EFL_UNUSED,
                                      Efl_Core_Env_Data *pd)
{
