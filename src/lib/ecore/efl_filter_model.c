...
struct _Efl_Filter_Model_Data
{
    ...
    Efl_Bool     counting_started : 1;
    Efl_Bool     processed        : 1;
};

static Eina_Rbtree_Direction
_filter_mapping_cmp_cb(const Eina_Rbtree *left,
                       ...
                       void *data         EFL_UNUSED)
{
    ...
    if (l->mapped < r->mapped) return EINA_RBTREE_LEFT;
    return EINA_RBTREE_RIGHT;
}

static int
_filter_mapping_looking_cb(const Eina_Rbtree *node,
                           const void        *key,
...
static void
_efl_filter_model_filter_set(Eo *obj                EFL_UNUSED,
                             Efl_Filter_Model_Data *pd,
                             void                  *filter_data,
                             EflFilterModel         filter,
                             Eina_Free_Cb           filter_free_cb)
{
    if (pd->filter.cb) pd->filter.free_cb(pd->filter.data);
...
_efl_filter_model_filter(Eo *o EFL_UNUSED, void *data, const Eina_Value v)
{
    ...
    if (!mapping)
    {
        ret = eina_value_bool_init(EFL_FALSE);
        goto end;
    }
    ...
    r->pd->mapping = eina_rbtree_inline_insert(r->pd->mapping,
                                               ...
                                               _filter_mapping_cmp_cb,
                                               NULL);

    child = _efl_filter_lookup(efl_class_get(r->parent),
                               r->parent,
                               r->child,
                               mapping);
    if (!child) goto end;
    ...
    efl_event_callback_call(r->parent,
                            EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                            NULL);

    efl_unref(cevt.child);

    ret = eina_value_bool_init(EFL_TRUE);
...
static void
_efl_filter_model_filter_clean(Eo *o                          EFL_UNUSED,
                               void                          *data,
...
_efl_filter_model_child_added(void *data, const Efl_Event *event)
{
    ...
    {
        Efl_Filter_Model_Data *cpd =
            efl_data_scope_get(child, EFL_FILTER_MODEL_CLASS);
    ...
    if (!child)
    {
        f = efl_model_children_slice_get(efl_ui_view_model_get(r->parent),
                                         r->index,
                                         1);
...
_efl_filter_model_child_removed(void *data, const Efl_Event *event)
{
    ...
    mapping = (void *)eina_rbtree_inline_lookup(pd->mapping,
                                                &removed,
                                                sizeof(uint64_t),
                                                _filter_mapping_looking_cb,
                                                NULL);
    if (!mapping) return;

    pd->mapping = eina_rbtree_inline_remove(pd->mapping,
                                            EINA_RBTREE_GET(mapping),
                                            _filter_mapping_cmp_cb,
                                            NULL);
   ...
   // Update the tree for the index to reflect the removed child
    for (removed++; removed < pd->filter.count; removed++)
    {
        ...
        pd->mapping = eina_rbtree_inline_remove(pd->mapping,
                                                ...
                                                NULL);
        mapping->mapped--;
        pd->mapping = eina_rbtree_inline_insert(pd->mapping,
                                                EINA_RBTREE_GET(mapping),
                                                _filter_mapping_cmp_cb,
                                                NULL);
    }
...
EFL_CALLBACKS_ARRAY_DEFINE(filters_callbacks,
                           { EFL_MODEL_EVENT_CHILD_ADDED,
                             _efl_filter_model_child_added },
                           { EFL_MODEL_EVENT_CHILD_REMOVED,
                             _efl_filter_model_child_removed });
...
_filter_remove_array(Eo *o EFL_UNUSED, void *data, const Eina_Value v)
{
    ...
    {
        Efl_Filter_Model_Data *pd =
            efl_data_scope_get(target, EFL_FILTER_MODEL_CLASS);
...
static Eina_Future *
_efl_filter_model_efl_model_children_slice_get(Eo                    *obj,
                                               Efl_Filter_Model_Data *pd,
                                               unsigned int           start,
                                               unsigned int           count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        mapping[i] =
            (void *)eina_rbtree_inline_lookup(pd->mapping,
                                              &lookup,
                                              sizeof(unsigned int),
                                              _filter_mapping_looking_cb,
                                              NULL);
    ...
    {
        r[i] =
            efl_model_children_slice_get(efl_super(obj, EFL_FILTER_MODEL_CLASS),
                                         mapping[i]->original,
                                         1);
        r[i] = efl_future_then(obj,
                               r[i],
                               .success_type = EINA_VALUE_TYPE_ARRAY,
    ...
    f = efl_future_then(obj,
                        eina_future_all_array(r),
...
static Eina_Value
_efl_filter_model_array_result_request(Eo *o            EFL_UNUSED,
                                       void            *data,
                                       const Eina_Value v)
{
    ...
    EINA_VALUE_ARRAY_FOREACH(&v, len, i, request)
    {
        ...
        if (eina_value_type_get(&request) != EINA_VALUE_TYPE_BOOL) continue;
        ...
        cevt.index = mapping->mapped;
        cevt.child =
            _efl_filter_lookup(efl_class_get(o), o, req->targets[i], mapping);
...
static void
_efl_filter_model_array_result_free(Eo *o                          EFL_UNUSED,
                                    void                          *data,
                                    const Eina_Future *dead_future EFL_UNUSED)
{
    Efl_Filter_Model_Result *req = data;
...
static unsigned int
_efl_filter_model_efl_model_children_count_get(const Eo              *obj,
                                               Efl_Filter_Model_Data *pd)
{
    if (!pd->counting_started && pd->filter.cb)
    {
        pd->counting_started = EFL_TRUE;
        ...
        pd->counted = efl_model_children_count_get(
            efl_super(obj, EFL_FILTER_MODEL_CLASS));
        if (pd->counted > 0)
        {
            Eina_Future *f;

            f = efl_model_children_slice_get(efl_ui_view_model_get(obj),
                                             0,
                                             pd->counted);
            efl_future_then(obj,
                            f,
...
static Eina_Value *
_efl_filter_model_efl_model_property_get(const Eo              *obj,
                                         Efl_Filter_Model_Data *pd,
                                         const char            *property)
{
    ...
    return efl_model_property_get(efl_super(obj, EFL_FILTER_MODEL_CLASS),
                                  property);
}

static unsigned int
_efl_filter_model_efl_composite_model_index_get(const Eo              *obj,
                                                Efl_Filter_Model_Data *pd)
{
    if (pd->self) return pd->self->mapped;
    return efl_composite_model_index_get(
        efl_super(obj, EFL_FILTER_MODEL_CLASS));
}
