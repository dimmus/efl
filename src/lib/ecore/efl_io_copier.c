...
typedef struct _Efl_Io_Copier_Data
{
    ...
    Efl_Bool closed;
    Efl_Bool done;
    Efl_Bool force_dispatch;
    Efl_Bool close_on_exec;
    Efl_Bool close_on_invalidate;
} Efl_Io_Copier_Data;
            ...
            if (!pd->source) DBG("source=NULL");                               \
                ...
                DBG("source={%p %s, refs=%d, can_read=%d, eos=%d, closed=%d}", \
                    ...
                    efl_isa(pd->source, EFL_IO_CLOSER_INTERFACE)               \
                        ...
                        : 0);                                                  \
            if (!pd->destination) DBG("destination=NULL");                     \
            else                                                               \
...
static Eina_Value
_efl_io_copier_timeout_inactivity_cb(Eo              *o,
...
_efl_io_copier_timeout_inactivity_reschedule(Eo *o, Efl_Io_Copier_Data *pd)
{
    ...
    efl_future_then(o,
                    efl_loop_timeout(efl_loop_get(o), pd->timeout_inactivity),
...
_efl_io_copier_job(Eo *o, void *data EFL_UNUSED, const Eina_Value v)
{
    ...
    if (!pd->source || efl_io_reader_eos_get(pd->source))
    {
        if ((!pd->done) &&
            ...
            efl_io_copier_done_set(o, EFL_TRUE);
    }
 ...
 */
static Eina_Slice
_efl_io_copier_dispatch_data_events(Eo                 *o,
                                    Efl_Io_Copier_Data *pd,
                                    Eina_Slice          slice_of_binbuf)
{
    Eina_Slice tmp;
    size_t     offset;
    ...
    {
        CRI("slice_of_binbuf=" EINA_SLICE_FMT
            " must be inside binbuf=" EINA_SLICE_FMT,
            EINA_SLICE_PRINT(slice_of_binbuf),
            EINA_SLICE_PRINT(tmp));
    ...
    if (pd->closed)
        return (Eina_Slice){ .mem = NULL, .len = 0 }; /* cb may call close */
    ...
    */
    ...
    if (pd->line_delimiter.len > 0)
    {
        ...
        if (pd->closed)
            return (Eina_Slice){ .mem = NULL,
                                 .len = 0 }; /* cb may call close */
...
_efl_io_copier_read(Eo *o, Efl_Io_Copier_Data *pd)
{
    ...
    if (pd->buffer_limit > 0)
    {
        ...
        else if (pd->buffer_limit > used)
        {
            ...
            if (expand_size > available) expand_size = available;
        }
    }
    ...
    efl_io_copier_done_set(o, EFL_FALSE);
    ...
    if ((!pd->destination) && (eina_binbuf_length_get(pd->buf) > used))
    {
         ...
         * here.
         */
        _efl_io_copier_dispatch_data_events(o,
                                            pd,
                                            eina_rw_slice_slice_get(rw_slice));
    }
...
_efl_io_copier_write(Eo *o, Efl_Io_Copier_Data *pd)
{
        ...
        (pd->source && !efl_io_reader_eos_get(pd->source)))
    {
        ...
        if (p) ro_slice.len = p - ro_slice.bytes + pd->line_delimiter.len;
...
static void
_efl_io_copier_source_can_read_changed(void                  *data,
...
EFL_CALLBACKS_ARRAY_DEFINE(source_cbs,
                           { EFL_IO_READER_EVENT_CAN_READ_CHANGED,
                             _efl_io_copier_source_can_read_changed },
                           { EFL_IO_READER_EVENT_EOS,
                             _efl_io_copier_source_eos });
...
_efl_io_copier_source_set(Eo *o, Efl_Io_Copier_Data *pd, Efl_Io_Reader *source)
{
    ...
    if (pd->source)
    {
        ...
        {
            efl_event_callback_del(pd->source,
                                   EFL_IO_SIZER_EVENT_SIZE_CHANGED,
                                   _efl_io_copier_source_resized,
                                   o);
            pd->progress.total = 0;
        }
    ...
    if (source)
    {
        ...
        if (efl_isa(pd->source, EFL_IO_SIZER_MIXIN))
        {
            efl_event_callback_add(pd->source,
                                   EFL_IO_SIZER_EVENT_SIZE_CHANGED,
                                   _efl_io_copier_source_resized,
                                   o);
            _efl_io_copier_source_size_apply(o, pd);
...
static void
_efl_io_copier_destination_can_write_changed(void                  *data,
                                             const Efl_Event *event EFL_UNUSED)
{
    Eo                 *o  = data;
    Efl_Io_Copier_Data *pd = efl_data_scope_get(o, MY_CLASS);
...
_efl_io_copier_destination_closed(void *data, const Efl_Event *event EFL_UNUSED)
{
    ...
    {
        if (!pd->done) efl_io_copier_done_set(o, EFL_TRUE);
    }
    else
    {
...
EFL_CALLBACKS_ARRAY_DEFINE(destination_cbs,
                           { EFL_IO_WRITER_EVENT_CAN_WRITE_CHANGED,
                             _efl_io_copier_destination_can_write_changed });
...
EOLIAN static void
_efl_io_copier_destination_set(Eo                 *o,
                               Efl_Io_Copier_Data *pd,
                               Efl_Io_Writer      *destination)
{
    ...
    if (pd->destination)
    {
        ...
        if (efl_isa(pd->destination, EFL_IO_CLOSER_INTERFACE))
        {
            efl_event_callback_del(pd->destination,
                                   EFL_IO_CLOSER_EVENT_CLOSED,
                                   _efl_io_copier_destination_closed,
                                   o);
        }
    ...
    if (destination)
    {
        ...
        if (efl_isa(pd->destination, EFL_IO_SIZER_MIXIN) && pd->source &&
            efl_isa(pd->source, EFL_IO_SIZER_MIXIN))
        {
            efl_io_sizer_resize(pd->destination, pd->progress.total);
        }
    }
...
EOLIAN static void
_efl_io_copier_line_delimiter_set(Eo *o               EFL_UNUSED,
                                  Efl_Io_Copier_Data *pd,
                                  Eina_Slice          slice)
{
...
_efl_io_copier_line_delimiter_get(const Eo *o         EFL_UNUSED,
                                  Efl_Io_Copier_Data *pd)
{
...
EOLIAN static void
_efl_io_copier_read_chunk_size_set(Eo *o               EFL_UNUSED,
                                   Efl_Io_Copier_Data *pd,
                                   size_t              size)
{
    EINA_SAFETY_ON_TRUE_RETURN(pd->closed);
...
EOLIAN static size_t
_efl_io_copier_read_chunk_size_get(const Eo *o         EFL_UNUSED,
                                   Efl_Io_Copier_Data *pd)
{
...
_efl_io_copier_efl_io_closer_close(Eo *o, Efl_Io_Copier_Data *pd)
{
    ...
    while (pd->buf)
    {
                 ...
                 efl_io_writer_can_write_get(pd->destination))
        {
            DBG("copier %p destination %p closed with %zd bytes pending, do "
                ...
                pd->destination,
                pending);
            pd->force_dispatch = EFL_TRUE;
            _efl_io_copier_write(o, pd);
            pd->force_dispatch = EFL_FALSE;
        }
        else if (!pd->destination)
        {
    ...
    if (pd->job) eina_future_cancel(pd->job);
...
_efl_io_copier_efl_io_closer_closed_get(const Eo *o         EFL_UNUSED,
                                        Efl_Io_Copier_Data *pd)
{
    return pd->closed;
}
...
EOLIAN static void
_efl_io_copier_progress_get(const Eo *o         EFL_UNUSED,
                            Efl_Io_Copier_Data *pd,
                            uint64_t           *read,
                            uint64_t           *written,
                            uint64_t           *total)
...
EOLIAN static void
_efl_io_copier_timeout_inactivity_set(Eo                 *o,
                                      Efl_Io_Copier_Data *pd,
                                      double              seconds)
{
...
_efl_io_copier_timeout_inactivity_get(const Eo *o         EFL_UNUSED,
                                      Efl_Io_Copier_Data *pd)
{
                     ...
                     Efl_Bool            ignore_line_delimiter)
{
    ...
    {
        efl_event_callback_call(o, EFL_IO_COPIER_EVENT_PROGRESS, NULL);
        if (pd->closed) return EFL_TRUE; /* cb may call close */
        _efl_io_copier_timeout_inactivity_reschedule(o, pd);
    }
...
_efl_io_copier_done_get(const Eo *o, Efl_Io_Copier_Data *pd)
{
    DBG("%p done=%d pending=%zd source={%p %s, eos=%d, closed=%d}, "
        ...
        pd->source ? (efl_isa(pd->source, EFL_IO_CLOSER_INTERFACE)
                          ? efl_io_closer_closed_get(pd->source)
                          : 0)
                   : 1,
        pd->destination,
        pd->destination ? efl_class_name_get(pd->destination) : "",
        pd->destination ? (efl_isa(pd->destination, EFL_IO_CLOSER_INTERFACE)
                               ? efl_io_closer_closed_get(pd->destination)
                               : 0)
                        : 1);
...
EOLIAN static void
_efl_io_copier_done_set(Eo *o, Efl_Io_Copier_Data *pd, Efl_Bool value)
{
...
{
    pd->buf                 = eina_binbuf_new();
    pd->close_on_exec       = EFL_TRUE;
    pd->close_on_invalidate = EFL_TRUE;
...
EOLIAN static Efl_Bool
_efl_io_copier_efl_io_closer_close_on_exec_set(Eo *o               EFL_UNUSED,
                                               Efl_Io_Copier_Data *pd,
                                               Efl_Bool close_on_exec)
{
...
EOLIAN static Efl_Bool
_efl_io_copier_efl_io_closer_close_on_exec_get(const Eo *o         EFL_UNUSED,
                                               Efl_Io_Copier_Data *pd)
{
...
EOLIAN static Efl_Bool
_efl_io_copier_efl_io_closer_close_on_invalidate_get(const Eo *o EFL_UNUSED,
                                                     Efl_Io_Copier_Data *pd)
{
