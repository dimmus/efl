...
static Efl_Bool    _ecore_exe_data_generic_handler(void             *data,
                                                   Ecore_Fd_Handler *fd_handler,
                                                   Ecore_Exe_Flags   flags);
static Efl_Bool    _ecore_exe_data_error_handler(void             *data,
                                                 Ecore_Fd_Handler *fd_handler);
static Efl_Bool    _ecore_exe_data_read_handler(void             *data,
                                                Ecore_Fd_Handler *fd_handler);
static Efl_Bool    _ecore_exe_data_write_handler(void             *data,
                                                 Ecore_Fd_Handler *fd_handler);
static void        _ecore_exe_flush(Ecore_Exe *obj);
static void _ecore_exe_make_sure_its_dead(void *data, const Efl_Event *event);
static void _ecore_exe_make_sure_its_really_dead(void            *data,
...
#define E_IF_NO_ERRNO(result, foo, ok)                      \
    while (((ok) = _ecore_exe_check_errno((result) = (foo), \
                                          __FILE__,         \
                                          __LINE__)) == -1) \
        sleep(1);                                           \
    if (ok)
...
#  warning "Have support for this"
            ...
            if (exe->cmd)
            {
                if (flags & ECORE_EXE_PIPE_ERROR) /* Setup the error stuff. */
                {
                                  ...
                                  ok){}
                    {
                        ...
                        exe->error_fd_handler = ecore_main_fd_handler_add(
                            exe->child_fd_error,
                            ECORE_FD_READ,
                ...
                if (ok &&
                    (flags & ECORE_EXE_PIPE_READ)) /* Setup the read stuff. */
                {
                    E_IF_NO_ERRNO(
                        result,
                    ...
                    {
                        // XXX: eoify ecore fd handlers
                        exe->read_fd_handler = ecore_main_fd_handler_add(
                            exe->child_fd_read,
                            ECORE_FD_READ,
                ...
                if (ok &&
                    (flags & ECORE_EXE_PIPE_WRITE)) /* Setup the write stuff. */
                {
                    E_IF_NO_ERRNO(
                        result,
                    ...
                    {
                        // XXX: eoify ecore fd handlers
                        exe->write_fd_handler = ecore_main_fd_handler_add(
                            exe->child_fd_write,
                            ECORE_FD_WRITE,
                ...
                Efl_Loop_Data *loop =
                    efl_data_scope_get(exe->loop, EFL_LOOP_CLASS);
    ...
    else
    {
        ...
        if (e)
        {
            ...
            /* Send the event. */
            ecore_event_add(ECORE_EXE_EVENT_ADD,
                            e,
                            _ecore_exe_event_add_free,
                            NULL);
        }
                               ...
                               Ecore_Exe_Flags flags)
{
    ...
    {
        flags = ECORE_EXE_PIPE_READ;
        if (exe->flags & ECORE_EXE_PIPE_READ_LINE_BUFFERED) is_buffered = 1;
    }
    else
    {
        flags = ECORE_EXE_PIPE_ERROR;
        if (exe->flags & ECORE_EXE_PIPE_ERROR_LINE_BUFFERED) is_buffered = 1;
    }
            ...
            for (i = 0; i < inbuf_num; i++) /* Find the lines. */
            {
                if (inbuf[i] == '\n')
                {
                    if (count >= max)
                    {
                        ...
                        lines = realloc(
                            e->lines,
                            sizeof(Ecore_Exe_Event_Data_Line) *
                                (max +
                                 1)); /* Allow room for the NULL termination. */
                        if (!lines)
                        {
...
void
_impl_ecore_exe_signal(Ecore_Exe *obj EFL_UNUSED, Ecore_Exe_Data *exe, int num)
{
        ...
        (ecore_main_fd_handler_active_get(fd_handler, ECORE_FD_READ)))
    {
        ...
        for (;;)
        {
             ...
             * save ourselves a memcpy(). */
                lost_exe =
                    ((errno == EIO) || (errno == EBADF) || (errno == EPIPE) ||
...
{
    return _ecore_exe_data_generic_handler(data,
                                           fd_handler,
                              ...
                              Ecore_Fd_Handler *fd_handler EFL_UNUSED)
{
    ...
    // XXX: use eo method
    if ((exe->write_fd_handler) &&
        (ecore_main_fd_handler_active_get(exe->write_fd_handler,
                                          ECORE_FD_WRITE)))
...
_ecore_exe_flush(Ecore_Exe *obj)
{
    ...
    if (count < 1)
    {
        if (errno == EIO || errno == EBADF || errno == EPIPE ||
            errno == EINVAL || errno == ENOSPC) /* we lost our exe! */
        {
