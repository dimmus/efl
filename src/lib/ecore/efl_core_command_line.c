...
_unescape(const char *s)
{
    ...
    Efl_Bool in_quote_dbl = EFL_FALSE;
    Efl_Bool in_quote     = EFL_FALSE;
...
_efl_core_command_line_command_get(const Eo *obj               EFL_UNUSED,
...
_efl_core_command_line_command_access(Eo *obj                     EFL_UNUSED,
                                      Efl_Core_Command_Line_Data *pd)
{
...
_remove_invalid_chars(char *command)
{
    ...
    {
        char c = command[i];
        if (c < 0x20 || c == 0x7f) command[i] = '\x12';
    }
}
...
_efl_core_command_line_command_array_set(Eo *obj                     EFL_UNUSED,
                                         Efl_Core_Command_Line_Data *pd,
                                         Eina_Array                 *array)
{
    ...
    for (i = 0; i < (array ? eina_array_count(array) : 0); ++i)
    {
        ...
        //build the command
        if (i != 0) eina_strbuf_append(command, " ");
        esc = _escape(content);
...
EOLIAN static Efl_Bool
_efl_core_command_line_command_string_set(Eo *obj EFL_UNUSED,
                                          Efl_Core_Command_Line_Data *pd,
                                          const char                 *str)
{
    EINA_SAFETY_ON_TRUE_RETURN_VAL(pd->filled, EFL_FALSE);
    ...
    if (pd->string_command) _remove_invalid_chars(pd->string_command);
    pd->command = _unescape(str);
    if (!pd->command)
    {
        if (pd->string_command) free(pd->string_command);
...
EOLIAN static void
_efl_core_command_line_efl_object_destructor(Eo *obj EFL_UNUSED,
                                             Efl_Core_Command_Line_Data *pd)
{
