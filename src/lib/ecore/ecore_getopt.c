...
static Efl_Bool _ecore_getopt_desc_is_sentinel(const Ecore_Getopt_Desc *desc);
static Ecore_Getopt_Desc_Arg_Requirement
            _ecore_getopt_desc_arg_requirement(const Ecore_Getopt_Desc *desc);
static void _ecore_getopt_help_desc_setup_metavar(const Ecore_Getopt_Desc *desc,
                                                  char *metavar,
                                                  int  *metavarlen,
                                                  int   maxsize);
...
_ecore_getopt_help_usage(FILE *fp, const Ecore_Getopt *parser)
{
    ...
    if (!parser->usage)
    {
        ...
        if (d->metavar)
        {
            for (; d->metavar != NULL; d++)
            {
                ...
                _ecore_getopt_help_desc_setup_metavar(d,
                                                      metavar,
                                                      &metavarlen,
                                                      sizeof(metavar));

                fputc(' ', fp);
                if (ar != ECORE_GETOPT_DESC_ARG_REQUIREMENT_YES) fputc('[', fp);
                fputs(metavar, fp);
    ...
    _ecore_getopt_help_print_replace_program(fp,
                                             parser,
                                 ...
                                 Efl_Bool                          show_attr)
{
    ...
    if (requirement == ECORE_GETOPT_DESC_ARG_REQUIREMENT_NO) return 0;
                              ...
                              const Ecore_Getopt_Desc *desc)
{
    ...
    used = _ecore_getopt_help_line(fp,
                                   base,
                                   total,
                                   used,
                                   _("Type: "),
    ...
    if (store->arg_req == ECORE_GETOPT_DESC_ARG_REQUIREMENT_YES) goto end;

    used =
        _ecore_getopt_help_line(fp, base, total, used, ". ", sizeof(". ") - 1);
                                ...
                                const Ecore_Getopt_Desc *desc)
{
    ...
    {
        used =
            _ecore_getopt_help_line(fp, base, total, used, *itr, strlen(*itr));
        if (itr[1])
                                   ...
                                   const Ecore_Getopt_Desc *desc)
{
    ...
    return _ecore_getopt_help_line(fp,
                                   base,
                                   total,
                                   used,
                        ...
                        const Ecore_Getopt      *parser)
{
    ...
    if (arg_req != ECORE_GETOPT_DESC_ARG_REQUIREMENT_NO)
        _ecore_getopt_help_desc_setup_metavar(desc,
                                              metavar,
                                              &metavarlen,
                                              sizeof(metavar));
    ...
    if (desc->shortname)
    {
        ...
        used += 2;
        used += _ecore_getopt_help_desc_show_arg(fp,
                                                 arg_req,
                                                 metavar,
                                                 metavarlen,
                                                 EFL_TRUE);
    ...
    switch (desc->action)
    {
        ...
        case ECORE_GETOPT_ACTION_HELP:
            _ecore_getopt_help_desc_categories(fp,
                                               helpcol,
                                               cols,
                                               used,
                                               parser->descs);
...
{
    if (!_ecore_getopt_help_prepare(parser)) return;
                           ...
                           const char         *category)
{
    ...
    if (!category || !_ecore_getopt_help_prepare(parser)) return EFL_FALSE;
    for (desc = parser->descs; !_ecore_getopt_desc_is_sentinel(desc); desc++)
    {
                                      ...
                                      char              **argv)
{
    ...
    while (src < argc)
    {
        ...
        else desc = _ecore_getopt_parse_find_short(parser, arg[1]);
        ...
        if (desc->action == ECORE_GETOPT_ACTION_BREAK) abreak = 1;
        ...
        arg_req = _ecore_getopt_desc_arg_requirement(desc);
        if (arg_req == ECORE_GETOPT_DESC_ARG_REQUIREMENT_NO) continue;
...
static Efl_Bool
_ecore_getopt_parse_bool(const char *str, Efl_Bool *v)
{
                          ...
                          const char                *arg_val)
{
    ...
    switch (store->type)
    {
        ...
        case ECORE_GETOPT_TYPE_SHORT:
            if (!_ecore_getopt_parse_long(arg_val, &v)) goto error;
            *value->shortp = v;
            return EFL_TRUE;
        ...
        case ECORE_GETOPT_TYPE_DOUBLE:
            if (!_ecore_getopt_parse_double(arg_val, &d)) goto error;
            *value->doublep = d;
            break;
    }
                         ...
                         const char                   *arg_val)
{
    ...
    if (arg_val) return ecore_getopt_help_category(stdout, parser, arg_val);
                          ...
                          const char              *arg_val)
{
    switch (desc->action)
    {
        ...
        case ECORE_GETOPT_ACTION_STORE_CONST:
            return _ecore_getopt_parse_store_const(parser,
                                                   desc,
                                                   value,
                                                   arg_val);
        ...
        case ECORE_GETOPT_ACTION_STORE_FALSE:
            return _ecore_getopt_parse_store_false(parser,
                                                   desc,
                                                   value,
                                                   arg_val);
                        ...
                        int                *nonargs)
{
    ...
    if (arg[1] == '-')
        return _ecore_getopt_parse_arg_long(parser,
                                            values,
                                            argc,
                                            argv,
                                            idx,
        ...
        return _ecore_getopt_parse_arg_short(parser,
                                             values,
                                             argc,
                                             argv,
                                             idx,
...
_ecore_getopt_find_help(const Ecore_Getopt *parser)
{
    const Ecore_Getopt_Desc *desc = parser->descs;
    for (; !_ecore_getopt_desc_is_sentinel(desc); desc++)
        if (desc->action == ECORE_GETOPT_ACTION_HELP) return desc;
    return NULL;
}
                   ...
                   char              **argv)
{
    ...
    if ((argc < 1) || (!argv)) ecore_app_args_get(&argc, &argv);
                              ...
                              int                 start)
{
    ...
    for (desc = parser->descs; !_ecore_getopt_desc_is_sentinel(desc); desc++)
        ;
    while (desc->metavar)
        if (!_ecore_getopt_parse_pos(parser,
                                     &desc,
                                     values,
                                     argc,
                                     argv,
