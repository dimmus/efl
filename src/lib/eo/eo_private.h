...
struct _Eo_Object
{
  ...
  Efl_Bool condtor_done       : 1;
  Efl_Bool finalized          : 1;
  Efl_Bool super              : 1;
  Efl_Bool invalidate         : 1;
  Efl_Bool is_invalidating    : 1;
  Efl_Bool parent             : 1;
  Efl_Bool unref_compensate   : 1;
  Efl_Bool allow_parent_unref : 1;

  Efl_Bool      noref_event     : 1;
  Efl_Bool      del_triggered   : 1;
  Efl_Bool      destructed      : 1;
  Efl_Bool      manual_free     : 1;
  unsigned char auto_unref      : 1; // unref after 1 call - hack for parts
  Efl_Bool      ownership_track : 1;
};
...
#define EO_OPTIONAL_COW_END(_cow, _obj) eina_cow_done(efl_object_optional_cow, (const Eina_Cow_Data**)&(_obj->opt), _cow, EFL_TRUE)
...
struct _Efl_Class
{
  ...
  unsigned int
    class_id; /**< the id which can be used to find the slot in _eo_classes and vtables chains */
  ...
  Eina_Thread
    construction_thread; /** < the thread which called the class constructor */

  Efl_Bool constructed   : 1;
  Efl_Bool functions_set : 1;
  /* [extensions*] + NULL */
  /* [mro*] + NULL */
...
void _eo_log_obj_report(const Eo_Id id,
                        int         log_level,
...
static inline Eo *
_eo_header_id_get(const Eo_Header *header)
{
  return (Eo *)header->id;
}
...
static inline Efl_Class *
_eo_class_id_get(const _Efl_Class *klass)
{
  return _eo_header_id_get((Eo_Header *)klass);
}

static inline Eo *
_eo_obj_id_get(const _Eo_Object *obj)
{
...
static inline void
_efl_del_internal(_Eo_Object *obj,
                  const char *func_name,
                  const char *file,
                  int         line)
{
  ...
  if (obj->parent)
  {
    ...
    ERR("Destructor path being taken while object [%s] still has a parent "
        "[%s] in state %i:%i.",
    ...
    if (obj->parent)
    {
      CRI("Something is preventing [%s] from disconnecting from its "
          "parent, bypassing.",
          efl_debug_name_get(_eo_obj_id_get(obj)));
      _efl_object_parent_set(
        _eo_obj_id_get(obj),
        efl_data_scope_get(_eo_obj_id_get(obj), EFL_OBJECT_CLASS),
        NULL);
    }
  }
...
void _eo_free(_Eo_Object *obj, Efl_Bool manual_free);
...
static inline void
_efl_unref_internal(_Eo_Object *obj,
                    const char *func_name,
                    const char *file,
                    int         line)
{
  ...
  if (EINA_UNLIKELY(obj->refcount <= 0))
  {
    ...
    {
      ERR("Object %p is still refcounted %i by users, but internal "
          "refcount reached 0. This should never happen. Please report a "
          "bug and send a backtrace to EFL developer.",
          (Eo *)obj->header.id,
          obj->user_refcount);
      _eo_log_obj_report((Eo_Id)_eo_obj_id_get(obj),
                         EINA_LOG_LEVEL_ERR,
                         __func__,
                         __FILE__,
                         __LINE__);
    ...
    if (obj->refcount < 0)
    {
      ERR("in %s:%d: func '%s' Obj:%p. Refcount (%d) < 0. Too many "
          "unrefs.",
          file,
          line,
          func_name,
    ...
    {
      ERR("in %s:%d: func '%s' Object %p deletion already triggered. You "
          "wrongly call efl_unref() within a destructor.",
          file,
          line,
          func_name,
          _eo_obj_id_get(obj));
    ...
    if (EINA_LIKELY(!obj->manual_free))
    {
      ...
      while (obj->xrefs)
      {
        ...
        Eo_Xref_Node *xref =
          EINA_INLIST_CONTAINER_GET(obj->xrefs, Eo_Xref_Node);
      ...
      while (obj->data_xrefs)
      {
        ...
        {
          WRN("in %s:%d: func '%s' Object %p still has a reference "
              "to its own data (subclass: %s). Origin: %s:%d",
              file,
              line,
              func_name,
              obj_id,
        ...
        {
          ERR("in %s:%d: func '%s' Data of object %p (subclass: %s) "
              "is still referenced by object %p. Origin: %s:%d",
              file,
              line,
              func_name,
              obj_id,
      ...
      _eo_free(obj, EFL_FALSE);
    }
    else _efl_ref(obj); /* If we manual free, we keep a phantom ref. */
  }
}
