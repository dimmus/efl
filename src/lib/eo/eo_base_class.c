...
#define EFL_OBJECT_EVENT_CALLBACK(Event) Efl_Bool event_cb_##Event : 1;
...
struct _Efl_Object_Data
{
  ...
  EFL_OBJECT_EVENT_CALLBACK(
    EFL_EVENT_DESTRUCT);  // No proper count: minor optimization triggered at destruction only
  Efl_Bool callback_stopped : 1;
  Efl_Bool need_cleaning    : 1;

  Efl_Bool allow_parent_unref : 1;  // Allows unref to zero even with a parent
};
...
_efl_pending_futures_clear(Efl_Object_Data *pd)
{
  ...
  {
    Efl_Future_Pending *pending =
      EINA_INLIST_CONTAINER_GET(pd->pending_futures, Efl_Future_Pending);
...
_efl_object_extension_noneed(Efl_Object_Data *pd)
{
  ...
  if ((!ext) || (ext->name) || (ext->comment) || (ext->generic_data) ||
      (ext->wrefs) || (ext->composite_parent) || (ext->providers) ||
      (ext->schedulers) || (ext->forwarders))
    return;
...
_efl_object_invalidate(Eo *obj_id, Efl_Object_Data *pd)
{
  ...
  if (!obj->is_invalidating) efl_parent_set(obj_id, NULL);
...
_eo_generic_data_node_free(Eo_Generic_Data_Node *node)
{
  switch (node->d_type)
  {
    ...
    case DATA_OBJ:
      ...
      efl_event_callback_del(node->d.obj,
                             EFL_EVENT_DEL,
                             _key_generic_cb_del,
                             node);
...
static void
_eo_generic_data_del_all(Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
  ...
  while (ext->generic_data)
  {
    ...
    ext->generic_data =
      eina_inlist_remove(ext->generic_data, EINA_INLIST_GET(node));
...
static void
_eo_key_generic_direct_del(Efl_Object_Data      *pd,
                           Eo_Generic_Data_Node *node,
                           Efl_Bool              call_free)
{
  Efl_Object_Extension *ext = pd->ext;
...
static void
_eo_key_generic_del(const Eo *obj    EFL_UNUSED,
                    Efl_Object_Data *pd,
                    const char      *key,
                    Efl_Bool         call_free)
{
                 ...
                 Efl_Bool                  call_free)
{
  ...
  if (ext)
  {
    ...
    EINA_INLIST_FOREACH(ext->generic_data, node)
    {
      if (!strcmp(node->key, key))
      {
        if ((node->d_type == d_type) && (node->d.ptr == data)) return NULL;
        ext->generic_data =
          eina_inlist_remove(ext->generic_data, EINA_INLIST_GET(node));
        _eo_generic_data_node_free(node);
        break;
  ...
  if (ext)
  {
    ...
    node->d_type = d_type;
    ext->generic_data =
      eina_inlist_prepend(ext->generic_data, EINA_INLIST_GET(node));
    return node;
  }
                 ...
                 Eo_Generic_Data_Node_Type d_type)
{
  ...
  EINA_INLIST_FOREACH(ext->generic_data, node)
  {
    if (node->key && !strcmp(node->key, key))
    {
      ...
      {
        ext->generic_data =
          eina_inlist_promote(ext->generic_data, EINA_INLIST_GET(node));
        return node->d.ptr;
      }
      else
      {
...
static void
_key_generic_cb_del(void *data, const Efl_Event *event EFL_UNUSED)
{
  Eo_Generic_Data_Node *node = data;
  Efl_Object_Data      *pd   = efl_data_scope_get(node->obj, EFL_OBJECT_CLASS);
  _eo_key_generic_direct_del(pd, node, EFL_TRUE);
}

EOLIAN static void
_efl_object_key_data_set(Eo              *obj,
                         Efl_Object_Data *pd,
                         const char      *key,
                         const void      *data)
{
  _key_generic_set(obj, pd, key, data, DATA_PTR, EFL_TRUE);
}

EO_API EFL_VOID_FUNC_BODYV(efl_key_data_set,
                           EFL_FUNC_CALL(key, data),
                           const char *key,
                           const void *data);
...
EO_API EFL_FUNC_BODYV_CONST(efl_key_data_get,
                            void *,
                            NULL,
                            EFL_FUNC_CALL(key),
                            const char *key);
...
EOLIAN static void
_efl_object_key_ref_set(Eo *obj          EFL_UNUSED,
                        Efl_Object_Data *pd,
                        const char      *key,
                        const Eo        *objdata)
{
  ...
  node = _key_generic_set(obj, pd, key, objdata, DATA_OBJ, EFL_TRUE);
  if (node)
  {
    efl_ref(objdata);
    efl_event_callback_add((Eo *)objdata,
                           EFL_EVENT_DEL,
                           _key_generic_cb_del,
                           node);
  }
...
EO_API EFL_VOID_FUNC_BODYV(efl_key_ref_set,
                           EFL_FUNC_CALL(key, objdata),
                           const char       *key,
...
EO_API EFL_FUNC_BODYV_CONST(efl_key_ref_get,
                            Efl_Object *,
                            NULL,
                            EFL_FUNC_CALL(key),
                            const char *key);
...
EOLIAN static void
_efl_object_key_wref_set(Eo               *obj,
                         Efl_Object_Data  *pd,
                         const char       *key,
                         const Efl_Object *objdata)
{
  ...
  if (!_eo_id_domain_compatible(obj, objdata)) return;
  node = _key_generic_set(obj, pd, key, objdata, DATA_OBJ_WEAK, EFL_TRUE);
  if (node)
  {
    efl_event_callback_add((Eo *)objdata,
                           EFL_EVENT_DEL,
...
EO_API EFL_VOID_FUNC_BODYV(efl_key_wref_set,
                           EFL_FUNC_CALL(key, objdata),
                           const char       *key,
                           const Efl_Object *objdata);
...
EO_API EFL_FUNC_BODYV_CONST(efl_key_wref_get,
                            Efl_Object *,
                            NULL,
                            EFL_FUNC_CALL(key),
                            const char *key);
...
EOLIAN static void
_efl_object_key_value_set(Eo *obj          EFL_UNUSED,
                          Efl_Object_Data *pd,
                          const char      *key,
                          Eina_Value      *value)
{
  _key_generic_set(obj, pd, key, value, DATA_VAL, EFL_TRUE);
}

EO_API EFL_VOID_FUNC_BODYV(efl_key_value_set,
                           EFL_FUNC_CALL(key, value),
                           const char *key,
                           Eina_Value *value);
...
EO_API EFL_FUNC_BODYV_CONST(efl_key_value_get,
                            Eina_Value *,
                            NULL,
                            EFL_FUNC_CALL(key),
                            const char *key);
...
EOLIAN static void
_efl_object_name_set(Eo *obj EFL_UNUSED, Efl_Object_Data *pd, const char *name)
{
...
_efl_object_name_get(const Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
...
_name_match(const char *match, Efl_Bool is_glob, const char *str)
{
  if (str)
  {
    if (is_glob)
    {
      ...
      if (!match[0]) return EFL_TRUE;
      // if match string is "*" special case it and match
      if ((match[0] == '*') && (match[1] == 0)) return EFL_TRUE;
      // actual compare
      if (eina_fnmatch(match, str, 0)) return EFL_TRUE;
    }
    else
    {
      // if match string is empty - then it matches - same as "*"
      ...
      if (match == str) return EFL_TRUE;
      // actual compare
      if (!strcmp(match, str)) return EFL_TRUE;
    }
  }
  return EFL_FALSE;
}
...
{
  if (strpbrk(match, "*?[")) return EFL_TRUE;
  return EFL_FALSE;
}
...
_efl_object_name_find(const Eo *obj    EFL_UNUSED,
                      Efl_Object_Data *pd,
                      const char      *search)
{
  Eo         *child;
  ...
  else
  {
    ...
    if ((p = strchr(search, ':')))
    {
      ...
      Efl_Bool klass_glob     = EFL_FALSE;
      Efl_Bool name_glob      = EFL_FALSE;
    ...
    else
    {
      if (_hasglob(search))
      {
        ...
        EINA_INLIST_FOREACH(pd->children, child_eo)
        {
          child = _eo_obj_id_get(child_eo);
          name  = efl_name_get(child);
          if ((name) && (_name_match(search, EFL_TRUE, name))) return child;
          child = efl_name_find(child, search);
          if (child) return child;
        }
      }
      else
      {
        ...
        EINA_INLIST_FOREACH(pd->children, child_eo)
        {
          child = _eo_obj_id_get(child_eo);
          name  = efl_name_get(child);
          if ((name) && (_name_match(search, EFL_FALSE, name))) return child;
          child = efl_name_find(child, search);
          if (child) return child;
        }
      }
...
EOLIAN static void
_efl_object_comment_set(Eo *obj          EFL_UNUSED,
                        Efl_Object_Data *pd,
                        const char      *comment)
{
...
EOLIAN static const char *
_efl_object_comment_get(const Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
  if (!pd->ext) return NULL;
...
EOLIAN static void
_efl_object_debug_name_override(Eo *obj_id          EFL_UNUSED,
                                Efl_Object_Data *pd EFL_UNUSED,
                                Efl_Strbuf *sb      EFL_UNUSED)
{}
...
efl_del(const Eo *obj)
{
  ...
  {
    ERR("Calling efl_del on object %s with no parent is not advised any "
        "more.",
...
{
  obj->is_invalidating = EFL_FALSE;
  obj->invalidate      = EFL_FALSE;
}
...
_efl_object_parent_set(Eo *obj, Efl_Object_Data *pd, Eo *parent_id)
{
  ...
  if (pd->parent)
  {
    Efl_Object_Data *old_parent_pd =
      efl_data_scope_get(pd->parent, EFL_OBJECT_CLASS);

    if (old_parent_pd)
      old_parent_pd->children =
        eina_inlist_remove(old_parent_pd->children, EINA_INLIST_GET(eo_obj));
  ...
  if (parent_id)
  {
    Efl_Object_Data *parent_pd =
      efl_data_scope_get(parent_id, EFL_OBJECT_CLASS);

    if (EINA_LIKELY(parent_pd != NULL))
    {
      pd->parent = parent_id;
      parent_pd->children =
        eina_inlist_append(parent_pd->children, EINA_INLIST_GET(eo_obj));
    ...
    eo_obj->parent = EFL_TRUE;
  }
  else
  {
    pd->parent     = NULL;
    eo_obj->parent = EFL_FALSE;
...
err_parent:
  if (obj == parent_id)
    ERR("New parent %p for object %p will not be set: THIS IS THE SAME "
        "OBJECT.",
        parent_id,
        obj);
  else
...
EOLIAN static Eo *
_efl_object_parent_get(const Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
  return pd->parent;
}

EOLIAN static Efl_Bool
_efl_object_finalized_get(const Eo *obj_id, Efl_Object_Data *pd EFL_UNUSED)
{
  Efl_Bool finalized;
  EO_OBJ_POINTER_RETURN_VAL(obj_id, obj, EFL_FALSE);
...
{
  Efl_Bool invalidate;
  EO_OBJ_POINTER_RETURN_VAL(obj_id, obj, EFL_TRUE);
  invalidate = obj->invalidate;
  EO_OBJ_DONE(obj_id);
  if (!invalidate && pd && pd->parent) return efl_invalidated_get(pd->parent);
  return invalidate;
}

EOLIAN static Efl_Bool
_efl_object_invalidating_get(const Eo *obj_id, Efl_Object_Data *pd EFL_UNUSED)
{
  Efl_Bool invalidating;
  EO_OBJ_POINTER_RETURN_VAL(obj_id, obj, EFL_TRUE);
...
EOLIAN static Efl_Object *
_efl_object_provider_find(const Eo         *obj,
                          Efl_Object_Data  *pd,
...
static Efl_Bool
_efl_object_provider_register(Eo *obj           EFL_UNUSED,
                              Efl_Object_Data  *pd,
                              const Efl_Class  *klass,
                              const Efl_Object *provider)
{
  // The passed object does not provide that said class.
  if (!efl_isa(provider, klass)) return EFL_FALSE;

  _efl_object_extension_need(pd);
  if (!pd->ext) return EFL_FALSE;
  if (!pd->ext->providers)
    pd->ext->providers = eina_hash_pointer_new(EINA_FREE_CB(efl_unref));

  // Prevent double insertion for the same class
  if (eina_hash_find(pd->ext->providers, &klass)) return EFL_FALSE;
...
static Efl_Bool
_efl_object_provider_unregister(Eo *obj           EFL_UNUSED,
                                Efl_Object_Data  *pd,
                                const Efl_Class  *klass,
                                const Efl_Object *provider)
{
...
{
  if (!it->current) return EFL_FALSE;
...
EOLIAN static void
_efl_object_dbg_info_get(Eo *obj                 EFL_UNUSED,
                         Efl_Object_Data *pd     EFL_UNUSED,
                         Efl_Dbg_Info *root_node EFL_UNUSED)
{ /* No info required in the meantime */
  return;
}

EO_API EFL_VOID_FUNC_BODYV(efl_dbg_info_get,
                           EFL_FUNC_CALL(root_node),
...
EO_API
EFL_VOID_FUNC_BODYV(efl_wref_add, EFL_FUNC_CALL(wref), Efl_Object **wref);
...
EO_API
EFL_VOID_FUNC_BODYV(efl_wref_del, EFL_FUNC_CALL(wref), Efl_Object **wref);
...
efl_object_legacy_only_event_description_get(const char *_event_name)
{
  ...
  Efl_Event_Description *event_desc =
    eina_hash_find(_legacy_events_hash, event_name);
  if (!event_desc)
  {
    event_desc              = calloc(1, sizeof(Efl_Event_Description));
    event_desc->name        = event_name;
    event_desc->legacy_is   = EFL_TRUE;
    event_desc->unfreezable = EFL_TRUE;
...
#define CB_COUNT_INC(cnt)             \
    do                                \
    {                                 \
        if ((cnt) != 0xffff) (cnt)++; \
    }                                 \
    while (0)
#define CB_COUNT_DEC(cnt)             \
    do                                \
    {                                 \
        if ((cnt) != 0xffff) (cnt)--; \
    }                                 \
...
#define EFL_OBJECT_EVENT_CB_INC(Obj, It, Pd, Event, Update_Hash) \
    ...
    {                                                            \
        Update_Hash          = EFL_FALSE;                        \
        Pd->event_cb_##Event = EFL_TRUE;                         \
    }
...
#define EFL_OBJECT_EVENT_CB_DEC(Obj, It, Pd, Event) \
    ...
    {                                               \
        if (!efl_event_callback_count(Obj, Event))  \
            Pd->event_cb_##Event = EFL_FALSE;       \
    }

static inline void
_special_event_count_inc(Eo                            *obj_id,
                         Efl_Object_Data               *pd,
                         const Efl_Callback_Array_Item *it)
{
  Efl_Bool update_hash = EFL_TRUE;

  EFL_OBJECT_EVENT_CB_INC(obj_id, it, pd, EFL_EVENT_CALLBACK_ADD, update_hash)
  else EFL_OBJECT_EVENT_CB_INC(obj_id, it, pd, EFL_EVENT_CALLBACK_DEL, update_hash) else EFL_OBJECT_EVENT_CB_INC(
    obj_id,
    it,
    pd,
    EFL_EVENT_DEL,
    update_hash) else EFL_OBJECT_EVENT_CB_INC(obj_id,
                                              it,
                                              pd,
                                              ...
                                              update_hash) else EFL_OBJECT_EVENT_CB_INC(obj_id,
                                                                                        ...
                                                                                        pd,
                                                                                        EFL_EVENT_DESTRUCT,
                                                                                        update_hash) else if (it->desc ==
                                                                                                                EFL_EVENT_NOREF &&
                                                                                                              !pd->event_cb_EFL_EVENT_NOREF)
  {
    update_hash = EFL_FALSE;
    EO_OBJ_POINTER_RETURN(obj_id, obj);
    obj->noref_event = EFL_TRUE;
    EO_OBJ_DONE(obj_id);
    pd->event_cb_EFL_EVENT_NOREF = EFL_TRUE;
  }
  else if (it->desc == EFL_EVENT_OWNERSHIP_SHARED ||
           it->desc == EFL_EVENT_OWNERSHIP_UNIQUE)
  {
    EO_OBJ_POINTER_RETURN(obj_id, obj);
    obj->ownership_track = EFL_TRUE;
    EO_OBJ_DONE(obj_id);
  }
  ...
  if (pd->ext && pd->ext->forwarders)
  {
    ...
    EINA_LIST_FOREACH(eina_hash_find(pd->ext->forwarders, it->desc),
                      l,
                      forwarder)
    {
...
static inline void
_special_event_count_dec(Eo                            *obj_id,
                         Efl_Object_Data               *pd,
                         const Efl_Callback_Array_Item *it)
{
  EFL_OBJECT_EVENT_CB_DEC(obj_id, it, pd, EFL_EVENT_CALLBACK_ADD)
  else EFL_OBJECT_EVENT_CB_DEC(obj_id, it, pd, EFL_EVENT_CALLBACK_DEL) else EFL_OBJECT_EVENT_CB_DEC(
    obj_id,
    it,
    pd,
    EFL_EVENT_DEL) else EFL_OBJECT_EVENT_CB_DEC(obj_id,
                                                it,
                                                pd,
                                                EFL_EVENT_INVALIDATE) else if (it->desc ==
                                                                                 EFL_EVENT_NOREF &&
                                                                               pd->event_cb_EFL_EVENT_NOREF)
  {
    ...
    {
      EO_OBJ_POINTER_RETURN(obj_id, obj);
      obj->noref_event = EFL_FALSE;
      EO_OBJ_DONE(obj_id);

      pd->event_cb_EFL_EVENT_NOREF = EFL_FALSE;
    }
  }
}
...
_eo_callback_remove_all(Efl_Object_Data *pd)
{
  ...
  pd->event_cb_EFL_EVENT_DESTRUCT     = EFL_FALSE;
  pd->event_cb_EFL_EVENT_CALLBACK_ADD = EFL_FALSE;
  pd->event_cb_EFL_EVENT_CALLBACK_DEL = EFL_FALSE;
  pd->event_cb_EFL_EVENT_DEL          = EFL_FALSE;
  pd->event_cb_EFL_EVENT_NOREF        = EFL_FALSE;
  pd->event_cb_EFL_EVENT_INVALIDATE   = EFL_FALSE;
}
...
_eo_callbacks_clear(Eo *obj, Efl_Object_Data *pd)
{
  ...
  else
  {
    ...
    remove_callbacks = EFL_TRUE;
    /* clap to 0 generation */
    generation_clamp = 0;
    /* we dont need to clean later */
    pd->need_cleaning = EFL_FALSE;
  }
...
_eo_callback_search_sorted_near(const Efl_Object_Data         *pd,
                                const Eo_Callback_Description *look)
{
...
_eo_callbacks_sorted_insert(Efl_Object_Data *pd, Eo_Callback_Description *cb)
{
  ...
  if (length > 0)
    memmove(itr + 1, itr, length * sizeof(Eo_Callback_Description *));
  *itr = cb;
  ...
  {
    if ((itr - pd->callbacks) < (ptrdiff_t)frame->idx) frame->inserted_before++;
  }
}
...
EOLIAN static Efl_Bool
_efl_object_event_callback_priority_add(Eo                          *obj,
                                        Efl_Object_Data             *pd,
                                        const Efl_Event_Description *desc,
                                        Efl_Callback_Priority        priority,
                                        Efl_Event_Cb                 func,
                                        const void                  *user_data)
{
  const Efl_Callback_Array_Item_Full arr[] = {
    { desc, priority, func, (void *)user_data },
    { NULL,        0, NULL,              NULL }
  };
  ...
  {
    Eo_Callback_Description **cb_dbg;

    cb_dbg = pd->callbacks + idx - 1;
    if (!(*cb_dbg)->func_array)
    {
      if (((*cb_dbg)->items.item.desc == desc) &&
          ((*cb_dbg)->items.item.func == func) &&
          ((*cb_dbg)->priority == priority) &&
          ((*cb_dbg)->generation == _efl_event_generation(pd)))
        r++;
    }
    entries++;
  }
  if (r > 1)
    INF("Object '%s' got %i callback with event '%s' registered.",
        efl_debug_name_get(obj),
        r,
        desc->name);
  if (entries > 10)
    INF("Object '%s' got %i callbacks.", efl_debug_name_get(obj), entries);
#endif
  ...
  if (cb->generation) pd->need_cleaning = EFL_TRUE;
...
EO_API EFL_FUNC_BODYV(efl_event_callback_priority_add,
                      Efl_Bool,
                      0,
                      EFL_FUNC_CALL(desc, priority, cb, data),
                      const Efl_Event_Description *desc,
                      Efl_Callback_Priority        priority,
                      Efl_Event_Cb                 cb,
                      const void                  *data);
...
static void
_efl_object_event_callback_clean(Eo                                 *obj,
                                 Efl_Object_Data                    *pd,
                                 const Efl_Callback_Array_Item_Full *array,
                                 Eo_Callback_Description           **cb)
{
  (*cb)->delete_me = EFL_TRUE;
  if (pd->event_frame) pd->need_cleaning = EFL_TRUE;
  else _eo_callback_remove(obj, pd, cb);
  if (pd->event_cb_EFL_EVENT_CALLBACK_DEL)
    efl_event_callback_call(obj, EFL_EVENT_CALLBACK_DEL, (void *)array);
}

EOLIAN static Efl_Bool
_efl_object_event_callback_del(Eo                          *obj,
                               Efl_Object_Data             *pd,
                               const Efl_Event_Description *desc,
                               Efl_Event_Cb                 func,
                               const void                  *user_data)
{
  ...
  DBG("Callback of object %p with function %p and data %p not found.",
      obj,
      func,
      user_data);
  return EFL_FALSE;
...
EO_API EFL_FUNC_BODYV(efl_event_callback_del,
                      Efl_Bool,
                      0,
                      EFL_FUNC_CALL(desc, func, user_data),
                      const Efl_Event_Description *desc,
                      Efl_Event_Cb                 func,
                      const void                  *user_data);
  ...
  const void                    *user_data)
{
  ...
  for (idx = pd->callbacks_count; idx > 0; idx--)
  {
    ...
    if ((*cb_dbg)->func_array)
    {
      if (((*cb_dbg)->items.item_array == array) &&
          ((*cb_dbg)->priority == priority) &&
          ((*cb_dbg)->generation == _efl_event_generation(pd)))
        r++;
    }
...
err:
  ERR("Tried adding array of callbacks with invalid values: cb: %p array: "
      "%p.",
      cb,
      array);
  _eo_callback_free(cb);
...
EO_API EFL_FUNC_BODYV(efl_event_callback_array_priority_add,
                      Efl_Bool,
                      0,
                      EFL_FUNC_CALL(array, priority, data),
                      const Efl_Callback_Array_Item *array,
                      Efl_Callback_Priority          priority,
                      const void                    *data);

EOLIAN static Efl_Bool
_efl_object_event_callback_array_del(Eo                            *obj,
                                     Efl_Object_Data               *pd,
                                     const Efl_Callback_Array_Item *array,
                                     const void                    *user_data)
{
  ...
  for (cb = pd->callbacks, j = 0; j < pd->callbacks_count; cb++, j++)
  {
    if (!(*cb)->delete_me && ((*cb)->items.item_array == array) &&
...
static void
_futures_dispatch_cb(void *data, const Efl_Event *ev EFL_UNUSED)
{
                            ...
                            Eina_Value               value)
{
  ...
  entry =
    eina_mempool_malloc(_efl_future_scheduler_entry_mempool, sizeof(*entry));
...
_efl_object_event_future_scheduler_get(const Eo                *obj,
                                       Efl_Object_Data         *pd,
                                       Efl_Callback_Array_Item *array)
{
  ...
  if (!ext->schedulers)
    ext->schedulers = eina_hash_pointer_new(_futures_cancel_cb);
...
EO_API unsigned int
_efl_object_event_callback_count(const Eo *obj                EFL_UNUSED,
                                 Efl_Object_Data             *pd,
                                 const Efl_Event_Description *desc)
{
  unsigned int r = 0;
...
static Efl_Bool
_cb_desc_match(const Efl_Event_Description *a,
               const Efl_Event_Description *b,
               Efl_Bool                     legacy_compare)
{
  /* If one is legacy and the other is not, strcmp. Else, pointer compare. */
  if (!EINA_UNLIKELY(legacy_compare &&
                     (_legacy_event_desc_is(a) != _legacy_event_desc_is(b))))
    return (a == b);
...
#define EFL_OBJECT_EVENT_CALLBACK_BLOCK(Pd, Desc, Event, Need_Hash) \
    if (Desc == Event)                                              \
    {                                                               \
        if (!(Pd->event_cb_##Event)) return EFL_TRUE;               \
        Need_Hash = EFL_FALSE;                                      \
    }

static inline Efl_Bool
_event_callback_call(Eo                          *obj_id,
                     Efl_Object_Data             *pd,
                     const Efl_Event_Description *desc,
                     void                        *event_info,
                     Efl_Bool                     legacy_compare)
{
  Eo_Callback_Description **cb;
  Efl_Event_Callback_Frame *restart_lookup =
    NULL;  //a pointer to a frame, which is high up the stack, which we use to restore
  Efl_Event                ev;
  unsigned int             idx;
  Efl_Bool                 callback_already_stopped, ret;
  Efl_Event_Callback_Frame frame = {
    .desc            = desc,
    .next            = NULL,
    .idx             = 0,
  ...
  Efl_Bool need_hash = EFL_TRUE;

  if (pd->callbacks_count == 0) return EFL_TRUE;
  else
    EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd, desc, EFL_EVENT_CALLBACK_ADD, need_hash)
  else EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd, desc, EFL_EVENT_CALLBACK_DEL, need_hash) else EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd, desc, EFL_EVENT_DEL, need_hash) else EFL_OBJECT_EVENT_CALLBACK_BLOCK(
    pd,
    desc,
    EFL_EVENT_INVALIDATE,
    need_hash) else EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd,
                                                    desc,
                                                    EFL_EVENT_NOREF,
                                                    need_hash) else EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd,
                                                                                                    desc,
    ...
    if (EINA_LIKELY(!legacy_compare && need_hash))
  {
    ...
    if (!(pd->callbacks_mask & (1ULL << event_hash))) return EFL_TRUE;
  }
...
restart_back:
  ...
  for (; idx > 0; idx--)
  {
    ...
    if (!(*cb)->delete_me)
    {
      if ((*cb)->generation >= frame.generation) continue;
      ...
      if ((*cb)->func_array)
      {
        ...
        for (it = (*cb)->items.item_array; it->func; it++)
        {
          ...
          if (!legacy_compare &&
            ...
            break;
          if (!_cb_desc_match(it->desc, desc, legacy_compare)) continue;
...
restart:
  ...
  for (restart_lookup = frame.next; restart_lookup;
  ...
  if (!idx) idx = pd->callbacks_count;

  goto restart_back;
}

EOLIAN static Efl_Bool
_efl_object_event_callback_call(Eo                          *obj_id,
                                Efl_Object_Data             *pd,
                                const Efl_Event_Description *desc,
                                void                        *event_info)
{
  return _event_callback_call(obj_id, pd, desc, event_info, EFL_FALSE);
}

EO_API EFL_FUNC_BODYV(efl_event_callback_call,
                      Efl_Bool,
                      0,
                      EFL_FUNC_CALL(desc, event_info),
                      const Efl_Event_Description *desc,
                      void                        *event_info);

EOLIAN static Efl_Bool
_efl_object_event_callback_legacy_call(Eo                          *obj_id,
                                       Efl_Object_Data             *pd,
                                       const Efl_Event_Description *desc,
                                       void                        *event_info)
{
  return _event_callback_call(obj_id, pd, desc, event_info, EFL_TRUE);
}
...
EOLIAN static void
_efl_object_event_callback_stop(Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
  pd->callback_stopped = EFL_TRUE;
}
...
EOLIAN static void
_efl_object_event_callback_forwarder_priority_add(
  Eo                          *obj,
  Efl_Object_Data *pd          EFL_UNUSED,
  const Efl_Event_Description *desc,
  short                        priority,
  Eo                          *new_obj)
{
  ...
  {
    if (forwarder->desc == desc && forwarder->new_obj == new_obj &&
        forwarder->source == obj)
    {
      ERR("Forwarder added on '%s' for event '%s' toward '%s' has "
          "already been set.\n",
          efl_debug_name_get(obj),
          desc->name,
          efl_debug_name_get(new_obj));
      return;
    }
  }
  ...
  {
    efl_event_callback_priority_add(obj,
                                    desc,
                                    priority,
                                    _efl_event_forwarder_callback,
                                    new_obj);
...
EOLIAN static void
_efl_object_event_callback_forwarder_del(Eo                 *obj,
                                         Efl_Object_Data *pd EFL_UNUSED,
                                         const Efl_Event_Description *desc,
                                         Eo                          *new_obj)
{
...
EOLIAN static void
_efl_object_event_freeze(Eo *obj EFL_UNUSED, Efl_Object_Data *pd)
{
...
EOLIAN static int
_efl_object_event_freeze_count_get(const Eo *obj    EFL_UNUSED,
                                   Efl_Object_Data *pd)
{
...
EOLIAN static Efl_Bool
_efl_object_composite_attach(Eo                 *parent_id,
                             Efl_Object_Data *pd EFL_UNUSED,
                             Eo                 *comp_obj_id)
{
  ...
  EO_OBJ_POINTER_RETURN_VAL(comp_obj_id, comp_obj, EFL_FALSE);
  ...
  opt->composite_objects =
    eina_list_prepend(opt->composite_objects, comp_obj_id);
...
EOLIAN static Efl_Bool
_efl_object_composite_detach(Eo                 *parent_id,
                             Efl_Object_Data *pd EFL_UNUSED,
                             Eo                 *comp_obj_id)
{
  Efl_Object_Optional *opt;
  ...
  opt = EO_OPTIONAL_COW_WRITE(parent);
  opt->composite_objects =
    eina_list_remove(opt->composite_objects, comp_obj_id);
  EO_OPTIONAL_COW_END(opt, parent);

  /* Clear the comp parent on the child. */
  {
    Efl_Object_Data *comp_pd =
      efl_data_scope_get(comp_obj_id, EFL_OBJECT_CLASS);
...
EOLIAN static Efl_Bool
_efl_object_composite_part_is(Eo *comp_obj_id EFL_UNUSED, Efl_Object_Data *pd)
{
...
static Efl_Bool
_eo_dbg_info_flush(const Eina_Value_Type *type EFL_UNUSED, void *_mem)
{
...
static Efl_Bool
_eo_dbg_info_copy(const Eina_Value_Type *type EFL_UNUSED,
                  const void                 *_src,
                  void                       *_dst)
{
...
static Efl_Bool
_eo_dbg_info_convert_to(const Eina_Value_Type *type EFL_UNUSED,
...
static Efl_Bool
_eo_dbg_info_pset(const Eina_Value_Type *type EFL_UNUSED,
                  void                       *_mem,
                  const void                 *_ptr)
{
...
static Efl_Bool
_eo_dbg_info_pget(const Eina_Value_Type *type EFL_UNUSED,
                  const void                 *_mem,
                  void                       *_ptr)
{
...
static const Eina_Value_Type _EFL_DBG_INFO_TYPE = { EINA_VALUE_TYPE_VERSION,
                                                    sizeof(Efl_Dbg_Info *),
                                                    "Efl_Dbg_Info_Ptr",
                                                    _eo_dbg_info_setup,
                                                    _eo_dbg_info_flush,
               ...
               const Eina_Future *dead_future)
{
  ...
  pd->pending_futures =
    eina_inlist_remove(pd->pending_futures, EINA_INLIST_GET(pending));
  efl_ref(o);
  EASY_FUTURE_DISPATCH(ret,
                       value,
                       dead_future,
                       &pending->desc,
                       (void *)o,
                       (void *)pending->desc.data);
  efl_unref(o);
...
efl_future_cb_from_desc(const Eo *o, const Efl_Future_Cb_Desc desc)
{
  ...
  pd->pending_futures =
    eina_inlist_append(pd->pending_futures, EINA_INLIST_GET(pending));
  storage = pending->desc.storage;
end:
  return (Eina_Future_Desc){ .cb      = _efl_future_cb,
                             .data    = pending,
                             .storage = storage };
}
                       ...
                       const Efl_Future_Cb_Desc descs[])
{
  ...
  for (i = 0; descs[i].success || descs[i].error || descs[i].free ||
              descs[i].success_type;
       i++)
  {
...
EOLIAN static Eo *
_efl_object_constructor(Eo *obj, Efl_Object_Data *pd EFL_UNUSED)
{
...
err_parent_back:
  ...
  if (EINA_UNLIKELY(pd->event_cb_EFL_EVENT_DESTRUCT))
    _event_callback_call(obj, pd, EFL_EVENT_DESTRUCT, NULL, EFL_FALSE);
...
composite_obj:
  {
    ...
    EINA_LIST_FOREACH_SAFE(obj_data2->opt->composite_objects,
                           itr,
                           next,
                           emb_obj_id)
    {
...
err_parent:
  if (EINA_LIKELY(!pd->allow_parent_unref))
    ERR("Object '%p' of type '%s' still has a parent at the time of "
        "destruction.",
        obj,
        efl_class_name_get(obj));
...
EOLIAN static void
_efl_object_allow_parent_unref_set(Eo              *obj_id,
                                   Efl_Object_Data *pd,
                                   Efl_Bool         allow)
{
  EO_OBJ_POINTER_RETURN(obj_id, obj);
...
EOLIAN static Efl_Bool
_efl_object_allow_parent_unref_get(const Eo *obj_id EFL_UNUSED,
                                   Efl_Object_Data *pd)
{
...
EO_API void
___efl_auto_unref_set(Eo *obj_id, Efl_Bool enable)
{
  // Write-only property
  EO_OBJ_POINTER_RETURN(obj_id, obj);
...
EOLIAN static Eo *
_efl_object_finalize(Eo *obj, Efl_Object_Data *pd EFL_UNUSED)
{
  return obj;
}

EOLIAN static void
_efl_object_class_constructor(Efl_Class *klass EFL_UNUSED)
{
  event_freeze_count  = 0;
  _legacy_events_hash = eina_hash_stringshared_new(_legacy_events_hash_free_cb);

  _eo_callback_mempool = eina_mempool_add("chained_mempool",
                                          NULL,
                                          NULL,
                                          sizeof(Eo_Callback_Description),
                                          256);

  _efl_pending_future_mempool = eina_mempool_add("chained_mempool",
                                                 NULL,
                                                 NULL,
                                                 sizeof(Efl_Future_Pending),
                                                 256);
  ...
  _efl_future_scheduler_entry_mempool =
    eina_mempool_add("chained_mempool",
                     NULL,
                     NULL,
                     sizeof(Efl_Future_Scheduler_Entry),
                     256);
...
EOLIAN static void
_efl_object_class_destructor(Efl_Class *klass EFL_UNUSED)
{
...
#define EFL_OBJECT_EXTRA_OPS                                               \
    EFL_OBJECT_OP_FUNC(efl_event_callback_priority_add,                    \
                       _efl_object_event_callback_priority_add),           \
        EFL_OBJECT_OP_FUNC(efl_event_callback_del,                         \
                           _efl_object_event_callback_del),                \
        ...
        EFL_OBJECT_OP_FUNC(efl_event_future_scheduler_get,                 \
                           _efl_object_event_future_scheduler_get),        \
        EFL_OBJECT_OP_FUNC(efl_event_callback_count,                       \
                           _efl_object_event_callback_count),              \
