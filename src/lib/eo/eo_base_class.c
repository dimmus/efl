                 ...
                 Eo_Generic_Data_Node_Type d_type)
{
    ...
    EINA_INLIST_FOREACH(ext->generic_data, node)
    {
        if (node->key && !strcmp(node->key, key))
        {
            ...
            {
                ext->generic_data = eina_inlist_promote(ext->generic_data,
                                                        EINA_INLIST_GET(node));
                return node->d.ptr;
            }
            else
        ...
        else
        {
            if (_hasglob(search))
            {
                ...
                EINA_INLIST_FOREACH(pd->children, child_eo)
                {
                    child = _eo_obj_id_get(child_eo);
                    name  = efl_name_get(child);
                    if ((name) && (_name_match(search, EFL_TRUE, name)))
                        return child;
                    child = efl_name_find(child, search);
                    if (child) return child;
                }
            ...
            else
            {
                ...
                EINA_INLIST_FOREACH(pd->children, child_eo)
                {
                    child = _eo_obj_id_get(child_eo);
                    name  = efl_name_get(child);
                    if ((name) && (_name_match(search, EFL_FALSE, name)))
                        return child;
                    child = efl_name_find(child, search);
                    if (child) return child;
                }
...
_eo_callbacks_sorted_insert(Efl_Object_Data *pd, Eo_Callback_Description *cb)
{
    ...
    if (_eo_nostep_alloc || (pd->callbacks_count & 0xF) == 0x0)
    {
        ...
        tmp =
            realloc(pd->callbacks, new_len * sizeof(Eo_Callback_Description *));
                            ...
                            Eina_Value               value)
{
    ...
    entry = eina_mempool_malloc(_efl_future_scheduler_entry_mempool,
                                sizeof(*entry));
                     ...
                     Efl_Bool                     legacy_compare)
{
    ...
    if (pd->callbacks_count == 0) return EFL_TRUE;
        ...
        EFL_OBJECT_EVENT_CALLBACK_BLOCK(pd,
                                        desc,
        ...
        if (!(*cb)->delete_me)
        {
            ...
            if ((*cb)->func_array)
            {
                ...
                for (it = (*cb)->items.item_array; it->func; it++)
                {
                    ...
                    if (!legacy_compare && ((const unsigned char *)desc <
                                            (const unsigned char *)it->desc))
                        break;
            ...
            {
                if (!_cb_desc_match((*cb)->items.item.desc,
                                    desc,
                                    legacy_compare))
                    continue;
                             ...
                             Eo                 *comp_obj_id)
{
    ...
    /* Don't composite if we already have a composite object of this type */
    {
        ...
        EINA_LIST_FOREACH(parent->opt->composite_objects, itr, emb_obj_id)
        {
            ...
            if (EINA_UNLIKELY(emb_obj->klass == comp_obj->klass))
                goto err_klass;
        }
        emb_obj_id = NULL;
    }
