...
#  include <memcheck.h>
#endif
static Efl_Bool _eo_trash_bypass = EFL_FALSE;
...
static int _eo_log_objs_backtrace =
    EO_LOG_OBJS_BACKTRACE_DEFAULT;  // 0 = enable tracking but no bt, N>0 = enable tracking and bt of size N
...
static const char  *_eo_ref_op_str[] = {
    "ERROR", "New  ", "Free ", "Ref  ", "Unref", "Reuse",
};
...
static inline void
_eo_log_obj_ref_op(const _Eo_Object *obj EFL_UNUSED,
                   Eo_Ref_Op ref_op      EFL_UNUSED)
{}

void
_eo_log_obj_report(const Eo_Id id        EFL_UNUSED,
                   int log_level         EFL_UNUSED,
                   const char *func_name EFL_UNUSED,
...
static inline void *_efl_data_scope_get(const _Eo_Object *obj,
                                        const _Efl_Class *klass);
static inline void *_efl_data_xref_internal(const char       *file,
                                            int               line,
                                            _Eo_Object       *obj,
                                            const _Efl_Class *klass,
                                            const _Eo_Object *ref_obj);
static inline void  _efl_data_xunref_internal(_Eo_Object       *obj,
                                              void             *data,
                                              const _Eo_Object *ref_obj);
...
#define ID_CLASS_GET(id)                                           \
    ({                                                             \
        (_Efl_Class *)(((_UNMASK_ID(id) <= _eo_classes_last_id) && \
                        (_UNMASK_ID(id) > 0))                      \
                           ? (_eo_classes[_UNMASK_ID(id) - 1])     \
                           : NULL);                                \
    })

#define EFL_OBJECT_OP_CLASS_PART(op) op >> 16
#define EFL_OBJECT_OP_FUNC_PART(op)  op & 0xffff
#define EFL_OBJECT_OP_CREATE_OP_ID(class_id, func_id) \
    ((unsigned short)class_id) << 16 | ((unsigned short)func_id & 0xffff)
...
_vtable_mro_free(const _Efl_Class *klass)
{
    ...
    for (; *mro_itr; mro_itr++)
    {
        ...
        if ((*mro_itr) == klass) continue;
        for (unsigned int i = 0; i < mro_vtable->size; ++i)
        {
            if (i == klass->class_id) continue;
            if (vtable->chain[i].funcs &&
                mro_vtable->chain[i].funcs == vtable->chain[i].funcs)
            {
 ...
 */
static void
_vtable_prepare_empty_node(Eo_Vtable   *dest,
                           unsigned int length,
                           unsigned int class_id)
{
    dest->chain[class_id].count = length;
    dest->chain[class_id].funcs =
        _vtable_alloc(sizeof(op_type_funcs), dest->chain[class_id].count);
}
 ...
 */
static void
_vtable_merge_defined_api(Eo_Vtable       *dest,
                          const Eo_Vtable *src,
                          Efl_Bool        *hitmap)
{
    for (unsigned int i = 0; i < src->size; ++i)
    {
        ...
        if (src->chain[i].funcs)
        {
            ...
            {
                dest->chain[i] = src->chain[i];
                EINA_SAFETY_ON_FALSE_RETURN(hitmap[i] == EFL_FALSE);
            }
            else
            {
                if (!hitmap[i])
                {
                    ...
                    if (!node.count) _vtable_insert_empty_funcs(dest, i);
                    else
                        _vtable_copy_node(
                            &dest->chain[i],
                            &node);  //we copy what we have, and overwrite in the later for loop
                    hitmap[i] = EFL_TRUE;
                }
 ...
 */
static void
_vtable_merge_empty(Eo_Vtable *dest, const Eo_Vtable *src, Efl_Bool *hitmap)
{
    for (unsigned int i = 0; i < src->size; ++i)
    {
...
_vtable_func_get(const Eo_Vtable *vtable, Efl_Object_Op op)
{
    ...
    if (EINA_UNLIKELY(vtable->size <= class_id)) return NULL;
    if (EINA_UNLIKELY(vtable->chain[class_id].count <= func_id)) return NULL;

    return &vtable->chain[class_id].funcs[func_id];
}

static inline Efl_Bool
_vtable_func_set(Eo_Vtable        *vtable,
                 const _Efl_Class *klass,
                 const _Efl_Class *hierarchy_klass,
                 Efl_Object_Op     op,
                 Eo_Op_Func_Type   func,
                 Efl_Bool          allow_same_override)
{
    op_type_funcs  *fsrc;
    unsigned short  class_id     = EFL_OBJECT_OP_CLASS_PART(op);
    unsigned short  func_id      = EFL_OBJECT_OP_FUNC_PART(op);
    ...
    EINA_SAFETY_ON_FALSE_RETURN_VAL(vtable->size >= class_id, EFL_FALSE);
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(node->funcs, EFL_FALSE);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(node->count >= func_id, EFL_FALSE);
    ...
    else
    {
        if (!allow_same_override && (fsrc->src == klass))
        {
            ...
            ERR("Class '%s': Overriding already set func %p for op %d (%s) "
                "with %p.",
                klass->desc->name,
                fsrc->func,
                op,
...
#  ifdef HAVE_DLADDR
    static Dl_info info;
    if (dladdr(desc->api_func, &info) != 0) fct_name = info.dli_sname;
#  endif
#else
...
static inline const op_type_funcs *
_eo_kls_itr_next(const _Efl_Class *orig_kls,
    ...
    if (*kls_itr)
    {
        ...
        while (*kls_itr)
        {
            const op_type_funcs *fsrc =
                _vtable_func_get(&(*kls_itr)->vtable, op);
            if (!fsrc || !fsrc->func)
            {
                kls_itr++;
                continue;
...
_apply_auto_unref(_Eo_Object *obj, const Eo *eo_obj)
{
    ...
    {
        if (obj->finalized && !(--obj->auto_unref)) efl_unref(eo_obj);
    }
}
...
static Eo *
_efl_super_cast(const Eo *eo_id, const Efl_Class *cur_klass, Efl_Bool super)
{
    EO_CLASS_POINTER_GOTO(cur_klass, super_klass, err);

#ifdef EO_DEBUG
    if (EINA_UNLIKELY(!_eo_is_a_obj(eo_id) && !_eo_is_a_class(eo_id)))
        goto err_obj;
#endif

    if (EINA_UNLIKELY(!_eo_is_a_obj(eo_id))) goto do_klass;
...
#ifdef EO_DEBUG
err_obj:
    _EO_POINTER_ERR(eo_id,
                    "Object (%p) is an invalid ref, class=%p (%s).",
                    eo_id,
                    cur_klass,
...
err_obj_hierarchy:
    _EO_POINTER_ERR(
        eo_id,
        "Object (%p) class=%p (%s) is not an instance of class=%p (%s).",
        eo_id,
        efl_class_get(eo_id),
        efl_class_name_get(eo_id),
        cur_klass,
        efl_class_name_get(cur_klass));
    return NULL;
}
...
{
    return _efl_super_cast(eo_id, cur_klass, EFL_TRUE);
}

EO_API Eo *
efl_cast(const Eo *eo_id, const Efl_Class *cur_klass)
{
    return _efl_super_cast(eo_id, cur_klass, EFL_FALSE);
}

EO_API Efl_Bool
_efl_object_call_resolve(Eo                      *eo_id,
                         const char              *func_name,
                         Efl_Object_Op_Call_Data *call,
                         Efl_Object_Op            op,
                         const char              *file,
                         int                      line)
{
    ...
    Efl_Bool             super = EFL_TRUE;

    if (EINA_UNLIKELY(!eo_id)) goto on_null;

    EO_OBJ_POINTER_RETURN_VAL_PROXY(eo_id, _obj, EFL_FALSE);
...
composite_continue:
    ...
    if (cur_klass)
    {
        ERR("in %s:%d: func '%s' (%d) could not be resolved on %s for class "
            "'%s' for super of '%s'.",
            file,
            line,
            func_name,
            op,
            efl_debug_name_get(eo_id),
...
on_null:
    ...
    {
        _efl_del_api_op_id = _efl_object_api_op_id_get_internal(
            EFL_FUNC_COMMON_OP_FUNC(efl_del));
...
EO_API Efl_Object_Op
_efl_object_op_api_id_get(const void *api_func,
                          const Eo   *eo_obj,
 ...
 * use when validating. */
static Efl_Bool
_eo_class_funcs_set(Eo_Vtable            *vtable,
                    const Efl_Object_Ops *ops,
                    const _Efl_Class     *hierarchy_klass,
                    const _Efl_Class     *klass,
                    Efl_Bool              override_only,
                    unsigned int          class_id,
                    Efl_Bool             *hitmap)
{
    unsigned int              i, j;
    ...
    if (!override_only) _vtable_insert_empty_funcs(vtable, class_id);
    if (!op_descs || !ops->count) return EFL_TRUE;
    ...
    if (!override_only)
    {
        if (number_of_new_functions)
        {
            ...
            _vtable_prepare_empty_node(vtable,
                                       number_of_new_functions,
                                       class_id);
        }
        hitmap[class_id] = EFL_TRUE;
    }
    ...
    for (i = 0, j = 0, op_desc = op_descs; i < ops->count; i++, op_desc++)
    {
        ...
        if (!hitmap[op2_class_id])
        {
            ...
            hitmap[op2_class_id] = EFL_TRUE;
        }
        if (!_vtable_func_set(vtable,
                              klass,
                              override_class,
                              op2,
                              op_desc->func,
    ...
    const Efl_Object_Property_Reflection_Ops *reflection_table)
{
    ...
    }
    {
        ...
        {
            Efl_Object_Op op = _efl_object_api_op_id_get_internal(
                object_ops->descs[i].api_func);
            if (op == EFL_NOOP)
                continue;  //EFL_NOOP means that this function is not yet defined, this will be handled later
            short class_id = EFL_OBJECT_OP_CLASS_PART(op);
            if (klass->vtable.chain[class_id].count == 0)
            {
                ...
                if (klass->desc->type == EFL_CLASS_TYPE_MIXIN)
                {
                    ...
                    _vtable_prepare_empty_node(
                        &klass->vtable,
                        required_klass->vtable.chain[class_id].count,
                        class_id);
                    hitmap[class_id] = EFL_TRUE;
                }
                else
                {
                    ERR("There is an API implemented, whoms type is not part "
                        "of this class. %s vs. %s",
                        klass->desc->name,
    ...
    return _eo_class_funcs_set(&klass->vtable,
                               object_ops,
                               klass,
                               klass,
                               EFL_FALSE,
...
static Eo *
_efl_add_internal_start_do(const char            *file,
                           int                    line,
                           const Efl_Class       *klass_id,
                           Eo                    *parent_id,
                           Efl_Bool               ref,
                           Efl_Bool               is_fallback,
                           ...
                           void                  *sub_ctor_data)
{
    ...
    if (!ref && !parent_id)
        ERR("Creation of '%s' object at line %i in '%s' is done without "
            "parent. This should use efl_add_ref.",
            klass->desc->name,
            line,
            file);
...
err_newid:
    ...
    return NULL;
err_noreg:
    ERR("in %s:%d: Class '%s' is not instantiate-able. Aborting.",
        file,
        line,
        klass->desc->name);
    if (parent_id) EO_OBJ_DONE(parent_id);
...
err_klass:
    _EO_POINTER_ERR(klass_id,
                    "in %s:%d: Class (%p) is an invalid ref.",
                    file,
                    line,
                    klass_id);
err_parent:
...
EO_API Eo *
_efl_add_internal_start(const char      *file,
                        int              line,
                        const Efl_Class *klass_id,
                        Eo              *parent_id,
                        Efl_Bool         ref,
                        Efl_Bool         is_fallback)
{
    return _efl_add_internal_start_do(file,
                                      line,
                                      klass_id,
                                      parent_id,
                                      ref,
...
EO_API Eo *
_efl_add_internal_start_bindings(const char            *file,
                                 int                    line,
                                 const Efl_Class       *klass_id,
                                 Eo                    *parent_id,
                                 Efl_Bool               ref,
...
EO_API Eo *
_efl_add_end(Eo *eo_id, Efl_Bool is_ref, Efl_Bool is_fallback)
{
...
void
_eo_free(_Eo_Object *obj, Efl_Bool manual_free EFL_UNUSED)
{
    ...
    if (manual_free)
    {
        ...
        if (obj->datarefcount)
        {
            ERR("Object %p data still referenced %d time(s).",
                obj_id,
                obj->datarefcount);
        }
        while (obj->xrefs)
...
static Eina_List *
_eo_class_mro_init(const Efl_Class_Description *desc,
                   ...
                   Eina_List                   *extensions)
{
    ...
    {
        if (extn->desc->type != EFL_CLASS_TYPE_MIXIN) continue;
    ...
    /* Check if we can create a consistent mro */
    {
        ...
        EINA_LIST_FOREACH(extensions, extn_itr, extn)
        {
            ...
            if (eina_list_data_find_list(extn_list, extn))
            {
                ...
                ERR("Cannot create a consistent method resolution order for "
                    "class '%s' because of '%s'.",
                    desc->name,
                    extn->desc->name);
                return NULL;
            }
...
EO_API const Efl_Class *
efl_class_new(const Efl_Class_Description *desc,
              ...
              ...)
{
    ...
    /* Prepare mixins list */
    {
        ...
        mixins_sz =
            sizeof(Eo_Extension_Data_Offset) * (eina_list_count(mixins) + 1);
    ...
    klass->extn_data_off =
        (Eo_Extension_Data_Offset *)((char *)klass->mro + mro_sz);
    ...
    /* Feed the mixins data offsets and free the mixins list. */
    {
        ...
        EINA_LIST_FREE(mixins, kls_itr)
        {
            ...
            extn_data_off +=
                EO_ALIGN_SIZE(extn_data_itr->klass->desc->data_size);
            extn_data_itr++;
...
{
    EO_OBJ_POINTER_RETURN_VAL(eo_id, obj, EFL_FALSE);
    ...
    if (ops)
    {
        ...
        hitmap = alloca(vtable->size * sizeof(Efl_Bool));
        memset(hitmap, 0, vtable->size);

        if (!_eo_class_funcs_set(vtable,
                                 ops,
                                 obj->klass,
                                 klass,
                                 EFL_TRUE,
...
efl_isa(const Eo *eo_id, const Efl_Class *klass_id)
{
    ...
    Efl_Bool          isa = EFL_FALSE;

    if (EINA_UNLIKELY(!eo_id)) return EFL_FALSE;
    ...
    if (EINA_UNLIKELY(klass_id == EFL_OBJECT_OVERRIDE_CLASS)) return EFL_TRUE;
    ...
    {
        if ((tdata->cache.isa_id == eo_id) && (tdata->cache.klass == klass_id))
        {
            isa = tdata->cache.isa;
            return isa;
        }
        ...
        if (EINA_UNLIKELY(vtable.size <= klass->class_id)) return EFL_FALSE;
...
efl_xunref(Eo *obj_id, const Eo *ref_obj_id)
{
    ...
    {
        if (xref->ref_obj == ref_obj_id) break;
    }
...
efl_ref(const Eo *obj_id)
{
    ...
    ++(obj->user_refcount);
    if (EINA_UNLIKELY(obj->user_refcount == 1)) _efl_ref(obj);
...
efl_unref(const Eo *obj_id)
{
    ...
    if (EINA_UNLIKELY(
            (!obj->unref_compensate && obj->user_refcount == 1 &&
             obj->parent) ||
                       ...
                       (obj->user_refcount == 2 && obj->parent))))
    {
        ...
        obj->unref_compensate = EFL_FALSE;
    }
    ...
    if (EINA_UNLIKELY((obj->user_refcount <= 0)))
    {
        if (obj->user_refcount < 0)
        {
            ERR("Obj:%s@%p. User refcount (%d) < 0. Too many unrefs.",
                ...
                obj_id,
                obj->user_refcount);
            _eo_log_obj_report((Eo_Id)obj_id,
                               EINA_LOG_LEVEL_ERR,
                               __func__,
                               __FILE__,
                               __LINE__);
...
static inline void *
_efl_data_xref_internal(const char       *file,
                        int               line,
                        _Eo_Object       *obj,
                        const _Efl_Class *klass,
                        const _Eo_Object *ref_obj)
{
    ...
    obj->data_xrefs =
        eina_inlist_prepend(obj->data_xrefs, EINA_INLIST_GET(xref));
#else
    (void)ref_obj;
    (void)file;
    (void)line;
...
static inline void
_efl_data_xunref_internal(_Eo_Object *obj           EFL_UNUSED,
                          void *data                EFL_UNUSED,
                          const _Eo_Object *ref_obj EFL_UNUSED)
{
#ifdef EO_DEBUG
    const _Efl_Class *klass    = obj->klass;
    Eo_Xref_Node     *xref     = NULL;
    Efl_Bool          in_range = (((char *)data >= (((char *)obj) + _eo_sz)) &&
    ...
    {
        if (xref->ref_obj == _eo_obj_id_get(ref_obj)) break;
    }
    if (xref)
    {
        obj->data_xrefs =
            eina_inlist_remove(obj->data_xrefs, EINA_INLIST_GET(xref));
        eina_freeq_ptr_main_add(xref, free, sizeof(*xref));
    }
    else
    {
        ERR("ref_obj %p (%s) does not reference data %p of obj %p (%s).",
            _eo_obj_id_get(ref_obj),
            ref_obj->klass->desc->name,
            data,
            _eo_obj_id_get(obj),
            obj->klass->desc->name);
    }
...
efl_data_scope_get(const Eo *obj_id, const Efl_Class *klass_id)
{
    ...
    else
    {
        ERR("Tried getting data of class '%s' from object of class '%s', but "
            "the former is not a direct inheritance of the latter.",
            klass->desc->name,
            obj->klass->desc->name);
    }
#endif
...
EO_API void *
efl_data_xref_internal(const char      *file,
                       int              line,
                       const Eo        *obj_id,
                       const Efl_Class *klass_id,
                       const Eo        *ref_obj_id)
...
err_ret:
    ...
    goto err;
err_mro:
    ERR("Tried getting data of class '%s' from object of class '%s', but the "
        "former is not a direct inheritance of the latter.",
        klass->desc->name,
        obj->klass->desc->name);
err:
    EO_OBJ_DONE(obj_id);
...
efl_object_init(void)
{
    ...
    if (_efl_object_init_count++ > 0) return EFL_TRUE;
    ...
    eina_magic_string_static_set(EO_FREED_EINA_MAGIC, EO_FREED_EINA_MAGIC_STR);
    eina_magic_string_static_set(EO_CLASS_EINA_MAGIC, EO_CLASS_EINA_MAGIC_STR);
#ifndef _WIN32
    ...
    efl_object_optional_cow = eina_cow_add("Efl Object Optional Data",
                                           sizeof(Efl_Object_Optional),
                                           64,
                                           &efl_object_optional_cow_default,
                                           EFL_TRUE);

    _efl_add_fallback_init();

    eina_log_timing(_eo_log_dom, EINA_LOG_STATE_STOP, EINA_LOG_STATE_INIT);
...
efl_object_shutdown(void)
{
    ...
    eina_log_timing(_eo_log_dom, EINA_LOG_STATE_START, EINA_LOG_STATE_SHUTDOWN);

    _efl_add_fallback_shutdown();

    for (i = 0; i < _eo_classes_last_id; i++, cls_itr--)
    {
        if (*cls_itr) eo_class_free(*cls_itr);
    }
...
efl_domain_switch(Efl_Id_Domain domain)
{
    ...
    if ((data) && (data->local_domain == domain)) return EFL_TRUE;
...
efl_compatible(const Eo *obj, const Eo *obj_target)
{
    ...
    if (domain1 == domain2) return EFL_TRUE;
...
{
    EINA_SAFETY_ON_NULL_RETURN_VAL(klass, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(override, EFL_FALSE);
    EINA_SAFETY_ON_TRUE_RETURN_VAL(!efl_isa(override, klass), EFL_FALSE);
    if (!class_overrides) class_overrides = eina_hash_pointer_new(NULL);
    EINA_SAFETY_ON_NULL_RETURN_VAL(class_overrides, EFL_FALSE);
...
efl_class_override_unregister(const Efl_Class *klass, const Efl_Class *override)
{
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(klass, EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(override, EFL_FALSE);
    if (!class_overrides) return EFL_TRUE;

    set = eina_hash_find(class_overrides, &klass);
    if (set != override) return EFL_FALSE;
...
EO_API void
efl_manual_free_set(Eo *obj_id, Efl_Bool manual_free)
{
    EO_OBJ_POINTER_RETURN(obj_id, obj);
...
efl_manual_free(Eo *obj_id)
{
    ...
    // rare to use goto to keep instruction cache cleaner
    if (obj->manual_free == EFL_FALSE) goto err_manual_free;
    // rare to use goto to keep instruction cache cleaner
    if (!obj->destructed) goto err_not_destructed;
    _eo_free(obj, EFL_TRUE);
    EO_OBJ_DONE(obj_id);
    return EFL_TRUE;

err_manual_free:
    ERR("Tried to manually free the object %p while the option has not been "
        "set; see efl_manual_free_set for more information.",
        obj);
    goto err;
err_not_destructed:
...
efl_debug_name_get(const Eo *obj_id)
{
    ...
    if (_eo_is_a_class(obj_id))
    {
        ...
        return eina_slstr_printf("%s(%s)@%p",
                                 klass->desc->name,
                                 clstype,
                                 obj_id);
    }
    ...
    else
    {
        if (obj->super)
            efl_debug_name_override(
                efl_super(obj_id, (Efl_Class *)obj->cur_klass->header.id),
                sb);
        else
            efl_debug_name_override(
                efl_cast(obj_id, (Efl_Class *)obj->cur_klass->header.id),
                sb);
        obj->super     = EFL_FALSE;
        obj->cur_klass = NULL;
    }
...
EO_API int
efl_callbacks_cmp(const Efl_Callback_Array_Item *a,
...
#  elif defined(__APPLE__) && defined(__MACH__)
    ...
    if (EINA_UNLIKELY(clk_conv < 0))
    {
        ...
        if (err == 0) clk_conv = 1e-9 * (double)info.numer / (double)info.denom;
        else clk_conv = 1e-9;
    }
...
static void
_eo_log_obj_find(const Eo_Id              id,
                 const Eo_Log_Obj_Entry **added,
                 int                     *added_idx,
                 const Eo_Log_Obj_Entry **deleted)
{
    ...
    *deleted   = NULL;
    ...
    EINA_ARRAY_ITER_NEXT(&_eo_log_objs, idx, entry, it)
    {
        ...
        {
            if (entry->ref_op == EO_REF_OP_FREE) *deleted = entry;
...
static void
_eo_log_obj_entry_show(const Eo_Log_Obj_Entry *entry,
                       int                     log_level,
                       const char             *func_name,
                       const char             *file,
                       int                     line,
                       double                  now)
{
    ...
    eina_log_print(
        ...
        entry->timestamp - _eo_log_time_start,
        now - entry->timestamp,
        entry->bt_hits);
        ...
        if (dladdr(entry->bt[i], &info))
        {
            if (info.dli_sname)
            {
                eina_log_print(
                    ...
                    info.dli_sname,
                    (unsigned long long)(uintptr_t)((char *)entry->bt[i] -
                                                    (char *)info.dli_saddr),
...
#    endif
        ...
        eina_log_print(_eo_log_objs_dom,
                       log_level,
                       func_name,
                       file,
                       line,
...
static Efl_Bool
_eo_log_obj_entry_is_new_backtrace(const Eina_List  *entries,
...
inline void
_eo_log_obj_report(const Eo_Id id,
                   int         log_level,
                   const char *func_name,
                   const char *file,
                   int         line)
    ...
    if ((!added) && (!deleted))
    {
        if ((!_eo_log_objs_debug.len) && (!_eo_log_objs_no_debug.len))
        {
            eina_log_print(_eo_log_objs_dom,
                           ...
                           func_name,
                           line,
                           "obj_id=%p was neither created or deleted.",
                           (void *)id);
        }
        else if ((_eo_log_objs_debug.len) && (_eo_log_objs_no_debug.len))
        {
            eina_log_print(
                ...
                line,
                "obj_id=%p was neither created or deleted "
                "(EO_LIFECYCLE_DEBUG='%s', EO_LIFECYCLE_NO_DEBUG='%s').",
                (void *)id,
                getenv("EO_LIFECYCLE_DEBUG"),
                getenv("EO_LIFECYCLE_NO_DEBUG"));
        }
...
#    ifdef HAVE_BACKTRACE
    if ((_eo_log_objs_backtrace >= 0) && deleted)
    {
        ...
        current->bt_hash   = 0;
        current->bt_hits   = 1;
        if (bt && size) memcpy(current->bt, bt, size * sizeof(void *));
    }
#    endif
...
_eo_log_obj_entry_ref_op(const _Eo_Object *obj,
                         Eo_Ref_Op         refop,
                         unsigned          size,
                         void *const      *bt)
{
    ...
    entry->bt_hash   = 0;
    entry->bt_hits   = 1;
    if (size && bt) memcpy(entry->bt, bt, size * sizeof(void *));
...
_eo_log_obj_desired(const _Eo_Object *obj)
{
    ...
    {
        if (eina_inarray_search_sorted(&_eo_log_objs_no_debug,
                                       &cls_name,
                                       _eo_class_name_slice_cmp) >= 0)
            return EFL_FALSE;
    }

    if (!_eo_log_objs_debug.len) return EFL_TRUE;

    if (eina_inarray_search_sorted(&_eo_log_objs_debug,
                                   &cls_name,
                                   _eo_class_name_slice_cmp) >= 0)
        return EFL_TRUE;
...
_eo_log_obj_init(void)
{
    ...
    _eo_log_objs_dom =
        eina_log_domain_register("eo_lifecycle", EINA_COLOR_BLUE);
...
#  endif
    eina_inarray_step_set(&_eo_log_objs_debug,
                          sizeof(Eina_Inarray),
                          sizeof(Eina_Slice),
                          0);
    eina_inarray_step_set(&_eo_log_objs_no_debug,
                          sizeof(Eina_Inarray),
                          sizeof(Eina_Slice),
                          0);
    ...
    if (s && *s)
    {
        ...
        else if (_eo_log_objs_backtrace < 0) _eo_log_objs_backtrace = 0;
    }
    else _eo_log_objs_backtrace = EO_LOG_OBJS_BACKTRACE_DEFAULT;
...
#  endif
    ...
    DBG("logging object allocation and free, use "
        "EINA_LOG_LEVELS=eo_lifecycle:4");
    ...
    if ((s) && (s[0] != '\0'))
    {
        if ((strcmp(s, "*") == 0) || (strcmp(s, "1") == 0))
        {
            EINA_LOG_DOM_ERR(
                _eo_log_objs_dom,
                "expected class names to not log allocation and free, got '%s'",
                s);
        }
        else
        {
...
#  ifdef HAVE_BACKTRACE
    ...
    if (eina_log_domain_level_check(_eo_log_objs_dom, EINA_LOG_LEVEL_INFO))
    {
        ...
        for (; itr < itr_end; itr++)
        {
            ...
            for (cur = itr + 1; cur < itr_end; cur++)
            {
                ...
                if (EINA_UNLIKELY((cur_entry->id == entry->id) &&
                                  (cur_entry->ref_op == EO_REF_OP_FREE)))
                    break;
            }
        ...
        if (leaks)
            EINA_LOG_DOM_WARN(_eo_log_objs_dom,
                              "Leaked %zd objects! Check details with "
                              "EINA_LOG_LEVELS=eo_lifecycle:4",
                              leaks);
        else EINA_LOG_DOM_INFO(_eo_log_objs_dom, "No leaked objects!");
    }
    EINA_ARRAY_ITER_NEXT(&_eo_log_objs, idx, entry, it)
...
_eo_classes_iterator_next(Eina_Iterator *it, void **data)
{
    ...
    if (eo_it->cur_kl_id == _eo_classes_last_id) return EFL_FALSE;
...
_eo_objects_iterator_next(Eina_Iterator *it, void **data)
{
    ...
    if (!eo_it->tdata) return EFL_FALSE;
        ...
        if (tdata->eo_ids_tables[mid_table_id])
        {
            while (table_id < MAX_TABLE_ID)
            {
                if (TABLE_FROM_IDS)
                {
                    ...
                    {
                        _Eo_Id_Entry *entry =
                            &(TABLE_FROM_IDS->entries[entry_id]);
                        if (entry->active)
                        {
                            Eo *obj =
                                _eo_header_id_get((Eo_Header *)entry->ptr);
                            *data               = obj;
...
eo_objects_iterator_new(void)
{
    ...
    Eo_Id_Table_Data     *tdata =
        _eo_table_data_table_get(_eo_table_data_get(), EFL_ID_DOMAIN_MAIN);
...
static Efl_Bool
_eo_value_setup(const Eina_Value_Type *type EFL_UNUSED, void *mem)
{
    Eo **tmem = mem;
    *tmem     = NULL;
    return EFL_TRUE;
...
static Efl_Bool
_eo_value_flush(const Eina_Value_Type *type EFL_UNUSED, void *mem)
{
    Eo **tmem = mem;
    if (*tmem)
    {
...
static Efl_Bool
_eo_value_vset(const Eina_Value_Type *type EFL_UNUSED, void *mem, va_list args)
{
    Eo **dst = mem;
...
static Efl_Bool
_eo_value_pset(const Eina_Value_Type *type EFL_UNUSED,
               void                       *mem,
               const void                 *ptr)
{
    Eo       **dst = mem;
...
static Efl_Bool
_eo_value_pget(const Eina_Value_Type *type EFL_UNUSED,
               const void                 *mem,
               void                       *ptr)
{
    Eo *const *src = mem;
...
static Efl_Bool
_eo_value_convert_to(const Eina_Value_Type *type EFL_UNUSED,
                     ...
                     void                       *convert_mem)
{
        ...
        convert == EINA_VALUE_TYPE_STRING)
    {
        ...
        snprintf(buf,
                 sizeof(buf),
                 "Object id: %p, class: %s, name: %s",
                 *eo,
                 efl_class_name_get(efl_class_get(*eo)),
...
static Efl_Bool
_eo_value_copy(const Eina_Value_Type *type EFL_UNUSED,
               const void                 *mem,
               void                       *ptr)
{
    Eo *const *src = mem;
...
EO_API Eina_Error
efl_property_reflection_set(Eo         *obj_id,
...
efl_property_reflection_get(const Eo *obj_id, const char *property_name)
{
    ...
    if (reflection && reflection->get) r = reflection->get(obj_id);
...
efl_ownable_get(const Eo *obj)
{
    ...
    if (efl_parent_get(obj)) ref--;

    if (ref <= 0)
        ERR("There is no free reference to pass this object. Please check that "
            "this object is really owned by you.");
    return (ref > 0);
}
