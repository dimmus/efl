                 ...
                 Efl_Bool          allow_same_override)
{
  ...
  if (hierarchy_klass) hirachy_node = &hierarchy_klass->vtable.chain[class_id];
...
on_null:
  ...
  {
    _efl_del_api_op_id =
      _efl_object_api_op_id_get_internal(EFL_FUNC_COMMON_OP_FUNC(efl_del));
    efl_del_api_generation = _efl_object_init_generation;
  }
  if (op != _efl_del_api_op_id) WRN("NULL passed to function %s().", func_name);
  return EFL_FALSE;
}
...
eo_class_free(_Efl_Class *klass)
{
  ...
  if ((self != _efl_object_main_thread) && (self != klass->construction_thread))
...
efl_unref(const Eo *obj_id)
{
  ...
  if (EINA_UNLIKELY(
        (!obj->unref_compensate && obj->user_refcount == 1 && obj->parent) ||
...
_efl_data_scope_get(const _Eo_Object *obj, const _Efl_Class *klass)
{
  ...
  else
  {
    ...
    {
      if (doff_itr->klass == klass) return ((char *)obj) + doff_itr->offset;
      doff_itr++;
    }
  }
  ...
  if ((s) && (s[0] != '\0'))
  {
    ...
    if ((es != s) && (*es == ':'))
    {
      ...
      else if (lvl == 2)
      {
        ...
        EINA_LOG_DOM_DBG(_eo_log_objs_dom, "will log new, free, ref and unref");
      }
      s = es + 1;
    }
