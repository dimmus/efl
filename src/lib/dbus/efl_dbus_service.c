...
static inline void
_introspect_arguments_append(Efl_Strbuf *buf, const Efl_Dbus_Arg_Info *args,
...
static inline void
_instrospect_append_property(Efl_Strbuf *buf, const Efl_Dbus_Property *prop, const Efl_Dbus_Service_Interface *iface)
{
...
_cb_property_get(const Efl_Dbus_Service_Interface *piface, const Efl_Dbus_Message *msg)
{
   ...
   if (!iface)
     return efl_dbus_message_error_new(msg, DBUS_ERROR_UNKNOWN_INTERFACE,
                                    "Interface not found.");
   ...
   variant = efl_dbus_message_iter_container_new(main_iter, 'v',
                                              prop->property->type);
...
not_found:
   return efl_dbus_message_error_new(msg, DBUS_ERROR_UNKNOWN_PROPERTY,
                                  "Property not found.");
}

static Efl_Bool
_props_getall(Efl_Dbus_Service_Interface *iface, Eina_Iterator *iterator, Efl_Dbus_Message_Iter *dict, const Efl_Dbus_Message *input_msg, Efl_Dbus_Message **error_reply)
{
   Property *prop;
   EINA_ITERATOR_FOREACH(iterator, prop)
     {
        ...
        var = efl_dbus_message_iter_container_new(entry, 'v',
                                               prop->property->type);
...
static Efl_Dbus_Message *
_cb_property_getall(const Efl_Dbus_Service_Interface *piface, const Efl_Dbus_Message *msg)
{
   ...
   if (!efl_dbus_message_arguments_get(msg, "s", &iface_name))
     return NULL;
...
static Efl_Dbus_Message *
_cb_property_set(const Efl_Dbus_Service_Interface *piface, const Efl_Dbus_Message *msg)
{
   const char *propname, *iface_name;
   Efl_Dbus_Service_Object *obj = piface->obj;
   Efl_Dbus_Service_Interface *iface;
   ...
   if (!efl_dbus_message_arguments_get(msg, "ssv", &iface_name, &propname, &variant))
     return NULL;
   ...
   if (!setter)
     return efl_dbus_message_error_new(msg, DBUS_ERROR_PROPERTY_READ_ONLY,
...
static Efl_Dbus_Message *
cb_introspect(const Efl_Dbus_Service_Interface *_iface, const Efl_Dbus_Message *message)
{
   ...
   efl_dbus_message_arguments_append(reply, "s", eina_strbuf_string_get(obj->introspection_data));
   return reply;
fail:
...
static const Efl_Dbus_Method introspect = {
   "Introspect", NULL, EFL_DBUS_ARGS({ "s", "xml" }), cb_introspect, 0
};
...
static const Efl_Dbus_Method _property_methods[] = {
   {
    "Get", EFL_DBUS_ARGS({"s", "interface"}, {"s", "property"}),
    EFL_DBUS_ARGS({"v", "value"}), _cb_property_get, 0
   },
   {
    "Set", EFL_DBUS_ARGS({"s", "interface"}, {"s", "property"}, {"v", "value"}),
    NULL, _cb_property_set, 0
   },
   {
    "GetAll", EFL_DBUS_ARGS({"s", "interface"}), EFL_DBUS_ARGS({"a{sv}", "props"}),
...
static const Efl_Dbus_Signal _properties_signals[] = {
   {
    "PropertiesChanged",
    EFL_DBUS_ARGS({"s", "interface"}, {"a{sv}", "changed_properties"}, {"as", "invalidated_properties"}), 0
   }
};
...
static Efl_Bool
_propmgr_iface_props_append(Efl_Dbus_Service_Interface *iface, Efl_Dbus_Message_Iter *array)
{
   ...
   efl_dbus_message_iter_arguments_append(iface_entry, "sa{sv}", iface->name, &props_array);
   iterator = eina_hash_iterator_data_new(iface->properties);
...
static Efl_Bool
_managed_obj_append(Efl_Dbus_Service_Object *obj, Efl_Dbus_Message_Iter *array, Efl_Bool first)
{
   ...
   efl_dbus_message_iter_arguments_append(obj_entry, "oa{sa{sv}}", obj->path,
                                       &array_interface);
...
static Efl_Dbus_Message *
_cb_managed_objects(const Efl_Dbus_Service_Interface *iface, const Efl_Dbus_Message *msg)
{
   ...
   EINA_SAFETY_ON_NULL_RETURN_VAL(reply, NULL);
   main_iter =  efl_dbus_message_iter_get(reply);
   efl_dbus_message_iter_arguments_append(main_iter, "a{oa{sa{sv}}}", &array_path);

   ret = _managed_obj_append(iface->obj, array_path, EFL_TRUE);
   if (!ret)
     {
        efl_dbus_message_unref(reply);
        return efl_dbus_message_error_new(msg, "org.freedesktop.DBus.Error",
...
static Efl_Dbus_Method get_managed_objects = {
   "GetManagedObjects", NULL, EFL_DBUS_ARGS({"a{oa{sa{sv}}}", "objects"}),
   _cb_managed_objects, 0
};

static const Efl_Dbus_Signal _object_manager_signals[] = {
   {
    "InterfacesAdded", EFL_DBUS_ARGS({"o", "object"}, {"a{sa{sv}}", "interfaces"}), 0
   },
   {
    "InterfacesRemoved", EFL_DBUS_ARGS({"o", "object"}, {"as", "interfaces"}), 0
   }
};
...
static Efl_Dbus_Service_Object *
_efl_dbus_service_object_add(Efl_Dbus_Connection *conn, const char *path, Efl_Bool fallback)
{
                                 ...
                                 Efl_Dbus_Service_Object *parent)
{
   ...
   Efl_Dbus_Message *sig = efl_dbus_message_signal_new(parent->path,
                                                 EFL_DBUS_FDO_INTERFACE_OBJECT_MANAGER,
                                                 "InterfacesAdded");
   EINA_SAFETY_ON_NULL_RETURN(sig);
   iter = efl_dbus_message_iter_get(sig);
   efl_dbus_message_iter_arguments_append(iter, "oa{sa{sv}}", obj->path,
                                       &array);
...
_efl_dbus_service_interface_add(Efl_Dbus_Service_Object *obj, const char *interface)
{
   Efl_Dbus_Service_Interface *iface;
   ...
   if (!obj->idle_enterer_iface_changed)
     obj->idle_enterer_iface_changed = core_idle_enterer_add(_object_manager_changes_process,
                                                obj);
                            ...
                            const Efl_Dbus_Property *property)
{
   ...
   EINA_SAFETY_ON_TRUE_RETURN_VAL(!!eina_hash_find(interface->properties,
                                  property->name), EFL_FALSE);
   EINA_SAFETY_ON_NULL_RETURN_VAL(property->type, EFL_FALSE);
   EINA_SAFETY_ON_FALSE_RETURN_VAL(
      dbus_signature_validate_single(property->type, NULL), EFL_FALSE);
...
static Efl_Dbus_Service_Interface *
_efl_dbus_service_interface_register(Efl_Dbus_Connection *conn, const char *path, const Efl_Dbus_Service_Interface_Desc *desc, Efl_Bool fallback, unsigned int version)
{
   ...
   if (!obj)
     obj = _efl_dbus_service_object_add(conn, path, fallback);
   else
...
efl_dbus_service_interface_register(Efl_Dbus_Connection *conn, const char *path, const Efl_Dbus_Service_Interface_Desc *desc)
{
   return _efl_dbus_service_interface_register(conn, path, desc, EFL_FALSE, 1u);
}

EAPI Efl_Dbus_Service_Interface *
efl_dbus_service_interface_register2(Efl_Dbus_Connection *conn, const char *path, const Efl_Dbus_Service_Interface_Desc2 *desc)
{
   return _efl_dbus_service_interface_register(conn, path, &desc->description, EFL_FALSE, desc->version);
}

EAPI Efl_Dbus_Service_Interface *
efl_dbus_service_interface_fallback_register(Efl_Dbus_Connection *conn, const char *path, const Efl_Dbus_Service_Interface_Desc *desc)
{
   return _efl_dbus_service_interface_register(conn, path, desc, EFL_TRUE, 1u);
}

EAPI Efl_Dbus_Service_Interface *
efl_dbus_service_interface_fallback_register2(Efl_Dbus_Connection *conn, const char *path, const Efl_Dbus_Service_Interface_Desc2 *desc)
{
   return _efl_dbus_service_interface_register(conn, path, &desc->description, EFL_TRUE, desc->version);
}
...
_idle_enterer_propschanged(void *data)
{
   ...
   msg = efl_dbus_message_signal_new(iface->obj->path, properties_iface->name,
                                  properties_iface->signals[0].name);
   EINA_SAFETY_ON_NULL_GOTO(msg, error);

   main_iter = efl_dbus_message_iter_get(msg);
   if (!efl_dbus_message_iter_arguments_append(main_iter, "sa{sv}", iface->name, &dict))
     {
   ...
   while ((prop = eina_array_pop(iface->prop_invalidated)))
     {
        if (!prop->is_invalidate)
          continue;
        efl_dbus_message_iter_basic_append(array_invalidate, 's',
                                        prop->property->name);
     }
end:
...
_interface_free(Efl_Dbus_Service_Interface *interface)
{
   ...
   if (l)
   ...
   else
     {
        if (!obj->idle_enterer_iface_changed)
          {
             obj->idle_enterer_iface_changed = core_idle_enterer_add(
...
static void _children_ifaces_add_removed_flush(Efl_Dbus_Service_Object *obj)
{
   Efl_Dbus_Service_Object *children;
...
efl_dbus_service_object_unregister(Efl_Dbus_Service_Interface *iface)
{
    ...
    * by libdbus.
    */
   efl_dbus_connection_free_cb_del(iface->obj->conn, _on_connection_free, iface->obj);
...
static Efl_Dbus_Message*
_efl_dbus_method_call(Efl_Dbus_Method const* method, Efl_Dbus_Service_Interface* iface, Efl_Dbus_Message* msg)
{
  if(method->flags & EFL_DBUS_METHOD_FLAG_HAS_DATA)
    {
      Efl_Dbus_Method_Data_Cb cb = (void *)method->cb;
      return cb(((Efl_Dbus_Method2 const*)method)->data, iface, msg);
    }
  else
    {
...
_object_handler(DBusConnection *dbus_conn EFL_UNUSED, DBusMessage *msg, void *user_data)
{
   Efl_Dbus_Service_Object *obj;
   Efl_Dbus_Service_Interface *iface;
   const Efl_Dbus_Method *method;
   ...
   dbus_message_iter_init(efl_dbus_msg->dbus_msg, &efl_dbus_msg->iterator->dbus_iterator);

   efl_dbus_init();
   efl_dbus_connection_ref(conn);

   if (!_have_signature(method->in, efl_dbus_msg))
     reply = efl_dbus_message_error_new(efl_dbus_msg, DBUS_ERROR_INVALID_SIGNATURE,
   ...
   if (reply)
     _efl_dbus_connection_send(conn, reply, NULL, NULL, -1);
...
efl_dbus_service_signal_new(const Efl_Dbus_Service_Interface *iface, unsigned int signal_id)
{
   unsigned size;
   EFL_DBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, NULL);
   ...
   return efl_dbus_message_signal_new(iface->obj->path, iface->name,
                                   iface->signals[signal_id].name);
}

EAPI Efl_Bool
efl_dbus_service_signal_emit(const Efl_Dbus_Service_Interface *iface, unsigned int signal_id, ...)
{
...
EAPI Efl_Bool
efl_dbus_service_signal_send(const Efl_Dbus_Service_Interface *iface, Efl_Dbus_Message *signal_msg)
{
   EFL_DBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, EFL_FALSE);
...
EAPI void
efl_dbus_service_object_data_set(Efl_Dbus_Service_Interface *iface, const char *key, const void *data)
{
   EFL_DBUS_SERVICE_INTERFACE_CHECK(iface);
...
EAPI void *
efl_dbus_service_object_data_get(const Efl_Dbus_Service_Interface *iface, const char *key)
{
   EFL_DBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(key, NULL);
   return efl_dbus_data_get(&(((Efl_Dbus_Service_Object *)iface->obj)->data), key);
}

EAPI void *
efl_dbus_service_object_data_del(Efl_Dbus_Service_Interface *iface, const char *key)
{
   EFL_DBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(key, NULL);
   return efl_dbus_data_del(&(((Efl_Dbus_Service_Object *)iface->obj)->data), key);
}

EAPI Efl_Bool
efl_dbus_service_property_changed(const Efl_Dbus_Service_Interface *interface, const char *name)
{
   Property *prop;
   ...
   if (!iface->idle_enterer_propschanged)
     iface->idle_enterer_propschanged = core_idle_enterer_add(_idle_enterer_propschanged, iface);
   if (!iface->props_changed)
...
EAPI Efl_Bool
efl_dbus_service_property_invalidate_set(const Efl_Dbus_Service_Interface *interface, const char *name, Efl_Bool is_invalidate)
{
   Property *prop;
   Efl_Dbus_Service_Interface *iface = (Efl_Dbus_Service_Interface *) interface;
