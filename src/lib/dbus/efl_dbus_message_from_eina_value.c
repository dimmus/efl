...
static Efl_Bool
_array_append(const char *type, const Eina_Value *value_array, Efl_Dbus_Message_Iter *iter)
{
           ...
           for (i = 0; i < eina_value_array_count(value_array); i++)
             {
                ...
                efl_dbus_message_iter_arguments_append(array, type+1, &entry);
           ...
           for (i = 0; i < eina_value_array_count(value_array); i++)
             {
                ...
                efl_dbus_message_iter_arguments_append(array, type+1, &sub_array);
      ...
      case 'b'://boolean
        {
           ...
           for (i = 0; i < eina_value_array_count(value_array); i++)
             {
                eina_value_array_get(value_array, i, &z);
                efl_dbus_message_iter_basic_append(array, type[1], (uint32_t)z);
             }
           break;
        }
      case 'n'://int16
...
static Efl_Bool
_basic_append_value_struct(char type, const Eina_Value *value, const Eina_Value_Struct_Desc *desc, unsigned idx, Efl_Dbus_Message_Iter *iter)
{
   EINA_SAFETY_ON_FALSE_RETURN_VAL(
            _compatible_type(type, desc->members[idx].type), EFL_FALSE);
   switch (type)
     {
      case 'i'://int
      case 'h'://fd
      ...
      case 'b'://boolean
        {
           ...
           efl_dbus_message_iter_basic_append(iter, type, (uint32_t)boolean);
           break;
        }
      case 'n'://int16
        {
...
Efl_Bool
_message_iter_from_eina_value_struct(const char *signature, Efl_Dbus_Message_Iter *iter, const Eina_Value *value)
{
   unsigned i;
   ...
   EINA_SAFETY_ON_FALSE_RETURN_VAL(
            eina_value_type_get(value) == EINA_VALUE_TYPE_STRUCT, EFL_FALSE);
   EINA_SAFETY_ON_FALSE_RETURN_VAL(
      eina_value_pget(value, &st), EFL_FALSE);
...
EAPI Efl_Bool
efl_dbus_message_from_eina_value(const char *signature, Efl_Dbus_Message *msg, const Eina_Value *value)
{
...
static Efl_Bool
_basic_append_value(char type, const Eina_Value *value, Efl_Dbus_Message_Iter *iter)
{
   EINA_SAFETY_ON_NULL_RETURN_VAL(value, EFL_FALSE);
   const Eina_Value_Type *value_type = eina_value_type_get(value);
   EINA_SAFETY_ON_FALSE_RETURN_VAL(_compatible_type(type, value_type), EFL_FALSE);
   switch (type)
     {
      case 'i'://int
      case 'h'://fd
...
Efl_Bool
_message_iter_from_eina_value(const char *signature, Efl_Dbus_Message_Iter *iter, const Eina_Value *value)
{
   EINA_SAFETY_ON_NULL_RETURN_VAL(value, EFL_FALSE);
