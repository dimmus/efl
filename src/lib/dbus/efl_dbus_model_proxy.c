...
static void _efl_dbus_model_proxy_property_get_all_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);
static void _efl_dbus_model_proxy_property_set_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);
static void _efl_dbus_model_proxy_property_set_load_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);
static void _efl_dbus_model_proxy_start_monitor(Efl_Dbus_Model_Proxy_Data *);
static void _efl_dbus_model_proxy_property_changed_cb(void *, Efl_Dbus_Proxy *, void *);
static void _efl_dbus_model_proxy_property_invalidated_cb(void *, Efl_Dbus_Proxy *, void *);
static const char *_efl_dbus_model_proxy_property_type_get(Efl_Dbus_Model_Proxy_Data *, const char *);
static void _efl_dbus_model_proxy_create_methods_children(Efl_Dbus_Model_Proxy_Data *);
static void _efl_dbus_model_proxy_create_signals_children(Efl_Dbus_Model_Proxy_Data *);


typedef struct _Efl_Dbus_Model_Proxy_Property_Set_Data Efl_Dbus_Model_Proxy_Property_Set_Data;
...
static void
_efl_dbus_model_proxy_object_del(void *data, Efl_Dbus_Object *object EFL_UNUSED, void *event_info EFL_UNUSED)
{
...
static Efl_Object*
_efl_dbus_model_proxy_efl_object_constructor(Eo *obj, Efl_Dbus_Model_Proxy_Data *pd)
{
...
static Efl_Object*
_efl_dbus_model_proxy_efl_object_finalize(Eo *obj, Efl_Dbus_Model_Proxy_Data *pd)
{
   if (!pd->object ||
       !pd->name ||
       !pd->interface)
   ...
   if (!efl_dbus_model_connection_get(obj))
     efl_dbus_model_connection_set(obj, efl_dbus_object_connection_get(pd->object));

   efl_dbus_object_event_callback_add(pd->object, EFL_DBUS_OBJECT_EVENT_DEL, _efl_dbus_model_proxy_object_del, pd);
...
static void
_efl_dbus_model_proxy_interface_set(Eo *obj EFL_UNUSED,
                                  Efl_Dbus_Model_Proxy_Data *pd,
...
static void
_efl_dbus_model_proxy_efl_object_invalidate(Eo *obj, Efl_Dbus_Model_Proxy_Data *pd)
{
   Eo *child;
   ...
   if (pd->object)
     {
        efl_dbus_object_event_callback_del(pd->object, EFL_DBUS_OBJECT_EVENT_DEL, _efl_dbus_model_proxy_object_del, pd);
...
static void
_efl_dbus_model_proxy_efl_object_destructor(Eo *obj, Efl_Dbus_Model_Proxy_Data *pd)
{
                                           ...
                                           Eina_Value *value)
{
   ...
   if (!pd->is_loaded)
     {
        pending = _efl_dbus_model_proxy_load_all(pd, data->promise, property,
                                               _efl_dbus_model_proxy_property_set_load_cb, data);
     }
   else
     {
        pending = efl_dbus_proxy_property_value_set(pd->proxy, property, signature, (Eina_Value*)value,
                                                  _efl_dbus_model_proxy_property_set_cb, data);
     }

   if (pending) pd->pendings = eina_list_append(pd->pendings, pending);
   return efl_future_then(obj, eina_future_new(data->promise),
                          .data = data, .free = _efl_dbus_model_proxy_cancel_cb);
                                           ...
                                           const char *property)
{
   ...
   pending = _efl_dbus_model_proxy_load_all(pd, NULL, property,
                                          _efl_dbus_model_proxy_property_get_all_cb, pd);
   if (pending) pd->pendings = eina_list_append(pd->pendings, pending);
   else goto on_error;
...
static Eina_Future*
_efl_dbus_model_proxy_efl_model_children_slice_get(Eo *obj EFL_UNUSED,
                                                 Efl_Dbus_Model_Proxy_Data *pd,
                                                 unsigned start,
                                                 unsigned count)
{
...
static unsigned int
_efl_dbus_model_proxy_efl_model_children_count_get(const Eo *obj EFL_UNUSED,
                                                 Efl_Dbus_Model_Proxy_Data *pd)
{
   _efl_dbus_model_proxy_listed(pd);
   ...
   EINA_LIST_FOREACH(pd->interface->methods, it, method)
     {
        ...
        child = efl_add_ref(EFL_DBUS_MODEL_METHOD_CLASS, pd->obj,
   ...
   EINA_LIST_FOREACH(pd->interface->signals, it, signal)
     {
        ...
        child = efl_add_ref(EFL_DBUS_MODEL_SIGNAL_CLASS, pd->obj, efl_dbus_model_signal_constructor(efl_added, pd->proxy, signal));
...
static const char *
_efl_dbus_model_proxy_proxy_name_get(const Eo *obj EFL_UNUSED, Efl_Dbus_Model_Proxy_Data *pd)
{
   return pd->name;
}
                                        ...
                                        void *event_info)
{
   ...
   Efl_Dbus_Object_Event_Property_Changed *event = (Efl_Dbus_Object_Event_Property_Changed*)event_info;
...
{
   Efl_Dbus_Model_Proxy_Data *pd = (Efl_Dbus_Model_Proxy_Data*)data;
   Efl_Dbus_Proxy_Event_Property_Changed *event = (Efl_Dbus_Proxy_Event_Property_Changed*)event_info;

   efl_model_property_invalidated_notify(pd->obj, event->name);
}

static Eina_Array *
_efl_dbus_model_proxy_property_get_all_load(const Efl_Dbus_Message *msg, Efl_Dbus_Model_Proxy_Data *pd)
{
...
_efl_dbus_model_proxy_property_set_load_cb(void *data,
                                         ...
                                         Efl_Dbus_Pending *pending)
{
   Efl_Dbus_Model_Proxy_Property_Set_Data *set_data = (Efl_Dbus_Model_Proxy_Property_Set_Data *)data;
   ...
   pending = efl_dbus_proxy_property_value_set(pd->proxy, set_data->property,
                                             signature, set_data->value,
                                             _efl_dbus_model_proxy_property_set_cb, set_data);
...
_efl_dbus_model_proxy_property_set_cb(void *data,
                                    ...
                                    Efl_Dbus_Pending *pending)
{
   Efl_Dbus_Model_Proxy_Property_Set_Data *sd = (Efl_Dbus_Model_Proxy_Property_Set_Data *)data;
