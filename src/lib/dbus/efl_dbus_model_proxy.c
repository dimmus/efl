                                    ...
                                    const char                *property)
{
    Efl_Dbus_Introspection_Property *property_introspection =
        efl_dbus_introspection_property_find(pd->interface->properties,
                                             property);
    unsigned char r = 0;
                                             ...
                                             Eina_Value *value)
{
    ...
    if (!pd->is_loaded)
    {
        pending = _efl_dbus_model_proxy_load_all(
            pd,
            data->promise,
            property,
    ...
    {
        pending = efl_dbus_proxy_property_value_set(
            pd->proxy,
            property,
            signature,
            (Eina_Value *)value,
...
_efl_dbus_model_proxy_create_methods_children(Efl_Dbus_Model_Proxy_Data *pd)
{
    ...
    EINA_LIST_FOREACH(pd->interface->methods, it, method)
    {
        ...
        child =
            efl_add_ref(EFL_DBUS_MODEL_METHOD_CLASS,
                        pd->obj,
                                            ...
                                            Efl_Dbus_Model_Proxy_Data *pd)
{
    ...
    while (efl_dbus_message_iter_get_and_next(values, 'e', &entry))
    {
        ...
        if (!efl_dbus_message_iter_arguments_get(entry,
                                                 "sv",
                                                 &property,
                                                 &variant))
            continue;
