...
static const Efl_Dbus_Version _version         = { EFL_VERSION_MAJOR,
                                                   EFL_VERSION_MINOR,
                                                   EFL_VERSION_MICRO,
                                                   0 };
...
static void
            _efl_dbus_connection_event_callback_call(Efl_Dbus_Connection           *conn,
                                                     Efl_Dbus_Connection_Event_Type type,
                                                     const void *event_info);
static void _efl_dbus_connection_context_event_cb_del(
  Efl_Dbus_Connection_Context_Event    *ce,
  Efl_Dbus_Connection_Context_Event_Cb *ctx);
static void efl_dbus_dispatch_name_owner_change(Efl_Dbus_Connection_Name *cn,
                                                const char *old_id);
...
efl_dbus_init(void)
{
  if (_efl_dbus_init_count++ > 0) return _efl_dbus_init_count;
  ...
  efl_dbus_model_log_dom =
    eina_log_domain_register("efl_dbus_model", EINA_COLOR_CYAN);
  ...
  eina_magic_string_set(EFL_DBUS_SIGNAL_HANDLER_MAGIC,
  ...
  eina_magic_string_set(EFL_DBUS_MESSAGE_ITERATOR_MAGIC,
                        "Efl_Dbus_Message_Iterator");
  eina_magic_string_set(EFL_DBUS_SERVICE_INTERFACE_MAGIC,
...
{
  if (!conn->names) ERR("conn=%p has no alive objects", conn);
  else
  {
  ...
  if (!conn->pendings) ERR("conn=%p has no alive pending calls", conn);
  else
  {
    Efl_Dbus_Pending *p;
...
efl_dbus_shutdown(void)
{
  ...
  if (--_efl_dbus_init_count) return _efl_dbus_init_count;
  ...
  if (shared_connections[EFL_DBUS_CONNECTION_TYPE_SESSION - 1])
  {
    ...
    print_live_connection(
      shared_connections[EFL_DBUS_CONNECTION_TYPE_SESSION - 1]);
  }
  if (shared_connections[EFL_DBUS_CONNECTION_TYPE_SYSTEM - 1])
  {
    CRI("Alive TYPE_SYSTEM connection");
    print_live_connection(
      shared_connections[EFL_DBUS_CONNECTION_TYPE_SYSTEM - 1]);
  }
  if (shared_connections[EFL_DBUS_CONNECTION_TYPE_STARTER - 1])
  {
    CRI("Alive TYPE_STARTER connection");
    print_live_connection(
      shared_connections[EFL_DBUS_CONNECTION_TYPE_STARTER - 1]);
  }
  if (shared_connections[EFL_DBUS_CONNECTION_TYPE_ADDRESS - 1])
  {
    ...
    address_connections =
      shared_connections[EFL_DBUS_CONNECTION_TYPE_ADDRESS - 1] = NULL;
  }
...
static void
efl_dbus_connection_name_gc(Efl_Dbus_Connection      *conn,
                            Efl_Dbus_Connection_Name *cn)
{
  if (cn->refcount > 0) return;
  if (cn->objects && eina_hash_population(cn->objects) > 0) return;
  if (cn->event_handlers.list != NULL) return;
  ...
  if (cn->name_owner_get) efl_dbus_pending_cancel(cn->name_owner_get);
  if (cn->unique_id) eina_stringshare_del(cn->unique_id);
  eina_list_free(cn->event_handlers.to_delete);
  free(cn);
}

void
efl_dbus_connection_name_object_del(Efl_Dbus_Connection   *conn,
                                    const Efl_Dbus_Object *obj)
{
...
void
efl_dbus_connection_name_object_set(Efl_Dbus_Connection *conn,
                                    Efl_Dbus_Object     *obj)
{
  Efl_Dbus_Connection_Name *cn;
                  ...
                  Efl_Dbus_Pending *pending EFL_UNUSED)
{
  ...
  {
    if (!strcmp(error_name, EFL_DBUS_ERROR_PENDING_CANCELED)) return;
...
void
efl_dbus_connection_name_unref(Efl_Dbus_Connection      *conn,
                               Efl_Dbus_Connection_Name *cn)
{
...
efl_dbus_connection_name_get(Efl_Dbus_Connection *conn, const char *name)
{
  ...
  if (name[0] == ':') cn->unique_id = eina_stringshare_add(name);
  else
    cn->name_owner_get =
      efl_dbus_name_owner_get(conn, cn->name, on_get_name_owner, cn);

  cn->name_owner_changed = _efl_dbus_signal_handler_add(conn,
                                                        EFL_DBUS_FDO_BUS,
                                                        EFL_DBUS_FDO_PATH,
                                                        ...
                                                        cn);
  efl_dbus_signal_handler_match_extra_set(cn->name_owner_changed,
                                          "arg0",
                                          cn->name,
                                          NULL);
...
Efl_Dbus_Object *
efl_dbus_connection_name_object_get(Efl_Dbus_Connection *conn,
                                    const char          *name,
                                    const char          *path)
{
  Efl_Dbus_Connection_Name *cn;
...
cb_watch_add(DBusWatch *watch, void *data)
{
  ...
  conn->fd_handlers =
    eina_inlist_append(hd->conn->fd_handlers, EINA_INLIST_GET(hd));
...
cb_timeout_add(DBusTimeout *timeout, void *data)
{
  ...
  if (!dbus_timeout_get_enabled(timeout)) return EFL_TRUE;
  ...
  td->handler =
    core_timer_add(td->interval / 1000.0, efl_dbus_timeout_handler, td);
...
cb_timeout_toggle(DBusTimeout *timeout, void *data EFL_UNUSED)
{
  ...
  {
    td->interval = dbus_timeout_get_interval(timeout);
    td->handler  = core_timer_add(td->interval, efl_dbus_timeout_handler, td);
...
efl_dbus_idle_enterer(void *data)
{
  ...
  DBG("Connection@%p: Dispatch status: %d",
      conn,
...
static void
cb_dispatch_status(DBusConnection *dbus_conn EFL_UNUSED,
...
extra_arguments_check(DBusMessage *msg, Efl_Dbus_Signal_Handler *sh)
{
  ...
  EINA_INLIST_FOREACH(sh->args, arg)
  {
    ...
    if (arg_index != arg->index) return EFL_FALSE;

    type = dbus_message_iter_get_arg_type(&iter);
    if (!(type == 's' || type == 'o')) return EFL_FALSE;

    dbus_message_iter_get_basic(&iter, &arg_msg);
    if (strcmp(arg_msg, arg->value)) return EFL_FALSE;
  }
  return EFL_TRUE;
}
...
cb_signal_dispatcher(Efl_Dbus_Connection *conn, DBusMessage *msg)
{
  ...
  while (next != NULL)
  {
    ...
    if (sh->sender)
    {
      if (sh->bus)
      {
        if ((sh->bus->unique_id == NULL) || (!sh->bus->unique_id[0]) ||
            (!dbus_message_has_sender(msg, sh->bus->unique_id)))
          continue;
      }
      else if (!dbus_message_has_sender(msg, sh->sender)) continue;
    }
...
efl_dbus_filter(DBusConnection *conn_dbus EFL_UNUSED,
                DBusMessage              *message,
                void                     *user_data)
{
...
_connection_get(Efl_Dbus_Connection_Type type,
                const char              *address,
                Efl_Bool                 shared)
{
  Efl_Dbus_Connection *conn;
  ...
  EINA_SAFETY_ON_FALSE_RETURN_VAL((type < EFL_DBUS_CONNECTION_TYPE_LAST) &&
                                    (type > EFL_DBUS_CONNECTION_TYPE_UNKNOWN),
                                  NULL);
  ...
  else conn->dbus_conn = dbus_bus_get_private(type - 1, &err);
...
efl_dbus_connection_get(Efl_Dbus_Connection_Type type)
{
  ...
  if (type == EFL_DBUS_CONNECTION_TYPE_ADDRESS)
  {
    ERR("CONNECTION_TYPE_ADDRESS must be used with appropriate "
        "address_connection_get() function");
    return NULL;
  }
...
efl_dbus_address_connection_get(const char *address)
{
  ...
  if (address_connections == NULL)
  {
    ...
    shared_connections[EFL_DBUS_CONNECTION_TYPE_ADDRESS - 1] =
      address_connections;
  }
  else
  {
...
_efl_dbus_connection_free(Efl_Dbus_Connection *conn)
{
  ...
  _efl_dbus_connection_event_callback_call(conn,
  ...
  EINA_ITERATOR_FOREACH(iter, cn)
  {
    while (cn->event_handlers.list)
    {
      ...
      cn->event_handlers.list =
        eina_inlist_remove(cn->event_handlers.list, cn->event_handlers.list);
      free(ctx);
    }
  ...
  if (conn->type && conn->shared)
  {
    ...
    {
      if (address_connections) eina_hash_del_by_data(address_connections, conn);
    }
...
EAPI void
efl_dbus_connection_free_cb_add(Efl_Dbus_Connection *conn,
                                Efl_Dbus_Free_Cb     cb,
                                const void          *data)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
...
EAPI void
efl_dbus_connection_free_cb_del(Efl_Dbus_Connection *conn,
                                Efl_Dbus_Free_Cb     cb,
                                const void          *data)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
...
EAPI void
efl_dbus_connection_data_set(Efl_Dbus_Connection *conn,
                             const char          *key,
                             const void          *data)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
...
static void
efl_dbus_dispatch_name_owner_change(Efl_Dbus_Connection_Name *cn,
                                    const char               *old_id)
{
  Efl_Dbus_Connection_Context_NOC_Cb *ctx;
  ...
  EINA_INLIST_FOREACH(cn->event_handlers.list, ctx)
  {
    ...
    if (!old_id && !ctx->allow_initial) continue;
  ...
  {
    cn->event_handlers.list =
      eina_inlist_remove(cn->event_handlers.list, EINA_INLIST_GET(ctx));
    free(ctx);
  }
...
dispach_name_owner_cb(void *context)
{
  ...
  data->ctx->cb((void *)data->ctx->cb_data,
                data->cn->name,
                "",
                data->cn->unique_id);
                                         ...
                                         Efl_Bool allow_initial_call)
{
  ...
  cn->event_handlers.list =
    eina_inlist_append(cn->event_handlers.list, EINA_INLIST_GET(ctx));
                                         ...
                                         const void                    *cb_data)
{
  ...
  if (cn->event_handlers.walking)
  {
    ...
    cn->event_handlers.to_delete =
      eina_list_append(cn->event_handlers.to_delete, found);
    return;
  }

  cn->event_handlers.list =
    eina_inlist_remove(cn->event_handlers.list, EINA_INLIST_GET(found));
...
static void
_efl_dbus_connection_context_event_cb_del(
  Efl_Dbus_Connection_Context_Event    *ce,
  Efl_Dbus_Connection_Context_Event_Cb *ctx)
{
...
void
efl_dbus_connection_event_callback_call(Efl_Dbus_Connection           *conn,
                                        Efl_Dbus_Connection_Event_Type type,
                                        const void *event_info)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
...
void
efl_dbus_connection_signal_handler_add(Efl_Dbus_Connection     *conn,
                                       Efl_Dbus_Signal_Handler *handler)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
  EINA_SAFETY_ON_NULL_RETURN(handler);
  conn->signal_handlers =
    eina_inlist_append(conn->signal_handlers, EINA_INLIST_GET(handler));
}

void
efl_dbus_connection_pending_add(Efl_Dbus_Connection *conn,
                                Efl_Dbus_Pending    *pending)
{
  EFL_DBUS_CONNECTION_CHECK(conn);
  EINA_SAFETY_ON_NULL_RETURN(pending);
  conn->pendings = eina_inlist_append(conn->pendings, EINA_INLIST_GET(pending));
}

void
efl_dbus_connection_signal_handler_del(Efl_Dbus_Connection     *conn,
                                       Efl_Dbus_Signal_Handler *handler)
{
  EINA_SAFETY_ON_NULL_RETURN(conn);
  EINA_SAFETY_ON_NULL_RETURN(handler);
  conn->signal_handlers =
    eina_inlist_remove(conn->signal_handlers, EINA_INLIST_GET(handler));
}

void
efl_dbus_connection_pending_del(Efl_Dbus_Connection *conn,
                                Efl_Dbus_Pending    *pending)
{
  EINA_SAFETY_ON_NULL_RETURN(conn);
  EINA_SAFETY_ON_NULL_RETURN(pending);
  conn->pendings = eina_inlist_remove(conn->pendings, EINA_INLIST_GET(pending));
}
