...
efl_dbus_connection_name_get(Efl_Dbus_Connection *conn, const char *name)
{
    ...
    cn->name_owner_changed =
        _efl_dbus_signal_handler_add(conn,
                                     EFL_DBUS_FDO_BUS,
                                     EFL_DBUS_FDO_PATH,
                ...
                Efl_Bool                 shared)
{
    ...
    EINA_SAFETY_ON_FALSE_RETURN_VAL(
        (type < EFL_DBUS_CONNECTION_TYPE_LAST) &&
...
efl_dbus_address_connection_get(const char *address)
{
    ...
    {
        conn =
            (Efl_Dbus_Connection *)eina_hash_find(address_connections, address);
    }
...
efl_dbus_private_address_connection_get(const char *address)
{
    ...
    return _connection_get(EFL_DBUS_CONNECTION_TYPE_ADDRESS,
                           address,
                           EFL_FALSE);
}
    ...
    */
    ...
    EINA_ITERATOR_FOREACH(iter, cn)
    {
        while (cn->event_handlers.list)
        {
            ...
            cn->event_handlers.list =
                eina_inlist_remove(cn->event_handlers.list,
                                   cn->event_handlers.list);
            free(ctx);
        }
    ...
    for (i = 0; i < EFL_DBUS_CONNECTION_EVENT_LAST; i++)
    {
        ...
        while (ce->list)
        {
            ...
            ctx =
                EINA_INLIST_CONTAINER_GET(ce->list,
                                          Efl_Dbus_Connection_Context_Event_Cb);
                                ...
                                Efl_Dbus_Pending    *pending)
{
    ...
    conn->pendings =
        eina_inlist_append(conn->pendings, EINA_INLIST_GET(pending));
}
                                ...
                                Efl_Dbus_Pending    *pending)
{
    ...
    EINA_SAFETY_ON_NULL_RETURN(pending);
    conn->pendings =
        eina_inlist_remove(conn->pendings, EINA_INLIST_GET(pending));
}
