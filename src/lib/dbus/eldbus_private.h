...
Efl_Bool              eldbus_message_init(void);
void                   eldbus_message_shutdown(void);
Eldbus_Message         *eldbus_message_new(Efl_Bool writable);

Efl_Bool              eldbus_signal_handler_init(void);
void                   eldbus_signal_handler_shutdown(void);

Efl_Bool              eldbus_pending_init(void);
void                   eldbus_pending_shutdown(void);

Efl_Bool              eldbus_object_init(void);
void                   eldbus_object_shutdown(void);

Efl_Bool              eldbus_proxy_init(void);
void                   eldbus_proxy_shutdown(void);

Efl_Bool              eldbus_service_init(void);
...
Efl_Bool              eldbus_object_proxy_del(Eldbus_Object *obj, Eldbus_Proxy *proxy, const char *interface) EINA_ARG_NONNULL(1, 2);
...
Efl_Bool              eldbus_object_proxy_add(Eldbus_Object *obj, Eldbus_Proxy *proxy) EINA_ARG_NONNULL(1, 2);
...
void                   eldbus_connection_name_owner_monitor(Eldbus_Connection *conn, Eldbus_Connection_Name *cn, Efl_Bool enable);
...
Efl_Bool              _message_iter_from_eina_value(const char *signature, Eldbus_Message_Iter *iter, const Eina_Value *value);
Efl_Bool              _message_iter_from_eina_value_struct(const char *signature, Eldbus_Message_Iter *iter, const Eina_Value *value);
...
Eldbus_Message         *eldbus_message_signal_new(const char *path, const char *interface, const char *name) EINA_ARG_NONNULL(1, 2, 3) EFL_WARN_UNUSED_RESULT;
