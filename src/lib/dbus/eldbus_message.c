...
Eldbus_Message *
eldbus_message_new(Efl_Bool writable)
{
...
EAPI Eldbus_Message *
eldbus_message_method_call_new(const char *dest,
                               const char *path,
                               const char *iface,
                               const char *method)
{
   ...
   // added to libdbus:
    ...
    msg = eldbus_message_new(EFL_TRUE);
...
eldbus_message_unref(Eldbus_Message *msg)
{
    ...
    if (msg->dbus_msg) dbus_message_unref(msg->dbus_msg);
    msg->dbus_msg = NULL;
...
EAPI Efl_Bool
eldbus_message_error_get(const Eldbus_Message *msg,
                         const char          **name,
                         const char          **text)
{
    if (name) *name = NULL;
    ...
    ELDBUS_MESSAGE_CHECK_RETVAL(msg, EFL_FALSE);

    if (dbus_message_get_type(msg->dbus_msg) != DBUS_MESSAGE_TYPE_ERROR)
        return EFL_FALSE;

    if (name) *name = dbus_message_get_error_name(msg->dbus_msg);

    if (text)
        dbus_message_get_args(msg->dbus_msg,
                              NULL,
                              DBUS_TYPE_STRING,
                              text,
                              DBUS_TYPE_INVALID);
...
static Efl_Bool
_eldbus_message_arguments_vget(Eldbus_Message *msg,
                               const char     *signature,
                               va_list         ap)
{
    Eldbus_Message_Iter *iter;
    iter = eldbus_message_iter_get(msg);
    ELDBUS_MESSAGE_ITERATOR_CHECK_RETVAL(iter, EFL_FALSE);
...
EAPI Efl_Bool
eldbus_message_arguments_get(const Eldbus_Message *msg,
                             const char           *signature,
                             ...)
{
    Efl_Bool ret;
...
EAPI Efl_Bool
eldbus_message_arguments_vget(const Eldbus_Message *msg,
                              const char           *signature,
                              va_list               ap)
{
    ELDBUS_MESSAGE_CHECK_RETVAL(msg, EFL_FALSE);
...
static Efl_Bool
_eldbus_message_iter_arguments_vappend(Eldbus_Message_Iter *iter,
                                       const char          *signature,
                                       va_list             *aq)
{
    ...
    while ((type = dbus_signature_iter_get_signature(&signature_iter)))
    {
        ...
        else
        {
            ...
            iter->iterators =
                eina_inlist_append(iter->iterators, EINA_INLIST_GET(sub));
...
EAPI Efl_Bool
eldbus_message_iter_arguments_vappend(Eldbus_Message_Iter *iter,
                                      const char          *signature,
                                      va_list              ap)
{
    va_list  aq;
...
EAPI Efl_Bool
eldbus_message_iter_arguments_append(Eldbus_Message_Iter *iter,
                                     const char          *signature,
                                     ...)
{
    Efl_Bool r;
...
static Efl_Bool
_eldbus_message_arguments_vappend(Eldbus_Message *msg,
                                  const char     *signature,
                                  va_list        *aq)
{
    DBusSignatureIter    signature_iter;
...
EAPI Efl_Bool
eldbus_message_arguments_vappend(Eldbus_Message *msg,
                                 const char     *signature,
                                 va_list         ap)
{
    va_list  aq;
...
EAPI Eldbus_Message_Iter *
eldbus_message_iter_container_new(Eldbus_Message_Iter *iter,
                                  int                  type,
                                  const char          *contained_signature)
{
    ...
    if (!dbus_message_iter_open_container(&iter->dbus_iterator,
                                          type,
                                          contained_signature,
                                          &sub->dbus_iterator))
        goto cleanup;
...
EAPI Efl_Bool
eldbus_message_iter_container_close(Eldbus_Message_Iter *iter,
                                    Eldbus_Message_Iter *sub)
{
    ELDBUS_MESSAGE_ITERATOR_CHECK_RETVAL(iter, EFL_FALSE);
    ELDBUS_MESSAGE_ITERATOR_CHECK_RETVAL(sub, EFL_FALSE);
    EINA_SAFETY_ON_FALSE_RETURN_VAL(iter->writable, EFL_FALSE);
...
EAPI Efl_Bool
eldbus_message_iter_fixed_array_append(Eldbus_Message_Iter *iter,
                                       int                  type,
                                       const void          *array,
                                       unsigned int         size)
{
    ...
    return dbus_message_iter_append_fixed_array(&iter->dbus_iterator,
                                                type,
                                                &array,
                                                (int)size);
}
...
EAPI Efl_Bool
eldbus_message_iter_fixed_array_get(Eldbus_Message_Iter *iter,
                                    int                  signature,
                                    void                *value,
                                    int                 *n_elements)
{
...
static Efl_Bool
_eldbus_message_iter_arguments_vget(Eldbus_Message_Iter *iter,
                                    const char          *signature,
                                    va_list             *aq)
{
    int               iter_type;
    ...
    while (iter_type != DBUS_TYPE_INVALID)
    {
        ...
        if (sig_type == DBUS_TYPE_INVALID) break;

        if (sig_type != iter_type)
        {
            ERR("Type in iterator different of signature expected:%c got %c",
                iter_type,
                sig_type);
            return EFL_FALSE;
        }
        ...
        else
        {
            ...
            iter->iterators =
                eina_inlist_append(iter->iterators, EINA_INLIST_GET(sub_itr));
            *user_itr = sub_itr;
        }

        dbus_message_iter_next(&iter->dbus_iterator);
        if (!dbus_signature_iter_next(&sig_iter)) break;
        iter_type = dbus_message_iter_get_arg_type(&iter->dbus_iterator);
    }
...
EAPI Efl_Bool
eldbus_message_iter_arguments_get(Eldbus_Message_Iter *iter,
                                  const char          *signature,
                                  ...)
{
    va_list  ap;
...
EAPI Efl_Bool
eldbus_message_iter_arguments_vget(Eldbus_Message_Iter *iter,
                                   const char          *signature,
                                   va_list              ap)
{
    va_list  aq;
...
EAPI Eldbus_Message *
eldbus_message_error_new(const Eldbus_Message *msg,
                         ...
                         const char           *error_msg)
{
    ...
    reply = eldbus_message_new(EFL_FALSE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(reply, NULL);

    reply->dbus_msg =
        dbus_message_new_error(msg->dbus_msg, error_name, error_msg);
...
eldbus_message_method_return_new(const Eldbus_Message *msg)
{
    ...
    reply = eldbus_message_new(EFL_TRUE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(reply, NULL);
...
EAPI Eldbus_Message *
eldbus_message_signal_new(const char *path,
