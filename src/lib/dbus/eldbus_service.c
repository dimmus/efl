...
static inline void
_introspect_append_signal(Efl_Strbuf *buf, const Eldbus_Signal *sig)
{
...
_instrospect_append_property(Efl_Strbuf                     *buf,
                             const Eldbus_Property          *prop,
                             const Eldbus_Service_Interface *iface)
{
    eina_strbuf_append_printf(buf,
...
static inline void
_introspect_append_method(Efl_Strbuf *buf, const Eldbus_Method *method)
{
...
static void
_introspect_append_interface(Efl_Strbuf *buf, Eldbus_Service_Interface *iface)
{
...
_cb_property_get(const Eldbus_Service_Interface *piface,
                 const Eldbus_Message           *msg)
{
    ...
    variant =
        eldbus_message_iter_container_new(main_iter, 'v', prop->property->type);
...
static Efl_Bool
_props_getall(Eldbus_Service_Interface *iface,
              Eina_Iterator            *iterator,
              Eldbus_Message_Iter      *dict,
              const Eldbus_Message     *input_msg,
              Eldbus_Message          **error_reply)
{
    Property *prop;
    EINA_ITERATOR_FOREACH(iterator, prop)
    {
        ...
        var =
            eldbus_message_iter_container_new(entry, 'v', prop->property->type);
...
static Eldbus_Message *
_cb_property_getall(const Eldbus_Service_Interface *piface,
                    const Eldbus_Message           *msg)
{
    ...
    if (!eldbus_message_arguments_get(msg, "s", &iface_name)) return NULL;
...
static Eldbus_Message *
_cb_property_set(const Eldbus_Service_Interface *piface,
                 const Eldbus_Message           *msg)
{
    const char               *propname, *iface_name;
    Eldbus_Service_Object    *obj = piface->obj;
...
static Eldbus_Message *
cb_introspect(const Eldbus_Service_Interface *_iface,
...
static const Eldbus_Method introspect = { "Introspect",
                                          NULL,
                                          ELDBUS_ARGS({ "s", "xml" }),
                                          cb_introspect,
                                          0 };
...
static const Eldbus_Method _property_methods[] = {
    { "Get",
     ELDBUS_ARGS({ "s", "interface" }, { "s", "property" }),
     ELDBUS_ARGS({ "v", "value" }),
     _cb_property_get, 0 },
    { "Set",
     ELDBUS_ARGS({ "s", "interface" }, { "s", "property" }, { "v", "value" }),
     NULL, _cb_property_set,
     0 },
    { "GetAll",
     ELDBUS_ARGS({ "s", "interface" }),
     ELDBUS_ARGS({ "a{sv}", "props" }),
...
static Efl_Bool
_propmgr_iface_props_append(Eldbus_Service_Interface *iface,
                            Eldbus_Message_Iter      *array)
{
    ...
    eldbus_message_iter_arguments_append(iface_entry,
                                         "sa{sv}",
                                         iface->name,
                                         &props_array);
    iterator = eina_hash_iterator_data_new(iface->properties);
...
static Efl_Bool
_managed_obj_append(Eldbus_Service_Object *obj,
                    Eldbus_Message_Iter   *array,
                    Efl_Bool               first)
{
    ...
    eldbus_message_iter_arguments_append(obj_entry,
                                         "oa{sa{sv}}",
                                         obj->path,
                                         &array_interface);
...
static Eldbus_Message *
_cb_managed_objects(const Eldbus_Service_Interface *iface,
                    const Eldbus_Message           *msg)
{
...
static Eldbus_Method get_managed_objects = { "GetManagedObjects",
                                             NULL,
                                             ELDBUS_ARGS({ "a{oa{sa{sv}}}",
                                                           "objects" }),
                                             _cb_managed_objects,
                                             0 };
...
static const Eldbus_Signal _object_manager_signals[] = {
    {  "InterfacesAdded",
     ELDBUS_ARGS({ "o", "object" }, { "a{sa{sv}}", "interfaces" }),
     0},
    {"InterfacesRemoved",
     ELDBUS_ARGS({ "o", "object" },        { "as", "interfaces" }),
     0}
};
...
static Eldbus_Service_Object *
_eldbus_service_object_add(Eldbus_Connection *conn,
                           const char        *path,
                           Efl_Bool           fallback)
{
    ...
    */
    ...
    EINA_INLIST_FOREACH_SAFE(conn->root_objs, safe, rootobj)
    {
        ...
        obj->children =
            eina_inlist_append(obj->children, EINA_INLIST_GET(rootobj));
        rootobj->parent = obj;
    }
                                 ...
                                 Eldbus_Service_Object *parent)
{
    ...
    Eldbus_Message           *sig =
        eldbus_message_signal_new(parent->path,
                                  ELDBUS_FDO_INTERFACE_OBJECT_MANAGER,
                                  "InterfacesAdded");
    EINA_SAFETY_ON_NULL_RETURN(sig);
    iter = eldbus_message_iter_get(sig);
    eldbus_message_iter_arguments_append(iter, "oa{sa{sv}}", obj->path, &array);
...
static Eldbus_Service_Interface *
_eldbus_service_interface_register(Eldbus_Connection                   *conn,
                                   const char                          *path,
                                   const Eldbus_Service_Interface_Desc *desc,
                                   Efl_Bool     fallback,
                                   unsigned int version)
{
    ...
    if (!obj) obj = _eldbus_service_object_add(conn, path, fallback);
...
eldbus_service_interface_register(Eldbus_Connection                   *conn,
                                  const char                          *path,
                                  const Eldbus_Service_Interface_Desc *desc)
{
    return _eldbus_service_interface_register(conn, path, desc, EFL_FALSE, 1u);
}

EAPI Eldbus_Service_Interface *
eldbus_service_interface_register2(Eldbus_Connection                    *conn,
                                   const char                           *path,
                                   const Eldbus_Service_Interface_Desc2 *desc)
{
    return _eldbus_service_interface_register(conn,
                                              path,
                                              &desc->description,
                                              EFL_FALSE,
                                              desc->version);
...
eldbus_service_interface_fallback_register(
    ...
    const Eldbus_Service_Interface_Desc *desc)
{
    return _eldbus_service_interface_register(conn, path, desc, EFL_TRUE, 1u);
}
...
_idle_enterer_propschanged(void *data)
{
    ...
    msg   = eldbus_message_signal_new(iface->obj->path,
...
invalidate:
    ...
    while ((prop = eina_array_pop(iface->prop_invalidated)))
    {
        ...
        eldbus_message_iter_basic_append(array_invalidate,
                                         's',
                                         prop->property->name);
    }
end:
...
eldbus_service_object_unregister(Eldbus_Service_Interface *iface)
{
    ...
    * by libdbus.
    */
    eldbus_connection_free_cb_del(iface->obj->conn,
                                  _on_connection_free,
                                  iface->obj);
...
static Eldbus_Message *
_eldbus_method_call(const Eldbus_Method      *method,
                    Eldbus_Service_Interface *iface,
                    Eldbus_Message           *msg)
{
    if (method->flags & ELDBUS_METHOD_FLAG_HAS_DATA)
    {
        ...
        return cb(((const Eldbus_Method2 *)method)->data, iface, msg);
    }
    else
    {
                ...
                void                     *user_data)
{
    ...
    if (!_have_signature(method->in, eldbus_msg))
        reply = eldbus_message_error_new(
            eldbus_msg,
            DBUS_ERROR_INVALID_SIGNATURE,
            "See introspectable to know the expected signature");
    else
    {
        if (iface->obj) reply = _eldbus_method_call(method, iface, eldbus_msg);
        else
        {
    ...
    if (reply) _eldbus_connection_send(conn, reply, NULL, NULL, -1);
...
EAPI Eldbus_Message *
eldbus_service_signal_new(const Eldbus_Service_Interface *iface,
                          unsigned int                    signal_id)
{
    ...
    return eldbus_message_signal_new(iface->obj->path,
                                     iface->name,
                                     iface->signals[signal_id].name);
}

EAPI Efl_Bool
eldbus_service_signal_emit(const Eldbus_Service_Interface *iface,
                           unsigned int                    signal_id,
                           ...)
{
    Eldbus_Message *sig;
    ...
    ELDBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, EFL_FALSE);
    size = eina_array_count(iface->sign_of_signals);
...
EAPI Efl_Bool
eldbus_service_signal_send(const Eldbus_Service_Interface *iface,
...
EAPI void
eldbus_service_object_data_set(Eldbus_Service_Interface *iface,
                               const char               *key,
                               const void               *data)
{
    ELDBUS_SERVICE_INTERFACE_CHECK(iface);
...
EAPI void *
eldbus_service_object_data_get(const Eldbus_Service_Interface *iface,
                               const char                     *key)
{
    ELDBUS_SERVICE_INTERFACE_CHECK_RETVAL(iface, NULL);
...
EAPI Efl_Bool
eldbus_service_property_changed(const Eldbus_Service_Interface *interface,
                                const char                     *name)
{
    Property                 *prop;
...
EAPI Efl_Bool
eldbus_service_property_invalidate_set(
    const Eldbus_Service_Interface *interface,
    const char                     *name,
