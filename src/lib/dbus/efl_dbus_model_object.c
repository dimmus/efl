...
static void _efl_dbus_model_object_introspect_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);
static void _efl_dbus_model_object_create_children(Efl_Dbus_Model_Object_Data *, Efl_Dbus_Object *, Eina_List *);

static Efl_Object*
_efl_dbus_model_object_efl_object_constructor(Eo *obj, Efl_Dbus_Model_Object_Data *pd)
{
...
static Efl_Object*
_efl_dbus_model_object_efl_object_finalize(Eo *obj, Efl_Dbus_Model_Object_Data *pd)
{
   if (!pd->bus || !pd->path)
     return NULL;
...
static void
_efl_dbus_model_object_efl_object_invalidate(Eo *obj, Efl_Dbus_Model_Object_Data *pd)
{
...
static void
_efl_dbus_model_object_efl_object_destructor(Eo *obj, Efl_Dbus_Model_Object_Data *pd)
{
                                ...
                                const char *path)
{
   ...
   object = efl_dbus_object_get(efl_dbus_model_connection_get(obj),
                              bus, path);
   if (!object)
     {
   ...
   pending = efl_dbus_object_introspect(object, &_efl_dbus_model_object_introspect_cb, pd);
...
static Eina_Future *
_efl_dbus_model_object_efl_model_children_slice_get(Eo *obj EFL_UNUSED,
                                                  Efl_Dbus_Model_Object_Data *pd,
                                                  unsigned start,
                                                  unsigned count)
{
...
static unsigned int
_efl_dbus_model_object_efl_model_children_count_get(const Eo *obj EFL_UNUSED,
                                                  Efl_Dbus_Model_Object_Data *pd)
{
...
static const char *
_efl_dbus_model_object_bus_get(const Eo *obj EFL_UNUSED, Efl_Dbus_Model_Object_Data *pd)
{
   return pd->bus;
}

static const char *
_efl_dbus_model_object_path_get(const Eo *obj EFL_UNUSED, Efl_Dbus_Model_Object_Data *pd)
{
   return pd->path;
}
   ...
   EINA_LIST_FOREACH(nodes, it, node)
     {
        ...
        absolute_path = _efl_dbus_model_object_concatenate_path(current_path, relative_path);
...
static void
_efl_dbus_model_object_create_children(Efl_Dbus_Model_Object_Data *pd, Efl_Dbus_Object *object, Eina_List *interfaces)
{
   ...
   EINA_LIST_FOREACH(interfaces, l, interface)
     {
        ...
        child = efl_add_ref(EFL_DBUS_MODEL_PROXY_CLASS, pd->obj,
                                   ...
                                   Efl_Dbus_Pending *pending)
{
   ...
   _efl_dbus_model_object_introspect_nodes(pd, current_path, pd->introspection->nodes);
   _efl_dbus_model_object_create_children(pd, object, pd->introspection->interfaces);
