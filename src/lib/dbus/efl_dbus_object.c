...
_efl_dbus_object_call_del(Efl_Dbus_Object *obj)
{
  ...
  while (ce->list)
  {
    ...
    ctx = EINA_INLIST_CONTAINER_GET(ce->list, Efl_Dbus_Object_Context_Event_Cb);
...
_efl_dbus_object_free(Efl_Dbus_Object *obj)
{
  ...
  for (i = 0; i < EFL_DBUS_OBJECT_EVENT_LAST; i++)
  {
    ...
    while (ce->list)
    {
      ...
      ctx =
        EINA_INLIST_CONTAINER_GET(ce->list, Efl_Dbus_Object_Context_Event_Cb);
      _efl_dbus_object_context_event_cb_del(ce, ctx);
    }
    eina_list_free(ce->to_delete);
  }

  if (obj->interfaces_added) efl_dbus_signal_handler_del(obj->interfaces_added);
    ...
    case EFL_DBUS_OBJECT_EVENT_IFACE_ADDED:
      {
        ...
        obj->interfaces_added =
          _efl_dbus_signal_handler_add(obj->conn,
                                       obj->name,
                                       NULL,
                                   ...
                                   const void                *cb_data)
{
  ...
  switch (type)
  {
    case EFL_DBUS_OBJECT_EVENT_IFACE_ADDED:
      {
        if (obj->event_handlers[EFL_DBUS_OBJECT_EVENT_IFACE_ADDED].list) break;
                          ...
                          const void         *data)
{
  ...
  EFL_DBUS_OBJECT_CHECK_RETVAL(obj, NULL);
  msg =
    efl_dbus_object_method_call_new(obj, EFL_DBUS_FDO_INTEFACE_PEER, "Ping");
