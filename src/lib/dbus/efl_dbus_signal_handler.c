...
_match_append(Efl_Strbuf *match, const char *key, const char *value)
{
  ...
  if ((eina_strbuf_length_get(match) + strlen(",=''") + strlen(key) +
       strlen(value)) >= DBUS_MAXIMUM_MATCH_RULE_LENGTH)
  {
    ERR("cannot add match %s='%s' to %s: too long!",
        key,
        value,
                                         ...
                                         va_list                  ap)
{
  ...
  dbus_bus_remove_match(sh->conn->dbus_conn,
                        eina_strbuf_string_get(sh->match),
                        NULL);
                            ...
                            const void          *cb_data)
{
  ...
  sh = _efl_dbus_signal_handler_add(conn,
                                    sender,
                                    path,
                                    interface,
                                    member,
...
_efl_dbus_signal_handler_add(Efl_Dbus_Connection *conn,
                             const char          *sender,
                             const char          *path,
                             const char          *interface,
                             const char          *member,
...
_efl_dbus_signal_handler_clean(Efl_Dbus_Signal_Handler *handler)
{
  ...
  dbus_bus_remove_match(handler->conn->dbus_conn,
                        eina_strbuf_string_get(handler->match),
                        NULL);
...
_efl_dbus_signal_handler_del(Efl_Dbus_Signal_Handler *handler)
{
  ...
  if (handler->bus) efl_dbus_connection_name_unref(handler->conn, handler->bus);
  free(handler);
}
...
EAPI void
efl_dbus_signal_handler_free_cb_add(Efl_Dbus_Signal_Handler *handler,
                                    Efl_Dbus_Free_Cb         cb,
                                    const void              *data)
{
  EFL_DBUS_SIGNAL_HANDLER_CHECK(handler);
...
EAPI void
efl_dbus_signal_handler_free_cb_del(Efl_Dbus_Signal_Handler *handler,
                                    Efl_Dbus_Free_Cb         cb,
                                    const void              *data)
{
  EFL_DBUS_SIGNAL_HANDLER_CHECK(handler);
