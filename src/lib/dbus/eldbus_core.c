...
static void
            _eldbus_connection_event_callback_call(Eldbus_Connection           *conn,
                                                   Eldbus_Connection_Event_Type type,
                                                   const void                  *event_info);
static void _eldbus_connection_context_event_cb_del(
...
eldbus_init(void)
{
    if (_eldbus_init_count++ > 0) return _eldbus_init_count;
    ...
    eldbus_model_log_dom =
        eina_log_domain_register("eldbus_model", EINA_COLOR_CYAN);
    ...
    eina_magic_string_set(ELDBUS_MESSAGE_ITERATOR_MAGIC,
                          "Eldbus_Message_Iterator");
    eina_magic_string_set(ELDBUS_SERVICE_INTERFACE_MAGIC,
...
eldbus_shutdown(void)
{
    ...
    if (shared_connections[ELDBUS_CONNECTION_TYPE_ADDRESS - 1])
    {
        ...
        address_connections =
            shared_connections[ELDBUS_CONNECTION_TYPE_ADDRESS - 1] = NULL;
    }
...
eldbus_connection_name_gc(Eldbus_Connection *conn, Eldbus_Connection_Name *cn)
{
    ...
    if (cn->name_owner_get) eldbus_pending_cancel(cn->name_owner_get);
...
void
eldbus_connection_name_object_del(Eldbus_Connection   *conn,
                                  const Eldbus_Object *obj)
{
...
on_get_name_owner(void                   *data,
                  ...
                  Eldbus_Pending *pending EFL_UNUSED)
{
    ...
    {
        if (!strcmp(error_name, ELDBUS_ERROR_PENDING_CANCELED)) return;
...
void
eldbus_connection_name_unref(Eldbus_Connection      *conn,
                             Eldbus_Connection_Name *cn)
{
...
eldbus_connection_name_get(Eldbus_Connection *conn, const char *name)
{
    ...
    cn->name_owner_changed = _eldbus_signal_handler_add(conn,
                                                        ELDBUS_FDO_BUS,
                                                        ELDBUS_FDO_PATH,
                                                        ...
                                                        cn);
    eldbus_signal_handler_match_extra_set(cn->name_owner_changed,
                                          "arg0",
                                          cn->name,
                                          NULL);
...
Eldbus_Object *
eldbus_connection_name_object_get(Eldbus_Connection *conn,
                                  const char        *name,
                                  const char        *path)
{
    Eldbus_Connection_Name *cn;
...
cb_signal_dispatcher(Eldbus_Connection *conn, DBusMessage *msg)
{
    ...
    eldbus_msg = eldbus_message_new(EFL_FALSE);
...
eldbus_filter(DBusConnection *conn_dbus EFL_UNUSED,
              DBusMessage              *message,
              void                     *user_data)
{
                ...
                Efl_Bool               shared)
{
    ...
    eldbus_signal_handler_add(conn,
                              NULL,
                              DBUS_PATH_LOCAL,
...
eldbus_address_connection_get(const char *address)
{
    ...
    if (address_connections == NULL)
    {
        ...
        shared_connections[ELDBUS_CONNECTION_TYPE_ADDRESS - 1] =
            address_connections;
    }
    else
    {
        conn =
            (Eldbus_Connection *)eina_hash_find(address_connections, address);
    }
    ...
    conn = _connection_get(ELDBUS_CONNECTION_TYPE_ADDRESS, address, EFL_TRUE);
    EINA_SAFETY_ON_NULL_RETURN_VAL(conn, NULL);
...
eldbus_private_address_connection_get(const char *address)
{
    ...
    return _connection_get(ELDBUS_CONNECTION_TYPE_ADDRESS, address, EFL_FALSE);
}
...
EAPI void
eldbus_connection_free_cb_add(Eldbus_Connection *conn,
                              Eldbus_Free_Cb     cb,
                              const void        *data)
{
    ELDBUS_CONNECTION_CHECK(conn);
...
EAPI void
eldbus_connection_free_cb_del(Eldbus_Connection *conn,
                              Eldbus_Free_Cb     cb,
                              const void        *data)
{
    ELDBUS_CONNECTION_CHECK(conn);
...
EAPI void
eldbus_connection_data_set(Eldbus_Connection *conn,
                           const char        *key,
                           const void        *data)
{
    ELDBUS_CONNECTION_CHECK(conn);
...
static void
eldbus_dispatch_name_owner_change(Eldbus_Connection_Name *cn,
                                  const char             *old_id)
{
    Eldbus_Connection_Context_NOC_Cb *ctx;
...
EAPI void
eldbus_name_owner_changed_callback_add(Eldbus_Connection           *conn,
                                       const char                  *bus,
                                       Eldbus_Name_Owner_Changed_Cb cb,
                                       const void                  *cb_data,
...
EAPI void
eldbus_name_owner_changed_callback_del(Eldbus_Connection           *conn,
                                       const char                  *bus,
                                       Eldbus_Name_Owner_Changed_Cb cb,
                                       const void                  *cb_data)
{
...
EAPI void
eldbus_connection_event_callback_add(Eldbus_Connection           *conn,
                                     Eldbus_Connection_Event_Type type,
                                     Eldbus_Connection_Event_Cb   cb,
                                     const void                  *cb_data)
{
...
static void
_eldbus_connection_context_event_cb_del(Eldbus_Connection_Context_Event    *ce,
                                        Eldbus_Connection_Context_Event_Cb *ctx)
{
...
EAPI void
eldbus_connection_event_callback_del(Eldbus_Connection           *conn,
                                     Eldbus_Connection_Event_Type type,
                                     Eldbus_Connection_Event_Cb   cb,
                                     const void                  *cb_data)
{
...
void
eldbus_connection_event_callback_call(Eldbus_Connection           *conn,
                                      Eldbus_Connection_Event_Type type,
                                      const void                  *event_info)
{
    ELDBUS_CONNECTION_CHECK(conn);
...
void
eldbus_connection_signal_handler_add(Eldbus_Connection     *conn,
...
void
eldbus_connection_signal_handler_del(Eldbus_Connection     *conn,
                                     Eldbus_Signal_Handler *handler)
{
    EINA_SAFETY_ON_NULL_RETURN(conn);
    EINA_SAFETY_ON_NULL_RETURN(handler);
