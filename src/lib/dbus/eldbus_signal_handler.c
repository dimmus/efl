...
eldbus_signal_handler_match_extra_vset(Eldbus_Signal_Handler *sh, va_list ap)
{
    ...
    ELDBUS_SIGNAL_HANDLER_CHECK_RETVAL(sh, EFL_FALSE);
...
eldbus_signal_handler_add(Eldbus_Connection *conn,
                          const char        *sender,
                          const char        *path,
                          ...
                          const void        *cb_data)
{
    ...
    sh = _eldbus_signal_handler_add(conn,
                                    sender,
                                    path,
                                    interface,
                                    member,
...
_eldbus_signal_handler_add(Eldbus_Connection *conn,
                           const char        *sender,
                           const char        *path,
                           const char        *interface,
                           const char        *member,
...
_eldbus_signal_handler_del(Eldbus_Signal_Handler *handler)
{
   ...
   /* after cbs_free dispatch these shouldn't exit, error if they do */
    ...
    if (handler->bus) eldbus_connection_name_unref(handler->conn, handler->bus);
    free(handler);
}
