...
static void _eldbus_proxy_event_callback_call(Eldbus_Proxy           *proxy,
                                              Eldbus_Proxy_Event_Type type,
                                              const void *event_info);
static void
            _eldbus_proxy_context_event_cb_del(Eldbus_Proxy_Context_Event    *ce,
...
_eldbus_proxy_call_del(Eldbus_Proxy *proxy)
{
    ...
    */
    ...
    while (ce->list)
    {
        ...
        ctx =
            EINA_INLIST_CONTAINER_GET(ce->list, Eldbus_Proxy_Context_Event_Cb);
...
_eldbus_proxy_free(Eldbus_Proxy *proxy)
{
    ...
    {
        if (h->dangling)
            eldbus_signal_handler_free_cb_del(h,
...
eldbus_proxy_get(Eldbus_Object *obj, const char *interface)
{
    ...
    if (proxy) return eldbus_proxy_ref(proxy);
    ...
    if (!eldbus_object_proxy_add(obj, proxy)) goto cleanup;
...
EAPI void
eldbus_proxy_free_cb_add(Eldbus_Proxy  *proxy,
                         Eldbus_Free_Cb cb,
                         const void    *data)
{
    ELDBUS_PROXY_CHECK(proxy);
...
EAPI void
eldbus_proxy_free_cb_del(Eldbus_Proxy  *proxy,
                         Eldbus_Free_Cb cb,
                         const void    *data)
{
    ELDBUS_PROXY_CHECK(proxy);
...
_iter_hash_value_set(Eina_Hash           *props,
                     const char          *key,
                     Eldbus_Message_Iter *var) EINA_ARG_NONNULL(1, 2, 3)
{
...
_properties_changed(void *data, const Eldbus_Message *msg)
{
    ...
    if (proxy->props)
        eldbus_message_iter_dict_iterate(array,
                                         "sv",
...
EAPI void
eldbus_proxy_event_callback_add(Eldbus_Proxy           *proxy,
                                Eldbus_Proxy_Event_Type type,
                                Eldbus_Proxy_Event_Cb   cb,
                                const void             *cb_data)
{
...
EAPI void
eldbus_proxy_event_callback_del(Eldbus_Proxy           *proxy,
                                Eldbus_Proxy_Event_Type type,
                                Eldbus_Proxy_Event_Cb   cb,
                                const void             *cb_data)
{
    ...
    if (type == ELDBUS_PROXY_EVENT_PROPERTY_CHANGED)
    {
        ...
        ce_prop_remove =
            proxy->event_handlers + ELDBUS_PROXY_EVENT_PROPERTY_REMOVED;
    ...
    else if (type == ELDBUS_PROXY_EVENT_PROPERTY_REMOVED)
    {
        ...
        ce_prop_changed =
            proxy->event_handlers + ELDBUS_PROXY_EVENT_PROPERTY_CHANGED;
...
static void
_eldbus_proxy_event_callback_call(Eldbus_Proxy           *proxy,
                                  Eldbus_Proxy_Event_Type type,
                                  const void             *event_info)
{
    Eldbus_Proxy_Context_Event    *ce;
...
_eldbus_proxy_send(Eldbus_Proxy     *proxy,
                   Eldbus_Message   *msg,
                   ...
                   double            timeout)
{
    ...
    pending = _eldbus_connection_send(proxy->obj->conn,
                                      msg,
...
_eldbus_proxy_send_and_block(Eldbus_Proxy   *proxy,
                             Eldbus_Message *msg,
                             double          timeout)
{
...
EAPI Eldbus_Pending *
eldbus_proxy_send(Eldbus_Proxy     *proxy,
                  Eldbus_Message   *msg,
                  Eldbus_Message_Cb cb,
                  const void       *cb_data,
                  double            timeout)
...
EAPI Eldbus_Message *
eldbus_proxy_send_and_block(Eldbus_Proxy   *proxy,
                            Eldbus_Message *msg,
                            double          timeout)
{
    ELDBUS_PROXY_CHECK_RETVAL(proxy, NULL);
...
eldbus_proxy_method_call_new(Eldbus_Proxy *proxy, const char *member)
{
    ...
    msg = eldbus_message_method_call_new(eldbus_object_bus_name_get(proxy->obj),
...
static Eldbus_Pending *
_eldbus_proxy_vcall(Eldbus_Proxy     *proxy,
                    const char       *member,
                    Eldbus_Message_Cb cb,
                    const void       *cb_data,
                    double            timeout,
...
EAPI Eldbus_Pending *
eldbus_proxy_call(Eldbus_Proxy     *proxy,
                  const char       *member,
                  Eldbus_Message_Cb cb,
                  const void       *cb_data,
                  double            timeout,
                  ...
                  ...)
{
    ...
    pending =
        _eldbus_proxy_vcall(proxy, member, cb, cb_data, timeout, signature, ap);
    va_end(ap);
...
EAPI Eldbus_Pending *
eldbus_proxy_vcall(Eldbus_Proxy     *proxy,
                   const char       *member,
                   Eldbus_Message_Cb cb,
                   const void       *cb_data,
                   double            timeout,
                   ...
                   va_list           ap)
{
    ...
    return _eldbus_proxy_vcall(proxy,
                               member,
                               cb,
                               cb_data,
                               timeout,
...
eldbus_proxy_signal_handler_add(Eldbus_Proxy    *proxy,
                                const char      *member,
                                Eldbus_Signal_Cb cb,
                                const void      *cb_data)
{
    ...
    handler = _eldbus_signal_handler_add(proxy->obj->conn,
                                         name,
                                         path,
                                         proxy->interface,
                                         member,
...
EAPI Eldbus_Pending *
eldbus_proxy_property_get(Eldbus_Proxy     *proxy,
                          const char       *name,
                          Eldbus_Message_Cb cb,
                          const void       *data)
{
    ...
    return eldbus_proxy_call(proxy->obj->properties,
                             "Get",
                             cb,
                             data,
                             -1,
...
EAPI Eldbus_Pending *
eldbus_proxy_property_set(Eldbus_Proxy     *proxy,
                          const char       *name,
                          const char       *sig,
                          const void       *value,
                          Eldbus_Message_Cb cb,
...
EAPI Eldbus_Pending *
eldbus_proxy_property_value_set(Eldbus_Proxy     *proxy,
                                const char       *name,
                                const char       *sig,
...
EAPI Eldbus_Pending *
eldbus_proxy_property_get_all(Eldbus_Proxy     *proxy,
                              Eldbus_Message_Cb cb,
                              const void       *data)
{
    ELDBUS_PROXY_CHECK_RETVAL(proxy, NULL);
...
EAPI Eldbus_Signal_Handler *
eldbus_proxy_properties_changed_callback_add(Eldbus_Proxy    *proxy,
                                             Eldbus_Signal_Cb cb,
                                             const void      *data)
{
...
_props_get_all(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending)
{
    ...
    eldbus_proxy_event_callback_del(proxy,
...
EAPI Efl_Bool
eldbus_proxy_properties_monitor(Eldbus_Proxy *proxy, Efl_Bool enable)
{
    Eldbus_Pending *pending;
    ELDBUS_PROXY_CHECK_RETVAL(proxy, EFL_FALSE);
