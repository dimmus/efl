...
static void _eldbus_model_proxy_property_get_all_cb(void *,
                                                    const Eldbus_Message *,
                                                    Eldbus_Pending *);
static void _eldbus_model_proxy_property_set_cb(void *,
                                                const Eldbus_Message *,
                                                Eldbus_Pending *);
static void _eldbus_model_proxy_property_set_load_cb(void *,
                                                     const Eldbus_Message *,
                                                     Eldbus_Pending *);
static void _eldbus_model_proxy_start_monitor(Eldbus_Model_Proxy_Data *);
static void
_eldbus_model_proxy_property_changed_cb(void *, Eldbus_Proxy *, void *);
static void
_eldbus_model_proxy_property_invalidated_cb(void *, Eldbus_Proxy *, void *);
static const char *
_eldbus_model_proxy_property_type_get(Eldbus_Model_Proxy_Data *, const char *);
static void
_eldbus_model_proxy_create_methods_children(Eldbus_Model_Proxy_Data *);
static void
_eldbus_model_proxy_create_signals_children(Eldbus_Model_Proxy_Data *);

typedef struct _Eldbus_Model_Proxy_Property_Set_Data
...
static void
_eldbus_model_proxy_object_del(void                 *data,
                               Eldbus_Object *object EFL_UNUSED,
...
_eldbus_model_proxy_efl_object_finalize(Eo *obj, Eldbus_Model_Proxy_Data *pd)
{
    ...
    if (!eldbus_model_connection_get(obj))
        eldbus_model_connection_set(obj,
                                    eldbus_object_connection_get(pd->object));

    eldbus_object_event_callback_add(pd->object,
                                     ELDBUS_OBJECT_EVENT_DEL,
                                     _eldbus_model_proxy_object_del,
                                     pd);
...
static void
_eldbus_model_proxy_object_set(Eo *obj                  EFL_UNUSED,
                               Eldbus_Model_Proxy_Data *pd,
...
static void
_eldbus_model_proxy_interface_set(
    Eo *obj                               EFL_UNUSED,
    Eldbus_Model_Proxy_Data              *pd,
...
_eldbus_model_proxy_efl_object_invalidate(Eo *obj, Eldbus_Model_Proxy_Data *pd)
{
    ...
    if (pd->object)
    {
        eldbus_object_event_callback_del(pd->object,
                                         ELDBUS_OBJECT_EVENT_DEL,
                                         _eldbus_model_proxy_object_del,
                                         pd);
...
_eldbus_model_proxy_efl_model_properties_get(const Eo *obj EFL_UNUSED,
                                             Eldbus_Model_Proxy_Data *pd)
{
...
static void
_eldbus_model_proxy_cancel_cb(Efl_Loop_Consumer *consumer    EFL_UNUSED,
                              void                          *data,
...
static Eina_Future *
_eldbus_model_proxy_efl_model_property_set(Eo *obj                  EFL_UNUSED,
                                           Eldbus_Model_Proxy_Data *pd,
                                           const char              *property,
                                           Eina_Value              *value)
{
    ...
    if (!pd->is_loaded)
    {
        pending = _eldbus_model_proxy_load_all(
            pd,
            data->promise,
            property,
            _eldbus_model_proxy_property_set_load_cb,
            data);
    }
    else
    {
        pending =
            eldbus_proxy_property_value_set(pd->proxy,
                                            property,
                                            signature,
                                            (Eina_Value *)value,
                                            _eldbus_model_proxy_property_set_cb,
                                            data);
    }
    ...
    return efl_future_then(obj,
                           ...
                           .data = data,
                           .free = _eldbus_model_proxy_cancel_cb);
...
static Eina_Value *
_eldbus_model_proxy_efl_model_property_get(const Eo *obj            EFL_UNUSED,
                                           Eldbus_Model_Proxy_Data *pd,
                                           const char              *property)
{
    Eldbus_Pending *pending;
    ...
    pending =
        _eldbus_model_proxy_load_all(pd,
                                     NULL,
                                     property,
                                     _eldbus_model_proxy_property_get_all_cb,
                                     pd);
    if (pending) pd->pendings = eina_list_append(pd->pendings, pending);
    else goto on_error;
...
static Eina_Future *
_eldbus_model_proxy_efl_model_children_slice_get(Eo *obj EFL_UNUSED,
                                                 Eldbus_Model_Proxy_Data *pd,
                                                 unsigned                 start,
                                                 unsigned                 count)
{
...
static unsigned int
_eldbus_model_proxy_efl_model_children_count_get(const Eo *obj EFL_UNUSED,
                                                 Eldbus_Model_Proxy_Data *pd)
{
    _eldbus_model_proxy_listed(pd);
...
_eldbus_model_proxy_create_methods_children(Eldbus_Model_Proxy_Data *pd)
{
    ...
    EINA_LIST_FOREACH(pd->interface->methods, it, method)
    {
        ...
        child = efl_add_ref(ELDBUS_MODEL_METHOD_CLASS,
                            pd->obj,
...
static const char *
_eldbus_model_proxy_proxy_name_get(const Eo *obj            EFL_UNUSED,
                                   Eldbus_Model_Proxy_Data *pd)
{
    return pd->name;
}
                                            ...
                                            void               *event_info)
{
    ...
    Eldbus_Proxy_Event_Property_Changed *event =
        (Eldbus_Proxy_Event_Property_Changed *)event_info;

    efl_model_property_invalidated_notify(pd->obj, event->name);
}

static Eina_Array *
_eldbus_model_proxy_property_get_all_load(const Eldbus_Message    *msg,
                                          Eldbus_Model_Proxy_Data *pd)
{
    ...
    while (eldbus_message_iter_get_and_next(values, 'e', &entry))
    {
        ...
        if (!eldbus_message_iter_arguments_get(entry,
                                               "sv",
                                               &property,
                                               &variant))
            continue;
...
static void
_eldbus_model_proxy_promise_clean(Eldbus_Property_Promise *p, Eina_Error err)
{
...
_eldbus_model_proxy_property_set_load_cb(void                 *data,
                                         ...
                                         Eldbus_Pending       *pending)
{
    Eldbus_Model_Proxy_Property_Set_Data *set_data =
...
_eldbus_model_proxy_property_set_cb(void                 *data,
                                    ...
                                    Eldbus_Pending       *pending)
{
    Eldbus_Model_Proxy_Property_Set_Data *sd =
        (Eldbus_Model_Proxy_Property_Set_Data *)data;
