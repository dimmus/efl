   ...
   if (pd->type == EFL_DBUS_CONNECTION_TYPE_ADDRESS)
     {
        if (pd->private)
          ...
          pd->connection = efl_dbus_private_address_connection_get(pd->address);
     }
   else
     {
        if (pd->private)
          pd->connection = efl_dbus_private_connection_get(pd->type);
        else
          pd->connection = efl_dbus_connection_get(pd->type);
     }
...
_efl_dbus_model_connection_get(const Eo *obj EFL_UNUSED, Efl_Dbus_Model_Data *pd)
{
