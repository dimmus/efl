...
static void _efl_dbus_proxy_event_callback_call(Efl_Dbus_Proxy *proxy, Efl_Dbus_Proxy_Event_Type type, const void *event_info);
static void _efl_dbus_proxy_context_event_cb_del(Efl_Dbus_Proxy_Context_Event *ce, Efl_Dbus_Proxy_Context_Event_Cb *ctx);
   ...
   EINA_LIST_FOREACH_SAFE(proxy->handlers, iter, iter_next, h)
     {
        DBG("proxy=%p delete owned signal handler %p %s",
            proxy, h, efl_dbus_signal_handler_match_get(h));
...
_efl_dbus_proxy_free(Efl_Dbus_Proxy *proxy)
{
   ...
   EINA_LIST_FREE(proxy->handlers, h)
     {
        if (h->dangling)
	  efl_dbus_signal_handler_free_cb_del(h, _on_signal_handler_free, proxy);
        else
...
efl_dbus_proxy_get(Efl_Dbus_Object *obj, const char *interface)
{
   ...
   if (proxy)
     return efl_dbus_proxy_ref(proxy);
   ...
   if (!efl_dbus_object_proxy_add(obj, proxy))
     goto cleanup;
...
EAPI void
efl_dbus_proxy_free_cb_add(Efl_Dbus_Proxy *proxy, Efl_Dbus_Free_Cb cb, const void *data)
{
   EFL_DBUS_PROXY_CHECK(proxy);
...
EAPI void
efl_dbus_proxy_free_cb_del(Efl_Dbus_Proxy *proxy, Efl_Dbus_Free_Cb cb, const void *data)
{
   EFL_DBUS_PROXY_CHECK(proxy);
   EINA_SAFETY_ON_NULL_RETURN(cb);
...
EAPI void
efl_dbus_proxy_data_set(Efl_Dbus_Proxy *proxy, const char *key, const void *data)
{
   EFL_DBUS_PROXY_CHECK(proxy);
...
static Eina_Value *
_iter_hash_value_set(Eina_Hash *props, const char *key, Efl_Dbus_Message_Iter *var) EINA_ARG_NONNULL(1, 2, 3)
{
...
_property_changed_iter(void *data, const void *key, Efl_Dbus_Message_Iter *var)
{
   ...
   Efl_Dbus_Proxy_Event_Property_Changed event = {.name = skey,
                                                .value = value,
                                                .proxy = proxy};
   _efl_dbus_proxy_event_callback_call(proxy, EFL_DBUS_PROXY_EVENT_PROPERTY_CHANGED,
                                     &event);
}
...
_properties_changed(void *data, const Efl_Dbus_Message *msg)
{
   ...
   if (!efl_dbus_message_arguments_get(msg, "sa{sv}as", &iface, &array, &invalidate))
     {
        ERR("Error getting data from properties changed signal.");
        return;
     }
   if (proxy->props)
     efl_dbus_message_iter_dict_iterate(array, "sv", _property_changed_iter,
                                     proxy);

   while (efl_dbus_message_iter_get_and_next(invalidate, 's', &invalidate_prop))
     {
        ...
        _efl_dbus_proxy_event_callback_call(proxy, EFL_DBUS_PROXY_EVENT_PROPERTY_REMOVED,
                                         &event);
     }
}
...
EAPI void
efl_dbus_proxy_event_callback_add(Efl_Dbus_Proxy *proxy, Efl_Dbus_Proxy_Event_Type type, Efl_Dbus_Proxy_Event_Cb cb, const void *cb_data)
{
   Efl_Dbus_Proxy_Context_Event *ce;
   Efl_Dbus_Proxy_Context_Event_Cb *ctx;
...
static void
_efl_dbus_proxy_context_event_cb_del(Efl_Dbus_Proxy_Context_Event *ce, Efl_Dbus_Proxy_Context_Event_Cb *ctx)
{
...
EAPI void
efl_dbus_proxy_event_callback_del(Efl_Dbus_Proxy *proxy, Efl_Dbus_Proxy_Event_Type type, Efl_Dbus_Proxy_Event_Cb cb, const void *cb_data)
{
   Efl_Dbus_Proxy_Context_Event *ce;
...
static void
_efl_dbus_proxy_event_callback_call(Efl_Dbus_Proxy *proxy, Efl_Dbus_Proxy_Event_Type type, const void *event_info)
{
   Efl_Dbus_Proxy_Context_Event *ce;
...
static void
_on_proxy_message_cb(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending)
{
...
_efl_dbus_proxy_send(Efl_Dbus_Proxy *proxy, Efl_Dbus_Message *msg, Efl_Dbus_Message_Cb cb, const void *cb_data, double timeout)
{
   ...
   pending = _efl_dbus_connection_send(proxy->obj->conn, msg,
...
_efl_dbus_proxy_send_and_block(Efl_Dbus_Proxy *proxy, Efl_Dbus_Message *msg, double timeout)
{
   return _efl_dbus_connection_send_and_block(proxy->obj->conn, msg, timeout);
}

EAPI Efl_Dbus_Pending *
efl_dbus_proxy_send(Efl_Dbus_Proxy *proxy, Efl_Dbus_Message *msg, Efl_Dbus_Message_Cb cb, const void *cb_data, double timeout)
{
   EFL_DBUS_PROXY_CHECK_RETVAL(proxy, NULL);
...
efl_dbus_proxy_send_and_block(Efl_Dbus_Proxy *proxy, Efl_Dbus_Message *msg, double timeout)
{
   EFL_DBUS_PROXY_CHECK_RETVAL(proxy, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(msg, NULL);
...
static Efl_Dbus_Pending *
_efl_dbus_proxy_vcall(Efl_Dbus_Proxy *proxy, const char *member, Efl_Dbus_Message_Cb cb, const void *cb_data, double timeout, const char *signature, va_list ap)
{
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_call(Efl_Dbus_Proxy *proxy, const char *member, Efl_Dbus_Message_Cb cb, const void *cb_data, double timeout, const char *signature, ...)
{
   Efl_Dbus_Pending *pending;
   va_list ap;
   ...
   pending = _efl_dbus_proxy_vcall(proxy, member, cb, cb_data, timeout,
                                signature, ap);
   va_end(ap);
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_vcall(Efl_Dbus_Proxy *proxy, const char *member, Efl_Dbus_Message_Cb cb, const void *cb_data, double timeout, const char *signature, va_list ap)
{
   EFL_DBUS_PROXY_CHECK_RETVAL(proxy, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(member, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(signature, NULL);

   return _efl_dbus_proxy_vcall(proxy, member, cb, cb_data, timeout,
                             signature, ap);
}
...
efl_dbus_proxy_signal_handler_add(Efl_Dbus_Proxy *proxy, const char *member, Efl_Dbus_Signal_Cb cb, const void *cb_data)
{
   ...
   handler = _efl_dbus_signal_handler_add(proxy->obj->conn, name, path,
   ...
   efl_dbus_signal_handler_free_cb_add(handler, _on_signal_handler_free, proxy);
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_property_get(Efl_Dbus_Proxy *proxy, const char *name, Efl_Dbus_Message_Cb cb, const void *data)
{
   EFL_DBUS_PROXY_CHECK_RETVAL(proxy, NULL);
   EINA_SAFETY_ON_NULL_RETURN_VAL(name, NULL);
   return efl_dbus_proxy_call(proxy->obj->properties, "Get", cb, data, -1,
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_property_set(Efl_Dbus_Proxy *proxy, const char *name, const char *sig, const void *value, Efl_Dbus_Message_Cb cb, const void *data)
{
   Efl_Dbus_Message *msg;
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_property_value_set(Efl_Dbus_Proxy *proxy, const char *name, const char *sig, const Eina_Value *value, Efl_Dbus_Message_Cb cb, const void *data)
{
   Efl_Dbus_Message *msg;
   Efl_Dbus_Message_Iter *iter, *variant;
...
EAPI Efl_Dbus_Pending *
efl_dbus_proxy_property_get_all(Efl_Dbus_Proxy *proxy, Efl_Dbus_Message_Cb cb, const void *data)
{
   EFL_DBUS_PROXY_CHECK_RETVAL(proxy, NULL);
   return efl_dbus_proxy_call(proxy->obj->properties, "GetAll", cb, data, -1,
                            "s", proxy->interface);
}

EAPI Efl_Dbus_Signal_Handler *
efl_dbus_proxy_properties_changed_callback_add(Efl_Dbus_Proxy *proxy, Efl_Dbus_Signal_Cb cb, const void *data)
{
...
static void
_on_monitored_proxy_del(void *data, Efl_Dbus_Proxy *proxy EFL_UNUSED, void *event_info EFL_UNUSED)
{
...
static void
_props_get_all(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending)
{
   Efl_Dbus_Proxy *proxy = data;
   ...
   efl_dbus_proxy_event_callback_del(proxy, EFL_DBUS_PROXY_EVENT_DEL,
   ...
   event.proxy = proxy;
   _efl_dbus_proxy_event_callback_call(proxy, EFL_DBUS_PROXY_EVENT_PROPERTY_LOADED,
                                     &event);
}
...
efl_dbus_proxy_properties_monitor(Efl_Dbus_Proxy *proxy, Efl_Bool enable)
{
   ...
   if (!enable)
     {
        ...
        ce_prop_changed = proxy->event_handlers + EFL_DBUS_PROXY_EVENT_PROPERTY_CHANGED;
        ce_prop_removed = proxy->event_handlers + EFL_DBUS_PROXY_EVENT_PROPERTY_REMOVED;
   ...
   efl_dbus_proxy_event_callback_add(proxy, EFL_DBUS_PROXY_EVENT_DEL,
                                   _on_monitored_proxy_del, pending);
