...
_efl_dbus_proxy_call_del(Efl_Dbus_Proxy *proxy)
{
  ...
  while (ce->list)
  {
    ...
    ctx = EINA_INLIST_CONTAINER_GET(ce->list, Efl_Dbus_Proxy_Context_Event_Cb);
...
_efl_dbus_proxy_free(Efl_Dbus_Proxy *proxy)
{
  ...
  if (proxy->pendings) CRI("Proxy %p released with live pending calls!", proxy);
  ...
  for (i = 0; i < EFL_DBUS_PROXY_EVENT_LAST; i++)
  {
    ...
    while (ce->list)
    {
      Efl_Dbus_Proxy_Context_Event_Cb *ctx;
      ctx =
        EINA_INLIST_CONTAINER_GET(ce->list, Efl_Dbus_Proxy_Context_Event_Cb);
      _efl_dbus_proxy_context_event_cb_del(ce, ctx);
    }
...
efl_dbus_proxy_method_call_new(Efl_Dbus_Proxy *proxy, const char *member)
{
  ...
  msg =
    efl_dbus_message_method_call_new(efl_dbus_object_bus_name_get(proxy->obj),
                    ...
                    ...)
{
  ...
  pending =
    _efl_dbus_proxy_vcall(proxy, member, cb, cb_data, timeout, signature, ap);
  va_end(ap);
                                  ...
                                  const void         *data)
{
  ...
  {
    if (!_message_iter_from_eina_value_struct(sig, variant, value)) goto error;
  }
  efl_dbus_message_iter_container_close(iter, variant);
