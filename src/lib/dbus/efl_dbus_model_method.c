...
static void _efl_dbus_model_method_call_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);

static Efl_Object*
_efl_dbus_model_method_efl_object_constructor(Eo *obj, Efl_Dbus_Model_Method_Data *pd)
{
...
static Efl_Object*
_efl_dbus_model_method_efl_object_finalize(Eo *obj, Efl_Dbus_Model_Method_Data *pd)
{
   if (!pd->proxy ||
       !pd->method)
     return NULL;
...
static void
_efl_dbus_model_method_call(Eo *obj EFL_UNUSED, Efl_Dbus_Model_Method_Data *pd EFL_UNUSED)
{
   Efl_Dbus_Model_Arguments_Data *data = efl_data_scope_get(obj, EFL_DBUS_MODEL_ARGUMENTS_CLASS);
   Efl_Dbus_Message *msg = efl_dbus_proxy_method_call_new(data->proxy, data->name);
   ...
   pending = efl_dbus_proxy_send(data->proxy, msg, _efl_dbus_model_method_call_cb, pd, -1);
...
static void
_efl_dbus_model_method_call_cb(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending)
{
   ...
   if (efl_dbus_model_arguments_process_arguments(args_data, msg, pending))
     efl_event_callback_call(pd->obj, EFL_DBUS_MODEL_METHOD_EVENT_SUCCESSFUL_CALL, NULL);
}
