...
EINA_API Eina_File *
eina_file_virtualize(const char *virtual_name, const void *data, unsigned long long length, Efl_Bool copy)
{
   Eina_File *file;
   Eina_Nano_Time tp;
   long int ti;
        ...
        if (file->virtual && !file->copied)
          {
             ...
             r = eina_file_virtualize(file->filename, file->global_map, file->length, EFL_TRUE);
             eina_lock_release(&file->lock);
...
static Efl_Bool
_eina_file_copy_splice_internal(int s, int d, off_t total, Eina_File_Copy_Progress cb, const void *cb_data, Efl_Bool *splice_unsupported)
{
#ifdef HAVE_SPLICE
