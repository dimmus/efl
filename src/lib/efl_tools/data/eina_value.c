   ...
   if (desc_array)
     {
        ...
        EINA_SAFETY_ON_FALSE_RETURN_VAL
          (desc_array->member_size == desc->subtype->value_size, EFL_FALSE);
...
static void
_eina_value_type_struct_convert_to_string_member(const Eina_Value_Struct *st, const Eina_Value_Struct_Member *member, Efl_Strbuf *str)
{
...
_eina_value_type_struct_desc_check(const Eina_Value_Struct_Desc *desc)
{
   ...
   EINA_SAFETY_ON_FALSE_RETURN_VAL
     (desc->version == EINA_VALUE_STRUCT_DESC_VERSION, EFL_FALSE);
        ...
        for (; itr < itr_end; itr++)
          {
             ...
             EINA_SAFETY_ON_FALSE_RETURN_VAL
               (eina_value_type_check(itr->type), EFL_FALSE);
             EINA_SAFETY_ON_FALSE_RETURN_VAL
               (itr->type->value_size > 0, EFL_FALSE);
...
eina_value_init(void)
{
   ...
   EINA_SAFETY_ON_FALSE_RETURN_VAL((sizeof(_EINA_VALUE_TYPE_BASICS)/sizeof(_EINA_VALUE_TYPE_BASICS[0])) == 16, EFL_FALSE);
