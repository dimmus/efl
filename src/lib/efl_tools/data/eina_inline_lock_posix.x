...
struct _Eina_Lock
{
   ...
   Efl_Bool         locked : 1;  /**< Indicates locked or not locked */
   Efl_Bool         recursive : 1;  /**< Indicates recursive lock */
#endif
};
...
EINA_API extern Efl_Bool _eina_threads_activated;
...
EINA_API Efl_Bool _eina_lock_new(Eina_Lock *mutex, Efl_Bool recursive);
EINA_API void      _eina_lock_free(Eina_Lock *mutex);
EINA_API Efl_Bool _eina_condition_new(Eina_Condition *cond, Eina_Lock *mutex);
EINA_API void      _eina_condition_free(Eina_Condition *cond);
EINA_API Efl_Bool _eina_rwlock_new(Eina_RWLock *mutex);
EINA_API void      _eina_rwlock_free(Eina_RWLock *mutex);
EINA_API Efl_Bool _eina_spinlock_new(Eina_Spinlock *spinlock);
EINA_API void      _eina_spinlock_free(Eina_Spinlock *spinlock);
EINA_API Efl_Bool _eina_semaphore_new(Eina_Semaphore *sem, int count_init);
EINA_API Efl_Bool _eina_semaphore_free(Eina_Semaphore *sem);
...
{
   Efl_Bool ret = _eina_lock_new(mutex, EFL_FALSE);
#ifdef EINA_HAVE_DEBUG_THREADS
   mutex->recursive = EFL_FALSE;
...
{
   Efl_Bool ret = _eina_lock_new(mutex, EFL_TRUE);
#ifdef EINA_HAVE_DEBUG_THREADS
   mutex->recursive = EFL_TRUE;
   mutex->lock_thread_id = 0;
   mutex->lock_bt_num = 0;
   mutex->locked = 0;
#endif
   ...
   if (t >= 0.0)
     {
        ...
        if (cond->clkid)
          {
             if (clock_gettime(cond->clkid, &ts) != 0) return EFL_FALSE;
          }
        else
#endif
          {
             ...
             struct timeval tv;
             if (gettimeofday(&tv, NULL) != 0) return EFL_FALSE;
             ts.tv_sec = tv.tv_sec;
...
{
   if (pthread_key_create(key, delete_cb) == 0) return EFL_TRUE;
   return EFL_FALSE;
}
...
{
   if (pthread_setspecific(key, data) == 0) return EFL_TRUE;
   return EFL_FALSE;
}
...
eina_barrier_wait(Eina_Barrier *barrier)
{
   ...
   if (ok == 0) return EFL_TRUE;
   else if (ok == PTHREAD_BARRIER_SERIAL_THREAD) return EFL_TRUE;
...
EINA_API Efl_Bool _eina_barrier_new(Eina_Barrier *barrier, int needed);
...
eina_semaphore_release(Eina_Semaphore *sem, int count_release EINA_UNUSED)
{
   if (sem)
#if defined(EINA_HAVE_OSX_SEMAPHORE)
     return (semaphore_signal(*sem) == KERN_SUCCESS) ? EFL_TRUE : EFL_FALSE;
#else
     return (sem_post(sem) == 0) ? EFL_TRUE : EFL_FALSE;
#endif
   return EFL_FALSE;
}
