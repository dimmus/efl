...
_ecore_wl_window_shell_surface_init(Ecore_Wl_Window *window)
{
    ...
    if (window->display->wl.session_recovery)
    {
        if (window->uuid)
        {
            ...
            zwp_e_session_recovery_set_uuid(
                window->display->wl.session_recovery,
                window->surface,
                window->uuid);
...
ecore_wl_window_free(Ecore_Wl_Window *window)
{
    ...
    if (window->uuid && window->surface && window->display->wl.session_recovery)
        zwp_e_session_recovery_destroy_uuid(
            window->display->wl.session_recovery,
            window->surface,
            window->uuid);
                                              ...
                                              Efl_Bool          resize)
{
    ...
    ecore_event_add(ECORE_WL2_EVENT_WINDOW_ROTATION_CHANGE_PREPARE,
                    ev,
                    NULL,
                    NULL);
}
...
ecore_wl_window_frame_callback_del(Ecore_Wl_Frame_Cb_Handle *handle)
{
    ...
    handle->win->frame_callbacks =
        eina_inlist_remove(handle->win->frame_callbacks,
                           EINA_INLIST_GET(handle));
    free(handle);
}
