...
static const char EFL_MODEL_ERROR_NOT_SUPPORTED_STR[] =
...
static const char EFL_FACTORY_ERROR_NOT_SUPPORTED_STR[] =
    "Operation not supported";

static const char EFL_PROPERTY_ERROR_INVALID_KEY_STR[] =
...
#undef _ERROR
#define _ERROR(Name)           \
    EFL_FACTORY_ERROR_##Name = \
        eina_error_msg_static_register(EFL_FACTORY_ERROR_##Name##_STR);
    _ERROR(NOT_SUPPORTED);

#undef _ERROR
#define _ERROR(Name)            \
    EFL_PROPERTY_ERROR_##Name = \
        eina_error_msg_static_register(EFL_PROPERTY_ERROR_##Name##_STR);
    _ERROR(INVALID_KEY);
...
_efl_model_properties_changed_internal(const Efl_Model *model, ...)
{
    ...
    efl_event_callback_call((Efl_Model *)model,
...
efl_model_property_invalidated_notify(Efl_Model *model, const char *property)
{
    ...
    Efl_Bool ret = eina_array_push(invalidated_properties, sp);
...
efl_model_value_struct_description_new(
    unsigned int                           member_count,
    Efl_Model_Value_Struct_Member_Setup_Cb setup_cb,
    void                                  *data)
{
    ...
    desc = malloc(sizeof(Efl_Model_Value_Struct_Desc) +
                  member_count * sizeof(Eina_Value_Struct_Member));
    ...
    for (i = 0; i < member_count; ++i)
    {
        Eina_Value_Struct_Member *m =
            (Eina_Value_Struct_Member *)desc->members + i;
        unsigned int size;
        ...
        if (size % sizeof(void *) != 0) size += size - (size % sizeof(void *));
