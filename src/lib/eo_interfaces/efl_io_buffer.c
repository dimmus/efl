...
_efl_io_buffer_realloc(Eo *o, Efl_Io_Buffer_Data *pd, size_t size)
{
  ...
  EINA_SAFETY_ON_TRUE_RETURN_VAL(pd->readonly, EFL_FALSE);

  if ((limit > 0) && (size > limit)) size = limit;

  if (pd->allocated == size) return EFL_FALSE;
  ...
  else
  {
    ...
    EINA_SAFETY_ON_NULL_RETURN_VAL(tmp, EFL_FALSE);
  }
...
{
  if ((size > 0) && (size < 128)) size = ((size / 32) + 1) * 32;
  else if (size < 1024) size = ((size / 128) + 1) * 128;
  else if (size < 8192) size = ((size / 1024) + 1) * 1024;
  else size = ((size / 4096) + 1) * 4096;
...
_efl_io_buffer_preallocate(Eo *o, Efl_Io_Buffer_Data *pd, size_t size)
{
  ...
  if (pd->allocated < size) _efl_io_buffer_realloc_rounded(o, pd, size);
}
...
_efl_io_buffer_limit_set(Eo *o, Efl_Io_Buffer_Data *pd, size_t limit)
{
  ...
  efl_io_reader_can_read_set(o,
                             efl_io_buffer_position_read_get(o) <
                               efl_io_sizer_size_get(o));
  if (pd->closed) return;
  efl_io_writer_can_write_set(o,
                              (limit == 0) ||
                                (efl_io_buffer_position_write_get(o) < limit));
}

EOLIAN static size_t
_efl_io_buffer_limit_get(const Eo *o EFL_UNUSED, Efl_Io_Buffer_Data *pd)
{
  return pd->limit;
}
...
_efl_io_buffer_binbuf_steal(Eo *o, Efl_Io_Buffer_Data *pd)
{
  ...
  ret = eina_binbuf_manage_new(pd->bytes, efl_io_sizer_size_get(o), EFL_FALSE);
...
_efl_io_buffer_efl_object_finalize(Eo *o, Efl_Io_Buffer_Data *pd EFL_UNUSED)
{
  size_t limit;
...
_efl_io_buffer_efl_io_reader_read(Eo                 *o,
                                  Efl_Io_Buffer_Data *pd,
                                  Eina_Rw_Slice      *rw_slice)
{
  ...
  if (rw_slice->len > available)
  {
    ...
    if (rw_slice->len == 0) return EAGAIN;
  }
...
_efl_io_buffer_efl_io_reader_can_read_get(const Eo *o         EFL_UNUSED,
                                          Efl_Io_Buffer_Data *pd)
{
  return pd->can_read;
}
...
EOLIAN static void
_efl_io_buffer_efl_io_reader_can_read_set(Eo                 *o,
                                          Efl_Io_Buffer_Data *pd,
                                          Efl_Bool            can_read)
{
  EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
...
EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_reader_eos_get(const Eo              *o,
                                     Efl_Io_Buffer_Data *pd EFL_UNUSED)
{
...
EOLIAN static void
_efl_io_buffer_efl_io_reader_eos_set(Eo                    *o,
                                     Efl_Io_Buffer_Data *pd EFL_UNUSED,
                                     Efl_Bool               is_eos)
{
  EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
...
EOLIAN static Eina_Error
_efl_io_buffer_efl_io_writer_write(Eo                 *o,
                                   Efl_Io_Buffer_Data *pd,
                                   Eina_Slice         *slice,
                                   Eina_Slice         *remaining)
{
  ...
  err = ENOSPC;
  if (available >= slice->len) todo = slice->len;
  else if ((limit > 0) && (pd->allocated == limit)) goto error;
  else
  {
    _efl_io_buffer_realloc_rounded(o, pd, write_pos + slice->len);
    if (pd->allocated >= write_pos + slice->len) todo = slice->len;
    else todo = pd->allocated - write_pos;
  ...
  if (remaining)
  {
    ...
    if (remaining->len) remaining->mem = slice->bytes + todo;
    else remaining->mem = NULL;
  }
  slice->len = todo;
...
EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_writer_can_write_get(const Eo *o         EFL_UNUSED,
                                           Efl_Io_Buffer_Data *pd)
{
  return pd->can_write;
}
...
EOLIAN static void
_efl_io_buffer_efl_io_writer_can_write_set(Eo                 *o,
                                           Efl_Io_Buffer_Data *pd,
                                           Efl_Bool            can_write)
{
  EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
...
EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_closer_closed_get(const Eo *o         EFL_UNUSED,
                                        Efl_Io_Buffer_Data *pd)
{
  return pd->closed;
}
...
EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_closer_close_on_exec_set(Eo *o EFL_UNUSED,
                                               Efl_Io_Buffer_Data *pd
                                                        EFL_UNUSED,
                                               Efl_Bool close_on_exec)
{
...
EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_closer_close_on_exec_get(const Eo *o EFL_UNUSED,
                                               Efl_Io_Buffer_Data *pd
                                                 EFL_UNUSED)
{
  return EFL_TRUE;
...
EOLIAN static void
_efl_io_buffer_efl_io_closer_close_on_invalidate_set(
  Eo *o                        EFL_UNUSED,
  Efl_Io_Buffer_Data *pd       EFL_UNUSED,
  Efl_Bool close_on_invalidate EFL_UNUSED)
{}

EOLIAN static Efl_Bool
_efl_io_buffer_efl_io_closer_close_on_invalidate_get(const Eo *o EFL_UNUSED,
                                                     Efl_Io_Buffer_Data *pd
                                                       EFL_UNUSED)
{
  return EFL_TRUE;
...
{
  Eina_Error ret         = 0;
  Efl_Bool   reallocated = EFL_FALSE;
  ...
  if (old_size < size) memset(pd->bytes + old_size, 0, size - old_size);

end:
  pos_read = efl_io_buffer_position_read_get(o);
  if (pos_read > size) efl_io_buffer_position_read_set(o, size);
  else efl_io_reader_can_read_set(o, pos_read < size);

  if (pd->closed) return 0;

  pos_write = efl_io_buffer_position_write_get(o);
  if (pos_write > size) efl_io_buffer_position_write_set(o, size);
  else
  {
    size_t limit = efl_io_buffer_limit_get(o);
...
_efl_io_buffer_efl_io_sizer_size_get(const Eo *o         EFL_UNUSED,
                                     Efl_Io_Buffer_Data *pd)
{
  return pd->used;
}
...
EOLIAN static Eina_Error
_efl_io_buffer_efl_io_positioner_seek(Eo                      *o,
                                      Efl_Io_Buffer_Data *pd   EFL_UNUSED,
                                      int64_t                  offset,
                                      Efl_Io_Positioner_Whence whence)
{
  size_t size;
...
EOLIAN static uint64_t
_efl_io_buffer_efl_io_positioner_position_get(const Eo              *o,
                                              Efl_Io_Buffer_Data *pd EFL_UNUSED)
{
...
EOLIAN static Efl_Bool
_efl_io_buffer_position_read_set(Eo                 *o,
                                 Efl_Io_Buffer_Data *pd,
                                 uint64_t            position)
{
  size_t   size;
  ...
  size = efl_io_sizer_size_get(o);
  EINA_SAFETY_ON_TRUE_RETURN_VAL(position > size, EFL_FALSE);

  if (pd->position_read == position) return EFL_TRUE;
  ...
  if (pd->closed) return EFL_TRUE;
  efl_io_reader_eos_set(o, position >= size);
  return EFL_TRUE;
}

EOLIAN static uint64_t
_efl_io_buffer_position_read_get(const Eo *o EFL_UNUSED, Efl_Io_Buffer_Data *pd)
{
  return pd->position_read;
}

EOLIAN static Efl_Bool
_efl_io_buffer_position_write_set(Eo                 *o,
                                  Efl_Io_Buffer_Data *pd,
                                  uint64_t            position)
{
  size_t   size;
  ...
  size = efl_io_sizer_size_get(o);
  if (position < size) EINA_SAFETY_ON_TRUE_RETURN_VAL(pd->readonly, EFL_FALSE);
  EINA_SAFETY_ON_TRUE_RETURN_VAL(position > size, EFL_FALSE);

  if (pd->position_write == position) return EFL_TRUE;
...
EOLIAN static uint64_t
_efl_io_buffer_position_write_get(const Eo *o         EFL_UNUSED,
                                  Efl_Io_Buffer_Data *pd)
{
...
EOLIAN static void
_efl_io_buffer_adopt_readonly(Eo                 *o,
                              Efl_Io_Buffer_Data *pd,
                              const Eina_Slice    slice)
{
  Efl_Bool changed_size;

  EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));

  if (!pd->readonly) free(pd->bytes);
  pd->readonly  = EFL_TRUE;
...
EOLIAN static void
_efl_io_buffer_adopt_readwrite(Eo                 *o,
                               Efl_Io_Buffer_Data *pd,
                               Eina_Rw_Slice       slice)
{
  Efl_Bool changed_size;
  ...
  efl_io_writer_can_write_set(
    o,
    (pd->limit == 0) || (efl_io_buffer_position_write_get(o) < pd->limit));
  if (pd->closed) return;
