                          ...
                          double             pos_map)
{
  ...
  else
  {
    ...
    interv =
      interpolate(from_pd->current_ctrl.x, to_pd->current_ctrl.x, pos_map);
    pd->current_ctrl.x = interv;

    interv =
      interpolate(from_pd->current_ctrl.y, to_pd->current_ctrl.y, pos_map);
                      ...
                      unsigned int       pts_count)
{
  if (pd->reserved_cmd_cnt < cmd_count)
  {
    ...
    pd->commands         = realloc(pd->commands,
                              ...
                              double             y)
{
  ...
  if (!efl_gfx_path_grow(EFL_GFX_PATH_COMMAND_TYPE_CUBIC_TO, pd, &offset_point))
    return;
...
_efl_gfx_t_for_arc_angle(double angle)
{
  ...
  tc -=
    ((((2 - 3 * PATH_KAPPA) * tc + 3 * (PATH_KAPPA - 1)) * tc) * tc + 1 -
     cos_angle)  // value
  ...
  ts -=
    ((((3 * PATH_KAPPA - 2) * ts - 6 * PATH_KAPPA + 3) * ts + 3 * PATH_KAPPA) *
       ts -
     sin_angle) /
  ...
  *point_count = 0;
  ...
  for (i = start_segment; i != end; i += delta)
  {
    ...
    if (i == start_segment)
    {
      ...
      else if (split_at_start) eina_bezier_on_interval(&b, start_t, 1, &res);
    }
