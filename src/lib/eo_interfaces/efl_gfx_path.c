...
static void _path_interpolation(Eo                *obj,
                  ...
                  double             **offset_point)
{
    ...
    if (_efl_gfx_path_command_length(command))
    {
        ...
        {
            double *pts_tmp =
                realloc(pd->points, sizeof(double) * (pts_length * 2));
            if (!pts_tmp) return EFL_FALSE;
   ...
   //grow up twice
    if ((cmd_length + 1) > pd->reserved_cmd_cnt)
    {
        Efl_Gfx_Path_Command *cmd_tmp =
            realloc(pd->commands,
                    (cmd_length * 2) * sizeof(Efl_Gfx_Path_Command));
        if (!cmd_tmp) return EFL_FALSE;
...
_efl_gfx_path_current_search(const Efl_Gfx_Path_Command *cmd,
                             ...
                             double                     *current_x,
                             double                     *current_y,
                             double                     *current_ctrl_x,
                             double                     *current_ctrl_y)
{
    unsigned int i;
    ...
    if (!cmd || !points) return EFL_FALSE;
                       ...
                       const double               *points)
{
   ...
   //full reserved memory
    ...
    _efl_gfx_path_current_search(pd->commands,
                                 pd->points,
                                 &pd->current.x,
                                 &pd->current.y,
                                 &pd->current_ctrl.x,
...
EOLIAN static void
_efl_gfx_path_path_get(const Eo *obj                EFL_UNUSED,
                       Efl_Gfx_Path_Data           *pd,
                       const Efl_Gfx_Path_Command **commands,
                       const double               **points)
{
...
EOLIAN static void
_efl_gfx_path_length_get(const Eo *obj      EFL_UNUSED,
                         Efl_Gfx_Path_Data *pd,
...
EOLIAN static void
_efl_gfx_path_bounds_get(const Eo *obj      EFL_UNUSED,
                         Efl_Gfx_Path_Data *pd,
                         Eina_Rect         *r)
{
    ...
    EINA_RECTANGLE_SET(r,
                       floor(minx),
                       floor(miny),
                       (ceil(maxx) - floor(minx)),
                       (ceil(maxy) - floor(miny)));
}

EOLIAN static void
_efl_gfx_path_current_get(const Eo *obj      EFL_UNUSED,
                          Efl_Gfx_Path_Data *pd,
                          double            *x,
                          double            *y)
{
...
EOLIAN static void
_efl_gfx_path_current_ctrl_get(const Eo *obj      EFL_UNUSED,
                               Efl_Gfx_Path_Data *pd,
                               double            *x,
                               double            *y)
{
                                      ...
                                      Efl_Gfx_Path_Data *b)
{
    ...
    if (a->commands_count != b->commands_count) return EFL_FALSE;
    if (a->commands_count <= 0) return EFL_TRUE;

    for (i = 0; a->commands[i] == b->commands[i] &&
                a->commands[i] != EFL_GFX_PATH_COMMAND_TYPE_END;
         i++)
    {
        ;
    }
...
EOLIAN static Efl_Bool
_efl_gfx_path_interpolate(Eo                *obj,
                          Efl_Gfx_Path_Data *pd,
                          const Eo          *from,
                          const Eo          *to,
                          double             pos_map)
{
   ...
   //just in case
    if (pd == from_pd || pd == to_pd) return EFL_FALSE;

    if (from_pd->path_data && to_pd->path_data)
    {
        _efl_gfx_path_reset(obj, pd);
        _path_interpolation(obj,
                            pd,
                            from_pd->path_data,
                            to_pd->path_data,
                            pos_map);
    ...
    else
    {
        ...
        if (!cmds && (from_pd->commands_count > 0)) return EFL_FALSE;

        pd->commands = cmds;

        pts = realloc(pd->points, sizeof(double) * from_pd->points_count);
        if (!pts && (from_pd->points_count > 0)) return EFL_FALSE;
        ...
        interv             = interpolate(from_pd->current_ctrl.x,
                             to_pd->current_ctrl.x,
                             pos_map);
        pd->current_ctrl.x = interv;

        interv             = interpolate(from_pd->current_ctrl.y,
...
EOLIAN static Efl_Bool
_efl_gfx_path_equal_commands(Eo *obj            EFL_UNUSED,
                             Efl_Gfx_Path_Data *pd,
                             const Eo          *with)
{
    ...
    if (!with_pd) return EFL_FALSE;

    return _efl_gfx_path_equal_commands_internal(with_pd, pd);
}

EOLIAN static void
_efl_gfx_path_reserve(Eo *obj            EFL_UNUSED,
                      Efl_Gfx_Path_Data *pd,
                      ...
                      unsigned int       pts_count)
{
    if (pd->reserved_cmd_cnt < cmd_count)
    {
        ...
        pd->commands =
            realloc(pd->commands,
                    sizeof(Efl_Gfx_Path_Command) * pd->reserved_cmd_cnt);
    }
...
EOLIAN static void
_efl_gfx_path_reset(Eo *obj EFL_UNUSED, Efl_Gfx_Path_Data *pd)
{
    free(pd->commands);
...
EOLIAN static void
_efl_gfx_path_append_move_to(Eo *obj            EFL_UNUSED,
                             Efl_Gfx_Path_Data *pd,
                             double             x,
                             double             y)
{
...
EOLIAN static void
_efl_gfx_path_append_line_to(Eo *obj            EFL_UNUSED,
                             Efl_Gfx_Path_Data *pd,
                             double             x,
                             double             y)
{
...
EOLIAN static void
_efl_gfx_path_append_cubic_to(Eo *obj            EFL_UNUSED,
                              Efl_Gfx_Path_Data *pd,
                              double             ctrl_x0,
                              double             ctrl_y0,
                              double             ctrl_x1,
                               ...
                               double             ctrl_y)
{
    ...
    _efl_gfx_path_append_cubic_to(obj,
                                  pd,
                                  ctrl_x0,
                                  ctrl_y0,
                                  ctrl_x,
...
EOLIAN static void
_efl_gfx_path_append_quadratic_to(Eo                *obj,
                                  Efl_Gfx_Path_Data *pd,
                                  double             x,
                                  double             y,
                                  double             ctrl_x,
...
EOLIAN static void
_efl_gfx_path_append_squadratic_to(Eo                *obj,
                                   Efl_Gfx_Path_Data *pd,
                                   double             x,
                                   double             y)
{
 ...
 */
EOLIAN static void
_efl_gfx_path_append_arc_to(Eo                *obj,
                            Efl_Gfx_Path_Data *pd,
                            double             x,
                            double             y,
                            double             rx,
                            ...
                            Efl_Bool           sweep)
{
    ...
    if ((fabs(x - sx) < (1 / 256.0)) && (fabs(y - sy) < (1 / 256.0))) return;
                ...
                int   *point_count)
{
    ...
    Efl_Bool    split_at_start, split_at_end;
    Eina_Bezier b, res;
...
EOLIAN static void
_efl_gfx_path_append_arc(Eo                *obj,
                         Efl_Gfx_Path_Data *pd,
                         double             x,
                         double             y,
                         double             w,
                         ...
                         double             sweep_length)
{
    int   i, point_count;
    Point pts[15];
    Point curve_start = _curves_for_arc(x,
                                        y,
                                        w,
                                        h,
                                        start_angle,
    ...
    if (pd->commands_count && (pd->commands[pd->commands_count - 2] !=
                               EFL_GFX_PATH_COMMAND_TYPE_CLOSE))
        _efl_gfx_path_append_line_to(obj, pd, curve_start.x, curve_start.y);
    else _efl_gfx_path_append_move_to(obj, pd, curve_start.x, curve_start.y);
...
EOLIAN static void
_efl_gfx_path_append_close(Eo *obj EFL_UNUSED, Efl_Gfx_Path_Data *pd)
{
    double *offset_point;
...
static void
_efl_gfx_path_append_circle(Eo                *obj,
                            Efl_Gfx_Path_Data *pd,
                            double             xc,
                            double             yc,
                            double             radius)
{
    Efl_Bool first = (pd->commands_count <= 0);

    _efl_gfx_path_append_arc(obj,
                             pd,
                             (xc - radius),
                             (yc - radius),
                             (2 * radius),
...
EOLIAN static void
_efl_gfx_path_append_rect(Eo                *obj,
                          Efl_Gfx_Path_Data *pd,
                          double             x,
                          double             y,
                          double             w,
...
EOLIAN static void
_efl_gfx_path_append_horizontal_to(Eo                *obj,
                                   Efl_Gfx_Path_Data *pd,
                                   double             d,
                                   double current_x   EFL_UNUSED,
                                   double             current_y)
{
...
EOLIAN static void
_efl_gfx_path_append_vertical_to(Eo                *obj,
                                 Efl_Gfx_Path_Data *pd,
                                 double             d,
                                 double             current_x,
                                 double current_y   EFL_UNUSED)
{
...
_parse_number(char **content, double *number)
{
    ...
    *number   = strtod(*content, &end);
    ...
    if ((*content) == end) return EFL_FALSE;
    //skip comma if any
                ...
                double            *start_y)
{
    ...
    switch (cmd)
    {
        ...
        case 'S':
            {
                _efl_gfx_path_append_scubic_to(obj,
                                               pd,
                                               arr[2],
                                               arr[3],
                                               arr[0],
        ...
        case 'Q':
            {
                _efl_gfx_path_append_quadratic_to(obj,
                                                  pd,
                                                  arr[2],
                                                  arr[3],
                                                  arr[0],
        ...
        case 'H':
            {
                _efl_gfx_path_append_horizontal_to(obj,
                                                   pd,
                                                   arr[0],
                                                   *cur_x,
                                                   *cur_y);
        ...
        case 'V':
            {
                _efl_gfx_path_append_vertical_to(obj,
                                                 pd,
                                                 arr[0],
                                                 *cur_x,
                                                 *cur_y);
        ...
        case 'A':
            {
                _efl_gfx_path_append_arc_to(obj,
                                            pd,
                                            arr[5],
                                            arr[6],
                                            arr[0],
                    ...
                    double             pos)
{
    ...
    if (cur_locale) cur_locale = strdup(cur_locale);
...
EOLIAN static void
_efl_gfx_path_append_svg_path(Eo                *obj,
                              Efl_Gfx_Path_Data *pd,
