...
struct _Efl_File_Data
{
  ...
  time_t            mtime;
  Efl_Bool file_opened : 1; /* if `file` was opened implicitly during load */
  Efl_Bool
    setting : 1; /* set when this file is internally calling methods to avoid infinite recursion */
  Efl_Bool
    loaded : 1; /* whether the currently set file properties have been loaded */
};
...
_efl_file_unload(Eo *obj, Efl_File_Data *pd)
{
  ...
  pd->setting = 1;
  eina_file_close(
    pd->file);  // close matching open (dup in _efl_file_mmap_set) OK
  pd->file = NULL;
  efl_file_mmap_set(obj, NULL);
  pd->setting = 0;
  pd->loaded = pd->file_opened = EFL_FALSE;
}
...
_efl_file_load(Eo *obj, Efl_File_Data *pd)
{
  ...
  {
    Eina_File *f;
    f = eina_file_open(pd->vpath, EFL_FALSE);
    if (!f) return errno ?: ENOENT;
    pd->file_opened = EFL_TRUE;
    pd->setting     = 1;
    ret             = efl_file_mmap_set(obj, f);
    pd->setting     = 0;
    if (ret) pd->file_opened = EFL_FALSE;
...
_efl_file_mmap_set(Eo *obj, Efl_File_Data *pd, const Eina_File *f)
{
  ...
  if (pd->file)
    eina_file_close(pd->file);  // close matching open (dup above) OK
  pd->file   = file;
  pd->loaded = EFL_FALSE;
...
_efl_file_file_set(Eo *obj, Efl_File_Data *pd, const char *file)
{
  ...
  tmp = (char *)(file);
  if (tmp) tmp = eina_vpath_resolve(tmp);
...
{
  if (eina_stringshare_replace(&pd->key, key)) pd->loaded = 0;
}
...
efl_file_simple_load(Eo *obj, const char *file, const char *key)
{
  ...
  {
    EINA_LOG_ERR("File set to '%s' on '%s' failed.",
                 file,
...
EAPI void
efl_file_simple_mmap_get(const Eo         *obj,
...
#include "efl_file.eo.c"
#include "efl_file_save.eo.c"
