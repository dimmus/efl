#ifndef EFL_MODEL_H__
#define EFL_MODEL_H__
...
EAPI extern Eina_Error
  EFL_PROPERTY_ERROR_INVALID_KEY; /**< Returned when the given key during a efl_ui_property_bind does not exist on the object. */

EAPI extern Eina_Error
  EFL_FACTORY_ERROR_NOT_SUPPORTED; /**< Returned error when factory got a request that it can't fullfil due to a set of unsupported parameters @since 1.22 */
 ...
 * @since 1.17
 */
EAPI void efl_model_property_invalidated_notify(Efl_Model  *model,
                                                const char *property);
 ...
 */
typedef void (*Efl_Model_Value_Struct_Member_Setup_Cb)(
  void                     *data,
  int                       index,
  Eina_Value_Struct_Member *member);
 ...
 * @since 1.17
 */
EAPI Eina_Value_Struct_Desc *efl_model_value_struct_description_new(
