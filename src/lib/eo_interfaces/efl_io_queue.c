...
_efl_io_queue_adjust_and_realloc_if_needed(Eo *o, Efl_Io_Queue_Data *pd)
{
    ...
    if (pd->limit > 0)
    {
        if (pd->position_write + slack >= pd->limit) _efl_io_queue_adjust(pd);
    }
    else if (pd->position_read > slack) _efl_io_queue_adjust(pd);
    ...
    if (spare > slack)
    {
         ...
         * problems with that.
         */
        if (new_size < pd->preallocated) new_size = pd->preallocated;
...
_efl_io_queue_update_cans(Eo *o, Efl_Io_Queue_Data *pd)
{
    ...
    if (pd->pending_eos) efl_io_writer_can_write_set(o, EFL_FALSE);
    else efl_io_writer_can_write_set(o, (limit == 0) || (used < limit));
}
...
_efl_io_queue_preallocate(Eo *o, Efl_Io_Queue_Data *pd, size_t size)
{
    ...
    if (pd->allocated < size) _efl_io_queue_realloc_rounded(o, pd, size);
...
_efl_io_queue_limit_set(Eo *o, Efl_Io_Queue_Data *pd, size_t limit)
{
    ...
    if (pd->allocated > limit) _efl_io_queue_realloc(o, pd, limit);
...
_efl_io_queue_clear(Eo *o, Efl_Io_Queue_Data *pd)
{
    ...
    efl_event_callback_call(o, EFL_IO_QUEUE_EVENT_SLICE_CHANGED, NULL);
    if (pd->closed) return;
    if (pd->pending_eos) efl_io_reader_eos_set(o, EFL_TRUE);
}
...
_efl_io_queue_eos_mark(Eo *o, Efl_Io_Queue_Data *pd)
{
    ...
    if (efl_io_queue_usage_get(o) > 0) pd->pending_eos = EFL_TRUE;
    else efl_io_reader_eos_set(o, EFL_TRUE);
}
...
_efl_io_queue_efl_io_reader_read(Eo                *o,
                                 Efl_Io_Queue_Data *pd,
...
_efl_io_queue_efl_io_reader_can_read_get(const Eo *o        EFL_UNUSED,
                                         Efl_Io_Queue_Data *pd)
{
    return pd->can_read;
}
...
EOLIAN static void
_efl_io_queue_efl_io_reader_can_read_set(Eo                *o,
                                         Efl_Io_Queue_Data *pd,
                                         Efl_Bool           can_read)
{
    EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_reader_eos_get(const Eo *o           EFL_UNUSED,
                                    Efl_Io_Queue_Data *pd EFL_UNUSED)
{
    return pd->eos;
}

EOLIAN static void
_efl_io_queue_efl_io_reader_eos_set(Eo                   *o,
                                    Efl_Io_Queue_Data *pd EFL_UNUSED,
                                    Efl_Bool              is_eos)
{
    EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
    ...
    if (is_eos)
    {
        pd->pending_eos = EFL_FALSE;
        efl_event_callback_call(o, EFL_IO_READER_EVENT_EOS, NULL);
    }
}

EOLIAN static Eina_Error
_efl_io_queue_efl_io_writer_write(Eo                *o,
                                  Efl_Io_Queue_Data *pd,
                                  Eina_Slice        *slice,
                                  Eina_Slice        *remaining)
{
    ...
    else
    {
        ...
        if (pd->allocated >= pd->position_write + slice->len) todo = slice->len;
        else todo = pd->allocated - pd->position_write;
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_writer_can_write_get(const Eo *o        EFL_UNUSED,
                                          Efl_Io_Queue_Data *pd)
{
    return pd->can_write;
}
...
EOLIAN static void
_efl_io_queue_efl_io_writer_can_write_set(Eo                *o,
                                          Efl_Io_Queue_Data *pd,
                                          Efl_Bool           can_write)
{
    EINA_SAFETY_ON_TRUE_RETURN(efl_io_closer_closed_get(o));
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_closer_closed_get(const Eo *o        EFL_UNUSED,
                                       Efl_Io_Queue_Data *pd)
{
    return pd->closed;
}
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_closer_close_on_exec_set(Eo *o                 EFL_UNUSED,
                                              Efl_Io_Queue_Data *pd EFL_UNUSED,
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_closer_close_on_exec_get(const Eo *o           EFL_UNUSED,
                                              Efl_Io_Queue_Data *pd EFL_UNUSED)
{
    return EFL_TRUE;
}
...
EOLIAN static void
_efl_io_queue_efl_io_closer_close_on_invalidate_set(
    Eo *o                        EFL_UNUSED,
    Efl_Io_Queue_Data *pd        EFL_UNUSED,
...
EOLIAN static Efl_Bool
_efl_io_queue_efl_io_closer_close_on_invalidate_get(const Eo *o EFL_UNUSED,
                                                    Efl_Io_Queue_Data *pd
                                                        EFL_UNUSED)
{
    return EFL_TRUE;
