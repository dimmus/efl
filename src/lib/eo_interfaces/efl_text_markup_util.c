...
#define EVAS_TEXTBLOCK_IS_VISIBLE_FORMAT_CHAR(ch)                           \
    (((ch) == _REPLACEMENT_CHAR) || ((ch) == _NEWLINE) || ((ch) == _TAB) || \
...
#define _IS_PARAGRAPH_SEPARATOR_SIMPLE(item) (!strcmp(item, "ps"))

#define _IS_PARAGRAPH_SEPARATOR(o, item)     \
    (_IS_PARAGRAPH_SEPARATOR_SIMPLE(item) || \
     (o->legacy_newline &&                   \
      _IS_LINE_SEPARATOR(item))) /* Paragraph separator */

static void
_markup_get_text_utf8_append(Efl_Strbuf *sbuf, const char *text)
{
  int ch, pos = 0, pos2 = 0;
  ...
  for (;;)
  {
    ...
    if (ch == _NEWLINE) eina_strbuf_append(sbuf, "<br/>");
    else if (ch == _TAB) eina_strbuf_append(sbuf, "<tab/>");
    else if (ch == '<') eina_strbuf_append(sbuf, "&lt;");
    else if (ch == '>') eina_strbuf_append(sbuf, "&gt;");
    else if (ch == '&') eina_strbuf_append(sbuf, "&amp;");
    else if (ch == '"') eina_strbuf_append(sbuf, "&quot;");
    else if (ch == _PARAGRAPH_SEPARATOR) eina_strbuf_append(sbuf, "<ps/>");
    else if (ch == _REPLACEMENT_CHAR) eina_strbuf_append(sbuf, "&#xfffc;");
    else if (ch != '\r')
    {
...
static inline int
_escaped_is_eq_and_advance(const char  *s,
                           const char  *s_end,
                           const char **p_m,
                           const char  *m_end)
{
  Efl_Bool reached_end;
  ...
  reached_end = !**p_m;
  if (*p_m < m_end) _escaped_advance_after_end_of_string(p_m);
...
_escaped_char_get(const char *s, const char *s_end)
{
  ...
  if (s[1] == '#')
  {
    ...
    len = s_end - s;
    if (len > sizeof(ustr)) len = sizeof(ustr);

    memcpy(ustr, s, len);
    ustr[len - 1] = '\0';
    uchar[0]      = strtol(ustr, NULL, base);

    if (uchar[0] == 0) return NULL;
  ...
  else
  {
    ...
    while (map_itr < map_end)
    {
      if (_escaped_is_eq_and_advance(s, s_end, &map_itr, map_end))
        return map_itr;
      if (map_itr < map_end) _escaped_advance_after_end_of_string(&map_itr);
    }
  }
...
_text_util_markup_to_text(const char *text)
{
  ...
  for (;;)
  {
    ...
    if ((*p == 0) || (tag_end) || (esc_end) || (tag_start) || (esc_start))
    {
      if (tag_end)
      {
        ...
        if (ttag)
        {
          ...
          else if (_IS_TAB(ttag)) eina_strbuf_append(sbuf, _TAB_UTF8);
...
#include "efl_text_markup_util.eo.c"
