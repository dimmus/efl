...
static void
_low_mem_key_changed_cb(keynode_t *node EFL_UNUSED, void *data EFL_UNUSED)
{
  int status;

  if (vconf_get_int(VCONFKEY_SYSMAN_LOW_MEMORY, &status) < 0) return;

  if (status == VCONFKEY_SYSMAN_LOW_MEMORY_NORMAL)
    ecore_memory_state_set(ECORE_MEMORY_STATE_NORMAL);
  else ecore_memory_state_set(ECORE_MEMORY_STATE_LOW);
}

static void
_charge_key_changed_cb(keynode_t *node EFL_UNUSED, void *data EFL_UNUSED)
{
  int charging, status;

  if (vconf_get_int(VCONFKEY_SYSMAN_BATTERY_CHARGE_NOW, &charging) < 0) return;
  ...
  if (vconf_get_int(VCONFKEY_SYSMAN_BATTERY_STATUS_LOW, &status) < 0) return;

  if (status == VCONFKEY_SYSMAN_BAT_NORMAL ||
      status == VCONFKEY_SYSMAN_BAT_FULL)
    ecore_power_state_set(ECORE_POWER_STATE_BATTERY);
  else ecore_power_state_set(ECORE_POWER_STATE_LOW);
}

static void
_low_batt_key_changed_cb(keynode_t *node EFL_UNUSED, void *data EFL_UNUSED)
{
  _charge_key_changed_cb(NULL, NULL);
}

static void
_lang_key_changed_cb(keynode_t *node EFL_UNUSED, void *first)
{
  char *lang;
...
static void
_region_fmt_key_changed_cb(keynode_t *node EFL_UNUSED, void *first)
{
  char *region;
