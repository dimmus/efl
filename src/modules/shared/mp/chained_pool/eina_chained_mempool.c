...
struct _Chained_Mempool
{
  ...
  unsigned int usage;
  Chained_Pool
    *first_fill;  //All allocation will happen in this chain,unless it is filled
#ifdef EINA_DEBUG_MALLOC
  int minimal_size;
#endif
#ifdef EINA_HAVE_DEBUG_THREADS
...
_eina_chained_mp_pool_cmp(const Eina_Rbtree *left,
...
static inline int
_eina_chained_mp_pool_key_cmp(const Eina_Rbtree *node,
                              const void        *key,
...
_eina_chained_mp_pool_new(Chained_Mempool *pool)
{
...
#if defined(EINA_DEBUG_MALLOC) && defined(HAVE_MALLOC_USABLE_SIZE)
  {
    ...
    if (sz - pool->minimal_size > 0)
      INF("Just allocated %0.2f%% to much memory in '%s' for one block "
          "of size %i that means %lu bytes to much.",
...
_eina_chained_mempool_free_in(Chained_Mempool *pool, Chained_Pool *p, void *ptr)
{
  ...
  if (ptr < pmem)
  {
    ERR("%p is inside the private part of %p pool from %p '%s' "
        "Chained_Mempool (could be the sign of a buffer underrun).",
        ptr,
        p,
        pool,
        pool->name);
  ...
  {
    ERR(
      "%p is %lu bytes inside a pointer served by %p '%s' "
      "Chained_Mempool (You are freeing the wrong pointer man !).",
      ptr,
      ((((unsigned char *)ptr) - (unsigned char *)(p + 1)) % pool->item_alloc),
      pool,
      pool->name);
    return EFL_FALSE;
  }
  ...
  if (p->usage == 0)
  {
    ...
    pool->root  = eina_rbtree_inline_remove(pool->root,
                                           EINA_RBTREE_GET(p),
...
static void *
eina_chained_mempool_malloc(void *data, EFL_UNUSED unsigned int size)
{
  ...
  if (!p)
  {
    ...
    pool->first =
      eina_inlist_prepend(pool->first, EINA_INLIST_GET(pool->first_fill));
...
eina_chained_mempool_free(void *data, void *ptr)
{
  ...
  r = eina_rbtree_inline_lookup(pool->root,
                                ptr,
                                0,
                                _eina_chained_mp_pool_key_cmp,
                                NULL);
                                 ...
                                 unsigned int size EFL_UNUSED)
{
  ...
  if (after)
  {
    Eina_Rbtree *r = eina_rbtree_inline_lookup(pool->root,
                                               after,
                                               0,
                                               _eina_chained_mp_pool_key_cmp,
                                               NULL);
...
retry:
  ...
  if (!eina_iterator_next(it->walker, (void **)&it->current)) return EFL_FALSE;
...
eina_chained_mempool_iterator_new(void *data)
{
  ...
  it->iterator.get_container =
    FUNC_ITERATOR_GET_CONTAINER(eina_mempool_iterator_get_container);
                            ...
                            void                  *cb_data)
{
  ...
  while (tail && tail->usage == pool->pool_size)
    tail =
      EINA_INLIST_CONTAINER_GET((EINA_INLIST_GET(tail)->prev), Chained_Pool);
...
static void *
eina_chained_mempool_realloc(EFL_UNUSED void        *data,
                          ...
                          va_list                args)
{
  ...
  mp->pool_size =
    (((((mp->item_alloc * mp->pool_size + aligned_chained_pool) / page_size) +
       1) *
      page_size) -
     aligned_chained_pool) /
    mp->item_alloc;
...
eina_chained_mempool_shutdown(void *data)
{
  ...
  while (mp->first)
  {
    ...
    mp->root  = eina_rbtree_inline_remove(mp->root,
                                         EINA_RBTREE_GET(p),
                                         _eina_chained_mp_pool_cmp,
                                         NULL);
    _eina_chained_mp_pool_free(p);
...
chained_init(void)
{
  ...
  _eina_chained_mp_log_dom =
    eina_log_domain_register("eina_mempool", EINA_LOG_COLOR_DEFAULT);
