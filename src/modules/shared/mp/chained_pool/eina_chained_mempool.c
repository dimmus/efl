...
#if defined(EINA_DEBUG_MALLOC) && defined(HAVE_MALLOC_USABLE_SIZE)
    {
        ...
        if (sz - pool->minimal_size > 0)
            INF("Just allocated %0.2f%% to much memory in '%s' for one block "
                ...
                ((float)(sz - pool->minimal_size) * 100) /
                    (float)(pool->alloc_size),
                pool->name,
                pool->alloc_size,
...
_eina_chained_mempool_free_in(Chained_Mempool *pool, Chained_Pool *p, void *ptr)
{
    ...
    if ((((unsigned char *)ptr) - (unsigned char *)(p + 1)) % pool->item_alloc)
    {
        ERR("%p is %lu bytes inside a pointer served by %p '%s' "
            ...
            ptr,
            ((((unsigned char *)ptr) - (unsigned char *)(p + 1)) %
             pool->item_alloc),
            pool,
            pool->name);
        return EFL_FALSE;
    ...
    if (p->usage == 0)
    {
        ...
        if (pool->first_fill == p)
        {
            ...
            pool->first_fill =
                EINA_INLIST_CONTAINER_GET(pool->first, Chained_Pool);
        }
...
eina_chained_mempool_malloc(void *data, EFL_UNUSED unsigned int size)
{
    ...
    if (!p)
    {
        ...
        pool->root =
            eina_rbtree_inline_insert(pool->root,
                                 ...
                                 unsigned int size EFL_UNUSED)
{
    ...
    {
        Eina_Rbtree *r =
            eina_rbtree_inline_lookup(pool->root,
                                      after,
                                      0,
                                      _eina_chained_mp_pool_key_cmp,
                                      NULL);
                          ...
                          va_list                args)
{
    ...
    mp->pool_size =
        (((((mp->item_alloc * mp->pool_size + aligned_chained_pool) /
            page_size) +
           1) *
          page_size) -
