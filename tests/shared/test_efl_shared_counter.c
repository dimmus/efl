...
EFL_START_TEST(efl_shared_counter_simple)
{
  Eina_Counter *cnt;
  char         *dump;
  int           i;
...
EFL_START_TEST(efl_shared_counter_break)
{
  Eina_Counter *cnt;
...
#  endif
    TEST_MAGIC_SAFETY("eina_counter_new", "safety check failed: name == NULL");
...
void
eina_test_counter(TCase *tc)
{
  tcase_add_test(tc, efl_shared_counter_simple);
  tcase_add_test(tc, efl_shared_counter_break);
}
