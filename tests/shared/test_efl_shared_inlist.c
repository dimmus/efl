...
EFL_START_TEST(efl_shared_inlist_simple)
{
...
#  ifdef SHOW_LOG
  fprintf(stderr, "you should have a safety check failure below:\n");
#  endif
  TEST_MAGIC_SAFETY("eina_inlist_append", "safety check failed: new_l == NULL");
...
#  ifdef SHOW_LOG
  fprintf(stderr, "you should have a safety check failure below:\n");
#  endif
  TEST_MAGIC_SAFETY("eina_inlist_find", "safety check failed: item == NULL");
...
#  ifdef SHOW_LOG
  fprintf(stderr, "you should have a safety check failure below:\n");
#  endif
  TEST_MAGIC_SAFETY("eina_inlist_demote", "safety check failed: list == NULL");
...
#  ifdef SHOW_LOG
  fprintf(stderr, "you should have a safety check failure below:\n");
#  endif
  TEST_MAGIC_SAFETY("eina_inlist_promote", "safety check failed: list == NULL");
...
#  endif
  ...
  tmp  = EINA_INLIST_CONTAINER_GET(eina_inlist_find(lst, EINA_INLIST_GET(prev)),
                                  Eina_Test_Inlist);
  lst  = eina_inlist_remove(lst, EINA_INLIST_GET(tmp));
...
_eina_test_inlist_check_insert_reverse(const Eina_Inlist *list)
{
  ...
  EINA_INLIST_FOREACH(list, t)
  {
    if (last_value) ck_assert_int_le(t->value, last_value);
    last_value = t->value;
  }
}

EFL_START_TEST(efl_shared_inlist_sorted)
{
...
EFL_START_TEST(efl_shared_inlist_sorted_state)
{
  ...
  for (i = 0; i < EINA_C_ARRAY_LENGTH(values_unsorted); ++i)
  {
    ...
    list = eina_inlist_sorted_state_insert(list,
                                           EINA_INLIST_GET(&tmp[i]),
                                           _eina_test_inlist_cmp,
                                           state);
...
EFL_START_TEST(efl_shared_inlist_sorted2)
{
  Eina_Test_Inlist_Sorted *t, tmp[EINA_C_ARRAY_LENGTH(values_unsorted)];
  Eina_Inlist             *list   = NULL;
  Eina_Inlist             *sorted = NULL;
...
EFL_START_TEST(efl_shared_inlist_sorted_state2)
{
  Eina_Test_Inlist_Sorted   tmp[EINA_C_ARRAY_LENGTH(values_unsorted)];
  Eina_Inlist_Sorted_State *state;
  Eina_Inlist              *list = NULL;
...
{
  tcase_add_test(tc, efl_shared_inlist_simple);
  tcase_add_test(tc, efl_shared_inlist_sorted);
  tcase_add_test(tc, efl_shared_inlist_sorted_state);
  tcase_add_test(tc, efl_shared_inlist_sorted2);
  tcase_add_test(tc, efl_shared_inlist_sorted_state2);
}
