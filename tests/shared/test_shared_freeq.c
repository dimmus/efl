...
TEST(freeq_simple)
{
    ...
    efl_assert_fail_if(fq == NULL);
    efl_assert_fail_if(eina_freeq_type_get(fq) != EINA_FREEQ_DEFAULT);
...
TEST(freeq_tune)
{
    ...
    efl_assert_fail_if(eina_freeq_count_max_get(eina_freeq_main_get()) != 3);

    eina_freeq_mem_max_set(eina_freeq_main_get(), 20);
    efl_assert_fail_if(eina_freeq_mem_max_get(eina_freeq_main_get()) != 20);
    ...
    efl_assert_fail_if(_n > 2);

    eina_freeq_count_max_set(eina_freeq_main_get(), 1);
    efl_assert_fail_if(_n > 1);
...
TEST(freeq_postponed)
{
    ...
    efl_assert_fail_if(!fq);
    efl_assert_fail_if(eina_freeq_type_get(fq) != EINA_FREEQ_POSTPONED);

   // by default: no limit
    efl_assert_int_eq(eina_freeq_count_max_get(fq), -1);
    efl_assert_int_eq(eina_freeq_mem_max_get(fq), 0);
    ...
    efl_assert_int_eq(_n, EINA_C_ARRAY_LENGTH(values));

    efl_assert_fail_if(!eina_freeq_ptr_pending(fq));
    while (eina_freeq_ptr_pending(fq))
        eina_freeq_reduce(fq, 1);
    efl_assert_fail_if(eina_freeq_ptr_pending(fq));
    efl_assert_int_eq(_n, 0);

    for (k = 0; k < EINA_C_ARRAY_LENGTH(values); k++)
        efl_assert_int_eq(*(values[k]), 0xDEADBEEF);
