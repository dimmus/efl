...
EFL_START_TEST(efl_shared_test_xattr_set)
{
  ...
  ret =
    eina_xattr_fd_set(fd, attribute1, data1, strlen(data1), EINA_XATTR_CREATED);
  fail_if(ret != EFL_TRUE);
...
EFL_START_TEST(efl_shared_test_xattr_list)
{
  char          *filename    = "tmpfile";
...
EFL_START_TEST(efl_shared_test_xattr_types)
{
  char        *filename    = "tmpfile";
  ...
  ret =
    eina_xattr_int_set(test_file_path, int_attr, int_data, EINA_XATTR_INSERT);
  fail_if(ret != EFL_TRUE);
...
{
#ifdef XATTR_TEST_DIR
  tcase_add_test(tc, efl_shared_test_xattr_set);
  tcase_add_test(tc, efl_shared_test_xattr_list);
  tcase_add_test(tc, efl_shared_test_xattr_types);
#else
  (void)tc;
#endif
}
