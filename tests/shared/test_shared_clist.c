...
free_list(void)
{
    ...
    EINA_CLIST_FOR_EACH_ENTRY_SAFE(str,
                                   tmp,
                                   &string_list,
                                   struct test_string,
                                   entry)
...
static void
add_string_after(const char *str_to_add)
{
...
static void
add_string_head(const char *string_to_add)
{
    struct test_string *entry_to_add;
    ...
    efl_assert_fail_if(eina_clist_head(&string_list) != &entry_to_add->entry);
}

static void
iterating_two_phase_with_add_head(unsigned int n, const char *str, int n_ent)
{
    int                 i;
...
TEST(clist_basic)
{
    ...
    efl_assert_fail_if(!eina_clist_empty(&string_list));
    ...
    efl_assert_fail_if(eina_clist_count(&string_list) != 0);
}
