...
check_iterator(Eina_Iterator *it, struct test_rect *cur_test)
{
  ...
  {
    fail_if(cur_test[i].col != tile->col || cur_test[i].row != tile->row ||
...
EFL_START_TEST(efl_shared_test_tile_grid_slicer_iterator)
{
  Eina_Iterator    *it;
...
EFL_START_TEST(efl_shared_test_tiler_all)
{
  Eina_Tiler     *tl;
  Eina_Iterator  *it;
  Eina_Rectangle *rp;
...
EFL_START_TEST(efl_shared_test_tiler_stable)
{
  Eina_Tiler     *tl;
  Eina_Rectangle *rp;
  Eina_Iterator  *it;
...
EFL_START_TEST(efl_shared_test_tiler_calculation)
{
  Eina_Tiler    *t1, *t2, *t;
  Eina_Iterator *itr;
...
EFL_START_TEST(efl_shared_test_tiler_size)
{
  Eina_Rectangle *r;
  Eina_Iterator  *it;
  Eina_Tiler     *t;
...
{
  tcase_add_test(tc, efl_shared_test_tile_grid_slicer_iterator);
  tcase_add_test(tc, efl_shared_test_tiler_all);
  tcase_add_test(tc, efl_shared_test_tiler_stable);
  tcase_add_test(tc, efl_shared_test_tiler_calculation);
  tcase_add_test(tc, efl_shared_test_tiler_size);
}
