...
_eina_quadtree_rectangle_vert(const void *object, size_t middle)
{
    ...
    if (r->y + r->h < (int)middle) return EINA_QUAD_LEFT;

    if (r->y > (int)middle) return EINA_QUAD_RIGHT;
...
_eina_quadtree_rectangle_hort(const void *object, size_t middle)
{
    ...
    if (r->x + r->w < (int)middle) return EINA_QUAD_LEFT;

    if (r->x > (int)middle) return EINA_QUAD_RIGHT;

    return EINA_QUAD_BOTH;
}

TEST(quadtree_collision)
{
    struct
    {
        Eina_Rectangle      r;
    ...
    for (i = 0; tests[i].count != -1; ++i)
    {
        ...
        while (head)
        {
            ...
            {
                if (&objects[tests[i].result[k]].r == r) break;
            }
            efl_assert_fail_if(k == tests[i].count);

            head = head->next;
            count++;
        }
        efl_assert_fail_if(count != tests[i].count);
    }
    ...
    while (head)
    {
        ...
        efl_assert_fail_if(r != &objects[tests[1].result[show[count]]].r);

        head = head->next;
        count++;
    }
    efl_assert_fail_if(count != 3);
