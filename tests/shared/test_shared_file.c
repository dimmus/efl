...
#  undef TEST_MAGIC_SAFETY
    ...
    efl_assert_fail_if(!ea);
    efl_assert_fail_if(eina_array_count(ea) != 5);
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 0), "this"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 1), "is"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 2), "a"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 3), "small"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 4), "test"));

    eina_array_free(ea);

#ifdef _WIN32
    ea = eina_file_split(
        strdup("this\\/\\is\\//\\\\a \\more/\\complex///case\\\\\\"));
#else
    ea = eina_file_split(strdup("this//is///a /more/complex///case///"));
#endif

    efl_assert_fail_if(!ea);
    efl_assert_fail_if(eina_array_count(ea) != 6);
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 0), "this"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 1), "is"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 2), "a "));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 3), "more"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 4), "complex"));
    efl_assert_fail_if(strcmp(eina_array_data_get(ea, 5), "case"));
...
get_eina_test_file_tmp_dir(void)
{
    ...
    Efl_Bool created = eina_file_mkdtemp("EinaFileTestXXXXXX", &tmp_dir);
...
TEST(file_direct_ls_simple)
{
    ...
    */
    ...
    const char *good_dirs[] = { "eina_file_direct_ls_simple_dir",
#ifndef _WIN32
                                "a.",
#endif
                                "$a$b",
    ...
    efl_assert_fail_if(test_dirname == NULL);
    ...
    for (int i = 0; i != good_dirs_count; ++i)
    {
        ...
        rmdir(dirname);
        efl_assert_fail_if(mkdir(dirname, default_dir_rights) != 0);

        Eina_File_Direct_Info *dir_info;
        Eina_Iterator         *it        = eina_file_direct_ls(test_dirname);
        Efl_Bool               found_dir = EFL_FALSE;

        efl_assert_fail_if(!eina_iterator_container_get(it));
        ...
        efl_assert_fail_if(!found_dir);
        efl_assert_fail_if(rmdir(dirname) != 0);

        eina_tmpstr_del(dirname);
    }
    efl_assert_fail_if(rmdir(test_dirname) != 0);
...
TEST(file_ls_simple)
{
    ...
    */
    ...
    const char *good_dirs[] = { "eina_file_ls_simple_dir",
#ifndef _WIN32
                                "b.",
#endif
                                "$b$a",
...
TEST(file_map_new_test)
{
    ...
    int fd, correct_file_open_check, correct_map_check, map_offset, map_length,
   ...
   // memory allocation
    ...
    test_full_filename2_size =
        test_file2_name_part_size + test_dirname_size + 1;
   ...
   // Set first symbol == '/0' to strcat corret work
    ...
    last_chunk_size =
        big_buffer_size - iteration_number * template_size - file_min_offset;
    ...
    efl_assert_fail_if(correct_file_open_check != 0);

    efl_assert_fail_if(eina_file_refresh(e_file));

    e_file2 = eina_file_open(test_file2_path, EFL_FALSE);
    efl_assert_fail_if(!e_file2);
    file2_length = eina_file_size_get(e_file2);
    correct_file_open_check =
        file2_length - (big_buffer_size - file_min_offset);
    ...
    file_map =
        eina_file_map_new(e_file, EINA_FILE_WILLNEED, map_offset, map_length);
    efl_assert_fail_if(file_map);
    ...
    file_map =
        eina_file_map_new(e_file, EINA_FILE_WILLNEED, map_offset, map_length);
    efl_assert_fail_if(file_map);
    efl_assert_fail_if(eina_file_map_faulted(e_file, file_map));
    ...
    efl_assert_fail_if(correct_map_check != 0);
    ...
    file2_map =
        eina_file_map_new(e_file2, EINA_FILE_WILLNEED, map_offset, map_length);
    efl_assert_fail_if(!file2_map);
    correct_map_check =
        strcmp((char *)file2_map, big_buffer + memory_page_size);
    efl_assert_fail_if(correct_map_check != 0);

    // test no crash with eina_file_map_populate()
    eina_file_map_populate(e_file, EINA_FILE_POPULATE, file_map, 0, 0);
    eina_file_map_populate(e_file,
                           EINA_FILE_POPULATE,
                           file_map,
                           file_length / 2,
                           0);
...
TEST(test_file_virtualize)
{
    ...
    f = eina_file_virtualize("gloubi",
                             virtual_file_data,
                             strlen(virtual_file_data),
                             EFL_FALSE);
    efl_assert_fail_if(!f);

    efl_assert_fail_if(!eina_file_virtual(f));

    tmp = eina_file_dup(f);
    efl_assert_fail_if(!tmp);
    eina_file_close(tmp);

    efl_assert_fail_if(strcmp("gloubi", eina_file_filename_get(f)));

    map = eina_file_map_new(f, EINA_FILE_WILLNEED, 7, 7);
    efl_assert_fail_if(map != (virtual_file_data + 7));
    ...
    {
        i++;
        efl_assert_fail_if(ln->index != i);

        if (i == 4 || i == 5) efl_assert_fail_if(ln->length != 0);
    }
    efl_assert_fail_if(eina_iterator_container_get(it) != f);
    eina_iterator_free(it);

    efl_assert_fail_if(i != 7);
...
_eina_test_file_thread(void *data EFL_UNUSED, Eina_Thread t EFL_UNUSED)
{
    ...
    efl_assert_fail_if(!GetSystemDirectoryA(filename, MAX_PATH));
    ...
    */
    efl_assert_fail_if(MAX_PATH < len + 1 + sizeof(test_file));
...
TEST(test_file_path)
{
    ...
    for (i = 0; i < sizeof(sanitize) / sizeof(sanitize[0]); i++)
    {
        ...
        efl_assert_str_eq(path, sanitize[i].result);
        free(path);
    }
}

#ifdef XATTR_TEST_DIR
TEST(test_file_xattr)
{
    Eina_File     *ef;
    char          *filename    = "tmpfile";
    const char    *attribute[] = { "user.comment1",
                                   "user.comment2",
                                   "user.comment3" };
    ...
    fd = open(test_file_path,
              O_RDONLY | O_BINARY | O_CREAT | O_TRUNC,
    ...
    {
        ret = eina_xattr_set(test_file_path,
                             attribute[i],
                             data[i],
                             strlen(data[i]),
                             EINA_XATTR_INSERT);
    ...
    ef = eina_file_open(test_file_path, EFL_FALSE);
    efl_assert_fail_if(!ef);
    ...
    efl_assert_fail_if(count != sizeof(attribute) / sizeof(attribute[0]));
    eina_iterator_free(it);
    ...
    efl_assert_fail_if(count != sizeof(data) / sizeof(data[0]));
    eina_iterator_free(it);
...
TEST(test_file_copy)
{
    ...
    efl_assert_fail_if(fd1 <= 0);

    fd2 = eina_file_mkstemp(test_file2_name, &test_file2_path);
    efl_assert_fail_if(fd2 <= 0);

    efl_assert_fail_if(write(fd1, data, strlen(data)) != (ssize_t)strlen(data));

    close(fd1);
    close(fd2);

    //Copy file data
    ret = eina_file_copy(test_file1_path,
                         test_file2_path,
                         EINA_FILE_COPY_DATA,
                         NULL,
                         NULL);
    ...
    e_file1 = eina_file_open(test_file1_path, EFL_FALSE);
    efl_assert_fail_if(!e_file1);
    ...
    efl_assert_fail_if(e_file1 == e_file2);

    efl_assert_fail_if(file1_len != strlen(data));
    efl_assert_fail_if(file1_len != file2_len);

    //Check the contents of both the file
    content1 = eina_file_map_all(e_file1, EINA_FILE_POPULATE);
    efl_assert_fail_if(content1 == NULL);

    content2 = eina_file_map_all(e_file2, EINA_FILE_POPULATE);
    efl_assert_fail_if(content2 == NULL);

    rval = memcmp(content1, content2, strlen(data));
    efl_assert_fail_if(rval != 0);
...
TEST(test_file_statat)
{
    ...
    fd = open(test_file1_path,
              O_WRONLY | O_BINARY | O_CREAT | O_TRUNC,
              S_IWUSR | S_IRUSR);
    efl_assert_fail_if(fd == 0);
    efl_assert_fail_if(write(fd, template, template_size) != template_size);
    close(fd);
    ...
    EINA_ITERATOR_FOREACH(it, info)
    {
        ...
        efl_assert_fail_if(st.size != (unsigned int)template_size);
    }
...
TEST(test_file_mktemp)
{
...
#ifdef F_GETPATH
    /* most likely Mac OS */
    if (fcntl(fd, F_GETPATH, buf) != -1) unlink(buf);
#elif !defined _WIN32
    sprintf(fmt, "/proc/self/fd/%d", fd);
    if (readlink(fmt, buf, sizeof(buf))) unlink(buf);
#else
    {
        char   buf[4096];
        HANDLE h;
        ...
        efl_assert_fail_if(h == INVALID_HANDLE_VALUE);
        l = GetFinalPathNameByHandle(h, buf, sizeof(buf), 0);
        efl_assert_fail_if(l == 0);
    ...
    for (unsigned int k = 0; k < sizeof(patterns) / sizeof(patterns[0]); k++)
    {
        ...
        assert(fd >= 0);
        assert(!!tmpfile);
        file = eina_file_open(tmpfile, EFL_FALSE);
    ...
    efl_assert_fail_if((fd < 0) || !tmpfile);
    efl_assert_fail_if(close(fd));
    ...
    /* should be empty! */
    EINA_ITERATOR_FOREACH(it, info)
        efl_assert_fail_if(strcmp(info->path, tmpfile));

    eina_iterator_free(it);

    efl_assert_fail_if(unlink(tmpfile));
    efl_assert_fail_if(rmdir(tmpdir));
    eina_tmpstr_del(tmpdir);
}

static int
create_file_not_empty(const char   *file_name,
                      Eina_Tmpstr **test_file_path,
                      Efl_Bool      close_file)
{
    const char *data      = "abcdefghijklmnopqrstuvwxyz";
    ...
    if (close_file == EFL_TRUE)
    {
        close(fd);
        fd = 0;
    }
    efl_assert_fail_if(wr_size != data_size);
    return fd;
}

TEST(test_file_unlink)
{
    int          fd;
    Eina_Tmpstr *test_file_path;
    Eina_File   *test_file;
    ...
    fd = create_file_not_empty(tmpfile, &test_file_path, EFL_TRUE);
    efl_assert_fail_if(fd != 0);
    efl_assert_fail_if(!eina_file_unlink(test_file_path));

    /*If file was opened as 'eina'*/
    fd = create_file_not_empty(tmpfile, &test_file_path, EFL_TRUE);
    efl_assert_fail_if(fd != 0);

    test_file = eina_file_open(test_file_path, EFL_FALSE);
    efl_assert_fail_if(!test_file);
    efl_assert_fail_if(!eina_file_unlink(test_file_path));
