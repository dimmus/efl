...
#define TEST_TEXT \
  "This test should be so long that it is longer than the initial size of strbuf"
   ...
   efl_assert_fail_if(strlen(eina_strbuf_string_get(buf)) != eina_strbuf_length_get(buf));
   eina_strbuf_append(buf, TEST_TEXT);
   efl_assert_fail_if(strcmp(eina_strbuf_string_get(buf), TEST_TEXT TEST_TEXT));
   efl_assert_fail_if(strlen(eina_strbuf_string_get(buf)) != eina_strbuf_length_get(buf));
...
TEST(test_strbuf_insert)
{
   ...
   efl_assert_fail_if(strlen(eina_strbuf_string_get(
...
TEST(test_strbuf_realloc)
{
   ...
   efl_assert_fail_if(memcmp(eina_strbuf_string_get(buf) + sz, pattern, sizeof(pattern) -
                  1));
   sz += sizeof(pattern) - 1;
...
TEST(test_strbuf_prepend_print)
{
   ...
   efl_assert_fail_if(ret != EINA_TRUE);
   str = eina_strbuf_string_get(buf);
   efl_assert_fail_if(str == NULL || strcmp(str, "hello   This is 1 string") != 0);
   eina_strbuf_reset(buf);
