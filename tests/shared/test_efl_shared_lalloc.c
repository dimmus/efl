...
EFL_START_TEST(efl_shared_lalloc_simple)
{
    ...
    for (i = 0; i < 10; ++i)
        fail_if(eina_lalloc_element_add(test) != EFL_TRUE);

    fail_if(eina_lalloc_element_add(test) != EFL_FALSE);
    fail_if(eina_lalloc_elements_add(test, 5) != EFL_TRUE);

    for (i = 0; i < 21; ++i)
        fail_if(eina_lalloc_element_add(test) != EFL_TRUE);

    fail_if(eina_lalloc_elements_add(test, 50) != EFL_FALSE);
...
{
    tcase_add_test(tc, efl_shared_lalloc_simple);
}
