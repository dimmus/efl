...
TEST(inlist_simple)
{
    ...
    EINA_INLIST_FOREACH(lst, tmp)
    {
        ...
        {
            case 0:
                efl_assert_fail_if(tmp->i != 27);
                break;

            case 1:
                efl_assert_fail_if(tmp->i != 3227);
                break;
...
_eina_test_inlist_check_insert(const Eina_Inlist *list)
{
    ...
    {
        efl_assert_int_ge(t->value, last_value);
        last_value = t->value;
    }
}
...
_eina_test_inlist_check_insert_reverse(const Eina_Inlist *list)
{
    ...
    EINA_INLIST_FOREACH(list, t)
    {
        if (last_value) efl_assert_int_le(t->value, last_value);
        last_value = t->value;
    }
}
