...
EFL_START_TEST(efl_shared_rectangle_pool)
{
  ...
  fail_if(eina_rectangle_pool_geometry_get(pool, &w, &h) != EFL_TRUE);
  fail_if(w != 256 || h != 256);
...
EFL_START_TEST(efl_shared_rectangle_pool_skyline)
{
  Eina_Rectangle_Pool *pool;
  Eina_Rectangle      *rects[8][8];
  int                  x;
...
EFL_START_TEST(efl_shared_rectangle_union_intersect)
{
  ...
  fail_if(rd.x != r2.x || rd.y != r2.y || rd.w != r2.w || rd.h != r2.h);

  rd = r1;

  fail_if(!eina_rectangle_intersection(&rd, &r4));

  fail_if(rd.x != 30 || rd.y != 30 || rd.w != 30 || rd.h != 30);

  rd = r1;
  eina_rectangle_union(&rd, &r2);
  fail_if(rd.x != r1.x || rd.y != r1.y || rd.w != r1.w || rd.h != r1.h);
  ...
  fail_if(rd.x != 0 || rd.y != 0 || rd.w != 60 || rd.h != 60);

  rd = r3;
  eina_rectangle_union(&rd, &r4);
  fail_if(rd.x != 0 || rd.y != 0 || rd.w != 80 || rd.h != 80);

  rd = r5;
  eina_rectangle_union(&rd, &r6);
  fail_if(rd.x != 10 || rd.y != 10 || rd.w != 20 || rd.h != 20);
}

EFL_END_TEST

EFL_START_TEST(efl_shared_rectangle_position_test)
{
...
{
  tcase_add_test(tc, efl_shared_rectangle_pool);
  tcase_add_test(tc, efl_shared_rectangle_pool_skyline);
  tcase_add_test(tc, efl_shared_rectangle_union_intersect);
  tcase_add_test(tc, efl_shared_rectangle_position_test);
}
