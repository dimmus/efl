...
EFL_START_TEST(efl_shared_unicode_utf8)
{
   ...
   /* First possible sequence of a certain length */
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\x00", &ind) != 0x00) || (ind != 0));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\x01", &ind) != 0x01) || (ind != 1));
    ind = 0;
    ...
    fail_if((eina_unicode_utf8_get_next("\xF0\x90\x80\x80", &ind) != 0x10000) ||
            (ind != 4));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xF8\x88\x80\x80\x80", &ind) !=
             0x200000) ||
            (ind != 5));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFC\x84\x80\x80\x80\x80", &ind) !=
             0x4000000) ||
            (ind != 6));

   /* Last possible sequence of a certain length */
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\x7F", &ind) != 0x7F) || (ind != 1));
    ind = 0;
    ...
    ind = 0;
    fail_if(
        (eina_unicode_utf8_get_next("\xF7\xBF\xBF\xBF", &ind) != 0x1FFFFF) ||
        (ind != 4));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFB\xBF\xBF\xBF\xBF", &ind) !=
             0x3FFFFFF) ||
            (ind != 5));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFD\xBF\xBF\xBF\xBF\xBF", &ind) !=
             0x7FFFFFFF) ||
            (ind != 6));
   ...
   /* Other boundary conditions */
    ...
    ind = 0;
    fail_if(
        (eina_unicode_utf8_get_next("\xF4\x8F\xBF\xBF", &ind) != 0x10FFFF) ||
        (ind != 4));
    ind = 0;
    fail_if(
        (eina_unicode_utf8_get_next("\xF4\x90\x80\x80", &ind) != 0x110000) ||
        (ind != 4));

   /* Error cases */
   /* Standalone continuation bytes */
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\x80", &ind) != 0xDC80) || (ind != 1));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xBF", &ind) != 0xDCBF) || (ind != 1));
    ind = 0;
...
#define _FIRST_SEQUENCES(start, end)                                        \
    ...
    {                                                                       \
        ...
        {                                                                   \
            fail_if(                                                        \
                (eina_unicode_utf8_get_next(buf, &ind) != (0xDC00 | ch)) || \
                (ind != ++i));                                              \
...
#define _INCOMPLETE_SEQUENCES(first, conti, len)                              \
   ...
   /* Impossible bytes */
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFE", &ind) != 0xDCFE) || (ind != 1));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFF", &ind) != 0xDCFF) || (ind != 1));
   ...
   /* Overlong sequences */
    ...
    ind = 0;
    fail_if(
        (eina_unicode_utf8_get_next("\xF8\x80\x80\x80\xAF", &ind) != 0xDCF8) ||
        (ind != 1));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFC\x80\x80\x80\x80\xAF", &ind) !=
             0xDCFC) ||
            (ind != 1));
   ...
   /* Maximum overlong sequences */
    ...
    ind = 0;
    fail_if(
        (eina_unicode_utf8_get_next("\xF8\x87\xBF\xBF\xBF", &ind) != 0xDCF8) ||
        (ind != 1));
    ind = 0;
    fail_if((eina_unicode_utf8_get_next("\xFC\x83\xBF\xBF\xBF\xBF", &ind) !=
             0xDCFC) ||
            (ind != 1));
...
EFL_START_TEST(efl_shared_alignof)
{
...
eina_test_abi(TCase *tc)
{
    ...
    tcase_add_test(tc, efl_shared_alignof);
}
