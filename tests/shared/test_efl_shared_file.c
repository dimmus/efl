...
EFL_START_TEST(efl_shared_file_split_simple)
{
    Eina_Array *ea;
...
EFL_START_TEST(efl_shared_file_direct_ls_simple)
{
   /*
...
EFL_START_TEST(efl_shared_file_ls_simple)
{
   /*
    * Windows: naming conventions
    * https://msdn.microsoft.com/en-us/library/windows/desktop/aa365247%28v=vs.85%29.aspx
...
EFL_START_TEST(efl_shared_file_map_new_test)
{
...
EFL_START_TEST(efl_shared_test_file_virtualize)
{
    Eina_File      *f;
    Eina_File      *tmp;
    Eina_Iterator  *it;
    ...
    EINA_ITERATOR_FOREACH(it, ln)
    {
        ...
        if (i == 4 || i == 5) fail_if(ln->length != 0);
    }
...
EFL_START_TEST(efl_shared_test_file_thread)
{
    Eina_Thread  th[4];
    unsigned int i;

    for (i = 0; i < 4; i++)
        fail_if(!(eina_thread_create(&th[i],
                                     EINA_THREAD_NORMAL,
                                     -1,
...
EFL_START_TEST(efl_shared_test_file_path)
{
    unsigned int i;
    char        *path;
...
EFL_START_TEST(efl_shared_test_file_xattr)
{
    Eina_File     *ef;
...
EFL_START_TEST(efl_shared_test_file_copy)
{
...
EFL_START_TEST(efl_shared_test_file_statat)
{
    Eina_Tmpstr           *test_file1_path, *test_file2_path;
    Eina_Iterator         *it;
    Eina_Stat              st;
...
EFL_START_TEST(efl_shared_test_file_mktemp)
{
...
EFL_START_TEST(efl_shared_test_file_unlink)
{
    int          fd;
    Eina_Tmpstr *test_file_path;
    Eina_File   *test_file;
...
{
    tcase_add_test(tc, efl_shared_file_split_simple);
    tcase_add_test(tc, efl_shared_file_direct_ls_simple);
    tcase_add_test(tc, efl_shared_file_ls_simple);
    tcase_add_test(tc, efl_shared_file_map_new_test);
    tcase_add_test(tc, efl_shared_test_file_virtualize);
    tcase_add_test(tc, efl_shared_test_file_thread);
    tcase_add_test(tc, efl_shared_test_file_path);
#ifdef XATTR_TEST_DIR
    tcase_add_test(tc, efl_shared_test_file_xattr);
#endif
    tcase_add_test(tc, efl_shared_test_file_copy);
    tcase_add_test(tc, efl_shared_test_file_statat);
    tcase_add_test(tc, efl_shared_test_file_mktemp);
    tcase_add_test(tc, efl_shared_test_file_unlink);
}
