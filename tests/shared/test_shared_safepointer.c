...
TEST(test_safepointer_reusable)
{
    unsigned int i;
    ...
    for (i = 0; i < EINA_C_ARRAY_LENGTH(test_array); i++)
    {
        ...
        efl_assert_ptr_ne(ptr, NULL);
        efl_assert_ptr_ne(ptr, &test_array[i]);
        efl_assert_ptr_eq(&test_array[i], eina_safepointer_get(pointers[i]));
    }
    ...
    for (i = 0; i < EINA_C_ARRAY_LENGTH(test_array2); i++)
    {
        ...
        efl_assert_ptr_ne(ptr, NULL);
        efl_assert_ptr_ne(ptr, &test_array2[i]);
        efl_assert_ptr_eq(&test_array2[i], eina_safepointer_get(ptr));
    ...
    for (i = 0; i < EINA_C_ARRAY_LENGTH(pointers); i++)
    {
        ...
        {
            efl_assert_ptr_ne(pointers[j], pointers[i]);
        }
    }
}
...
_thread1(void *data EFL_UNUSED, Eina_Thread t EFL_UNUSED)
{
    ...
    efl_assert_int_ne(eina_barrier_wait(&b), 0);
    ...
    for (i = 0; i < EINA_C_ARRAY_LENGTH(test_array); i++)
    {
        ...
        efl_assert_ptr_ne(ptr, NULL);
        efl_assert_ptr_ne(ptr, &test_array[i]);
        efl_assert_ptr_eq(&test_array[i], eina_safepointer_get(ptr));
    }
...
TEST(test_safepointer_threading)
{
    Eina_Thread  t1, t2;
    unsigned int i;
   ...
   // to start running in parallel
    efl_assert_int_ne(eina_barrier_new(&b, 2), 0);
...
TEST(test_safepointer_lowestbit)
{
    unsigned int i;
    ...
    for (i = 0; i < EINA_C_ARRAY_LENGTH(test_array); i++)
    {
        ...
        efl_assert_int_eq((((uintptr_t)ptr) & 0x3), 0);
        ...
        EXPECT_ERROR_START;
        efl_assert_ptr_eq(eina_safepointer_get(ptr), NULL);
        EXPECT_ERROR_END;
    }
}
