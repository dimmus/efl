...
EFL_START_TEST(efl_shared_convert_simple)
{
  char tmp[128];
  char ref[128];
...
_eina_convert_check(double test, int length)
{
  ...
  fail_if(eina_convert_atod(tmp, 128, &m, &e) != EFL_TRUE);
  r = ldexp((double)m, e);
  fail_if(fabs(r - test) > DBL_MIN);
}

EFL_START_TEST(efl_shared_convert_double)
{
  long long int m = 0;
  long          e = 0;
  ...
  fail_if(eina_convert_atod("ah ah ah", 8, &m, &e) != EFL_FALSE);
  fail_if(eina_convert_atod("0xjo", 8, &m, &e) != EFL_FALSE);
  fail_if(eina_convert_atod("0xp", 8, &m, &e) != EFL_FALSE);
}
...
EFL_START_TEST(efl_shared_convert_fp)
{
...
_eina_convert_strtod_c_check(const char *str)
{
  ...
  switch (fpclassify(d2))
  {
    ...
    case FP_INFINITE:
      ...
      break;
    default:
      fail_if((fpclassify(d1) != FP_ZERO) && (fpclassify(d1) != FP_SUBNORMAL) &&
...
EFL_START_TEST(efl_shared_convert_strtod_c_simple)
{
  char *old;
...
{
  tcase_add_test(tc, efl_shared_convert_simple);
  tcase_add_test(tc, efl_shared_convert_double);
  tcase_add_test(tc, efl_shared_convert_fp);
  tcase_add_test(tc, efl_shared_convert_strtod_c_simple);
}
