         ...
         if (data[i][j] != 0)
           {
              r = eina_matrixsparse_data_idx_set(matrix, i, j, &data[i][j]);
                efl_assert_fail_if(r == EINA_FALSE);
           }
...
TEST(test_simple)
{
   ...
   efl_assert_fail_if(r == EINA_FALSE || cell == NULL);
   ...
   r = eina_matrixsparse_data_idx_replace(matrix,
                                          ...
                                          &value4,
                                          (void **)&test1);
   efl_assert_fail_if(r == EINA_FALSE || test1 != NULL);
   data[6][5] = value4;
   ...
   efl_assert_fail_if(r != EINA_TRUE);
   data[0][3] = 0;
   data[1][3] = 0;
   data[4][3] = 0;
   data[5][3] = 0;
...
TEST(test_iterators)
{
   ...
   EINA_ITERATOR_FOREACH(it, cell)
   {
      ...
      test1 = eina_matrixsparse_cell_data_get(cell);
      if (test1)
         efl_assert_fail_if(*test1 != data[row][col]);
   }
         eina_iterator_free(it);
