...
static void
eina_matrixsparse_free_cell_cb(void *user_data EFL_UNUSED,
                               void *cell_data EFL_UNUSED)
{}

static void
matrixsparse_initialize(Eina_Matrixsparse *matrix,
    ...
    for (i = 0; i < nrows; i++)
        for (j = 0; j < ncols; j++)
            if (data[i][j] != 0)
            {
                ...
                efl_assert_fail_if(r == EFL_FALSE);
            }
}

static void
matrixsparse_check(Eina_Matrixsparse  *matrix,
                   long                data[MAX_ROWS][MAX_COLS],
    ...
    for (i = 0; i < MAX_ROWS; i++)
        for (j = 0; j < MAX_COLS; j++)
        {
            if (data[i][j] != 0)
            {
                ...
                efl_assert_fail_if(test1 == NULL || *test1 != data[i][j]);
            }
            else
            {
                test1 = eina_matrixsparse_data_idx_get(matrix, i, j);
...
TEST(test_simple)
{
    ...
    matrix = eina_matrixsparse_new(MAX_ROWS,
                                   MAX_COLS,
                                   eina_matrixsparse_free_cell_cb,
                                   data);
    efl_assert_fail_if(matrix == NULL);

    r = eina_matrixsparse_cell_idx_get(matrix, 3, 5, &cell);
    efl_assert_fail_if(r == EFL_FALSE);
    efl_assert_fail_if(cell != NULL);
   ...
   /* data fetching */
    ...
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != data[3][0]);

    test1 = eina_matrixsparse_data_idx_get(matrix, 3, 5);
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != data[3][5]);

    test1 = eina_matrixsparse_data_idx_get(matrix, 3, 6);
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != data[3][6]);
    ...
    efl_assert_fail_if(r == EFL_FALSE);
    efl_assert_fail_if(row != 3 || col != 5);

    test1 = eina_matrixsparse_data_idx_get(matrix, 4, 3);
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != data[4][3]);

    test1 = eina_matrixsparse_data_idx_get(matrix, 1, 3);
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != data[1][3]);
   ...
   /* data changing */
    ...
    test1 = eina_matrixsparse_data_idx_get(matrix, 4, 3);
    efl_assert_fail_if(test1 == NULL || *test1 != value);
    ...
    test1 = eina_matrixsparse_data_idx_get(matrix, 3, 5);
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != value2);

    r = eina_matrixsparse_cell_idx_get(matrix, 4, 2, &cell);
    efl_assert_fail_if(r == EFL_FALSE || cell == NULL);
    ...
    efl_assert_fail_if(test1 == NULL || *test1 != value3);
    ...
    r = eina_matrixsparse_data_idx_replace(matrix,
                                           ...
                                           &value4,
                                           (void **)&test1);
    efl_assert_fail_if(r == EFL_FALSE || test1 != NULL);
    data[6][5] = value4;
   ...
   /* cell deletion */
    ...
    efl_assert_fail_if(!test1);
    efl_assert_fail_if(*test1 != data[5][3]);

    r = eina_matrixsparse_column_idx_clear(matrix, 3);
    efl_assert_fail_if(r != EFL_TRUE);
    data[0][3] = 0;
    data[1][3] = 0;
    data[4][3] = 0;
    data[5][3] = 0;
...
TEST(test_resize)
{
   ...
   /* cell insertion */
    ...
    efl_assert_fail_if(nrows != MAX_ROWS || ncols != MAX_COLS);
...
TEST(test_iterators)
{
    ...
    EINA_ITERATOR_FOREACH(it, cell)
    {
        ...
        test1 = eina_matrixsparse_cell_data_get(cell);
        efl_assert_fail_if(test1 == NULL || *test1 != data[row][col]);
    }
    ...
    EINA_ITERATOR_FOREACH(it, cell)
    {
        ...
        test1 = eina_matrixsparse_cell_data_get(cell);
        if (test1) efl_assert_fail_if(*test1 != data[row][col]);
    }
    eina_iterator_free(it);
