...
EFL_START_TEST(efl_shared_iterator_filter_simple)
{
...
EFL_START_TEST(efl_shared_iterator_filter_free)
{
  Eina_Iterator    *it, *filtered;
  Eina_Array       *ea;
...
EFL_START_TEST(efl_shared_iterator_array_simple)
{
  Eina_Iterator *it;
  Eina_Array    *ea;
  int           *tmp;
  ...
  fail_if(eina_iterator_next(it, (void **)&tmp) != EFL_FALSE);
...
EFL_START_TEST(efl_shared_iterator_hash_simple)
{
  Eina_Iterator *it;
  Eina_Hash     *hash;
  ...
  fail_if(eina_hash_add(hash, "7", &array[2]) != EFL_TRUE);
  fail_if(eina_hash_add(hash, "8", &array[3]) != EFL_TRUE);
...
EFL_START_TEST(efl_shared_iterator_inlist_simple)
{
...
EFL_START_TEST(efl_shared_iterator_list_simple)
{
  Eina_List     *list = NULL;
  Eina_Iterator *it;
...
EFL_START_TEST(efl_shared_reverse_iterator_list_simple)
{
  Eina_List     *list = NULL;
  Eina_Iterator *it;
  int            data[] = { 6, 9, 42, 1, 7, 1337, 81, 1664 };
                                       ...
                                       int                        *fdata)
{
  ...
  {
    case 0:
      fail_if(data->value != 10);
      break;

    case 1:
      fail_if(data->value != 27);
      break;

    case 2:
      fail_if(data->value != 42);
      break;

    case 3:
      fail_if(data->value != 69);
      break;

    case 4:
      fail_if(data->value != 1337);
      break;
  }
...
EFL_START_TEST(efl_shared_iterator_rbtree_simple)
{
...
EFL_START_TEST(efl_shared_iterator_carray_length)
{
...
EFL_START_TEST(efl_shared_iterator_multi)
{
  ...
  fail_if(j < EINA_C_ARRAY_LENGTH(array1) + EINA_C_ARRAY_LENGTH(array2) +
...
EFL_START_TEST(efl_shared_iterator_process)
{
...
{
  tcase_add_test(tc, efl_shared_iterator_array_simple);
  tcase_add_test(tc, efl_shared_iterator_hash_simple);
  tcase_add_test(tc, efl_shared_iterator_inlist_simple);
  tcase_add_test(tc, efl_shared_iterator_list_simple);
  tcase_add_test(tc, efl_shared_reverse_iterator_list_simple);
  tcase_add_test(tc, efl_shared_iterator_rbtree_simple);
  tcase_add_test(tc, efl_shared_iterator_filter_simple);
  tcase_add_test(tc, efl_shared_iterator_filter_free);
  tcase_add_test(tc, efl_shared_iterator_carray_length);
  tcase_add_test(tc, efl_shared_iterator_multi);
  tcase_add_test(tc, efl_shared_iterator_process);
}
