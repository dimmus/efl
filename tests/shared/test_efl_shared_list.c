...
EFL_START_TEST(efl_shared_test_list_simple)
{
    Eina_List *list = NULL;
    Eina_List *tmp;
    int       *test1;
...
EFL_START_TEST(efl_shared_test_list_merge)
{
    Eina_List *l1;
    Eina_List *l2;
    Eina_List *l3;
...
EFL_START_TEST(efl_shared_test_list_sorted_insert)
{
...
EFL_START_TEST(efl_shared_test_list_split)
{
    ...
    for (i = 0; i < 200; i++)
    {
        ...
        if (i % 2 == 0)
            fail_if(eina_list_count(left) == 1 &&
                    eina_list_count(right) + eina_list_count(left) == i + 7);
        else
            fail_if(eina_list_count(left) == 2 &&
                    eina_list_count(right) + eina_list_count(left) == i + 7);
...
EFL_START_TEST(efl_shared_test_list_shuffle)
{
    unsigned int *p;
    unsigned int  i;
...
EFL_START_TEST(efl_shared_test_list_clone)
{
    unsigned int  i;
    unsigned int *d, *rd;
...
EFL_START_TEST(efl_shared_test_list_move)
{
...
{
    tcase_add_test(tc, efl_shared_test_list_simple);
    tcase_add_test(tc, efl_shared_test_list_merge);
    tcase_add_test(tc, efl_shared_test_list_sorted_insert);
    tcase_add_test(tc, efl_shared_test_list_split);
    tcase_add_test(tc, efl_shared_test_list_shuffle);
    tcase_add_test(tc, efl_shared_test_list_clone);
    tcase_add_test(tc, efl_shared_test_list_move);
}
