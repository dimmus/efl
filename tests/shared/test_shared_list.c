...
static Efl_Bool
eina_list_sorted_check(const Eina_List *list)
{
    const Eina_List *n;
    void            *d;
    ...
    EINA_LIST_FOREACH(list->next, n, d)
    {
        ...
        {
            fprintf(stderr,
                    "list is not sorted: last=%d, current=%d\n",
                    last,
                    current);
            return 0;
        }
...
static int
eina_int_cmp(const void *a, const void *b)
{
    const int *ia = a;
    const int *ib = b;
...
TEST(test_list_simple)
{
    ...
    efl_assert_fail_if(test1 == NULL);
    efl_assert_fail_if(*test1 != 6);
    ...
    efl_assert_fail_if(eina_list_data_find_list(list, &data[2]) != NULL);
    efl_assert_fail_if(eina_list_data_find(list, &data[2]) != NULL);
    efl_assert_fail_if(eina_list_data_find(list, &data[5]) != &data[5]);

    efl_assert_fail_if(eina_list_count(list) != 5);
    efl_assert_fail_if(eina_list_nth(list, 4) != &data[3]);
    efl_assert_fail_if(eina_list_nth(list, 10) != NULL);
    efl_assert_fail_if(eina_list_nth_list(list, 10) != NULL);

    for (tmp = list, i = 0; tmp != NULL; tmp = eina_list_next(tmp), ++i)
    {
        int *d = eina_list_data_get(tmp);
        efl_assert_fail_if(d == NULL);
        efl_assert_fail_if(*d != result[i]);
    }
    ...
    for (tmp = list; tmp != NULL; tmp = eina_list_next(tmp), --i)
    {
        int *d = eina_list_data_get(tmp);
        efl_assert_fail_if(d == NULL);
        efl_assert_fail_if(*d != result[i - 1]);
    }
    ...
    efl_assert_fail_if(test1 == NULL || test2 == NULL || test3 == NULL);
    efl_assert_fail_if(*test1 > *test2);
    efl_assert_fail_if(*test3 == *test2);
    ...
    {
        int *d = eina_list_data_get(tmp);
        efl_assert_fail_if(*test1 > *d);

        test1 = d;
    }

    test3 = eina_list_nth(list, 5);
    efl_assert_fail_if(test3 == NULL);
    ...
    efl_assert_fail_if(list == NULL);
    efl_assert_fail_if(eina_list_data_get(list) != test1);
    efl_assert_fail_if(eina_list_data_get(eina_list_next(list)) != test2);
...
TEST(test_list_merge)
{
    ...
    efl_assert_fail_if(l1 == NULL);

    l2 = eina_list_append(NULL, &data[4]);
    l2 = eina_list_append(l2, &data[5]);
    efl_assert_fail_if(l2 == NULL);

    l1 = eina_list_merge(l1, l2);
    efl_assert_fail_if(l1 == NULL);
    efl_assert_fail_if(eina_list_count(l1) != 6);
    for (i = 0, l2 = l1; ((l2 != NULL) && (i < 6)); ++i, l2 = l2->next)
        efl_assert_fail_if(l2->data != &data[i]);

    efl_assert_fail_if(i != 6);
    efl_assert_fail_if(l2 != NULL);
    ...
    efl_assert_fail_if(l1 == NULL);
    efl_assert_fail_if(eina_list_count(l1) != 9);

    l1 = eina_list_sorted_merge(l1, l4, eina_int_cmp);
    efl_assert_fail_if(l1 == NULL);
    efl_assert_fail_if(eina_list_count(l1) != 12);

    l1 = eina_list_sorted_merge(l1, l5, eina_int_cmp);
    efl_assert_fail_if(l1 == NULL);
    efl_assert_fail_if(eina_list_count(l1) != 15);

    efl_assert_fail_if(!eina_list_sorted_check(l1));
}

TEST(test_list_sorted_insert)
{
    const int  data[]  = { 6,    9, 42, 1,  7, 9, 81,  1664,
                           1337, 3, 21, 10, 0, 5, 2008 };
    ...
    efl_assert_fail_if(*res != 1664);

    res = eina_list_search_sorted(l1, eina_int_cmp, &val);
    efl_assert_fail_if(res != NULL);
    ...
    efl_assert_fail_if(l2 == NULL);
    efl_assert_fail_if(!eina_list_sorted_check(l2));
    eina_list_free(l2);
...
TEST(test_list_split)
{
    ...
    efl_assert_fail_if(list == NULL);
    efl_assert_fail_if(eina_list_count(list) != 8);

    for (i = 0; i < 200; i++)
    {
        left =
            eina_list_split_list(list, eina_list_nth_list(list, i % 2), &right);
...
TEST(test_list_clone)
{
    ...
    efl_assert_fail_if(clist == NULL);

    for (i = 0; i < DATA_SIZE; i++)
    {
        efl_assert_fail_if(eina_list_nth(list, i) != eina_list_nth(clist, i));
    }

    rclist = eina_list_reverse_clone(list);
    efl_assert_fail_if(rclist == NULL);

    for (i = 0; i < DATA_SIZE; i++)
    {
        d  = eina_list_nth(list, i);
        rd = eina_list_nth(rclist, (DATA_SIZE - 1 - i));
        efl_assert_fail_if(d != rd);
    }
...
TEST(test_list_move)
{
    ...
    efl_assert_fail_if(eina_list_count(list1) != 5);
    efl_assert_fail_if(eina_list_count(list2) != 5);

    ret = eina_list_move(&list1, &list2, &data2[4]);
    efl_assert_fail_if(ret != EFL_TRUE);
    efl_assert_fail_if(eina_list_count(list1) != 6);
    efl_assert_fail_if(eina_list_count(list2) != 4);
    list_data = eina_list_nth(list1, 5);
    efl_assert_fail_if(*list_data != 10);

    ret = eina_list_move_list(&list1, &list2, eina_list_nth_list(list2, 1));
    efl_assert_fail_if(ret != EFL_TRUE);
    efl_assert_fail_if(eina_list_count(list1) != 7);
    efl_assert_fail_if(eina_list_count(list2) != 3);
    list_data = eina_list_nth(list1, 6);
    efl_assert_fail_if(*list_data != 7);
