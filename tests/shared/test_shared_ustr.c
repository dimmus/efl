   ...
   for (i = 0; i < sizeof(str)/sizeof(str[0]); ++i)
     {
        ...
        efl_assert_fail_if(!buf);
        efl_assert_fail_if(eina_unicode_strlen(buf) != eina_unicode_strlen(result_str[i]));
...
TEST(unicode_utf8)
{
   ...
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xC2\x80", &ind) != 0x80) ||
           (ind != 2));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xE0\xA0\x80", &ind) != 0x800) ||
           (ind != 3));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF0\x90\x80\x80", &ind) != 0x10000) ||
           (ind != 4));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF8\x88\x80\x80\x80", &ind) != 0x200000) || (ind != 5));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xFC\x84\x80\x80\x80\x80", &ind) != 0x4000000) || (ind != 6));
   ...
   efl_assert_fail_if((eina_unicode_utf8_next_get("\x7F", &ind) != 0x7F) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xDF\xBF", &ind) != 0x7FF) ||
           (ind != 2));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xEF\xBF\xBF", &ind) != 0xFFFF) ||
           (ind != 3));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF7\xBF\xBF\xBF", &ind) != 0x1FFFFF) ||
           (ind != 4));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xFB\xBF\xBF\xBF\xBF", &ind) != 0x3FFFFFF) || (ind != 5));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xFD\xBF\xBF\xBF\xBF\xBF", &ind) != 0x7FFFFFFF) || (ind != 6));

   /* Other boundary conditions */
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xED\x9F\xBF", &ind) != 0xD7FF) ||
           (ind != 3));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xEE\x80\x80", &ind) != 0xE000) ||
           (ind != 3));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xEF\xBF\xBD", &ind) != 0xFFFD) ||
           (ind != 3));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF4\x8F\xBF\xBF", &ind) != 0x10FFFF) ||
           (ind != 4));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF4\x90\x80\x80", &ind) != 0x110000) ||
           (ind != 4));
   ...
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xBF", &ind) != 0xDCBF) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\x80\xBF", &ind) != 0xDC80) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xBF\x80", &ind) != 0xDCBF) ||
           (ind != 1));
   ...
   for (ch = 0x80 ; ch <= 0xBF ; ch++)
     {
        char buf[] = {ch, 0};
        ind = 0;
        efl_assert_fail_if((eina_unicode_utf8_next_get(buf, &ind) != (0xDC00 | ch)) ||
                (ind != 1));
     }
     ...
     { \
        ...
        for (i = 0, ch = start ; ch <= end ; ch++) \
          { \
             efl_assert_fail_if((eina_unicode_utf8_next_get(buf, &ind) != (0xDC00 | ch)) || \
                     (ind != ++i)); \
             efl_assert_fail_if((eina_unicode_utf8_next_get(buf, &ind) != 0x20) || \
                     (ind != ++i)); \
          } \
     } \
   while (0)
...
#define _INCOMPLETE_SEQUENCES(first, conti, len) \
   ...
   /* Overlong sequences */
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xC0\xAF", &ind) != 0xDCC0) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xE0\x80\xAF", &ind) != 0xDCE0) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF0\x80\x80\xAF", &ind) != 0xDCF0) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF8\x80\x80\x80\xAF", &ind) != 0xDCF8) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xFC\x80\x80\x80\x80\xAF", &ind) != 0xDCFC) ||
           (ind != 1));

   /* Maximum overlong sequences */
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xC1\xBF", &ind) != 0xDCC1) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xE0\x9F\xBF", &ind) != 0xDCE0) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF0\x8F\xBF\xBF", &ind) != 0xDCF0) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xF8\x87\xBF\xBF\xBF", &ind) != 0xDCF8) ||
           (ind != 1));
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_next_get("\xFC\x83\xBF\xBF\xBF\xBF", &ind) != 0xDCFC) ||
           (ind != 1));
   /* Add some more error cases here */

   /* Just to cover prev/len. General utf-8 parsing was covered above */
   efl_assert_fail_if(eina_unicode_utf8_get_len("\xF4\x90\x80\x80\xF4\x8F\xBF\xBF") != 2);
   ind = 0;
   efl_assert_fail_if((eina_unicode_utf8_get_prev("\xED\x9F\xBF", &ind) != 0xD7FF) ||
           (ind != 0));
   ind = 3;
   efl_assert_fail_if((eina_unicode_utf8_get_prev("\xED\x9F\xBF", &ind) != 0x00) ||
           (ind != 0));
