...
thread_fn_cancel(void *arg, Eina_Thread t EFL_UNUSED)
{
  ...
  ck_assert(eina_thread_cancellable_set(EFL_TRUE, NULL));
...
EFL_START_TEST(efl_shared_thread_test_cleanup_execute)
{
  Eina_Thread t;
  int         flag = 0;
  ck_assert(
    eina_thread_create(&t, EINA_THREAD_NORMAL, -1, thread_fn_execute, &flag));
  eina_thread_join(t);
...
EFL_START_TEST(efl_shared_thread_test_cleanup_skip)
{
  Eina_Thread t;
  int         flag = 2;
  ck_assert(
...
EFL_START_TEST(efl_shared_thread_test_cancel)
{
  Eina_Thread    t;
  Eina_Lock      mutex;
  Eina_Condition cond;
  ...
  ck_assert(
    eina_thread_create(&t, EINA_THREAD_NORMAL, -1, thread_fn_cancel, &cond));
...
{
  tcase_add_test(tc, efl_shared_thread_test_cleanup_skip);
  tcase_add_test(tc, efl_shared_thread_test_cleanup_execute);
  tcase_add_test(tc, efl_shared_thread_test_cancel);
}
