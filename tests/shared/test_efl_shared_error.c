...
EFL_START_TEST(efl_shared_error_errno)
{
    int test;
...
EFL_START_TEST(efl_shared_error_test_find)
{
    int         test, r;
    const char *str;
...
EFL_START_TEST(efl_shared_error_test_modify)
{
    int         test, r;
...
EFL_START_TEST(efl_shared_error_test_lots)
{
    char         buf[64];
    int          codes[512];
    unsigned int i;
...
EFL_START_TEST(efl_shared_error_test_failures)
{
    struct log_ctx ctx;
...
#  define TEST_MAGIC_SAFETY(fn, _msg) \
    ...
    ck_assert_int_eq(eina_error_msg_modify(0, "X"), EFL_FALSE);

    TEST_MAGIC_SAFETY(
        "eina_error_msg_modify",
        "safety check failed: EINA_ERROR_REGISTERED_CHECK(error) is false");
    ck_assert_int_eq(eina_error_msg_modify(4096, "X"), EFL_FALSE);

    TEST_MAGIC_SAFETY("eina_error_msg_modify",
                      "safety check failed: msg == NULL");
    ck_assert_int_eq(eina_error_msg_modify(ENOMEM, NULL), EFL_FALSE);
    fail_unless(ctx.did);
    ...
    TEST_MAGIC_SAFETY("eina_error_msg_modify",
                      "safety check failed: msg == NULL");
    ck_assert_int_eq(eina_error_msg_modify(local_error, NULL), EFL_FALSE);
    fail_unless(ctx.did);
...
{
    tcase_add_test(tc, efl_shared_error_errno);
    tcase_add_test(tc, efl_shared_error_test_find);
    tcase_add_test(tc, efl_shared_error_test_modify);
    tcase_add_test(tc, efl_shared_error_test_lots);
#ifdef EINA_SAFETY_CHECKS
    tcase_add_test(tc, efl_shared_error_test_failures);
#endif
}
