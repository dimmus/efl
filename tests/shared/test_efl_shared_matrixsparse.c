...
EFL_START_TEST(efl_shared_test_simple)
{
  ...
  r =
    eina_matrixsparse_data_idx_replace(matrix, 6, 5, &value4, (void **)&test1);
  fail_if(r == EFL_FALSE || test1 != NULL);
  data[6][5] = value4;
...
EFL_START_TEST(efl_shared_test_resize)
{
  Eina_Matrixsparse *matrix = NULL;
  Efl_Bool           r;
  unsigned long      i, j;
...
EFL_START_TEST(efl_shared_test_iterators)
{
  Eina_Matrixsparse      *matrix = NULL;
  Eina_Matrixsparse_Cell *cell   = NULL;
  ...
  EINA_ITERATOR_FOREACH(it, cell)
  {
    ...
    test1 = eina_matrixsparse_cell_data_get(cell);
    if (test1) fail_if(*test1 != data[row][col]);
  }
  eina_iterator_free(it);
...
{
  tcase_add_test(tc, efl_shared_test_simple);
  tcase_add_test(tc, efl_shared_test_resize);
  tcase_add_test(tc, efl_shared_test_iterators);
}
