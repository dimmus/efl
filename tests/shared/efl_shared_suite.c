...
SUITE_INIT(efl_shared)
{
   ck_assert_int_eq(eina_init(), 1);
}

SUITE_SHUTDOWN(efl_shared)
{
...
EFL_START_TEST(mempool_module_load)
{
    _modules = eina_module_list_get(NULL,
                                 EFL_BUILD_DIR "/src/modules/shared",
                                 EFL_TRUE,
                                 NULL,
                                 NULL);
...
main(int argc, char **argv)
{
   ...
   failed_count = _efl_suite_build_and_run(argc - 1, (const char **)argv + 1,
                                           "eina_init_module", mp_etc, SUITE_INIT_FN(efl_shared), SUITE_SHUTDOWN_FN(efl_shared));

   eina_init();
   eina_module_list_load(_modules);
   failed_count += _efl_suite_build_and_run(argc - 1, (const char **)argv + 1,
                                           "Efl_Shared", etc, NULL, NULL);
