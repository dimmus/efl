...
EFL_START_TEST(core_test_core_main_loop_fd_handler)
{
   ...
   fd_handler = core_main_fd_handler_add
     (comm[0], CORE_FD_READ, _fd_handler_cb, &did, NULL, NULL);
...
EFL_START_TEST(core_test_core_main_loop_fd_handler_valid_flags)
{
   ...
   if (fd_handler)
	   core_main_fd_handler_del(fd_handler);
...
EFL_START_TEST(core_test_core_main_loop_fd_handler_activate_modify)
{
   ...
   fd_handler = core_main_fd_handler_add
     (comm[0], CORE_FD_ERROR, _fd_handler_cb, &did, NULL, NULL);
   fail_if(fd_handler == NULL);
...
_event_handler_cb(void *data, int type, void *event)
{
   ...
   if (*e1 == *e2 && (CORE_EVENT_CUSTOM_1 == *e1 || CORE_EVENT_CUSTOM_2 == *e1))
      (*did)++;
...
_event_free(void *user_data, void *func_data)
{
   ...
   if (CORE_EVENT_CUSTOM_1 == *ev || CORE_EVENT_CUSTOM_2 == *ev)
      (*did)++;
...
static Efl_Bool
_filter(void *data, void *loop_data EFL_UNUSED, int type EFL_UNUSED, void *event)
{
...
EFL_START_TEST(core_test_core_main_loop_event)
{
   ...
   filter_handler = core_event_filter_add(_filter_start, _filter, _filter_end, &filter);
