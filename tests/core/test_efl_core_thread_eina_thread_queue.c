...
static void
th1_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
  ...
  for (;;)
  {
    ...
    if (val == 1000 || (core_thread_check(th))) break;
    val++;
  }
}

static void
th2_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val;
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t1)
{
  int          val = 99;
  Core_Thread *eth1, *eth2;

  thq1 = eina_thread_queue_new();
  if (!thq1) fail();
  thq2 = eina_thread_queue_new();
  if (!thq2) fail();
  eth1 = core_thread_feedback_run(th1_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth2 = core_thread_feedback_run(th2_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  ...
  core_thread_wait(eth1, 0.1);
  core_thread_wait(eth2, 0.1);
...
static void
thspeed2_do(void *data EFL_UNUSED, Core_Thread *th)
{
  Msg2 *msg;
  void *ref;
  ...
  for (;;)
  {
    ...
    if (msgs == 1000 || (core_thread_check(th)))
    {
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t2)
{
  Msg2        *msg;
  void        *ref;
  int          i, mcount;
  ...
  thq1 = eina_thread_queue_new();
  if (!thq1) fail();
  th = core_thread_feedback_run(thspeed2_do, NULL, NULL, NULL, NULL, EFL_TRUE);
...
static void
th31_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
  ...
  for (;;)
  {
    ...
    eina_thread_queue_send_done(thq1, ref);
    val++;
    if (val == 1100 || (core_thread_check(th))) break;
  }
}

static void
th32_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t3)
{
  ...
  eth1 = core_thread_feedback_run(th31_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth2 = core_thread_feedback_run(th32_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  for (;;)
  {
    ...
    if (DEBUG)
      printf("V %09i: %p - %i  [%i]\n",
             ...
             thq,
             msg->value,
             eina_thread_queue_pending_get(thqmaster));
    if (thq == thq1)
    {
      if ((val1 + 1) != msg->value)
      {
...
static void
th41_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
...
static void
th42_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 10000;
  ...
  for (;;)
  {
    ...
    eina_thread_queue_send_done(thq1, ref);
    val++;
    if (val == 11000 || (core_thread_check(th))) break;
  }
}

EFL_START_TEST(core_test_core_thread_eina_thread_queue_t4)
{
  int          cnt  = 0;
  ...
  thq1 = eina_thread_queue_new();
  if (!thq1) fail();
  eth1 = core_thread_feedback_run(th41_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth2 = core_thread_feedback_run(th42_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  for (;;)
  {
    Msg4 *msg;
    void *ref;
    ...
    if (DEBUG)
      printf("V %08i: %i  [%i]\n",
             cnt,
             msg->value,
             eina_thread_queue_pending_get(thq1));
    if (msg->value >= 10000)
    {
      if ((val2 + 1) != msg->value)
      {
...
static void
th51_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
...
static void
th52_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val;
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t5)
{
  int          val = 99;
  Core_Thread *eth1, *eth2;
  ...
  thq2 = eina_thread_queue_new();
  if (!thq2) fail();
  eth1 = core_thread_feedback_run(th51_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth2 = core_thread_feedback_run(th52_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  ...
  for (;;)
  {
    ...
    if (msg)
    {
      if (DEBUG)
        printf("V: %i   [%i]\n",
               msg->value,
               eina_thread_queue_pending_get(thq2));
      if (msg->value != (val + 1))
      {
        ck_abort_msg("ERRR %i not next after %i\n", msg->value, val);
      }
...
static void
th61_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int val = 100;
...
static void
th62_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int cnt = 0;
  ...
  for (;;)
  {
    ...
    val = msg->value;
    eina_thread_queue_wait_done(thq1, ref);
    if (val == EXIT_MESSAGE || (core_thread_check(th))) break;
    cnt++;
...
static void
th63_do(void *data EFL_UNUSED, Core_Thread *th)
{
  int cnt = 0;
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t6)
{
  int          do_break = 0;
  Msg6        *msg;
  void        *ref;
  Core_Thread *eth1, *eth2, *eth3;
  ...
  thq1 = eina_thread_queue_new();
  fail_if(!thq1);
  eth1 = core_thread_feedback_run(th61_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth2 = core_thread_feedback_run(th62_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  eth3 = core_thread_feedback_run(th63_do, NULL, NULL, NULL, NULL, EFL_TRUE);
...
static void
thspeed7_do(void *data EFL_UNUSED, Core_Thread *th)
{
  Msg7 *msg;
  void *ref;
  int   i;
...
EFL_START_TEST(core_test_core_thread_eina_thread_queue_t7)
{
  Msg7        *msg;
  void        *ref;
  int          msgcnt = 0, ret;
  ...
  eth1 =
    core_thread_feedback_run(thspeed7_do, NULL, NULL, NULL, NULL, EFL_TRUE);
  for (;;)
  {
    char buf;
...
void
core_test_core_thread_eina_thread_queue(TCase *tc EFL_UNUSED)
{
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t1);
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t2);
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t3);
}

void
core_test_core_thread_eina_thread_queue2(TCase *tc EFL_UNUSED)
{
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t4);
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t5);
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t6);
  tcase_add_test(tc, core_test_core_thread_eina_thread_queue_t7);
}
