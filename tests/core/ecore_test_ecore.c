...
static Efl_Bool
_fd_handler_cb(void *data, Ecore_Fd_Handler *handler EFL_UNUSED)
{
...
EFL_START_TEST(ecore_test_ecore_main_loop_fd_handler)
{
    ...
    fd_handler = ecore_main_fd_handler_add(comm[0],
                                           ECORE_FD_READ,
                                           _fd_handler_cb,
                                           &did,
                                           NULL,
...
EFL_START_TEST(ecore_test_ecore_main_loop_fd_handler_valid_flags)
{
    ...
    fd_handler =
        ecore_main_fd_handler_add(comm[0], 0, _fd_handler_cb, NULL, NULL, NULL);
    fail_if(fd_handler != NULL);

    if (fd_handler) ecore_main_fd_handler_del(fd_handler);
...
_event_handler_cb(void *data, int type, void *event)
{
    ...
    if (*e1 == *e2 &&
        (ECORE_EVENT_CUSTOM_1 == *e1 || ECORE_EVENT_CUSTOM_2 == *e1))
        (*did)++;
...
_event_free(void *user_data, void *func_data)
{
    ...
    if (ECORE_EVENT_CUSTOM_1 == *ev || ECORE_EVENT_CUSTOM_2 == *ev) (*did)++;
...
EFL_START_TEST(ecore_test_ecore_main_loop_event)
{
    ...
    filter_handler =
        ecore_event_filter_add(_filter_start, _filter, _filter_end, &filter);
