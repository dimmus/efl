...
EFL_START_TEST(efl_app_test_efl_provider_register)
{
  ...
  n = efl_add(EFL_LOOP_TIMER_CLASS,
              efl_app_main_get(),
...
static void
efl_app_test_efl_loop_concentric_fail(void *data          EFL_UNUSED,
...
static void
loop_idle_enter(void *data EFL_UNUSED, const Efl_Event *ev EFL_UNUSED)
{
  static int num = 0;
...
static void
loop_idle(void *data, const Efl_Event *ev EFL_UNUSED)
{
  efl_loop_iterate(data);
}

static void
loop_timer_tick(void *data, const Efl_Event *ev EFL_UNUSED)
{
  efl_loop_iterate(data);
}
...
EFL_START_TEST(efl_app_test_efl_loop_concentric)
{
  ...
  efl_event_callback_add(loop,
                         EFL_LOOP_EVENT_IDLE_ENTER,
                         loop_idle_enter,
                         NULL);
  timer    = efl_add(EFL_LOOP_TIMER_CLASS,
                  loop2,
                  ...
                  efl_event_callback_add(efl_added,
                                         ...
                                         loop_timer_tick,
                                         loop));
  timer2   = efl_add(EFL_LOOP_TIMER_CLASS,
                   loop,
                   efl_loop_timer_interval_set(efl_added, 0.5),
                   efl_event_callback_add(efl_added,
                                          EFL_LOOP_TIMER_EVENT_TIMER_TICK,
                                          efl_app_test_efl_loop_concentric_fail,
                                          NULL));
...
{
  ck_assert_str_eq(efl_class_name_get(efl_loop_realized_class_get()),
                   "Efl.Loop_Realized");
}
...
void
efl_app_test_efl_loop(TCase *tc)
{
