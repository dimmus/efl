...
Efl_Bool _poller_handler(void *data)
{
   int *val = data;
   (*val)++;
...
EFL_START_TEST(core_test_core_main_loop_poller_add)
{
   ...
   Core_Poller *poll_ptr = core_poller_add(CORE_POLLER_CORE, 1, _poller_loop_add, NULL);
...
{
   /* Create renewable main poller */
   Core_Poller *poller = core_poller_add(CORE_POLLER_CORE, 2, _poller_loop_fail, NULL);
...
EFL_START_TEST(core_test_core_main_loop_poller)
{
   ...
   fail_unless(CHECK_FP(core_poller_poll_interval_get(CORE_POLLER_CORE), interval));

   /* Create three pollers with different poller interval */
   poll1_ptr = core_poller_add(CORE_POLLER_CORE, poll1_interval, _poller_cb, &poll1_counter);
   poll2_ptr = core_poller_add(CORE_POLLER_CORE, poll2_interval, _poller_cb, &poll2_counter);
   poll3_ptr = core_poller_add(CORE_POLLER_CORE, poll3_interval, _poller_cb, &poll3_counter);
