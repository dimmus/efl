...
_timer2_cb(void *data)
{
   ...
   // check set new precision
    ...
    ck_assert_msg(EINA_DBL_EQ(timer->precision[timer->num_elem],
                              core_timer_precision_get()),
...
EFL_START_TEST(core_test_timers)
{
    ...
    ck_assert_msg((timer.timer1 || timer.timer2 || timer.timer3 ||
                   timer.timer4 || timer.timer5),
                  "Error add timer\n");
