...
_timer2_cb(void *data)
{
   ...
   ck_assert_msg(EINA_DBL_EQ(timer->interval_1[timer->num_elem], core_timer_interval_get(timer->timer1)), "Error set new interval");

   // check set new precision
   core_timer_precision_set(timer->precision[timer->num_elem]);
   ck_assert_msg(EINA_DBL_EQ(timer->precision[timer->num_elem], core_timer_precision_get()), "Error set new precision");
...
EFL_START_TEST(core_test_timers)
{
   ...
   if (timer.timer1)
     core_timer_del(timer.timer1);
   if (timer.timer2)
     core_timer_del(timer.timer2);
   if (timer.timer3)
     core_timer_del(timer.timer3);
   if (timer.timer4)
     core_timer_del(timer.timer4);
   if (timer.timer5)
     core_timer_del(timer.timer5);
}
EFL_END_TEST
...
{
   Core_Timer *t = NULL;
   ck_assert_msg(!(t = core_timer_add(0.5, NULL, NULL)), "ERROR: Invalid callback func!\n");
}
EFL_END_TEST
