...
{
    Efl_Bool success_called;
    Efl_Bool error_called;
    Efl_Bool free_called;
    Efl_Bool stop_loop;
} Easy_Ctx;
      ...
      {                                                                   \
          struct Func_Ctx tmp[] = {                                       \
              ...
              {NULL, NULL}                                              \
                                                                          ...
                                                                          \
          _ctx.func_ctx =                                                 \
              alloca(EINA_C_ARRAY_LENGTH(tmp) * sizeof(struct Func_Ctx)); \
          memcpy(_ctx.func_ctx, tmp, sizeof(tmp));                        \
          _ctx.level        = EINA_LOG_LEVEL_ERR;                         \
          _ctx.did          = EFL_FALSE;                                  \
          _ctx.just_fmt     = EFL_FALSE;                                  \
          _ctx.func_ctx_idx = 0;                                          \
      }                                                                   \
      while (0)

#  define LOG_CTX_SET(_ctx, _fnc, _msg) \
      LOG_CTX_MULTIPLE_FUNC_CTX_SET(_ctx, { _fnc, _msg })
...
static void
_cancel(void *data, const Eina_Promise *dead_ptr EFL_UNUSED)
{
    PromiseCtx *ctx = data;
...
static void
_promise_cancel_test(void *data, const Eina_Promise *dead_ptr EFL_UNUSED)
{
    Efl_Bool *cancel_called = data;
    *cancel_called          = EFL_TRUE;
}

static void
_simple_timeout(void *data, const Efl_Event *ev EFL_UNUSED)
{
    PromiseCtx *ctx = data;
...
_future_get(PromiseCtx *ctx, double timeout)
{
    ...
    fail_if(!f);
    ctx->t = efl_add(EFL_LOOP_TIMER_CLASS,
                     efl_main_loop_get(),
...
{
    return _int_future_with_value_and_timeout(DEFAULT_INT_VALUE,
                                              DEFAULT_TIMEOUT);
}
...
static Eina_Value
_simple_err(void *data                     EFL_UNUSED,
            const Eina_Value               v,
...
static Eina_Value
_simple_ok(void *data                     EFL_UNUSED,
           const Eina_Value               v,
           const Eina_Future *dead_future EFL_UNUSED)
{
    const char *msg;
...
static void
_inner_promise_cancel(void *data                   EFL_UNUSED,
                      const Eina_Promise *dead_ptr EFL_UNUSED)
{
...
static Eina_Value
_future_promise_create(void                          *data,
...
static Eina_Value
_inner_future_last(void                          *data,
                   const Eina_Value               v,
                   const Eina_Future *dead_future EFL_UNUSED)
{
    if (data) ERROR_CHECK(v, DEFAULT_ERROR);
    else
    {
        const char *msg;
        VALUE_TYPE_CHECK(v, EINA_VALUE_TYPE_STRING);
...
static Eina_Value
_convert_check(void *data                     EFL_UNUSED,
               const Eina_Value               v,
               const Eina_Future *dead_future EFL_UNUSED)
{
    const char *number;
...
_easy_error(void *data, const Eina_Error err)
{
    ...
    ctx->error_called = EFL_TRUE;
    return v;
}

static void
_easy_free(void *data, const Eina_Future *dead_future EFL_UNUSED)
{
    Easy_Ctx *ctx    = data;
    ctx->free_called = EFL_TRUE;
    if (ctx->stop_loop) ecore_main_loop_quit();
}

static Eina_Value
_all_cb(void *data, const Eina_Value array, const Eina_Future *dead EFL_UNUSED)
{
...
static Eina_Value
_race_cb(void *data, const Eina_Value v, const Eina_Future *dead EFL_UNUSED)
{
    ...
    else fail_if(EFL_TRUE); //This is not supposed to happen!
    free(future_ctx);
    return v;
}

static Eina_Value
_race_end_cb(void *data, const Eina_Value v, const Eina_Future *dead EFL_UNUSED)
{
    Race_Ctx          *ctx = data;
    unsigned int       idx;
...
static Eina_Value
_promise_empty_done(void                          *data,
                    const Eina_Value               value,
                    const Eina_Future *dead_future EFL_UNUSED)
{
    Efl_Bool *pdone = data;

    if (!value.type) *pdone = EFL_TRUE;
...
EFL_START_TEST(efl_test_promise_future_success)
{
    Eina_Future *f;
    fail_if(!ecore_init());
    f = eina_future_then(_str_future_get(), _simple_ok, NULL, NULL);
    fail_if(!f);
    ecore_main_loop_begin();
    ecore_shutdown();
}
...
EFL_START_TEST(efl_test_promise_future_failure)
{
    Eina_Future *f;
    fail_if(!ecore_init());
    f = eina_future_then(_fail_future_get(), _simple_err, NULL, NULL);
    fail_if(!f);
    ecore_main_loop_begin();
    ecore_shutdown();
}
...
EFL_START_TEST(efl_test_promise_future_cancel)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        int           cancel_count  = 0;
        Efl_Bool      cancel_called = EFL_FALSE;

        p = eina_promise_new(_future_scheduler_get(),
                             _promise_cancel_test,
                             &cancel_called);
        fail_if(!p);
        ...
        if (i == 2)
        {
            Eina_Future *f;
            last = NULL;
            f    = eina_future_then(first, _cancel_cb, &cancel_count, NULL);
            fail_if(!f);
            middle = eina_future_then(f, _cancel_cb, &cancel_count, NULL);
            fail_if(!middle);
            f = eina_future_then(middle, _cancel_cb, &cancel_count, NULL);
            fail_if(!f);
        }
        else
        {
            middle = NULL;
            last =
                eina_future_chain(first,
                                  { .cb = _cancel_cb, .data = &cancel_count },
                                  { .cb = _cancel_cb, .data = &cancel_count },
                                  { .cb = _cancel_cb, .data = &cancel_count });
            fail_if(!last);
        }
        if (i == 0) eina_future_cancel(last);
        else if (i == 1) eina_future_cancel(first);
        else eina_future_cancel(middle);
...
EFL_START_TEST(efl_test_promise_future_easy)
{
    ...
    ecore_shutdown();
    fail_if(
        !(easy1.success_called && !easy1.error_called && easy1.free_called));
    fail_if(
        !(easy2.success_called && !easy2.error_called && easy2.free_called));
    fail_if(
        !(!easy3.success_called && easy3.error_called && easy3.free_called));
}
EFL_END_TEST
...
EFL_START_TEST(efl_test_promise_future_all)
{
    ...
    {
        Eina_Future *f;
        if (i % 2 == 0) f = _str_future_get();
        else f = _int_future_get();
        fail_if(!f);
        futures[i] =
            eina_future_then(f, _future_all_count, &futures_called, NULL);
        fail_if(!futures[i]);
    }

    futures[--len] = EINA_FUTURE_SENTINEL;
    fail_if(
        !eina_future_then(eina_future_all_array(futures), _all_cb, &len, NULL));
    ecore_main_loop_begin();
    ecore_shutdown();
...
EFL_START_TEST(efl_test_promise_future_all_iterator)
{
    ...
    fail_if(
        !eina_future_then(eina_future_all_iterator(it), _all_cb, &len, NULL));
...
EFL_START_TEST(efl_test_promise_future_race)
{
    ...
    double timeouts[10] = { 2.0, 1.0, 0.5, 0.1, 4.5, 2.3, 5.6, 1.0, 0.5, 0.3 };
    ...
    for (i = 0; i < len - 1; i++)
    {
        ...
        futures[i]           = eina_future_then(
            _int_future_with_value_and_timeout(future_ctx->value, timeouts[i]),
            _race_cb,
            future_ctx,
            NULL);
        fail_if(!futures[i]);
...
static Eina_Value
_eo_future1_ok(Eo *eo EFL_UNUSED, void *data EFL_UNUSED, const Eina_Value v)
{
    const char *number;
...
static Eina_Value
_eo_future1_err(Eo *eo EFL_UNUSED, void *data EFL_UNUSED, Eina_Error err)
{
   //Should not happen
    fail_if(EFL_TRUE);
...
static Eina_Value
_eo_future2_ok(Eo *eo EFL_UNUSED, void *data EFL_UNUSED, const Eina_Value v)
{
   //Should not happen
    fail_if(EFL_TRUE);
    return v;
...
static Eina_Value
_eo_future2_err(Eo *eo EFL_UNUSED, void *data EFL_UNUSED, Eina_Error err)
{
    Eina_Value v;
...
static Eina_Value
_eo_chain_stop(void *data                     EFL_UNUSED,
               const Eina_Value               v,
               const Eina_Future *dead_future EFL_UNUSED)
{
    int ivalue = 0;
   ...
   //Use a random object..
    ...
    f = eina_future_chain(_int_future_get(),
                          ...
                          efl_future_cb(obj,
                                        ...
                                        _eo_future1_err,
                                        _eo_future_free,
                                        EINA_VALUE_TYPE_STRING, NULL, NULL),
                          efl_future_cb(obj,
                                        _eo_future2_ok,
                                        _eo_future2_err,
                                        _eo_future_free,
                                        EINA_VALUE_TYPE_INT, NULL, NULL),
                          { .cb = _eo_chain_stop });
    fail_if(!f);
    ecore_main_loop_begin();
    efl_unref(obj);
...
static Eina_Value
_eo_future_link_success(Eo *eo           EFL_UNUSED,
                        void *data       EFL_UNUSED,
                        const Eina_Value v)
{
...
static Eina_Value
_eo_future_link_err(Eo *eo, void *data EFL_UNUSED, Eina_Error err)
{
...
static Eina_Value
_eo_link_chain_end(void *data              EFL_UNUSED,
                   const Eina_Value        v,
                   const Eina_Future *dead EFL_UNUSED)
{
    int *err_called = data;
...
EFL_START_TEST(efl_test_promise_eo_link)
{
   ...
   //Use a random object..
    ...
    f = efl_future_chain(
        ...
        { .success = _eo_future_link_success, .error = _eo_future_link_err });
    fail_if(!f);
    f = eina_future_then(f, _eo_link_chain_end, &err_called, NULL);
    fail_if(!f);
   /*
...
static Eina_Value
_err_ignored(void *data           EFL_UNUSED,
             const Eina_Value     v,
...
EFL_START_TEST(efl_test_promise_future_ignore_error)
{
    ...
    fail_if(!f);
    eina_future_chain(f,
                      eina_future_cb_ignore_error(DEFAULT_ERROR),
                      { .cb = _err_ignored });
    ecore_main_loop_begin();
    ecore_shutdown();
}
...
typedef struct _Promise_Log_Ctx
{
    ...
    Efl_Bool                dbg_log_ok;
    Efl_Bool                crit_log_ok;
    Efl_Bool                warn_log_ok;
    Efl_Bool                info_log_ok;
    Efl_Bool                err_log_ok;
} Promise_Log_Ctx;

static Eina_Value
_log_quit(void *data              EFL_UNUSED,
          const Eina_Value        v,
          const Eina_Future *dead EFL_UNUSED)
{
    int ivalue;
          ...
          va_list                args)
{
    ...
    if (strcmp(d->name, PROMISE_LOG_DOMAIN_STR)) return;
...
EFL_START_TEST(efl_test_promise_log)
{
    ...
    ctx.dbg =
        (Eina_Future_Cb_Log_Desc){ "dbg prefix:", " dbg suffix",      __FILE__,
                                   __func__,      EINA_LOG_LEVEL_DBG, domain,
                                   __LINE__ };
    ctx.crit = (Eina_Future_Cb_Log_Desc){
        NULL,   NULL,    __FILE__, __func__, EINA_LOG_LEVEL_CRITICAL,
        domain, __LINE__
    };
    ctx.warn = (Eina_Future_Cb_Log_Desc){
        "warn prefix:",      NULL,   __FILE__, __func__,
        EINA_LOG_LEVEL_WARN, domain, __LINE__
    };
    ctx.err = (Eina_Future_Cb_Log_Desc){ NULL,     " err suffix",      __FILE__,
                                         __func__, EINA_LOG_LEVEL_ERR, domain,
                                         __LINE__ };
    ctx.info = (Eina_Future_Cb_Log_Desc){
        "info prefix:",      " info suffix", __FILE__, __func__,
        EINA_LOG_LEVEL_INFO, domain,         __LINE__
    };
...
static void
_dummy_cancel(void *data EFL_UNUSED, const Eina_Promise *dead EFL_UNUSED)
{}
...
EFL_START_TEST(efl_test_promise_null)
{
    ...
    LOG_CTX_SET(ctx,
                "eina_promise_new",
                "safety check failed: scheduler == NULL");
    ...
    LOG_CTX_SET(ctx,
                "eina_promise_new",
                "safety check failed: cancel_cb == NULL");
...
static Eina_Value
_future_easy_null_success(void *data EFL_UNUSED, const Eina_Value v)
{
   //Should not happen
    fail_if(EFL_TRUE);
    return v;
...
EFL_START_TEST(efl_test_future_null)
{
    ...
    LOG_CTX_SET(ctx,
                ...
                "safety check failed: (prev) == NULL");
    eina_log_print_cb_set(_eina_test_safety_print_cb, &ctx);
    f = eina_future_then(NULL, _future_null_cb, &cb_called, NULL);
    ...
    f = eina_future_chain(NULL,
                          eina_future_cb_easy(_future_easy_null_success,
                                              ...
                                              NULL,
                                              &easy_cb_calls),
                          { _future_null_cb, &cb_called, NULL },
                          { _future_null_cb, &cb_called, NULL },
                          { _future_null_cb, &cb_called, NULL },
                          { _future_null_cb, &cb_called, NULL },
                          { _future_null_cb, &cb_called, NULL });
...
static Eina_Value
_future_race_all_null_cb(void                   *data,
                         const Eina_Value        v,
                         const Eina_Future *dead EFL_UNUSED)
{
    int  err;
...
EFL_START_TEST(efl_test_future_all_null)
{
    ...
    LOG_CTX_MULTIPLE_FUNC_CTX_SET(
        ctx,
        { "promise_proxy_of_future_array_create",
          "safety check failed: array[i] == NULL" },
...
static Eina_Value
test_got_int(Eo *o EFL_UNUSED, void *data EFL_UNUSED, const Eina_Value value)
{
    int i = 0;
...
static Eina_Value
test_error_not_reached(Eo *o            EFL_UNUSED,
                       void *data       EFL_UNUSED,
                       Eina_Error error EFL_UNUSED)
{
    abort();
...
static Eina_Value
test_not_reached(Eo *o            EFL_UNUSED,
                 void *data       EFL_UNUSED,
                 const Eina_Value value)
{
    abort();
...
static Eina_Value
test_failure(Eo *o, void *data EFL_UNUSED, Eina_Error error)
{
...
static void
test_cleanup(Eo *o, void *data, const Eina_Future *dead_future EFL_UNUSED)
{
    ck_assert_ptr_eq(o, efl_main_loop_get());
    ck_assert_ptr_eq(data, test_strings[1]);
...
static void
efl_main_test(void *data EFL_UNUSED, const Efl_Event *ev)
{
    Eina_Future *f = { 0 };

    f = efl_future_chain(
        ev->object,
        efl_loop_idle(ev->object),
        { .success = test_generate_int, .data = test_strings[0], NULL },
        { .success_type = EINA_VALUE_TYPE_INT,
          .success      = test_got_int,
          .error        = test_error_not_reached,
        },
        { .success = test_not_reached, .error = test_failure, NULL });
    efl_future_then(ev->object,
                    f,
                    .success = test_not_reached,
                    .error   = test_end_future,
                    .data    = test_strings[1],
                    .free    = test_cleanup);
}
...
void
efl_app_test_promise_2(TCase *tc)
{
    tcase_add_checked_fixture(tc, promise_init, promise_shutdown);
...
void
efl_app_test_promise_3(TCase *tc)
{
    tcase_add_checked_fixture(tc, promise_init, promise_shutdown);
...
void
efl_app_test_promise_safety(TCase *tc)
{
#ifdef EINA_SAFETY_CHECKS
    tcase_add_checked_fixture(tc, promise_init, promise_shutdown);
