...
EFL_START_TEST(efl_test_promise_future_easy)
{
  ...
  f = eina_future_then_easy(f,
                            ...
                            &easy2);
  fail_if(!f);
  f = eina_future_chain_easy(
    f,
    { _easy_success, _easy_error, _easy_free, EINA_VALUE_TYPE_INT, &easy3 });
  fail_if(!f);
  core_main_loop_begin();
  core_shutdown();
...
EFL_START_TEST(efl_test_promise_future_all)
{
  ...
  for (i = 0; i < len - 1; i++)
  {
    ...
    fail_if(!f);
    futures[i] = eina_future_then(f, _future_all_count, &futures_called, NULL);
    fail_if(!futures[i]);
  }
...
EFL_START_TEST(efl_test_promise_future_all_iterator)
{
  ...
  fail_if(!eina_future_then(eina_future_all_iterator(it), _all_cb, &len, NULL));
...
EFL_START_TEST(efl_test_promise_log)
{
  ...
  ctx.crit = (Eina_Future_Cb_Log_Desc){
    NULL, NULL, __FILE__, __func__, EINA_LOG_LEVEL_CRITICAL, domain, __LINE__
  };
  ctx.warn =
    (Eina_Future_Cb_Log_Desc){ "warn prefix:",      NULL,   __FILE__, __func__,
  ...
  ctx.err = (Eina_Future_Cb_Log_Desc){ NULL,     " err suffix",      __FILE__,
                                       ...
                                       __LINE__ };
  ctx.info =
    (Eina_Future_Cb_Log_Desc){ "info prefix:", " info suffix",      __FILE__,
                               __func__,       EINA_LOG_LEVEL_INFO, domain,
                               __LINE__ };
