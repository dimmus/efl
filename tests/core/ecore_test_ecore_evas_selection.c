   ...
   //this is just running this and checking that we do not error
    for (int i = 0; i < ECORE_EVAS_SELECTION_BUFFER_LAST; ++i)
    {
        ...
        Eina_Content *content =
            eina_content_new((Eina_Slice)EINA_SLICE_STR_FULL("asdf"),
                             "text/plain");
        ck_assert_int_eq(ecore_evas_selection_set(ee, 0, i, content), EFL_TRUE);
    }
    for (int i = 0; i < ECORE_EVAS_SELECTION_BUFFER_LAST; ++i)
    {
        if (i == ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER) continue;
...
EFL_END_TEST
void
ecore_test_ecore_evas_selection(TCase *tc)
{
