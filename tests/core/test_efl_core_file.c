...
static void
file_monitor_cb(void *data EFL_UNUSED, Core_File_Monitor *em EFL_UNUSED,
                Core_File_Event event, const char *path)
{
   switch (event)
     {
...
EFL_START_TEST(core_test_core_file_download)
{
   ...
   res = core_file_download("xxyyzz", dest_name, completion_cb,
                             progress_cb, NULL, &job);
   fail_if(res != EFL_FALSE);

   int status = 0;
   res = core_file_download(download_url, dest_name, err_completion_cb,
   ...
   res = core_file_download_full(download_url, dest_name, completion_cb,
