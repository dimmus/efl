...
static Eina_Value
_verify_and_exit(void *data, const Eina_Value value, const Eina_Future *dead_future EFL_UNUSED)
{
...
EFL_START_TEST(core_test_core_evas_fallback_selection)
{
   ...
   core_evas_selection_set(ee, 0, CORE_EVAS_SELECTION_BUFFER_SELECTION_BUFFER, content);

   const char *types[] = {eina_stringshare_add("text/plain")};

   Eina_Future *f = core_evas_selection_get(ee, 0, CORE_EVAS_SELECTION_BUFFER_SELECTION_BUFFER, EINA_C_ARRAY_ITERATOR_NEW(types));
