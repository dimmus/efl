...
_poller_loop_del(void *data)
{
    ...
    if (count++ == 0) ecore_poller_del(data);
...
EFL_START_TEST(ecore_test_ecore_main_loop_poller_add)
{
   /* Create renewable main poller */
    Ecore_Poller *poll_ptr =
        ecore_poller_add(ECORE_POLLER_CORE, 1, _poller_loop_add, NULL);
...
EFL_START_TEST(ecore_test_ecore_main_loop_poller_del)
{
   /* Create renewable main poller */
    Ecore_Poller *poller =
        ecore_poller_add(ECORE_POLLER_CORE, 2, _poller_loop_fail, NULL);
...
EFL_START_TEST(ecore_test_ecore_main_loop_poller_modify)
{
   /* Create renewable main poller */
    Ecore_Poller *poller =
        ecore_poller_add(ECORE_POLLER_CORE, 4, _quit_cb, NULL);
...
EFL_START_TEST(ecore_test_ecore_main_loop_poller)
{
   ...
   /* Check ECORE_POLLER_CORE poll interval */
    ...
    fail_unless(
        CHECK_FP(ecore_poller_poll_interval_get(ECORE_POLLER_CORE), interval));

   /* Create three pollers with different poller interval */
    poll1_ptr = ecore_poller_add(ECORE_POLLER_CORE,
                                 poll1_interval,
                                 _poller_cb,
                                 &poll1_counter);
    poll2_ptr = ecore_poller_add(ECORE_POLLER_CORE,
                                 poll2_interval,
                                 _poller_cb,
                                 &poll2_counter);
    poll3_ptr = ecore_poller_add(ECORE_POLLER_CORE,
                                 poll3_interval,
                                 _poller_cb,
                                 &poll3_counter);
...
void
ecore_test_ecore_poller(TCase *tc)
{
