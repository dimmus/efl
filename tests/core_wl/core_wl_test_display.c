...
#include "core_wl_tests_helpers.h"
...
EFL_START_TEST(wl2_display_name_get)
{
    ...
    disp_name = ecore_wl_display_name_get(disp);
    ck_assert(disp_name != NULL);
}
EFL_END_TEST
                                    ...
                                    void *event EFL_UNUSED)
{
   ...
   /* NB: Enlightenment uses "seat0" here, but Weston uses "default" */
    if (getenv("E_START"))
        test_input = ecore_wl_display_input_find_by_name(td->display, "seat0");
    else if (getenv("WESTON"))
        test_input =
    ...
    if (getenv("E_START"))
    {
        test_input = ecore_wl_display_input_find(td->display, 13);
        ck_assert(test_input != NULL);
    }
...
EFL_START_TEST(wl2_display_sync_is_done)
{
    ...
    ecore_event_handler_add(ECORE_WL2_EVENT_SYNC_DONE, _test_sync_done, td);
...
void
ecore_wl_test_display(TCase *tc)
{
