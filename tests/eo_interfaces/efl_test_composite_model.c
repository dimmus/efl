...
static void
_cleanup(Eo *o EFL_UNUSED, void *data EFL_UNUSED, const Eina_Future *dead_future EFL_UNUSED)
{
   core_main_loop_quit();
}
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, child)
     {
        ...
        assert(eina_value_int_get(p_int, &v_int));
        assert(eina_value_bool_get(p_true, &v_true));
        assert(eina_value_bool_get(p_false, &v_false));

        all[i] = efl_model_property_set(child, "test_odd_even", v_int & 0x1 ? p_true : p_false);

        efl_assert_fail_if(v_int != base_ints[i]);
        efl_assert_fail_if(v_true != EFL_TRUE);
        efl_assert_fail_if(v_false != EFL_FALSE);
...
static Eina_Value
_children_odd_even_ready(Eo *model, void *data EFL_UNUSED, const Eina_Value v EFL_UNUSED)
{
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, child)
     {
        ...
        assert(eina_value_bool_get(p_odd_even, &v_odd_even));
        assert(eina_value_int_get(p_int, &v_int));

        efl_assert_fail_if(v_int & 0x1 ? !v_odd_even : v_odd_even);
...
static Eina_Value
_children_odd_even_check_after_removal(Eo *model EFL_UNUSED, void *data EFL_UNUSED,
                                       const Eina_Value v)
{
   unsigned int i, len;
   Efl_Model *child;
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, child)
     {
        ...
        efl_assert_fail_if(v_int & 0x1 ? !v_odd_even : v_odd_even);
        efl_assert_fail_if(v_int != base_ints[i + 1]);
...
{
   testlog("Assert on error triggered duing Boolean Model test with error '%s'.\n",
...
static Eina_Value
_children_random_index(Eo *model EFL_UNUSED, void *data EFL_UNUSED, const Eina_Value v)
{
   unsigned int i, len;
   Efl_Model *child;
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, child)
     {
        ...
        efl_assert_int_eq(index, request[i]);
     }

   return v;
}

TEST(efl_test_boolean_model)
{
   ...
   for (i = 0; i < child_number; ++i)
     {
        ...
        assert(!!child);
        assert(eina_value_set(&v, base_ints[i]));
...
_fetch_and_switch(Eo *o EFL_UNUSED, void *data EFL_UNUSED, const Eina_Value v)
{
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, target)
     {
        all[i] = efl_model_property_set(target, "ready", eina_value_bool_new(EFL_TRUE));
     }
   all[i] = EINA_FUTURE_SENTINEL;
...
static Eina_Value
_count_and_fetch(Eo *filter, void *data EFL_UNUSED, const Eina_Value v EFL_UNUSED)
{
   efl_assert_int_eq(efl_model_children_count_get(filter), child_number);
   ...
   EINA_VALUE_ARRAY_FOREACH(&v, len, i, target)
     {
        ...
        efl_assert_fail_if(!eina_value_uint64_convert(p_original, &original));
        assert(original < (uint64_t)child_number);
        assert(idx < (uint64_t)child_number);
...
TEST(efl_test_filter_model)
{
   ...
   assert(!!filter);
   efl_assert_int_eq(efl_model_children_count_get(filter), 0);
