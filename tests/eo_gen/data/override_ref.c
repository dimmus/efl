...
static void __eolian_override_b_set(Eo *obj EFL_UNUSED, Override_Data *pd, int idx EFL_UNUSED, float a, char b, int c)
{
   pd->a = a;
   pd->b = b;
   pd->c = c;
...
static int __eolian_override_c_get(const Eo *obj EFL_UNUSED, Override_Data *pd EFL_UNUSED, int idx EFL_UNUSED)
{
   return 50;
}
...
static void __eolian_override_bar(Eo *obj EFL_UNUSED, Override_Data *pd EFL_UNUSED, int idx EFL_UNUSED, int *a, char **str)
{
   if (a) *a = 250;
   if (str) *str = NULL;
}
...
static void __eolian_override_base_z_set(Eo *obj EFL_UNUSED, Override_Data *pd, int a, char b, float c)
{
   pd->a = a;
   pd->b = b;
   pd->c = c;
...
static void __eolian_override_base_z_get(const Eo *obj EFL_UNUSED, Override_Data *pd, int *a, char *b, float *c)
{
   if (a) *a = pd->a;
   if (b) *b = pd->b;
   if (c) *c = pd->c;
