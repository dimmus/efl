...
EFL_START_TEST(eolian_namespaces)
{
  ...
  fail_if(
    !(class112 = eolian_unit_class_by_name_get(unit, "nmsp1.nmsp11.class2")));
...
EFL_START_TEST(eolian_consts)
{
  ...
  fail_if(EFL_FALSE == eolian_function_object_is_const(fid));
...
EFL_START_TEST(eolian_complex_type)
{
  ...
  fail_if(!(type_name = eolian_type_c_type_get(type)));
  fail_if(!eolian_type_is_move(type));
  fail_if(strcmp(type_name, "Efl_Strbuf *"));
  eina_stringshare_del(type_name);
  ...
  fail_if(!(type_name = eolian_parameter_c_type_get(param, EFL_FALSE)));
...
EFL_START_TEST(eolian_scope)
{
  ...
  fail_if(eolian_function_scope_get(fid, EOLIAN_METHOD) !=
          EOLIAN_SCOPE_PROTECTED);
  fail_if(
    !(fid = eolian_class_function_by_name_get(class, "foobar", EOLIAN_METHOD)));
  fail_if(eolian_function_scope_get(fid, EOLIAN_METHOD) != EOLIAN_SCOPE_PUBLIC);
...
EFL_START_TEST(eolian_simple_parsing)
{
  ...
  fail_if(strcmp(eolian_object_file_get((const Eolian_Object *)class),
                 "class_simple.eo"));
...
EFL_START_TEST(eolian_enum)
{
  ...
  fail_if(!(exp = eolian_typedecl_enum_field_value_get(field, EFL_FALSE)));
  v = eolian_expression_eval(exp, EOLIAN_MASK_ALL);
  fail_if(v.type != EOLIAN_EXPR_INT);
  fail_if(v.value.i != 0);

  fail_if(!(field = eolian_typedecl_enum_field_get(tdl, "bar")));
  fail_if(eolian_typedecl_enum_field_value_get(field, EFL_FALSE));
...
EFL_START_TEST(eolian_import)
{
  ...
  fail_if(strcmp(eolian_object_file_get((const Eolian_Object *)tdl),
                 "import_types.eot"));
...
EFL_START_TEST(eolian_docs)
{
  ...
  fail_if(strcmp(eolian_documentation_since_get(doc), "1.66"));
  ...
  fail_if(!(doc = eolian_typedecl_documentation_get(tdl)));
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for enum Bar."));
  fail_if(eolian_documentation_description_get(doc));
  fail_if(strcmp(eolian_documentation_since_get(doc), "1.55"));
  ...
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for foo."));
  fail_if(eolian_documentation_description_get(doc));

  fail_if(!(efl = eolian_typedecl_enum_field_get(tdl, "bar")));
  fail_if(!(doc = eolian_typedecl_enum_field_documentation_get(efl)));
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for bar."));
  fail_if(eolian_documentation_description_get(doc));

  fail_if(!(tdl = eolian_unit_alias_by_name_get(unit, "Alias")));
  fail_if(!(doc = eolian_typedecl_documentation_get(tdl)));
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for typedef."));
  fail_if(strcmp(eolian_documentation_description_get(doc),
                 "More docs for typedef. See @Bar."));
  fail_if(strcmp(eolian_documentation_since_get(doc), "1.1337"));

  fail_if(!(var = eolian_unit_constant_by_name_get(unit, "pants")));
  fail_if(!(doc = eolian_constant_documentation_get(var)));
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for var."));
  fail_if(eolian_documentation_description_get(doc));
  ...
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Docs for class."));
  fail_if(strcmp(
  ...
  fail_if(
    !(fid = eolian_class_function_by_name_get(class, "prop", EOLIAN_PROPERTY)));
  ...
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Get documentation."));
  fail_if(eolian_documentation_description_get(doc));
  fail_if(!(doc = eolian_implement_documentation_get(fimp, EOLIAN_PROP_SET)));
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Set documentation."));
  fail_if(eolian_documentation_description_get(doc));
  ...
  fail_if(strcmp(eolian_documentation_summary_get(doc), "Event docs."));
  fail_if(eolian_documentation_description_get(doc));

  fail_if(!(
    fid =
      eolian_class_function_by_name_get(class, "no_doc_meth", EOLIAN_METHOD)));
  fail_if(!(fimp = eolian_function_implement_get(fid)));
  fail_if((doc = eolian_implement_documentation_get(fimp, EOLIAN_METHOD)));

  fail_if(!(fid = eolian_class_function_by_name_get(class,
                                                    "doc_with_empty_doc",
                                                    EOLIAN_METHOD)));
  fail_if(!(fimp = eolian_function_implement_get(fid)));
  fail_if(!(doc = eolian_implement_documentation_get(fimp, EOLIAN_METHOD)));
...
EFL_START_TEST(eolian_function_types)
{
  ...
  fail_if((eolian_function_return_type_get(
    fid,
