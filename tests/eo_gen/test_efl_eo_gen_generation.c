...
EFL_START_TEST(eolian_dev_impl_code)
{
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/object_impl.eo",
                                   "-gi",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/object_impl_ref.c",
                          output_filepath,
                          "c"));
  ...
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/object_impl_ref.c",
                          output_filepath,
                          "c"));
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/object_impl_add.eo",
                                   "-gi",
                                   output_filepath));
  fprintf(stderr, "[%s]\n", output_filepath);
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/object_impl_add_ref.c",
                          output_filepath,
                          "c"));
}
...
EFL_START_TEST(eolian_types_generation)
{
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/typedef.eo",
                                   "-gh",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/typedef_ref.h",
                          output_filepath,
                          "eo.h"));
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/struct.eo",
                                   "-gh",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/struct_ref.h",
                          output_filepath,
                          "eo.h"));
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/typedef.eo",
                                   "-gs",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/typedef_ref_stub.h",
                          output_filepath,
                          "eo.stub.h"));
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/struct.eo",
                                   "-gs",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/struct_ref_stub.h",
                          output_filepath,
                          "eo.stub.h"));
}
...
EFL_START_TEST(eolian_override_generation)
{
  ...
  snprintf(output_filepath,
           ...
           eina_environment_tmp_get());
  _remove_ref(output_filepath, "eo.c");
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/override.eo",
                                   "-gc",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/override_ref.c",
                          output_filepath,
                          "eo.c"));
}
...
EFL_START_TEST(eolian_import)
{
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/import_types.eot",
                                   "-gh",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/import_types_ref.h",
                          output_filepath,
                          "eot.h"));
}
...
EFL_START_TEST(eolian_docs)
{
  ...
  snprintf(output_filepath,
           ...
           eina_environment_tmp_get());
  _remove_ref(output_filepath, "eo.h");
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/eo_docs.eo",
                                   "-gh",
                                   output_filepath));
  fail_if(
    !_files_compare(TESTS_SRC_DIR "/data/docs_ref.h", output_filepath, "eo.h"));
}
...
EFL_START_TEST(eolian_function_pointers)
{
  ...
  snprintf(output_filepath,
           ...
           eina_environment_tmp_get());
  _remove_ref(output_filepath, "eot.h");
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/function_types.eot",
                                   "-gh",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/function_types_ref.h",
                          output_filepath,
                          "eot.h"));

   // .eo.h
  _remove_ref(output_filepath, "eo.h");
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR
                                   "/data/function_as_argument.eo",
                                   "-gh",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/function_as_argument_ref.h",
                          output_filepath,
                          "eo.h"));
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR
                                   ...
                                   "-gc",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/function_as_argument_ref.c",
                          output_filepath,
                          "eo.c"));
  ...
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR
                                   ...
                                   "-gi",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/function_as_argument_impl_ref.c",
                          output_filepath,
                          "c"));
   /* Check that nothing is added */
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR
...
EFL_START_TEST(owning)
{
  ...
  snprintf(output_filepath,
           ...
           eina_environment_tmp_get());
  _remove_ref(output_filepath, "eo.c");
  fail_if(0 != _eolian_gen_execute(TESTS_SRC_DIR "/data/owning.eo",
                                   "-gc",
                                   output_filepath));
  fail_if(!_files_compare(TESTS_SRC_DIR "/data/owning_ref.c",
                          output_filepath,
                          "eo.c"));
}
...
void
eolian_generation_test(TCase *tc)
{
