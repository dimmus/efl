...
static const char *bus_session = "org.freedesktop.Test.efl_dbus_test_efl_dbus_message";
...
_swap_value(const Efl_Dbus_Service_Interface *iface EINA_UNUSED, const Efl_Dbus_Message *msg)
{
...
static Efl_Dbus_Message *
_receive_container_data(const Efl_Dbus_Service_Interface *iface EINA_UNUSED, const Efl_Dbus_Message *msg)
{
   Efl_Dbus_Message *reply = efl_dbus_message_method_return_new(msg);
        ...
        if (!fail)
          {
             ...
             Efl_Dbus_Message_Iter *array = efl_dbus_message_iter_container_new(iter, 'a', "i");
             if (array)
               {
...
static void
_on_send_bool(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
...
static Efl_Dbus_Message *
_send_bool(const Efl_Dbus_Service_Interface *iface EINA_UNUSED, const Efl_Dbus_Message *msg)
{
...
static void
_on_send_array_int(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
        ...
        if (efl_dbus_message_arguments_get(msg, "ai", &array))
          {
             ...
             int array_count;
             if (efl_dbus_message_iter_fixed_array_get(array, 'i', &array_int, &array_count))
               {
...
static Efl_Dbus_Message *
_send_array_int(const Efl_Dbus_Service_Interface *iface EINA_UNUSED, const Efl_Dbus_Message *msg)
{
   Efl_Dbus_Message *reply = efl_dbus_message_method_return_new(msg);
   ...
   efl_dbus_message_iter_fixed_array_append(array, 'i', numbers_int, EINA_C_ARRAY_LENGTH(numbers_int));
   efl_dbus_message_iter_container_close(iter, array);
...
static const Efl_Dbus_Method methods[] = {
      {
        "SwapValue", EFL_DBUS_ARGS({"i", "integer"}),
        EFL_DBUS_ARGS({"i", "swap_value"}), _swap_value, 0
      },
      {
        "ArrayContainer", EFL_DBUS_ARGS({"as", "container_of_strings"}),
        EFL_DBUS_ARGS({"ai", "array_of_int"}), _receive_container_data, 0
      },
...
static void
_response_message_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
...
static void
_message_method_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg EINA_UNUSED, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_activatable_list_response_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_signal_name_owner_changed(void *data EINA_UNUSED, const Efl_Dbus_Message *msg)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_response_owner_has_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_message_without_body_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg EINA_UNUSED, Efl_Dbus_Pending *pending EINA_UNUSED)
{
}

static void
_message_response_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_message_vget_response_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_name_request(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   unsigned int reply;
   const char *errname, *errmsg;
   if (!efl_dbus_message_error_get(msg, &errname, &errmsg))
...
static void
_response_swap_value(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
...
static void
_response_swap_iter_vget(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   int swap_value;
...
static void
_container_receive_cb(void *data EINA_UNUSED, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
        ecore_timer_del(timeout);
...
static void
_hello_cb(void *data, const Efl_Dbus_Message *msg EINA_UNUSED, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   int *user_data = data;
...
_activable_list_get(Efl_Dbus_Message_Cb message_cb)
{
   ...
   Efl_Dbus_Pending *pending = efl_dbus_names_activatable_list(conn, message_cb, NULL);
...
EFL_START_TEST(utc_efl_dbus_message_info_data_get_p)
{
   ...
   Efl_Dbus_Message *msg = efl_dbus_message_method_call_new(bus, path, interface, method_name);
   ...
   Efl_Dbus_Pending *pending = efl_dbus_connection_send(conn, msg, _message_method_cb, NULL, timeout_send_ms);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_signal_new_p)
{
   ...
   Efl_Dbus_Pending *pending = efl_dbus_connection_send(conn, msg, _message_without_body_cb, NULL, timeout_send_ms);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_basic_eina_value_p)
{
   ...
   Efl_Dbus_Pending *pending = efl_dbus_name_owner_has(conn, bus, _response_owner_has_cb, NULL);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_arguments_vappend_p)
{
   ...
   message_vparam = efl_dbus_message_method_call_new(bus, path, interface, "GetNameOwner");
   ck_assert_ptr_ne(NULL, message_vparam);

   _varg_add("s", bus);

   Efl_Dbus_Pending *pending = efl_dbus_connection_send(conn, message_vparam, _message_response_cb, NULL, -1);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_arguments_vget_p)
{
   ...
   Efl_Dbus_Message *message = efl_dbus_message_method_call_new(bus, path, interface, "NameHasOwner");
   ck_assert_ptr_ne(NULL, message);

   Eina_Bool is_arguments_append = efl_dbus_message_arguments_append(message, "s", bus);
   ck_assert_msg(is_arguments_append, "Can't append arguments");

   Efl_Dbus_Pending *pending = efl_dbus_connection_send(conn, message, _message_vget_response_cb, NULL, -1);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_eina_p)
{
   ...
   Efl_Dbus_Connection *conn_server = efl_dbus_connection_get(EFL_DBUS_CONNECTION_TYPE_SESSION);
   ck_assert_ptr_ne(NULL, conn_server);

   Efl_Dbus_Service_Interface *iface = efl_dbus_service_interface_register(conn_server, dbus_session_path, &iface_desc);
   ck_assert_ptr_ne(NULL, iface);

   Efl_Dbus_Pending *pending_name = efl_dbus_name_request(conn_server, bus_session, EFL_DBUS_NAME_REQUEST_FLAG_DO_NOT_QUEUE,
                                                      _name_request, iface);
   ck_assert_ptr_ne(NULL, pending_name);

   Efl_Dbus_Connection *conn_client = efl_dbus_connection_get(EFL_DBUS_CONNECTION_TYPE_SESSION);
   ck_assert_ptr_ne(NULL, conn_client);

   Efl_Dbus_Object *obj = efl_dbus_object_get(conn_client, bus_session, dbus_session_path);
   ...
   Efl_Dbus_Pending *pending = efl_dbus_proxy_send(proxy, msg, _response_swap_value, NULL, -1);
   ck_assert_ptr_ne(NULL, pending);

   pending = efl_dbus_proxy_call(proxy, "SwapValue", _response_swap_iter_vget, NULL, -1 , "i", value_from_client);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_container_p)
{
   ...
   Efl_Dbus_Message *msg = efl_dbus_proxy_method_call_new(proxy, "ArrayContainer");
   ck_assert_ptr_ne(NULL, msg);

   Efl_Dbus_Message_Iter *iter = efl_dbus_message_iter_get(msg);

   Efl_Dbus_Message_Iter *container_iter = efl_dbus_message_iter_container_new(iter, 'a',"s");
   ck_assert_ptr_ne(NULL, container_iter);

   for (i = 0; i < CONTAINER_COUNT; i++)
     efl_dbus_message_iter_basic_append(container_iter, 's', container_data[i]);

   Eina_Bool is_container_iter_closed = efl_dbus_message_iter_container_close(iter, container_iter);
   ck_assert_msg(is_container_iter_closed, "Can't closed container");

   Efl_Dbus_Pending *pending = efl_dbus_proxy_send(proxy, msg, _container_receive_cb, NULL, -1);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_message_iter_del_p)
{
   ...
   Efl_Dbus_Message *msg = efl_dbus_object_method_call_new(obj, "org.freedesktop.Test", "SendByte");
   ck_assert_ptr_ne(NULL, msg);
...
EFL_START_TEST(utc_efl_dbus_message_iter_fixed_array_get_p)
{
   ...
   Efl_Dbus_Pending *pending = efl_dbus_proxy_call(proxy, "SendArrayInt", _on_send_array_int, NULL, -1 , "");
   ck_assert_ptr_ne(NULL, pending);
