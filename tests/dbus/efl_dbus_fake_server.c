...
#include "efl_dbus_fake_server.h"

#define FAKE_SERVER_DATA_KEY "data"

static void _fake_server_name_request_cb(void *, const Efl_Dbus_Message *, Efl_Dbus_Pending *);
static Eina_Bool _fakse_server_property_get(const Efl_Dbus_Service_Interface *, const char *, Efl_Dbus_Message_Iter *, const Efl_Dbus_Message *, Efl_Dbus_Message **);
static Efl_Dbus_Message *_fake_server_property_set(const Efl_Dbus_Service_Interface *, const char *, Efl_Dbus_Message_Iter *, const Efl_Dbus_Message *);
static Efl_Dbus_Message *_fake_server_sum(const Efl_Dbus_Service_Interface *, const Efl_Dbus_Message *);
static Efl_Dbus_Message *_fake_server_ping(const Efl_Dbus_Service_Interface *, const Efl_Dbus_Message *);
...
static const Efl_Dbus_Method methods[] = {
   {
     FAKE_SERVER_SUM_METHOD_NAME, EFL_DBUS_ARGS({"i", "a"}, {"i", "b"}), EFL_DBUS_ARGS({"i", "result"}),
     _fake_server_sum,
      0
   },
   {
     FAKE_SERVER_PING_METHOD_NAME, EFL_DBUS_ARGS({"i", "a"}), NULL,
     _fake_server_ping,
      0
   },
...
static const Efl_Dbus_Signal signals[] = {
   [FAKE_SERVER_PONG_SIGNAL] = {FAKE_SERVER_PONG_SIGNAL_NAME, EFL_DBUS_ARGS({ "i", NULL }), 0},
   { NULL, NULL, 0 }
};
...
fake_server_start(Fake_Server_Data *data, const char *name)
{
   ...
   Efl_Dbus_Service_Interface *interface = efl_dbus_service_interface_register(conn, FAKE_SERVER_PATH, &test_interface_desc);
   efl_dbus_service_object_data_set(interface, FAKE_SERVER_DATA_KEY, data);

   efl_dbus_name_request(conn, name, EFL_DBUS_NAME_REQUEST_FLAG_DO_NOT_QUEUE,
...
_fake_server_sum(const Efl_Dbus_Service_Interface *iface EINA_UNUSED, const Efl_Dbus_Message *msg)
{
   int a, b;
   if (!efl_dbus_message_arguments_get(msg, "ii", &a, &b))
     return efl_dbus_message_error_new(msg, "Invalid arguments", "Error getting arguments..");
...
static Efl_Dbus_Message *
_fake_server_ping(const Efl_Dbus_Service_Interface *iface , const Efl_Dbus_Message *msg)
{
   int a;
   if (!efl_dbus_message_arguments_get(msg, "i", &a))
     return efl_dbus_message_error_new(msg, "Invalid arguments", "Error getting arguments..");

   Fake_Server_Data *pd = efl_dbus_service_object_data_get(iface, FAKE_SERVER_DATA_KEY);
...
_fake_server_send_pong_signal(void *data)
{
   ...
   efl_dbus_service_signal_emit(iface, FAKE_SERVER_PONG_SIGNAL, pd->pong_response);
...
_fakse_server_property_get(const Efl_Dbus_Service_Interface *iface,
                           ...
                           const Efl_Dbus_Message *request_msg EINA_UNUSED,
                           Efl_Dbus_Message **error EINA_UNUSED)
{
   Fake_Server_Data *data = efl_dbus_service_object_data_get(iface, FAKE_SERVER_DATA_KEY);
   ck_assert_ptr_ne(NULL, data);

   if (strcmp(propname, FAKE_SERVER_READONLY_PROPERTY) == 0)
     efl_dbus_message_iter_arguments_append(iter, "i", data->readonly_property);
   else
   if (strcmp(propname, FAKE_SERVER_READWRITE_PROPERTY) == 0)
     efl_dbus_message_iter_arguments_append(iter, "i", data->readwrite_property);
   else
     return EINA_FALSE;
                          ...
                          const Efl_Dbus_Message *msg)
{
   ...
   if (strcmp(propname, FAKE_SERVER_WRITEONLY_PROPERTY) == 0)
     {
        if (!efl_dbus_message_iter_arguments_get(iter, "i", &data->writeonly_property))
   ...
   else
     {
        if (strcmp(propname, FAKE_SERVER_READWRITE_PROPERTY) == 0)
          {
             if (!efl_dbus_message_iter_arguments_get(iter, "i", &data->readwrite_property))
               ck_abort_msg("error getting iter arguments on _fake_server_property_set");
          }
        else
          return efl_dbus_message_error_new(msg, "Invalid property", "Invalid property.");
     }
...
_fake_server_name_request_cb(void *data EINA_UNUSED,
                             const Efl_Dbus_Message *msg,
                             Efl_Dbus_Pending *pending EINA_UNUSED)
{
