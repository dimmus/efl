...
_setup(void)
{
  ...
  efl_event_callback_add(dbus_object1,
  ...
  dbus_proxy =
    efl_dbus_model_proxy_from_object_get(dbus_object1, EFL_DBUS_FDO_INTERFACE);
...
EFL_START_TEST(properties_get)
{
  Eina_Iterator *properties = NULL;
   // EFL_DBUS_FDO_INTERFACE have no properties
...
{
  efl_event_callback_add(dbus_proxy,
                         EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                         _count_changed,
                         NULL);
...
void
efl_dbus_test_efl_dbus_model_proxy(TCase *tc)
{
