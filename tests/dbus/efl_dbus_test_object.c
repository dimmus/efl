...
static void
_object_message_cb(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   if (timeout != NULL)
     {
...
static const Efl_Dbus_Method methods[] = {
      ...
      {
        "GetMachineId", NULL,
        EFL_DBUS_ARGS({"s", "machine_id"}), NULL, 0
      },
...
static void
_peer_ping_cb(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   int *user_data = data;
...
static void
_machine_id_get(void *data, const Efl_Dbus_Message *msg, Efl_Dbus_Pending *pending EINA_UNUSED)
{
   int *user_data = data;
...
EFL_START_TEST(utc_efl_dbus_object_send_info_get_p)
{
   ...
   Efl_Dbus_Message *msg = efl_dbus_object_method_call_new(obj, interface, method_name);
   ck_assert_ptr_ne(NULL, msg);

   Efl_Dbus_Pending *pending = efl_dbus_object_send(obj, msg, _object_message_cb, &cb_data, -1);
...
EFL_START_TEST(utc_efl_dbus_introspect_p)
{
   ...
   Efl_Dbus_Pending *pending = efl_dbus_object_introspect(obj, _object_message_cb, &cb_data);
   ck_assert_ptr_ne(NULL, pending);
...
EFL_START_TEST(utc_efl_dbus_object_peer_p)
{
   ...
   Efl_Dbus_Service_Interface *iface = efl_dbus_service_interface_register(conn_server, dbus_path, &iface_desc);
   ...
   Efl_Dbus_Object *obj = efl_dbus_object_get(conn_client, bus_session, dbus_path);
   ck_assert_ptr_ne(NULL, obj);

   Efl_Dbus_Pending *pending1 = efl_dbus_object_peer_machine_id_get(obj, _machine_id_get, &cb_data);
   ck_assert_ptr_ne(NULL, pending1);

   Efl_Dbus_Pending *pending2 = efl_dbus_object_peer_ping(obj, _peer_ping_cb, &cb_data);
