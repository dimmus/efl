...
static Eina_Value
_failed_property_set(void                          *data,
                     const Eina_Value               v,
...
EFL_START_TEST(property_set)
{
  ...
  eina_future_then(future,
                   _failed_property_set,
                   eina_value_error_new(EFL_MODEL_ERROR_NOT_FOUND),
                   NULL);
  ...
  eina_future_then(future,
                   _failed_property_set,
                   eina_value_error_new(EFL_MODEL_ERROR_READ_ONLY),
                   NULL);
...
_test_children_count(Eo *efl_model)
{
  ...
  efl_event_callback_add(efl_model,
                         EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                         _count_changed,
                         NULL);
  if (!efl_model_children_count_get(efl_model)) core_main_loop_begin();
  efl_event_callback_del(efl_model,
                         EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                         _count_changed,
                         NULL);
...
{
  efl_event_callback_add(connection,
                         EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                         _count_changed,
                         NULL);
  if (!efl_model_children_count_get(connection)) core_main_loop_begin();
  efl_event_callback_del(connection,
                         EFL_MODEL_EVENT_CHILDREN_COUNT_CHANGED,
                         _count_changed,
                         NULL);
...
void
efl_dbus_test_efl_dbus_model_connection(TCase *tc)
{
