...
static void
_proxy_message_cb(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending EFL_UNUSED)
{
   if (timeout != NULL)
     {
...
EFL_START_TEST(utc_eldbus_proxy_send_and_block_p)
{
   ...
   Eldbus_Message *message_reply = eldbus_proxy_send_and_block(proxy, msg, timeout_in);
   ck_assert_ptr_ne(NULL, message_reply);

   //eldbus_message_error_get(message_reply, &errname, &errmsg)
   ck_assert(eldbus_message_error_get(message_reply, &errname, &errmsg) == EFL_FALSE);

   ck_assert(eldbus_message_arguments_get(message_reply, "s", &text_reply) == EFL_TRUE);
