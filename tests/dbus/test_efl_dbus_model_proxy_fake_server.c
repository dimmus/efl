...
static void
_property_ready(void *data EFL_UNUSED, const Efl_Event *ev)
{
  if (f) return;
...
EFL_START_TEST(property_get)
{
  ...
  efl_event_callback_add(fake_server_proxy,
                         EFL_MODEL_EVENT_PROPERTIES_CHANGED,
                         _property_ready,
                         NULL);
  value =
    efl_model_property_get(fake_server_proxy, FAKE_SERVER_READONLY_PROPERTY);
  eina_value_free(value);
  value =
    efl_model_property_get(fake_server_proxy, FAKE_SERVER_READWRITE_PROPERTY);
  eina_value_free(value);
  ...
  check_efl_model_property_int_eq(fake_server_proxy,
                                  ...
                                  FAKE_SERVER_READONLY_PROPERTY_VALUE);
  check_efl_model_property_int_eq(fake_server_proxy,
                                  FAKE_SERVER_READWRITE_PROPERTY,
                                  FAKE_SERVER_READWRITE_PROPERTY_VALUE);
...
static void
_check_property_set(const char *property_name,
                    int         expected_property_value,
...
EFL_START_TEST(property_set)
{
  ...
  _check_property_set(FAKE_SERVER_WRITEONLY_PROPERTY,
                      0x12345678,
                      &fake_server_data.writeonly_property);
  _check_property_set(FAKE_SERVER_READWRITE_PROPERTY,
                      0x76543210,
...
{
  Efl_Dbus_Model_Arguments *method1 =
    efl_model_nth_child_get(fake_server_proxy, 0);
  Efl_Dbus_Model_Arguments *method2 =
    efl_model_nth_child_get(fake_server_proxy, 1);
  Efl_Dbus_Model_Arguments *signal1 =
    efl_model_nth_child_get(fake_server_proxy, 2);

  const char *actual_method1_name =
    efl_dbus_model_arguments_arg_name_get(method1);
  const char *actual_method2_name =
    efl_dbus_model_arguments_arg_name_get(method2);
  const char *actual_signal1_name =
    efl_dbus_model_arguments_arg_name_get(signal1);
  ...
  else ck_assert(strcmp(FAKE_SERVER_SUM_METHOD_NAME, actual_method2_name) == 0);
...
EFL_START_TEST(child_del)
{
  ...
  ck_assert_msg(expected_children_count,
                "There must be at least 1 child to test");
...
void
efl_dbus_test_fake_server_efl_dbus_model_proxy(TCase *tc)
{
