...
#define DISABLE_ABORT_ON_CRITICAL_START                \
    ...
    {                                                  \
        ...
        eina_log_abort_on_critical_set((Efl_Bool)0)

#define DISABLE_ABORT_ON_CRITICAL_END                 \
    eina_log_abort_on_critical_set((Efl_Bool)___val); \
    }                                                 \
    while (0)
...
#define EXPECT_ERROR_START                                \
    ...
    {                                                     \
        ...
        do                                                \
        {                                                 \
            eina_log_print_cb_set(_efl_test_expect_error, \
                                  &expect_error_start);   \
        }                                                 \
        while (0)
...
#define EXPECT_ERROR_END                            \
    ...
    ck_assert_int_eq(expect_error_start, EFL_TRUE); \
...
_efl_test_expect_error(const Eina_Log_Domain *d EFL_UNUSED,
...
_efl_test_option_disp(int argc, char **argv, const Efl_Test_Case *etc)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            const char **nav =
                (const char **)alloca(sizeof(char *) * (argc + 3));
            int j, k;
            ...
            fprintf(stderr,
                    "Failed to execute valgrind due to '%s'\n",
                    strerror(errno));
            return 0;
        }
    }
...
_efl_test_use(int argc, const char **argv, const char *test_case)
{
    ...
    for (; argc > 0; argc--, argv++)
        if (strcasecmp(test_case, *argv) == 0) return 1;
    return 0;
}
...
{
    if (srunner_fork_status(sr) == CK_FORK) return 1;
    else if (srunner_fork_status(sr) == CK_NOFORK) return 0;
    else if (srunner_fork_status(sr) == CK_FORK_GETENV)
    {
        char *res;

        res = getenv("CK_FORK");
        if (res && (strcmp(res, "no") == 0)) return 0;
        else return 1;
    }
...
_timing_start(void)
{
    if (_timing_enabled()) _timing_start_time = _timing_time_get();
}
...
_timing_end(void)
{
    ...
    if (!thres_set)
    {
        ...
        if (env)
        {
            ...
            if (thres > 0) thres_set = 1;
        }
        if (!thres_set) thres = 0.0001;
        thres_set = 1;
    }
    diff = _timing_time_get() - _timing_start_time;

    if (diff > thres) printf("TIME %s: %.5g\n", tcase_name(), diff);
}
...
_efl_suite_run_end(SRunner *sr, const char *name)
{
    ...
    if (name)
    {
        ...
        snprintf(srunner_xml_buf,
                 sizeof(srunner_xml_buf),
                 TESTS_BUILD_DIR "/check-results-%s.xml",
                 n);
        srunner_set_xml(sr, srunner_xml_buf);
        free(n);
    }
    else srunner_set_xml(sr, TESTS_BUILD_DIR "/check-results.xml");
...
_efl_suite_wait_on_fork(int *num_forks, Efl_Bool *timeout)
{
    ...
    if (pid == timeout_pid) *timeout = EFL_TRUE;
    else
    {
...
EFL_UNUSED static int
_efl_suite_build_and_run(int                  argc,
                         const char         **argv,
                         const char          *suite_name,
                         const Efl_Test_Case *etc,
                         SFun                 init,
                         SFun                 shutdown)
{
    Suite   *s;
    ...
    int      num_forks       = 0;
    int      can_fork        = 0;
    Efl_Bool timeout_reached = EFL_FALSE;
#  ifdef ENABLE_TIMING_INFO
    double tcstart = 0.0;
#  endif
#endif
    ...
    if (timing) tstart = _timing_time_get();
#endif

#ifdef HAVE_FORK
#  ifdef ENABLE_TIMING_INFO
    if (timing) tcstart = tstart;
#  endif
#endif
    ...
    if (do_fork) can_fork = !!etc[1].test_case /* can't parallelize 1 test */;
#endif
    ...
    for (i = 0; etc[i].test_case; ++i)
    {
        ...
        if (!_efl_test_use(argc, argv, etc[i].test_case)) continue;
#ifdef HAVE_FORK
        if (do_fork && can_fork)
        {
            if (!timeout_pid)
            ...
            if (num_forks == eina_cpu_count())
                failed_count +=
                    _efl_suite_wait_on_fork(&num_forks, &timeout_reached);
            ...
            if (pid > 0)
            {
                ...
                num_forks++;
#  ifdef ENABLE_TIMING_INFO
                if (timing) tcstart = _timing_time_get();
#  endif
                continue;
            }
            else if (pid < 0)
        ...
        if (init || shutdown) tcase_add_checked_fixture(tc, init, shutdown);

#ifdef HAVE_FORK
        if (do_fork) tcase_set_timeout(tc, 0);
#endif
        ...
        if (do_fork && (!pid) && can_fork)
        {
            ...
            if (failed_count > 255) failed_count = 255;
#  ifdef ENABLE_TIMING_INFO
            if (timing)
                printf("TC TIME %s: %.5g\n",
                       etc[i].test_case,
