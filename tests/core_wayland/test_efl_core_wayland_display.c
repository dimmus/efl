...
static Efl_Bool
_test_input_find_configure_complete(void *data, int type EFL_UNUSED, void *event EFL_UNUSED)
{
   Test_Data *td = data;

   /* NB: Enlightenment uses "seat0" here, but Weston uses "default" */
   if (getenv("E_START"))
     test_wl_input = ecore_wl_display_input_find_by_name(td->display, "seat0");
   else
     test_wl_input = ecore_wl_display_input_find_by_name(td->display, "default");
...
EFL_START_TEST(core_wayland_display_sync_is_done)
{
   ...
   core_event_handler_add(ECORE_WL2_EVENT_SYNC_DONE,
                          _test_sync_done, td);
