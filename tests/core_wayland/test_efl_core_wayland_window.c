                         ...
                         void                        *data)
{
  ...
  else if (td->frame_callback_count % 4 == 1) glClearColor(0.0, 0.0, 1.0, 0.0);
  else if (td->frame_callback_count % 4 == 2) glClearColor(0.0, 0.0, 0.0, 1.0);
...
EFL_START_TEST(core_wayland_window_available_rotation)
{
  ...
  ret =
    efl_core_wayland_window_available_rotations_get(win, &ret_rots, &ret_count);
...
EFL_START_TEST(core_wayland_window_free)
{
  ...
  t_win =
    efl_core_wayland_display_window_find_by_surface(td->display, td->surface);
