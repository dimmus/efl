...
file_monitor_cb(void *data             EFL_UNUSED,
                Ecore_File_Monitor *em EFL_UNUSED,
...
EFL_START_TEST(ecore_test_ecore_file_operations)
{
   ...
   /* On Windows, symlink/readlink are not supported */
    ...
    fail_if(ecore_file_can_read(dest_file) != EFL_TRUE);
    fail_if(ecore_file_can_write(dest_file) != EFL_TRUE);
    fail_if(ecore_file_can_exec(dest_file) != EFL_FALSE);
    fail_if(ecore_file_remove(dest_file) != EFL_TRUE);
    ...
    src_file = get_tmp_file();
    fail_if(!src_file);
    fail_if(ecore_file_remove(src_file) != EFL_TRUE);
    fd = open(src_file, O_RDWR | O_BINARY | O_CREAT, 0700);
    fail_if(fd < 0);
    fail_if(close(fd) != 0);
    fail_if(ecore_file_can_exec(src_file) != EFL_TRUE);

    src_dir = get_tmp_dir();
    fail_if(!src_dir);
    snprintf(dir, sizeof(dir), "%s/%s", src_dir, dirs[0]);
    fail_if(ecore_file_mkdir(dir) != EFL_TRUE);
    ...
    fail_if(ecore_file_remove(src_file) != EFL_TRUE);
    fail_if(ecore_file_recursive_rm(src_dir) != EFL_TRUE);

    src_dir = get_tmp_dir();
    fail_if(!src_dir);
    fail_if(ecore_file_rmdir(src_dir) != EFL_TRUE);
    ...
    fail_if(ecore_file_remove(not_exist_file) != EFL_TRUE);
    fail_if(ecore_file_exists(not_exist_file) != EFL_FALSE);
    ...
    fail_if(fd < 0);
    fail_if(close(fd) != 0);
    fail_if(ecore_file_can_read(src_file) != EFL_TRUE);
#if defined(HAVE_GETUID) && defined(HAVE_GETEUID)
    if (getuid() || geteuid())
#endif
    {
        fail_if(ecore_file_can_write(src_file) != EFL_FALSE);
        fail_if(ecore_file_can_exec(src_file) != EFL_FALSE);
    }
    fail_if(ecore_file_cp(src_file, src_file) != EFL_FALSE);
    fail_if(ecore_file_remove(src_file) != EFL_TRUE);
...
#endif
    {
        fail_if(ecore_file_can_read(src_file) != EFL_FALSE);
    }
    fail_if(ecore_file_can_exec(src_file) != EFL_FALSE);
    fail_if(ecore_file_can_write(src_file) != EFL_TRUE);
    fail_if(ecore_file_remove(src_file) != EFL_TRUE);
    ...
    fail_if(ecore_file_unlink(not_exist_file) != EFL_FALSE);
    fail_if(ecore_file_remove(not_exist_file) != EFL_FALSE);
    fail_if(ecore_file_cp(not_exist_file, "test_file") != EFL_FALSE);
    fail_if(ecore_file_mv(not_exist_file, "test_file") != EFL_FALSE);

    ck_assert_int_eq(chdir(eina_environment_tmp_get()), 0);
    fail_if(ecore_file_mkpath(src_dir) != EFL_TRUE);
    fail_if(ecore_file_rmdir(src_dir) != EFL_TRUE);
    fail_if(ecore_file_mkpath(NULL) != EFL_FALSE);
    fail_if(ecore_file_mkpaths(dirs) != 4);
    for (i = 0; dirs[i]; i++)
        if (ecore_file_is_dir(dirs[i]))
            fail_if(ecore_file_recursive_rm(dirs[i]) != EFL_TRUE);
...
EFL_START_TEST(ecore_test_ecore_file_download)
{
    ...
    res = ecore_file_download("xxyyzz",
                              dest_name,
                              completion_cb,
                              progress_cb,
                              NULL,
    ...
    int status = 0;
    res        = ecore_file_download(download_url,
                              dest_name,
                              err_completion_cb,
                              progress_cb,
                              &status,
    ...
    fail_if(status != 1);
    if (!status) ecore_main_loop_begin();
    ...
    res = ecore_file_download_full(download_url,
                                   dest_name,
                                   completion_cb,
                                   progress_cb,
                                   NULL,
...
void
ecore_test_ecore_file(TCase *tc)
{
