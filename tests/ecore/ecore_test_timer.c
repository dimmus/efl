...
_timer2_cb(void *data)
{
   ...
   fail_if(!EINA_DBL_EQ(timer->interval_1[timer->num_elem], ecore_timer_interval_get(timer->timer1)));

   // check set new precision
   ecore_timer_precision_set(timer->precision[timer->num_elem]);
   fail_if(!EINA_DBL_EQ(timer->precision[timer->num_elem], ecore_timer_precision_get()));
...
EFL_START_TEST(ecore_test_timers)
{
   ...
   fail_if((!timer.timer1 || !timer.timer2 || !timer.timer3 || !timer.timer4 || !timer.timer5));
