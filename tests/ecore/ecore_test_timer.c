...
_timer2_cb(void *data)
{
   ...
   // check set new interval for timer 1
    ...
    fail_if(!EINA_DBL_EQ(timer->interval_1[timer->num_elem],
                         ecore_timer_interval_get(timer->timer1)));
...
_timer4_cb(void *data)
{
   ...
   // check frezze/thaw timer 3
    if (freeze_thaw)
    {
        fail_if(timer->check_freeze_thaw_timer3 != timer->count_timer3);
...
EFL_START_TEST(ecore_test_timers)
{
    ...
    fail_if((!timer.timer1 || !timer.timer2 || !timer.timer3 || !timer.timer4 ||
             !timer.timer5));

    ecore_main_loop_begin();

    if (timer.timer1) ecore_timer_del(timer.timer1);
    if (timer.timer2) ecore_timer_del(timer.timer2);
    if (timer.timer3) ecore_timer_del(timer.timer3);
    if (timer.timer4) ecore_timer_del(timer.timer4);
    if (timer.timer5) ecore_timer_del(timer.timer5);
}
...
static Efl_Bool
timeout_timer_cb(void *data EFL_UNUSED)
{
    ck_abort();
    return EFL_FALSE;
}
...
{
    Ecore_Timer *t = NULL;
    fail_if((t = ecore_timer_add(0.5, NULL, NULL)));
}
...
static Efl_Bool
_recursion(void *data EFL_UNUSED)
{
