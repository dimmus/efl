...
#define _GET_SET_FUNC(name)                                                   \
    static int _##name##_get(Eo *obj EFL_UNUSED, void *class_data)            \
    {                                                                         \
        const Private_Data *pd = class_data;                                  \
        printf("%s %d\n", __func__, pd->name);                                \
        return pd->name;                                                      \
    }                                                                         \
    static void _##name##_set(Eo *obj EFL_UNUSED, void *class_data, int name) \
    {                                                                         \
...
static Eo *
_constructor(Eo *obj, void *class_data EFL_UNUSED)
{
    my_init_count++;
...
static Eo *
_finalize(Eo *obj, void *class_data EFL_UNUSED)
{
    Eo           *ret;
    Private_Data *pd = class_data;
...
static void
_class_constructor(Efl_Class *klass EFL_UNUSED)
{
    class_var = malloc(10);
}

static void
_class_destructor(Efl_Class *klass EFL_UNUSED)
{
    free(class_var);
}
...
static const Efl_Class_Description class_desc = {
    EO_VERSION,           "Simple",           EFL_CLASS_TYPE_REGULAR,
    sizeof(Private_Data), _class_initializer, _class_constructor,
    _class_destructor
};

EFL_DEFINE_CLASS(simple_class_get, &class_desc, EO_CLASS, MIXIN_CLASS, NULL);
