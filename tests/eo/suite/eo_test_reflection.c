...
EFL_START_TEST(eo_test_reflection_simple)
{
    ...
    ck_assert_int_eq(efl_property_reflection_exist(simple, "simple_a"),
                     EFL_TRUE);

    ck_assert_int_eq(
        efl_property_reflection_set(simple, "should_fail", useless_val),
        EINA_ERROR_NOT_IMPLEMENTED);

    ck_assert_int_eq(efl_property_reflection_exist(simple, "should_fail"),
                     EFL_FALSE);
