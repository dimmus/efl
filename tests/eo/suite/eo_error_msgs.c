...
void
eo_test_print_cb(const Eina_Log_Domain *d,
                 ...
                 va_list args           EFL_UNUSED)
{
  ...
  if (myctx->msg) ck_assert_str_eq(myctx->msg, fmt);
  ck_assert_str_eq(myctx->fnc, fnc);
  myctx->did = EFL_TRUE;
}

void
eo_test_safety_print_cb(const Eina_Log_Domain *d,
                        Eina_Log_Level         level,
                        const char            *file,
                        const char            *fnc,
                        int                    line,
