...
EFL_START_TEST(efl_object_override_tests)
{
  ...
  EFL_OPS_DEFINE(overrides4, EFL_OBJECT_OP_FUNC(simple_a_get, NULL));
  fail_if(!efl_object_override(obj, NULL));
...
static void
_eo_signals_a_changed_cb2(void *_data            EFL_UNUSED,
...
static void
_eo_signals_a_changed_never(void *_data            EFL_UNUSED,
                            const Efl_Event *event EFL_UNUSED)
{
...
static void
_eo_signals_efl_del_cb(void *_data            EFL_UNUSED,
                       const Efl_Event *event EFL_UNUSED)
{
...
_eo_signals_cb_added_deled(void *data EFL_UNUSED, const Efl_Event *event)
{
  ...
  else fail_if(callback_array->func != _eo_signals_cb_added_deled);
}
...
EFL_START_TEST(eo_signals)
{
  ...
  efl_event_callback_add(obj,
                         ...
                         &_eo_signals_callbacks);
  r = efl_event_callback_add(obj,
                             EFL_EVENT_CALLBACK_DEL,
                             _eo_signals_cb_added_deled,
                             &_eo_signals_callbacks);
  fail_if(!r);
  check_is_deled = 1;
  efl_event_callback_array_priority_add(obj,
                                        _eo_signals_callbacks(),
                                        -100,
                                        (void *)1);
  efl_event_callback_array_add(obj, _eo_signals_callbacks(), (void *)3);
  r = efl_event_callback_array_priority_add(obj,
                                            _eo_signals_callbacks(),
                                            -50,
                                            (void *)2);
  fail_if(!r);
  ...
  {
    const Efl_Event_Description *a_desc =
      efl_object_legacy_only_event_description_get("a,changed");
    fail_if(!a_desc);
    ...
    const Efl_Event_Description *a_desc2 =
      efl_object_legacy_only_event_description_get("a,changed");
    fail_if(!a_desc2);
    fail_if(a_desc2 != a_desc);

    const Efl_Event_Description *bad_desc =
      efl_object_legacy_only_event_description_get("bad");
    fail_if(!bad_desc);
    ...
    efl_event_callback_priority_add(obj,
                                    EV_A_CHANGED2,
                                    -1000,
                                    _eo_signals_a_changed_never,
                                    (void *)1);
    efl_event_callback_priority_add(obj,
                                    EV_A_CHANGED2,
                                    0,
                                    _eo_signals_a_changed_within_cb,
                                    NULL);
    efl_event_callback_priority_add(obj,
                                    EV_A_CHANGED,
                                    -100,
                                    _eo_signals_a_changed_cb,
                                    (void *)1);
...
EFL_START_TEST(efl_data_fetch)
{
  ...
  static Efl_Class_Description class_desc = {
    EO_VERSION, "Simple2", EFL_CLASS_TYPE_REGULAR, 10, NULL, NULL, NULL
  };
...
EFL_START_TEST(efl_isa_tests)
{
...

  {
        /* Usually should be const, not const only for the test... */
    static Efl_Class_Description class_desc = {
      EO_VERSION, "Iface", EFL_CLASS_TYPE_INTERFACE, 0, NULL, NULL, NULL
    };
...

  {
        /* Usually should be const, not const only for the test... */
    static Efl_Class_Description class_desc = {
      EO_VERSION, "Mixin", EFL_CLASS_TYPE_MIXIN, 0, NULL, NULL, NULL
    };
...
{
  if (_man_should_con) efl_manual_free_set(obj, EFL_TRUE);
...
_man_des(Eo *obj, void *data EFL_UNUSED, va_list *list EFL_UNUSED)
{
  ...
  if (_man_should_des) efl_manual_free_set(obj, EFL_FALSE);
}
...
EFL_START_TEST(eo_man_free)
{
    /* Usually should be const, not const only for the test... */
  static Efl_Class_Description class_desc = {
    EO_VERSION, "Simple2", EFL_CLASS_TYPE_REGULAR, 10, _class_initializer,
    NULL,       NULL
  };
...
{
  EFL_OPS_DEFINE(ops, EFL_OBJECT_OP_FUNC(resolve_a_print, _a_print), );
...
EFL_START_TEST(efl_func_resolve)
{
    /* Usually should be const, not const only for the test... */
  static Efl_Class_Description class_desc = {
    EO_VERSION, "Inherit", EFL_CLASS_TYPE_REGULAR, 0, _multi_class_initializer,
    NULL,       NULL
  };
...
EFL_START_TEST(efl_add_do_and_custom)
{
  ...
  obj = efl_add_ref(SIMPLE_CLASS,
                    NULL,
                    efl_constructor(efl_added),
                    simple_a_set(efl_added, 7));
  fail_if(!obj);
  pd = efl_data_scope_get(obj, SIMPLE_CLASS);
  fail_if(pd->a != 7);
  efl_unref(obj);
  ...
  Efl_Bool finalized;
  obj =
    efl_add_ref(SIMPLE_CLASS, NULL, finalized = efl_finalized_get(efl_added));
  fail_if(finalized);
...
EFL_START_TEST(eo_pointers_indirection)
{
  static const Efl_Class_Description class_desc = {
    EO_VERSION, "Simple", EFL_CLASS_TYPE_REGULAR, 0, NULL, NULL, NULL
  };
...
#define NB_OBJS 10000
  ...
  for (obj_id = 0; obj_id < NB_OBJS; obj_id++)
  {
    ...
    if (!objs[obj_id]) fail_if(!objs[obj_id]);
    if (!efl_isa(objs[obj_id], klass)) fail_if(!efl_isa(objs[obj_id], klass));
  }
...
thr1(void *data, Eina_Thread t EFL_UNUSED)
{
  ...
  fail_if(efl_domain_switch(EFL_ID_DOMAIN_THREAD) != EFL_TRUE);
  ...
  fail_if(efl_domain_data_return(dom) != EFL_TRUE);
...
EFL_START_TEST(eo_domain)
{
  ...
  fail_if(efl_domain_switch(EFL_ID_DOMAIN_MAIN) != EFL_TRUE);

  fail_if(efl_domain_get() != EFL_ID_DOMAIN_MAIN);

  fail_if(efl_domain_current_get() != EFL_ID_DOMAIN_MAIN);

  fail_if(efl_domain_current_set(EFL_ID_DOMAIN_SHARED) != EFL_TRUE);

  fail_if(efl_domain_current_get() != EFL_ID_DOMAIN_SHARED);

  fail_if(efl_domain_current_set(EFL_ID_DOMAIN_MAIN) != EFL_TRUE);

  fail_if(efl_domain_current_get() != EFL_ID_DOMAIN_MAIN);

  fail_if(efl_domain_current_push(EFL_ID_DOMAIN_SHARED) != EFL_TRUE);

  fail_if(efl_domain_current_get() != EFL_ID_DOMAIN_SHARED);

  fail_if(efl_domain_current_push(EFL_ID_DOMAIN_THREAD) != EFL_TRUE);
  ...
  fail_if(efl_compatible(objs, obj) == EFL_TRUE);
...
static void
_destruct_test_del_cb(void *data, const Efl_Event *ev EFL_UNUSED)
{
  int *var = data;
  *var     = 1;
}
