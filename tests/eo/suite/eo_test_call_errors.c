...
EFL_START_TEST(eo_pure_virtual_fct_call)
{
    ...
    TEST_EO_ERROR(
        ...
        "in %s:%d: you called a pure virtual func '%s' (%d) of class '%s'.");
...
EFL_START_TEST(eo_api_not_implemented_call)
{
    ...
    TEST_EO_ERROR("simple_no_implementation",
...
EFL_START_TEST(eo_op_not_found_in_super)
{
    ...
    TEST_EO_ERROR("_efl_object_call_resolve",
                  "in %s:%d: func '%s' (%d) could not be resolved on %s for "
                  "class '%s' for super of '%s'.");
...
{
    EFL_OPS_DEFINE(ops, EFL_OBJECT_OP_FUNC(simple_error_test, _test), );

    return efl_class_functions_set(klass, &ops, NULL);
}

static const Efl_Class_Description errorcase_class_desc = {
    EO_VERSION, "Simple errorcase",           EFL_CLASS_TYPE_REGULAR,
    0,          _errorcase_class_initializer, NULL,
    NULL
};

EFL_DEFINE_CLASS(simple_errorcase_class_get,
                 &errorcase_class_desc,
                 EO_CLASS,
                 NULL)
