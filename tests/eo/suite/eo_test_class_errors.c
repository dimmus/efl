...
EFL_START_TEST(eo_inherit_errors)
{
  ...
  static Efl_Class_Description class_desc = {
    EO_VERSION, "General", EFL_CLASS_TYPE_MIXIN, 0, NULL, NULL, NULL
  };
  ...
  TEST_EO_ERROR("efl_class_new",
                "Non-regular classes ('%s') aren't allowed to inherit from "
                "regular classes ('%s').");
  ...
  TEST_EO_ERROR("efl_class_new",
                "Regular classes ('%s') aren't allowed to inherit from "
...
EFL_START_TEST(eo_inconsistent_mro)
{
  ...
  static const Efl_Class_Description class_desc_mixin2 = {
    EO_VERSION, "Mixin2", EFL_CLASS_TYPE_MIXIN, 0, NULL, NULL, NULL
  };

  static const Efl_Class_Description class_desc_mixin3 = {
    EO_VERSION, "Mixin3", EFL_CLASS_TYPE_MIXIN, 0, NULL, NULL, NULL
  };
  ...
  TEST_EO_ERROR("_eo_class_mro_init",
                "Cannot create a consistent method resolution order for "
                "class '%s' because of '%s'.");
  klass = efl_class_new(&class_desc_simple,
                        EO_CLASS,
                        klass_mixin,
                        klass_mixin2,
                        NULL);
...
static void
_stub_class_constructor(Efl_Class *klass EFL_UNUSED)
{}
...
EFL_START_TEST(eo_bad_interface)
{
  ...
  static Efl_Class_Description class_desc = {
    EO_VERSION, "Interface", EFL_CLASS_TYPE_INTERFACE, 10, NULL, NULL, NULL
  };

  TEST_EO_ERROR("efl_class_new",
                "safety check failed: !desc->data_size is false");
...
static void
_null_fct(Eo *eo_obj EFL_UNUSED, void *d EFL_UNUSED)
{}
...
{
  EFL_OPS_DEFINE(ops, EFL_OBJECT_OP_FUNC(NULL, _null_fct), );
...
EFL_START_TEST(eo_null_api)
{
  ...
  static Efl_Class_Description class_desc = {
    EO_VERSION, "Simple", EFL_CLASS_TYPE_REGULAR, 0, _null_class_initializer,
    NULL,       NULL
  };

  TEST_EO_ERROR("_eo_class_funcs_set",
                "Class '%s': NULL API not allowed (NULL->%p '%s').");
  klass = efl_class_new(&class_desc, NULL, NULL);
  fail_if(klass);
  fail_unless(ctx.did);
...
EFL_START_TEST(eo_api_redefined)
{
  ...
  TEST_EO_ERROR("_eo_class_funcs_set",
                "Class '%s': API previously defined (%p->%p '%s').");
  klass = efl_class_new(&class_desc, NULL, NULL);
  fail_if(klass);
  fail_unless(ctx.did);
